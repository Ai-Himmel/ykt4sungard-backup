...
public class WorkInfoServiceImpl extends BaseService implements WorkInfoService {
	...
	**/
	...
	/**
	 * 20100225海洋大学,新增人事考勤信息,显示某个时间段考勤总和
	 * gejinfeng 
	 */
	 public List makeCheckReportFinance(String begDate ,String endDate ,String deptId)throws Exception{
		   Iterator checkIter = sysService.getCheckClerkInfo(deptId,"1,2,3,4").iterator();
			List checkInfoList = new ArrayList(); 
			...
			while (checkIter.hasNext()){
				...
				checkInfoMap.put("custType",custType);
				checkInfoMap.put("custId",custId);
				Iterator dayIter = sysService.getAllDate(begDate, endDate).iterator();
			
				checkInfoMap = getCheckReportMapFinance(checkInfoMap,custId,dayIter);
				checkInfoList.add(checkInfoMap);
			}
			return checkInfoList;
	...
	/**
	 * 生成考勤报表----通用版,没有明细显示,没有半勤概念,只有合计考勤天数,20100225 海洋人事用报表
	 * @param checkInfoMap
	 * @param custId
	 * @param statDate
	 * @param dayMap
	 ...
	 * @return
	 */
	private Map getCheckReportMapFinance(
			Map checkInfoMap,
			String custId,
			Iterator dayIter){
		...
		while (dayIter.hasNext()){//如果没有参数设置这项，生成考勤信息也是不需要的，直接从考勤流水表里查，就可以了
			...
			//如果是专职教师
				...
				if (getHYCheckNum(custId,dayMap.get("date").toString(),teaBeginTime,teaEndTime)>0) {
