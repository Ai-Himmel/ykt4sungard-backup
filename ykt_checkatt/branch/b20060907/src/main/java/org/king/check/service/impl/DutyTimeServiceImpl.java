		...
		if (StringUtils.isNotEmpty((String) filter.get(CommonConfig.deptId))) {
			...
			if (!departList.isEmpty()) {
				...
				while (departIte.hasNext()) {
					...
					String hql = "from Clerkinfo c where c.deptId='"
							+ department.getId() + "'";
					...
					if (!clerkList.isEmpty()) {
						...
						while (clerkIte.hasNext()) {
							...
							Clerkinfo clerk = (Clerkinfo) clerkIte.next();
							String clerkName = "";
							...
							while (DateUtilExtend.diffDate(endDate, checkDate) <= 0) {
								...
								seriSql.append("from Attserial seri where seri.cardId="
								...
								if (resList.isEmpty()) {
									...
									while (dutyIter.hasNext()) {
										Attserial attS = (Attserial) dutyIter.next();
