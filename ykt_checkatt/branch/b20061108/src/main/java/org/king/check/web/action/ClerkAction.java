...
public class ClerkAction extends BaseAction {
	 ...
	 * @throws Exception
	 */
	/**hanjiwei delete it 20060914这步和查询页合并
	public ActionForward load4setupClerk(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	**/
	...
	public ActionForward setupSeatClerk(ActionMapping mapping, ActionForm form,
		...
		String[] ids = (String[]) request.getSession().getAttribute(
				Constants.SELECTED_PERSON);
		...
		if (ids != null) {
			for (int i = 0; i < idList.size(); i++) {
				...
				clerk.setCustId(Integer.valueOf((String) idList.get(i)));
				clerk.setIfoffice(isSeat);
				//clerk.setDeptId(entity);
				//clerk.setWorktimeconfid(workTimeConfId);
				clerkList.add(clerk);

				Workconfmap workconfmap = workConfMapService
						.getWorkconfmapByClerkId(Integer
								.valueOf((String) idList.get(i)));

				if (workconfmap == null) {
					workconfmap = new Workconfmap();
					workconfmap.setClerkid(Integer.valueOf((String) idList
							.get(i)));
