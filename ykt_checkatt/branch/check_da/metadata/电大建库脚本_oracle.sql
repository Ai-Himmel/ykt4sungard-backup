...
alter table YKT_CK.T_CHECKTYPE
  ...
  */
create table YKT_CK.T_CHECKRECORD
(
  SERIALNO  INTEGER not null,
  DEVICEID  INTEGER not null,
  CUSTID    INTEGER,
  CARDNO    INTEGER,
  CARDPHYID VARCHAR2(30),
  PUNCHDATE VARCHAR2(8) not null,
...
-- Add comments to the columns 
comment on column YKT_CK.T_CHECKRECORD.SERIALNO
  is '流水号';
comment on column YKT_CK.T_CHECKRECORD.DEVICEID
  is '设备号';
comment on column YKT_CK.T_CHECKRECORD.CUSTID
  is '客户号';
comment on column YKT_CK.T_CHECKRECORD.CARDNO
  is '卡号';
comment on column YKT_CK.T_CHECKRECORD.CARDPHYID
  is '物理卡号';
comment on column YKT_CK.T_CHECKRECORD.PUNCHDATE
  is '打卡日期';
comment on column YKT_CK.T_CHECKRECORD.PUNCHTIME
  is '打卡时间';
comment on column YKT_CK.T_CHECKRECORD.COLDATE
  is '上传日期';
comment on column YKT_CK.T_CHECKRECORD.STATUS
  is '状态';
-- Create/Recreate primary, unique and foreign key constraints 
alter table YKT_CK.T_CHECKRECORD
  add constraint PK_T_CHECKRECORD primary key (SERIALNO, DEVICEID, PUNCHDATE)
  using index 
  tablespace TS_YKT_CK
  pctfree 10
  initrans 2
...
grant select, insert, update, delete on YKT_CK.T_COURSEPLAN to YKT_CUR;
-- Create table
...
create or replace view T_CHECKRECORD as
  select DEVSEQNO as SERIALNO,
    ...
    where   transmark in ( 3,  4 , 5, 6, 7, 11, 52, 1025, 1026,1027)
    commit;
