...
public class TernarySearchTree<E> {
    ...
    public List<TSTNode<E>> getNodes(String key) {
        List<TSTNode<E>> result = new ArrayList<TSTNode<E>>();
        getNodes(result, key, rootNode);
        return result;
    }

    protected void getNodes(List<TSTNode<E>> list, String key, TSTNode<E> startNode) {
        ...
        getNodes(list, key, startNode.relatives[TSTNode.LOKID]);
        if (!list.contains(startNode)) {
            getNodes(list, key, startNode.relatives[TSTNode.EQKID]);
        }
        getNodes(list, key, startNode.relatives[TSTNode.HIKID]);
    }
...
//        TSTNode<E> startNode = getNode(prefix);
        List<TSTNode<E>> result = getNodes(prefix);
        for (TSTNode<E> startNode : result) {
