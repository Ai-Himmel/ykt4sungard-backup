...
public class SysServiceImpl implements SysService {
	...
	*/
	...
	sql.append(" select  SHORT_NAME  from    ykt_ck.department where dept_Id='"+deptId+"'");
	List deptList = jdbcTemplate.queryForList(sql.toString());
	if(deptList != null && deptList.size()>0){
	  Map 	deptMap =	(Map)deptList.get(0);
	  deptname  =     deptMap.get("SHORT_NAME").toString();
	}
	return deptname;
   }
	...
	public String getDeptName(String deptId){
		...
		StringBuffer sql = new StringBuffer(1024);
		sql.append("select DEPT_ID,SHORT_NAME from YKT_CK.DEPARTMENT ")
		.append("where DEPT_ID='").append(deptId).append("'");
		List deptList=jdbcTemplate.queryForList(sql.toString());
		if (deptList.size()!=0){
			Map deptMap = (Map)deptList.get(0);
			return deptMap.get("SHORT_NAME").toString();
		}else{
			return "";
		}
	}
	...
	public List getCustInfo(Map filter) throws Exception {
		StringBuffer sql = new StringBuffer(1024);
		sql.append("select cust.CUT_ID, cust.CUT_NAME,  cust.STUEMP_NO,dept.DEPT_ID,dept.SHORT_NAME as DEPT_NAME")
		.append(" from YKT_CUR.T_CIF_CUSTOMER cust")
