...
public class ClerkAction extends BaseAction {
	...
	public ActionForward queryClerkInfo(ActionMapping mapping, ActionForm form,
		...
		filterMap.put("department",deptStr.toString());
	 ...
	 * @throws Exception
	 */
	public ActionForward queryClerkCheckDetail(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		Date d = new Date();
		String  custId = request.getParameter("custId");
		String beginDate = DateUtilExtend.getNowMonthFirstDate(d);
		String endDate =DateUtilExtend.getNowMonthLastDate(d);
		List checkDetail = clerkService.queryClerkCheckDetail(Integer.valueOf(custId),beginDate,endDate);
	...
	public ActionForward load4queryAvailableClerk(ActionMapping mapping,
		...
		request.setAttribute("cutTypeList", personTypeList);
	...
	public ActionForward setupClerk(ActionMapping mapping, ActionForm form,
		...
		String aheadOverTime = "0";//request.getParameter("aheadOverTime");
		String delayOverTime = "0";//request.getParameter("delayOverTime");
		String overTime = "0";//request.getParameter("overTimeUnit");
		String twoReset = "0";//request.getParameter("twoReset");
		String holiday = "0";//request.getParameter("holiday");
		...
		if (custIds != null) {
			for (int i = 0; i < idList.size(); i++) {
				...
				Map custInfo = sysService.getCustInfoByCustId(Integer.parseInt(idList.get(i).toString()));
	...
	public ActionForward deleteClerk(ActionMapping mapping, ActionForm form,
		...
		HttpSession session = request.getSession();
		String  accountId1 = (String)session.getAttribute("account");
		
		List deptList = sysService.getDeptList(accountId1);
		//List deptList = sysService.getDeptList();
		request.setAttribute("departments", deptList);
		return mapping.findForward("success");
	}
