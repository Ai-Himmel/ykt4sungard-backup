...
public class WorkTimeConfAction extends BaseAction{
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		List usedWork = workTimeConfService.getUsedWorkTime(tempIds.toString());
		if (usedWork.size()>0){
			request.setAttribute("msg","要删除的工作时间选项中有已经被引用的项，不能删除！");
		}else{
			for(int i=0;i<workIds.length;i++)
			{
				workTimeConfService.deleteWorkTimeConf(workIds[i]);
			}
			request.setAttribute("msg","删除工作时间成功!");
		}		
		searchWorkTimeList(request);
		return mapping.findForward("searchRes");
	}
