...
public class SysServiceImpl implements SysService {
	...
	public List getCheckTypeInfo() throws Exception {
     ...
     {
    	 String sql="select dict.DICTVAL  value , dict.DICTCAPTION caption from " +
    	 		"YKT_CUR.T_DICTIONARY dict " +
    	 		"where dict.DICTTYPE=17";
	...
	public List getPersonType()throws Exception
	 {
		String sql="select cut.CUSTTYPE type , cut.CUSTTYPENAME name from " +
 		"YKT_CUR.T_CUSTTYPE cut ";
	...
	{
		String sql="select dept.DEPTCODE dept_code , dept.DEPTNAME dept_name from YKT_CUR.T_DEPT dept" +
				" where dept.FDEPTCODE!='NA' AND dept.FDEPTCODE is not null";
	...
	public List getSpecList()throws Exception{
		String sql="select SPECIALTYCODE AS s_code,SPECIALTYNAME AS s_name from T_SPECIALTY";
	...
	public List search(Map filter)throws Exception
	{
		...
		sql.append("select cust.CUSTNAME cust_name,cust.CUSTID cust_id,")
		.append(" cust.STUEMPNO stuemp_no,ctype.CUSTTYPENAME type_name ")
		.append(" from  YKT_CUR.T_CUSTOMER cust ")
		.append(" left join YKT_CUR.T_DEPT dept on dept.DEPTCODE=cust.DEPTCODE")
		.append(" left join YKT_CUR.T_CUSTTYPE ctype on  ctype.CUSTTYPE=cust.CUSTTYPE")
		.append(" left join YKT_CUR.T_SPECIALTY spec on spec.SPECIALTYCODE=cust.SPECIALTYCODE")
		.append(" where (cust.STUEMPNO is not null or cust.STUEMPNO<>'')")
		.append(" and cust.CUSTID not in (select CUST_ID from YKT_CK.T_CLERKINFO)");
		if (StringUtils.isNotEmpty((String)filter.get("personCode"))){
			sql.append(" and cust.STUEMPNO like '%"+(String)filter.get("personCode")+"%' ");
		}
		if (StringUtils.isNotEmpty((String)filter.get("personName"))){
			sql.append(" and cust.CUSTNAME like '%"+(String)filter.get("personName")+"%' ");
		}
		if((String)filter.get("department")!=null&&!((String)filter.get("department")).equals("all"))
		{
			sql.append(" and dept.DEPTCODE='"+(String)filter.get("department")+"'");
		}
		if((String)filter.get("spec")!=null&&!((String)filter.get("spec")).equals("all"))
		{
			sql.append(" and spec.SPECIALTYCODE='"+(String)filter.get("spec")+"'");
		}
	...
	{
		String sql="select customer.CUSTNAME  name from YKT_CUR.T_CUSTOMER customer where customer.CUSTID=?";
	...
	public Integer getCardNo(Integer custId) throws Exception {
		StringBuffer sql = new StringBuffer(1024);
		Integer cardNo = null;
		sql.append("select CARDNO cardNo from YKT_CUR.T_CARD")
		.append(" where CUSTID=? and STATUS='1'");
	...
	public List getLeaveList() throws Exception {
		...
		if ("ORACLE".equals(Constants.DBNAME)){
			sql.append("select DICTVAL AS DICT_VALUE, DICTCAPTION AS DICT_CAPTION")
			.append(" from YKT_CUR.T_DICTIONARY")
			.append(" where DICTTYPE=1210 order by to_number(DICTVAL)");
		}else if ("DB2".equals(Constants.DBNAME)){
			sql.append("select DICTVAL AS DICT_VALUE, DICTCAPTION AS DICT_CAPTION")
			.append(" from YKT_CUR.T_DICTIONARY")
			.append(" where DICTTYPE=1210 order by int(DICTVAL)");
		}
	...
	public String getLeaveTypeName(String leaveTypeId) throws Exception {
		StringBuffer sql = new StringBuffer(1024);
		sql.append("select DICTCAPTION AS DICT_CAPTION from YKT_CUR.T_DICTIONARY")
		.append(" where DICTTYPE=1210 and DICTVAL=?");
	...
	public int getCustIdByStuempNo(String stuempNo)throws Exception{
		StringBuffer sql = new StringBuffer(1024);
		sql.append("select CUSTID AS CUT_ID from YKT_CUR.T_CUSTOMER")
		.append(" where STUEMPNO=?");
	...
	public Map getCustInfoByCustId(int custId) throws Exception {
		StringBuffer sql = new StringBuffer(1024);
		sql.append("select CUSTNAME AS CUT_NAME,STUEMPNO AS STUEMP_NO from YKT_CUR.T_CUSTOMER")
		.append(" where CUSTID=?");
	...
	public List getCustInfo(Map filter) throws Exception {
		StringBuffer sql = new StringBuffer(1024);
		sql.append("select cust.CUSTID AS CUT_ID, cust.CUSTNAME AS CUT_NAME,  cust.STUEMPNO AS STUEMP_NO,dept.DEPT_ID,dept.DEPT_NAME")
		.append(" from YKT_CUR.T_CUSTOMER cust")
		.append(" left join YKT_CK.T_CLERKINFO clerk on cust.CUSTID=clerk.CUST_ID")
		.append(" left join YKT_CK.DEPARTMENT dept on clerk.DEPT_ID=dept.DEPT_ID")
		.append(" where cust.STUEMPNO=?");
	...
	public List getClerkInfo(String deptId) throws Exception {
		StringBuffer sql = new StringBuffer(1024);
		sql.append("select t.cust_id AS cust_id, cust.STUEMPNO AS stuemp_no,cust.CUSTNAME AS cut_name,dept.dept_name,cap.DICTCAPTION as dict_caption")
		.append(" from ykt_ck.t_clerkinfo t,ykt_cur.t_customer cust,")
		.append(" ykt_ck.department dept,ykt_cur.t_dictionary cap")
		.append(" where t.cust_id=cust.CUSTID and t.dept_id=dept.dept_id and")
		.append(" t.ifoffice=cap.DICTVAL and cap.DICTTYPE=1200");
		if (!"".equals(deptId)){
	...
	public List getCheckClerkInfo(String deptId,String ifoffice) throws Exception {
		StringBuffer sql = new StringBuffer(1024);
		sql.append("select t.cust_id AS cust_id, cust.STUEMPNO AS stuemp_no,cust.CUSTNAME AS cut_name,dept.dept_name,cap.DICTVAL AS DICT_VALUE,cap.DICTCAPTION AS dict_caption")
		.append(" from ykt_ck.t_clerkinfo t,ykt_cur.t_customer cust,")
		.append(" ykt_ck.department dept,ykt_cur.t_dictionary cap")
		.append(" where t.cust_id=cust.CUSTID and t.dept_id=dept.dept_id and")
		.append(" t.ifoffice=cap.DICTVAL and cap.DICTTYPE=1200");
