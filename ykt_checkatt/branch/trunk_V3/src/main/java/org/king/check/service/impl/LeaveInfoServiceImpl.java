...
public class LeaveInfoServiceImpl implements LeaveInfoService {
	...
	public String checkIsLeaveByClerkId(Integer clerkId,String inStart,String inEnd) throws Exception {
		...
		sql.append("select cust.CUSTNAME AS CUT_NAME,lea.CUST_ID, lea.STARTDATE,lea.ENDDATE, lea.STARTTIME, lea.ENDTIME ")
		.append(" from YKT_CK.T_LEAVEINFO lea left join YKT_CUR.T_CUSTOMER cust")
		.append(" on cust.CUSTID=lea.CUST_ID")
	...
	public List searchLeaveInfo(Map filter) throws Exception {
		...
		.append("lea.OPER_ID, lea.OPER_DATE, lea.OPER_TIME,cust.CUSTNAME AS CUT_NAME,dept.DEPT_NAME,cust.STUEMPNO AS STUEMP_NO,")
		.append("(select DICTCAPTION from YKT_CUR.T_DICTIONARY where DICTTYPE=1210 and DICTVAL=lea.LEAVE_TYPEID) leavetype")
		.append(" from YKT_CK.T_LEAVEINFO lea ")
		.append(" left join YKT_CUR.T_CUSTOMER cust on cust.CUSTID=lea.CUST_ID")
		...
		if (StringUtils.isNotEmpty((String) filter.get("trueName"))){
			sql.append(" and cust.CUSTNAME like '%"+(String) filter.get("trueName")+"%'");
		}
	...
	public List searchClerkForRegister(Map filter) throws Exception {
		...
		.append(" left join YKT_CUR.T_CUSTOMER cust on clerk.CUST_ID=cust.CUT_ID")
		.append(" left join YKT_CK.DEPARTMENT dept on dept.DEPT_ID=clerk.DEPT_ID")
	...
	public List getLeaveInfo(String[] custIds) throws Exception {
		...
		sql.append("select (select DICTCAPTION from YKT_CUR.T_DICTIONARY where DICTTYPE=1210 and DICTVAL=lea.LEAVE_TYPEID) leavetype,")
		.append(" lea.OPER_DATE, lea.OPER_TIME,")
		.append(" lea.CUST_ID, lea.STARTDATE, lea.ENDDATE, lea.STARTTIME, lea.ENDTIME,cust.CUSTNAME AS CUT_NAME,cust.STUEMPNO AS STUEMP_NO ")
		.append(" from YKT_CK.T_LEAVEINFO lea left join YKT_CUR.T_CUSTOMER cust on cust.CUSTID=lea.cust_id")
	...
	public List getLeaveQuery(Map filter) throws Exception {
		...
		while (iter.hasNext()) {
			...
			String repCustId = leaveMap.get("REPCUSTID").toString();
			if ("0".equals(repCustId)){
				leaveInfoList.put("repCustId","否");
			}else{
				leaveInfoList.put("repCustId","是");
			}
			leaveList.add(leaveInfoList);
		}
		return leaveList;
	...
	public void sendMailToManager(String deptId,String custId)throws Exception{
		...
		while(iter.hasNext()){
			...
			MailSender.mailsSender(mail,leaveInfo.toString(),"");
		}
	}
	...
	public void reportLeaveCheckCancel(String reportLeaveIds, Map filter) throws Exception {
		//删除请假审批信息
		//把是否审批修改为没有审批
		//if ("1".equals(filter.get("ifagree"))){
	...
	private String formatDate(String date,String time){
		...
		if ("".equals(time)){
			...
			fTime = time.substring(0,2)+":"+time.substring(3,5);
		}
		return fDate+" "+fTime;
	}
