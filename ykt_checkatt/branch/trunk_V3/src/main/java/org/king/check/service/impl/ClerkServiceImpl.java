...
public class ClerkServiceImpl implements ClerkService {
	...
	public List queryClerk(Map filterMap) {
		...
		sql.append("SELECT B.CUSTID CUT_ID,B.STUEMPNO STUEMPID,B.CUSTNAME STUEMPNAME,typeInfo.type_name,")
		.append(" C.DEPT_NAME,D.DICTCAPTION IFOFFICE FROM YKT_CK.T_CLERKINFO A ")
		...
		.append(" LEFT JOIN (SELECT DICT.DICTVAL,DICT.DICTCAPTION FROM ")
		.append(" YKT_CUR.T_DICTIONARY DICT WHERE DICT.DICTTYPE=1200) D")
		.append(" ON A.IFOFFICE=D.DICTVAL LEFT  JOIN YKT_CK.DEPARTMENT C")
		...
		if(filterMap!=null&&!filterMap.isEmpty()){
			...
			if (StringUtils.isNotEmpty(personCode))
				sql.append(" AND B.STUEMPNO LIKE "+"'%"+personCode+"%'");
			String personName = (String)filterMap.get("personName");
			if (StringUtils.isNotEmpty(personName))
				sql.append(" AND B.CUSTNAME LIKE "+"'%"+personName+"%'");			
	...
	public List queryAvailableClerk(Map filterMap) {
		...
		"SELECT * FROM (SELECT B.CARD,B.PERSON_CODE PERSON_CODE,B.PERSON_NAME PERSON_NAME,B.PERSON_TYPE,E.DEPT_NAME DEPT_NAME,D.DICTCAPTION IS_SEAT,1 is_clerk,'是' is_clerk_title "+
		"FROM  (SELECT A.CLERKID AS PERSON_CODE,C.PERSON_NAME,C.DEPT,A.ISSEAT,C.PERSON_TYPE,C.CARD FROM YKT_CK.CLERK A LEFT JOIN YKT_CK.USR_PERSON C ON A.CLERKID=C.PERSON_CODE ) B"+
	      " LEFT JOIN (SELECT DICT.DICT_VALUE,DICT.DICTCAPTION FROM YKT_CK.COM_DICTIONARY DICT WHERE DICT.DICT_NO=1002) D ON B.ISSEAT=D.DICT_VALUE"+
	...
	public List getCheckReportStat(String stuempNo,String beginDate,String endDate,String deptId) throws Exception {
		StringBuffer sql = new StringBuffer(2048);
		sql.append("select att.cust_id,cust.CUSTNAME as cut_name,cust.STUEMPNO as stuemp_no,dept.dept_name,")
		...
		.append(" from ykt_ck.t_clerkinfo info left join ykt_cur.t_customer cust")
		.append(" on info.cust_id = cust.CUSTID left join ykt_ck.t_checkatt_report att")
		...
		if (!"".equals(stuempNo)){
			sql.append(" and cust.STUEMPNO='").append(stuempNo).append("'");
		}
		sql.append(" group by att.cust_id,cust.CUSTNAME,cust.STUEMPNO,dept.dept_name")
	...
	public Map getClerkDetailByClerkId(String  clerkId)throws Exception{
		...
		strSql.append("SELECT B.CUSTID  CUT_ID,B.STUEMPNO STUEMPID,B.CUSTNAME STUEMPNAME,")
        .append(" A.DELAY_OVERTIME, A.OVERTIME_VALUE, A.TWOREST_OVERTIME, A.HOLIDAY_OVERTIME,")
		.append(" A.AHEAD_OVERTIME,C.DEPT_NAME DEPTNAME,C.DEPT_ID DEPTID,D.DICTCAPTION IFOFFICE ")
		.append(" FROM YKT_CK.T_CLERKINFO A LEFT JOIN YKT_CUR.T_CUSTOMER B ON A.CUST_ID=B.CUSTID")
		.append(" LEFT JOIN (SELECT DICT.DICT_VALUE,DICT.DICTCAPTION FROM ")
		.append(" YKT_CUR.T_DICTIONARY DICT WHERE DICT.DICTTYPE=1200) D")
		.append(" ON A.IFOFFICE=D.DICTVAL LEFT  JOIN YKT_CK.DEPARTMENT C")
		.append(" ON A.DEPT_ID=C.DEPT_ID WHERE B.CUSTID="+clerkId);
	...
	public List getCheckDetail(String stuempNo,String beginDate,String endDate,String deptId) throws Exception{
		...
		sql.append("select att.cust_id,att.check_date,cust.CUSTNAME AS cut_name,cust.STUEMPNO AS stuemp_no,dept.dept_name,")
	...
	public List getCheckSerial(String stuempNo,String beginDate,String endDate,String deptId) throws Exception{
		StringBuffer sql = new StringBuffer(1024);
		sql.append("select info.cust_id,info.check_date,cust.CUSTNAME,cust.STUEMPNO,dept.dept_name,")
		...
		.append(" on clerk.cust_id = info.cust_id left join ykt_cur.t_customer cust")
		.append(" on clerk.cust_id = cust.CUSTID left join ykt_ck.department dept")
		...
		sql.append(" order by cust.STUEMPNO,info.check_date");
		List clerkDetailList = jdbcTemplate.queryForList(sql.toString());
		if (clerkDetailList.size()==0){
			return null;
		}else{
			...
			while (serialIter.hasNext()){
				...
				checkSerialMap.put("check_date",seriMap.get("check_date"));
				checkSerialMap.put("week",DateUtilExtend.getWeekByDate(seriMap.get("check_date").toString()));
				checkSerialMap.put("cut_name",seriMap.get("CUSTNAME"));
				checkSerialMap.put("stuemp_no",seriMap.get("STUEMPNO"));
				checkSerialMap.put("dept_name",seriMap.get("dept_name"));
