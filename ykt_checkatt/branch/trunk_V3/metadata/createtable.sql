...
alter table YKT_CK.T_ATTSERIAL
  add constraint P_KEY_CK primary key (SERIAL_ID, DEVICE_ID, CARD_ID, ATT_DATE)
  using index 
  tablespace TS_YKT_CUR
  pctfree 10
  initrans 2
...
--常州刘国钧考勤用表
create table YKT_CK.T_CHECK_DEVICE_LOCATION
(
  DEVICE_ID   INTEGER not null,
...
comment on table YKT_CK.T_CHECK_DEVICE_LOCATION
  is '考勤设备区域关联表';
comment on column YKT_CK.T_CHECK_DEVICE_LOCATION.DEVICE_ID
  is '设备ID';
comment on column YKT_CK.T_CHECK_DEVICE_LOCATION.LOCATION_ID
  is '考勤区域';
comment on column YKT_CK.T_CHECK_DEVICE_LOCATION.CHECK_TYPE
  is '考勤事件：1为非记时考勤进入，2为非记时考勤离开，3为记时考勤';
alter table YKT_CK.T_CHECK_DEVICE_LOCATION
  add constraint PK_CHECK_DEVICE_LOCATION primary key (DEVICE_ID, LOCATION_ID)
  using index 
  tablespace TS_YKT_CUR
  pctfree 10
  initrans 2
...
create table YKT_CK.T_CHECK_INFO
(
  ID             VARCHAR2(32) not null,
  CUST_ID        INTEGER not null,
  CHECK_DATE     VARCHAR2(8) not null,
  CHECKIN_TIME   VARCHAR2(6),
  CHECKOUT_TIME  VARCHAR2(6),
...
comment on table YKT_CK.T_CHECK_INFO
  is '考勤人员进出情况表（常州）';
comment on column YKT_CK.T_CHECK_INFO.ID
  is '主键';
comment on column YKT_CK.T_CHECK_INFO.CUST_ID
  is '客户编号';
comment on column YKT_CK.T_CHECK_INFO.CHECK_DATE
  is '考勤日期';
comment on column YKT_CK.T_CHECK_INFO.CHECKIN_TIME
  is '刷卡进入时间';
comment on column YKT_CK.T_CHECK_INFO.CHECKOUT_TIME
  is '刷卡离开时间';
comment on column YKT_CK.T_CHECK_INFO.CHECK_DURATION
  is '从进入到离开的持续时间（分钟）';
comment on column YKT_CK.T_CHECK_INFO.LOCATION_ID
  is '考勤区域';
comment on column YKT_CK.T_CHECK_INFO.CHECK_NUM
  is '考勤次数';
comment on column YKT_CK.T_CHECK_INFO.DELAY_NUM
  is '迟到次数';
comment on column YKT_CK.T_CHECK_INFO.EARLY_NUM
  is '早退次数';
comment on column YKT_CK.T_CHECK_INFO.CHECK_TYPE
  is '考勤事件：1为非记时考勤进入，2为非记时考勤离开，3为记时考勤';
alter table YKT_CK.T_CHECK_INFO
  add constraint PK_CHECK_INFO primary key (ID)
  using index 
  tablespace TS_YKT_CUR
  pctfree 10
  initrans 2
...
create index YKT_CK.IDX_CHECK_INFO_DATE on YKT_CK.T_CHECK_INFO (CHECK_DATE)
  tablespace TS_YKT_CUR
  pctfree 10
  initrans 2
  maxtrans 255
...
create table YKT_CK.T_CHECK_LOCATION
(
  ID   VARCHAR2(32) not null,
  NAME VARCHAR2(100)
)
...
comment on table YKT_CK.T_CHECK_LOCATION
  is '考勤区域';
alter table YKT_CK.T_CHECK_LOCATION
  add constraint PK_CHECK_LOCATION primary key (ID)
  using index 
  tablespace TS_YKT_CUR
  pctfree 10
  initrans 2
