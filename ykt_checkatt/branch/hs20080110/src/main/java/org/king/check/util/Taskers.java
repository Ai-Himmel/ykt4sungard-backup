...
public class Taskers extends TimerTask {
    ...
    //public static long PEIROD = 50 * 1000;
	...
	public void runSumKQ() {
		...
		aWorkInfoService = (WorkInfoService) ServiceLocator.getInstance()
				.getService("workInfoService");

		Calendar cal = Calendar.getInstance();
		String sumMonth = formatter_yyyymm.format(cal.getTime());
        log.debug("sumMonth is " + sumMonth);

		cal = Calendar.getInstance();
        cal.add(Calendar.MONTH, -1);
		String sumMonth_Prior1 = formatter_yyyymm.format(cal.getTime());
        log.debug("sumMonth_Prior1 is " + sumMonth_Prior1);

		cal = Calendar.getInstance();
        cal.add(Calendar.MONTH, -2);
		String sumMonth_Prior2 = formatter_yyyymm.format(cal.getTime());
        log.debug("sumMonth_Prior2 is " + sumMonth_Prior2);

		cal = Calendar.getInstance();
        cal.add(Calendar.MONTH, -3);
		String sumMonth_Prior3 = formatter_yyyymm.format(cal.getTime());
        log.debug("sumMonth_Prior3 is " + sumMonth_Prior3);
		...
		try {
			...
			for (int i = 0; i < deptList.size(); i++) {
                ...
                log.debug("deptId is " + deptMap.get("deptId").toString());
				if (deptList.size() == 1) {
					deptStr.append(dep);
				} else {
					if (i == 0) {
            ...
            log.debug("deptStr is " + deptStr);
			
			//当前月的统计运算
			List kqList = aWorkInfoService.makeCheckReport(sumMonth, deptStr
					.toString());
			...
			//		deleterow++;
				...
				log.debug(deptName + "," + stuempNo + "," + custName + ","
						+ custType);
			...
			//前一个月的统计运算
			List kqList_Prior1 = aWorkInfoService.makeCheckReport(sumMonth_Prior1,
					deptStr.toString());
    ...
    public void testTaskSchedule(){
        System.out.println("test quartz."+ Calendar.getInstance().getTime());
    }
