		...
		<div id="filterDiv" style="text-align: left;">
			<jodd:form bean="request">
				<form id="queryform" method="post" action="clerkManager.do?method=QueryAvailablePerson" name="Search">
					...
					专业:&nbsp;
					<select name="search_spec" style="width: 150">
						<option value="all">
							所有
						</option>
						<c:forEach var="spec" items="${specList}">
							<option value="<c:out value="${spec.s_code}"/>">
								<c:out value="${spec.s_name}" />
							</option>
						</c:forEach>
					</select>
					&nbsp;&nbsp;
					<td class='tableHeader'>
						...
						&nbsp;&nbsp;
						<input name="Submit22" type="button" class="submitButton" value=" 重 置 " onclick="clearCmdValue(document.forms.setupForm,'<c:url value="clerkManager.do?method=load4queryAvailableClerk" />');">
					<p></p>
				</form>
			</jodd:form>
		</div>
				...
				<td align="center" valign="top">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr align="center" valign="top">
							<td width="45%">
								<font color="#FF0000"> 
									<select name="search_scrlist" size="20" style="width:360" multiple ondblclick="transferOption(this.form.search_scrlist, this.form.search_destlist,false)">
							...
							<td width="45%">
								<font color="#FF0000"> 
									<select name="search_destlist" size="20" style="width:360" multiple ondblclick="returnOption(this.form.search_destlist, this.form.search_scrlist,false)">
									</select> 
								</font>
							</td>
						</tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td width="15%" align="left" class="right">
					<select name="isSeat" style="width: 120">
				...
				<td width="40%" align="left" class="right">
					<select name="checkType" style="width: 120">
						<OPTION value="">-</OPTION>
						<c:forEach var="checktype" items="${checkTypeList}">
							<option value="<c:out value="${checktype.typeId}"/>">
								<c:out value="${checktype.typeName}" />
							</option>
						</c:forEach>
					</select>
				</td>
...
<script>
  	...
  	function clearCmdValue(form,action){
  	    form.action = action;
  	    form.submit();
  	}
</script>
