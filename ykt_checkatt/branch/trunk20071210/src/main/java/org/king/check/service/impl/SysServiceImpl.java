...
public class SysServiceImpl implements SysService {
	...
	public List getCheckClerkInfo(String deptId,String ifoffice) throws Exception {
		...
		.append(" ykt_ck.department dept,ykt_cur.t_pif_dictionary cap")
		.append(" where t.cust_id=cust.cut_id and t.dept_id=dept.dept_id and")
		.append(" t.ifoffice=cap.dict_value and cap.dict_no=1200");// and cust.stuemp_no='0181'
		if (!"".equals(ifoffice)){
	...
	public void importReportToExcel(String realPath,String deptId,String statMonth,List reportList,String operId) throws Exception{
	    ...
	    header.setCenter(HSSFHeader.font("Stencil-Normal", "Bold") +
	                    HSSFHeader.fontSize((short) 12) + "考勤签到记录");
	    ...
	    while (reportIter.hasNext()){
			...
			while (dayIter.hasNext()){
				...
				cell.setCellValue(new HSSFRichTextString(reportMap.get("day"+dayMap.get("date").toString()).toString()));
			...
			cell4.setCellValue(new HSSFRichTextString(reportMap.get("workNum").toString()));
			...
			cell6.setCellValue(new HSSFRichTextString(reportMap.get("restTotNum").toString()));
			...
			cell7.setCellValue(new HSSFRichTextString(reportMap.get("restHalfNum").toString()));
			...
			cell8.setCellValue(new HSSFRichTextString(reportMap.get("totLinkNum").toString()));
			cell8.setCellStyle(style);
			//半勤连班
			recordReport.setColumnWidth((short)(short)(dayList.size()+9),(short)1200);
			HSSFCell cell9 = row.createCell((short)(dayList.size()+9));
			cell9.setCellType(HSSFCell.CELL_TYPE_STRING);			
			cell9.setCellValue(new HSSFRichTextString(reportMap.get("halfLinkNum").toString()));
			cell9.setCellStyle(style);
			//工作日补贴
			recordReport.setColumnWidth((short)(short)(dayList.size()+10),(short)1200);
			HSSFCell cell10 = row.createCell((short)(dayList.size()+10));
			cell10.setCellType(HSSFCell.CELL_TYPE_STRING);			
			cell10.setCellValue(new HSSFRichTextString(reportMap.get("workPatch").toString()));
			cell10.setCellStyle(style);
			//双休日补贴
			recordReport.setColumnWidth((short)(short)(dayList.size()+11),(short)1200);
			HSSFCell cell11 = row.createCell((short)(dayList.size()+11));
			cell11.setCellType(HSSFCell.CELL_TYPE_STRING);			
			cell11.setCellValue(new HSSFRichTextString(reportMap.get("restPatch").toString()));
			cell11.setCellStyle(style);
			//补贴总计
			recordReport.setColumnWidth((short)(short)(dayList.size()+12),(short)1200);
			HSSFCell cell12 = row.createCell((short)(dayList.size()+12));
			cell12.setCellType(HSSFCell.CELL_TYPE_STRING);			
			cell12.setCellValue(new HSSFRichTextString(reportMap.get("totPatch").toString()));
			cell12.setCellStyle(style);
			*/
			setCellDisp(dayList,recordReport,row,reportMap,style,4,"workNum");
			setCellDisp(dayList,recordReport,row,reportMap,style,5,"halfNum");
			setCellDisp(dayList,recordReport,row,reportMap,style,6,"restTotNum");
			
			setCellDisp(dayList,recordReport,row,reportMap,style,7,"restHalfNum");
			setCellDisp(dayList,recordReport,row,reportMap,style,8,"totLinkNum");
			setCellDisp(dayList,recordReport,row,reportMap,style,9,"halfLinkNum");
			
			setCellDisp(dayList,recordReport,row,reportMap,style,10,"workPatch");
			setCellDisp(dayList,recordReport,row,reportMap,style,11,"restPatch");
			setCellDisp(dayList,recordReport,row,reportMap,style,12,"totPatch");
	    }
	...
	private void setCellDisp(List dayList,HSSFSheet recordReport,HSSFRow row,Map reportMap,HSSFCellStyle style,int coli,String disp){
		//工作日全勤
		recordReport.setColumnWidth((short)(short)(dayList.size()+coli),(short)1200);
		HSSFCell cell = row.createCell((short)(dayList.size()+coli));
		cell.setCellType(HSSFCell.CELL_TYPE_STRING);
		cell.setCellValue(new HSSFRichTextString(reportMap.get(disp).toString()));
		cell.setCellStyle(style);
	}
	...
	private void makeExcelTitle(String statMonth,HSSFSheet recordReport,HSSFCellStyle style,String deptId){
		...
		cell77.setCellValue(new HSSFRichTextString("全勤数"));
		...
		cell771.setCellValue(new HSSFRichTextString("半勤数"));
		...
		cell88.setCellValue(new HSSFRichTextString("连班数"));
		...
		*/
		createHeadRow(recordReport,dayNum,style,4,"工作日全勤");
		createHeadRow(recordReport,dayNum,style,5,"工作日半勤");
		createHeadRow(recordReport,dayNum,style,6,"双休日全勤");
		
		createHeadRow(recordReport,dayNum,style,7,"双休日半勤");
		createHeadRow(recordReport,dayNum,style,8,"全勤连班");
		createHeadRow(recordReport,dayNum,style,9,"半勤连班");
		
		createHeadRow(recordReport,dayNum,style,10,"工作日补贴");
		createHeadRow(recordReport,dayNum,style,11,"双休日补贴");
		createHeadRow(recordReport,dayNum,style,12,"补贴总计");
	}
	
	private void createHeadRow(HSSFSheet recordReport,int dayNum,HSSFCellStyle style,int celli,String dispName){
		HSSFRow row = recordReport.createRow(0);
		HSSFCell cell = row.createCell((short) (dayNum+celli));		 
		cell.setCellValue(new HSSFRichTextString(dispName));
		cell.setCellStyle(style);
		recordReport.createRow(1).createCell((short) (dayNum+celli)).setCellStyle(style);
		recordReport.createRow(2).createCell((short) (dayNum+celli)).setCellStyle(style);
	...
	private void makeExcelBottomStu(HSSFSheet recordReport,String operId,int cellRow){
		...
		HSSFCell cell00 = row0.createCell((short) 0);
		cell00.setCellType(HSSFCell.CELL_TYPE_STRING);
		cell00.setCellValue(new HSSFRichTextString("符号说明：√全勤，O半勤，×缺勤，//连班全勤，#连班半勤"));
