...
public class LeaveInfoServiceImpl implements LeaveInfoService {
	...
	public List getMailList(String deptId) throws Exception {
		...
		*/
		sqlStr.append("select PERSON_CODE, PERSON_NAME,dept, EMAIL from YKT_CK.USR_PERSON")
		.append(" where email is not null and email<>''and dept in (")
		.append(" select DEPT_ID from YKT_CK.DEPARTMENT where dept_id='").append(deptId)
		.append(" ' or dept_id in (select dept_parentid from ykt_ck.department where dept_id='")
