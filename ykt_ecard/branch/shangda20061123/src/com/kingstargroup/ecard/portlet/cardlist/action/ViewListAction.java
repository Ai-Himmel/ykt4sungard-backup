	...
	public ActionForward render(
		...
		try {
			if (!req.getWindowState().equals(WindowState.MAXIMIZED)) {
				req.setAttribute("result", InformationUtil.list(0, EcardConstants.INFORMATION_CARDLIST_TYPE, EcardConstants.DEFAULT_RESULT_SIZE));
				return mapping.findForward("portlet.ecardlist.view");
			} else {
				if (id == null) {
					req.setAttribute("result", InformationUtil.list(page, EcardConstants.INFORMATION_CARDLIST_TYPE, EcardConstants.LIST_MORE_SIZE));
					return mapping.findForward("portlet.ecardlist.viewMore");
				}
				else {
					req.setAttribute("result", CardListUtil.getInfoById(id));
