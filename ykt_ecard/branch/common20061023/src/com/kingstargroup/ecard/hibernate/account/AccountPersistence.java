...
public class AccountPersistence extends BasicPersistence {
	...
	public TAifAccount getAccountByCardId(String cardId) throws DBSystemException {		
		...
		try {
			...
			hsql.append("FROM com.kingstargroup.ecard.hibernate.account.TAifAccount account where")
	...
	public List getCardNoByCustNo(int custId) throws DBSystemException{
		...
		try {
			...
			sqlStr.append(" from com.kingstargroup.ecard.hibernate.card.TPifCard card where ")
	...
	public List getCustomerByStuempNo(String stuempNo)throws DBSystemException{
		...
		try {
			StringBuffer sqlStr = new StringBuffer();
			sqlStr.append(" from com.kingstargroup.ecard.hibernate.customer.TCifCustomer cust where ")
			.append(" cust.stuempNo='"+stuempNo+"' and cust.cutState<2");
			Query q = s.createQuery(sqlStr.toString());
			List list = q.list();
			return list;
		} catch (HibernateException he) {
