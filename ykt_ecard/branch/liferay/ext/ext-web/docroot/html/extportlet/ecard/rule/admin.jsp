...
<%@ include file="/html/extportlet/ecard/rule/permission.jsp"%>
<%
	...
	ResultPage curpage = EcardInformationServiceUtil.search(
			search_title, search_content,
			EcardConstants.INFORMATION_RULE_TYPE, pageNum,
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td width="72%" class="text_blue_12_c">
						<bean:message key="ecardrule.tab.managerule" />
					</td>
	...
	<tr>
		...
		<td valign="top">
			...
			<form name=InfoDelForm
				action="<portlet:actionURL><portlet:param name="struts_action" value="/ext/ecardruledelete" /></portlet:actionURL>"
				...
				<table border="0" cellpadding="0" cellspacing="0" width="100%">
					<c:if
						...
						<tr>
							<td>
								<font class="portlet-msg-error" style="font-size: 15px;">
									<bean:message key="ecard.error.choosedeleteitems" /> </font>
							</td>
						</tr>
					</c:if>
					<c:if
						...
						<tr>
							<td>
								<font class="portlet-msg-success" style="font-size: 15px;">
					...
					<c:if
						...
						<tr>
							<td>
								<font class="portlet-msg-error" style="font-size: 15px;">
									<bean:message key="ecard.error.deleteinfofailed" /> </font>
							</td>
						</tr>
					</c:if>
					<tr>
				...
				<table width="100%" class="inside_table">
					...
					%>
					<tr class="tr_<%=classId%>" style="font-size: 15px;">
						<td width=10>
						...
						<td align="left">
							<a class=text_blue_12> <%=format.format(info.getCreateDate())%>
							</a>
						</td>
						<td align=center>
...
<form name=rendereditfm method=post
	action="<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardruleedit" /></portlet:renderURL>">
...
function renderAddPage() {
	self.location = '<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardruleadd" /></portlet:renderURL>';
}
</script>