	...
	<c:if test="<%=PermissionUtil.hasMapAdmin(request)%>">
		<tr>
			<td>
				...
				<a class="txt_or"
					href="<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/distributionadmin" /></portlet:renderURL>">
			...
			<table cellpadding="4" cellspacing="0" width="100%">
				...
				<%
					...
					while (it.hasNext()) {
						...
						String[] attachIds = StringUtil.split(info.getAttachments(),
				...
				<tr>
					<td>
						<img src="/html/extthemes/classic/images/ecard/jiantou.jpg"
							width="8" height="8">
						<a
							href="<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/distributionview" /></portlet:renderURL>&id=<%=info.getId()%>"
...
<script>
function goto(page) {
	self.location = '<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/distributionview" /></portlet:renderURL>&page=' + page;
}
</script>