...
%>
<table width="95%" height="29" border="0" cellpadding="0"
	cellspacing="0">
	<c:if test="<%=PermissionUtil.hasTutorialAdmin(request)%>">
		<tr>
			<td>
				<a class="txt_or"
					href="<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardtutorialadmin" /></portlet:renderURL>">
				...
				<tr>
					<td>
						<img src="/html/extthemes/classic/images/ecard/jiantou.jpg"
							width="8" height="8">
						<a
							href="<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardtutorialview" /></portlet:renderURL>&id=<%=info.getId()%>"
							class=blue_link_line> <%=info.getInfoTitle()%>(<%=format.format(info.getCreateDate())%>)</a>
						<c:if test="<%=DateFormatUtil.isHotInfo(info.getCreateDate())%>">
							<img src="/html/extthemes/classic/images/ecard/hot.gif"
								width="17" height="8">
						</c:if>
					</td>
				</tr>
...
<script>
function goto(page) {
	self.location = '<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardtutorialview" /></portlet:renderURL>&page=' + page;
}
</script>