...
<table width="100%" height="29" border="0" cellpadding="0"
	cellspacing="0">
	<c:if test="<%=PermissionUtil.hasCardProcessAdmin(request)%>">
		<tr>
			<td>
				<a class="txt_or"
					href="<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardcardprocessadmin" /></portlet:renderURL>">
				...
				<tr>
					<td>
						<img src="/html/extthemes/classic/images/ecard/jiantou.jpg"
							width="8" height="8">
						<a
							href="<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardcardprocessview" /></portlet:renderURL>&id=<%=info.getId()%>"
...
<script>
function goto(page) {
	self.location = '<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardcardprocessview" /></portlet:renderURL>&page=' + page;
}
</script>