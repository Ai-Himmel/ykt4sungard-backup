	...
	<c:if test='<%=request.getAttribute("result") != null%>'>
				...
				<td>
					<display:table name="result" pagesize="10" class="isis"
						...
						<display:column property="pos"
							title="<%=LanguageUtil.get(pageContext, "totlesaving.pos")%>"
							format="{0,number,#,##0.00}"></display:column>
						<display:column property="transfer"
							title="<%=LanguageUtil.get(pageContext, "totlesaving.transfer")%>"
							format="{0,number,#,##0.00}"></display:column>
						<display:column property="cash"
							title="<%=LanguageUtil.get(pageContext, "totlesaving.cash")%>"
							format="{0,number,#,##0.00}"></display:column>
						<display:column property="outlay"
							title="<%=LanguageUtil.get(pageContext, "totlesaving.outlay")%>"
							format="{0,number,#,##0.00}"></display:column>
						<display:column property="ticket"
							title="<%=LanguageUtil.get(pageContext, "totlesaving.ticket")%>"
							format="{0,number,#,##0.00}"></display:column>		
					</display:table>
					<br>
				</td>
		...
		<table>
			...
			<cewolf:chart id="verticalBar"
				title='<%=LanguageUtil.get(pageContext, "totlesavingmoney.title")%>'
			...
			<cewolf:chart id="pieChart"
				title='<%=LanguageUtil.get(pageContext, "totlesaving.scale")%>'
				type="pie">
				<cewolf:colorpaint
					color='<%=LanguageUtil.get(pageContext, "chart.bgcolor")%>' />
				<cewolf:data>
