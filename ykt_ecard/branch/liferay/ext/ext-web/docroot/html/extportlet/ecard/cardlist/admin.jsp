			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td width="72%" class="text_blue_12_c">
						<bean:message key="ecardlist.tab.managelist" />
					</td>
...
<table width="719" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<td valign="top">
			<!-- content table -->
			<form name=InfoDelForm
				action="<portlet:actionURL><portlet:param name="struts_action" value="/ext/ecardlistdelete" /></portlet:actionURL>"
				...
				<table border="0" cellpadding="0" cellspacing="0" width="100%">
					<c:if
						...
						<tr>
							<td>
								<font class="portlet-msg-error" style="font-size: 11;"> <bean:message
						...
						<tr>
							<td>
								<font class="portlet-msg-success" style="font-size: 11;">
									<bean:message key="ecard.error.deleteinfosucceed" /> </font>
							</td>
						</tr>
					</c:if>
					<c:if
						...
						<tr>
							<td>
								<font class="portlet-msg-success" style="font-size: 11;">
									<bean:message key="ecard.error.editinfosuccess" /> </font>
							</td>
						</tr>
					</c:if>
					<c:if
					...
					<tr>
						<td>
							<html:button property="add" styleClass="button_blank"
								...
								<bean:message key="ecard.add" />
							</html:button>
							<html:button property="delete" styleClass="button_blank"
								onclick="javascript:cmdsend();">
								<bean:message key="ecard.delete" />
							</html:button>
						</td>
					</tr>
					<tr>
				...
				<table width="100%" class="inside_table">
					<tr class="list_tr">
						<td width=3%>
							<input name="ids_allbox" type="checkbox" onClick="chenkbos();">
						</td>
						<td width=70% align=center>
							<a class=text_blue_12_c><bean:message key="ecard.title" />
							</a>
						</td>
						<td width=15% align=center>
							<a class=text_blue_12_c><bean:message key="ecard.publishtime" />
							</a>
						</td>
						<td width=12% align=center>
							<a class=text_blue_12_c><bean:message key="ecard.publisher" />
							</a>
						</td>
					</tr>
					<%
					...
					<tr class="tr_<%=classId%>" style="font-size: x-small;">
						<td width=10>
							<input type="checkbox" name="ids"
								value="<%=String.valueOf(info.getId())%>"
								onClick="javascript:checkAllBox(document.InfoDelForm, 'ids', document.InfoDelForm.ids_allbox)" />

						</td>
						<td align=center>
							<a class=blue_link_line
								href="javascript:setRenderEditForm('<%=info.getId()%>', '<%=search_title%>' , '<%=search_content%>', '<%=pageNum%>');">
								<%=info.getInfoTitle()%> </a>
						</td>
						<td align=center>
							<a class=text_blue_12> <%=info.getCreateDate()==null?"":format.format(info.getCreateDate())%>
							</a>
						</td>
						<td align=center>
							<a class=text_blue_12> <%=info.getCreater()%> </a>
						</td>
					</tr>
					<%
						}
...
<form name=rendereditfm method=post
	action="<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardlistedit" /></portlet:renderURL>">
...
function cmdsend(){
	if (confirm('<%=UnicodeLanguageUtil.get(pageContext,
							"ecard.delete.confirm")%>')) 
	{  
	submitForm(document.InfoDelForm); 
	}
}
function chenkbos(){
...
function renderAddPage() {
	self.location = '<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardlistadd" /></portlet:renderURL>';
}
</script>