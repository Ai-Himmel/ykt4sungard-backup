          ...
          <tr>
            ...
            <script>
            	...
            	<c:if test="<%= PermissionUtil.hasTutorialAdmin(request)%>">
					document.InfoSearchForm.action = '<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ext/ecardtutorialadmin" /></portlet:renderURL>';
				</c:if>
				<c:if test="<%= !PermissionUtil.hasTutorialAdmin(request)%>">
					document.InfoSearchForm.action = '<portlet:actionURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ext/ecardtutorialsearch_result" /></portlet:actionURL>';
				</c:if>
      ...
      <tr>
		...
		<table width="100%"  border="0" cellspacing="0" cellpadding="0" background="/html/extthemes/classic/images/ecard/bbs_1_new.jpg">
          <tr>
			...
			<a class="txt_or" href="<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ext/ecardtutorialview" /></portlet:renderURL>" >
		...
		<table width="100%"  border="0" cellspacing="0" cellpadding="0" background="/html/extthemes/classic/images/ecard/bbs_1_new.jpg">
          <tr>
			...
			<a class="txt_or" href="<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ext/ecardtutorialadmin" /></portlet:renderURL>" >
