      ...
      <tr>
		...
		<bean:message key="ecard.goto" />
              <select id=infotype onchange="goto('0', this.options[this.selectedIndex].value)">
					...
					<%java.util.List typeList = InfoTypeServiceUtil.getChildList(EcardConstants.INFORMATION_FAQ_TYPE).getList();
...
<table width="719"  border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td valign="top">
          <!-- content table -->
          <form name=InfoDelForm action="<portlet:actionURL><portlet:param name="struts_action" value="/ext/ecardfaqdelete" /></portlet:actionURL>" method="POST"
	...
	<tr class="tr_<%= classId%>" style="font-size: x-small;">
		...
		<td align=center>
			...
			<%= InfoTypeServiceUtil.getTypeById(info.getInfoType()).getInfoName()%>
			</a>
		</td>
		<td align=center>
			<a class=text_blue_12>
...
<form name=rendereditfm method=post action="<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ext/ecardfaqedit" /></portlet:renderURL>">
...
function renderAddPage() {
	self.location = '<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ext/ecardfaqadd" /></portlet:renderURL>';
}
</script>