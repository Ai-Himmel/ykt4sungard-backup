...
<%
	...
	ResultPage curpage = InfoTypeServiceUtil.getChildList(
			EcardConstants.INFORMATION_FAQ_TYPE, pageNum,
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td width="72%" class="text_blue_12_c">
						<bean:message key="ecardfaq.tab.managesubtype" />
					</td>
	...
	<tr>
		...
		<td valign="top">
			...
			<form name=InfoDelForm
				action="<portlet:actionURL><portlet:param name="struts_action" value="/ext/ecardfaqdelete_type" /></portlet:actionURL>"
				...
				<table width="100%" class="inside_table">
					...
					for (var i = 0; i < InfoDelForm.elements.length; i++) {
						...
						<td width=97% align=center>
							<a class=text_blue_12_c><bean:message key="ecard.subtype" />
							</a>
						</td>
					</tr>
					<%
					...
					<tr class="tr_<%=classId%>" style="font-size: x-small;">
						<td width=10>
							<input type="checkbox" name="ids"
								value="<%=String.valueOf(info.getInfoType())%>"
								onClick="javascript:checkAllBox(document.InfoDelForm, 'ids', document.InfoDelForm.ids_allbox)" />

						</td>
						<td align=center>
							<a class=blue_link_line
								href="<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardfaqedit_type" /></portlet:renderURL>&id=<%=info.getInfoType()%>">
								<%=info.getInfoName()%> </a>
						</td>
					</tr>
					<%
...
<script>
function goto(page) {
	self.location = '<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardfaqadmin_type" /></portlet:renderURL>&page=' + page;
}
</script>
<script>
function renderAddPage() {	
	self.location = '<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardfaqadd_type" /></portlet:renderURL>';
}
</script>