...
<%
	...
	ResultPage curpage = EcardInformationServiceUtil.search(
			...
			EcardConstants.INFORMATION_TUTORIAL_TYPE, pageNum,
							...
							<tr>
								<td width="13" align="right">
									<c:if test="<%=curpage.isPreviousPage()%>">
										<a href="javascript:goto('<%=curpage.getPage() - 1%>')"> <img
												align="middle"
												src="<%=themeDisplay.getPathThemeRoot()%>/images/ecard/left.gif"
								...
								<td width="13">
									<c:if test="<%=curpage.isNextPage()%>">
										<a href="javascript:goto('<%=curpage.getPage() + 1%>')"> <img
												align="middle"
												src="<%=themeDisplay.getPathThemeRoot()%>/images/ecard/right.gif"
	...
	<tr>
		...
		<td valign="top">
			...
			<form name=InfoDelForm
				action="<portlet:actionURL><portlet:param name="struts_action" value="/ext/ecardtutorialdelete" /></portlet:actionURL>"
				...
				<table border="0" cellpadding="0" cellspacing="0" width="100%">
					<c:if
						...
						<tr>
							<td>
								<font class="portlet-msg-error" style="font-size: 13px;"> <bean:message
					...
					<c:if
						...
						<tr>
							<td>
								<font class="portlet-msg-success" style="font-size: 13px;">
				...
				<table width="100%" class="inside_table">
					...
					for (var i = 0; i < InfoDelForm.elements.length; i++) {
						...
						<td align=center>
							<a class="text_blue_12_c"><b><bean:message key="ecard.publisher" /></b>
							</a>
						</td>
					</tr>
					<%
					...
					%>
					<tr class="tr_<%=classId%>" style="font-size: 11px;">
						<td width=3%>
...
<form name=rendereditfm method=post
	action="<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardtutorialedit" /></portlet:renderURL>">
...
function renderAddPage() {
	self.location = '<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" portletMode="<%=PortletMode.VIEW.toString()%>"><portlet:param name="struts_action" value="/ext/ecardtutorialadd" /></portlet:renderURL>';
}
</script>