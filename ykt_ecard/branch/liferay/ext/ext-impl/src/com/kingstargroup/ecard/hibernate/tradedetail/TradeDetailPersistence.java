...
public class TradeDetailPersistence extends BasePersistence {
	...
	public List getcomsumtradeSerial(String beginDate,String endDate,int cardid) throws DBSystemException {	
		
		Session session = null;		
		try {
			session = openSession();			
			Query query = null;
			query = session.getNamedQuery("TradelogSQL");
			query.setLong("cardno",cardid);
			query.setString("startDate", beginDate);
			query.setString("endDate", endDate);			
			List seriinfo = query.list();
			return seriinfo;				
		} catch (HibernateException ex) {			
			_log.error(ex);			
			throw new DBSystemException(ex.getMessage());
