...
public class TradeDetailUtil {
	...
	public static ResultPage getTradeDetail(String cardId, String beginDate, String endDate, int page, int size) throws PortalException {			
		...
		result = getPersistence().getTradeDetailsByDate(cardId, beginDate, endDate, nowDate, page, size);
		...
		for (int i = 0; i < detailList.size(); i++) {
			 ...
			 detailMap.put("tradeName", TradeCodeUtil.getTradeCodeById(objects[0].toString()).getTransname());//交易名称
	...
	private static TradeDetailUtil _getUtil() {
		if (_util == null) {
			_util = (TradeDetailUtil)com.liferay.portal.kernel.bean.BeanLocatorUtil.locate(_UTIL);
		}
