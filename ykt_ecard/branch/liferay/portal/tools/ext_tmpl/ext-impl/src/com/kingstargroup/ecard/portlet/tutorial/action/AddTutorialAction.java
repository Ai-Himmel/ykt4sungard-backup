 ...
 * @since 1.0
 */
public class AddTutorialAction extends PortletAction {
...
//			String creater = GetterUtil.get((String)req.getSession().getAttribute(WebKeys.USER_ID), StringPool.BLANK);
			if (isTokenValid(req)) {				
				EcardInformationServiceUtil.add(title, content, "", String.valueOf(EcardConstants.INFORMATION_TUTORIAL_TYPE), creater);
				resetToken(req);
			}			
			...
			if (cmd.equals("saveandnext")) {
				...
				return mapping.findForward("portlet.ecardtutorial.add");
			}
			return mapping.findForward("portlet.ecardtutorial.admin");
