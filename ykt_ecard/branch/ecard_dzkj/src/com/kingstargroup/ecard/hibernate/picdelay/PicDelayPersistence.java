 ...
 * @since 1.0
 */
public class PicDelayPersistence extends BasicPersistence{
	...
	public List getDelays(String status) throws DBSystemException {
		...
		try {
			...
			criteria.add(org.hibernate.criterion.Expression.eq("status", status));
	 ...
	 * Modify History:
	 */
	public EcardPicDelay getDelayById(String curStuempId) throws DBSystemException {
		Session s = getSession();
		try {
	 ...
	 * Modify History:
	 */
	public ResultPage getPostponeFiles(int page, int size) throws DBSystemException {
		Session s = getSession();
		try {
	...
	private static final Log _log = LogFactory.getLog(PicDelayPersistence.class);
