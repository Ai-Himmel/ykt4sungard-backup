...
public class QueryDetailResultAction extends PortletAction {
	...
	public ActionForward execute(
		...
		try {
            ...
            if(Integer.parseInt(beginDate)<Integer.parseInt(currDate)){
                beginDate = currDate;
                req.setAttribute(PageContext.EXCEPTION, new PortalException("消费流水记录只能查询最近一个月的数据。"));
                return mapping.findForward(Constants.COMMON_ERROR);
            }
            if (Integer.parseInt(endDate)<Integer.parseInt(currDate)){
                req.setAttribute(PageContext.EXCEPTION, new PortalException("消费流水记录只能查询最近一个月的数据。"));
                return mapping.findForward(Constants.COMMON_ERROR);
            }else{
