...
<!--
   function query(){
    	 	...
    	 	if(elecForm.all.room.value==''){
    	 		alert('<%= LanguageUtil.get(pageContext, "room.null") %>');
    	 		elecForm.all.room.focus();
    	 		return false;
    	 	}
    	 	
    	 	if(elecForm.all.room.value.length!=6){
    	 		alert('<%= LanguageUtil.get(pageContext, "room.length") %>');
    	 		elecForm.all.room.focus();
    	 		return false;
    	 	}
        ...
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <td width="45%" valign="baseline" class="text_or_12">     
                <%String room ="";
                  room = request.getAttribute("room")==null?"":request.getAttribute("room").toString();
                %>
                <input  name="room" size="20" maxlength="10" value="<%=room%>"/>
				&nbsp;
            </td>
            <td width="40%" align=left>
            <font class="portlet-msg-error" style="font-size: 11;"><html:errors property="room" />
            <%=errmsg%>
            </font></td>	
          </tr>
          <%String  remainElec="";
               if(request.getAttribute("remainElec")!=null){
                  remainElec = request.getAttribute("remainElec").toString();
               }
             if(!"".equals(remainElec)){
          %>
           <tr>
            <td width="3%"></td>
            <td width="12%" valign="baseline" class="text_or_12"><bean:message key="room.elecmoney" />: 
              </td>
            <td width="45%" valign="baseline" class="text_or_12">            
				<%=remainElec%> 
            </td>
