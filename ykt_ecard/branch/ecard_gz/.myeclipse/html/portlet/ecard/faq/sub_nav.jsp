<%@ include file="/html/portlet/ecard/faq/init.jsp" %>
	...
	<table width="243" height="20"  border="1" cellpadding="0" cellspacing="0" bordercolor="#F5F9FF" background="<%= themeDisplay.getPathThemeRoot() %>/images/ecard/left_bg.jpg">
	  <tr>
	    <td valign="top" bordercolor="#cfcece">	    
	    <table width="100%" height="7"  border="0" cellpadding="0" cellspacing="0">
	      <tr>
	        ...
	        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
	          <tr>
	            <td width="17%">&nbsp;</td>
	            <td width="83%" class="text_w_12"><bean:message key="ecardfaq.tab.search" /></td>
	          </tr>
	        </table></td>
	      </tr>
	    </table>
        
        <html:form action="/ecardfaq/search_result" method="POST"
		...
		<input type=hidden name="infotype" value="<%= ParamUtil.getString(request, "infotype")%>">
	...
	<table width="239" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <script>
				...
				document.InfoSearchForm.action = '<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ecardfaq/admin" /></portlet:renderURL>';
      ...
      <tr>
		...
		<table width="100%"  border="0" cellspacing="0" cellpadding="0" background="<%= themeDisplay.getPathThemeRoot() %>/images/ecard/bbs_1_new.jpg">
          <tr>
			...
			<a class="txt_or" href="<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ecardfaq/view" /></portlet:renderURL>" >
			<bean:message key="ecardfaq.tab.faqlist" />
			</a></td>
          </tr>
        </table>
		<table width="191"  border="0" cellspacing="0" cellpadding="0">
		...
		<table width="100%"  border="0" cellspacing="0" cellpadding="0" background="<%= themeDisplay.getPathThemeRoot() %>/images/ecard/bbs_1_new.jpg">
          <tr>
			...
			<a class="txt_or" href="<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ecardfaq/admin" /></portlet:renderURL>" >
        ...
        <table width="100%"  border="0" cellspacing="0" cellpadding="0" background="<%= themeDisplay.getPathThemeRoot() %>/images/ecard/bbs_1_new.jpg">
          <tr>
            <td width="27%" height="29">&nbsp;</td>
			<td width="73%" style="cursor:hand" onClick="">
			<a class="txt_or" href="<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ecardfaq/admin_type" /></portlet:renderURL>" >
			<bean:message key="ecard.managesubtype" />
			</a></td>
          </tr>
        </table>
        ...
        <td align="right">
        <c:if test="<%= PermissionUtil.hasFAQAdmin(request)%>">
	        <img src="<%= themeDisplay.getPathThemeRoot() %>/images/ecard/left_kk.jpg" width="46" height="90">
        </c:if>
        <c:if test="<%= !PermissionUtil.hasFAQAdmin(request)%>">
    ...
    <table width="243" height="100"  border="1" cellpadding="0" cellspacing="0" bordercolor="#F5F9FF" background="<%= themeDisplay.getPathThemeRoot() %>/images/ecard/left_bg.jpg">
      <tr>
      <c:choose>
	      <c:when test="<%= !PermissionUtil.hasFAQAdmin(request)%>">
	        <td height="306" bordercolor="#cfcece">&nbsp;</td>
		  </c:when>
		  <c:otherwise>
		  	<td height="185" bordercolor="#cfcece">&nbsp;</td>
		  </c:otherwise>
	  </c:choose>
      </tr>
  </table>