...
public class AccountUtil extends BasicUtil {
	 ...
	 * @since 1.0
	 */
	public static void loginInfo(String cutId)throws PortalException,Exception{
		AccountPersistence persistence = (AccountPersistence) getPersistence(AccountPersistence.class.getName());
		...
		try {
			...
			if("admin".equals(cutId)){
				log.setCustId(Integer.valueOf("0"));
				log.setLoginName("admin");
			}else{
				cust = custper.getCustomerById(Integer.parseInt(cutId));
				if(null==cust){
					log.setCustId(Integer.valueOf("0"));
					log.setLoginName("");
				}else{
					log.setCustId(cust.getCutId());
					log.setLoginName(cust.getCutName());
				}
			}
			...
			log.setComments(cutId+"["+operateDate+operateTime+"]:成功登录综合查询系统");
	...
	public static TCifCustomer getCifCustomerbyStuempNo(String stuempNo)throws PortalException{
		AccountPersistence persistence = (AccountPersistence) getPersistence(AccountPersistence.class.getName());
		try {
			List list = persistence.getCustomerByStuempNo(stuempNo);
			TCifCustomer cust = null;
