...
import com.kingstargroup.ecard.hibernate.dictionary.DictonaryMsg;
...
public class GateHisTradeDetailUtil extends BasicUtil {
	...
	private static final Logger logger = Logger.getLogger(GateHisTradeDetailUtil.class);
	...
	public static ResultPage getGateHisTradeDetail(String cardId, String beginDate, String endDate, int page, int size) throws PortalException {			
		...
		//	 logger.info("-----------end-- Tpifdevice-------------");
			 ...
			 String  dictonary =getDictonary(Integer.parseInt(String.valueOf(objects[5])));
			 detailMap.put("mark", dictonary);//交易状�?
	...
	public static String getDictonary(int code) throws PortalException {
		DictonaryMsg error = (DictonaryMsg) getPersistence(DictonaryMsg.class.getName());
		try {
			return error.getErrorMsgByCode(code);
		} catch  (DBSystemException dbse)  {
			throw dbse;
