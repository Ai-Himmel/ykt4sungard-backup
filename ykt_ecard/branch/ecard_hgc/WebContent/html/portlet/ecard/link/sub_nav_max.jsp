	    ...
	    <td valign="top" bordercolor="#cfcece"><table width="100%" height="7"  border="0" cellpadding="0" cellspacing="0">
	      <tr>
	        ...
	        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
	          <tr>
	            <td width="17%">&nbsp;</td>
	            <td width="83%" class="text_w_12"><bean:message key="frindslink" /></td>
	          </tr>
	        </table></td>
	      </tr>
	    </table>
        
        <html:form action="/ecardlink/search_result" method="POST"
          ...
          <tr>
            ...
            <script>
            	...
            	<c:if test="<%= PermissionUtil.hasCardlistAdmin(request)%>">
					document.InfoSearchForm.action = '<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ecardlink/admin" /></portlet:renderURL>';
				</c:if>
				<c:if test="<%= !PermissionUtil.hasCardlistAdmin(request)%>">
					document.InfoSearchForm.action = '<portlet:actionURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ecardlink/search_result" /></portlet:actionURL>';
				</c:if>
      ...
      <tr>
		...
		<table width="100%"  border="0" cellspacing="0" cellpadding="0" background="<%= themeDisplay.getPathThemeRoot() %>/images/ecard/bbs_1_new.jpg">
          <tr>
			...
			<a class="txt_or" href="<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ecardlink/view" /></portlet:renderURL>" >
			<bean:message key="frindslink" />
			</a></td>
          </tr>
        </table>
		<table width="191"  border="0" cellspacing="0" cellpadding="0">
		...
		<table width="100%"  border="0" cellspacing="0" cellpadding="0" background="<%= themeDisplay.getPathThemeRoot() %>/images/ecard/bbs_1_new.jpg">
          <tr>
			...
			<a class="txt_or" href="<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>" portletMode="<%= PortletMode.VIEW.toString() %>"><portlet:param name="struts_action" value="/ecardlink/admin" /></portlet:renderURL>" >
			<bean:message key="frindslink.manager" />
			</a></td>
          </tr>
        </table>
		</c:if>        
