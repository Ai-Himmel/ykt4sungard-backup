...
public class AddPicDelayResultAction extends PortletAction {
	...
	public ActionForward execute(
		...
		throws Exception {
		try {
			String customerId = GetterUtil.get((String)req.getSession().getAttribute(WebKeys.USER_ID), StringPool.BLANK);
			String oldStuempNo = ParamUtil.get(req, "oldStuempNo", "");
			if (isTokenValid(req)) {
				
				PicDelayUtil.add(oldStuempNo, customerId);
				resetToken(req);
			}			
