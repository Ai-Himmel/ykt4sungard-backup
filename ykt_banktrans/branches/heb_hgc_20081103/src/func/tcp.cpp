...
bool CTcpSocket::Listen(short iPort,char *ip)
{
   ...
   {
      Close();
	  writelog(LOG_ERR, "can't bind socket from ip = [%s], port = [%d]", ip, iPort);
	  return(false);
   }
   if (listen(m_socket,TCP_LISTEN_NUM))
   {
      Close();
	  writelog(LOG_ERR, "can't listen socket from ip = [%s], port = [%d]", ip, iPort);
	  return(false);
   }
   return(true);
}
...
/*
bool CTcpSocket::Accept2(CTcpSocket &sock, char *remote)
{
	int tempsocket;
	struct sockaddr_in addr;
	addr.sin_family=m_nAddressFamily;
	SOCKADDRLEN addrlen = sizeof(addr);
	tempsocket = accept( m_socket, (struct sockaddr*)&addr, &addrlen);
	writelog(LOG_ERR, "accept temp socket = [%d]", tempsocket);
	if(tempsocket < 0)
	{
	  return false;
	}
	...
	sock.Close();
	sock.m_socket = tempsocket;
	sock.m_bConnected = true;
	sock.m_nAddressFamily = m_nAddressFamily;
