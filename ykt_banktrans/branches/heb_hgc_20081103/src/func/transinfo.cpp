...
int Pos_QueryBank(INNER_TRANS_REQUEST * pNode)
{
	...
	if(AUTH_MODE_SCHOOLPWD==g_Bank.AUTH_MODE)
	{
		...
		if (g_Bank.default_bank_id != BANK_HRBZH)	// 哈尔滨工行查询余额的时候不输入校园卡密码			
		{
		...
		memcpy(pNode->BankCardNo, bankcard_no, sizeof(pNode->BankCardNo));
			
	}

	// 测试查询到的银行卡号
	writelog(LOG_INFO,"verify_sch_bank_relationship bankcard_id=[%s], account_id=[%s]",bankcard_no, pNode->BankCardNo);
...
//	ST_PACK *out_pack = &(aPack.pack);
	...
	//向后台发起挂失申请
	ResetNormalCPack(&sPack, 0, 1);
	sPack.head.RequestType = 847102;
	SetHeadCol(&sPack, F_LVOL1,F_SCUST_LIMIT,F_LVOL6,F_LVOL7,F_LVOL8,F_SCUST_AUTH,F_LVOL2, 0);       // 设置发送windows BU的头部视图信息
	//memcpy(&(sPack.pack), in_pack, sizeof(sPack.pack));
	...
	if(g_Bank.LOGIN_MODE==LOGIN_MODE_CUT_NO)
	{
		...
		sPack.pack.lvol8=atoi(card_id);
	}
	else
	{
		des2src(sPack.pack.scust_auth,card_id);
		sPack.pack.lvol8=0;
	}
	sPack.pack.lvol1=0;
	...
	if (ret < 0||aPack.head.retCode!=0)
	{
		if(ret<0)
		{
			writelog(LOG_ERR,"Call 847102 error,errcode=[%d],retCode=[%d]",ret,aPack.head.retCode);
			return E_TRANS_UNKNOW_ERROR;
		}
		else
		{
...
//	ST_PACK *out_pack = &(aPack.pack);
	...
	{
		//判断绑定对应关系是否相符
		ret=verify_sch_bank_relationship(0, cur_card_id, "", 0, bankcard_no);
		if(ret)
		{
			writelog(LOG_INFO,"verify_sch_bank_relationship error!card_id=[%s],bankcard_id=[%s]",card_id,bankcard_no);
			return E_TRANS_RELATION_ERR;
		...
		ret=verify_card_password(cur_card_id, "", 0, card_pwd);
		if(ret)
		{
			writelog(LOG_ERR,"Verify card password faild!card_id=[%s]",card_id);
			return ret;
		...
		if (0 == strlen(pNode->BankCardNo))
		{
			memcpy(pNode->BankCardNo, bankcard_no, sizeof(pNode->BankCardNo));
		}
		writelog(LOG_ERR,"card_id = [%s], pnode card id = [%s], cur_card_id = [%d], bankcardno = [%s], pnode bankcardno = [%s]",card_id, pNode->CardNo, cur_card_id, bankcard_no, pNode->BankCardNo);
	}
	...
	SetHeadCol(&sPack, F_SORDER2,F_SCUST_AUTH,F_SEMP_PWD,F_SSERIAL0,F_SSTATION0,F_LVOL5,F_LVOL0,F_LVOL6,F_LVOL1,F_LVOL2,F_LVOL3, 0);       // 设置发送windows BU的头部视图信息
	//memcpy(&(sPack.pack), in_pack, sizeof(sPack.pack));
	des2src(sPack.pack.sorder2,pNode->TerminalId);
	...
	if (BANK_YNXH == g_Bank.BankUnit[g_Bank.cur_bankid].iBankID) { sPack.pack.lvol3 = 4; }		// 区分一卡通账户, 不同账户入不同交易
	else if (BANK_YNNH == g_Bank.BankUnit[g_Bank.cur_bankid].iBankID) { sPack.pack.lvol3 = 5; }
	...
	if(ret)
	{
		...
		if(E_TRANS_BANK_NOANSWER==ret)			//如果银行没有返回包，进行冲正处理
		{
			...
			SetHeadCol(&sPack, F_SORDER2,F_SCUST_AUTH,F_SEMP_PWD,F_SSERIAL0,F_LVOL5,F_LVOL0,F_LVOL6,F_LVOL1,F_LVOL2,F_LVOL3, 0);       // 设置发送windows BU的头部视图信息
			//memcpy(&(sPack.pack), in_pack, sizeof(sPack.pack));
			des2src(sPack.pack.sorder2,pNode->TerminalId);
			des2src(sPack.pack.scust_auth,pNode->BankCardNo);
			des2src(sPack.pack.semp_pwd,pNode->AccountPwd);
	...
	SetHeadCol(&sPack, F_SORDER2,F_SCUST_AUTH,F_SEMP_PWD,F_SSERIAL0,F_SSTATION0,F_LVOL5,F_LVOL0,F_LVOL6,F_LVOL1,F_LVOL2,F_LVOL7,F_LVOL3,0);       // 设置发送windows BU的头部视图信息
...
//	ST_PACK *out_pack = &(aPack.pack);
	...
	writelog(LOG_ERR,"out_pack_scust_limit=[%s], bank_card_no=[%s]",aPack.pack.scust_limit, bankcard_no);
	if (0 == strlen(bankcard_no))
	{
		strncpy(bankcard_no, aPack.pack.scust_limit, sizeof(aPack.pack.scust_limit));
	}
