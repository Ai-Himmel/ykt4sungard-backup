...
#define     BANK_HRBZH_TRADE_CODE_SIGN_IN		    "0100"	// 签到交易
#define     BANK_HRBZH_TRADE_CODE_QUERYBALANCE		"0240"	// 查询交易
#define     BANK_HRBZH_TRADE_CODE_TRANS		    	"0200"	// 转账交易
#define     BANK_HRBZH_TRADE_CODE_DETRANS		    "0400"	// 冲正交易
...
typedef struct  
{
	char AllPacketLen[4];	// 总体报文长度
	char PKgtype[1];		// 报文类型
	char MsgType[4];	
	char Prccde[6];
	char AcqInstID[8];		// 收单机构统一码24650000
	char TermID[8];			// 收单机构终端号21045201
	char Trcno[6];			// 流水号
	char Custype[2];		// 卡类别
	char Cusidt[25];		// 对账时填写的机构ID
	char Apcode[2];			// 应用类型21
	char SrvuID[8];			// 对账服务器代码00489900
	char Rspcde[2];			// 返回码
	char PKseq[2];			// 报文序列
	char Filler[6];			// 附加域
	char MsgLen[4];			// 数据域长度, 不足4位前补0	
}BANK_HRBZH_TRADE_STRUCT_HEAD;

// 统一中行8583数据类型格式
typedef struct BANK_HRBZH_TRADE_PACKET
{
	char msg_type_id[4];
	char bit_map[8];				// 00
	char bit_map_ext[8];			// 01
	char account_id[22];			// 02 LLVAR
	char process_code[6];			// 03
	char trans_amount[12];			// 04 余额
	char trans_date_time[10];		// 07
	char stan[6];					// 11
	char time_trans[6];				// 12
	char date_trans[4];				// 13
	char date_capture[4];			// 17
	char point_of_scc[2];			// 25
	char flush_trade_serial_no[6];	// 28
	char aiic[11];					// 32 LLVAR 电话银行网点号
	char css_id[11];				// 33 LLVAR
	char response_code[2];			// 39 返回码
	char acceptor_terminal_id[8];	// 41 银行终端号
	char acceptor_Id[15];			// 42
	char encrypt_pwd[999];			// 47 LLLVAR
	char reserve_id1[999];			// 48 LLLVAR
	char pid_data[16];				// 52
	char reserve_id2[16];			// 53
	char original_data[42];			// 90
	char reserve_id3[28];			// 102 LLVAR
	char reserve_id4[28];			// 103 LLVAR
}BANK_HRBZH_TRADE_PACKET;

typedef struct
{
	BANK_HRBZH_TRADE_STRUCT_HEAD hrbzh_head;		// 中行标识头
	BANK_HRBZH_TRADE_PACKET hrbzh_trade_packet;		// 中行标识体
}BANK_HRBZH_TRADE_STRUCT_PACKET;
