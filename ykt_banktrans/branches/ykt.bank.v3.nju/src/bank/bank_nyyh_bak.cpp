...
int Bank_QueryBalance(INNER_TRANS_REQUEST * pNode,double *balance)
{
	...
	//写银行卡密码字段
	//通过16进制格式传输，再解密
	//dec2hex(from_pos.BankCardPwd, (unsigned char*)BankReqPak.BankCardPwd, 8);
	//dec2hex((unsigned char*)from_pos.BankCardPwd,8,(char*)BankReqPak.BankCardPwd);
	memcpy(BankReqPak.BankCardPwd,from_pos.BankCardPwd,sizeof(BankReqPak.BankCardPwd));
		...
		writelog(LOG_INFO,"BankCardNo=[%s],BankCardPwd=[%s]",BankReqPak.BankCardNo,BankReqPak.BankCardPwd);
