...
int F900077(TRUSERID *handle,int iRequest,ST_PACK *in_pack,int *pRetCode,char *szMsg)
{
	...
	*/
	...
	SetCol(handle,0);
   	SetCol(handle,F_SCLOSE_EMP,F_SORDER0,F_SORDER1,F_SSERIAL1,F_SSERIAL2,F_LVOL0,F_LVOL1,F_SDATE0,F_STIME0,F_SDATE1,F_STIME1,F_SCUST_NO,F_SCHANGE_EMP,F_SSERIAL0,F_LVOL2,F_SCUST_AUTH,F_SSTATUS1,F_SEMP_PWD,F_SEMP_PWD2,F_SSTATION0,F_SSTATION1,F_SORDER2,0);
	...
	switch(atoi(tradecode))
	{
		...
		case TRADE_INNER_QUERYBANK:
			...
			if(strlen(in_pack->scust_auth)<10)
			{
				int cardno=atoi(in_pack->sserial0);
				ret=QryBankCardNo(cardno,from_pos.BankCardNo,szMsg);
				if(ret)
				{
					writelog(LOG_ERR,"qrybankcardno err ");
					*pRetCode=ret;
					return *pRetCode;
				}
			}
			...
			{
				*pRetCode=ret;
				strcpy(szMsg,from_pos.Reserve3);
				goto L_RETU;
			}
...
//			}
	...
	//		writelog(LOG_INFO,"pos_trans_money=[%d], bank_trans_money=[%d]", atoi(from_pos.TransMoney), g_Bank.trans_money);
			if (atoi(from_pos.TransMoney) > g_Bank.trans_money)
			{
				writelog(LOG_ERR,"transamt:%s,max transamt:%d",from_pos.TransMoney, g_Bank.trans_money);
				*pRetCode=E_TRANS_TRANSMONEY_OUTRANGE;
				goto L_RETU;
			}
			writelog(LOG_INFO,"Pos_Transfer Begin");
			if(strlen(in_pack->scust_auth)<10)
			{
				int cardno=atoi(in_pack->sserial0);
				ret=QryBankCardNo(cardno,from_pos.BankCardNo,szMsg);
	...
	Strncpy_t(out_pack->sserial2,to_pos.Reserve1,sizeof(out_pack->sserial2));
