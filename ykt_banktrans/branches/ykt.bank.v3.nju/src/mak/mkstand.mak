...
PREPCOMP_CXX=proc code=cpp cpp_suffix=cpp maxopencursors=50 MODE=ANSI TYPE_CODE=ANSI CLOSE_ON_COMMIT=yes def_sqlcode=yes dynamic=ANSI define=ESQL_ORA LINES=YES config="$(HOME)/bank/bin/proccfg.cfg" include="/usr/include"  include="$(ORACLE_HOME)/precomp/public" include="$(ORACLE_HOME)/rdbms/public" include="$(HOME)/bank/src/buinc" include="$(HOME)/bank/src/include" include="$(HOME)/bank/src/include/dbincl" include="$(HOME)/bank/src/public" $(PROC_OPTS)
...
ifeq ($(DBTYPE),DB2)
	INCLD= -I./ -I$(HOME)/bank/src/buinc -I$(HOME)/bank/src/public -I$(HOME)/bank/src/include -I$(HOME)/bank/src/include/dbincl -I$(DB2PATH)/include
	CCFLAGS += -DESQL_DB2
	LIBSUDF=-lstdc++ -L$(DB2PATH)/lib -ldb2 -ldb2apie
else
	INCLD= -I./ -I$(HOME)/bank/src/buinc -I$(HOME)/bank/src/public -I$(HOME)/bank/src/include -I$(HOME)/bank/src/include/dbincl -I$(ORACLE_HOME)/precomp/public -I$(ORACLE_HOME)/rdbms/public
	CCFLAGS += -DESQL_ORA
