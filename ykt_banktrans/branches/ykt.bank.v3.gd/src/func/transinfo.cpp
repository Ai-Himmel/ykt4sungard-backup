...
//	ST_PACK *out_pack = &(aPack.pack);
	...
	if(g_Bank.LOGIN_MODE==LOGIN_MODE_CUT_NO)
		custid=atoi(card_id);
	else if(g_Bank.LOGIN_MODE==LOGIN_MODE_STUEMP_NO)
		strcpy(stuempno,card_id);
	else
		cardno=atoi(card_id);
	writelog(LOG_INFO,"cardno[%d],stuempno[%s],custid[%d]",cardno,stuempno,custid);
	ret=verify_card_password(cardno,stuempno,custid, pNode->AccountPwd);
	if(ret)
	{
		writelog(LOG_INFO,"Verify card password faild!card_id=[%s]",card_id);
		return ret;
...
//	ST_PACK *out_pack = &(aPack.pack);
	...
	//如果上传的数据为小数形式的，则转换为以分为单位的
	char sTransAmt[12]={0};
	memcpy(sTransAmt,pNode->TransMoney,9);
	if(strchr(sTransAmt,'.')!=NULL)
	{
		double dTransAmt=atof(sTransAmt)*100;
		sPack.pack.lvol5=D4U5(dTransAmt,0);
	}
	else
		sPack.pack.lvol5=atoi(sTransAmt);
	if (sPack.pack.lvol5 > g_Bank.trans_money)
	{
		writelog(LOG_ERR,"transamt:%d,max transamt:%d",sPack.pack.lvol5, g_Bank.trans_money);
		SetErrMsg("转账金额超限");
		return E_TRANS_TRANSMONEY_OUTRANGE;
	}	
	writelog(LOG_INFO,"transamt:%d,max transamt:%d",sPack.pack.lvol5, g_Bank.trans_money);
	...
	//writelog(LOG_INFO,"g_Bank.BankUnit[g_Bank.cur_bankid].szBankAccountNO=[%s]", g_Bank.BankUnit[g_Bank.cur_bankid].szBankAccountNO);
	writelog(LOG_INFO,"svr drtpno[%d] bcc basefuncno[%d] timeout[%d]",g_Bank.DRTP_BRANCH,g_Bank.BCC_BASEFUNCNO,g_Bank.TIMEOUT);
	ret = ExtCall(0, g_Bank.DRTP_BRANCH, g_Bank.BCC_BASEFUNCNO, 0, g_Bank.TIMEOUT, &sPack, &aPack, &ArrayPack);
	if (ret < 0)
	{
		writelog(LOG_INFO,"test");
	...
	*/
	...
	int SendBankSeqno=aPack.pack.lserial0;
	int refno=atoi(aPack.pack.sname2+8);
	ret=Bank_Transfer(pNode,refno);
	if(ret)
	{
		writelog(LOG_INFO,"test");
