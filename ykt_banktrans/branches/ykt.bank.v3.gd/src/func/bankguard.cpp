		...
		//if(0>1)
		{
			...
			else
			{
				if(ykt_clt.HasMoreRecord())
				{
					...
					GET_PACK_STR(ykt_clt,"sholder_ac_no",refno);
					GET_PACK_STR(ykt_clt,"sdate1",mac);
					...
					if(ret == 0)
					{
						...
						ykt_clt.ResetHandler();
						ykt_clt.SetStringFieldByName("sholder_ac_no", refno);
						ykt_clt.SetStringFieldByName("sdate1", mac);
