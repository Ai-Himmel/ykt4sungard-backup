...
{         
   	{"0190","000000",0,
	{0xF2,0x3A,0x00,0x11,0x0A,0xC1,0x14,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}},
    	{"0180","000000",1,
	{0xF2,0x3A,0x00,0x11,0x0A,0xC1,0x14,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}},
    	{"0200","000000",2,
	{0xF2,0x3A,0x00,0x11,0x08,0x81,0x10,0x00,
	0x00,0x00,0x00,0x00,0x11,0x00,0x00,0x00}},
    	{"0210","000000",3,
	{0xF2,0x3A,0x00,0x11,0x0A,0x81,0x14,0x00,
	0x00,0x00,0x00,0x00,0x11,0x00,0x00,0x00}},
    	{"0400","000000",4,
	{0xF2,0x3A,0x00,0x11,0x0A,0xC1,0x14,0x00,
	0x00,0x00,0x00,0x00,0x11,0x00,0x00,0x00}},
...
/* FLD 4 */ {0,"AMOUNT, TRANSACTION ", 12, 0, 0, 0, NULL,0}, 
...
/* FLD 11 */ {0,"SYSTEM TRACE AUDIT NUMBER ", 20, 0, 0, 0, NULL,0}, 
...
/* FLD 52 */ {0,"ERSONAL IDENTIFICATION NUMBER DATA ", 8, 0, 0, 0, NULL,0}, 
...
//		writelog(LOG_ERR,"trans_type= [%d]",TRANS_TYPE);
		return -1;
	}
	if (transTypeSize < 0) {
		puts("error to get transTypeSize");
//		writelog(LOG_ERR,"trans_type= [%d]",transTypeSize);
		return -1;
	}
	...
	if (ret < 0) {
//		writelog(LOG_ERR,"trans_type= [%d]",ret);
		return -1;
	}
	...
	{
		if ((strncmp(trans_tb[i].msgType,TransMsg.msgType,4)==0))
			break;
	}
	if (i>=TRANS_TYPE)     
		return(-1);
	...
	if (bitData[3]==1)
	{
		...
		//sprintf(Tbl8583[3].data,"%012.0f",atoi(TransMsg.txnAmunt));
		memcpy(Tbl8583[3].data,TransMsg.txnAmunt,Tbl8583[3].length);
	...
	if (bitData[10]==1)
	{
		...
		memcpy(Tbl8583[10].data,TransMsg.tracAudNum,Tbl8583[10].length);
	}
	if (bitData[11]==1)
	{
	...
	if (bitData[14]==1)
	{
		Tbl8583[14].data=(char *)malloc(Tbl8583[14].length+1);
		memcpy(Tbl8583[14].data,TransMsg.bala_date,Tbl8583[14].length);
	}
	...
	if (bitData[27]==1)
	{
		...
		//sprintf(Tbl8583[27].data,"%06lu",TransMsg.trade_fee);
		memcpy(Tbl8583[27].data,TransMsg.trade_fee,Tbl8583[27].length);
...
//		writelog(LOG_DEBUG,"length of field 39 is [%d]",Tbl8583[38].length);
		Tbl8583[36].data=(char *)malloc(Tbl8583[36].length+1);
		memcpy(Tbl8583[36].data,TransMsg.field37,Tbl8583[36].length);
	}
	if (bitData[37]==1)
	{
//		writelog(LOG_DEBUG,"length of field 39 is [%d]",Tbl8583[38].length);
		Tbl8583[37].data=(char *)malloc(Tbl8583[37].length+1);
		memcpy(Tbl8583[37].data,TransMsg.field38,Tbl8583[37].length);
	}
...
//		writelog(LOG_DEBUG,"length of field 39 is [%d]",Tbl8583[38].length);
	...
	if (bitData[51]==1)
	{
		...
		memcpy(Tbl8583[51].data,TransMsg.pinDat,Tbl8583[51].length);
	}
	if (bitData[53]==1)
	{
	...
	if (bitData[103]==1)
	{  
		Tbl8583[103].data=(char *)malloc(strlen(TransMsg.bankCode)+1);
		Tbl8583[103].length=strlen(TransMsg.bankCode);
		memcpy(Tbl8583[103].data,TransMsg.bankCode,Tbl8583[103].length);
	}
	...
	if (bitData[127] == 1) {    /* FIELD127 if */
		...
		default:
			...
			{
//				writelog(LOG_ERR,"trans_type= [%d]",strlen(TransMsg.chkAct));
				return -1;
			}
			...
			if (lenTmp != 4)
			{
//				writelog(LOG_ERR,"trans_type= [%d]",lenTmp);
				return -1;
...
int UnPack8583(unsigned char *buf,MSG8583 *TransMsg,int convDirect)
{
	...
	if (bitData[3]==1)
	{
		memcpy(TransMsg->txnAmunt,Tbl8583[3].data,Tbl8583[3].length);
		memcpy(TransMsg->txnAmunt+Tbl8583[3].length,"\0",1);
		free(Tbl8583[3].data);
		Tbl8583[3].data = NULL;
	...
	//	sprintf(TransMsg->txnAmunt,"%d",atoi(tmp));
	//	free(tmp);
	//	tmp = NULL;
	//	free(Tbl8583[3].data);
	//	Tbl8583[3].data = NULL;
	...
	if (bitData[10]==1)
	{
		memcpy(TransMsg->tracAudNum,Tbl8583[10].data,Tbl8583[10].length);
		memcpy(TransMsg->tracAudNum+Tbl8583[10].length,"\0",1);
	...
	//	strcpy(TransMsg->tracAudNum,tmp);
	//	free(tmp);
		//tmp = NULL;
	//	free(Tbl8583[10].data);
	//	Tbl8583[10].data = NULL;
	...
	if (bitData[14]==1)
	{
		memcpy(TransMsg->bala_date,Tbl8583[14].data,Tbl8583[14].length);
		memcpy(TransMsg->bala_date+Tbl8583[14].length,"\0",1);
		free(Tbl8583[14].data);
		Tbl8583[14].data = NULL;
	}
	...
	if (bitData[27] == 1)
	{
		memcpy(TransMsg->pinDat1,Tbl8583[27].data,Tbl8583[27].length);
		memcpy(TransMsg->pinDat1+Tbl8583[27].length,"\0",1);
...
// 		Tbl8583[27].data = NULL;
	...
	if (bitData[36]==1)
	{
		memcpy(TransMsg->field47,Tbl8583[36].data,Tbl8583[36].length);
		memcpy(TransMsg->field47+Tbl8583[36].length,"\0",1);
		free(Tbl8583[36].data);
		Tbl8583[36].data = NULL;
	}
	if (bitData[37]==1)
	{
		memcpy(TransMsg->field48,Tbl8583[37].data,Tbl8583[37].length);
		memcpy(TransMsg->field48+Tbl8583[37].length,"\0",1);
		free(Tbl8583[37].data);
		Tbl8583[37].data = NULL;
	}
	if (bitData[38]==1)
	{
	...
	if (bitData[51]==1)
	{
		memcpy(TransMsg->txnCuuCod,Tbl8583[51].data,Tbl8583[51].length);
		memcpy(TransMsg->txnCuuCod+Tbl8583[51].length,"\0",1);
...
// 		Tbl8583[51].data = NULL;
	...
	if (bitData[103]==1)
	{  
		memcpy(TransMsg->bankCode,Tbl8583[103].data,Tbl8583[103].length);
		TransMsg->bankCode[Tbl8583[103].length]='\0';
		free(Tbl8583[103].data);
		Tbl8583[103].data = NULL;
	}
	if (bitData[127] == 1){
...
int init8583(MSG8583 *msg)
{
	...
	strcpy(msg->txnAmunt,"0"); 
	initstr(msg->txnDatTim);   /*6*/
	strcpy(msg->tracAudNum,"0");  /*10*/
