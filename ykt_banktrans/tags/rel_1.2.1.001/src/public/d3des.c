...
int EncodePwd(char *key,char *clearpwd,char *secretpwd,int flag)
{
	...
	{
		strcpy(k,key);
		strcpy(clear,clearpwd);		
		des2key(k,0);
		D2des(clear,secret);
		strcpy(secretpwd,secret);
		return 0;
	}
	if(flag==1)
	{
		strcpy(k,key);
		strcpy(secret,secretpwd);
		des2key(k,1);
		D2des(secret,clear);
		strcpy(clearpwd,clear);
		return 0;		
	}
	return -1;
}
...
int main(){//unsigned
	...
	char key[]="vxabcdef01dv7adg";
	char tmp1[100]="";
	char from[]="01234abcd0123456";
	char tmp2[100]="";
	char to[]="";
	char tmp3[100]="";
	char after[]="";
	...
	char stra[20]="";
	sprintf(stra,"%d",inta);
	printf("stra=%s,strlen=%d\n",stra,strlen(stra));
	
	printf("		------------------------------				\n");
	char a[20]="";
	char b[]="abcde";
	strcpy(a,b);
	printf("size of char[] a:%d,size of char[] b:%d\n",sizeof(a),sizeof(b));
	printf("size of string a:%d,size of string b:%d\n",strlen(a),strlen(b));
	printf("		------------------------------				\n");
	printf("-------------------test end------------------------\n");
	
	EncodePwd(key,from,to,0);
	
	EncodePwd(key,after,to,1);
	
	EncodePwd(key,after,toto,0);
	...
	printf("to=");
	int i;
	for(i=0;i<strlen(to);i++){printf("%c",to[i]);}
	printf("\n");
	
	printf("after=%s\n",after); 
	printf("after vs from :%d\n",strcmp(from,after)); 
	printf("to vs toto : %d\n",strcmp(to,toto));   
