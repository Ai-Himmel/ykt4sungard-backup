...
// 需要分离
// 流水认证
// int _stdcall SMT_WaterNewAuthCard()                                      // 这里修改过_stdcall
// {
...
//////////////////////////////////////////////////////////////////////////
// 计算KEYA
void _stdcall SMT_Cal_KeyA(unsigned char *ucRawData, unsigned char *ucSerialNo, unsigned char *ucDesData, unsigned char *ucKeyA)
{
	memcpy(ucRawData,ucSerialNo,4);
	for(int i = 0;i < 4; i++)
		ucRawData[i+4] = ~ucRawData[i];
...
// 计算KEYB
void _stdcall SMT_Cal_KeyB(unsigned char *ucRawData, unsigned char *ucSerialNo, unsigned char *ucDesData, unsigned char *ucKeyB)
{
	memcpy(ucRawData + 4, ucSerialNo, 4);
	for(int i = 0;i < 4; i++)
		ucRawData[i] = ~ucRawData[i + 4];
...
// 返回COMM.H文件中的执行命令(ExeCommand)函数
int _stdcall SMT_ExeCommand(unsigned char *Buffer, int Len)
{
	return ExeCommand(Buffer, Len);
}

void _stdcall SMT_CopyWorkingKey(unsigned char *SMT_UCWORKINGKEY)
{
	memcpy(UCWORKINGKEY, SMT_UCWORKINGKEY, 256);
	int i = 0;
}
//////////////////////////////////////////////////////////////////////////