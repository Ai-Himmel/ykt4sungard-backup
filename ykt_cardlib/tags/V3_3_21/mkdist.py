...
class CardlibDistUtil:
    ...
    def loadLcsFile(self):
        ...
        cmdline += ' -k %s -l %s' % (self.pubKeyFile,self.lcsFile)
    ...
    def loadCfgFile(self):
        ...
        self.pubKeyFile = parser.get('BASE','key')
        if len(self.pubKeyFile) == 0:
            self.pubKeyFile = 'pub.key'
    ...
    def copyLcsFiles(self,reader,writer):
        ...
        if self.lcsParams.has_key('kscardlib.psamcardtype'):
            psamCardtype = self.lcsParams['kscardlib.psamcardtype']
        else:
            pasmCardtype = 'ks_psamcard.dll'
