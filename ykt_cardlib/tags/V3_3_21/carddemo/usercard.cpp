...
BEGIN_EVENT_TABLE(UserCardPanel,wxPanel)
	...
	EVT_BUTTON(BTN_UC_HALTCARD,OnHaltCard)
	EVT_BUTTON(BTN_UC_DEPOSIT2,OnDeposit2)
END_EVENT_TABLE()
...
:wxPanel(parent,id)
{
	...
	btn = new wxButton(this,BTN_UC_HALTCARD,wxT("HALT"));
	sizer->Add(btn,wxGBPosition(row,7),wxGBSpan(1,1));
	// row 2
	++row;
	...
	sizer->Add(btn,wxGBPosition(row,6));
	btn = new wxButton(this,BTN_UC_DEPOSIT2,wxT("充值2"));
	sizer->Add(btn,wxGBPosition(row,7));
...
void UserCardPanel::OnHaltCard(wxCommandEvent& evt)
{
	int ret = ks_halt();
	if(ret)
	{
		wxMessageBox(wxT("HALT卡失败"));
		return;
	}
	ks_beep();
}

void UserCardPanel::OnDeposit2(wxCommandEvent& evt)
{
	ST_TRANSPACK transpack;
	memset(&transpack,0,sizeof transpack);
	...
	if(ks_readpsamtermno(transpack.sTermno,1))
	{
		LogMessage::instance()->LogCardMsg("未读到psam卡");
		return;
	}
	if( ks_card(transpack.sCardPhyID)!=0 )
	{
	...
	transpack.nBefBalance = atoi(sCardNo);
	transpack.nFundType = 1;
	strcpy(transpack.sOperCode,"1000");
	wxDateTime dt = wxDateTime::Now();
	wxString dtStr;
	dtStr = dt.Format("%Y%m%d");
	strcpy(transpack.sTransDate,dtStr.c_str());
	...
	nRet = ks_load4card(&transpack);
	if(!nRet)
	{
		ks_beep();
		wxString info = wxString::Format(wxT("充值成功钱包余额[%d]"),transpack.nAftBalance);
		LogMessage::instance()->Log(info);
		return;
	}
	if(transpack.cGetProveFlag)
	{
		nRet = ks_card(transpack.sCardPhyID);
		if(nRet)
		{
			wxMessageBox(wxT("重写卡寻卡失败"));
			return;
		}
		nRet = ks_load4card_retry(&transpack);
		if(nRet)
		{
			wxMessageBox(wxT("充值写卡重试失败"));
			return;
		}
		ks_beep();
		wxString info = wxString::Format(wxT("充值成功钱包余额[%d]"),transpack.nAftBalance);
