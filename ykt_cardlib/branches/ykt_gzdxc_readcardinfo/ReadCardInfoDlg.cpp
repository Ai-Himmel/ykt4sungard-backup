...
BOOL CReadCardInfoDlg::OnInitDialog()
{
	...
	GetPrivateProfileString("LOADLIBRARY", "CARD_FLAG", "", card_flag, sizeof(card_flag), iniDllFilePathBuf);
	GetPrivateProfileString("LOADLIBRARY", "RETURN_KEY", "", return_key, sizeof(return_key), iniDllFilePathBuf);
	m_card_flag = atoi(card_flag);
	m_return_key = atoi(return_key);
