...
int CCardFunction::ModifyMoney(int varMoney)
{
	TEST_COM_OPEN()
	int ret = 0;
	int money = 0;
	...
	if(money < varMoney)
	{
		 err_exce.SearchErr(KS_ERR_MONEYNOTENOUGH);
		 return KS_ERR_MONEYNOTENOUGH;
	}
	...
	if (ret) 
	{
		 err_exce.SearchErr(KS_ERR_WRITECARDMONEY); 
