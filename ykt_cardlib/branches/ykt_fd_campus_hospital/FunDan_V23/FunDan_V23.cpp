...
int _stdcall SMT_ReadCardInfo(TPublishCard *pc)
{
	...
	/*
	ret = SMT_FUDAN_ReadCardBlock(g_Port, ucTempBuffer, 0x01, 0x01); 
	if (ret != 0)
	{
		return ret;
	}
	hex2dec((char*)ucTempBuffer, ucBuffer, 32);
	// 图书证号
	memcpy(pc->ucLibraryNo, ucBuffer + 3, 13);
	*/
	return 0;
}
...
// 函数功能: 读客户信息
// 函数参数: CustomerInfo结构体(传入参数很多看结构说明)
// 函数返回: 返回0为成功，否则失败
int _stdcall SMT_ReadCustomerInfo(CustomerInfo *cut_info)
{
	int ret = 0;
	unsigned char ucTempBuffer[128] = "";
	unsigned char ucBuffer[256] = "";						 
	...
	hex2dec((char*)ucTempBuffer, ucBuffer, 32);
	memcpy(cut_info->szStuempNo, ucBuffer, 16);

	ret = SMT_FUDAN_ReadCardBlock(g_Port, ucTempBuffer, 0x02, 0x00);
	if (ret != 0)
	{
		return ret;
	}
	hex2dec((char*)ucTempBuffer, ucBuffer, 32);
	// 持卡人名字
	memcpy(cut_info->szName, ucBuffer, 8);
	
	memcpy(cut_info->szManId, ucBuffer + 12, 4);
	...
	hex2dec((char*)ucTempBuffer, ucBuffer, 32);
	// 证件号码
	memcpy(cut_info->szManId + 4, ucBuffer, 16);
	...
	hex2dec((char*)ucTempBuffer, ucBuffer, 32);
	// 性别
	memcpy(cut_info->szSex, ucBuffer + 14, 1);

	return 0;
}

// 函数功能: 读客户姓名信息
// 函数参数: unsigned char ucName[9]
// 函数返回: 返回0为成功，否则失败
int _stdcall SMT_ReadName(unsigned char ucName[9])
{
	int ret = 0;
	unsigned char ucTempBuffer[128] = "";
	unsigned char ucBuffer[256] = "";						 
...
// 函数功能: 读客户性别信息
// 函数参数: unsigned char ucSex[2]
// 函数返回: 返回0为成功，否则失败
int _stdcall SMT_ReadSex(unsigned char ucSex[2])
{
	int ret = 0;
	unsigned char ucTempBuffer[128] = "";
	unsigned char ucBuffer[256] = "";						 
