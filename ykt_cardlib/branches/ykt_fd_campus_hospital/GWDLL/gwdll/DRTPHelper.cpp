...
void CDRTPHelper::SetNextRequest()
{
	...
	memcpy(&m_requestPack.head.hook,&m_responsePack.head.hook,sizeof m_requestPack.head.hook);
}
...
int CDRTPHelper::GetNextPacketFlag()
{
	return m_nNextPacketFlag;
}

void CDRTPHelper::GlobalHelper(const char* ip, int port, int funcno, int drtp_no)
{
	strncpy(m_serverIP,ip,sizeof(m_serverIP));
	m_serverPort = port;
	m_funcNo = funcno;
