...
public:
	...
	\param ip DRTP 服务器IP
		...
		*/
	CDRTPHelper(const char* ip = "", int port = 0, int funcno = 0, int drtp_no = 0);
	//! destructor
	~CDRTPHelper(void);
	//! 申请Drtp连接
	int Connect(void);
	...
	//! 获取下一次包数据标志
	int GetNextPacketFlag();
	//! 全局构造DRTP对象
	void GlobalHelper(const char* ip,int port,int funcno,int drtp_no);
};
