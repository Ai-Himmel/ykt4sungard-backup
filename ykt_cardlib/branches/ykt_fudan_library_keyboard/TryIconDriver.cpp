// TryIconDriver.cpp: implementation of the CTryIconDriver class.
//
//////////////////////////////////////////////////////////////////////
...
CTryIconDriver::CTryIconDriver(CWnd *pWnd, UINT nCallbackMessage, LPCTSTR szTip, HICON icon, UINT nID)
{
    Create(pWnd,nCallbackMessage,szTip,icon,nID);
    m_bHidden = FALSE;
}
...
LRESULT CTryIconDriver::OnTrayNotify(WPARAM wParam, LPARAM lParam)
{
    ...
    if(LOWORD(lParam) == WM_RBUTTONUP)
    {
        ...
        pSubMenu->ModifyMenu(0,MF_BYPOSITION|MF_STRING,
            (UINT)pSubMenu->GetMenuItemID(0),(IsWindowVisible(m_tnd.hWnd)?"Òþ²Ø´°¿Ú":"ÏÔÊ¾´°¿Ú"));
        CPoint pos;
        GetCursorPos(&pos);
