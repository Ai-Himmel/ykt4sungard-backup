...
BOOL CReadCardInfoDlg::OnInitDialog()
{
	...
	m_key_cast_flag = GetPrivateProfileInt("LOADLIBRARY", "UPCAST", 2, iniDllFilePathBuf);
...
void CReadCardInfoDlg::SimulateKeyPress(char stuemp_no[21])
{
	...
	for (int i = 0; i < length; i++)
	{
		...
		if (keybd != 0xFF || keybd != 0x00)  // 空格不打开
		{
			...
			{
				if (keybd >= 0x61 && keybd <= 0x7a) { keybd -= 0x20; }

				if ((keybd >= 0x41 && keybd <= 0x5a))	
				{ 
					keybd -= 0x20;
					state = GetKeyState(VK_CAPITAL);
					if (state & 0x0F)
...
LONG CReadCardInfoDlg::OnReadCard(WPARAM wParam, LPARAM lParam)
{
	...
	if (0 == lParam) { err_msg.Format("后台查询状态失败--[%d]", wParam); }
	if (1 == lParam) 
	{ 
		err_msg.Format("联网获取卡状态失败, 请使用授权卡操作--[%d]", wParam); 
		m_btnReadAuthorCard.EnableWindow(TRUE);
		m_btnRecPassword.EnableWindow(TRUE);
		m_btnReadAuthorCard.EnableWindow(TRUE);
		m_btnTest.EnableWindow(TRUE);
		m_btnPause.EnableWindow(TRUE);
		m_Status.SetWindowText("网络故障");
		ShowWindow(SW_SHOWNORMAL);
	}

	if (2 == lParam) { err_msg.Format("此卡为非注册卡--[%d]", wParam); }
	if (3 == lParam) { err_msg.Format("此卡为挂失卡--[%d]", wParam); }
	if (4 == lParam) { err_msg.Format("此卡为冻结卡--[%d]", wParam); }

	if (NULL == dlgMsg->m_hWnd) { dlgMsg->Create(IDD_DIALOG_MESSAGE, NULL); }
	dlgMsg->ShowWindow(SW_SHOW);
	dlgMsg->UpdateWindow();
	CEdit * txtMsg = (CEdit*)(dlgMsg->GetDlgItem(IDC_STATIC_MSG));
	txtMsg->SetWindowText(err_msg);
