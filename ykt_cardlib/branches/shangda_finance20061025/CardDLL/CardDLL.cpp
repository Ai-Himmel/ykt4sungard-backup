...
// 网络通讯层的函数(来自于GwDll.dll)
HINSTANCE hGwDLL = NULL;
typedef BOOL (CALLBACK *LPKInitFunc)();
typedef INT  (CALLBACK *LPKRecPwdFromSvr)();
typedef INT  (CALLBACK *LPKRecCardStateId)(const char phyid[9]);
LPKInitFunc	KInitFunc = NULL;
LPKRecPwdFromSvr  KRecPwdFromSvr = NULL;
LPKRecCardStateId KRecCardStateId = NULL;
...
{
	int ret = 0;
	if(hDLL == NULL && hDesDLL == NULL && hGwDLL == NULL)
	{
		ret = Load_Dll();
		if(ret)
			return -1;
...
// 加载GwDll.dll库
	...
	hGwDLL = LoadLibrary("GwDll.dll");
	if (!hGwDLL)
	{
		SetErrMsg(-1,"装载GwDll.dll动态库失败");
		return -1;	
	}
	KInitFunc = (LPKInitFunc)GetProcAddress(hGwDLL, "KInitFunc");
	KRecPwdFromSvr	= (LPKRecPwdFromSvr)GetProcAddress(hGwDLL, "KRecPwdFromSvr");
	KRecCardStateId = (LPKRecCardStateId)GetProcAddress(hGwDLL, "KRecCardStateId");
	if (
		(!KInitFunc) ||
		(!KRecPwdFromSvr) ||
		(!KRecCardStateId)
...
// 函数作用: 验证个人密码
// 函数参数: 个人密码(3位)
int __stdcall SMT_VerifyPersonalPassword(unsigned char PersonalPsd[3])
{
	int ret = -1;
	unsigned char ucBuffer[16] = "";
	unsigned char ucPersonalPsdFromCard[3] = "";
	...
	if(ret != 0)	
		return -5;
	memcpy(ucPersonalPsdFromCard, ucBuffer, 3);
	if (0 == _mbsncmp(ucPersonalPsdFromCard, PersonalPsd, 3))
	{
		ret = ReadSerial(ucPhyId);
		if (ret)
		{
			return -2;				// 读取物理卡号错误
		}
		dec2hex(ucPhyId, 4, cPhyId);
		ret = KRecCardStateId(cPhyId);
		if (10 == ret || 11 == ret)
		{
			return ret;
		}
