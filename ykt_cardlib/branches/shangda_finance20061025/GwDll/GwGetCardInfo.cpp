...
/*调用CardDll动态库Load_WorkKey函数*/
#define KE_HANDLENOEXIST_ERR	-2
#define KE_FUNNOEXIST_ERR		-3
#define KE_GETLOSSSTATE_RET	10
#define KE_NORMALSTATE_RET		11

HINSTANCE hCardDll = NULL;
typedef INT  (CALLBACK* LPLoad_WorkKey)(char *ascWorkKey);
...
int __stdcall KRecCardStateId(const char phyid[9])
{
	char cStateId[5] = "";
	gw_init_errcode();
	// 读取本地配置
	...
	CDRTPHelper drtp(gs_svr_info.drtp_ip,gs_svr_info.drtp_port,gs_svr_info.drtp_mainfunc);
	ST_PACK *data;
	drtp.SetRequestHeader(847173);
//	drtp.AddField(F_SCUST_LIMIT, StaticSendPwd);
//	drtp.AddField(F_SPHONE, gs_svr_info.ip);
//	drtp.AddField(F_SPHONE2, gs_svr_info.mac);
	drtp.AddField(F_SBANK_ACC, phyid);
//	drtp.AddField(F_SBANK_ACC, "96388890");	
//	drtp.AddField(F_LCERT_CODE, 0);
//	drtp.AddField(F_LVOL0, 2006);
	if(drtp.Connect())
	{
	...
	{
		data = drtp.GetNextRecord();
		STR_COPY(cStateId, data->sbank_code);
		if ((strncmp(cStateId, "1000", 4) != 0 && strncmp(cStateId, "1001", 4) != 0))
		{
			return KE_GETLOSSSTATE_RET;
		}
		return KE_NORMALSTATE_RET;
	}
	else
	{
		gs_errno = KE_QUERY_ERROR;
...
int KLoad_Dll()
{
	...
	if(!hCardDll)
	{
		return KE_HANDLENOEXIST_ERR;										
	}
	Load_WorkKey = (LPLoad_WorkKey)GetProcAddress(hCardDll, "Load_WorkKey"); 
	if (!Load_WorkKey)
	{
		FreeLibrary(hCardDll);
		hCardDll = NULL;
		return KE_FUNNOEXIST_ERR;
	}
	return 0;
}
