...
/************************************************************************************/
/*修正介绍: 重新修改了对硬件和卡操作的封装(DESDLL-算法, SMARTCOM411DXQ-对硬件操作封装,                         
/*          CardDLL-对卡操作的封装                
/*日期:     2006-7-14
/*修改者：  李翔
/************************************************************************************/
...
typedef INT (CALLBACK* LPSMT_ReadWaterPackInfo)(WATER_PACK_INFO* info);
typedef INT (CALLBACK* LPSMT_InitWaterInfo)(const WATER_PACK_INFO* info);

LPRequestCard			RequestCard=NULL;
LPConnectMF				ConnectMF=NULL;
LPCloseMF				CloseMF=NULL;
LPReaderAuthor			ReaderAuthor=NULL;
LPSMT_ReadWaterPackInfo SMT_ReadWaterPackInfo = NULL;
LPSMT_InitWaterInfo     SMT_InitWaterInfo = NULL;
...
//static BYTE CARD_INIT_KEY[] = {0xb0, 0xb1, 0xb2, 0xb3, 0xb4, 0xb5};

//static BYTE CARD_INIT_KEY_NEW[] = {0xa0, 0xa1, 0xa2, 0xa3, 0xa4, 0xa5};
...
{	
	...
	hSmartComDLL = LoadLibrary(bufSmartCom);
	if(!hSmartComDLL)
	{
...
static int LoadCardDLL()
{
	...
	SMT_CreateGreyCard         = (LPSMT_CreateGreyCard)GetProcAddress(hCardDLL,"SMT_CreateGreyCard");
	SMT_WriteCardStateInfo     = (LPSMT_WriteCardStateInfo)GetProcAddress(hCardDLL,"SMT_WriteCardStateInfo");
	SMT_PacketAddMomey         = (LPSMT_PacketAddMomey)GetProcAddress(hCardDLL,"SMT_PacketAddMomey");
	SMT_PacketSetMoney         = (LPSMT_PacketSetMoney)GetProcAddress(hCardDLL,"SMT_PacketSetMoney");
	SMT_ReadCardStateInfo  	   = (LPSMT_ReadCardStateInfo)GetProcAddress(hCardDLL,"SMT_ReadCardStateInfo");
	SMT_WritePersonalInfo  	   = (LPSMT_WritePersonalInfo)GetProcAddress(hCardDLL,"SMT_WritePersonalInfo");
    SMT_ReadPersonalInfo       = (LPSMT_ReadPersonalInfo)GetProcAddress(hCardDLL,"SMT_ReadPersonalInfo");
    SMT_ChangeName             = (LPSMT_ChangeName)GetProcAddress(hCardDLL,"SMT_ChangeName");
    SMT_ChangeDutyNo           = (LPSMT_ChangeDutyNo)GetProcAddress(hCardDLL,"SMT_ChangeDutyNo");
    SMT_ChangeCerificateNo     = (LPSMT_ChangeCerificateNo)GetProcAddress(hCardDLL,"SMT_ChangeCerificateNo");
    SMT_ChangeDepartmentNo     = (LPSMT_ChangeDepartmentNo)GetProcAddress(hCardDLL,"SMT_ChangeDepartmentNo");
    SMT_ChangeIdentifyNo       = (LPSMT_ChangeIdentifyNo)GetProcAddress(hCardDLL,"SMT_ChangeIdentifyNo");
    SMT_ChangeSexNo            = (LPSMT_ChangeSexNo)GetProcAddress(hCardDLL,"SMT_ChangeSexNo");
    SMT_ReadPacketInfo         = (LPSMT_ReadPacketInfo)GetProcAddress(hCardDLL,"SMT_ReadPacketInfo");
    SMT_ChangeDeadLineDate     = (LPSMT_ChangeDeadLineDate)GetProcAddress(hCardDLL,"SMT_ChangeDeadLineDate");
    SMT_ChangeCardRightType    = (LPSMT_ChangeCardRightType)GetProcAddress(hCardDLL,"SMT_ChangeCardRightType");
    SMT_WritePersonalPassword  = (LPSMT_ChangeDeadLineDate)GetProcAddress(hCardDLL,"SMT_WritePersonalPassword");
    SMT_ReadPersonalPassword   = (LPSMT_ChangeCardRightType)GetProcAddress(hCardDLL,"SMT_ReadPersonalPassword");
	SMT_ClearCard			   = (LPSMT_ClearCard)GetProcAddress(hCardDLL,"SMT_ClearCard");
	SMT_WriteLibraryNo		   = (LPSMT_WriteLibraryNo)GetProcAddress(hCardDLL,"SMT_WriteLibraryNo");
	SMT_ReadLibraryNo		   = (LPSMT_ReadLibraryNo)GetProcAddress(hCardDLL,"SMT_ReadLibraryNo");
	SMT_ChangeCardNo		   = (LPSMT_ChangeCardNo)GetProcAddress(hCardDLL,"SMT_ChangeCardNo");
	SMT_ChangeShowCardNo	   = (LPSMT_ChangeShowCardNo)GetProcAddress(hCardDLL,"SMT_ChangeShowCardNo");
	SMT_ReadAuthCard		   = (LPSMT_ReadAuthCard)GetProcAddress(hCardDLL,"SMT_ReadAuthCard");
	SMT_ReadAuthCard2		   = (LPSMT_ReadAuthCard2)GetProcAddress(hCardDLL,"SMT_ReadAuthCard2");	
	//////////////////////////////////////////////////////////////////////////
	SMT_AutoDll				   = (LPSMT_AutoDll)GetProcAddress(hCardDLL, "SMT_AutoDll");
	SMT_JudgeAutoFlag  	       = (LPSMT_JudgeAutoFlag)GetProcAddress(hCardDLL, "SMT_JudgeAutoFlag");
	SMT_ReadWaterPackInfo      = (LPSMT_ReadWaterPackInfo)GetProcAddress(hCardDLL, "SMT_ReadWaterPackInfo");
	SMT_InitWaterInfo          = (LPSMT_InitWaterInfo)GetProcAddress(hCardDLL, "SMT_InitWaterInfo");
	if(
		(!RequestCard) ||
		(!ConnectMF)||
		(!CloseMF) ||
...
static int WriteCardBlock(int *err_code,PWriteDataFunc pFunc,LPProcessCallBack lpCallBack)
{
	...
	BYTE card_key_a[7] = "";
	BYTE card_key_b[7] = "";
//	BYTE card_key_c[] = {0x56, 0xDD, 0xA9, 0x59, 0xDD, 0xF0};				// 重新添加的部分
	int current_key, old_key;
	BOOL login_sect_succ = FALSE;
	
	CardKeyData card_key[3];				// 到时候3变4
...
//	current_key = 1;		// 修改过, 用完了还得返回去, 用keyA登陆 
	for(sect = 0;sect < SMARD_CARD_SECT_NO; ++sect)
	{
		...
		while(TRUE)
		{
			...
			current_key--;
			if( current_key < 0 )
	//			current_key = sizeof(card_key) / sizeof(card_key[0]) - 1;	    // 动态设置
				current_key = 2;
...
//	BYTE card_key_c[] = {0x56,0xDD, 0xA9, 0x59, 0xDD, 0xF0};
	...
	card_key[0].flag = CARD_KEY_A;
	// 这里可以读出回收后的KEYA
	memcpy(card_key[0].key,CARD_INIT_KEY,sizeof(card_key[0].key));
...
//	memcpy(card_key[3].key, card_key_c, sizeof(card_key[3].key));
	...
	for(sect = startSect;sect < endSect + 1; ++sect)
	{
		...
		while(TRUE)
		{
			...
			current_key++;
			if( current_key > 2 )					//	修改过
		//	if (current_key > (sizeof(card_key) / sizeof(card_key[0]) - 1))	
				current_key = 0;
			if (current_key == old_key)
				break;
		}
