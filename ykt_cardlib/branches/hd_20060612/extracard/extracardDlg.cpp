...
int LoadSmartCom411DXQ()
{
	...
	LOAD_DLLFUNC(h_CardDLL_Handler,SMT_ChangeShowCardNo);
	LOAD_DLLFUNC(h_CardDLL_Handler,ReadCardNO);
	return 0;
}
...
BEGIN_MESSAGE_MAP(CExtracardDlg, CDialog)
	...
	ON_BN_CLICKED(ID_BTN_INITCLEARSHOWNO, OnBtnInitclearshowno)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CExtracardDlg::OnBtnReadcard() 
{
	...
	// 读交易卡号
	ReadCardNO(nChangeNo);
	temp.Format("%d", nChangeNo[0]);
	GetDlgItem(ID_EDT_CHANGE_NO)->SetWindowText(temp);
	...
	if (water.cardtype == WATER_FIXED_CARD)
	{
		temp.Format("%d", water.termid);
		GetDlgItem(ID_EDT_WATER_NO)->SetWindowText(temp);
	}
}
...
void CExtracardDlg::OnBtnInitclearshowno() 
{
	// TODO: Add your control notification handler code here
	TEST_COM_OPEN();
	unsigned char ucShowCardNo[5] = "";
	if (SMT_ChangeShowCardNo(ucShowCardNo))
	{
		AfxMessageBox("清零显示卡号失败");
		return;
	}
	SMT_ControlBuzzer();
}
