...
protected:
	...
	afx_msg void OnBtnSetmoney();
	afx_msg void OnBtnSetcardno();
	afx_msg void OnBtnReadcardno();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};
...
DCLR_DLLFUNC(int,SMT_Login_With_UserKey,(int Sector,unsigned char *Buffer,int PaDSelect));
DCLR_DLLFUNC(int,MF_ReadSeriesNo,(unsigned char *Buffer));
...
DCLR_DLLFUNC(int,SMT_ReadCardNoFor14,(unsigned char ucCardNo[20], unsigned char ucCardReadKey[6]));
DCLR_DLLFUNC(int,SMT_WriteCardNoFor14,(unsigned char ucCardNo[20], unsigned char ucCardWriteKey[6]));
DCLR_DLLFUNC(int,ConnectMF,(int com,int baud));
DCLR_DLLFUNC(int,CloseMF,(void));
