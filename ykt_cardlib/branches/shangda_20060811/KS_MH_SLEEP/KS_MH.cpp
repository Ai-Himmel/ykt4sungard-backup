...
static struct t_CardStruct tCardStct = {0, 0};
...
lp_rf_authentication_2	rf_authentication_2=NULL;
...
int _stdcall SMT_SelectMode(int Select_Mode)
{
	...
	if (Select_Mode == SELECT_IDEL_MODE)
	{
		...
		{
			ret = rf_request(icdev, SELECT_IDEL_MODE, &tagtype);	
			if(ret)
			{
				SetLastErrMsg(ret, "读卡器感应区没有卡");
				return -2;
			...
			ret = rf_select(icdev, g_ucSerialNo, &g_size);
			if(ret)
			{
	...
	{
		ret = rf_request(icdev, SELECT_ALL_MODE, &tagtype);	
		if(ret)
		{
			SetLastErrMsg(ret, "读卡器感应区没有卡");
			return -2;
...
int _stdcall MF_ReadSeriesNo(unsigned char *serial)
{
	...
	char bufCardId[10] = "";
	ret = SMT_SelectMode(tCardStct.request_mode);
	if (ret)
	{
		return ret;
	}
	...
	*/
	// 计算字符串接受序列号
	memcpy(serial, (unsigned char*)&g_ucSerialNo, sizeof g_ucSerialNo);
	/*
...
int SMT_LoadDll()
{
	...
	if (!hDLL)
	{
		::AfxMessageBox("动态库mwrf32.dll加载失败!");
		return -1;
	}
	...
	rf_authentication_2 = (lp_rf_authentication_2)GetProcAddress(hDLL, "rf_authentication_2");
	if ((!rf_init) ||
		(!rf_card) ||
		(!rf_load_key) ||
		(!rf_authentication) ||
...
//	ret= rf_anticoll(icdev, 0, &serialNo);
	...
	*/
	if (_sector > 15)
	{
		ret = rf_load_key(icdev, _mode, _sector - 16, Buffer);	
	}
	else
	{
		ret = rf_load_key(icdev, _mode, _sector, Buffer);       // 多半PaDselect传入0
	}
	if (ret != 0)
	{
	...
	if (_sector >=0 && _sector <= 15)
	{
		ret = rf_authentication(icdev, _mode, _sector);
	}
	else
	{
		ret = rf_authentication_2(icdev, _mode, _sector - 16, _sector * 4);
	}
	if (ret != 0)
	{
