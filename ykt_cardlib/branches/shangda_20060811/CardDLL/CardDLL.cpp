...
// 出厂时的控制位
static BYTE CARD_INIT_CTRL_BIT[] = {0xFF,0x07,0x80,0x69};   // KEYA和KEYB具有相同权限(读写)
// 上大14扇区控制位
...
{
	int ret = 0; 
	unsigned char ucCardWriteKey[] = {0x00, 0x00, 0x00, 0x07, 0x01, 0x08};
...
//		return ret;		
	...
	ret = SMT_WriteCardNoFor14(pc->ucCardNo, ucCardWriteKey);
	if (ret)
	{
		SetErrMsg(ret, "写14扇区学工号失败");
//		return ret;
		return -1007;
	}
	SMT_ControlBuzzer();
...
/*	memset(pc,0,sizeof(TPublishCard));*/	// 交给外面来清零
...
// 		CTime tm = CTime::GetCurrentTime();
		...
		for(i=0;i<15;i++)	
			ucBuf1[15] += ucBuf1[i];
		SMT_WriteBlock(sector,0,ucBuf1);
		SMT_ReadBlock(sector,0,read_buf);
		if(memcmp(read_buf2,read_buf,16) == 0)
			return -8;
...
//		CTime tm = CTime::GetCurrentTime();
		...
		for(i=0;i<15;i++)	
			ucBuf2[15] += ucBuf2[i];
		SMT_WriteBlock(sector,0,ucBuf2);
		SMT_ReadBlock(sector,0,read_buf);
		if(memcmp(read_buf2,read_buf,16) == 0)
			return -12;
...
//	current_key = 1;					 // 修改过, 用完了还得返回去, 用keyA登陆 
	for(sect = 0;sect < SMARD_CARD_SECT_NO; ++sect)
	{
		...
		while(TRUE)
		{
			// 如果是14扇区, 同时写入新的只读密钥和新的可写密钥
			/*
			if (14 == sect)
			{	// 回收以后用FFFFFF登陆
				if (0 == LoginCardSect(sect, card_key[current_key].key, card_key[current_key].flag, card_type))
				{
					login_sect_succ = TRUE;
				}
				// 重复初始化
				else if (0 == LoginCardSect(sect, card_key[5].key, card_key[5].flag, card_type))
				{
					login_sect_succ = TRUE;
				}
				// 登陆不上报错
		...
		if( pFunc != NULL )
		{
			...
			if (14 == sect)
			{
				if ((ret = pFunc(sect, &card_key[4], &card_key[5], card_type, &card_key[current_key])) != 0)
				{
	...
	for(sect = 0;sect < AUTH_CARD_SECT_NO;++sect)
	{
		...
		if(LoginCardSect(sect, CARD_INIT_KEY, CARD_KEY_A, card_type) != 0)
		{
			...
			{	// 产生一个补救的机会, 如果7扇区或者15扇区坏掉了卡就废掉了, 补写7扇区和15扇区
				if (sect == pri_sect)
				{
					if (SMT_LoginWorkKeySector(NULL, NULL, NULL))
					{
						ret = 10;		// 产生补救的机会 
					}
					else
...
int __stdcall ReadAuthCardBlock(int startSect, int endSect, short block, LPGetBlockData lpFunc)
{
	...
	for(sect = startSect; sect < endSect + 1; ++sect)
	{
		...
		while(TRUE)
		{
			...
			if (current_key == old_key)
			{	// 循环密钥登陆不上, 看看是不是密钥扇区
				if (SMT_LoginWorkKeySector(NULL, NULL, NULL) != 0)
				{
					login_sect_succ = FALSE;
				}
				else
...
int SMT_LoginWorkKeySector(BYTE pri_key_a[6], BYTE pri_key_b[6], short *pri_sect)
{
	BYTE card_id[5] = "";
	BYTE card_type[2] = "";
	BYTE card_key[7] = "";
	...
	// 测试用
	/*
	int ret = SMT_Login_With_UserKey(priSect, priKeyA, 0);                     
	if(ret != 0)
	{
		return -2;	// 卡片登陆失败
	}
	if(SMT_ReadBlock(priSect, 3, buf) != 0)
	{
		return -5;
	}
	*/
	/*
	int ret = SMT_Login_With_UserKey(priSect, priKeyB, 1);                     
	if(ret != 0)
	{
		return -2;	// 卡片登陆失败
	}
	memset(buf, 0, sizeof(buf));
	if(SMT_ReadBlock(priSect, 2, buf) != 0)
	{
		return -5;
	}
	*/
...
int __stdcall SMT_ReadAuthorWorkKeyAndFlag(BYTE auth_work_key[8], BYTE *flag)
{
	...
	if (ret = SMT_LoginWorkKeySector(priKeyA, priKeyB, &priSect))
		return ret;
...
int __stdcall SMT_ModifyDeadLineDateAndFlag(BYTE DeadLineDate[8], BYTE flag)
{
	...
	*(buf + 4 + sizeof(BYTE) * 8) = flag;
	if (SMT_WriteBlock(priSect, 0, buf) != 0)
		return -11;
