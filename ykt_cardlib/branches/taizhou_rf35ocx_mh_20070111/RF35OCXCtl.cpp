...
int CRF35OCXCtrl::OpenCOM()
{
	if(!m_comopen)
	{
		int ret = rf_init(m_nPortNo,57600L);
...
short CRF35OCXCtrl::UndoLoss(LPCTSTR mkey,long cardno,LPCTSTR blkver) 
{
	...
	ret=rf_load_key(m_icdev,0,3,ucKeyA);
	if(ret != 0)
	{
	...
	ret=rf_authentication(m_icdev,0,3);	
	if(ret != 0)
	{
		AfxMessageBox(_T("验证密码失败"));
		return -4;
	}
	ret=rf_read(m_icdev,14,data);
	if(ret != 0)
	{
...
short CRF35OCXCtrl::ChangePassWord(LPCTSTR mkey,LPCTSTR newpwd,long cardno) 
{
	...
	ret=rf_load_key(m_icdev,0,1,ucKeyA);
	if(ret != 0)
	{
		AfxMessageBox(_T("装载密码失败"));
		return -3;
	...
	ret=rf_authentication(m_icdev,0,1);	
	if(ret != 0)
	{
		AfxMessageBox(_T("验证密码失败"));
		return -4;
	}
	ret=rf_read(m_icdev,5,data);
	if(ret != 0)
	{
		AfxMessageBox(_T("读卡信息失败"));
		return -5;
...
int CRF35OCXCtrl::ReadCardSNR(unsigned long * snr)
{
	...
	// 重新放卡
	rf_reset(m_icdev,10);
	ret = rf_request(m_icdev,1,&cardmode);
	if(ret != 0)
	{
		AfxMessageBox(_T("读卡序列号失败,请移走卡后重试一次!"));
		return -2;
	}
	ret= rf_anticoll(m_icdev,0,&cardid);
	if(ret)
	{
		AfxMessageBox(_T("读卡器感应区没有卡"));
		return ret;
	}
	ret = rf_select(m_icdev,cardid,&size);
	if(ret)
	{
