...
#define LOAD_DLLFUNC_NAME(handle,p,name,ret) do {\
	p = (LP##p)GetProcAddress(handle,#name); \
	if( NULL == p) {\
	DEBUG_CARD_MSGOUT(100,"Load funcation failed:"#name); \
	return ret; \
	}}while(0)

#define LOAD_DLLFUNC_EASY(handle,name) LOAD_DLLFUNC_NAME(handle,name,name,-1)

#ifdef NOUSE_MFC

#define LOAD_DLLFUNC(handle,name) LOAD_DLLFUNC_NAME(handle,name,name,-1)
#else
