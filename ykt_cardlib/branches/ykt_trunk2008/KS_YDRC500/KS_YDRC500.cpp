...
DEF_DLLFUNC(unsigned char, RC500_232_anticoll2,(unsigned char encoll,unsigned char
											bcnt,unsigned long &snr));
...
DCLR_DLLFUNC(RC500_232_anticoll2);
...
static int do_load_function()
{
	...
	LOAD_DLLFUNC(hDLL,RC500_232_anticoll2);
	return 0;
}
...
int _stdcall MF_ReadSeriesNo(unsigned char *serial)
{
	...
	ret= RC500_232_anticoll2(0,0,cardid);                 // 获取卡的序列号(第三个参数cardid), request指令之后应立即调用anticoll，除非卡的序列号已知, 成功返回为0
	if(ret)
	{
		return ret;
	}
	ret = RC500_232_select(cardid, size);
	if(ret)
	{
		SetLastErrMsg(ret, "选卡失败!");
		return ret;
	}
	pid = (unsigned char *)&cardid;
	serial[0] = pid[3];
	serial[1] = pid[2];
	serial[2] = pid[1];
	serial[3] = pid[0];
...
int _stdcall SMT_RequestCardExist(unsigned char ucSerialNo[4],unsigned char ucType[1])
{
	...
	unsigned short cardmode = 2; 

	//RC500_232_halt();
	/*
	ret = RC500_232_request(0,cardmode);               // 1表示ALL模式, 一次可以对多张卡的操作, 成功返回为0
	if(ret != 0)
	{
		return -2;
	}
