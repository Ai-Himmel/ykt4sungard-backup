...
// 修改后台841700
int __stdcall GW_ReadCardInfo(char physical_no[9],TPublishCard *pc)
{
	int ret = 0;
	if(strlen(physical_no) != 8)
		return GW_ERR_INPUTPHYNOFAILED;
	
	if (g_handle_flag)
	{
		if (ret = F841700PublishCard(g_gw_handle, physical_no, pc))
			return ret;
	}
	else 
		return GW_ERR_HANDLENOTCREATE;
...
int F841700PublishCard(KBDCHandle handle, char physical_no[9],TPublishCard *pc)
{
	int ret = 0;
	...
	do 
	{
		if ((ret = KBDCGetRetCode(g_gw_handle)) == 0)
		{
			...
			memcpy(pc->ShowCardNo, fvalue, sizeof(pc->ShowCardNo) - 1);				// 显示卡号
			KBDCGetFieldValue(g_gw_handle, "lvol0", fvalue, g_msg);
			memcpy(pc->CardNo, fvalue, sizeof(pc->CardNo) - 1);						// 交易卡号
			KBDCGetFieldValue(g_gw_handle, "sdate1", fvalue, g_msg);
			memcpy(pc->DeadLineDate, fvalue, sizeof(pc->DeadLineDate) - 1);			// 截止日期
			KBDCGetFieldValue(g_gw_handle, "lvol3", fvalue, g_msg);					
			pc->CardRightType = atoi(fvalue);										// 用户卡使用权限类型（1--254）
			KBDCGetFieldValue(g_gw_handle, "snote", fvalue, g_msg);					
			memcpy(pc->ucName, fvalue, sizeof(pc->ucName) - 1);						// 姓名
			KBDCGetFieldValue(g_gw_handle, "scust_auth", fvalue, g_msg);					
			memcpy(pc->ucCertificateNo, fvalue, sizeof(pc->ucCertificateNo) - 1);	// 证件号码
			KBDCGetFieldValue(g_gw_handle, "sorder0", fvalue, g_msg);					
			memcpy(pc->ucDepartmentNo, fvalue, sizeof(pc->ucDepartmentNo) - 1);		// 部门编号	
			KBDCGetFieldValue(g_gw_handle, "sstatus0", fvalue, g_msg);					
			memcpy(pc->ucSexNo, fvalue, sizeof(pc->ucSexNo) - 1);					// 性别
			KBDCGetFieldValue(g_gw_handle, "sname2", fvalue, g_msg);					
			memcpy(pc->ucCardNo, fvalue, sizeof(pc->ucCardNo) - 1);					//工号
			KBDCGetFieldValue(g_gw_handle, "scust_auth2", fvalue, g_msg);					
			memcpy(pc->ucLibraryNo, fvalue, sizeof(pc->ucLibraryNo) - 1);			//图书证号
		}
		else
		{
			KBDCGetFieldValue(g_gw_handle, "vsmess", fvalue, g_msg);
			strcpy(g_msg,fvalue);
			return ret;
		}
