...
int LoadCardManDLL()
{
	...
	hCardManDLL = LoadLibrary("KS_ThirdInterface.dll");
	if(!hCardManDLL)
	{
		return KS_ERR_LOADLIBRARY;
	}
	ConnectMF			= (LPConnectMF)GetProcAddress(hCardManDLL, "SMT_ConnectMF280");
	ReadCardInfo		= (LPReadCardInfo)GetProcAddress(hCardManDLL, "SMT_ReadCardInfo");
	GetLastErrMsg		= (LPGetLastErrMsg)GetProcAddress(hCardManDLL, "GetLastErrMsg");
	CloseMF				= (LPCloseMF)GetProcAddress(hCardManDLL, "SMT_CloseMF280");
	...
	ReadBalance			= (LPReadBalance)GetProcAddress(hCardManDLL, "SMT_ReadBalance");
	//SMT_ReadCardMainKey = (LPSMT_ReadCardMainKey)GetProcAddress(hCardManDLL,"SMT_ReadCardMainKey");
	SMT_AuthDevice = (LPSMT_AuthDevice)GetProcAddress(hCardManDLL,"SMT_AuthDevice");
	SMT_ControlBuzzer	= (LPSMT_ControlBuzzer)GetProcAddress(hCardManDLL, "SMT_ControlBuzzer");
	SMT_SleepCard		= (LPSMT_SleepCard)GetProcAddress(hCardManDLL, "SMT_SleepCard");
	/*
	...
	*/
 	...
 	if(
		...
		|| !SMT_PacketDelMomey || !ReadBalance || !SMT_AuthDevice || !SMT_ControlBuzzer
		|| !SMT_SleepCard
		)
	{
