...
#define CARDDLL_VERSION "1.4.003-nanjing"
...
typedef int (CALLBACK	*LPGW_ReadCardInfo)(char physical_no[9],TPublishCard *pc);
...
LPGW_ReadCardInfo		GW_ReadCardInfo = NULL;
// POS机硬件(统一封装)
...
typedef int (CALLBACK *LPPOS_GetCardID)(char *card_id,int *bala,int len);
typedef int (CALLBACK *LPPOS_GetCardNumAndMoney)(char *strCardNum, int *curBalance, int Len);
...
LPPOS_GetCardID			POS_GetCardID = NULL;
LPPOS_GetCardNumAndMoney POS_GetCardNumAndMoney =NULL;
// 计算KEYA
...
static int Login_Card(int sector)
{
	...
	else
	{
		...
		//dec2hex(ucKeyA,6,key_str);
		//sprintf(msg,"key[%s]",key_str);
...
// 新发卡时，用这个函数写卡金额
static int do_newcard_set_package(int nPacketNo,int nMoney)
{
	int ret,sector,i;
	...
	if(SMT_ReadBlock(sector,0,read_buf))
	{
		// 读卡时也失败了，返回 -8
		if(ret == 0)
			return 0;
	...
	{
		// 认为已经读取到数据，判断是不是写上了
		if(memcmp(ucBuf1,read_buf,16)==0)
		{
			return 0;
		}
	}
...
static void do_calc_showcardno(char * showcardno,unsigned char * bcdshowcard)
{
	int card_len = strlen(showcardno);
	if(card_len > 10)
		card_len = 10;
	memset(bcdshowcard,0xFF,5);
...
int __stdcall PublishCard(TPublishCard *pc)
{
	...
	//sprintf(msg,"showcardno[%s]",(char*)pc->ShowCardNo);
	//DEBUG_CARD_MSGOUT(100,msg);
	do_calc_showcardno((char*)pc->ShowCardNo,ucBcdShowCardNo);
	...
	int Money;
	Money = pc->Money;
	ret = do_newcard_set_package(PacketNo,Money);
	if(ret)
	{
		SetErrMsg(ret,"设置钱包余额失败");
		return ret;		
	}
	ret = do_newcard_set_package(2,0);
	if(ret)
	{
		SetErrMsg(ret,"设置钱包余额失败");
		return ret;		
	}
	ret = do_newcard_set_package(3,0);
	if(ret)
	{
		SetErrMsg(ret,"设置钱包余额失败");
		return ret;		
	}
	ret = do_newcard_set_package(4,0);
	if(ret)
	{
		SetErrMsg(ret,"设置钱包余额失败");
		return ret;		
...
#ifdef NJU_PACK1
#define IF_COMP(x,y,n) (strncmp((const char*)(x),(const char*)(y),n)==0)
#endif 
int __stdcall ReadCardInfo(TPublishCard *pc)
{
	...
	/*
	20090501 过期卡
	1.      以学号MG07，JM07,GM07,ZY07，MBA07,MFA07,MPA07 开头， 改成20100601
	2.      以学号JM06,GM06,ZY06,MBA06,MFA06,MPA06开头，改为20090601 
	20090701 过期卡
	1.    以学号0511开头的地理与科学学院（单位代码1116）的 改为20100801
	2.    以学号0611开头的地理与科学学院（单位代码1116）的 改为20110801
	3.    以学号0711开头的地理与科学学院（单位代码1116）的 改为20120801
	4.    以学号0811开头的地理与科学学院（单位代码1116）的 改为20130801
	5.    以学号0510开头的新闻传播学院（单位代码1187）的 改为20100801
	6.    以学号0710开头的新闻传播学院（单位代码1187）的 改为20120801
	7.    以学号0452，0412开头的医学院（单位代码1156）本科生-统招（权限代码10）改为20110801
	8.    以学号0552，0512开头的医学院（单位代码1156）本科生-统招（权限代码10）改为20120801
	9.    以学号0652，0612开头的医学院（单位代码1156）本科生-统招（权限代码10）改为20130801
	10.  以学号0752，0712开头的医学院（单位代码1156）本科生-统招（权限代码10）改为20140801
	11.  以学号0852，0812开头的医学院（单位代码1156）本科生-统招（权限代码10）改为20150801
	12.  以学号DG08开头的博士研究生（权限代码15）改为20110601
	13.  以学号DG07开头的博士研究生（权限代码15）改为20100601
	*/
	DEBUG_CARD_MSGOUT(1,"检查卡有效期");
	if(IF_COMP(pc->DeadLineDate,"20090501",8))
	{
		{
			unsigned char new_deadline[3] = {0x0A,0x06,0x01};
			char student_no[][6] = {"JM06","GM06","ZY06","MBA06","MFA06","MPA06"};
			for (i = 0;i < sizeof(student_no)/sizeof(student_no[0]);++i)
			{
				if(IF_COMP(student_no[i],pc->ucCardNo,strlen(student_no[i])))
				{
					// 更新有效期
					SMT_ChangeDeadLineDate(new_deadline);
					goto L_END;
				}
			}
		}
		{
			unsigned char new_deadline[3] = {0x09,0x06,0x01};
			char student_no[][6] = {"MG07","JM07","GM07","ZY07","MBA07","MFA07","MPA07"};
			for (i = 0;i < sizeof(student_no)/sizeof(student_no[0]);++i)
			{
				if(IF_COMP(student_no[i],pc->ucCardNo,strlen(student_no[i])))
				{
	...
	else if(IF_COMP(pc->DeadLineDate,"20090701",8))
	{
		if(IF_COMP("0511",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1116",4))
		{
			unsigned char new_deadline[3] = {0x0A,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
			goto L_END;
		}
		else if(IF_COMP("0611",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1116",4))
		{
			unsigned char new_deadline[3] = {0x0B,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
			goto L_END;
		}
		else if(IF_COMP("0711",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1116",4))
		{
			unsigned char new_deadline[3] = {0x0C,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
			goto L_END;
		}
		else if(IF_COMP("0811",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1116",4))
		{
			unsigned char new_deadline[3] = {0x0D,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
			goto L_END;
		}
		else if(IF_COMP("0510",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1187",4))
		{
			unsigned char new_deadline[3] = {0x0A,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
			goto L_END;
		}
		else if(IF_COMP("0710",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1187",4))
		{
			unsigned char new_deadline[3] = {0x0C,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
			goto L_END;
		}
		else if(IF_COMP("0452",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1156",4)
			&& pc->CardRightType == 10)
		{
			unsigned char new_deadline[3] = {0x0B,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
		...
		else if(IF_COMP("0412",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1156",4)
			&& pc->CardRightType == 10)
		{
			unsigned char new_deadline[3] = {0x0B,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
		...
		else if(IF_COMP("0552",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1156",4)
			&& pc->CardRightType == 10)
		{
			unsigned char new_deadline[3] = {0x0C,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
		...
		else if(IF_COMP("0512",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1156",4)
			&& pc->CardRightType == 10)
		{
			unsigned char new_deadline[3] = {0x0C,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
		...
		else if(IF_COMP("0652",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1156",4)
			&& pc->CardRightType == 10)
		{
			unsigned char new_deadline[3] = {0x0D,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
		...
		else if(IF_COMP("0612",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1156",4)
			&& pc->CardRightType == 10)
		{
			unsigned char new_deadline[3] = {0x0D,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
		...
		else if(IF_COMP("0752",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1156",4)
			&& pc->CardRightType == 10)
		{
			unsigned char new_deadline[3] = {0x0E,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
			goto L_END;
		}
		else if(IF_COMP("0712",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1156",4)
			&& pc->CardRightType == 10)
		{
			unsigned char new_deadline[3] = {0x0E,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
		...
		else if(IF_COMP("0852",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1156",4)
			&& pc->CardRightType == 10)
		{
			unsigned char new_deadline[3] = {0x0F,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
			goto L_END;
		}
		else if(IF_COMP("0812",pc->ucCardNo,4) && IF_COMP(pc->ucDepartmentNo,"1156",4)
			&& pc->CardRightType == 10)
		{
			unsigned char new_deadline[3] = {0x0F,0x08,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
		...
		else if(IF_COMP("DG07",pc->ucCardNo,4) && pc->CardRightType == 15)
		{
			unsigned char new_deadline[3] = {0x0A,0x06,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
			goto L_END;
		}
		else if(IF_COMP("DG08",pc->ucCardNo,4) && pc->CardRightType == 15)
		{
			unsigned char new_deadline[3] = {0x0B,0x06,0x01};
			SMT_ChangeDeadLineDate(new_deadline);
			goto L_END;
		}
	}
L_END:
#endif //
	DEBUG_CARD_MSGOUT(9999,"读取客户信息成功！");
	return 0;
}
...
static int do_load_pos_dll()
{
	LOAD_DLLFUNC_EASY(h_pos_dll,POS_ConnectMF);
	LOAD_DLLFUNC_EASY(h_pos_dll,POS_CloseMF);
	LOAD_DLLFUNC_EASY(h_pos_dll,POS_PacketDelMoney);
	LOAD_DLLFUNC_EASY(h_pos_dll,POS_ReadBalance);
	LOAD_DLLFUNC_EASY(h_pos_dll,POS_MendPacket);
	LOAD_DLLFUNC_EASY(h_pos_dll,POS_ControlBuzzer);
	LOAD_DLLFUNC_EASY(h_pos_dll,POS_ReadStuempNo);
	LOAD_DLLFUNC_EASY(h_pos_dll,POS_ReadSerial);
	LOAD_DLLFUNC_EASY(h_pos_dll,POS_ReadCardNO);
	LOAD_DLLFUNC_EASY(h_pos_dll,GetLastRecord);
	LOAD_DLLFUNC_NAME(h_pos_dll,POS_GetCardID,GetCardID,-1);
	LOAD_DLLFUNC_NAME(h_pos_dll,POS_GetCardNumAndMoney,GetCardNumAndMoney,-1);
	return 0;
}
int __stdcall Load_Dll()
{
	...
	if (NULL == hGwKbcDLL)
	{
		...
		GW_ReadCardInfo     = (LPGW_ReadCardInfo)GetProcAddress(hGwKbcDLL,"GW_ReadCardInfo");
		if (
			(!GW_Login)				|| 
			(!GW_CreateNewHandle)	||
			(!GW_CloseHandle)		||
...
L_RET_POS:
	...
	if (NULL == h_pos_dll)
	{
		...
		POS_GetCardID		= (LPPOS_GetCardID)GetProcAddress(h_pos_dll,"GetCardID");
		POS_GetCardNumAndMoney = (LPPOS_GetCardNumAndMoney)GetProcAddress(h_pos_dll,"GetCardNumAndMoney");
...
int __stdcall SMT_ReadPacketInfo(int nPacketNo, LOOPPURSEINFO *LoopPurseInfo)   
{
	...
	unsigned char ucCheck,ucCheck2;
	...
	if(ucCheck == ucBuf1[15] && ucCheck2 != 0)
	{
	...
	else
	{
		...
		if(ucCheck == ucBuf2[15] && ucCheck2 != 0)
		{
...
//		CTime tm = CTime::GetCurrentTime();                               这里修改过
		...
		for(i=0;i<15;i++)	
			ucBuf1[15] += ucBuf1[i];
		ret = SMT_WriteBlock(sector,0,ucBuf1);
		if(SMT_ReadBlock(sector,0,ucBuf2))
		{
			if(ret==0)
				return 0;
		}
...
//		CTime tm = CTime::GetCurrentTime();
		...
		if(SMT_ReadBlock(sector,0,ucBuf1))
		{
			// 如果写成功，读没有成功，认为写成功
			if(ret==0)
				return 0;
...
int __stdcall SMT_PacketSetMoney(int nPacketNo,int nMoney)
{
	int ret,sector;
	unsigned char ucBuffer[16];
	unsigned char read_buf[16],read_buf2[16];
	if(nPacketNo<1)
		return -1;
	if(nMoney<0)
		return -1;
...
// 		CTime tm = CTime::GetCurrentTime();
		...
		/*
		 1 写卡失败，读卡也失败，返回 -8
		 2 写卡成功，读卡失败，返回 -10
		 3 （写卡失败，读卡成功） （写卡成功，读卡成功） 都能正确判断写卡结果
		*/
		memset(read_buf,0,sizeof read_buf);
		ret = SMT_WriteBlock(sector,0,ucBuf1);
		if(ret)
...
//		CTime tm = CTime::GetCurrentTime();
		...
		if(ret)
		{
			...
			{
				if(memcmp(ucBuf2,read_buf,16)==0)
				{
					// 写卡成功
					return 0;
				}
...
int __stdcall YKTSVR_ReadCardInfo(char physical_no[9],TPublishCard *pc)
{
	if(!GW_ReadCardInfo) 
		return 0;
	
	return GW_ReadCardInfo(physical_no,pc);
}
...
int __stdcall THIRD_GetCard(char *card_id,int *cur_bala,int nLen)
{
	int ret;
	if(KS_POS == dll_flag)
	{
		ret = POS_GetCardID(card_id,cur_bala,nLen);
		if(ret != 1)
			return ret;
		return 0;
		/*
		char stuemp_no[21]="";
		ret = POS_ReadStuempNo((unsigned char*)stuemp_no);
		if(ret)
			return ret;
		strcpy(card_id,stuemp_no);
		int money;
		ret = POS_ReadBalance(money);
		if(ret)
			return ret;
	...
	else
	{
		int card_id_ary[1];
		char temp[10]="";
		ret = ReadCardNO(card_id_ary);
		if(ret)
			return ret;
		sprintf(temp,"%%.0%dd",nLen);
		sprintf(card_id,temp,card_id_ary[0]);
		int cnt[1];
		ret = ReadBalance(1,cur_bala,cnt);
		if(ret)
			return ret;
	}
	return ret;
...
int __stdcall THIRD_GetCardNumAndMoney(char *strCardNum, int *curBalance, int Len)
{
	int ret;
	if(KS_POS == dll_flag)
	{
		ret = POS_GetCardNumAndMoney(strCardNum,curBalance,Len);
		if(ret==1)
			return 0;
		return -2;
	}
	...
	{
		ret = SMT_ReadStuempNo((unsigned char*)strCardNum);
		if(ret)
			return ret;
		int cnt[1];
		ret = ReadBalance(1,curBalance,cnt);
		if(ret)
			return ret;
	}
	return ret;
}
int __stdcall THIRD_AuthDevice(int mode,int wantflag)
{
	int ret;
	if(KS_POS == dll_flag)
	{
	...
	else
	{
		...
		{
			// 读授权卡
			return ReadCardMainKey2(wantflag);
		}
		else
		{
			char seed_key[33] = "";
			...
			if(ret)
				return ret;
			ret = GW_CreateNewHandle();
			if(ret)
				return ret;

			ret = GW_GetSeedKey(seed_key);
			GW_CloseHandle();
			if(ret)
				return ret;
			return Load_WorkKey(seed_key);
		}
	}
}
...
int __stdcall SetConsumeLimit(unsigned int max_per_count,unsigned int max_per_day)
{
	...
	unsigned char ucBuffer[16],ucBuffer2[16];
	ret = Login_Card(1);
	if(ret != 0)	return ret;
	ret = SMT_ReadBlock(1,1,ucBuffer);
	if(ret != 0)
	...
	crc_byte = 0;
	ucBuffer[3] = max_per_count / 256;
	ucBuffer[4] = max_per_count % 256;

	ucBuffer[5] = max_per_day / 256 / 256;
	ucBuffer[6] = max_per_day / 256;
	ucBuffer[7] = max_per_day % 256;

 	for (i = 0;i < 15;++i)
		crc_byte += ucBuffer[i];
	
	ucBuffer[15] = crc_byte;

	memset(ucBuffer2,0,sizeof ucBuffer2);
	ret = SMT_WriteBlock(1,1,ucBuffer);
	if(ret)
		return -1;
	ret = SMT_ReadBlock(1,1,ucBuffer2);
	// 写卡失败
	if(memcmp(ucBuffer,ucBuffer2,16)!=0)
		return -7;
...
int __stdcall ReadConsumeLimit(unsigned int *max_per_count,unsigned int *max_per_day)
{
	...
	if(ucBuffer[15] != crc_byte)
		return -7;

	*max_per_count = ucBuffer[3] * 256 + ucBuffer[4];
	*max_per_day = ucBuffer[5] * 256 * 256 + ucBuffer[6] * 256 + ucBuffer[7];
	return 0;
}
...
int __stdcall SetCardLockFlag(int lock_flag)
{
	...
	if(memcmp(ucBuffer,ucBuffer2,16) !=0)
		return -6;
	ucBuffer[12] = lock_flag;
	ret = SMT_WriteBlock(1,0,ucBuffer);
	if(ret)
		return -7;
	SMT_ReadBlock(1,0,ucBuffer2);
	if(memcmp(ucBuffer,ucBuffer2,16)!=0)
		return -7;
	return 0;
}
...
int __stdcall ReadCardLockFlag(int* lock_flag)
{
	...
	if(memcmp(ucBuffer,ucBuffer2,16) !=0)
		return -6;
	*lock_flag = ucBuffer[12];
	return 0;
}

int __stdcall SMT_SetCardMoney(unsigned long card_id,int money,int tnx)
{
	int ret;
	int read_id,sector,curr_money,curr_tnx;
	unsigned long serial_no2;
	unsigned char buffer1[16],read_buf[16];
	// 读取卡交易信息
	/*
	sector = 1;
	ret = Login_Card(sector);
	...
	ret = SMT_ReadBlock(sector,0,buffer1);
	if(ret)
		return -5;
	// 交易卡号
	read_id = buffer1[0] * 256 * 256 + buffer1[1] * 256 + buffer1[2];
	if(read_id != card_id)
		return -20; // 不是同一张卡
	*/
	char serial_no[9] = "";
	char card_type[5] = "";
	ret = RequestCard(serial_no,card_type);
	if(ret)
		return ret;
	serial_no2 = strtoul((char*)serial_no,NULL,16);
	if(serial_no2 != card_id)
		return -20;
	...
	{
		// 比较主钱包
		curr_money = ucBuf1[2] * 256 * 256 + ucBuf1[1] * 256 + ucBuf1[0];
		curr_tnx = ucBuf1[3] * 256 + ucBuf1[4];
		if(curr_money == money && curr_tnx == tnx)
		{
			// 主钱包中已经这个结果，不用写卡，返回成功
			return 0;
		}
		memcpy(ucBufTmp,ucBuf1,sizeof(ucBufTmp));
		ret = SMT_WriteBlock(sector,2,ucBufTmp);
	...
	else
	{
		...
		if(ucCheck != ucBuf2[15] || ucCheck2 == 0)	
			return 2;
		// 比较主钱包
		curr_money = ucBuf2[2] * 256 * 256 + ucBuf2[1] * 256 + ucBuf2[0];
		curr_tnx = ucBuf2[3] * 256 + ucBuf2[4];
		if(curr_money == money && curr_tnx == tnx)
		{
			// 主钱包中已经这个结果，不用写卡，返回成功
			return 0;
...
int __stdcall SMT_SetCardMoney2(unsigned long card_id,int packno,int money,int tnx)
{
	int ret;
	int read_id,sector,curr_money,curr_tnx;
	unsigned long serial_no2;
	...
	*/
	...
	sector = buffer1[packno*2-1]; // 主钱包扇区
