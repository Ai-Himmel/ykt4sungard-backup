...
DWORD g_lastDecTick = 0;
#define DEC_TICKCOUNT 2500
#define CHECK_TICKCOUNT(r) do{ DWORD t = GetTickCount(); if(t-g_lastDecTick<DEC_TICKCOUNT) return (r);}while(0)
//////////////////////////////////////////////////////////////////////////
...
long __stdcall POS_ConnectMF(int port, int baud, int hwnd)
{
	...
	return SMT_ConnectMF280(port,b);
}
...
{
	return SMT_PacketDelMomey(money,flow_no);
}

int __stdcall POS_ReadBalance(int &money)
{
	return SMT_ReadBalance(money);
}
int __stdcall POS_MendPacket(int &bmoney, int &amoney)
{
	return SMT_MendPacket(bmoney,amoney);
}
...
{
	return SMT_ReadCardNo(stuemp_no);
}
...
int __stdcall POS_ReadSerial(unsigned char serial[5])
{
	...
	unsigned char ucBuffer[256] = "";
	int ret;
	ret = SMT_FUDAN_ReadCardBlock(g_Port, ucTempBuffer, 0x00, 0x00); // 第一扇区第零块
	if (ret != 0)
	{
		return ret;
	}
	memcpy(serial,ucTempBuffer,4);
	return 0;
}
...
int __stdcall POS_ReadCardNO(int nCardNo[1])
{
	int ret = 0;
	unsigned char ucTempBuffer[128] = "";
	unsigned char ucBuffer[256] = "";											 // 用户交易卡号(需要转一道)
	ret = SMT_FUDAN_ReadCardBlock(g_Port, ucTempBuffer, 0x01, 0x00); // 第一扇区第零块
	if (ret != 0)
	{
		return ret;
...
{
	int ret;
	ret = SMT_ReadCardNo((unsigned char*)strCardNum);
	if(ret)
		return ret;
	ret = POS_ReadBalance(*curBalance);
	if(ret)
		return ret;
	return 1;
}
long __stdcall GetCardID(char *strCardID, int *curBalance, int nLen)
{
	...
	ret = POS_ReadCardNO(&card_id);
	if(ret)
		return ret;
	sprintf(fmt,"%%0%d",nLen);
	sprintf(strCardID,fmt,card_id);
	ret = POS_ReadBalance(*curBalance);
	if(ret)
		return ret;
	return 0;
