...
type
  ...
  TPublishCardMes = packed record
    ...
    //广州大学城增加内容
    {
    area_code : array[1..3] of Char;        //院校代码
    cut_id : LongInt;                       //客户ID号
    certificate_type : array[1..3] of Char; //证件类型
    people:array[1..3]of Char;              //民族
    nationality:array[1..4]of Char;         //国籍
    open_card_flag : array[1..2] of Char;   //是否已经开卡,0未开卡,1已开卡
    card_version : array[1..2] of Char;     //制卡版本号
    pub_card_flag:array[1..11] of Char;     //发卡注册标识号
    ser_flag:array[1..11]of Char;           //服务注册标识号
    pwd_open_flag : array[1..2] of Char;    //启用密码开关0启用,1未启用
    }
  end;
...
//function encrypt_work_key_asc(src:PChar; des:PChar): longint;
//  stdcall;far; external 'desdll.dll' name 'encrypt_work_key_asc';
...
//读写器授权(加载密文) *
function LoadWorkKey(pcKey: pchar): longint;
  stdcall;far; external 'CardDLL.dll' name 'Load_WorkKey';
...
function UpdateCardInfo(const CardInfo: TPublishCardMes): longint;
  stdcall;external 'CardDLL.dll' name 'UpdateCardInfo2';

//更新卡交易次数及金额,补助批次号   *
{
function UpdateMoneyAndTxCnt(packet_no, money, cnt,pnum:LongInt): longint;
  stdcall;external 'CardDLL.dll' name 'SMT_UpdateMoneyAndTxCnt';
}
...
//回收老卡扇区密钥
function RefineOldCard(nBlkFlag:Integer; nSchoolIndex:Integer; const CardInfo:TPublishCardMes): longint;
  stdcall;far; external 'CardDLL.dll' name 'SMT_RefineOldCard';

//改写老卡结构为新卡结构
function ReWriteCardInfo(nBlkFlag:Integer; nSchoolIndex:Integer): longint;
  stdcall;far; external 'CardDLL.dll' name 'SMT_ReWriteCardInfo';

//读取老卡金额
function ReadCardBalance(pnBalance:PInteger; pnTxCnt: PInteger): longint;overload;
  stdcall;far; external 'CardDLL.dll' name 'ReadBalance2';

//读取新卡金额
function ReadCardBalance(PacketNo:Integer; Balance: PInteger; TxCnt: PInteger): longint;overload;
  stdcall;far; external 'CardDLL.dll' name 'ReadBalance';
  
//写院校代码
function WriteAreaCode(nAreaCode : Integer): longint;
  stdcall;far; external 'CardDLL.dll' name 'SMT_WriteAreaCode';

//读院校代码
function ReadAreaCode(ucAreaCode : pchar): longint;
  stdcall;far; external 'CardDLL.dll' name 'SMT_ReadAreaCode';

//测试登录不同类型的学校卡片
function CheckCard(nInSchoolType:Integer; nOutSchoolType:PInteger): LongInt;
  stdcall;far;external 'CardDLL.dll' name 'SMT_CheckCard';
...
//初始化卡函数
{
function SMT_InitNewCard4GDDXC(err_code:PInteger;test:Pointer):longint;
  stdcall;far; external 'CardDLL.dll' name 'SMT_InitNewCard4GDDXC';
}
//清空卡信息，但不清空水控数据块，格式化卡结构
{
function SMT_FormatCard4GZDXC(CleanFlag:longint):longint;
  stdcall;far;external 'CardDLL.dll' name 'SMT_FormatCard4GZDXC';
}
...
//------------------------------------------------------------------------------
//发行商户卡--------------------------------------------------------------------
function WriteCardTypeAndShopId(card_type:Integer;shop_id:Integer) : LongInt;
  stdcall;external 'CardDLL.dll' name 'SMT_WriteCardTypeAndShopId';

function ReadCardTypeAndShopId(const card_type:Integer;shop_id:Integer) : LongInt;
  stdcall;external 'CardDLL.dll' name 'SMT_ReadCardTypeAndShopId';

//读取批次号
function ReadSubsidy(batch_no:PInteger) : LongInt;
  stdcall;external 'CardDLL.dll' name 'SMT_ReadSubsidy';

//更新批次号
function UpdateSubsidy(batch_no:Integer) : LongInt;
  stdcall;external 'CardDLL.dll' name 'SMT_UpdateSubsidy';
function ByteToHex(Src: Byte): String;
function HexStrToCharBuf(sHexStr:string;var Buf:array of char):integer;
//------------------------------------------------------------------------------
...
type
  ...
  public
    ...
    //读写器授权(密文授权)
    function ReadMachineAuth2(pcKey: pchar): smallint;
    //读卡物理序列号
    ...
    //读取老卡余额
    function ReadCardBalance(pnBalance:PInteger; pnTxCnt: PInteger):Integer;overload;
    //读取新卡余额
    function ReadCardBalance(PacketNo:Integer; Balance: PInteger; TxCnt: PInteger):Integer;overload;
    //更新卡基本信息
    function UpdateCardBasicInfo(var CardInfo: TPublishCardMes): integer;
    //更新卡交易次数,批次号及金额   *
    function UpdateMoneyAndTxCnt(money, cnt,pnum:LongInt): longint;
    //回收老卡结构
    function RefineOldCard(nBlkFlag:Integer; nSchoolIndex:Integer; const CardInfo:TPublishCardMes): longint;
    //写入区域代码
    function WriteAreaCode(nAreaCode:Integer): longint;
    ...
    //更新卡余额
    function PacketSetMoney(m:string):Boolean;overload;
    //更新卡余额
    function PacketSetMoney(nPacketNo, nMoney:Integer): Integer;overload;
...
//发行卡公用模块
function pubCard(tradeCode:Integer): string;
...
//更新卡余额
function TCardManage.PacketSetMoney(nPacketNo, nMoney:Integer): Integer;
begin
  Result := SMT_PacketSetMoney(nPacketNo, nMoney);
end;
...
begin
  Result := utcarddll.PublishCard(PCM);
end;
...
//  FillChar(PCM.area_code, SizeOf(PCM.area_code), Ord('0'));
//  FillChar(PCM.certificate_type, SizeOf(PCM.certificate_type), Ord('0'));
//  FillChar(PCM.people, SizeOf(PCM.people), Ord('0'));
//  FillChar(PCM.nationality, SizeOf(PCM.nationality), Ord('0'));
//  FillChar(PCM.open_card_flag, SizeOf(PCM.open_card_flag), Ord('0'));
//  FillChar(PCM.card_version, SizeOf(PCM.card_version), Ord('0'));
//  FillChar(PCM.pub_card_flag, SizeOf(PCM.pub_card_flag), Ord('0'));
//  FillChar(PCM.ser_flag, SizeOf(PCM.ser_flag), Ord('0'));
//  FillChar(PCM.pwd_open_flag, SizeOf(PCM.pwd_open_flag), Ord('0'));
...
function TCardManage.ReadMachineAuth2(pcKey:pchar): smallint;
begin
  Result := LoadWorkKey(pcKey);
end;

// 回收老卡结构
function TCardManage.RefineOldCard(nBlkFlag:Integer; nSchoolIndex:Integer; const CardInfo:TPublishCardMes): longint;
begin
  Result := utcarddll.RefineOldCard(nBlkFlag, nSchoolIndex, CardInfo);
end;

//写入区域代码
function TCardManage.WriteAreaCode(nAreaCode:Integer): longint;
begin
  Result := utcarddll.WriteAreaCode(nAreaCode);
end;
...
begin
  Result := utcarddll.UpdateCardInfo(CardInfo);
end;
...
function TCardManage.CheckCard(nInSchoolType:Integer; nOutSchoolType:PInteger): longint;
begin
  Result := utcarddll.CheckCard(nInSchoolType, nOutSchoolType);
end;
{-------------------------------------------------------------------------------
...
{-------------------------------------------------------------------------------
  ...
  返回值:    string
-------------------------------------------------------------------------------}
function getReturn(s1:string):string ;
begin
end;
...
//  area_code:=workviewIn.fieldbyname('院校代码').Data.AsString;
//  cut_id:=workviewIn.fieldbyname('客户号').Data.AsInteger;
//  cardType:=workviewIn.fieldbyname('证件类型').Data.AsString;
//  people:=workviewIn.fieldbyname('民族').Data.AsString;
//  nation:=workviewIn.fieldbyname('国籍').Data.AsString;
//  card_version:=workviewIn.fieldbyname('制卡版本号').Data.AsString;
//  pub_card_flag:=workviewIn.fieldbyname('发卡注册标识号').Data.AsString;
//  ser_flag:=workviewIn.fieldbyname('服务注册标识号').Data.AsString;

//  strcopy(@PCM.certificate_type, PChar(''));
 ...
 {
  PCM.cut_id := cut_id;                                       //客户ID号
  strcopy(@PCM.area_code, PChar(area_code));                  //院校代码
  strcopy(@PCM.certificate_type, PChar(cardType));            //证件类型
  strcopy(@PCM.people, PChar(people));                        //民族
  strcopy(@PCM.nationality, PChar(nation));                   //国籍
  strcopy(@PCM.card_version, PChar(card_version));            //制卡版本号
  strcopy(@PCM.pub_card_flag, PChar(pub_card_flag));          //发卡注册标识号
  strcopy(@PCM.ser_flag, PChar(ser_flag));                    //服务注册标识号
  }
  re := 0;
  try
...
//  begin
//    Result:=InitNewCard(PInteger(@errcode),'0000000000',nil);
//  end
//  else
//    Result:=InitNewCard(PInteger(@errcode),PChar(showcardno),nil);
end;

function TCardManage.ReadCardBalance(pnBalance:PInteger; pnTxCnt: PInteger):Integer;
begin
   Result := utcarddll.ReadCardBalance(pnBalance, pnTxCnt);
end;

function TCardManage.ReadCardBalance(PacketNo:Integer; Balance: PInteger; TxCnt: PInteger):Integer;
begin
   Result := utcarddll.ReadCardBalance(PacketNo, Balance, TxCnt);
end;
...
function TCardManage.ReadCardTypeAndShopId(const card_type:Integer;shop_id: Integer): LongInt;
begin
  Result:=utcarddll.ReadCardTypeAndShopId(card_type,shop_id);
end;

function TCardManage.WriteCardTypeAndShopId(card_type:Integer;shop_id: Integer): LongInt;
begin
  Result := utcarddll.WriteCardTypeAndShopId(card_type,shop_id);
end;

function TCardManage.ReadSubsidy(batch_no: PInteger): LongInt;
begin
  Result := utcarddll.ReadSubsidy(batch_no);
end;

function TCardManage.UpdateMoneyAndTxCnt(money,
  cnt,pnum: Integer): longint;
begin
//  Result := utcarddll.UpdateMoneyAndTxCnt(1,money,cnt,pnum);
    Result := 0; 
end;

function TCardManage.UpdateSubsidy(batch_no: Integer): LongInt;
begin
  Result := utcarddll.UpdateSubsidy(batch_no);
end;
function ByteToHex(Src: Byte): String;
begin
  SetLength(Result, 2);
...
begin
  ...
  while sHexStr<>'' do
  begin
   Buf[i]:=Char(strtoint('$'+Copy(sHexStr,1,2)));
   delete(sHexStr,1,2);
   inc(i);
  end;
end;
