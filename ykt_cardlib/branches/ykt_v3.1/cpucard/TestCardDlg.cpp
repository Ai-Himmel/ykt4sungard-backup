...
void YCT_InitIndexTable()
{
	...
	sCardTable.nIndexNo=CF_DPT_CARDCNT;strcpy(sCardTable.sIndexName,"卡充值次数");		g_VsCardInfo.push_back(sCardTable);	memset(&sCardTable,0,sizeof sCardTable);
	sCardTable.nIndexNo=CF_PAY_CARDCNT;strcpy(sCardTable.sIndexName,"卡消费次数");	g_VsCardInfo.push_back(sCardTable);		memset(&sCardTable,0,sizeof sCardTable);
...
void CTestCardDlg::OnBtnRequestcard() 
{
	...
	sprintf(sValue,"物理卡号%s",sCardPhyID);	
	m_listCmd.SetItemText(m_nRow++,1,sValue);	
}
