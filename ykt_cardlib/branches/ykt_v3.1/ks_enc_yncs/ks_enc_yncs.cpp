...
#define BEGIN_SECT 1
#define END_SECT 15
#define HD_WATER_SECT 14
#define HD_PARAM_SECT 15
#if 1
#define XT_WATER_SECT 2
#else
#define XT_WATER_SECT 15
#endif
...
static uint8 g_hd_mainkey[8];
static uint8 g_hd_paramcardkey[]={0x23,0x55,0x33,0x78,0x12,0xAB};
static uint8 g_water_paramcardkeyb[] = {0x34,0x22,0x35,0x23,0x45,0x34};
...
int KSAPI get_card_param(const char *param_name,char *param_value)
{
	...
	{
		sprintf(param_value,"%d",BEGIN_SECT);
	}
	else if(strcmp(ENC_INIT_MAXSECT,param_name)==0)
	{
		sprintf(param_value,"%d",END_SECT);
	}
	else if(strcmp("yncs_hd_sect",param_name)==0)
	{
		sprintf(param_value,"%d",HD_WATER_SECT);
	}
	else if(strcmp("yncs_hd_paramsect",param_name)==0)
	{
		sprintf(param_value,"%d",HD_PARAM_SECT);
	}
	else if(strcmp("yncs_xt_sect",param_name)==0)
	{
		sprintf(param_value,"%d",XT_WATER_SECT);
	}
	else if(strcmp("yncs_hd_paramcardkey",param_name)==0)
	{
		dec2hex(g_hd_paramcardkey,6,param_value);
		param_value[12] = '\0';

	}
	else if(strcmp("yncs_hd_seedkey",param_name)==0)
	{
		dec2hex(g_hd_mainkey,8,param_value);
		param_value[16] = '\0';
	}
	else if(strcmp("yncs_waterparamcardkeyb",param_name)==0)
	{
		dec2hex(g_water_paramcardkeyb,6,param_value);
		param_value[12] = '\0';
	}
	else
	{
...
static void gen_hd_water_keya(int sectno,unsigned char *serialno,unsigned char *keya)
{
	int i,j;
...
static int KSAPI get_key(ks_card_info_t *card,ks_mf_cardkey_t *key,KS_MF_KEYTYPE t,int sect_no)
{
	...
	if(t == MF_KEYA)
	{
		if(sect_no< BEGIN_SECT || sect_no > END_SECT)
			return KS_BLOCK_OVERFLOW;
		if(sect_no == HD_WATER_SECT)
		{
			gen_hd_water_keya(sect_no,keytext,key->key);
		}
		else if(sect_no == XT_WATER_SECT)
		{
			key->key[0] = 0x01;
			key->key[1] = 0x02;
			key->key[2] = 0x03;
...
int KSAPI register_enc_op(ks_enc_op_t *op)
{
	...
	memset(g_hd_mainkey,0,sizeof g_hd_mainkey);
