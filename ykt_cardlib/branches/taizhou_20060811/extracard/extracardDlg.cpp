...
DEF_DLLFUNC(SMT_WritePersonalPassword);
DEF_DLLFUNC(ReadCardInfo);
DEF_DLLFUNC(SMT_ReadPersonalPassword);
DEF_DLLFUNC(bcd2dec);
DEF_DLLFUNC(dec2bcd);
DEF_DLLFUNC(SMT_ReadWaterPackBackUp);
...
int LoadSmartCom411DXQ()
{
	...
	LOAD_DLLFUNC(h_CardDLL_Handler,SMT_ReadWaterPackBackUp);
	...
	LOAD_DLLFUNC(h_CardDLL_Handler,SMT_WritePersonalPassword);
	LOAD_DLLFUNC(h_CardDLL_Handler,ReadCardInfo);
	LOAD_DLLFUNC(h_CardDLL_Handler,SMT_ReadPersonalPassword);
	LOAD_DLLFUNC(h_CardDLL_Handler,bcd2dec);
	LOAD_DLLFUNC(h_CardDLL_Handler,dec2bcd);
	return 0;
}
...
BEGIN_MESSAGE_MAP(CExtracardDlg, CDialog)
	...
	ON_BN_CLICKED(ID_BTN_WRITEPASSWORD, OnBtnWritepassword)
	ON_BN_CLICKED(ID_BTN_READPASSWORD, OnBtnReadpassword)
	ON_BN_CLICKED(ID_BTN_INITCLEARCARD2, OnBtnInitclearcard2)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CExtracardDlg::OnBtnReadcard() 
{
	...
	{
		AfxMessageBox("读取水控钱包信water息失败");
		return;
	}
	temp.Format("%d",water.remain);
	GetDlgItem(ID_EDT_LEFT)->SetWindowText(temp);
	if (SMT_ReadWaterPackBackUp(&nBackUpWater))
	{
		AfxMessageBox("读取备份剩余水量失败");
	}
	temp.Format("%d", nBackUpWater);
	GetDlgItem(ID_EDT_BACKUPWATER)->SetWindowText(temp);
}
...
void CExtracardDlg::OnBtnWritepassword() 
{
	// TODO: Add your control notification handler code here
	TEST_COM_OPEN();
	unsigned char ucgPwd[7] = "" ;//{0x12, 0x34, 0x56};
	unsigned char ucPwd[4] = "";
	GetDlgItem(ID_EDT_PASSWORD)->GetWindowText((char*)ucgPwd, 8);
	dec2bcd(ucgPwd, ucPwd, 3);	
	int ret = SMT_WritePersonalPassword(ucPwd);
	if (ret != 0)
	{
		AfxMessageBox("写入密钥失败");
		return ;
	}
	AfxMessageBox("卡已写完!");
	return ;
...
void CExtracardDlg::OnBtnReadpassword() 
{
	// TODO: Add your control notification handler code here
	TEST_COM_OPEN();	
	unsigned char PersonalPsd[7] = "";
	unsigned char cPersonalPsd[4] = "";
	CString cs;
	int ret = SMT_ReadPersonalPassword(cPersonalPsd);
	if (ret)
	{
	...
	{
		bcd2dec(cPersonalPsd, 3, PersonalPsd);
//		cs.Format("%s", cPersonalPsd);
		cs.Format("%s", PersonalPsd);
	/*	::MessageBox(NULL, (char*)PersonalPsd, NULL, NULL);*/
		GetDlgItem(ID_EDT_READPASSWORD)->SetWindowText(cs);
		return;
	}	
}

void CExtracardDlg::OnBtnInitclearcard2() 
{
	// TODO: Add your control notification handler code here
	TEST_COM_OPEN();
	WATER_PACK_INFO water;
	memset(&water, 0, sizeof(water));
	water.cardtype = WATER_CLEAR_CARD2;
	if (MessageBox("您是否确认？",NULL,MB_YESNO | MB_DEFBUTTON2) == IDNO)
	{
		return ;
	}
