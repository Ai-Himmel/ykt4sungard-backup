...
// key[4]为4字节DES算法公用密钥, 此密钥在POS机中写死
//unsigned char key[4] = {0x20, 0x05, 0x05, 0x18};
// CardNo[8]的前4字节为卡号数据，后4字节为卡号按位取反数据
//unsigned char CardNo[8]={0x11, 0x22, 0x33, 0x44};
// Source[4]为送入dess_ming函数中进行编码运算的4字节数据
//unsigned char Source[4];
// dest[4]为经过dess_ming函数运算后所获得的DES编码数据
//unsigned char dest[4];
// hxxuser[8]数组为生成的8字节DES编码数据，其中前6个字节为最终使用的DES密钥
unsigned char hxxuser[8];
...
int	__stdcall gen_card_ming_des(unsigned char *text, unsigned char *user_key)
{
	// key[4]为4字节DES算法公用密钥, 此密钥在POS机中写死
	unsigned char key[4] = {0x20, 0x05, 0x05, 0x18};
	// Source[4]为送入dess_ming函数中进行编码运算的4字节数据
...
//	unsigned char hxxuser[8] = "";
	...
	{
		Source[iCount] = text[iCount];
	}
	
	dess_ming(key, Source, dest);
	for (iCount = 0; iCount < 4; iCount++)
	{
		user_key[iCount] = dest[iCount];
		text[iCount + 4] = ~text[iCount];
		Source[iCount] = ~text[iCount];
	}
