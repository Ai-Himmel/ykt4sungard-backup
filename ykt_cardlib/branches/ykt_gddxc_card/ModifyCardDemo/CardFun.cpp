...
int CCardFunction::OpenCom()
{
	...
	if (!m_is_com_open)
	{
		...
		m_is_com_open = true;
		btn_open_com->SetWindowText("¹Ø±Õ´®¿Ú");
	}
	else
	{
		if (CloseMF())
		...
		m_is_com_open = false;
		btn_open_com->SetWindowText("´ò¿ª´®¿Ú");
	}
	return 0;
}
...
int CCardFunction::EncryptElecPwd()
{
	CString pwd;
	char mpwd[128] = "";
	char seedkey[32] = "";
	strcpy(seedkey,"4344334539413846");
	AfxGetMainWnd()->GetDlgItemText(IDC_EDT_ELEC_PWD,pwd);
	if(YKT_encrypt_cardpwd(0,seedkey,pwd,mpwd))
	{
		AfxMessageBox("¼ÆËãÃÜÂë´íÎó");
		return -1;
	}
	else
	{
		pwd.Format("%s",mpwd);
		AfxGetMainWnd()->SetDlgItemText(IDC_EDT_ELEC_PWD2,pwd);
		AfxGetMainWnd()->GetDlgItem(IDC_EDT_ELEC_PWD2)->SetFocus();
		return 0;
	}

}
int CCardFunction::DecryptElecPwd()
{
	CString mpwd;
	char pwd[128] = "";
	char seedkey[32] = "";
	strcpy(seedkey,"4344334539413846");
	AfxGetMainWnd()->GetDlgItemText(IDC_EDT_ELEC_PWD2,mpwd);
	if(YKT_decrypt_cardpwd(0,seedkey,mpwd,pwd))
	{
		AfxMessageBox("¼ÆËãÃÜÂë´íÎó");
		return -1;
	}
	...
	{
		AfxGetMainWnd()->SetDlgItemText(IDC_EDT_ELEC_PWD3,pwd);
		AfxGetMainWnd()->GetDlgItem(IDC_EDT_ELEC_PWD3)->SetFocus();
		return 0;
	}
}