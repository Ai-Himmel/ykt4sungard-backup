...
BEGIN_MESSAGE_MAP(CTestcardmanDlg, CDialog)
	...
	ON_BN_CLICKED(ID_JD_ADD, OnBnClickedJdAdd)
	ON_BN_CLICKED(ID_BTN_JD2XT, OnBnClickedBtnJd2xt)
	ON_BN_CLICKED(ID_BTN_XT2JD, OnBnClickedBtnXt2jd)
	ON_BN_CLICKED(ID_BTN_ADDXT, OnBnClickedBtnAddxt)
	ON_BN_CLICKED(ID_BTN_XTAMOUNT, OnBnClickedBtnXtamount)
	ON_BN_CLICKED(IDC_BTN_SAVECFG, OnBnClickedBtnSavecfg)
END_MESSAGE_MAP()
...
BOOL CTestcardmanDlg::OnInitDialog()
{
	...
	LoadInitSectConfig();
	//this->GetDlgItem(IDC_INIT_WATER)->SetDlgItemInt(1);
	((CButton*)GetDlgItem(IDC_INIT_WATER))->SetCheck(TRUE);
	UpdateData(FALSE);
...
void CTestcardmanDlg::UpdateSectInfo()
{
	int i;
	/*
	for(i=16;i<40;++i)
		GZ_SetInitSect(i,1);
	i = ((CButton*)GetDlgItem(IDC_INIT_WATER))->GetCheck();
	GZ_SetInitSect(33,i);
	*/
}
void CTestcardmanDlg::OnTestCard() 
{
	...
	if( start_sect > end_sect || end_sect > 40)
	{
	...
	*/
	...
	int t = ((CComboBox*)GetDlgItem(IDC_COMBO_SELECTCARD))->GetCurSel();
	GZ_SetWaterCardType(t);
								   ...
								   BYTE * key,long flag)
{
	...
	if( dlg )
	{
		...
		tmpKey[12] = '\0';
		tmpBuf[32] = '\0';
		if(strcmp(inikey,tmpKey)!=0 && sect == 33 )
			memset(tmpKey,'*',12);
...
void CTestcardmanDlg::SetInitCtrlDisable(BOOL b)
{
	if(b)
	{
		m_btn_init_card.SetWindowText("初始化校园卡");
		m_btn_COM_Ctrl.EnableWindow(TRUE);
		this->GetDlgItem(IDC_COMBO_SELECTCARD)->EnableWindow(TRUE);
		this->GetDlgItem(IDC_INIT_WATER)->EnableWindow(TRUE);
	}
	else
	{
		m_btn_init_card.SetWindowText("停止初始化校园卡");
		m_btn_COM_Ctrl.EnableWindow(FALSE);
		this->GetDlgItem(IDC_COMBO_SELECTCARD)->EnableWindow(FALSE);
		this->GetDlgItem(IDC_INIT_WATER)->EnableWindow(FALSE);
	}
}
...
void CTestcardmanDlg::OnBnClickedJdAdd()
{
...
void CTestcardmanDlg::OnBnClickedBtnJd2xt()
{
	if(JD2XT_Card())
	{
		AfxMessageBox("竟达转新天卡失败");
	}
	else
	{
		AfxMessageBox("竟达转新天卡成功");
	}
}

void CTestcardmanDlg::OnBnClickedBtnXt2jd()
{
	if(XT2JD_Card())
	{
		AfxMessageBox("新天转竟达卡失败");
	}
	else
	{
		AfxMessageBox("新天转竟达卡成功");
	}
}

void CTestcardmanDlg::OnBnClickedBtnAddxt()
{
	WATER_PACK_INFO3 info;
	char msg[256];
	if(SMT_ReadWaterPackInfo_XT(&info))
	{
		AfxMessageBox("读余额失败");
	}
	info.balance += 100;
	if(SMT_WriteWaterPackInfo_XT(&info))
	{
		AfxMessageBox("写余额失败");
	}
	else
	{
		sprintf(msg,"余额，%d ",info.balance);
		AfxMessageBox(msg);
	}
}

void CTestcardmanDlg::OnBnClickedBtnXtamount()
{
	WATER_PACK_INFO3 info;
	char msg[256];
	if(SMT_ReadWaterPackInfo_XT(&info))
...
void CTestcardmanDlg::OnBnClickedBtnSavecfg()
{
	int startid,i;
	CString sectinfo;
	char buffer[64];
	memset(buffer,0,sizeof buffer);
	for(startid=IDC_CHK_SECT0;startid <= IDC_CHK_SECT39;++startid)
	{
		if(this->IsDlgButtonChecked(startid))
		{
			GZ_SetInitSect(startid-IDC_CHK_SECT0,1);
			sectinfo += "1";
		}
		else
		{
			GZ_SetInitSect(startid-IDC_CHK_SECT0,0);
			sectinfo += "0";
		}

	}
	// 保存到配置文件中
	AfxGetApp()->WriteProfileString("KEYS","SECT",(LPCTSTR)sectinfo);
}
void CTestcardmanDlg::LoadInitSectConfig()
{
	CString sectinfo;
	int startid,i;
	sectinfo = AfxGetApp()->GetProfileString("KEYS","SECT");
	if(sectinfo.IsEmpty())
		return;

	for(startid=IDC_CHK_SECT0,i = 0;i < sectinfo.GetLength();++i)
	{
		char t = sectinfo[i];
		if(t == '1')
		{
			GZ_SetInitSect(i,1);
			this->CheckDlgButton(startid+i,1);
		}
		else
		{
			GZ_SetInitSect(i,0);
			this->CheckDlgButton(startid+i,0);
		}
	}
