...
class KsCardlibBuildSystem:
    ...
    def checkRebuildResult(self,outfile,rebuild):
        ...
        if not rebuild:
            ...
            buildRe = re.compile('全部重新生成: 成功 ([0-9]*) 个，失败 ([0-9]*) 个，跳过 ([0-9]*) 个')
    ...
    def guessVCEnv(self):
        import _winreg
        base_sub_key = "SOFTWARE\\Microsoft\\VisualStudio\\9.0"
        base_key = None
        try:
            base_key = _winreg.OpenKey(_winreg.HKEY_LOCAL_MACHINE,base_sub_key)
        except:
            print "Visual studio not install "
            sys.exit(1)
        
        keyinfo = _winreg.QueryInfoKey(base_key)
        find = False
        executable = ''
        for i in range(keyinfo[1]):
            #sub_value.append(_winreg.EnumValue(base_key,i))
            n,v,t = _winreg.EnumValue(base_key,i)
            if n == "InstallDir":
                executable=["devenv.exe","VCExpress.exe"]
                for e in executable:
                    f = os.path.join(v,e)
                    if os.path.exists(f):
                        executable = f
