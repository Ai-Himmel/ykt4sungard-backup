...
int __stdcall ks_init4load(ST_TRANSPACK *pack)
{
  TRACE_FUNC("ks_init4load");
  ks_cardtype_base *c;
	GET_CARDTYPE(c);
	CardOperator oper(c);
	int ret;
	ret = oper.DepositeInit(*pack);	
	if(ret)
	{
		return -1;
	}
...
int __stdcall ks_debit4load(ST_TRANSPACK *pack)
{
  TRACE_FUNC("ks_debit4load");
  ks_cardtype_base *c;
	GET_CARDTYPE(c);
  int ret = c->DebitForTrans(pack);
	if(ret)
	{
