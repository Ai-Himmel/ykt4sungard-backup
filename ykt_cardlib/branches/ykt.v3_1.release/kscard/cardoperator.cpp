...
{
	int ret = DepositeInit(pack);
  if(ret)
  {
    return ret;
  }

	ret = DepositeDebit(pack);
  if(ret)
    return ret;
...
int CardOperator::SendLoadInitial(ST_TRANSPACK &pack)
{
  ...
  clt.SetIntFieldByName("lvol3",pack.nPayCardCnt);
  ...
  char accdate[9];
  sprintf(accdate,"%08d",pack.nAccDate%100000000);
  clt.SetStringFieldByName("sdate1",accdate);
  clt.SetStringFieldByName("sphone",pack.sRefNo);
	clt.SetStringFieldByName("sstatus0","I");
	clt.SetIntFieldByName("lvol9",pack.nFundType);

	if(!clt.SendRequest(846803,10000))
	{
		// TODO : 可能是超时
		char szMsg[512]={0};
		char errmsg[512]={0};
	...
	GET_PACK_STR(clt,"sdate1",account_date_);
  pack.nAccDate = atoi(account_date_);
	return 0;
}
int CardOperator::SendLoadDebit(ST_TRANSPACK &pack,char *sMac2)
{
	...
	交易参考号
	*/
  clt.SetStringFieldByName("sphone3",app_serial_no_);
  clt.SetStringFieldByName("sstation0",pack.sCardPhyID);
  clt.SetStringFieldByName("semp",pack.sOperCode);
  clt.SetStringFieldByName("snote",pack.sTermno);
  clt.SetIntFieldByName("lserial0",pack.nTermSeqno);
  clt.SetStringFieldByName("sdate0",pack.sTransDate);
	clt.SetStringFieldByName("stime0",pack.sTransTime);
  clt.SetIntFieldByName("lvol9",pack.nFundType);
  ...
  clt.SetStringFieldByName("sdate1",accdate);
	clt.SetStringFieldByName("sphone",pack.sRefNo);
  clt.SetIntFieldByName("lvol5",pack.nDpsCardCnt);
  clt.SetIntFieldByName("lvol6",pack.nPayCardCnt);
  clt.SetIntFieldByName("lvol7",pack.nBefBalance);
  clt.SetIntFieldByName("lvol8",pack.nTransAmt);
  clt.SetIntFieldByName("lserial1",pack.cRetryFlag);
  /*
	clt.SetStringFieldByName("scust_auth",app_serial_no_); // 应该是应用序列号
	clt.SetStringFieldByName("sname2",pack.sCardPhyID);
	clt.SetStringFieldByName("sholder_ac_no",pack.sTermno);
  ...
  */
  ...
  pack.cRetryFlag = 0;
	...
	clt.GetNextPackage();
	char szRefno[15]={0};
	GET_PACK_STR(clt,"scert_no",szRefno);
...
int CardOperator::DepositeInit(ST_TRANSPACK &pack)
{
  char temp[64]={0};
	cardtype_->SetFieldReadMode(CF_CARDBAL);
  cardtype_->SetFieldReadMode(CF_PAYCNT);
  ...
  memset(temp,0,sizeof temp);
  cardtype_->GetFieldVal(CF_PAYCNT,temp);
  pack.nPayCardCnt = atoi(temp);
...
int CardOperator::DepositeDebit(ST_TRANSPACK &pack)
{
  // 发起圈存确认请求,超时重试 3 次
  int ret;
	int retries = 3;
