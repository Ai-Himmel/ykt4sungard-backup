...
void ConnDevDlg::OnConnectDevice(wxCommandEvent &evt)
{
	...
	if(val.StartsWith("COM"))
	{
		...
		cb = (wxComboBox*)this->FindWindowById(CMB_CD_DEVBAUD);
		val = cb->GetValue();
		long baud = 0;
		val.ToLong(&baud);
		ret = ks_openport(port,(int)baud);
		if(!ret)
		{
			ks_beep();
			Close();
	...
	else if(val.Cmp(wxT("USB"))==0)
	{
		ret = ks_openport(100,0);
		if(!ret)
		{
			ks_beep();
			Close();
		...
		{
			wxMessageBox(wxT("打开端口失败"),wxT("错误"));
		}
	}
	else
	{
