...
{
// 	int count1 = GetPrivateProfileString("LOADLIBRARY", "DLL1", "", buf1, sizeof(buf1), "CardDll.ini");
// 	int count2 = GetPrivateProfileString("LOADLIBRARY", "DLL2", "", buf2, sizeof(buf2), "CardDLL.ini");
...
typedef INT (CALLBACK *LPSMT_ConnectMF280)(int, int);
typedef INT (CALLBACK *LPSMT_CloseMF280)();
...
typedef INT (CALLBACK* LPSMT_WriteBlock)(int Sector, int BlockNo, unsigned char *BlockBuf);
typedef INT (CALLBACK* LPSMT_ReadBlock)(int Sector, int BlockNo, unsigned char *BlockBuf);
typedef INT (CALLBACK *LPMF_ReadSeriesNo)(unsigned char *serial);
...
//    hDLL = LoadLibrary("SmartCom411DXQ.dll");
	...
	if(!hDLL)
	{
		MessageBox(NULL, "动态库加载失败", NULL, MB_OK);
		return -1;
	}
...
//	SMT_Login_Card   = (LPSMT_Login_Card)GetProcAddress(hDLL, "SMT_Login_Card");
...
// 要用全局的SMT_UCWORKINGKEY来转成s_water_temp_key[4]
//////////////////////////////////////////////////////////////////////////
...
/*					明华一套产品的接口函数                              */
/************************************************************************/
// int __stdcall ConnectMF(char *_ComType, int _Syn)
int __stdcall ConnectMF(int port, int baud)
{
	...
	ret = SMT_ConnectMF280(port, baud);
	if (ret != 0)
	{
		return -1;
	}
...
{
/*	return SMT_Login_Card(sector);*/
	unsigned char ucKeyA[20] = "";
	unsigned char ucSerialNo[20] = "";
	int ret;
	ret = MF_ReadSeriesNo(ucSerialNo);
	if (ret != 0)
	{
...
// 这个是ChangePassWord
int __stdcall SMT_WritePersonalPassword(unsigned char newpwd[3]) 
{
	int ret = 0;
	...
	ret=SMT_ReadBlock(1,1,data);
	if(ret)
	{
		return ret;
	}
	dec2bcd(newpwd,data,3);
	ret=SMT_WriteBlock(1,1,data);
	if(ret)
	{
		return ret;
	}
...
int __stdcall ReadBalance(int nPacketNo,int nBalance[1],int nCnt[1])
{
	...
	if(ucBuffer[(nPacketNo-1)*2] != 0x00)
	{
//		::AfxMessageBox("钱包不允许使用");
		return 1; //钱包不允许使用
	}
	sector = ucBuffer[(nPacketNo-1)*2+1]; //钱包所在扇区
	if(sector<4)
	{
		//::AfxMessageBox("钱包格式错误");
		return -1;
	}
	ret = Login_Card(sector);
	if(ret != 0)	
