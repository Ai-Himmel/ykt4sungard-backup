...
int CCardFunction::ModifyDonghuaSect()
{
	...
	CEdit *txt_personal_id =(CEdit *)AfxGetMainWnd()->GetDescendantWindow(IDC_EDIT_PERSONAL_ID);
	char str_personal_id[15] = "";
	txt_personal_id->GetWindowText((LPTSTR)str_personal_id, 10);
	memcpy(df.personal_id, str_personal_id, 10);
	ret = SMT_ModifyDongHuaSector(&df);
	if (ret) { return err_exce.SearchErr(KS_ERR_WRTIEDPUBSECT); }
