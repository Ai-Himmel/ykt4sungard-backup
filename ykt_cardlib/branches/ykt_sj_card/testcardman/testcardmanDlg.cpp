...
void CTestcardmanDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COM_SCHOOL, m_com_school);
}
...
void CTestcardmanDlg::OnTestCard() 
{
	...
	cur_select = m_com_school.GetCurSel();
	if (cur_select > 8)
	{
		if (ReadCardBlock(start_sect,end_sect,block,
			CTestcardmanDlg::GetCardBlock) != 0)
		{
			AfxMessageBox("查询卡片失败");
			return ;
		}
	}
	else 
	{
		ret = SMT_LoginSJCard(cur_select, block, CTestcardmanDlg::GetCardBlock);
		if (0 == ret) 
		{
			return ;
		}
...
{
	int cur_select = ((CTestcardmanDlg*)m_pMainWnd)->m_com_school.GetCurSel();
	BYTE card_id[4] = "";
	BYTE card_type[2] = "";
	...
	//	ret = InitNewCard(&err_code,NULL,NULL);			// 校园卡
		ret = InitNewCard2(&err_code, NULL, cur_select);
	}
	else
	{
...
//		ret = InitNewCard(&err_code,(LPCTSTR)showcardno,NULL);
		ret = InitNewCard2(&err_code, (LPCTSTR)showcardno, cur_select);
	}
...
void CTestcardmanDlg::OnRefindCard() 
{
	...
	if (9 == m_com_school.GetCurSel())
	{
		if (RefineCard(&err_code,NULL) == 0)			// 一卡通校园卡
		{
			AfxMessageBox("回收卡成功!");
			return;
		}
	...
	{
		if (0 == SMT_CheckCard(m_com_school.GetCurSel() - 1, &school_index))		// 金邦达校园卡
		{
			msg.Format("检测卡类别错误[%d]", school_index);
			AfxMessageBox(msg);
			return ;
		}

		if (0 == SMT_RefineOldCard(0, school_index, &card_info))
		{
			msg.Format("回收卡[%d]成功", school_index);
			AfxMessageBox(msg);
			return ;
		}
	}
