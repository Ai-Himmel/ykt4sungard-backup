...
*
* 2007-12-24	 1.1.008		李翔            修改内容
*												(1) 新增SMT_RefineOldCard函数	
*												(2) 新增SMT_ReadAreaCode函数
*												(3) 新增SMT_WriteAreaCode函数
*												(4) 新增SMT_WriteSubsidy函数
*												(5) 新增SMT_ReadSubsidy函数
*
* 2008-01-28	 1.1.009		李翔            修改内容
*												(1) 新增SMT_RefineOldCard函数
*												(2) 新增SMT_ReWriteCardInfo函数
*												(3) 新增SMT_ReadAreaCode函数
*												(4) 新增SMT_WriteAreaCode函数
*												(5) 新增SMT_WriteSubsidy函数
*												(6) 新增SMT_ReadSubsidy函数
*												(7) 新增SMT_LoginSJCard函数
*												(8) 新增SMT_CheckCard函数
*
* 2008-01-28	 1.1.010		李翔            修改内容
*												(1) 修改SMT_LoginSJCard函数
*												(2) 修改SMT_CheckCard函数
*												(3) 新增RefineCardSector函数
*												(4) 新增SMT_ModifyDongHuaSector函数
* =================================================================================================
*/
...
static BYTE CARD_INIT_KEYA[] = {0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6};
static BYTE CARD_INIT_KEYB[] = {0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6};
static BYTE CARD_INIT_KEYC[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static unsigned char OLD_JBD_KEYB[] = {0xED, 0xF7, 0x45, 0x07, 0x20, 0x4E};

// 东化大学公共扇区密钥
static unsigned char DH_KEYA9[7] = {0x00, 0xC3, 0x4A, 0X42, 0X56, 0X17};
static unsigned char DH_KEYB9[7] = {0xD5, 0xB4, 0xAC, 0XA4, 0X24, 0X63};
static unsigned char DH_KEYC9[7] = {0xDD, 0xDD, 0xDC, 0xEE, 0xE5, 0x90};
static unsigned char DH_KEYA10[7] = {0xDD, 0xDD, 0xDC, 0XAA, 0XA6, 0X48};
static unsigned char DH_KEYB10[7] = {0xDD, 0xDD, 0xDC, 0XAB, 0XB6, 0X48};
...
//typedef INT  (CALLBACK* lpjbd_card_key)(unsigned char *serialno, unsigned char *seed_key, int sector, unsigned char *key);
...
//lpjbd_card_key			jbd_card_key = NULL;
...
static int Login_Card(int sector)
{
	...
	//1 --- 计算KEYA
	if (0 == sector)
		memcpy(ucKeyA, LOGIN_AREA_KEY, sizeof(ucKeyA));	
	else
...
void hex2dec2(char *sHex,unsigned char * sDec,int dec_len)
{
	int i = 0;
	unsigned long ul = 0;
	char sHexTmp[3] = "";
	...
	i = strlen(sHex) / 2 + strlen(sHex) % 2;
	// 如果hex串是奇数, dec串最后一个字节的后4位补0
	if (strlen(sHex) % 2)
	{
		sDec[i - 1] = sDec[i - 1] << 4; 
	}
...
{
	int ret = 0;
	if (hDLL == NULL || hDesDLL == NULL || hGwKbcDLL == NULL)
	{
		ret = Load_Dll();
		if (ret)
		{
...
int __stdcall UpdateCardInfo(TPublishCard *CardInfo)
{
	...
	iCardId = atoi((char*)CardInfo->CardNo);
...
int __stdcall UpdateCardInfo2(TPublishCard *CardInfo)
{
	int ret = 0; 
	unsigned char ucDeadLineDate[3];
	unsigned char ucCardRightType[1];
	...
	if (ret != 0)	
	{
		DEBUG_CARD_MSGOUT(ret, "登录扇区0失败")
		return ret;
	}
	/************************************************************************/
	/* 修改学工号															*/
	/************************************************************************/
	memcpy(ucBuffer, CardInfo->ucCardNo, 16);
	ret = SMT_WriteBlock(0, 1, ucBuffer);
	if (ret != 0)	
	{
		DEBUG_CARD_MSGOUT(ret, "写扇区0块1失败")
		return ret;	
	}
	
	memset(ucBuffer, 0xFF, 16);
	memcpy(ucBuffer, CardInfo->ucCardNo + 16, 4);
	ret = SMT_WriteBlock(0, 2, ucBuffer);
	if (ret != 0)	
	{
		DEBUG_CARD_MSGOUT(ret, "写扇区0块2失败")
		return ret;	
	}
	...
	if (ret != 0)
	{
		DEBUG_CARD_MSGOUT(ret, "读扇区1块0失败")
		return ret;	
	}
	/************************************************************************/
	/* 修改交易卡号															*/
	/************************************************************************/
	ucBuffer[0] = iCardId / 256 / 256;
	ucBuffer[1] = iCardId / 256;
	ucBuffer[2] = iCardId % 256;
	...
	if (10 == ret)	
	{
		//转换为BCD码
		dec2bcd(CardInfo->ShowCardNo, ucBcdShowCardNo, 5);
		memcpy(ucBuffer + 7, ucBcdShowCardNo, 5);	
	}

	ret = SMT_WriteBlock(1, 0, ucBuffer);
	if (ret != 0)
	{
		DEBUG_CARD_MSGOUT(ret, "写扇区1块0失败")
		return ret;	
	}
	...
	if (ret != 0)
	{
		DEBUG_CARD_MSGOUT(ret, "读扇区1块1失败")
		return ret;	
	}
	
	dec2bcd(CardInfo->ucPwd, ucPwd, 3);
	memcpy(ucBuffer, ucPwd, 3);

	ret = SMT_WriteBlock(1, 1, ucBuffer);
	if (ret != 0)
	{
		DEBUG_CARD_MSGOUT(ret, "写扇区1块1失败")
		return ret;	
	}
	...
	if (ret != 0)
	{
		DEBUG_CARD_MSGOUT(ret, "读扇区2块0失败")
		return ret;	
	}
	/************************************************************************/
	/* 修改个人姓名															*/
	/************************************************************************/
	memcpy(ucBuffer, CardInfo->ucName, 8);
	
	/************************************************************************/
	/* 修改职务代码															*/
	/************************************************************************/
	memcpy(ucBuffer + 8, CardInfo->ucDutyNo, 4);
	
	/************************************************************************/
	/* 修改证件号码															*/
	/************************************************************************/
	memcpy(ucBuffer + 12, CardInfo->ucCertificateNo, 4);
	ret = SMT_WriteBlock(2, 0, ucBuffer);
	if (ret != 0)
	{
		DEBUG_CARD_MSGOUT(ret, "写扇区2块0失败");
		return ret;	
	}
	
	memcpy(ucBuffer, CardInfo->ucCertificateNo + 4, 16);
	ret = SMT_WriteBlock(2, 1, ucBuffer);
	if (ret != 0)
	{
		DEBUG_CARD_MSGOUT(ret, "写扇区2块1失败")
		return ret;	
	}
	...
	if (ret != 0)
	{
		DEBUG_CARD_MSGOUT(ret, "读取扇区2块2失败")
		return ret;	
	}
	...
	/************************************************************************/
	memset(ucBuffer, 0, sizeof(ucBuffer));
	memcpy(ucBuffer, CardInfo->ucDepartmentNo, 10);
	
	/************************************************************************/
	/* 身份代码																*/
	/************************************************************************/
	memcpy(ucBuffer + 10, CardInfo->ucIdentifyNo, 4);
	
	/************************************************************************/
	/* 性别																	*/
	/************************************************************************/
	memcpy(ucBuffer + 14, CardInfo->ucSexNo, 1);
	ret = SMT_WriteBlock(2, 2, ucBuffer);
	if (ret != 0)
	{
		DEBUG_CARD_MSGOUT(ret, "写扇区2块2失败")
		return ret;	
	}
...
//		jbd_card_key		= (lpjbd_card_key)GetProcAddress(hDesDLL, "jbd_card_key");
		if (
			(!gen_user_card_keyA) || 
			(!gen_user_card_keyB) ||
			(!encrypt_work_key)   ||
...
int __stdcall SMT_ReadPacketInfo(int nPacketNo, LOOPPURSEINFO *LoopPurseInfo)   
{
	...
	for(i=0;i<15;i++)
		ucCheck+=ucBuf1[i];
	cmp = memcmp(temp,ucBuf1,15);
	if(ucCheck == ucBuf1[15] && cmp != 0)
	{
	...
	else
	{
		...
		for(i=0;i<15;i++)
			ucCheck+=ucBuf2[i];
		cmp = memcmp(temp,ucBuf2,15);
		if(ucCheck == ucBuf2[15] && cmp != 0)
		{
...
/*================================================================ 
* 函数名：    ReadBalance2
* 参数：      [out]  (int *balance)输出金额
* 参数：      [out]  (int *tx_cnt)输出交易次数
* 功能描述:   读取金帮达卡片中卡金额
* 返回值：    int, 正确返回0, 失败可以返回其他错误码
================================================================*/
int __stdcall ReadBalance2(int *balance, int *tx_cnt)
{
	int ret = 0;
	int i;
	unsigned char buf_main[17] = "";
	unsigned char buf_bak[17] = "";
	unsigned char check = 0;
	unsigned long balance1, balance2;
	unsigned int  amount1, amount2;
	unsigned int  series1, series2;
	
	ret = SMT_Login_With_UserKey(1, OLD_JBD_KEYB, 0);
	if (ret)
	{
		DEBUG_CARD_MSGOUT(ret, "登录老卡扇区失败")
		return ret;
	}
	
	ret = SMT_ReadBlock(1, 1, buf_main);
	if (ret)
	{
		DEBUG_CARD_MSGOUT(ret, "读取块1数据失败")
	}

	ret = SMT_ReadBlock(1, 2, buf_bak);
	if (ret)
	{
		DEBUG_CARD_MSGOUT(ret, "读取块2数据失败")
	}
	
	balance1 = buf_main[0] * 256 * 256 * 256 + buf_main[1] * 256 * 256 + buf_main[2] * 256 + buf_main[3];
	balance2 = buf_bak[0] * 256 * 256 * 256 + buf_bak[1] * 256 * 256 + buf_bak[2] * 256 + buf_bak[3];
	amount1 = buf_main[4]*256 + buf_main[5];
	amount2 = buf_bak[4]*256 + buf_bak[5];
	series1 = buf_main[6]*256 + buf_main[7];
	series2 = buf_bak[6]*256 + buf_bak[7];

	i = series1 - series2;
	switch(i)
	{
	case -1:
		if ((amount2 + balance2) == balance1)
		{
//			memcpy(mifsinfo, indata2, 16);
//			*useblock = 2;
			*balance = balance2;
			*tx_cnt = series2;
			return 0;
		}
		else if ((balance2 - amount2) == balance1)
		{
//			memcpy(mifsinfo, indata2, 16);
//			*useblock = 2;
			*balance = balance2;
...
//			*useblock = 2;
		...
		{
			DEBUG_CARD_MSGOUT(4, "备份钱包交易次数大")
			ret = 100;
		}
		break;
	case  0:
		if (memcmp(buf_main, buf_bak, 8) == 0)
		{
//			memcpy(mifsinfo,indata1,16);
//			*useblock = 1;
			*balance = balance1;
			*tx_cnt = series1;
			return 0;
		...
		{
			DEBUG_CARD_MSGOUT(4, "交易次数相同")
			ret = 100;
		}
		break;
	case  1:
		if ((amount1 + balance1) == balance2)
		{
//			memcpy(mifsinfo,indata1,16);
//			*useblock = 1;
			*balance = balance1;
		...
		else if ((balance1 - amount1) == balance2)
		{
//			memcpy(mifsinfo,indata1,16);
//			*useblock = 1;
			*balance = balance1;
...
//			*useblock = 1;
		...
		{
			DEBUG_CARD_MSGOUT(5, "主钱包交易次数大")
			ret = 100;
		}
		break;
	default:
		DEBUG_CARD_MSGOUT(6, "钱包交易次数不正确")
		ret = 100;
		break;
	}

//	memcpy(outdata, mifsinfo,16);
/*
	for (int i = 0; i < 15; i++) { check += buf_main[i]; }

	if (buf_main[15] == check) { main_flag = true; }

	check = 0;
	for (int j = 0; j < 15; j++) { check += buf_bak[i]; }
	
	if (buf_bak[15] == check) { bak_flag = true; }
*/
	/*
	if (main_flag)
	{
...
/*================================================================ 
* 函数名:    InitNewCard2
* 参数:      [out]  (int *err_code)输出错误码
* 参数:      [in]   (const char* showcardno)输入的交易卡号
* 参数:      [in]   (int school_index)输入的学校标识
* 功能描述:  初始化松江大学城卡片
* 返回值:    int, 正确返回0, 失败可以返回其他错误码
================================================================*/
int __stdcall InitNewCard2(int *err_code, const char* showcardno, int school_index)
{
	BYTE card_id[5] = "";
	BYTE card_type[2] = "";
	BYTE card_key_a[7] = "";
...
//	int current_key, old_key;
	...
	for (sect = 0; sect < 9; ++sect)
	{
		if (0 == sect)
		{
			ret = SMT_Login_With_UserKey(sect, LOGIN_AREA_KEY, 0);		// 扇区1已经初始化过了
			if (0 == ret) { continue; }
		}

		if (1 == sect)
		{
			ret = SMT_Login_With_UserKey(sect, OLD_JBD_KEYB, 0);
			if (0 == ret) { goto L_WRITE_CARD; }

			ret = SMT_Login_With_UserKey(sect, sector2[school_index].card_key, 0);
			if (0 == ret) { goto L_WRITE_CARD; }

			ret = SMT_Login_With_UserKey(sect, CARD_INIT_KEYC, 0);
			if (0 == ret) { goto L_WRITE_CARD; }
		}
		
		ret = SMT_Login_With_UserKey(sect, SJCard[school_index].sector[sect].card_key, 0);		// 大学城某个学校的卡片
		if (0 == ret) { goto L_WRITE_CARD; }
		
		ret = SMT_Login_With_UserKey(sect, CARD_INIT_KEY, 0);			// 6个FF
		if (0 == ret) { goto L_WRITE_CARD; }
		
		ret = SMT_Login_With_UserKey(sect, CARD_INIT_KEYA, 0);
		if (0 == ret) { goto L_WRITE_CARD; }

		ret = SMT_Login_With_UserKey(sect, card_key[1].key, 0);			// KEYA
		if (0 == ret) { continue; }
		
		ret = SMT_Login_With_UserKey(sect, card_key[2].key, 1);
		if (0 == ret) { goto L_WRITE_CARD; }
		else
		{
			// 所有扇区都已经检测到, 扇区sect初始化失败
			*err_code = sect;
			return -2;
		}
// 改写为金仕达一卡通扇区格式
L_WRITE_CARD:
		...
		if (0 == sect)
		{
			memcpy(sect_data, LOGIN_AREA_KEY, CARD_KEY_LEN);
		}
		else
		{
			memcpy(sect_data, card_key_a, CARD_KEY_LEN);
		}
		// control bit
		memcpy(sect_data + CARD_KEY_LEN, CARD_NEW_CTRL_BIT, sizeof(CARD_NEW_CTRL_BIT));
		// key B
		memcpy(sect_data + CARD_KEY_LEN + sizeof(CARD_NEW_CTRL_BIT), card_key_b, CARD_KEY_LEN);
		ret = WriteCardData(sect, PASSWD_BLOCK_NUM, sect_data);
		if (ret) 
		{
			*err_code = sect;
			return -3; 
									 ...
									 ,short block,LPGetBlockData lpFunc)
{
	...
	BYTE card_key_a[7] = "";
	BYTE card_key_b[7] = "";
	BYTE card_key_c[] = {0xED, 0xF7, 0x45, 0x07, 0x20, 0x4E};
...
/*================================================================ 
* 函数名：    SMT_RefineOldCard
* 参数：      [in]  (blk_flag)黑卡标志
* 参数：      [in]  (school_index)学校索引
* 参数：      [in]  (TPublishCard *CardInfo)卡结构数据
* 功能描述:   回收松江大学城老卡
* 返回值：    int, 正确返回0, 失败可以返回其他错误码
================================================================*/
int __stdcall SMT_RefineOldCard(int blk_flag, int school_index, TPublishCard *CardInfo)
{
	// 计算老扇区, 回收0-8扇区块3的密钥, 写入一卡通密钥
	unsigned char ucSerialNo[5] = "";
	unsigned char ucKeyA[6] = "";
	unsigned char ucKeyB[6] = "";
	...
	gen_user_card_keyB(SMT_UCWORKINGKEY, ucSerialNo, ucKeyB);
	
	// key A
	memcpy(sect_data, ucKeyA, CARD_KEY_LEN);
	// control bit
	memcpy(sect_data + CARD_KEY_LEN, CARD_NEW_CTRL_BIT, sizeof(CARD_NEW_CTRL_BIT));
	// key B
	memcpy(sect_data + CARD_KEY_LEN + sizeof(CARD_NEW_CTRL_BIT), ucKeyB, CARD_KEY_LEN);

	// 计算OLDKEYA, OLDKEYB(一般用OLDKEYB登录), 默认密钥是6个FF
	for (int sect = 0; sect < 9; sect++)
	{
		memset(ucBuffer, 0, sizeof(ucBuffer));

		if (3 == sect)	// 如果是扇区3需要格式化钱包结构和指定黑名单版本
		{	
			ret = SMT_Login_With_UserKey(sect, SJCard[school_index].sector[sect].card_key, 0);
			if (ret != 0)
			{
				sprintf(sMsg, "登录卡片扇区%d失败", sect);
				DEBUG_CARD_MSGOUT(ret, sMsg)
				return ret;		// 卡片登陆失败
			}

			for (int i = 0; i < 4; i++)
			{
				ucBuffer[j] = 0x00;							   // 打开四个钱包扇区
				j += 1;
				ucBuffer[j] = sect + 1 + i;                  // 注意只能改第四个
				j += 1;
			}
			// 写钱包扇区格式到块0
			ret = SMT_WriteBlock(sect, 0, ucBuffer);
			if (ret != 0)
			{
				sprintf(sMsg, "写卡片扇区%d块%d失败", sect, 0);
				DEBUG_CARD_MSGOUT(ret, sMsg)
				return ret;	//写卡失败
			}
			...
			memset(ucBuffer, 0x00, 16);
			// 写空白数据到块1
			ret = SMT_WriteBlock(sect, 1, ucBuffer);
			if (ret != 0)
			{
				sprintf(sMsg, "写卡片扇区%d块%d失败", sect, 1);
				DEBUG_CARD_MSGOUT(ret, sMsg)
				return ret;	
			}
			
			if (1 == blk_flag) { ucBuffer[0] = 0x00; }	// 黑卡
			else { ucBuffer[0] = 0xFF; }	// 白卡
			
			// 写黑名单版本信息到块2
			ret = SMT_WriteBlock(sect, 2, ucBuffer);
			if (ret != 0)
			{
				sprintf(sMsg, "写卡片扇区%d块%d失败", sect, 2);
				DEBUG_CARD_MSGOUT(ret, sMsg)
				return ret;	
			}
		}
...
//			if ((0 == sector) && (0 == block)) { continue; }
			...
			else if (1 == sect)
			{
				...
				dec2bcd(CardInfo->ucPwd, ucPwd, 3);
				memcpy(ucBuffer, ucPwd, 3);
				memset(ucBuffer + 3, 0, sizeof(ucBuffer) - 3);
			...
			else
			{
				...
				{
					ret = SMT_WriteBlock(sect, block, ucBuffer);
					if (ret != 0)
					{
						sprintf(sMsg, "写卡片扇区%d块%d失败", sect, block);
						DEBUG_CARD_MSGOUT(ret, sMsg)
						return ret;
					}
				}
		...
		if (0 == sect)
		{
			memcpy(sect_data, LOGIN_AREA_KEY, CARD_KEY_LEN);
			memcpy(sect_data + CARD_KEY_LEN, CARD_FOR14_CTRL_BIT, sizeof(CARD_FOR14_CTRL_BIT));
			ret = SMT_WriteBlock(sect, 3, sect_data);	
			memcpy(sect_data, ucKeyA, CARD_KEY_LEN);
			memcpy(sect_data + CARD_KEY_LEN, CARD_NEW_CTRL_BIT, sizeof(CARD_NEW_CTRL_BIT));
		}
		else
...
/*================================================================ 
* 函数名：    SMT_ReWriteCardInfo
* 参数：      [in]  (int blk_flag)黑卡标志
* 参数：      [in]  (int school_index)学校索引标志
* 功能描述:   重写松江大学城老卡结构
* 返回值：    int, 正确返回0, 失败可以返回其他错误码
================================================================*/
int __stdcall SMT_ReWriteCardInfo(int blk_flag, int school_index)
{
	// 计算老扇区, 回收0-8扇区块3的密钥, 写入一卡通密钥
	unsigned char ucSerialNo[5] = "",
				  ucKeyA[6] = "",
	...
	if (school_index < 0 || school_index > 6)
	{
		DEBUG_CARD_MSGOUT(school_index, "学校索引不正确")
		return -1;
	}

	dec2hex(ucSerialNo, 4, serial_no);
	gen_user_card_keyA(SMT_UCWORKINGKEY, ucSerialNo, ucKeyA);
	gen_user_card_keyB(SMT_UCWORKINGKEY, ucSerialNo, ucKeyB);
	...
	for (int sector = 0; sector < 9; sector++)
	{
		...
		// 用6个FF登录第0扇区
		if (3 == sector)	// 如果是扇区3需要格式化钱包结构和指定黑名单版本
		{	
			if (0 == SMT_Login_With_UserKey(sector, CARD_INIT_KEY, 0)) { NULL; }
			else if (0 == SMT_Login_With_UserKey(sector, ucKeyB, 1)) { NULL; }
			else if (0 == SMT_Login_With_UserKey(sector, SJCard[school_index].sector[sector].card_key, 0)) { NULL; }
			else
			{
				sprintf(sMsg, "卡号:%s登录卡片扇区%d失败", serial_no, sector);
				DEBUG_CARD_MSGOUT(ret, sMsg)
				return ret;		// 卡片登陆失败
			}
			...
			for (int i = 0; i < 4; i++)
			{
				ucBuffer[j] = 0x00;							   // 打开四个钱包扇区
				j += 1;
				ucBuffer[j] = sector + 1 + i;                  // 注意只能改第四个
				j += 1;
			}
			// 写钱包扇区格式到块0
			ret = SMT_WriteBlock(sector, 0, ucBuffer);
			if (ret != 0)
			{
				sprintf(sMsg, "卡号:%s写卡片扇区%d块%d失败", serial_no, sector, 0);
				DEBUG_CARD_MSGOUT(ret, sMsg)
				return ret;	//写卡失败
			}
			...
			memset(ucBuffer, 0x00, 16);
			// 写空白数据到块1
			ret = SMT_WriteBlock(sector, 1, ucBuffer);
			if (ret != 0)
			{
				sprintf(sMsg, "卡号:%s写卡片扇区%d块%d失败", serial_no, sector, 1);
				DEBUG_CARD_MSGOUT(ret, sMsg)
				return ret;	
			}
			...
			if (ret != 0)
			{
				sprintf(sMsg, "卡号:%s写卡片扇区%d块%d失败", serial_no, sector, 2);
				DEBUG_CARD_MSGOUT(ret, sMsg)
				return ret;	
			}
		}
		else
		{
			if (1 == sector)
			{
				if (0 == SMT_Login_With_UserKey(sector, OLD_JBD_KEYB, 0)) { NULL; }
				else if (0 == SMT_Login_With_UserKey(sector, ucKeyB, 1)) { NULL; }
				else
				{
					sprintf(sMsg, "卡号:%s登录卡片扇区%d失败", serial_no, sector);
			...
			for (int block = 0; block < 3; block++)
			{
				...
				ret = SMT_WriteBlock(sector, block, ucBuffer);
				if (ret != 0)
				{
					sprintf(sMsg, "卡号:%s写卡片扇区%d块%d失败", serial_no, sector, block);
					DEBUG_CARD_MSGOUT(ret, sMsg)
					return ret;
				}	
			}
		...
		if (0 == sector)
		{
			memcpy(sect_data, LOGIN_AREA_KEY, CARD_KEY_LEN);
			memcpy(sect_data + CARD_KEY_LEN, CARD_FOR14_CTRL_BIT, sizeof(CARD_FOR14_CTRL_BIT));
			ret = SMT_WriteBlock(sector, 3, sect_data);	
			memcpy(sect_data, ucKeyA, CARD_KEY_LEN);
			memcpy(sect_data + CARD_KEY_LEN, CARD_NEW_CTRL_BIT, sizeof(CARD_NEW_CTRL_BIT));
		}
		else
		...
		if (ret != 0)
		{
			sprintf(sMsg, "卡号:%s写卡片扇区%d失败", serial_no, sector);
			DEBUG_CARD_MSGOUT(ret, sMsg)
			return ret;	//写卡失败
		}
	}
...
/*================================================================ 
* 函数名：    SMT_LoginSJCard
* 参数：      [in] (int school_index)登录学校索引编号
* 参数：      [in] (short block)选择应该显示的数据块
* 参数：      [in] (LPGetBlockData lpFunc)显示卡片块数据的回调函数
* 功能描述:   以不同索引号加载不同密钥登录松江大学城卡片              
* 返回值：    int, 正确返回0, 失败返回其他值
================================================================*/
int __stdcall SMT_LoginSJCard(int school_index, short block, LPGetBlockData lpFunc)
{
	BYTE card_id[5] = "";
	BYTE card_type[2] = "";
	BYTE buf[17] = "";
	...
	if (school_index < 0 || school_index > 9)
	{
		DEBUG_CARD_MSGOUT(school_index, "学校索引不正确")
		return -1;
	}
	...
	{
		// 查找可以登录的 key
		if (SMT_Login_With_UserKey(sect, SJCard[school_index].sector[sect].card_key, 0))
		{
			sprintf(msg, "登录扇区:%d失败--学校索引:%d", sect, school_index);
			DEBUG_CARD_MSGOUT(sect, msg)
		}
		...
		for (int b = 0; b < 4; ++b)
		{
			if (block & (1 << b))
			{
				...
				{
					lpFunc(sect, b, buf,
						SJCard[school_index].sector[sect].card_key,
						SJCard[school_index].sector[sect].card_flag);	
				}
			}
		}
	}
...
* 函数名：    SMT_CheckCard
* 参数：      [in] (int school_index)登录学校索引编号
* 功能描述:   以不同索引号检测不同密钥的松江大学城卡片              
* 返回值：    int, 正确返回0, 失败返回其他值
================================================================*/
int __stdcall SMT_CheckCard(int in_school_index, int *out_school_index)
{
	BYTE card_id[5] = "";
	BYTE card_type[2] = "";
	BYTE buf[17] = "";
	...
	if (in_school_index < 0 || in_school_index > 9)
	{
		DEBUG_CARD_MSGOUT(in_school_index, "学校索引不正确")
		return -4;
	}

	// 查找可以登录的 key
	if (0 == SMT_Login_With_UserKey(sect, SJCard[in_school_index].sector[sect].card_key, SJCard[in_school_index].sector[sect].card_flag))
	{
		*out_school_index = in_school_index;
		return 0;
	}
	/*
	else if (0 == SMT_Login_With_UserKey(sect, CARD_INIT_KEY, 0))
	{
		*out_school_index = 7;
		return 0;
	}
	*/
	...
	{
		DEBUG_CARD_MSGOUT(sect, "登录密钥不正确")
		return -1;		
	}
...
int __stdcall SMT_ModifyDongHuaSector(DongHua *dh)
{
	BYTE card_id[5] = "";
	BYTE sect_data[17] = "";
	BYTE card_type[2] = "";
	BYTE card_type_right = 0;
	BYTE ucBuffer[17] = "";
	BYTE ucDeadLineDate[4] = "";
	...
	// 9扇区改写密钥
	ret = SMT_Login_With_UserKey(sect, CARD_INIT_KEY, 0);			// 6个FF
	if (ret == 0) { goto L_DONGHUA9; }

	ret = SMT_Login_With_UserKey(sect, DH_KEYC9, 0);				// 东华原始密钥
	if (ret == 0) { goto L_DONGHUA9; }
	
	ret = SMT_Login_With_UserKey(sect, DH_KEYA9, 0);				// 东华新密钥
	if (ret == 0) { goto L_DONGHUA9; }
...
L_DONGHUA9:
	{
		memset(sect_data, 0, sizeof(sect_data));
		memcpy(sect_data, DH_KEYA9, CARD_KEY_LEN);
		// control bit
		memcpy(sect_data + CARD_KEY_LEN, CARD_INIT_CTRL_BIT, sizeof(CARD_INIT_CTRL_BIT));
		// key B
		memcpy(sect_data + CARD_KEY_LEN + sizeof(CARD_INIT_CTRL_BIT), DH_KEYB9, CARD_KEY_LEN);
		ret = WriteCardData(sect, PASSWD_BLOCK_NUM, sect_data);
		if (ret) 
		{
			return -3; 
	...
	ret = SMT_ReadBlock(1, 0, ucBuffer);
	if (ret != 0) { return -6; }
	card_type_right = ucBuffer[3];
	memcpy(ucDeadLineDate, ucBuffer + 4, 3);

	// 读取卡号
	ret = ReadCardNO(&cardid);
	if (ret != 0) { return -7; }

	// 读取姓名, 性别, 有效期, 证件号码, 部门号
	...
	unsigned char temp_card_id[5] = "";
	char dead_line_date[10] = "";
	
	ret = SMT_ReadPersonalInfo(ucName, ucDutyNo, ucCertificateNo, ucDepartmentNo, ucIdentifyNo, ucSexNo, ucCardNo);
	if (ret != 0) { return -8; }

	ret = SMT_Login_With_UserKey(sect, CARD_INIT_KEY, 0);			// 6个FF	
	if (ret == 0) { goto L_DONGHUA10; }

	ret = SMT_Login_With_UserKey(sect, DH_KEYA10, 0);
	if (ret == 0) { goto L_DONGHUA10; }
	
	return -4; 
L_DONGHUA10:
	{
		unsigned char ucBCDCertificateNo[10] = "";
//		memcpy((char *)ucCertificateNo, "42010619901212003X", 18);
		unsigned char check_code = 0;
		// 块0
		memset(ucBuffer, 0, sizeof(ucBuffer));
		ucBuffer[0] = card_type_right;
		memcpy(temp_card_id, &cardid, 3);
		ucBuffer[2] = temp_card_id[2];
		ucBuffer[3] = temp_card_id[1];
		ucBuffer[4] = temp_card_id[0];
//		memcpy(ucBuffer + 5, dh->personal_id, 10);	个人编号是学工号
		memcpy(ucBuffer + 5, ucCardNo, 10);
//		memcpy(ucBuffer + 5, "050120105", 10);

		if ('X' == ucCertificateNo[strlen((char *)ucCertificateNo) - 1] || 'x' == ucCertificateNo[strlen((char *)ucCertificateNo) - 1])
		{
			ucBuffer[1] = 0x01;
			ucCertificateNo[strlen((char *)ucCertificateNo) - 1] = '0';
		}	

		for (int i = 0; i < 15; i ++) { check_code = check_code ^ ucBuffer[i]; }
		ucBuffer[15] = check_code;
		...
		check_code = 0;
		memset(ucBuffer, 0, sizeof(ucBuffer));
		memcpy(ucBuffer, ucName, 7);
		ucBuffer[8] = atoi((char *)ucSexNo);
//		ucBuffer[8] = 0x01;
		itoa(ucDeadLineDate[0], dead_line_date, 10);
		itoa(ucDeadLineDate[1], dead_line_date + 2, 10);
		itoa(ucDeadLineDate[2], dead_line_date + 4, 10);
		hex2dec(dead_line_date, ucDeadLineDate, 3);
		memcpy(ucBuffer + 9, ucDeadLineDate, 3);
		...
		check_code = 0;
		memset(ucBuffer, 0, sizeof(ucBuffer));
//		dec2bcd(ucCertificateNo, ucBCDCertificateNo, 10);
		hex2dec2((char *)ucCertificateNo, ucBCDCertificateNo, 10);
		memcpy(ucBuffer, ucBCDCertificateNo, 9);
		memcpy(ucBuffer + 9, ucDepartmentNo, 5);
		for (int i = 0; i < 15; i ++) { check_code = check_code ^ ucBuffer[i]; }
		ucBuffer[15] = check_code;
		ret = SMT_WriteBlock(sect, 2, ucBuffer);
		if (ret != 0) return -9;
		...
		// 块3
		memset(sect_data, 0, sizeof(sect_data));
		memcpy(sect_data, DH_KEYA10, CARD_KEY_LEN);
		// control bit
		memcpy(sect_data + CARD_KEY_LEN, CARD_INIT_CTRL_BIT, sizeof(CARD_INIT_CTRL_BIT));
		// key B
		memcpy(sect_data + CARD_KEY_LEN + sizeof(CARD_INIT_CTRL_BIT), DH_KEYB10, CARD_KEY_LEN);
		ret = WriteCardData(sect, PASSWD_BLOCK_NUM, sect_data);
		if (ret) 
		{
			return -3; 
