...
// 金帮达卡片密钥算法
/*
int __stdcall jbd_card_key(unsigned char *serialno, unsigned char *seed_key, int sector, unsigned char *key)
{
	unsigned char GENTOR[9] = "";

	GENTOR[0] = serialno[0] ^ ((sector << 4) + sector);
	GENTOR[1] = serialno[1] ^ ((sector << 4) + sector);
	GENTOR[2] = serialno[2] ^ ((sector << 4) + sector);
	GENTOR[3] = serialno[3] ^ ((sector << 4) + sector);
	GENTOR[4] = ~serialno[0] ^ ((sector << 4) + sector);
	GENTOR[5] = ~serialno[1] ^ ((sector << 4) + sector);
	GENTOR[6] = ~serialno[2] ^ ((sector << 4) + sector);
	GENTOR[7] = ~serialno[3] ^ ((sector << 4) + sector);
	
	encrypt_des(seed_key, 8, GENTOR, 8, key);	
	return 0;
}
*/