...
CLogFile g_LogFile;
std::map<DWORD, std::string> hmap_collect_err_msg;
...
//	GetPrivateProfileString("LOADLIBRARY", "COM", "", bufCom, sizeof(bufCom), iniDllFilePathBuf);
	str_com = ini_file.ReadString("INTERFACE", "COM", "1");
	csCom.Format("COM%s", str_com.c_str());
	m_com_portNo.SetWindowText(csCom);

	str_com = ini_file.ReadString("LOADLIBRARY","HTTP_URL","");
	memset(m_http_url,0,sizeof m_http_url);
	strncpy(m_http_url,str_com.c_str(),sizeof(m_http_url)-1);
...
void CReadCardInfoDlg::SimulateKeyPress(const char cardno[30])
{
	char time_msg[20] = "";
	int ret_code = 0;
	int count = 0;
...
//	ret_code = curl_http.curl_easy_http_request(&chunk, hexBuffer, m_http_url, m_user);
	strcpy(http_info.card_id, cardno);
...
//		AfxGetMainWnd()->PostMessage(WM_POST_MSG_DLG, 0, 0);
		g_LogFile.WriteLogEx(0, "%s,%s,%s,%d", cardno, time_msg, m_user, ret_code);
		g_LogFile.ToFlush();
