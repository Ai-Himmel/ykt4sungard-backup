...
int LoadSmartCom411DXQ()
{
	...
	h_SmartComDXQ_Handler = LoadLibrary("KS_MH.dll");
	if(h_SmartComDXQ_Handler == NULL)
		return -1;
...
BEGIN_MESSAGE_MAP(CExtracardDlg, CDialog)
	...
	ON_BN_CLICKED(ID_BTN_MODIFY_SMALL_PACKET, OnBtnModifySmallPacket)
	ON_BN_CLICKED(IDC_BTN_TRANSFER_SMALL_PACKET, OnBtnTransferSmallPacket)
	ON_BN_CLICKED(IDC_BTN_TRANSFER_PACKET, OnBtnTransferPacket)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CExtracardDlg::OnBtnReadcard() 
{
	...
	*/
	memset(&LoopPurseInfo,0,sizeof(LoopPurseInfo));
	if(SMT_ReadPacketInfo(2,&LoopPurseInfo))
	{
		AfxMessageBox("读取999钱包信息失败");
		return;
	}
	temp.Format("%.2f",(double)LoopPurseInfo.RemainMoney / 100);
	GetDlgItem(ID_EDT_SMALL_MONEY)->SetWindowText(temp);
}
...
void CExtracardDlg::OnBtnModifySmallPacket() 
{
	// TODO: Add your control notification handler code here
	TEST_COM_OPEN();
	int packet_999_money = 0;
	double transfer_money = 0;
	CString str_transfer_money = "";
	LOOPPURSEINFO LoopPurseInfo;
	memset(&LoopPurseInfo, 0, sizeof LoopPurseInfo);
	GetDlgItem(ID_EDT_MODIFY_SMALL_MONEY)->GetWindowText(str_transfer_money);
	transfer_money = atof(str_transfer_money);
	if(SMT_ReadPacketInfo(2, &LoopPurseInfo))
	{
		AfxMessageBox("读取999钱包信息失败");
		return;
	}
	if (LoopPurseInfo.RemainMoney < 0)
	{
		AfxMessageBox("999钱包余额不足");
		return ;
	}
	packet_999_money = (int)(transfer_money * 100);
//	packet_999_money = LoopPurseInfo.RemainMoney;
//	packet_999_money += (int)(transfer_money * 100);
	if (SMT_PacketSetMoney(2, packet_999_money))
	{
		AfxMessageBox("写入999钱包失败");
		return ;
	}
	AfxMessageBox("修改999钱包成功");
	return ;
}

void CExtracardDlg::OnBtnTransferSmallPacket() 
{
	// TODO: Add your control notification handler code here
	TEST_COM_OPEN();
	int ret = 0;
	...
	GetDlgItem(ID_EDT_TRANSFER_MONEY)->GetWindowText(str_transfer_money);
	transfer_money_use = (int)(atof(str_transfer_money) * 100);
	if (transfer_money_use <= 0)
	{
		AfxMessageBox("请输入正数金额");
		return ;
	}
	if(SMT_ReadPacketInfo(purse_packet_one, &LoopPurseInfoOne))
	{
	...
	if(SMT_ReadPacketInfo(purse_packet_two, &LoopPurseInfoTwo))
	{
		AfxMessageBox("读取999钱包信息失败");
		return;
	}
	...
	ret = SMT_PacketSetMoney(purse_packet_one, (packet_one_money - transfer_money_use));
	if (ret)
	{
		AfxMessageBox("写入大钱包失败");
		return ;
	}
	ret = SMT_PacketSetMoney(purse_packet_two, (packet_two_money + transfer_money_use));
	if (ret)
	{
		AfxMessageBox("写入999钱包失败");
		return ;
...
void CExtracardDlg::OnBtnTransferPacket() 
{
	// TODO: Add your control notification handler code here
	TEST_COM_OPEN();
	int packet_money = 0;
	...
	packet_money = LoopPurseInfo.RemainMoney;
	packet_money += (int)(transfer_money * 100);
	if (SMT_PacketSetMoney(1, packet_money))
	{
		AfxMessageBox("写入大钱包失败");
		return ;
	}
	AfxMessageBox("写入大钱包成功");
	return ;
}
