...
void GetIniFile(char *FilePathBuf)
{
	...
	if (rename(FilePathBuf, FilePathBuf))
		strcpy(FilePathBuf, "C:\\CardDll.ini");

	if (rename(FilePathBuf, FilePathBuf))
		strcpy(FilePathBuf, "C:\\WINDOWS\\CardDll.ini");
}
...
//	BYTE card_key_c[] = {0x56,0xDD, 0xA9, 0x59, 0xDD, 0xF0};
	...
	CardKeyData card_key[3];		// 4改成3, 原来4表示上大图书馆旧卡回收
