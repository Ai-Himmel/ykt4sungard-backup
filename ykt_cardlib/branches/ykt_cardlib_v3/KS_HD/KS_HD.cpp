...
#define COM_OPEN_FLAG(com_open_flag) if (!com_open_flag) { return 102; }
...
//	if (!com_is_open) { return 102; }
	...
	amount = (double)amoney / (double)100;
	round_float(&amount, 2);
...
//	if (!com_is_open) { return 102; }
	...
	amoney = int(curAmount * 100 + 0.5);
	ret = SMT_ReduceCash(strCardID, curAmount);
	if (ret != 1)
	{
