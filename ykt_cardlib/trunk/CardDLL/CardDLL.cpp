...
static BYTE CARD_PUB_KEYA[] = {0x01,0x02,0x03,0x04,0x05,0x06};
static BYTE CARD_PUB_KEYB[] = {0x52,0x88,0x52,0xCC,0x25,0x01};
...
typedef int (CALLBACK   *LPGW_DepositHandin)(int card_info[3],int out_depinfo[3]);
typedef int (CALLBACK   *LPGW_DepositHandback)(int card_info[3],int out_depinfo[3]);
typedef int (CALLBACK   *LPGW_DepositBackoff)(int card_id,int serial_no);
...
LPGW_DepositHandin		GW_DepositHandin = NULL;
LPGW_DepositHandback	GW_DepositHandback = NULL;
LPGW_DepositBackoff		GW_DepositBackoff = NULL;
// 计算KEYA
...
int __stdcall Load_Dll()
{
	...
	if (NULL == hGwKbcDLL)
	{
		...
		GW_DepositHandin	= (LPGW_DepositHandin)GetProcAddress(hGwKbcDLL,"GW_DepositHandin");
		GW_DepositHandback  = (LPGW_DepositHandback)GetProcAddress(hGwKbcDLL,"GW_DepositHandback");
		GW_DepositBackoff   = (LPGW_DepositBackoff)GetProcAddress(hGwKbcDLL,"GW_DepositBackoff");
		if (
			(!GW_Login)				|| 
			(!GW_CreateNewHandle)	||
			(!GW_CloseHandle)		||
...
int __stdcall ReadCardMainKey2(short wantFlag)
{
	...
	unsigned char work_key[9] = "";
	ret = SMT_ReadAuthCard2(work_key, &AUTH_CARD_FLAG);
	if ( ret == 0)
	{
		if(AUTH_CARD_FLAG == wantFlag || -1 == wantFlag)
		{
...
int __stdcall PublishFeeRateCard(FEE_RATE_CARD_INFO *fee_rate_card_info)
{
	...
	if(fee_count > 20)
	{
		DEBUG_CARD_MSGOUT(1,"允许卡类不能超过20个.");
...
//	fee_rate_data_buf[fee_rate_data_length + 2] = check_crc;
	...
	offset = 2;
	DEBUG_CARD_MSGOUT(1001,"开始读取卡费率");
	for (right_type_num = 0; right_type_num < 32; right_type_num++)
	{
		if (right_type_bit[31 - right_type_num] != 0)
		{
			for (int bit = 0; bit < 8; bit++)
			{
				...
				{
					int feetype = right_type_num * 8 + bit;
					fee_rate_card_info->fee_right_num[feetype].right_flag = 1;
					fee_rate_card_info->fee_right_num[feetype].right_num = fee_rate_data_buf[offset];
					offset += 1;
					for (int count = 0; count < fee_rate_card_info->fee_right_num[feetype].right_num; count++)
					{
						memcpy(&(fee_rate_card_info->fee_right_num[feetype].fee_rate[count]), fee_rate_data_buf + offset, sizeof(BYTE) * 3);
						offset += 3;
					}
					sprintf(msg,"读取卡信息，类别[%d]费率[%d]",feetype,
						fee_rate_card_info->fee_right_num[feetype].right_num);
					DEBUG_CARD_MSGOUT(1,msg);
//					offset += 1;
				}			
			}
		}
...
//	memcpy(buf + 8, auth_key, sizeof(BYTE) * 8);
	...
	// 收费扇区设置
	memset(buf, 0x00, sizeof(buf) - 1);
	DEBUG_CARD_MSGOUT(1000,"开始设置卡权限分组...");
	for (right_type_num = 0; right_type_num < 256; right_type_num++)
	{
		if (0 == fee_rate_card_info->fee_right_num[right_type_num].right_flag)
			continue;
		...
		sprintf(msg,"收费类别[%d]费率个数[%d]",right_type_num,right_num);
	...
	block = 2;															// block = 2的时候进来
	int block_total_num = fee_rate_data_length/16 +1;
	while (block_num < block_total_num)				// 计算出来的扇区块数目进行比较
	{
		if (Login_Collect_Card(sect) != 0)
		{
			return E_CARD_LOGIN_ERROR;
...
int __stdcall RefineCardSector(int sector, unsigned char login_key[7])
{
	...
	ret = WriteCardData(sector, PASSWD_BLOCK_NUM, sect_data);

	if ((ret != -1) && (ret != 0)) 
	{
		DEBUG_CARD_MSGOUT(ret, "回收固定扇区失败");
		return ret; 
	}
...
int __stdcall ModifyPubSector(int sector, unsigned char login_key[7], DataInfoEx *data_info_ext)
{
	...
	if (LoginCardSect(sector, CARD_PUB_KEYA
		,CARD_KEY_A, card_type) == 0)
	{
		key_pos = 4;
	}
	else if (LoginCardSect(sector, card_key[2].key
		,card_key[2].flag, card_type) == 0)
	{
		key_pos = 2;
	}
	...
	{
		return KS_ERR_WRTIEDPUBSECT;
	}

	// 扇区0
	memcpy(ucBuffer, data_info_ext->stuemp_no, 16);
	if (ret = WriteCardData(sector, 0, ucBuffer)) { return ret; }
	
	// 扇区1
	memset(ucBuffer, 0, sizeof(ucBuffer));
	memcpy(ucBuffer, data_info_ext->stuemp_no + 16, 4);
	memcpy(ucBuffer, data_info_ext->cut_name, 8);
	if (ret = WriteCardData(sector, 1, ucBuffer)) { return ret; }

	// 扇区3
	memset(sect_data, 0, sizeof(sect_data));
	// key A, 这里可以在回收的时候写入KEYA
	memcpy(sect_data, CARD_PUB_KEYA, CARD_KEY_LEN);
	// control bit
	memcpy(sect_data + CARD_KEY_LEN, CARD_FOR14_CTRL_BIT, sizeof(CARD_INIT_CTRL_BIT));
	// key B
	memcpy(sect_data + CARD_KEY_LEN + sizeof(CARD_FOR14_CTRL_BIT), CARD_PUB_KEYA, CARD_KEY_LEN);
...
int __stdcall YKTSVR_DepositHandin(int card_info[3],int out_depinfo[3])
{
	if(!GW_DepositHandin)
		return 0;
	return GW_DepositHandin(card_info,out_depinfo);
}

int __stdcall YKTSVR_DepositHandback(int card_info[3],int out_depinfo[3])
{
	if(!GW_DepositHandback)
		return 0;
	return GW_DepositHandback(card_info,out_depinfo);
}

int __stdcall YKTSVR_DepositBackoff(int card_id,int serial_no)
{
	if(!GW_DepositBackoff)
		return 0;
	return GW_DepositBackoff(card_id,serial_no);
}
