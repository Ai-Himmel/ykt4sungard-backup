...
//		if (ret = F847173(g_gw_handle, physical_no, card_info))
		if (ret = F847173(g_gw_handle, physical_no, &g_cardinfo))
			return ret;
		if(card_info)
...
int __stdcall GW_GetCardInfo_Param(const char *param_name,char *value,int maxlen)
{
	if(strcmp(param_name,"card_id")==0)
		sprintf(value,"%d",g_cardinfo.card_id);
	
	else if(strcmp(param_name,"customer_id")==0)
		sprintf(value,"%d",g_cardinfo.customer_id);

	else if(strcmp(param_name,"physical_no")==0)
		strncpy(value,g_cardinfo.physical_no,maxlen);
	
	else if(strcmp(param_name,"state_id")==0)
		strncpy(value,g_cardinfo.state_id,maxlen);

	else if(strcmp(param_name,"password")==0)
		strncpy(value,g_cardinfo.password,maxlen);
	else
		return -1;
...
int F847175(int transcode,int serial_no,int card_info[3],int out_depinfo[3] = NULL)
{
	int ret = 0;
	char fvalue[1024] = "";
	double in_bala = card_info[2]/100;
	if ((ret = KBDCBeginRequest(g_gw_handle,847175,g_msg)) < 0)
		return GW_ERR_REQUESTFUNCFAILED;

	itoa(transcode,fvalue,10);
	KBDCSetFieldValue(g_gw_handle,"lvol3",fvalue);
	itoa(card_info[0],fvalue,10);
	KBDCSetFieldValue(g_gw_handle, "lvol0", fvalue);
	itoa(card_info[1],fvalue,10);
	KBDCSetFieldValue(g_gw_handle, "lvol10", fvalue);	
	itoa(serial_no,fvalue,10);
	KBDCSetFieldValue(g_gw_handle, "lserial0", fvalue);	
	sprintf(fvalue,"%.2f",in_bala);	//分转换为元传到后台
	//itoa(card_info[2],fvalue,10);
	KBDCSetFieldValue(g_gw_handle, "damt1", fvalue);
	...
	if ((ret = KBDCGetRetCode(g_gw_handle)) == 0)
	{
		...
		if (out_depinfo)
		{
			KBDCGetFieldValue(g_gw_handle, "lserial0", fvalue, g_msg);
			out_depinfo[0] = atoi(fvalue);
			KBDCGetFieldValue(g_gw_handle, "lvol4", fvalue, g_msg);
			out_depinfo[1] = atoi(fvalue);
			KBDCGetFieldValue(g_gw_handle, "lvol5", fvalue, g_msg);
			out_depinfo[2] = atoi(fvalue);
		}
	}		
...
int __stdcall GW_CheckDeposit(int card_id)
{
	if (!g_handle_flag)
		return GW_ERR_HANDLENOTCREATE;
	int card_info[3] = {0,0,0};
	card_info[0] = card_id;
	return F847175(10,0,card_info);	 
}
...
int __stdcall GW_DepositHandin(int card_info[3],int out_depinfo[3])
{
	...
	return F847175(11,0,card_info,out_depinfo);
}
...
int __stdcall GW_DepositHandback(int card_info[3],int out_depinfo[3])
{
	...
	return  F847175(13,0,card_info,out_depinfo);	
}
...
int __stdcall GW_DepositBackoff(int card_id,int serial_no)
{
	...
	int card_info[3] = {0,0,0};
	card_info[0] = card_id;
	return F847175(12,serial_no,card_info);
}

/*
int F847175(KBDCHandle handle,int card_id)
{
	int ret = 0;
	int transcode = 10;
	char fvalue[1024] = "";
