...
void CTestDemoDlg::OnBnClickedCheckdeposit()
{
	int ret = 0;
	/*
	int cardno = 0;
	char *field_name = "cardno";
	unsigned char field_value[10]="";
	ret = SMT_GetCardRecord();
	if(ret)
		return ret;
	ret = SMT_ReadCardInfoExA(field_name,field_value);
	if(ret)
		return ret;
	cardno = atoi(field_value);
	ret = YKTSVR_CheckDeposit(cardno);
	*/
	CString cs;
	while (1)
	{
		ret = YKTSVR_CheckDeposit(9619);
		cs.Format("返回值 %d\n", ret);	
		GetDlgItem(IDC_DEPOSIT)->SetWindowText(cs);
		AfxMessageBox(cs);
	}	
	//AfxMessageBox(cs);
	return;
