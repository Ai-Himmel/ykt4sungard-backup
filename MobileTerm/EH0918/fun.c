...
void discernID_task()
{
	/*
        ...
        */
        if ( !readCpu7Card(cpucardReadcard) ) {
		DispStr_CE(0,14,"按任意键返回",DISP_CENTER);
		delay_and_wait_key( 3, EXIT_KEY_ALL, 0 );
	}
...
void adminPsd_init()
{
	...
	if ( *psdctrl != 0xA5 ) {	//需要初始化
		*psdctrl = 0xA5;
		...
		|| eeprom_write("12345678", 16, EEPROM_OFFSET_SEVPASSWORD) || eeprom_write("00000000", 8, EEPROM_OFFSET_PHYCNT) ) {
			DispStr_CE(0, 6, "EEPROM异常", DISP_CENTER|DISP_CLRSCR);
			DispStr_CE(0, 10, "按任意键继续", DISP_CENTER);
			delay_and_wait_key( 0, EXIT_KEY_ALL, 0 );
			return;
...
{
	if ( !consumpionNM() ) {
		//DispStr_CE(0,12,"消费成功",DISP_CENTER|DISP_CLRSCR);
		return;
	}
	else {
		DispStr_CE(0,12,"消费不成功",DISP_CENTER|DISP_CLRSCR);
	}
	DispStr_CE(0,14,"按任意键返回",DISP_CENTER);
	delay_and_wait_key( 3, EXIT_KEY_ALL, 0 );
}
