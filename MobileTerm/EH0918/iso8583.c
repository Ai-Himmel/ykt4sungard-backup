...
void hex2dec(const char *sHexStr,int iHexLen,unsigned char *uDecBuf,int* iDecBUfLen)	//压缩
{
	int i=0;
	unsigned long ul;
	char sHexTmp[3];
...
{
			memcpy(aBuffer, "001B", 4);	//长度为27（(58-4) /2）
			memcpy(aBuffer+4, "60", 2);
			memcpy(aBuffer+6, "0031", 4);	//目的
			memcpy(aBuffer+10, "0000", 4);	//源
			memcpy(aBuffer+14, "0800", 4);	//消息类型
			memcpy(aBuffer+18, "0000000000000008", 16);	//位图域
