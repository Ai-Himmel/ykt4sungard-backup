...
****************************************************************************/
#include "in_call.h"
USER_INFO SchindlersList[NUM_OF_FUDAN] = {
	{"YKT027", 0},
};
void initList()
{
	...
	for (i = 1; i <= 300; i++) {
		sprintf(num+3, "%03d", i);
		strcpy(SchindlersList[i].num, num);
		//debugFun(num);
	} 
}
...
char discID_fudan()
{
	...
	ret = CardMFCAuthKey(PICC_AUTHENT1A, PICCsnr, keya_0, 15*4 + 3);	//认证第15扇区
	if (ret != RCX_OK) {
		return -1;
	}
	if (ret == RCX_OK) {
		//读取学号，第15扇区第0块和第1块前4字节是学号
		sectno = 15;
		blockno = sectno*4+0;
		ret = CardMFCRead16Bytes(blockno,rdata);
		memcpy(num, rdata, 6);
		...
		//memcpy(num+16, rdata, 4);
		//debugFun(num);
		sprintf(dispBuf[0], "编号: %s", num);
	}
	if (ret == RCX_OK) {
		//读取学号，第15扇区第1块的后5-13字节是姓名
		sectno = 15;
		blockno = sectno*4+1;
		ret = CardMFCRead16Bytes(blockno,rdata);
		memcpy(name, rdata+4, 8);
		//debugFun(name);
		sprintf(dispBuf[1], "姓名: %s", name);
	}
	
	for (numofRecord = 0; numofRecord < NUM_OF_FUDAN; numofRecord++) {
		if ( !strncmp( num, SchindlersList[numofRecord].num, 6 ) ) {
			strcpy(str, "√");
			//debugFun("num");
			isIn = 1;
			break;
	...
	//debugFun(SchindlersList[numofRecord].num);
	sprintf(dispBuf[3], "备注: %s", str);
	sprintf(dispBuf[2], "次数: %d", 1);

	count = DB_count_records(0);
	for (i = 0; i < count; i++) {
		puserinfo = (USER_INFO*)DB_jump_to_record(0, i, &flag);
		if ( !flag ) {
			if ( !strncmp(num, puserinfo->num, 6) ) {
				isCnt = 1;
				//debugFun("fuckfuck");
				strncpy(tmp.num, puserinfo->num, 6);
				strncpy(tmp.count, puserinfo->count, 8);
			}
		}
	}
	if ( !isCnt ) {	//第一次刷卡
		strcpy(tmp.num, num);
		tmpint = 1;
		set_4byte_int(tmpstr, tmpint);
		mydec2hex(tmpstr, 4, tmp.count);
		//debugFun(tmp.count);
	}
	else {
		myhex2dec(puserinfo->count, 8, tmpstr, &tmplen);
		tmpint = get_4byte_int(tmpstr);
		tmpint++;
		sprintf(dispBuf[2], "次数: %d", tmpint);
		memset(tmpstr, 0x00, sizeof(tmpstr));
		set_4byte_int(tmpstr, tmpint);
		mydec2hex(tmpstr, 4, tmp.count);
		//debugFun(tmp.count);
	}
	ret = DB_add_record(ATTENDANCE_INDEX,  (void *)(&tmp));
	if ( !ret ) {
		DispStr_CE(0,12,"记录添加失败",DISP_CENTER|DISP_CLRSCR);
		return -1;
	}
	DispStr_CE(50,4,dispBuf[0],DISP_POSITION|DISP_CLRSCR);
	DispStr_CE(50,6,dispBuf[1],DISP_POSITION);
	DispStr_CE(50,8,dispBuf[2],DISP_POSITION);
	DispStr_CE(50,10,dispBuf[3],DISP_POSITION);
	return 0;
}
/***********************************************************************
		module		:	[M1卡读写]
