		...
		try {
			...
			Query query = session
					.createSQLQuery(
							" Select a.device_id as {serialost.deviceId},"
									+ "sum(cast(a.serial_no as float)) as {serialost.truth},"
									+ "(((max(cast(a.serial_no as float))+min(cast(a.serial_no as float)))*(max(cast(a.serial_no as float))-min(cast(a.serial_no as float))+1))/2) as {serialost.logic}, "
									...
									+ "having (sum(cast(a.serial_no as float))<>(((max(cast(a.serial_no as float))+min(cast(a.serial_no as float)))*(max(cast(a.serial_no as float))-min(cast(a.serial_no as float))+1))/2)) "
									+ "and (max(a.serial_no)<65535) "
									+ "and ((max(a.serial_no)-min(a.serial_no)+1)-count(a.serial_no))>0 order by {serialost.diff} ")
					...
					Query queryCheck = session
							.createSQLQuery(
									"select {rcvdtl.*} from ykt_cur.t_tif_rcvdtl rcvdtl "
											...
											+ "order by rcvdtl.tx_date desc,rcvdtl.tx_time desc ")
					...
					Query queryTwo = session
							.createSQLQuery(
									"select a.serial_no as serialNo "
											+ "from YKT_CUR.T_TIF_RCVDTL a where concat(a.tx_date,a.tx_time)>:startTime "
											+ "and not concat(a.tx_date,a.tx_time)>:endTime "
											+ "and (a.tx_date=:dateTime or a.tx_date=:dateTime1) "
											+ "and a.device_id=:deviceId order by a.serial_no ")
			...
			for (int i = 0; i < listTwo.size(); i++) {
				...
				Query queryThree = session
						.createSQLQuery(
								"select concat(tx_date,tx_time) dateTime "
