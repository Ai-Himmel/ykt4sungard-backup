			...
			while (iter.hasNext()) {
				...
				if ((0 > DateUtil.checkTimeSequence(serverTime, devMonitor
					...
					Query queryOne = session
							.createSQLQuery(
									"select {a.*} from ykt_cur.t_pif_device a "
					...
					if (iterOne.hasNext()) {
						...
						if (true == recordOrNot) {
							...
							Query devquery = session.createSQLQuery(
									"select dev.DEVPHY999_ID devphy999Id from YKT_CUR.T_PIF_DEVICE dev "
											+ "where dev.device_id="
...
//							mesList.setDevid(device.getDeviceId());
//							mesList.setFunid(Integer.valueOf("930001"));
//							mesList.setIncontent("<CLOCK>"
//									+ OracleTimeUtil.getSysDate(0).substring(2)
//									+ "</CLOCK>");
