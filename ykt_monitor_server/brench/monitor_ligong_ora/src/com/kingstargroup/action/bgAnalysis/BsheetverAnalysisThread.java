			...
			Query query = session
					.createSQLQuery(
							"select {rcvdtl.*} from ykt_cur.t_tif_rcvdtl  rcvdtl "
									+ "where rcvdtl.status='7' and rcvdtl.col_timestamp>:startTime and rcvdtl.col_timestamp<=:endTime ")
				...
				Query queryDiv = session
						.createSQLQuery(
								"select b.card_id  cardId "
										...
										+ "and b.card_id=:cardId and concat('20',b.volume)<=:txTime ")
				...
				Query queryDiv = session
						.createSQLQuery(
								"select b.card_id  cardId "
										...
										+ "and card_id=:cardId and not concat('20',b.volume)>:txTime ")
						.addScalar("cardId", Hibernate.INTEGER);
				queryDiv.setString("txTime", rcvdtl.getId().getTxDate()
						+ rcvdtl.getId().getTxTime());
				queryDiv.setInteger("cardId", rcvdtl.getId().getCardNo()
