...
public class DeviceStatusAnalysisThread extends Thread implements
	...
	// default analysis cycle is ten mins
	private double cycle = 10.0;

	// inteval param
	private int doubleCycle = 0;
	
	//default timeinteval is 1 min
	private double timeInteval = 1.5;
	
	private int intTimeInteval = 0;
	...
	private void getParameter(Session session) {
		...
		while (iterParam.hasNext()) {
			...
			if("TimeInterval".equals(mapParamTable.getParameter())){
				this.timeInteval = mapParamTable.getValue().doubleValue();
			}
		}
		String s = String.valueOf(cycle * 60);
		this.doubleCycle = Integer.parseInt(s.substring(0, s.indexOf('.')));
		String t = String.valueOf(timeInteval);
		this.intTimeInteval = Integer.parseInt(t.substring(0,t.indexOf('.')));
		// // contact list
	...
	public void anaylize() {
		...
		try {
			...
			String serverTime = DateUtil.getNewTime(DB2TimeUtil.getSysDate(0),-intTimeInteval*60);
			Query query = session
					.createSQLQuery(
							"select {dev.*} from YKT_CUR.T_PIF_DEVICE dev,YKT_CUR.T_MIF_DEVMONITOR a "
									+ "where a.device_id=dev.device_id and dev.state_id<>5 and a.op_time<:serverTime with ur ")
					.addEntity("dev", TPifDevice.class);
			query.setString("serverTime",serverTime.substring(2));
			...
			while (iter.hasNext()) {
				...
				if (true == recordOrNot) {
					...
					Query devquery = session.createSQLQuery("select dev.DEVPHY999_ID as devphy999Id from YKT_CUR.T_PIF_DEVICE as dev " +
							"where dev.device_id="+device.getFdevId()).addScalar("devphy999Id",Hibernate.STRING);
