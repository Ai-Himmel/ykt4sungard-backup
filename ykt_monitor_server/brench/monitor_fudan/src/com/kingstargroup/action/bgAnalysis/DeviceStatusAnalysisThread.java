		...
		try {
			...
			Query query = session
					.createSQLQuery(
							"select {dev.*} from YKT_CUR.T_PIF_DEVICE dev,YKT_CUR.T_MIF_DEVMONITOR a "
									+ "where a.device_id=dev.device_id and dev.state_id<>5 and dev.fdevphy_id<>'FFFFFFFF' "
									+ "and not dev.fdevphy_id='' and a.beatstate=1 with ur ")
			...
			while (iter.hasNext()) {
				...
				if (true == recordOrNot) {
					...
					queryContact.setString("monitorNo", device.getFdevphyId());
