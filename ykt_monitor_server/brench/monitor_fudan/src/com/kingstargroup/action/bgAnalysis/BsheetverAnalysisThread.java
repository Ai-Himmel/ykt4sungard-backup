		...
		try {
			...
			Query query = session
					.createSQLQuery(
							"select {rcvdtl.*} from ykt_cur.t_tif_rcvdtl as rcvdtl "
									+ "where rcvdtl.status='7' and rcvdtl.col_timestamp>:startTime and rcvdtl.col_timestamp<=:endTime with ur")
			...
			while (iter.hasNext()) {
				...
				if (true == recordOrNot) {
					...
					seriExcept.setAbnormaltype(Short.valueOf("51"));
			...
			Query queryOne = session
					.createSQLQuery(
							"select {rcvdtl.*} from ykt_cur.t_tif_rcvdtl as rcvdtl "
									+ "where rcvdtl.status<>'7' and rcvdtl.err_code=390000 "
				...
				Query queryDiv = session
						.createSQLQuery(
								"select b.card_id as cardId "
										+ "from YKT_CUR.V_BLKLST as b where b.is_ad=1 "
										+ "and b.card_id=:cardId and concat('20',b.volume)<=:txTime with ur")
			...
			while (iterThree.hasNext()) {
				...
				Query queryDiv = session
						.createSQLQuery(
								"select b.card_id as cardId "
										+ "from YKT_CUR.V_BLKLST as b where b.is_ad=0 "
										+ "and card_id=:cardId and not concat('20',b.volume)>:txTime with ur")
						.addScalar("cardId", Hibernate.INTEGER);
				queryDiv.setString("txTime", rcvdtl.getId().getTxDate()
						+ rcvdtl.getId().getTxTime());
				queryDiv.setInteger("cardId", rcvdtl.getId().getCardNo()
				...
				if (iterDiv.hasNext()) {
					if (true == recordOrNot) {
						...
						seriExcept.setTxMark(rcvdtl.getTxMark());
						seriExcept.setContactname(contactName);
						seriExcept.setAbnormaltype(Short.valueOf("53"));
						session.save(seriExcept);
					}
					mailRecord++;
					if (!"".equals(mailList)) {
