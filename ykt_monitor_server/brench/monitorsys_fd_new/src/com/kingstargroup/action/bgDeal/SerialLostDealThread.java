...
public class SerialLostDealThread extends Thread implements
	...
	public void anaylize() {
		...
		try {
			...
			Query query = session
					.createQuery("from MapSerialostTable a where a.id.analysistime>=:startTime "
							+ "and a.id.analysistime<:endTime and a.serialReason=7 and a.serialStatus=0");
			...
			while (iter.hasNext()) {
				...
				// a.id.txCode=930031 ");
				Query queryTwo = session.createSQLQuery(
						"select a.serial_no serialNo "
								+ "from ykt_cur.t_tif_rcvdtl a "
								+ "where a.serial_no=:serialno "
								+ "and (a.tx_date=:txdate1 or a.tx_date=:txdate2 or a.tx_date=:txdate3) "
				...
				if (listTwo.size() > 0) {
					...
					Query queryOne = session.createSQLQuery(
							"select a.subsystem_id subsystemId from ykt_cur.t_pif_device a "
									+ "where a.devphy999_id=:devId and a.state_id <> 5 with ur")
					...
					if (listOne.size() > 0) {
						if("0201".equals(DeviceNameDictionary.getInstance().getType(mapSerialostTable.getId().getDeviceId()))){
							...
							mesList.setDeviceId(Integer.valueOf(listOne.get(0)
									.toString()));
							mesList.setDevphy999Id(mapSerialostTable.getId()
									.getDeviceId());
							mesList.setFunid(Integer.valueOf("930018"));
...
//			e.printStackTrace();
	...
	public void terminate() {
		...
		if (this.isAlive()) {
			...
			SerialLostDealThread.requestStop = true;
		} else {
