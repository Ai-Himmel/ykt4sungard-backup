...
public class ExceptSerilAnalysisThread extends Thread implements
	...
	public void anaylize() {
		...
		try {
			...
			// query.setString("curDate", DB2TimeUtil.getSysDate(0));

			Query firstquery = session.createSQLQuery("select {t.*} from " +
					"(select device_id as deviceId,sum(serial_no) " +
					"over (PARTITION BY device_id order by tx_date,tx_time rows  " +
					"between 1 preceding and 0 following)- serial_no as formerNo,serial_no as nowNo, " +
					"sum(serial_no) over (PARTITION BY device_id " +
					"order by tx_date,tx_time rows between 0 preceding and 1 following)- serial_no as latterNo, " +
					"tx_date as txDate,tx_time as txTime from YKT_CUR.T_TIF_RCVDTL " +
					"where col_timestamp>=:startTime and col_timestamp<=:endTime and serial_no<=999999) t " +
					"where ((nowNo-formerNo>50 and latterNo-nowNo<-50) or (nowNo-formerNo<-50 and latterNo-nowNo>50))").addEntity("t", DeviceExceptRt.class);
			...
			while(firstIter.hasNext()){
				...
				Query queryThree = session.createSQLQuery(
						"select {a.*} from ykt_cur.t_tif_rcvdtl as a "
								...
								+ "and a.col_timestamp<:endTime with ur ")
						.addEntity("a", TTifRcvdtl.class);
				queryThree.setString("deviceId", rt.getDeviceId());
				queryThree.setInteger("serialNo", rt.getNowNo()
						.intValue());
				...
				while (iterThree.hasNext()) {
					...
					if (true == recordOrNot) {
						...
						seriExcept.setAbnormaltype(Short.valueOf("206"));
			...
			Query queryFour = session
					.createSQLQuery(
							"select {a.*} from YKT_CUR.T_TIF_RCVDTL a,"
									...
									+ "and (a.tx_code=930031 or a.tx_code=930034) and a.err_code=0 "// 交易、冲正、充值流水
									+ "and (b.tx_code=930031 or b.tx_code=930034) and b.err_code=0 "// 排除黑卡消费
									+ "and a.out_bala<>b.in_bala "
									+ "order by a.card_no,a.total_cnt with ur")
					.addEntity("a", TTifRcvdtl.class);
			...
			Query queryOne = session.createSQLQuery(
					"select {rcvdtl.*} from ykt_cur.t_tif_rcvdtl rcvdtl "
							...
							+ "and rcvdtl.err_code=100172 with ur ").addEntity(
