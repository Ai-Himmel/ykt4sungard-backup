...
public class DeviceNameDictionary {
	...
	private void initDictionary(){
		...
		try {
			...
			List devList = null;
			Query dev_query = session.createSQLQuery("select {dev.*} from ykt_cur.t_pif_device dev where dev.state_id<>5 with ur").addEntity("dev",TPifDevice.class);
			...
			while(dev_iter.hasNext()){
				...
				HashMap_deviceType.put(device.getDev999Id(), device.getDevtype());
			}
