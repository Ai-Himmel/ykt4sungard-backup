...
public class SerialLostCkAnalysisThread extends Thread implements
	...
	public void initParam() {
		if (logger.isInfoEnabled()) {
			logger.info("**************流水缺失回归分析线程初始化**************"); //$NON-NLS-1$
		}
		...
		try {
			...
			logger.error("流水缺失回归分析线程初始化失败！", e); //$NON-NLS-1$
		...
		if (logger.isInfoEnabled()) {
			logger.info("**************流水缺失回归分析线程初始化成功**************"); //$NON-NLS-1$
		}
	}
	...
	public void anaylize() {
		if (logger.isInfoEnabled()) {
			logger.info("流水缺失回归分析线程开始分析任务"); //$NON-NLS-1$
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger
						.info("流水缺失回归分析线程开始分析时间:" + DateUtil.getNow("yyyy/MM/dd/HH:mm:ss")); //$NON-NLS-1$
			}
			if (1 > DateUtil.checkTimeSequence(endTime, DateUtil.getNow())) {
				if (logger.isInfoEnabled()) {
					logger.info("流水缺失回归分析线程分析时间段：" + startTime + " -- "
							+ endTime);
				}
			} else {
					...
					Query queryCheck = session
							.createSQLQuery(
									"select {rcvdtl.*} from ykt_cur.t_tif_rcvdtl rcvdtl "
											...
											+ "and concat(rcvdtl.tx_date,rcvdtl.tx_time)>=:startTime "
											+ "and concat(rcvdtl.tx_date,rcvdtl.tx_time)<=:endTime "
											+ "and rcvdtl.tx_date>=:startDate and rcvdtl.tx_date<=:endDate "
					...
					while (returnIter.hasNext()) {
						...
						if (listCheck.size() == 0) {
							...
							if (!CheckSerialost(session, serialLost
								...
								if (iterOne.hasNext()) {
									// insert into serialosttable
									TTifRcvdtl rcvdtl = (TTifRcvdtl) iterOne
											.next();
									if (true == recordOrNot) {
										MapSerialostTable mapSerialost = new MapSerialostTable();
										MapSerialostTableKey mapSerialostKey = new MapSerialostTableKey();
			...
			Query queryTwo = session
					.createSQLQuery(
							"Select distinct a.device_id deviceId from YKT_CUR.T_TIF_RCVDTL a "
									+ "where concat(a.tx_date,a.tx_time)>=:startTime "
									+ "and concat(a.tx_date,a.tx_time)<=:endTime "
									+ "and a.tx_date>=:startDate and a.tx_date<=:endDate "
									+ "and a.serial_no=65535 with ur")
			...
			for (int i = 0; i < listTwo.size(); i++) {
				...
				Query queryThree = session
						.createSQLQuery(
								"select concat(a.tx_date,a.tx_time) dateTime "
										+ "from YKT_CUR.T_TIF_RCVDTL a "
										+ "where concat(a.tx_date,a.tx_time)>=:startTime "
										+ "and concat(a.tx_date,a.tx_time)<=:endTime "
										+ "and a.tx_date>=:startDate and a.tx_date<=:endDate "
				...
				if ((returnSerialNo != null) && (returnSerialNo.size() < 50)) {// 假设五十条以上的都有问题，属于流水突变的范畴
					...
					while (returnIter.hasNext()) {
						...
						queryCheck.setString("startTime", startTime);
						queryCheck.setString("endTime", endTime);
						queryCheck.setString("startDate", startTime.substring(0, 8));
						queryCheck.setString("endDate", endTime.substring(0, 8));
						List listCheck = queryCheck.list();
						if (listCheck.size() == 0) {// can not find this serial
							// the serial is lost really
							if (!CheckSerialost(session, deviceId, serialNo)) {
								...
								queryFive.setString("startDate", startTime.substring(0, 8));
								queryFive.setString("endDate", endTime.substring(0, 8));
								List listFive = queryFive.list();
								Iterator iterTwo = listFive.iterator();
								if (iterTwo.hasNext()) {
									// insert into serialosttable
									TTifRcvdtl rcvdtl = (TTifRcvdtl) iterTwo
											.next();
									if (true == recordOrNot) {
										MapSerialostTable mapSerialost = new MapSerialostTable();
										MapSerialostTableKey mapSerialostKey = new MapSerialostTableKey();
					...
					while (returnIter.hasNext()) {
						...
						if (listCheck.size() == 0) {// can not find this serial
							...
							if (!CheckSerialost(session, deviceId, serialNo)) {
								...
								querySeven.setString("startDate", startTime.substring(0, 8));
								querySeven.setString("endDate", endTime.substring(0, 8));
			...
			logger.error("流水缺失回归分析线程出现以下异常：", e); //$NON-NLS-1$
		...
		try {
			...
			logger.error("流水缺失回归分析线程出现以下异常：可能被强行停止！ ", e); //$NON-NLS-1$
		} catch (Exception e) {
			logger.error("流水缺失回归分析线程出现以下异常：可能被强行停止！ ", e); //$NON-NLS-1$
		}
	}
	...
	private boolean CheckSerialost(Session session, String deviceId,
		...
		try {
			...
			queryHasFound.setInteger("serialNo", serialNo);
	...
	public void terminate() {
		// TODO Auto-generated method stub
		if (this.isAlive()) {
			logger.warn("流水缺失回归分析线程状态被请求，此时线程状态：活动线程!"); //$NON-NLS-1$
			this.requestStop = true;
		} else {
			logger.warn("流水缺失回归分析线程状态被请求，此时线程状态：死亡线程!"); //$NON-NLS-1$
			this.destroy();
	...
	public void sendMail() {
		...
		if ((true == mailOrNot) && (!"".equals(mailList))
				&& (0 != buffer.size())) {
			if (logger.isInfoEnabled()) {
				logger.info("流水缺失回归分析线程发送邮件给联系人:" + contactName);
			}
			String tmpStr = "";
			if (buffer.size() > 5000) {
	...
	public void sendSMG() {
		...
		if ((true == smgOrNot) && (!"".equals(phoneNumber)) && mailRecord > 0) {
			if (logger.isInfoEnabled()) {
				logger.info("流水缺失回归分析线程发送短信给联系人:" + contactName);
			}
			String[] tempList = null;
			String[] nameList = null;
