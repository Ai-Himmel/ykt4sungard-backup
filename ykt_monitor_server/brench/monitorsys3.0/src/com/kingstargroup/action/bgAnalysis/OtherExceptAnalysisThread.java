 ...
 * File name: OtherExceptAnalysisThread.java
 * Description: 其他异常流水检查类
 * Modify History（或Change Log）:  
 * 操作类型（创建、修改等）   操作日期       操作者             操作内容简述
 * 创建  				 2005-10-19    何林青            其他异常流水检查类
 * 修改					 2006-07-20    何林青			    日志记录方式修改
 * <p>
 *
 * @author      何林青
 * @version     1.0
...
public class OtherExceptAnalysisThread extends Thread implements
		AnalyzeThreadInterface {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger
			.getLogger(OtherExceptAnalysisThread.class);
	...
	// default analysis is to analysis all
	...
	public void initParam() {
		...
		try {
			...
			queryCycle.setString("paramRefer", "OtherCycle");
			...
			logger.error("其他异常流水分析线程初始化失败！", e); //$NON-NLS-1$
		} finally {
			HibernateSessionFactory.closeSession();
		}
		if (logger.isInfoEnabled()) {
			logger.info("**************其他异常流水分析线程初始化成功**************"); //$NON-NLS-1$
		}
	}
	...
	private void getParameter(Session session) {
		...
		Query queryOrNot = session
				.createQuery("from MapAlertTable alert where alert.parameter='OtheRecordException'");
		...
		Query queryContact = session
				.createQuery("from MapContactMonitorTable monitor where monitor.monitorno='OTHER_EXCEPTION'");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.kingstargroup.commons.AnalyzeThreadInterface#anaylize()
	 */
	public void anaylize() {
		if (logger.isInfoEnabled()) {
			logger.info("其他异常流水分析线程开始分析任务"); //$NON-NLS-1$
		}
		Transaction tx = null;
		try{
		try {
			...
			if (logger.isInfoEnabled()) {
				logger
						.info("其他异常流水分析线程开始分析时间:" + DateUtil.getNow("yyyy/MM/dd/HH:mm:ss")); //$NON-NLS-1$
			}
			if (1 > DateUtil.checkTimeSequence(endTime, DateUtil.getNow())) {
				if (logger.isInfoEnabled()) {
					logger.info("其他异常流水分析线程分析时间段：" + startTime + " -- "
							+ endTime);
				}
			} else {
			...
			BgMonitor
					.monitorThreadStatusReport(
							"com.kingstargroup.action.bgAnalysis.OtherExceptAnalysisThread",
							"其他异常流水分析线程", DateUtil
									.getNow("yyyy/MM/dd/HH:mm:ss"), String
									.valueOf(this.doubleCycle), "运行中...");
			Query query = session.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.OtherExceptAnalysisThread.getotherexcept"))
			...
			while (iter.hasNext()) {
				...
				if (true == recordOrNot) {
					...
					seriExcept.setAbnormaltype(Long.parseLong("6"));
				...
				if (!"".equals(mailList)) {
					...
					Query queryCustomer = session
							.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.OtherExceptAnalysisThread.getCutName"))
			...
			if (tempAnalysis == null) {
				...
				analysisPoint.setAnalysisname("OTHER_EXCEPTION");
			...
			logger.error("其他异常流水分析线程出现以下异常：", e); //$NON-NLS-1$
		...
		try {
			...
			logger.error("其他异常流水分析线程出现以下异常：可能被强行停止！ ", e); //$NON-NLS-1$
		} catch (Exception e) {
			logger.error("其他异常流水分析线程出现以下异常：可能被强行停止！ ", e); //$NON-NLS-1$
		}
	}
	...
	public void terminate() {
		if (this.isAlive()) {
			logger.warn("其他异常流水分析线程状态被请求，此时线程状态：活动线程!"); //$NON-NLS-1$
			requestStop = true;
		} else {
			logger.warn("其他异常流水分析线程状态被请求，此时线程状态：死亡线程!"); //$NON-NLS-1$
			this.destroy();
	...
	public void sendMail() {
		...
		if ((true == mailOrNot) && (!"".equals(mailList))
				&& (0 != buffer.size())) {
			if (logger.isInfoEnabled()) {
				logger.info("其他异常流水分析线程发送邮件给联系人:" + contactName);
			}
			String tmpStr = "";
			if (buffer.size() > 5000) {
	...
	public void sendSMG() {
		...
		if ((true == smgOrNot) && (!"".equals(phoneNumber)) && mailRecord > 0) {
			if (logger.isInfoEnabled()) {
				logger.info("其他异常流水分析线程发送短信给联系人:" + contactName);
			}
			String[] tempList = null;
			String[] nameList = null;
