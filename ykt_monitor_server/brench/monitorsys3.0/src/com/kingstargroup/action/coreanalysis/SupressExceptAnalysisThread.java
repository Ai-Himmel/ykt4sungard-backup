...
public class SupressExceptAnalysisThread extends Thread implements
	...
	public void anaylize() {
		...
		try {
			...
			Query query = session
					.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.SupressExceptAnalysisThread.getsupressDeviceId"))
			...
			for (int i = 0; i < list.size(); i++) {
				...
				Query queryOne = session
						.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.SupressExceptAnalysisThread.getcerttimedeviceserial"))
				...
				if (iterOne.hasNext()) {
					...
					if (true == recordOrNot) {						
						...
						mapSerialostKey.setDeviceId(rcvdtl.getId().getDevphyid());
						mapSerialostKey.setSerialNo(rcvdtl.getId().getDevseqno());
						mapSerialost.setId(mapSerialostKey);
						mapSerialost.setTxDate(rcvdtl.getId().getTransdate());
						mapSerialost.setTxTime(rcvdtl.getTranstime());
						mapSerialost.setContactname(contactName);
						mapSerialost.setSerialReason(Long.parseLong("8"));
						mapSerialost.setSerialStatus(Long.parseLong("0"));
					...
					if (!"".equals(mailList)) {
						...
						buffer.append(StringUtil.suppressExceptStringGenerate(
								...
								rcvdtl.getId().getTransdate(), rcvdtl.getTranstime(), recordTime, rcvdtl
										.getId().getDevseqno(),
								"SUPRESS_EXCEPTION"));
					}
				} else {
					...
					if (!"".equals(mailList)) {
						...
						buffer.append(StringUtil.suppressExceptStringGenerate(
								...
								recordTime, 0, "SUPRESS_EXCEPTION"));
					}
