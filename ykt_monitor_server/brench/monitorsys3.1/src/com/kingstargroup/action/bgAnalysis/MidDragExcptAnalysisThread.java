		...
		try {
			try {
				...
				if (1 > DateUtil.checkTimeSequence(endTime, DateUtil.getNow())) {
					if (logger.isInfoEnabled()) {
						logger.info("中途拔卡分析线程分析时间段：" + startTime + " -- "
								+ endTime);
					}
				} else {
				...
				while (iter.hasNext()) {
					...
					if (true == recordOrNot) {
						...
						seriExcept.setAbnormaltype(Long.parseLong("4"));
					...
					if (!"".equals(mailList)) {
						...
						buffer.append(StringUtil.solidStringGenerate(rcvdtl
								.getId().getDevphyid(), device_name, rcvdtl
								.getCardno(), cust_name, rcvdtl.getId()
								.getDevseqno(), rcvdtl.getId().getTransdate(),
								rcvdtl.getTranstime(), "MID_EXCEPTION"));
					}
				}
