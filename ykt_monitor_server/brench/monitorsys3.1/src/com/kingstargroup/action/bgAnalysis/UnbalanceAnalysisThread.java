...
import com.kingstargroup.form.MapCaunbalanceTableId;
...
import com.kingstargroup.form.TAccount;
...
public class UnbalanceAnalysisThread extends Thread implements
	...
	public void anaylize() {
		...
		try {
			...
			Query query = session
					.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.UnbalanceAnalysisThread.getunbalanceacc"))
					.addEntity("a", TAccount.class);
			List list = query.list();
			Iterator iter = list.iterator();
			while (iter.hasNext()) {
				TAccount account = (TAccount) iter.next();
				if (true == recordOrNot) {
					MapCaunbalanceTable caUnbalance = new MapCaunbalanceTable();
					MapCaunbalanceTableId unbalanceKey = new MapCaunbalanceTableId();
					unbalanceKey.setAccno(account.getAccno());
					unbalanceKey.setAnalysistime(recordTime);
					caUnbalance.setAccname(account.getAccname());
					caUnbalance.setAvailbal(account.getAvailbal());
					caUnbalance.setBalance(account.getBalance());
					caUnbalance.setCardno(account.getCardno());//	
					caUnbalance.setCustid(account.getCustid());
					caUnbalance.setFrozebal(account.getFrozebal());
					caUnbalance.setId(unbalanceKey);
					caUnbalance.setLastaccdate(account.getLastaccdate());
					caUnbalance.setLastcardbal(account.getLastcardbal());
					caUnbalance.setLastcardcnt(account.getLastcardcnt());
					caUnbalance.setLasttransdate(account.getLasttransdate());
					caUnbalance.setOpendate(account.getOpendate());
					caUnbalance.setPurseno(account.getPurseno());
					caUnbalance.setStatus(account.getStatus());
