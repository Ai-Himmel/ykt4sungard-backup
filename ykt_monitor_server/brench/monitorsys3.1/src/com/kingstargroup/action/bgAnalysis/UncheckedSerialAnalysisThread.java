...
public class UncheckedSerialAnalysisThread extends Thread implements
	...
	public void anaylize() {
		...
		try {
			...
			Query query = session.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.UncheckedSerialAnalysisThread.getunceckserial"))
			...
			while (iter.hasNext()) {
				...
				if (true == recordOrNot) {
					...
					seriExcept.setAbnormaltype(Long.parseLong("0"));
				...
				if (!"".equals(mailList)) {
					...
					Query queryCustomer = session
							.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.UncheckedSerialAnalysisThread.getCutName"))
							.addScalar("cutName", Hibernate.STRING);
					queryCustomer.setLong("cutId", rcvdtl.getCardno());
