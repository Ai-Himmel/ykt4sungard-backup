 ...
 * File name: MidDragExcptAnalysisThread
 * Description: 中途拔卡流水检查线程类
 * Modify History（或Change Log）:  
 * 操作类型（创建、修改等）   操作日期     操作者    操作内容简述
 * 创建  				2005-10-19   何林青    中途拔卡流水检查线程
...
public class MidDragExcptAnalysisThread extends Thread implements
		AnalyzeThreadInterface {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger
			.getLogger(MidDragExcptAnalysisThread.class);
	...
	public void initParam() {
		...
		try {
			...
			queryCycle.setString("paramRefer", "MidDrawCycle");
			...
			logger.error("中途拔卡分析线程初始化失败！", e); //$NON-NLS-1$
		} finally {
			HibernateSessionFactory.closeSession();
		}
		if (logger.isInfoEnabled()) {
			logger.info("**************中途拔卡分析线程初始化成功**************"); //$NON-NLS-1$
		}
	}
	...
	private void getParameter(Session session) {
		...
		Query queryOrNot = session
				.createQuery("from MapAlertTable alert where alert.parameter='MidDrawRecord'");
		...
		Query queryContact = session
				.createQuery("from MapContactMonitorTable monitor where monitor.monitorno='MID_EXCEPTION'");
	...
	public void anaylize() {
		if (logger.isInfoEnabled()) {
			logger.info("中途拔卡分析线程开始分析任务"); //$NON-NLS-1$
		}
		Transaction tx = null;
		try {
			Session session = HibernateSessionFactory.currentSession();
			...
			if (logger.isInfoEnabled()) {
				logger
						.info("中途拔卡分析线程开始分析时间:" + DateUtil.getNow("yyyy/MM/dd/HH:mm:ss")); //$NON-NLS-1$
			}
			if (1 > DateUtil.checkTimeSequence(endTime, DateUtil.getNow())) {
				if (logger.isInfoEnabled()) {
					logger
							.info("中途拔卡分析线程分析时间段：" + startTime + " -- "
									+ endTime);
				}
			} else {
			...
			BgMonitor
					.monitorThreadStatusReport(
							"com.kingstargroup.action.bgAnalysis.MidDragExcptAnalysisThread",
							"中途拔卡分析线程", DateUtil.getNow("yyyy/MM/dd/HH:mm:ss"),
							String.valueOf(this.doubleCycle), "运行中...");

			Query query = session.createSQLQuery(
					"select {rcvdtl.*} from ykt_cur.t_tif_rcvdtl as rcvdtl "
							+ "where rcvdtl.err_code=390002 "
							+ "and rcvdtl.col_timestamp>=:startTime "
							+ "and rcvdtl.col_timestamp<:endTime with ur")
			...
			while (iter.hasNext()) {
				...
				if (true == recordOrNot) {
					...
					seriExcept.setAbnormaltype(Short.valueOf("4"));
			...
			if (tempAnalysis == null) {
				...
				analysisPoint.setAnalysisname("MID_EXCEPTION");
			...
			logger.error("中途拔卡分析线程出现以下异常：", e); //$NON-NLS-1$
		...
		try {
			...
			logger.error("中途拔卡分析线程出现以下异常：可能被强行停止！ ", e); //$NON-NLS-1$
		} catch (Exception e) {
			logger.error("中途拔卡分析线程出现以下异常：可能被强行停止！ ", e); //$NON-NLS-1$
		}
	}
	...
	public void terminate() {
		...
		if (this.isAlive()) {
			logger.warn("中途拔卡分析线程状态被请求，此时线程状态：活动线程!"); //$NON-NLS-1$
			this.requestStop = true;
		} else {
			logger.warn("中途拔卡分析线程状态被请求，此时线程状态：死亡线程!"); //$NON-NLS-1$
			this.destroy();
	...
	public void sendMail() {
		...
		if ((true == mailOrNot) && (!"".equals(mailList))
				&& (0 != buffer.size())) {
			if (logger.isInfoEnabled()) {
				logger.info("中途拔卡分析线程发送邮件给联系人:" + contactName);
			}
			String tmpStr = "";
			if (buffer.size() > 5000) {
			...
			for (int i = 0; i < tempList.length; i++) {
				if (!"".equals(tempList)) {
					CommonSendMailUtil.sendMail(GetApplicationResources
							...
							contactName, tempList[i],
							StringUtil.wholeMailGenerate(nameList[i], DateUtil
									.reFormatTime(recordTime), mailRecord,
									tmpStr, "MID_EXCEPTION"));
				}
			}
		}
	}
	...
	public void sendSMG() {
		...
		if ((true == smgOrNot) && (!"".equals(phoneNumber)) && mailRecord > 0) {
			if (logger.isInfoEnabled()) {
				logger.info("中途拔卡分析线程发送短信给联系人:" + contactName);
			}
			String[] tempList = null;
			String[] nameList = null;
