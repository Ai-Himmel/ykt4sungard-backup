...
//	private static String xmlfile="sql_db2.xml";
	...
	public boolean load(String file){
		...
		try {
			InputStream in = this.getClass().getClassLoader().getResourceAsStream("sql_"+file+".xml");
			Document doc = sax.build(in);
			Element element = doc.getRootElement();
			List list = element.getChildren();
			for(int i=0; i<list.size();i++){
				Element sqlelement = (Element)list.get(i);
				sqlDictionary.put(sqlelement.getChild("sql-name").getText().trim(),sqlelement.getChild("sql-statement").getText().trim());
			}
	 ...
	 * @return sql语句
	 */
	public String getNamedParamSQL(String key, Map params) {
		String temp = "";
		try {
			if (sqlDictionary.containsKey(key)) {
				temp = sqlDictionary.get(key).toString();
	 ...
	 * @return sql语句
	 */
	public String getSequenceParamSQL(String key, Map params) {
		String temp = "";
		try {
			if (sqlDictionary.containsKey(key)) {
				temp = sqlDictionary.get(key).toString();
