...
import com.kingstargroup.form.DeviceExceptRt;
...
//
			...
			Query firstquery = session.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.ExceptSerilAnalysisThread.SerialExcept"))
			.addEntity("t", DeviceExceptRt.class);
			firstquery.setString("startTime", startTime);
			firstquery.setString("endTime", endTime);
			
			List firstList = firstquery.list();
			Iterator firstIter = firstList.iterator();
			while(firstIter.hasNext()){
				DeviceExceptRt rt = (DeviceExceptRt)firstIter.next();			
				
				Query queryThree = session.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.ExceptSerilAnalysisThread.getcertSerial"))
						.addEntity("a", TPosdtl.class);
				queryThree.setString("deviceId", rt.getDeviceId());
				queryThree.setLong("serialNo", rt.getNowNo());
				...
				while (iterThree.hasNext()) {
					...
					if (true == recordOrNot) {
						...
						seriExcept.setAbnormaltype(Long.parseLong("206"));
					...
					if (!"".equals(mailList)) {
						...
						buffer.append(StringUtil.solidStringGenerate(rcvdtl.getId()
								...
								cust_name, rcvdtl.getId().getDevseqno(), rcvdtl
										...
										.getTranstime(),
								"BSHEETVER_EXCEPTION.SERIAL_ERR"));
					}
				}			
			}
...
//			session.clear();
			...
			//时间突变
			Query query = session.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.ExceptSerilAnalysisThread.serialTimeerror"))
			...
			while (iter.hasNext()) {
				...
				if (true == recordOrNot) {
					...
					seriExcept.setContactname(contactName);
					seriExcept.setAbnormaltype(Long.parseLong("201"));
			...
			Query queryFour = session
					.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.ExceptSerilAnalysisThread.moneyinouterror"))
					.addEntity("a", TPosdtl.class);
			...
			while (iterFour.hasNext()) {
				TPosdtl rcvdtl = (TPosdtl) iterFour.next();
				if (true == recordOrNot) {
					MapSerialexceptTable seriExcept = new MapSerialexceptTable();
					MapSerialexceptTableId seriExptKey = new MapSerialexceptTableId();
					...
					seriExcept.setContactname(contactName);
					seriExcept.setAbnormaltype(Long.parseLong("203"));
					session.save(seriExcept);
				}
				mailRecord++;
				if (!"".equals(mailList)) {
			...
			// 设备号不存在
			recordTime = DateUtil.getNow();
			Query queryOne = session.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.ExceptSerilAnalysisThread.devicenotexist")).addEntity(
					"posdtl", TPosdtl.class);
			...
			while (iterOne.hasNext()) {
				...
				if (true == recordOrNot) {
					...
					seriExcept.setContactname(contactName);
					seriExcept.setAbnormaltype(Long.parseLong("204"));
					session.save(seriExcept);
				}
				mailRecord++;
				if (!"".equals(mailList)) {
			...
			Query queryTwo = session.createQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.ExceptSerilAnalysisThread.dupserial"));
			...
			while (iterTwo.hasNext()) {
				DupliSerialResult serialResult = (DupliSerialResult) iterTwo
						.next();
				Query queryThree = session.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgAnalysis.ExceptSerilAnalysisThread.getspecialSerial"))
						.addEntity("a", TPosdtl.class);
				queryThree.setString("deviceId", serialResult.getDeviceId());
				queryThree.setLong("serialNo", serialResult.getSerialNo());
				queryThree.setLong("cardNo", serialResult.getCardNo());
				queryThree.setString("startTime", startTime);
				queryThree.setString("endTime", endTime);
				List listThree = queryThree.list();
				Iterator iterThree = listThree.iterator();
				while (iterThree.hasNext()) {
					...
					if (true == recordOrNot) {
						...
						seriExcept.setContactname(contactName);
						seriExcept.setAbnormaltype(Long.parseLong("205"));
						session.save(seriExcept);
					}
					mailRecord++;
					if (!"".equals(mailList)) {
