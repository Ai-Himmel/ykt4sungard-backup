 ...
 * @since 1.0
 */
public class SerialSupressDealThread extends Thread implements
		AnalyzeThreadInterface {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger
			.getLogger(SerialSupressDealThread.class);
	...
	public void initParam() {
		if (logger.isInfoEnabled()) {
			logger.info("**************流水压数处理线程初始化**************"); //$NON-NLS-1$
		}
		try {
			...
			logger.error("流水压数处理线程初始化失败！", e); //$NON-NLS-1$
		...
		if (logger.isInfoEnabled()) {
			logger.info("**************流水压数处理线程初始化成功**************"); //$NON-NLS-1$
		}
	}
	...
	public void anaylize() {
		if (logger.isInfoEnabled()) {
			logger.info("流水压数处理线程开始分析任务"); //$NON-NLS-1$
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger
						.info("流水压数处理线程开始分析时间:" + DateUtil.getNow("yyyy/MM/dd/HH:mm:ss")); //$NON-NLS-1$
			}
			if (1 > DateUtil.checkTimeSequence(endTime, DateUtil.getNow())) {
				if (logger.isInfoEnabled()) {
					logger
							.info("流水压数处理线程分析时间段：" + startTime + " -- "
									+ endTime);
				}
			} else {
			...
			BgMonitor.monitorThreadStatusReport(
					...
					"流水压数处理线程", DateUtil.getNow("yyyy/MM/dd/HH:mm:ss"), String
							.valueOf(this.doubleCycle), "运行中...");

			Query query = session
					.createQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgDeal.SerialSupressDealThread.getSupreeserial"));
			...
			while (iter.hasNext()) {
				MapSerialostTable mapSerialostTable = (MapSerialostTable) iter
						.next();
				Query queryTwo = session.createSQLQuery(SqlStatementDictionary.getIntance().getSQL("com.kingstargroup.action.bgDeal.SerialSupressDealThread.checksupressserial"))
			...
			logger.error("流水压数处理线程出现以下异常：", e); //$NON-NLS-1$
//			e.printStackTrace();
			if (tx != null) {
				tx.rollback();
			}
		...
		try {
			...
			logger.error("流水压数处理线程出现以下异常：可能被强行停止！ ", e); //$NON-NLS-1$
		} catch (Exception e) {
			logger.error("流水压数处理线程出现以下异常：可能被强行停止！ ", e); //$NON-NLS-1$
		}
	}
	...
	public void terminate() {
		...
		if (this.isAlive()) {
			logger.warn("流水压数处理线程状态被请求，此时线程状态：活动线程!"); //$NON-NLS-1$
			requestStop = true;
		} else {
			logger.warn("流水压数处理线程状态被请求，此时线程状态：死亡线程!"); //$NON-NLS-1$
			this.terminate();
	...
	public void sendMail() {
		...
		if ((true == mailOrNot) && (!"".equals(mailList))
				&& (0 != buffer.size())) {
			if (logger.isInfoEnabled()) {
				logger.info("流水压数处理线程发送邮件给联系人:" + contactName);
			}
			String tmpStr = "";
			if (buffer.size() > 5000) {
	...
	public void sendSMG() {
		...
		if ((true == smgOrNot) && (!"".equals(phoneNumber)) && mailRecord > 0) {
			if (logger.isInfoEnabled()) {
				logger.info("流水压数处理线程发送短信给联系人:" + contactName);
			}
			String[] tempList = null;
			String[] nameList = null;
