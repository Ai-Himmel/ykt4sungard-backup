...
#if defined(HINOKI) || defined (HINOKI2) || defined (SATSUKI2) || defined(STOCKHM2) || defined(HINOKI3) /* HINOKI2 2001/11/12 T.Takagi */
...
CONST UBYTE DBG_ICE_AD_R = 0;  /* ICE_R対応 O.Oshima 2003/09/27 */


void SetVBR(void); /* ini_int.src */

#if (PRO_ENERGY_SAVE == ENABLE)		/* 2001/12/10 T.Takagi */
...
** 	for (count = 0; count < 0x0A4; count++) { /* 156byte TBD 1998/09/10 <-0xA4 */
** 		*dst = *src;
** 		dst++;
** 		src++;
** 	}
...
void MachineUniqCPUInitial(void)
{
	...
	/* ここに日立ＤＲＡＭリフレッシュ不具合救済処理を入れること！ by nose 1999/01/26 */
	/* ＤＲＡＭコントローラ設定の後ろへ移動しました T.Nose 1999/01/26 */
	...
	/* ここに日立ＤＲＡＭリフレッシュ不具合救済処理を入れること！ by nose 1999/01/26 */
	/* ポートＢの設定がまだなのでここでやっても RAS がでません。後ろに移動します。 by H.Kubo 1999/01/27 */
	...
	/* ここに日立ＤＲＡＭリフレッシュ不具合救済処理を入れること！ by nose 1999/01/26 */
	/* 上から移動してきました。 by H.Kubo 1999/01/27 */
	/**********************************************/
	/* ＤＲＡＭ ＲＡＳオンリリフレッシュ ここから */
	/**********************************************/
	/* リフレッシュ停止 */
	...
	/* ＰＢ３，ＰＢ４をパワーオンリセットの時と同じ入力に戻す */
	PFC__PBIOR &= ~0x0018;

	/**********************************************/
	/* ＤＲＡＭ ＲＡＳオンリリフレッシュ ここまで */
	/**********************************************/
...
#if (PRO_RS_INTERNAL == ENABLE)				/* SH704X内蔵のシリアル有効／無効 By O.Kimoto 1998/09/25 */
	/** シリアル(ＣＨ０)初期化 */
	InitializeSCI();
#endif
...
/*	SubResetOn(); */
	...
	/* 最低１ms開くようにカウンター値をセット */
	count = 0x025e;	/* about 1ms */
	while (count) {
		count--;
	}
...
**	OutputWORD(SH_ISR, 0x00);
	...
	/*
	** どこにも初期化するパスがないことを発見
	** この変数を初期化しないと、後はこの変数にＯＲでライトして使用するので
	** 初期化しておいたほうがよい
	** 1999/04/06 by T.Soneoka
	*/
#if (PRO_ENERGY_SAVE == ENABLE)
	RelayPortStatus = 0x4000;	/* サブリセットは解除したまま */
#else
	RelayPortStatus = 0;
#endif

	/*
	** 電源Ｏｎ時にパワーコントロールが入らない不具合
	** スキャナモータおよびプリンタモータを動かさないとパワーコントロールがＯｎしない
	** プリンタカバーを開けた状態で、電源Ｏｎして、シェーディング取り込みをすると
	** ランプがＯｎせず、シェーディング取り込みをしてしまう
	** この状態でコピーをすると全黒コピーになってしまう
...
void InitializeInternalDevice(void)
{
	...
	/*---------------------------------------------------------------------
	** SAKAKIで電源ON時に立ち上がらずに、周辺回路が誤動作しているかもしれない症状が発生しました。
	** 出力ポート設定にするものについては、ＰＦＣの設定を行う前に、ポート出力値を設定しておきます。
	** まずは、テスト用としてSAKAKIのみ入れてみます。
	** 成功した暁には、出力ポートをしようしていないレジスターについても、0x0000をWriteするようにして、
	** 全機種共通ソースにし、xxx_cpu.hの#define値で切り替えるようにします。
	** 1998/10/30 H.Yoshikawa
	*/
	...
	*/
	...
	/* ＰＡＩＯＲＨ（ポートＡ・ＩＯレジスタＨ）の設定               */
	/*--------------------------------------------------------------*/
#if defined(HINOKI2) || defined(HINOKI3)	/* LC821033仕様対応 Y.Kano 2004/04/01 */
	if (LC821033_PGB9()) {
		PFC__PAIORH = PFC__PAIORH_DATA_2;
	}
	else {
		PFC__PAIORH = PFC__PAIORH_DATA;
	}
	...
	SYS_HaltClock = FALSE;			/* ここで落とさないと最初にメインが一周するまでつじつまが合わず、電源ON直後の送信等で
									   DMAが起動しない。O.Oshima 2003/11/25 */
 #endif
#endif
	...
	*/
	...
	SH__IPRB &= ~IRQ6_LEVEL;		/* IRQ06マスク *//* ソフトリセット時DTCが動作していた場合、AEが立つ（？） by O.Kimoto 2002/12/12 */
#endif
	...
	/* Ａ／Ｄ変換器の設定                             */
	/*------------------------------------------------*/
#if (PRO_PRINT_TYPE == THERMAL_TRANS)	/* 1998/10/09 H.Yoshikawa */
	/*----------------------------------------------------------------*/
...
#elif (PRO_PRINT_TYPE == THERMAL)	/* HINOKI by O.Kimoto 1998/10/12 */
	/*----------------------------------------------------------------*/
	/* Ａ／Ｄコントロール／ステータスレジスタ（ＡＤＣＳＲ）の設定     */
	/*----------------------------------------------------------------*/
	AD__ADCSR0 = AD__ADCSR0_DATA;
	...
	/* CPUがRバージョンのICEを使用する場合  *//* ICE_R対応 O.Oshima 2003/09/27 */
	if (DBG_ICE_AD_R) {
		(*(UBYTE *)0xffff83E0) = 0x1F;
		(*(UBYTE *)0xffff83E1) = 0x08;
		(*(UBYTE *)0xffff83E0) |= 0x20;
	}
...
** 	UWORD dbg_bcr;
#endif
#if (PRO_ENERGY_SAVE == ENABLE)		/* HINOKI2 By O.Kimoto 2002/02/26 */
	UWORD	count;
#endif	

#if 0 /*(PRO_RDS_FLASH_ROM_WRITE == ENABLE) /@ 1998/04/07  By T.Yamaguchi */
//	デバッグ用外すこと/*1998/04/28  By T.Yamaguchi */
//	UWORD	*src_adr;
//	UWORD	*tgt_adr;
//	UDWORD	cnt;
//	UWORD sumcheck;
...
//	}
	...
	case NO_STATUS:
		...
		** 素通りして通常の立ち上げを行います
		*/
#if 0 /* defined (SAKAKI) */ /* リセットのデバッグ用に入れます。by H.Kubo 1999/01/20 */ /* Deleted by H.Kubo 1999/01/26 */
@@		if (SH__BCR1 == (BCR1_MTU_WRITE_ENABLE + BCR1_CS3_WORD + BCR1_CS1_WORD + BCR1_CS0_WORD)) {
@@			*((UBYTE *) LED1_PORT) = (MODE_GRAY_LED | MODE_FINE_LED);
@@		}
@@		else {
@@			*((UBYTE *) LED1_PORT) = (MODE_SFINE_LED);
@@		}
#endif
		break;
