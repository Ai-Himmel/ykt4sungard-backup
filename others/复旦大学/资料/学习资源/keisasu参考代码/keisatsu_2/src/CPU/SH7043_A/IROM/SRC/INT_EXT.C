...
/* CONST UBYTE INT_Modem_Debug = 0; */ /* added by H.Kubo 1998/11/09 */
#endif

#if (PRO_EXT_INT_TYPE != EXT_INT_NONE)
...
void INT_SlaveInterrupt(void)
{
	...
	if (i_status & IO_BIT_INT70) {		/* 外部追加割込み７０（ＤＩＰＰ割込み） */
		...
		outpw(INT_MASK_PORT,SYS_InterruptPortStatus);
		/* 割り込み処理中の割り込みを無視しないように追加します。 by H.Kubo 1998/11/10 */
		SYS_InterruptPortStatus &= IO_BIT_INTCLR70;	/* 割込検出フラグ出力を許可 */
		outpw(INT_MASK_PORT,SYS_InterruptPortStatus);
...
#if defined(ANZU_L) || defined (ANZU_V)	/** ＡＮＺＵ＿ＬタイプはＲＳ２３２Ｃです */
...
#endif	/* #if (PRO_EXT_INT_TYPE != EXT_INT_OTHER) */
