...
/************************************************************************
共通ベクタテーブル
パワーＯＮリセットのベクタのみ全機種共通で内蔵ＲＯＭに配置します。
その他の割込みは外付けＲＯＭに配置されている各機種毎のベクタを使用します。
*************************************************************************/
#pragma section VECTOR /* locate section name "CVECTOR" */
const void(* const INT_VectorTableCPUROM[])(void) = {
...
/************************************************************************
機種別ベクタテーブル
以下のベクタは外付けＲＯＭ上に配置し、条件コンパイル等で各機種毎に
定義してください。
*************************************************************************/
#if defined(SAKAKI)
#include "\src\atlanta\sakaki\def_v\vector.c"
#endif
#if defined(HINOKI) || defined (HINOKI2) || defined(HINOKI3)	/* Ｆｏｒ ＨＩＮＯＫＩ By O.Kimoto 1998/09/07 */ /* HINOKI2 2001/11/12 T.Takagi */
#include "\src\atlanta\hinoki\def_v\vector.c"
#endif
#if defined (SATSUKI2)
#include "\src\atlanta\satsuki2\def_v\vector.c"
#endif
#if defined (STOCKHM2)
#include "\src\atlanta\stockhm2\def_v\vector.c"
#endif
