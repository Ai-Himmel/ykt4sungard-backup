						 		...
						 		void )				/* ‚È‚µ */
{
	...
	if (CMN_CheckCategory2()) {	/* F-101—p‚Ìˆ—‚ð’Ç‰Á‚µ‚Ü‚· 1997/12/26 Y.Matsukuma */
		do {
			/** ƒvƒŠƒ“ƒ^[ƒXƒ^[ƒg */
							...
							if ( ((MainFunction_Category2[i].NextLevel+j)->Available == OPR_YES) &&
								...
								if ( page == 1 ) {	/* ‚Pƒy[ƒW–Ú‚ÌŽž */
									if ( j && (MainFunction_Category2[i+1].Available == OPR_YES) &&
										...
										for ( dummy = 1; ; dummy++ ){
											if (MainFunction_Category2[i+dummy].FirstCode == '\0') {	/* ‘S‚ÄˆóŽš‚µ‚½‚©H */
												break;
											}
								...
								if ( line == 0 ) { /*  1ƒy[ƒW‚ÉˆóŽš‚Å‚«‚È‚­‚È‚Á‚½Žž */
									...
									if ((MainFunction_Category2[i].NextLevel+pointer)->FirstCode == '\0') { /* ‹@”\Ý’èÊß×Ò°À—L‚èH*/
										pointer = 0;	/* Added by S.Fukui Oct.3,1997 */
										i++;
										for ( ; ; i++ ){
											if (MainFunction_Category2[i].FirstCode == '\0') {	/* ‘S‚ÄˆóŽš‚µ‚½‚©H */
												break;
											}
 ...
 #else	/* ‚±‚±‚©‚ç‰º‚ªAÎßÌß×—p‚Å‚· 1997/12/26 Y.Matsukuma */
	...
	do {
		...
		/** ˆóü‰Â”\s”¾¯Ä */
				...
				/* ‹L˜^Ž†‚ªƒ[ƒ‹Ž†‚Ì‹@Ží‚ÍAŠª‚«•¨‚ÅˆóŽš‚µ‚Ü‚·B
		...
		page++;
	/* } while ( line == 0 ); @** 2ƒy[ƒW‚ÉˆóŽš‚·‚éˆ×‚Ìƒ‹[ƒv */
	/** line‚ª0‚É‚È‚Á‚Ä‹@”\ƒpƒ‰ƒ[ƒ^‚ª‚È‚¢‚Æ‚«‚É2–‡–Ú‚Éƒ^ƒCƒgƒ‹‚¾‚¯ˆóŽš‚·‚éƒoƒO BY H.H 1996/12/10 */
...
#if (0)
** 	/* ‹@”\ƒŠƒXƒg•ÏX By O.Kimoto 1998/04/22 */
** 	CMN_StringCopy((UBYTE  *)&List.S.Print[40], (UBYTE  *)ROM_Revision );
** 	CMN_StringCopy((UBYTE  *)&List.S.Print[5],  (UBYTE  *)PWRD_ProgramList_K);
	...
	/* ‰üs’Ç‰Á By O.Kimoto 1998/04/22 */
	if ((ret = CharacterPrint( LST_LINE_FEED )) != OK) {
		return ( ret );
	}
...
** #endif
	...
	case PAPER_TYPE_MODE:				/* ‹L˜^Ž†ƒ^ƒCƒv */
		return(TRUE);
#endif
	}
	...
	UBYTE func_no)	/* ‹@”\”Ô† */
{
	...
	case NUMBER_DISPLAY_MODE:		/** ƒiƒ“ƒo[ƒfƒBƒXƒvƒŒƒC		*/
		menu_no	= SET_LIST_INST;
		break;
#endif
	default:
	...
	for (i = 0;; i++) {
		...
		*/
		...
		if ( (--EnablePrintLine) <= 0) {
			if (menu_no < RS232C_SPEED_MENU) {
				if (SoftKeyTable[menu_no][i+1] == 0) {	/** ‹@”\€–ÚI—¹ */
					return(END_FUNC);
				}
			}
			else {
...
#if (0)
** /* ‹@”\ƒŠƒXƒg‚Ì\‘¢‚ð•ÏX‚µ‚Ü‚·‚Ì‚ÅAˆÈ‘O‚Ì‚â‚Â‚ðƒRƒƒ“ƒg‚ÅŽc‚µ‚Ä‚¨‚«‚Ü‚·B By O.Kimoto 1998/04/22 */
** /*************************************************************************
** 	module		:[‹@”\Ý’è“à—eˆóü]
** 	function	:[]
** 	common		:[]
...
** 
** #if (0)
** ** 	/* ‚o‚n‚o‚k‚`‚qQ‚g‚Í“ú–{Žd—l‚µ‚©‚È‚¢‚Ì‚ÅAˆÈ‰º‚Ìˆ—‚Í‚à‚Æ‚É–ß‚µ‚Ü‚·B By O.Kimoto 1998/04/22 */
