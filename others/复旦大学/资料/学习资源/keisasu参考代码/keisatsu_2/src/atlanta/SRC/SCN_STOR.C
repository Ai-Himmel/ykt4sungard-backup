...
void SCN_Task(void)
{
	...
	UBYTE	break_flag = 0;	/* T.Nose 1998/05/13 */
	...
	/* V851 省ｴﾈﾓｰﾄﾞからのｺﾋﾟｰ、蓄積時に前のﾓｰﾄﾞが表示される。By Y.Suzuki 2002/09/04 */
...
//#endif
	...
	/*　ｲﾝﾀｰﾈｯﾄFAX用に読み取りモードを変更　*/
	if (SYS_DocumentStoreItem == SYS_MEM_MAIL_TX_FILE) {
		doc_mode = GetInternetFaxScanMode(SYB_ModeSw);
		SYB_ModeSw = doc_mode;
	}
#endif
	SCN_Control.DocumentMode = doc_mode;
	...
	/*----------------------------------------------------*
	 * 混在同報時のＦＡＸ送信原稿用のインデックスデータ
	 * (二重オープン用のパラメータ)をセットする.
	 * メール送信用のデータはSCN_StoreDataにセットされる。
	 *----------------------------------------------------*/
	if (SYS_INF_MixedTxRequest == TRUE) {
		SYS_INF_MixedTxIndex.Item = SYS_MEM_TX_FILE;
		SYS_INF_MixedTxIndex.No   = (UWORD)SYS_INF_MixedTxFaxCommandNo;
		SYS_INF_MixedTxIndex.Page = SYS_DocumentStorePage;
		SYS_INF_MixedTxIndex.BoxNumber = SYS_DocumentStoreBoxNumber;
	}
#endif
	...
	*/
	...
	/*	i = 2;ランプチェックの時間 400ms*2 --> 400ms*25(10s)に変更 By Y.Suzuki 1999/02/25 */
		i = 25;
		is_lamp_ok = TRUE;
		while (i > 0) {
	...
	*/
	if ((SYS_DocumentStoreItem == SYS_MEM_MULTI_COPY_FILE)
	 ...
	 && (SYB_MaintenanceSwitch[MNT_SW_C9] & 0x10)) {
		...
		if (SYB_MultiCopyFile[SYB_MultiCopyWritePoint].Reduction == 86) {
			 SYB_MultiCopyFile[SYB_MultiCopyWritePoint].Reduction = 100;
		}
		else if (SYB_MultiCopyFile[SYB_MultiCopyWritePoint].Reduction == 81) {
			 SYB_MultiCopyFile[SYB_MultiCopyWritePoint].Reduction = ((81 * 100)/ 86);
		}
		else if (SYB_MultiCopyFile[SYB_MultiCopyWritePoint].Reduction == 70) {
	 ...
	 && (SYB_MaintenanceSwitch[MNT_SW_C9] & 0x10)) {
		...
		/* 原稿Ａ３、記録紙Ａ５縦、縮小率５０％の組み合わせの場合のみ
		** 縮小率を強制的に４９％とする。但し表示は５０％のまま
		** 記録紙Ａ５縦は手差しコピーにしかない仕様なのでここに入れました
		** T.Nose 1998/03/30
		*/
		if ((SCN_DocumentReduction == 50)
		 && (SYB_ManualCopyFile.Size == SYS_A5_PAPER)
		 && (SYB_MaintenanceSwitch[MNT_SW_E0] & 0x08)) {
			SCN_DocumentReduction = 49;
		}
	}
#endif
...
******/
		...
		*/	/* ICHOU追加  1996/08/08 s.takeuchi */
...
#if (PRO_DIALUP_INTERNET_FAX == ENABLE)	/* by Y.Tanimoto 1998/03/25 */
		/*　ｲﾝﾀｰﾈｯﾄFAX用に蓄積サイズを変換　*/
		if (SYS_DocumentStoreItem == SYS_MEM_MAIL_TX_FILE) {
			SCN_DocumentReduction = GetInternetFaxScanReducion(SYS_DocumentScanSize);
		}
#endif
		...
		if (!SCN_CheckMotorStopAfterScan()) { /** モーター停止要求がなければ */
			...
			/*　ｲﾝﾀｰﾈｯﾄFAX用に読み取りモードを変更　*/
			if (SYS_DocumentStoreItem == SYS_MEM_MAIL_TX_FILE) {
				SYB_ModeSw = SCN_ModeSw = GetInternetFaxScanMode(SCN_ModeSw);
			}
#endif
...
//		else if ((is_copy_file) && (SCN_ModeSw == SYS_SFINE)) @
 ...
 #if (PRO_CLASS1 == ENABLE)
			/* Class1の時はSFINEのまま蓄積します */
			/* 1998/07/20 by Y.Tanimoto */
			if (SYS_DocumentStoreItem == SYS_MEM_CLASS1_TX_FILE) {
				SCN_StoreData.Mode = SCN_ModeSw;
			}
 #endif
		...
		/*if ((SYS_DocumentStoreItem == SYS_MEM_MAIL_TX_FILE)
			&& (SYS_INF_MinimumSetRequest == TRUE)) { 1998/05/23 Eguchi*/

		if ((SYS_DocumentStoreItem == SYS_MEM_MAIL_TX_FILE)
		&&	(CHK_UNI_INF_MinimumSetAvail())) { /*1998/05/23 Eguchi*/
...
#if (PRO_DIALUP_INTERNET_FAX == ENABLE)	/* by Y.Tanimoto 1998/03/12 */
			if ((SYS_INF_MixedTxRequest == TRUE) && (SYS_DocumentStoreItem == SYS_MEM_MAIL_TX_FILE)) {
				/* FAX＆Mailの混在同報　SYS_MultiProgramFlagは絶対０にしておくこと　*/
				if (ret_code = MEM_DualOpen(&SCN_StoreData , &SYS_INF_MixedTxIndex , &SCN_ExecData)) {
					MEM_ClearBreak(&SCN_IndexData); /** 蓄積途中のファイルをクリア *//*２重インデックスもクリアします*/
					StoreTaskMsg.Message = MSG_SCN_DOCSTORE_MEM_OVER;
					SCN_ADF_MotorStopRequest = ADF_MOTOR_MEMORY_OVER;	/* 停止要求を出す */
					/*	タスクの終了処理はループの外でする  1996/10/31 s.takeuchi */
					break;
...
#else	/* by Y.Tanimoto 1998/03/12 PRO_DIALUP_INTERNET_FAX == ENABLE */
...
#endif	/* by Y.Tanimoto 1998/03/12 PRO_DIALUP_INTERNET_FAX == ENABLE */
		}
...
#if (PRO_DIALUP_INTERNET_FAX == ENABLE)	/* by Y.Tanimoto 1998/03/12 */
			if (SYS_DocumentStoreItem == SYS_MEM_MAIL_TX_FILE) {
				/*　ここで最後の転送バイト数を足し込むDMAを止めてからカウンター値を読み込みます */
				DMA_RequestMask(DMA_SCNCDC_DRAM_CH);
				SYS_INF_StoredMailDataCount[SYS_INF_IndexTableNo] += ( (UDWORD)MEM_BLOCK_SIZE - (UDWORD)( DMA_CheckDMA_Counter(DMA_SCNCDC_DRAM_CH) * 2 ) );
			}
#endif
...
#if (PRO_DIALUP_INTERNET_FAX == ENABLE)	/* by Y.Tanimoto 1998/03/12 */
				if ((SYS_INF_MixedTxRequest == TRUE) && (SYS_DocumentStoreItem == SYS_MEM_MAIL_TX_FILE)) {
					MEM_DualClose(&SCN_ExecData, SCN_TotalEncodeLine);
				}
				else {
					MEM_WriteClose(&SCN_ExecData, SCN_TotalEncodeLine);
				}
#else	/* by Y.Tanimoto 1998/03/12 */
				MEM_WriteClose(&SCN_ExecData, SCN_TotalEncodeLine);
#endif	/* by Y.Tanimoto 1998/03/12 */
			}
		}
		else {
...
#if (PRO_DIALUP_INTERNET_FAX == ENABLE)	/* by Y.Tanimoto 1998/03/12 */
					if ((SYS_INF_MixedTxRequest == TRUE) && (SYS_DocumentStoreItem == SYS_MEM_MAIL_TX_FILE)) {
						SYS_INF_MixedTxIndex.Page++;
					}
#endif
				}
				/* ここより後でＡＤＦに原稿が差し込まれると
				** Item,Number,Page が同一のインデックスがもう一つ作られてしまう
				** この時点でＡＤＦに原稿がなければループを終了するフラグを立てる
				** T.Nose 1998/05/13
				*/
				else {
					break_flag = 1;
		...
		if (SYS_ScannerExecMode == SCN_ADF) {		/** ADF読取り */
			...
			*/	/* 枚数指定チェック  1997/02/18 */
			...
			if ((SYS_MachineStatus[SYS_COVER_STATUS] & SYS_COVER_PRINTER_OPEN)
			 || (SCN_MirrorMoveStatus == MIRROR_MOVE_RXIL_ABORT)) { /* RXIL瞬断対策 T.Nose 1998/03/09 */
				StoreTaskMsg.Message = MSG_SCN_DOCSTORE_TOP_COVER_OPEN;
				LampOff();
				snd_msg( mbxno.MAN_Task, &StoreTaskMsg );
				rcv_msg(mbxno.SCN_Task, &SCN_Msg);
...
void SCN_DMA_WriteTask(void)
{
	...
	if (SCN_MotorSpeedControl != SCN_MOTOR_SPEED_ADJUST) {
		SCN_StopMotor();
/*		SCN_MotorCurrentStopCounter = 0; 10mS割り込みで励磁を止めるのでクリアしてはいけない T.Nose 2000/1/28 */
...
{
#if (PRO_DOC_IN == ENABLE) /* #if is added by H.Kubo 1998/07/08 */
