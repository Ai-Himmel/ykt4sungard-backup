...
void ContCopyTest(void); /* ＦＢＳ連続動作テスト 1998/2/2 */
...
#if (PRO_PAPER_TYPE == CUT_PAPER) && defined(STOCKHM2) /* Added by SMuratec 李 2004/4/16 */
	...
	UWORD	sub_message2)
{
	...
	case MSG_DOCUMENT_IN_SETUP:
		...
		OPR_FeedinDispTime = 1;	/* 原稿繰り込みフラグ */
#endif
#if (PRO_PAPER_TYPE == CUT_PAPER) && defined(STOCKHM2) /* Added by SMuratec 李 2004/4/16 */
		Paper.TxMotorOn = TRUE;	/* リアルタイムコピーの時にプリンタは原稿繰り込みフラグを待ちので、ここで通知します。 */
#endif /* (PRO_PAPER_TYPE == CUT_PAPER) && defined(STOCKHM2) */
		break;
	case MSG_DOCUMENT_IN_NOTHING: /** 原稿引き抜き処理 */
			LampOff();
			GearBackLashRequest = TRUE; /* バックラッシュを予約 T.Nose 1997/10/24 */
#if (0)
...
//		}
#endif
		/*-----------------------------------------------------------------
		** DS1のチャタリングまたは、原稿セット引き抜きを繰り返すと、
		** DS1がONにも関わらず、繰り込みをせずに、MSG_DOCUMENT_IN_NOTHINGが返送されてくることがあるので、
		** if (SYS_MachineStatus[SYS_SENSOR_STATUS] & SYS_SENSOR_DS1_ON)の場合には、
		** cre_tsk(&tskno_DocumentInTask);を再度行うようにします。
		** ANZU_LのようにGearBackLash処理があるものについては、この処理は入れません。
		** 
		** 1998/11/02 H.Yoshikawa
		*/
#if (PRO_GEAR_BACK_LASH == DISABLE)	/* 1998/11/02 H.Yoshikawa */
 ...
 #if defined(STOCKHM2)	/* 原稿繰込み時、カバーオープンした時にフィード・エラーフラグが残ってしまう不具合対策 by Y.Kano 2004/10/25 */
		if (QueryCloseInterLock4Scan()) {
			/* スキャナカバーが閉まっている */
			SYS_MachineStatus[SYS_SCANNER_STATUS] |= SYS_SCANNER_FEED_ERROR; /** フィード・エラーをセット */
		}
		/* スキャナカバーが開いてる時は、フィード・エラーをセットしない */
 #else
		SYS_MachineStatus[SYS_SCANNER_STATUS] |= SYS_SCANNER_FEED_ERROR; /** フィード・エラーをセット */
 #endif
		break;
	...
	UWORD	sub_message2)
{
	...
	UWORD	doc_sub_message1;
	UWORD	doc_sub_message2;
#if (PRO_JAM_OPR == ENABLE) /* T.Nose 1997/10/22 */
	UBYTE	is_recover_opr;
#endif
#if (PRO_QUICK_TX_TEST == ENABLE)	/* 1998/10/15 Y.Murata for KAKI */
	UWORD	result;
	UWORD	page;
	...
	*/
...
#if (PRO_JAM_OPR == ENABLE) /* T.Nose 1997/10/22 */
	is_recover_opr = FALSE; /* ジャムリカバーＯＰフラグリセット T.Nose 1997/10/22 */
#endif
	...
	case MSG_SCN_DOCSTORE_OK: /** 蓄積正常終了であれば */
		...
		case SYS_MEM_TX_FILE: /** メモリ送信ファイル */
			...
			result = CheckQuickTx();
			if (result != 0xFFFF) {
				if (SYB_ExecQueue[result].Status == SYS_EXEC_ERASED) {
					SetFaxComStopExecQueue(result);
				}
				else {
					/* メモリ送信コマンド・ファイルの登録 */
					MemoryTxDocStoreOK(SYS_COMMAND_TRX);
...
** #endif
		...
		case SYS_MEM_SUB_BULLETIN_FILE:	/* Ｆコード原稿蓄積起動 */
			...
			FcodeDocStoreOK(SYS_DocumentStoreBoxNumber);	/* 抜けていた 1997/11/19 */
			/* 原稿蓄積完了表示起動 */
			if (timer_store_fax_complete == 0xff) {
				CMN_MultiTimer10msStart(&timer_store_fax_complete,&StoreCompleteMessage);
			}
...
/*				CipherReadIndex.Page = SYS_DocumentStorePage;*/
	...
	case MSG_SCN_DOCSTORE_MEM_OVER: /** 蓄積メモリー・オーバー時の処理 */
		...
		*/
		...
		/* 2000/01/14 Y.Murata MEMORY_OVER_DISP
		 * ＮＴＴファネット指摘項目
		 * メモリオーバーが発生し、ストップキー押下でエラー表示が消える。
		 * その後、「ツウシン／コピーデキマス」表示となるので原稿が詰まったいるのがわからない。
		 * 対策として、原稿セットやり直し表示を出すようにします。
		*/
		...
		case SYS_MEM_TX_FILE:
			...
			/* クイックメモリオーバー処理 */
			/*----------------------------*/
			page = MEM_CountFinalPage(SYS_DocumentStoreItem, SYS_DocumentStoreNo, SYS_DocumentStoreBoxNumber);
			if (page != MEM_NO_INDEX) { /* １ページ以上蓄積されている */
				...
				result = CheckQuickTx();
				if (result != 0xFFFF) {
					SYS_QucikTxStopQueueNo = result;
					SetFaxComStopExecQueue(SYS_QucikTxStopQueueNo);
					SetQucikTxStop(SYS_QucikTxStopQueueNo);
					SYS_MemoryOverListRequest = 1;
...
#endif
#if (PRO_DIALUP_INTERNET_FAX == ENABLE)	/* Y.Tanimoto 1998/03/12 */
		case SYS_MEM_MAIL_TX_FILE:
#endif
			...
			if ((MEM_CountTotalPage(SYS_DocumentStoreItem, SYS_DocumentStoreNo, SYS_DocumentStoreBoxNumber)!= MEM_NO_INDEX) /* １ページ以上蓄積されている */
			 ...
			 && (SYS_DocumentStoreItem != SYS_MEM_CIPHER_FILE) /* スクランブル元原稿でない */
			 &&	!CMN_CheckScanFilePrinting())  {
				...
				switch (SYS_DocumentStoreItem)	{ /** 原稿の種別を解析 */
				case SYS_MEM_TX_FILE: /** メモリ送信ファイル */
				/*	if (SYS_DocumentStoreItem != SYS_MEM_CIPHER_FILE) {By Y.Suzuki 1998/02/09 */
						MemoryTxDocStoreNG(SYS_COMMAND_TRX); /** メモリ送信コマンド・ファイルのキャンセル */
				/*	}*/
					if (CHK_UNI_MemoryOverMessagePrint()) { /** メモリ・オーバー・メッセージ印字ON? */
					...
					break;
#if (PRO_CIPHER == ENABLE)/* スクランブル処理中のメモリ送信原稿のメモリーオーバオペレーションでスクランブルも動作不良になる。*/
						/* メモリ送信原稿とスクランブル元原稿の処理を分けるBy Y.Suzuki 1998/04/15 */
				case SYS_MEM_CIPHER_FILE:/**	暗号化前原稿 By Y.Suzuki 1997/06/10	*/
					if ((SYS_DocumentStoreItem == SYS_MEM_CIPHER_FILE) && (ScrambleOn == 0)) {
						break;
					}
					if (MEM_CountTotalPage(SYS_DocumentStoreItem, SYS_DocumentStoreNo, SYS_DocumentStoreBoxNumber)
						!= MEM_NO_INDEX) {/* １ページ以上蓄積がある */
				...
				case SYS_MEM_MAIL_TX_FILE:
					MailTxDocStoreNG();
					if (CHK_UNI_MemoryOverMessagePrint()) {
						SYS_MemoryOverListRequest = 1;
					}
					break;
#endif
	...
	case MSG_SCN_DOCSTORE_STOPSW_ON: /** 蓄積停止処理	*/
		...
		case SYS_MEM_TX_FILE: /** メモリ送信ファイル */
			...
			/* クイックストップＳＷ処理 */
			/*--------------------------*/
			if (MEM_CountFinalPage(SYS_DocumentStoreItem, SYS_DocumentStoreNo, SYS_DocumentStoreBoxNumber)!= MEM_NO_INDEX) { /* １ページ以上蓄積されている */
				result = CheckQuickTx();
				if (result != 0xFFFF) {
					MemoryTxDocStoreOK(SYS_COMMAND_TRX);
					SetFaxComStopExecQueue(result);
	...
	case MSG_SCN_DOCSTORE_1PAGE_START:
		...
		CheckQuickMemoryTxOK(SYS_DocumentStoreItem, SYS_DocumentStoreNo);
#endif
...
#if (0)
** #if (PRO_FBS == ENABLE) /* RXIL瞬断対策 T.Nose 1998/03/03 */
** 	case MSG_SCN_DOCSTORE_RXIL_ABORT:
** #endif
#endif
		SYS_MachineStatus[SYS_SCANNER_STATUS] &= ~SYS_SCANNER_JAM_RECOVER_OPR;
		SYS_MachineStatus[SYS_SCANNER_STATUS] &= ~SYS_SCANNER_NEXT_SCAN_OPR;
		SYS_MachineStatus[SYS_SCANNER_STATUS] &= ~SYS_SCANNER_NEXT_SCAN_START_OPR;
		SYS_MachineStatus[SYS_SCANNER_STATUS] &= ~SYS_SCANNER_NEXT_SCAN_START;
		/* ADFカバー・トップカバー開の時はFEED_ERRORを立てない */
#if (PRO_SEPERATE_24V == DISABLE)
#if (PRO_FBS == ENABLE)
#if (0)
//		if (!(SYS_MachineStatus[SYS_COVER_STATUS] & SYS_COVER_SCANNER_OPEN)
//		 && !((SYS_MachineStatus[SYS_COVER_STATUS] & SYS_COVER_PRINTER_OPEN) && (SYS_ScannerExecMode == SCN_FBS))) {
//			SYS_MachineStatus[SYS_SCANNER_STATUS] |= SYS_SCANNER_FEED_ERROR;
//		}
#endif
		/* RXIL瞬断対策 T.Nose 1998/03/04 */
#if (0)
** 		if (doc_message == MSG_SCN_DOCSTORE_RXIL_ABORT) {
...
** 		}
		...
		if (!(SYS_MachineStatus[SYS_COVER_STATUS] & SYS_COVER_SCANNER_OPEN)
		 && !((SYS_MachineStatus[SYS_COVER_STATUS] & SYS_COVER_PRINTER_OPEN) && (SYS_ScannerExecMode == SCN_FBS))
 #if (PRO_CIPHER == ENABLE)/*By Y.Suzuki 1998/06/12*/
		 /*スクランブル原稿長エラー */
		...
		switch (SYS_DocumentStoreItem) {
		case SYS_MEM_MULTI_COPY_FILE:	/* マルチコピー */
#if (PRO_FBS == ENABLE)
			/* ＦＢＳからのマルチコピーには連続読みが無い為、ソートＯＮ／ＯＦＦに関係なく中止します。
			** ＦＢＳの手差しコピー時にも連続読みは無いが、スキャン終了後にプリント起動がかかる為下記の処理を入れる
       		** 必要がないので入れません。 By S.Fukui Jan.29,1998 */
			if (SYS_ScannerExecMode == SCN_FBS) {
				MultiCopyDocStoreNG();
			...
			/* ノン･ソートマルチコピーの場合は中断→終了 */
				if (((message == MSG_SCN_DOCSTORE_DOC_ERROR)
#if (0)
** 	#if (PRO_FBS == ENABLE) /* RXIL瞬断対策 T.Nose 1998/03/04 */
** 				 || (doc_message == MSG_SCN_DOCSTORE_RXIL_ABORT)
** 	#endif
#endif
				 || (message == MSG_SCN_DOCSTORE_TOP_COVER_OPEN))
				 && CMN_CheckScanFilePrinting()) {
...
#if (PRO_JAM_OPR == ENABLE) /* 1997/10/21 T.Nose */
 #if (PRO_PRINT_TYPE == THERMAL_TRANS)	/* 1999.3.23 K.Kawata */
			if ((MEM_CountTotalPage(SYS_DocumentStoreItem, SYS_DocumentStoreNo, SYS_DocumentStoreBoxNumber) == MEM_NO_INDEX)
				|| CHK_OCA_Specification()){ /* 1ページも蓄積されていないかOCA仕様か 1999.3.5 K.Kawata */
 #else
			...
			break;
		case SYS_MEM_MANUAL_COPY_FILE:	/* 手差しコピー */
...
** 	#endif
 ...
 #if (PRO_PRINT_TYPE == THERMAL_TRANS)	/* 1999.3.23 K.Kawata */
			if ((MEM_CountTotalPage(SYS_DocumentStoreItem, SYS_DocumentStoreNo, SYS_DocumentStoreBoxNumber) == MEM_NO_INDEX)
				|| CHK_OCA_Specification()){ /* 1ページも蓄積されていないかOCA仕様か 1999.3.23 K.Kawata */
 #else
...
#else /* PRO_JAM_OPR else */
			MEM_ClearDocAndFATArea(SYS_DocumentStoreItem, SYS_DocumentStoreNo, SYS_DocumentStoreBoxNumber); /* 蓄積原稿の消去 */
			FcodeDocStoreNG(SYS_DocumentStoreBoxNumber);
#endif /* PRO_JAM_OPR end */
			break;
#endif /* PRO_F_CODE end */

#if (PRO_BATCH_TX == ENABLE)	/* バッチ送信ファイルのＪＡＭ時にステータスが残る等の不具合修正 by T.Soneoka 1997/10/21 */
		case SYS_MEM_BATCH_TX_FILE1:	/** 一括送信原稿 */
		case SYS_MEM_BATCH_TX_FILE2:	/** 一括送信原稿 */
		case SYS_MEM_BATCH_TX_FILE3:	/** 一括送信原稿 */
		case SYS_MEM_BATCH_TX_FILE4:	/** 一括送信原稿 */
		case SYS_MEM_BATCH_TX_FILE5:	/** 一括送信原稿 */
#if (PRO_JAM_OPR == ENABLE)
		...
		case SYS_MEM_CLASS1_TX_FILE:	/* CLASS1 ローカルスキャン・ファイル */
			...
			break;
#endif
#if (PRO_CIPHER == ENABLE)/*By Y.Suzuki 1998/02/05*/
...
#else /* PRO_JAM_OPR else */
#if (0)	/* 1999.3.8 K.Kawata */
			MemoryTxDocStoreNG(SYS_COMMAND_TRX); /** メモリ送信コマンド・ファイルのキャンセル */
#endif
			MEM_ClearDocAndFATArea(SYS_DocumentStoreItem, SYS_DocumentStoreNo, SYS_DocumentStoreBoxNumber); /* 蓄積原稿の消去 */
#endif /* PRO_JAM_OPR end */
			break;	/* 1999.3.8 K.Kawata */
		case SYS_MEM_TX_FILE:			/** メモリ送信ファイル */
		default:
#if (PRO_JAM_OPR == ENABLE) /* 1997/10/21 T.Nose */
 #if (PRO_PRINT_TYPE == THERMAL_TRANS)	/* 1999.3.3 K.Kawata */
...
#if (PRO_JAM_OPR == ENABLE) /* T.Nose 1997/10/21 */
		if (is_recover_opr == TRUE) {
#if (0)
** #if (PRO_FBS == ENABLE) /* RXIL瞬断対策 T.Nose 1998/03/04 */
** 			if ((doc_message == MSG_SCN_DOCSTORE_TOP_COVER_OPEN)
** 			 || (doc_message == MSG_SCN_DOCSTORE_RXIL_ABORT))
...
** #endif
	...
	default:
		...
		case SYS_MEM_CIPHER_FILE:/**	暗号化前原稿 By Y.Suzuki 1997/07/11	*/
			...
			else if (SYS_DocumentStoreItem == SYS_MEM_CIPHER_FILE) {/*By Y.Suzuki 1998/02/05 */
				/* スクランブル処理中をクリア */
				SYS_CipherComExec = 0;
				SYS_MachineStatus[SYS_CIPHER_STATUS] &= ~SYS_SCRAMBLE_EXEC;
			}
	...
	case SYS_MEM_MANUAL_COPY_FILE: /** 手差し・コピー・ファイル */
#endif
#if (PRO_CLASS1 == ENABLE ) /*   ANZU_L CLASS 1 -- WC97303 1997/10/21 19:20 */
	case SYS_MEM_CLASS1_TX_FILE: /** クラス１ファイル */
#endif
#if (PRO_DIALUP_INTERNET_FAX == ENABLE)	/* Y.Tanimoto 1998/03/12 */
	case SYS_MEM_MAIL_TX_FILE:
#endif
	...
	case SYS_MEM_MULTI_COPY_FILE: /** マルチ・コピー・ファイル */
	case SYS_MEM_MANUAL_COPY_FILE: /** 手差し・コピー・ファイル */
 #if (PRO_FBS == ENABLE)	/* T.Nose 1998/01/08 */
		if (SYB_MaintenanceSwitch[MNT_SW_G8] & 0x08) {
			if (SYS_ScannerExecMode == SCN_ADF) {
				ClearCopyParameter();
			}
		}
  #if (PRO_ADF_AUTO_CASSETTE == ENABLE)
		/* ＡＵＴＯ時のみ、カセット／倍率を初期化 ClearCopyParameter()処理との重複を避ける。By S.Fukui Jan.23,1998 */
		else {
...
#if (PRO_PRINT_TYPE == LED)		/* 1998/02/09 */
	if ((SYS_DocumentStoreItem == SYS_MEM_MULTI_COPY_FILE)
	 || (SYS_DocumentStoreItem == SYS_MEM_MANUAL_COPY_FILE)) {
		if ((SYS_MachineStatus[SYS_MACHINE_STATUS] & SYS_MULTI_COPY)
		 || (SYS_MachineStatus[SYS_MACHINE_STATUS_RESERVE] & SYS_MANUAL_COPY)) {	/* Add By M.Tachibana 1998/02/09 */
	...
	*/
	if ((doc_message != MSG_SCN_DOCSTORE_1PAGE_START)
	 &&	(doc_message != MSG_SCN_DOCSTORE_1PAGE_FINISH)
	 &&	!(SYS_MachineStatus[SYS_SCANNER_STATUS] & SYS_SCANNER_MIRROR_ERROR)
	 && !(SYS_MachineStatus[SYS_ERROR_STATUS] & SYS_MOMENT_TOP_COVER_OPEN)) { /* RXIL瞬断対策 T.Nose 1998/03/06 */
		SYS_MachineStatus[SYS_RESOURCE_STATUS] |= SYS_SCANNER_BUSY;
	...
	UWORD	sub_message2)
{
	...
	if (doc_message == MSG_MIRROR_MOVE_OK) {	/** 正常に終了した */
		...
		case	MIRROR_TASK_TO_FBS:		/** FBS部への移動 */
			...
			/* ADF読み取り前の準備中表示のため  1997/07/04 T.Nose */
			SYS_MachineStatus[SYS_SCANNER_STATUS] &= ~(SYS_SCANNER_PREPARATION_ADF);
			...
			** T.Nose 1997/10/04
			*/
			if (SYB_MaintenanceSwitch[MNT_SW_C9] & TEST_PRINT_MIRROR_MOVE) {
				MirrorTaskInstruction = MIRROR_TASK_TO_ADF_READY;
				MirrorTaskRequest = TRUE;
		...
		*/
		...
		case	MIRROR_MOVE_LAMP_ERROR:
			...
			break;
		case	MIRROR_MOVE_FAILURE:
			/* ＡＤＦに原稿をセットした時、ＡＤＦ側でミラーが止まらなくなる件
			** T.Nose 1997/12/17
			*/
			MirrorTaskInstruction = MIRROR_TASK_INITIALIZE;
			MirrorTaskRequest = TRUE;
			DEBUG_MirrorNeedInitial = 0; /* ミラーキャリッジ初期化要求フラグリセット 1997/12/20 */
			DEBUG_OperationExitFlag = 1; /* シェーディングオペレーション終了フラグ */
			break;
		case	MIRROR_MOVE_RXIL_ABORT:	/* RXIL瞬断対策 T.Nose 1998/03/05 */
			SYS_MachineStatus[SYS_ERROR_STATUS] |= SYS_MOMENT_TOP_COVER_OPEN;
			break;
		default:
			/* ありえない */
	...
	UWORD	sub_message2)
{
	...
	/* 原稿排出テストタスクの削除 */
#if 0 /* 原稿排出タスクを 33 回続けると、リセットに飛ぶ不具合対策 by H.Kubo 1999/11/29 */
@	if (tskno_SCN_DocumentStoreTask != 0xFFFF) {
@		del_tsk(&tskno_SCN_DocumentStoreTask);
@	}
#else
	if (tskno_SCN_StampTestTask != 0xFFFF) {
		del_tsk(&tskno_SCN_StampTestTask);
	}
#endif
	/* ギアバックラッシュの予約 */
	GearBackLashRequest = TRUE;
...
#if (PRO_IMAGE_PROCESSOR ==	IMAGE_PROCESSOR_LC82103)
/*************************************************************************
	module		:[シェーディング取り込みタスクからのメッセージ処理]
	function	:[
		1.
	]
	return		:[]
	...
	author		:[曽根岡拓]
*************************************************************************/
void	MAN_FromSCN_GetShadingTask(
	UWORD	item,
	UWORD	message,
	UWORD	sub_message1,
	UWORD	sub_message2)
{
	...
	/* シェーディングタスクの停止 */
	if (tskno_SCN_GetShade != 0xffff) {
		del_tsk(&tskno_SCN_GetShade);
	}
	SYS_MachineStatus[SYS_RESOURCE_STATUS] &= ~SYS_SCANNER_BUSY;
	SYS_MachineStatus[SYS_MACHINE_STATUS] &= ~SYS_DOC_STORE;
}
...
/*************************************************************************
	...
	author		:[T.Soneoka]
*************************************************************************/
void	MAN_FromSCN_RealTimeCopyStore(
	UWORD	item,
	UWORD	message,
	UWORD	sub_message1,
	UWORD	sub_message2)
{
	...
	if (tskno_SCN_CopyStoreTask != 0xffff) {
		del_tsk(&tskno_SCN_CopyStoreTask);
	}
...
#endif
	switch (doc_message) {
	case MSG_SCN_DOCSTORE_OK:
	case MSG_SCN_DOCSTORE_STOPSW_ON:
		break;
	case MSG_SCN_DOCSTORE_MAXLINE_OVER:
		SYS_MachineStatus[SYS_SCANNER_STATUS] |= SYS_SCANNER_FEED_ERROR;
		break;
	case MSG_SCN_DOCSTORE_TOP_COVER_OPEN:

#if (0)
** 		/* リアルタイムコピー中にスキャナーカバー開け閉めさえると、実際はカバーが閉まっているにも関わらず
** 		** カバーを閉めてくださいというエラーメッセージが表示される。
** 		** でも、実際はセンサータスクがカバーの閉めを検出しているので、ここでは、マシーンステータスは
** 		** セットしなくても問題はない。
...
** 		SYS_MachineStatus[SYS_COVER_STATUS] |= SYS_COVER_SCANNER_OPEN;
	...
	CMN_ResetMemoyFeederTx();
	CMN_ResetStamp();
#if !defined(STOCKHM2)/* Del by SMuratec C.P 2004/07/05 for scanner copy display */
	CMN_ResetPrimaryMode();
...
#if (PRO_DOC_IN == ENABLE)
 #if (PRO_IMAGE_PROCESSOR == IMAGE_PROCESSOR_LC82103)
	/* カバーオープンをチェックします 1999/03/04 by T.Soneoka */
	if (!(SYS_MachineStatus[SYS_COVER_STATUS] & SYS_COVER_SCANNER_OPEN)) {
		if (DS1()) {
			/* ＡＤＦに原稿がセットされていたら原稿を繰り込む */
			if (tskno_DocumentInTask == 0xffff) {
...
UWORD CheckQuickTx(void)
{
	...
	for (i = 0; i < SYS_EXEC_QUEUE_MAX; i++) {
		if (((SYB_ExecQueue[i].Status == SYS_EXEC_EXECUTE) || (SYB_ExecQueue[i].Status == SYS_EXEC_ERASED))
		&&  (SYB_ExecQueue[i].Kind == SYS_COMMAND_TRX)
		&&  (SYB_ExecQueue[i].Number.Command == (UBYTE)SYS_DocumentStoreNo)) {	/* クイックメモリ送信中 */
...
void SetFaxComStopExecQueue(UWORD queue_no)
{
	...
	if (SYB_ExecQueue[queue_no].Status == SYS_EXEC_EXECUTE) {
		line_no = SYB_ExecQueue[queue_no].StackNumber;
		if (line_no == 0) {
			SYS_MachineStatus[SYS_STOP_SWITCH_STATUS] |= SYS_STOP_FAXCOM;
			CommandErased[0] = 1;
		}
		else {
			SYS_OptionLineStatus[line_no] |= SYS_OPTION_STOP_FAXCOM;
			CommandErased[line_no] = 1;
		}
		SYB_ExecQueue[queue_no].Status = SYS_EXEC_ERASED;
	}
	else {
		line_no = SYB_ExecQueue[queue_no].StackNumber;
		if (line_no == 0) {
...
void SetQucikTxStop(UWORD queue_no)
{
	...
	if (SYB_ExecQueue[queue_no].Status == SYS_EXEC_EXECUTE) {
		line_no = SYB_ExecQueue[queue_no].StackNumber;
		data = (0x01 << line_no);
		SYS_QucikTxStopStatus |= (UWORD)data;
	}
	else {
		line_no = SYB_ExecQueue[queue_no].StackNumber;
		data = (0x01 << line_no);
