...
void	SCN_MirrorMoveTask( void )
{
	...
	case	MIRROR_TASK_TRANSPORT:		/* 輸送用の固定位置へ */
		result = MirrorMoveFromHS(2120);	/* HS ～ 137mm */
		break;
	case	MIRROR_TASK_CCD_ADJUST:		/* CCD調整位置へ */
		result = MirrorMoveFromHS(3457);	/* HS ～ 217+7.5mm */
		break;
	...
	*/
	...
	if (result == MIRROR_MOVE_OK) {
		...
		if ((SYB_MaintenanceSwitch[MNT_SW_C0] & MIRROR_MOVE_FORCE_END)
		 && (result == MIRROR_MOVE_FAILURE)) {
			DEBUG_MirrorNeedInitial = 1;   /* 初期化フラグセット */
		}
	}
	...
	UBYTE	is_speed_up)	/* モーター加速指定 */
{
	...
	if (!HS()) {
		...
		CMN_EnableInterrupt();
		 ...
		 /* ミラーキャリッジが ADF 待機位置にいるときに RXIL がチャタリングを起こすと、
		 ** ミラーキャリッジが ADF 待機位置よりさらに左側に走り出してミラーキャリッジエラーになる
		 ** 不具合修正。
		 ** H.Kubo 1999/11/06
		 */
...
UBYTE	MirrorMoveToFBS( void )
{
	...
	SCN_SetHS_OFF_Int( SCN_DISABLE );		/* ホームセンサOFF割り込み禁止 */
	SCN_SetMotorClockwise( SCN_ENABLE );	/* モーター正転 */
	SCN_StartMirror( TRUE );				/* スルーアップでミラー起動 */
...
UBYTE MirrorMoveFBSEndToADF(void)
{
	...
	/* ＡＤＦに原稿をセットした時、ＡＤＦ側でミラーが止まらなくなる件 T.Nose 1997/12/20 */
	if (SYB_MaintenanceSwitch[MNT_SW_C0] & MIRROR_MOVE_FORCE_END) {
		DEBUG_MirrorMoveCountStart = 0;   /* カウント開始フラグリセット */
	}
...
/*			SCN_SetMotorIntStep( (UWORD)SYB_MaintenanceSwitch[MNT_SW_C7], SCN_MotorPointer->ExciteMethod ); */
			SCN_SetMotorIntStep( FBS_STEP_ES_ON_TO_LAMP_CHECK, SCN_MotorPointer->ExciteMethod ); /* 1998/02/08 QAT031 */
			SCN_LatchMotorSpeedAndInterval();
			SCN_MirrorMotorPulseCount = 0;
			SCN_WatchEndSensorStatus = WATCH_ES_OFF;
	...
	case	WATCH_ES_OFF:
		...
		*/
		if ((SCN_MirrorCarriageStatus == MIRROR_MOVE_TO_ADF)
		 ...
		 && !ES()) {
			...
			/* ＡＤＦに原稿をセットした時、ＡＤＦ側でミラーが止まらなくなる件 T.Nose 1997/12/17 */
			if (SYB_MaintenanceSwitch[MNT_SW_C0] & MIRROR_MOVE_FORCE_END) {
				DEBUG_MirrorMoveTimeCounter = (BYTE)43; /* ES OFF から 430mS 監視 */
				DEBUG_MirrorMoveCountStart = 1;   /* カウント開始フラグセット */
			}
		...
		*/
		...
		/* ミラーキャリッジ待機位置切り替え可能仕様 */
			...
			/* ＡＤＦに原稿をセットした時、ＡＤＦ側でミラーが止まらなくなる件 T.Nose 1997/12/17 */
			if (SYB_MaintenanceSwitch[MNT_SW_C0] & MIRROR_MOVE_FORCE_END) {
				DEBUG_MirrorMoveTimeCounter = (BYTE)55; /* ES OFF から 550mS 監視 */
				DEBUG_MirrorMoveCountStart = 1;   /* カウント開始フラグセット */
			}
		}
		break;
...
/*************************************************************************
	...
	function	:[
		１０ｍＳ割り込みからか呼ばれる
		ミラーがＡＤＦ読み取り位置へ移動中の時、モーター割り込みが正常に
		入っているか監視する
	]
	return		:[無し]
	common		:[
...
{
	CMN_DisableInterrupt();
	if (((SCN_MirrorCarriageStatus == MIRROR_MOVE_TO_ADF)         /* ミラーがＡＤＦへ移動中 */
	 || (SCN_MirrorCarriageStatus == MIRROR_MOVE_FBS_END_TO_ADF)) /* ミラーがＦＢＳ読取り終了後、ＡＤＦへ移動中 */
	 && (DEBUG_MirrorMoveCountStart == 1)) {                     /* ＥＳ ＯＦＦを通過 */

		DEBUG_MirrorMoveTimeCounter--;
		if (DEBUG_MirrorMoveTimeCounter <= 0) { /* 規定のカウント数をオーバー */
			...
			/* 異常終了 */
			SCN_MirrorMoveStatus = MIRROR_MOVE_FAILURE;
			SYB_MaintenanceSwitch[MNT_SW_C5] = 10;
		}
	}
	CMN_EnableInterrupt();
}
