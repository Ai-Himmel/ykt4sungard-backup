...
/* ＰＯＰＬＡＲ＿Ｈモデムチェッカー用プログラムです。 By O.Kimoto 1998/03/05 */
	...
	MEM_IndexTable[0].LineCount = 2008;
	...
	/** ダミーデータの蓄積 */
	dma_address = (UWORD *)MEM_STOREAREA_START_ADDRESS;

	for (count = 0; count < (250 - 8); count ++) {
		*(dma_address + count) = 0xffff;
	}
	
	*(dma_address + 242) = 0x0008;
	*(dma_address + 244) = 0x8000;
	*(dma_address + 246) = 0x0880;
	*(dma_address + 248) = 0x0008;
	*(dma_address + 250) = 0x8000;
