...
/*	void DPR_RemoteDataExecTask(void){} */
	...
	/* void DPR_GetBatchFile(void){} */ /* Deleted by H.Kubo 1998/02/23 */
...
/*#if(0) アリロ用アブスを作るときは、、コメントにしとく */
#if (PRO_CIPHER == DISABLE)
	void SCD_CipherTask(void){}

 #if (PRO_CPU_VERSION == SH7043_R)
	void SCD1LineImgToRlIntRom(void) {}
	void SCD1LineRlToImgIntRom(void) {}
 #endif
#else
#if(0)/*By Y.Suzuki 1998/02/19*/
	void AlirooScramble(void) {}
	void AlirooDescramble(void) {}
	void init_freeptr(void){}
	int		should_ask_cutting = 0;
	int		glb_err = 0;
	int		loc_slider = 0;
	int		slider_size = 0;
#endif
#endif
/*#endif* アリロ用アブスを作るときは、、コメントにしとく */
...
#if defined(POPLAR_F)	/* 1999/10/05 Y.Murata */
