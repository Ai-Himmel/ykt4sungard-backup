...
/* #include "\src\atlanta\define\scn_def.h"	1998/09/14 H.Yoshikawa */
#include "\src\atlanta\define\scn_pro.h"
#include "\src\atlanta\define\mon_pro.h"
#include "\src\atlanta\define\cmn_pro.h"
#include "\src\atlanta\define\uni_pro.h" /* ブザー音ＯＦＦのＳＷを追加(SAKAKI) 1999/08/23 M.Maeda */
#include "\src\atlanta\define\unisw_h.h" /* ブザー音ＯＦＦのＳＷを追加(SAKAKI) 1999/08/23 M.Maeda */
...
/* ＨＩＮＯＫＩはブザーの鳴らし方が従来の方式と違うので
** 別ファイルにします。
** By O.Kimoto 1998/09/26
*/
...
void	AckBuzzerSound(void)
{
	...
	/* ブザー音ＯＦＦのＳＷを追加 1999/08/23 M.Maeda */
	if( CHK_UNI_BuzzerVolume() == BUZZER_VOLUME_OFF ){
		return;
	}
#endif
...
void	CMN_PaperSupplyBuzzer(void)
{
	...
	SCN_SetBuzzerFrequency(PAPER_SUPPLY_BUZZER_FREQ);
	while (cycle--) {
		SCN_SetBuzzer(buzzer_status);
