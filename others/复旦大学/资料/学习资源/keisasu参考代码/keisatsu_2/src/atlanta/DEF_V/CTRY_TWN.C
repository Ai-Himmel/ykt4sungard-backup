...
const unsigned char far CountryTable[COUNTRY_MAX][COUNTRY_DATA_MAX] = {
	{
		/*----------------------------------*/					/* とりあえずUSAをそのまま移植 仕様を確認すること!!  1996.08.24 by H.Yagi */
		/*				TWN					*/
		/*----------------------------------*/
/*0*/	0
		|	RELAY_TX				/* 	中継指示送信 			*/
...
/*6*/	99							/*	最大ポーズ動作回数		*/
		,
/*7*/
#if defined (HINOKI) || defined (HINOKI2) || defined (HINOKI3) || defined (KEISATSU) || defined (SATSUKI2) || defined (STOCKHM2)/* HINOKI2 2001/11/12 T.Takagi */
		2							/*	ポーズ時間最小値		*//* 3 -> 2 1999/01/27 by H.Hirao */
#else
		3							/*	ポーズ時間最小値		*//* 5 -> 3  1996/09/04 by H.Yagi */
#endif
		,
/*8*/	10							/*	ポーズ時間最大値		*/	
		,
...
/*15*/
		...
		9							/* 	トーンアッテネータの初期値 */
#endif
#if (PRO_MODEM == R288F)	/* 1998/02/09 Y.M */
 #if defined (HINOKI) || defined (HINOKI2) || defined (HINOKI3) || defined (KEISATSU) || defined (SATSUKI2) || defined (STOCKHM2)/* HINOKI2 2001/11/12 T.Takagi */
		10							/* 	トーンアッテネータの初期値 *//* 8 -> 10 1999/01/28 by H.Hirao */
 #else
		8							/* 	トーンアッテネータの初期値 */
 #endif
#endif
...
/*18*/
#if defined (HINOKI) || defined (HINOKI2) || defined (HINOKI3) || defined (KEISATSU) || defined (SATSUKI2) || defined (STOCKHM2)/* HINOKI2 2001/11/12 T.Takagi */
		2							/*	ポーズ時間の初期値		*//* 3 -> 2 1999/01/27 by H.Hirao */
#else
		3							/*	ポーズ時間の初期値		*//* 5 -> 3  1996/09/04 by H.Yagi */
#endif
		,
/*19*/	2							/*	呼出ベル回数の初期値	*/
		,
...
/*24*/	NOT_USED					/*	未使用					*/
	...
	{
		/*----------------------------------*/					/*  9/Feb/98 By M.Harada CTRY_AUS.CよりSGPのﾊﾟﾗﾒｰﾀを移動 */
		/*		HKG/SGP/SAU/MYS/IND			*/
		/*----------------------------------*/
/*0*/	0
		|	RELAY_TX				/* 	中継指示送信 			*/
...
/*24*/	NOT_USED					/*	未使用					*/
	...
	{
		/*----------------------------------*/					/*  9/Feb/98 By M.Harada CTRY_AUS.CよりTHAのﾊﾟﾗﾒｰﾀを移動 */
		/*				THA					*/
		/*----------------------------------*/
/*0*/	0
		|	RELAY_TX				/*	中継指示送信			*/
...
/*15*/
#if (PRO_MODEM == ORANGE3)
		9							/*	トーンアッテネータの初期値 */ /* 1996/08/29 by H,Yagi */
#endif
#if (PRO_MODEM == R288F)
		12							/*	トーンアッテネータの初期値 */
#endif
...
/*4*/	NOT_USED					/* 未使用					*/
		...
		/*=======================================================
			MYS申請機にAL-100がｵﾌﾟｼｮﾝである場合、DTMFの送出
			ﾚﾍﾞﾙを2dB下げる設定にﾊﾟｯﾁ変更する。
										25/Feb/98 by M.Harada
		=======================================================*/
	{
		/*----------------------------------*/
		/*	2			THA					*/
