...
*/
void	FLA_WaitSoftWareTimer(UWORD time)
{
	while (time > 0) {
		time--;
	}
...
void	FLA_LCD_Initialize(void)
{
	...
	/* 15msec以上待つ */
	FLA_WaitSoftWareTimer(50000);	/* 約16msec(10000) */
	...
	/* 4,1msec以上待つ */
	FLA_WaitSoftWareTimer(30000);	/* 約10msec(6000) */
	...
	/* 100usec以上待つ */
	FLA_WaitSoftWareTimer(2500);		/* 約1msec(500) */
	...
	/* 15msec以上待つ */
	FLA_WaitSoftWareTimer(45000);	/* 約14msec(9000) */
	...
	/* 表示ＯＦＦ 			*/
	/*----------------------*/
	FLA_LCD_WriteInstruction(LCD_DISPLAY_OFF);
	...
	/* 表示クリア 			*/
	/*----------------------*/
	FLA_LCD_WriteInstruction(LCD_CLEAR_DISPLAY);
	...
	/* エントリーモードセット */
	/*----------------------*/
	FLA_LCD_WriteInstruction(LCD_ENTRY_MODE);
	...
	/* 表示ＯＮ 			*/
	/*----------------------*/
	FLA_LCD_WriteInstruction(LCD_DISPLAY_ON);
...
void FLA_InitializeGateArray(void)
{
