...
void Timer_int(void);	/* モニター１０ｍｓ割込処理 */
void Oneshot_int(void);	/* モニタワンショットタイマー割込み処理 */
...
/******************************************************************************
	...
	author		:[川崎真史]
******************************************************************************/
extern UWORD SYS_Modem2PortStatus;	/* 1998/08/04 */
...
void INT_Timer10msProcess(void)
{
	...
	if ((PrinterStatus[PRN_STATUS_0] & PRN_S0_PRINTING)
	 && (PrinterStatus[PRN_STATUS_8] & PRN_S8_PSS)) {	/* PSS追加 1998/02/06 */
		if (PRN_PSS_ChatCounter > 0) {	/* PSSチャチリング防止カウンタ 1999/06/10 */
			PRN_PSS_ChatCounter--;
		}
		if (!PSS() && !PRN_PSS_ChatCounter) {
/*		if (!PSS()) {	Del 1999/06/10 */
...
/*#if (PRO_KEYPANEL != PANEL_HINOKI) *//* HINOKIはGA割り込みの中で処理します 1998/09/22 by T.Soneoka */
