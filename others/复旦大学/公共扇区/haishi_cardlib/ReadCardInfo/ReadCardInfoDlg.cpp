...
BOOL CReadCardInfoDlg::OnInitDialog()
{
	...
	GetPrivateProfileString("LOADLIBRARY", "COM", "1", bufCom, sizeof(bufCom), iniDllFilePathBuf);
	GetPrivateProfileString("LOADLIBRARY", "BAUD", "19200", bufBaudRate, sizeof(bufBaudRate), iniDllFilePathBuf);
	//csCom.Format("COM%s", bufCom);
	//m_com_portNo.SetWindowText(csCom);//
	//m_com_baudRate.SetCurSel(2);
	m_isOpenCom = FALSE;
	m_nSize = 0;
	//this->SendMessage(WM_COMMAND, MAKEWORD(BN_CLICKED, ID_BTN_CONNECT), (LPARAM)m_btnConnect.m_hWnd);
...
// TODO: Add your control notification handler code here
	...
	else
	{
		...
		//csCom.Format("%s", bufCom);
		//csCom.Format("%s", csBaudRateStr);
		//m_com_baudRate.GetWindowText(csBaudRateStr);
		m_nPort = atoi(bufCom);
		m_nBaudRate = atoi(bufBaudRate);
...
void CReadCardInfoDlg::OnBnClickedBtnRecpassword()
{
	...
	if (IsOpneCom())
	{
		...
		if (ret)
		{
			::AfxMessageBox("从服务器获取密钥失败, 请读取授权卡!");
			m_btnReadAuthorCard.EnableWindow(TRUE);
			return ;
		}
		m_btnRecPassword.EnableWindow(FALSE);
