...
int __stdcall PublishCard(TPublishCard *pc)
{
	...
	// 修改公共扇区
	ret = ModifyPubSector(PUB_SECT1, CARD_INIT_KEY);
	if (ret)
	{
		DEBUG_CARD_MSGOUT(ret, "修改公共扇区失败");
		return ret;
	}
...
int __stdcall SMT_ClearCard()
{
	...
	DEBUG_CARD_MSGOUT(1000,"初始化12个扇区成功");
	memset(ucRawData,0xFF,16);
	j = 0;
	for(i=0;i<4;i++)
	{
	...
	DEBUG_CARD_MSGOUT(1000,"初始化钱包成功");
	//1 使用用户密码登陆
	ret = SMT_Login_With_UserKey(3,ucKeyA,0);
	if(ret != 0)
	...
	if(ret != 0)
		return -7;//无法写卡
	DEBUG_CARD_MSGOUT(1000,"初始化版本号成功");
	// 删除公共扇区内容
	return 0;
}
...
static int WriteCardBlock(int *err_code,PWriteDataFunc pFunc,LPProcessCallBack lpCallBack)
{
	...
	//////////////////////////////////////////////////////////////////////////
	// 海事大学更新 15 扇区密钥
	//BYTE card_key_d[] = {0x09, 0x04, 0x00, 0x05, 0x02, 0x07};		
	BYTE card_key_d[7];		// 只读
	BYTE card_key_e[7];		// 可写

	memcpy(card_key_d,CARD_PUB_KEYB,sizeof card_key_d);
	memcpy(card_key_e,CARD_PUB_KEYB,sizeof card_key_e);
	//////////////////////////////////////////////////////////////////////////
									   ...
									   short * readFlag)
{
	...
	{
		//if(AUTH_CARD_FLAG == wantFlag || -1 == wantFlag)	//del by chen 2011-110-2
		{
			has_read_main_key = TRUE;
			ret = 0;
									 ...
									 ,short block,LPGetBlockData lpFunc)
{
	...
	memcpy(card_key_d,CARD_PUB_KEYA,sizeof card_key_d);
	memcpy(card_key_e,CARD_PUB_KEYB,sizeof card_key_e);
...
//	memcpy(fee_rate_data_buf, &fee_rate_data_length, sizeof(BYTE) * 2);
	...
	// 第0扇区
	if(strcmp((char*)fee_rate_card_info->water_card_flag,"TJSK")==0)
	{
		strcpy((char*)fee_rate_card_info->water_card_flag,"LYCS");
		//memcpy(buf,"LYCS",offset += (sizeof(fee_rate_card_info->water_card_flag) -1));
		key_flag = 0x33;
	}
	...
	//sprintf(keystr,"%s",fee_rate_card_info->water_card_flag);
...
//	MessageBox(NULL, (char*)fee_rate_card_info->main_work_key, NULL, NULL);
	...
	//dec2hex(fee_rate_card_info->main_work_key,8,keystr);
	//DEBUG_CARD_MSGOUT(-50,keystr);
...
*/		
	...
	int block_total_num = 1 + fee_rate_data_length / 16;
...
int __stdcall ReadCollectCardInfo(FEE_RATE_CARD_INFO *fee_rate_card_info)
{
	...
	if(buf[9] == 0x33)
	{
		memcpy(fee_rate_card_info->water_card_flag,"TJSK",4);
	}
...
int __stdcall ModifyPubSector(int sector, unsigned char login_key[7])
{
	CardKeyData card_key[4];
	unsigned char card_id[5] = "";
	unsigned char card_type[2] = "";
	...
	unsigned char stuemp_no[21] = "";
	unsigned char name[9] = "";
	unsigned char sex[2] = {0};
	...
	ret = SMT_ReadBlock(2, 0, ucBuffer);
	if (ret != 0 )	return -6;
	memcpy(name, ucBuffer, 8);
	//读性别
	ret = SMT_ReadBlock(2, 2, ucBuffer);
	memcpy(sex, ucBuffer+14, 1);

	if (LoginCardSect(sector, CARD_PUB_KEYB
		,CARD_KEY_B, card_type) == 0)
	{
		key_pos = 4;
	}
	...
	// 扇区0
	memcpy(ucBuffer, stuemp_no, 16);
	if (ret = WriteCardData(sector, 0, ucBuffer)) { return ret; }
	
	// 扇区1
	memset(ucBuffer, 0, sizeof(ucBuffer));
	memcpy(ucBuffer, stuemp_no + 16, 4);
	memcpy(ucBuffer + 4, name, 8);
	...
	// key B 标准 KEY B
	memcpy(sect_data + CARD_KEY_LEN + sizeof(CARD_FOR14_CTRL_BIT), card_key_b, CARD_KEY_LEN);
...
static int ModifyPubInfo()
{
	...
	if (LoginCardSect(sector, card_key_b,CARD_KEY_B, card_type) == 0)
	{
		key_pos = 4;
	}
	else
...
int __stdcall ClearPubSector()
{
	...
	DEBUG_CARD_MSGOUT(0,"清空公共扇区");
	// 检查卡是否存在
	if (SMT_RequestCardExist(card_id, card_type) != 0 )
	{
		DEBUG_CARD_MSGOUT(0,"检查卡失败");
		return E_READ_CARD_ERROR;
	}
	...
	if (gen_user_card_keyB(SMT_UCWORKINGKEY,card_id
		,card_key_b) != 0)
	{
		DEBUG_CARD_MSGOUT(0,"登录失败");
		return E_GEN_CARD_KEY_B_ERROR;
	}

	DEBUG_CARD_MSGOUT(0,"登录公共扇区");
	if (LoginCardSect(sector, card_key_b,CARD_KEY_B, card_type) == 0)
	{
		DEBUG_CARD_MSGOUT(0,"登录公共扇区成功");
		key_pos = 4;
	}
	else
	{
		DEBUG_CARD_MSGOUT(1,"登录公共扇区失败");
		return KS_ERR_WRTIEDPUBSECT;
	}

	DEBUG_CARD_MSGOUT(0,"写公共扇区数据");
	// 扇区0
	memset(ucBuffer, 0, sizeof(ucBuffer));
	if ((ret = WriteCardData(sector, 0, ucBuffer)) !=0) 
	{ 
		DEBUG_CARD_MSGOUT(1,"清空公共扇区失败");
		return ret; 
	}

	// 扇区1
	memset(ucBuffer, 0, sizeof(ucBuffer));
	if ((ret = WriteCardData(sector, 1, ucBuffer))!=0) 
	{ 
		DEBUG_CARD_MSGOUT(1,"清空公共扇区失败");
		return ret; 
	}
	DEBUG_CARD_MSGOUT(0,"清空公共扇区成功");
	return 0;
}
