...
public class GetQueryConListDAO {
	...
	public List getShopTreeList(String operCode) {
		if(null == operCode)
			return null;
		StringBuffer sqlStr = new StringBuffer(2048);
		if (isShopOwner(operCode)){
		//	sqlStr.append("Select shop_id,shop_name,dept_id,'' dept_name from YKT_CUR.T_CIF_SHOP where shop_state=1");
			sqlStr.append("select shop_id,shop_name,dept_id from (")
			.append(" select t.shop_state,t.shop_id, t.dept_id, SUBSTR(LPAD('  ', 2*level-1)||shop_name,2) as shop_name from t_cif_shop t")
			.append(" start with t.dept_id = '1'")
			.append(" connect by prior t.shop_id = t.dept_id")
			.append(" order siblings by t.dept_id )t")
			.append(" where t.shop_state=1");
		}else{
	...
	public List getCardTypeList(){
		Session session = HibernateSessionFactory.currentSession();
		try {
			SQLQuery sqlquery = session.createSQLQuery("select dict_value,DICT_CAPTION from ykt_cur.t_pif_dictionary where dict_no=18 order by dict_value");
			sqlquery.addScalar("dict_value",Hibernate.STRING);
			sqlquery.addScalar("DICT_CAPTION",Hibernate.STRING);
			List list = sqlquery.list();
			//Collections.sort(list,new SortListColl());
			return list;
		} catch (HibernateException e) {
	 ...
	 * @return
	 */
	public List getDevUseList(){
		Session session = HibernateSessionFactory.currentSession();
		try {
			SQLQuery sqlquery = session.createSQLQuery("select dict_value,DICT_CAPTION from ykt_cur.t_pif_dictionary where dict_no=81 order by dict_value");
			sqlquery.addScalar("dict_value",Hibernate.STRING);
			sqlquery.addScalar("DICT_CAPTION",Hibernate.STRING);
			List list = sqlquery.list();
			//Collections.sort(list,new SortListColl());
	 ...
	 * @return
	 */
	public List getSerialTypeList(){
		Session session = HibernateSessionFactory.currentSession();
		try {
			SQLQuery sqlquery = session.createSQLQuery("select tradecode,trademsg from YKT_CUR.t_pif_tradecode order by tradecode");
			sqlquery.addScalar("tradecode",Hibernate.STRING);
			sqlquery.addScalar("trademsg",Hibernate.STRING);
			List list = sqlquery.list();
			Collections.sort(list,new SortListColl());
			return list;
		} catch (HibernateException e) {
	 ...
	 * @return
	 */
	public List getDeptTreeList(){
		Session session = HibernateSessionFactory.currentSession();
		StringBuffer sqlstr = new StringBuffer(512);
		sqlstr.append("select t.dept_code, t.parentdept_code, SUBSTR(LPAD(' ', 2*level-1)||dept_name,2) as dept_name from t_cif_dept t")
		.append(" start with t.parentdept_code = 'NA'")
		.append(" connect by prior t.dept_code = t.parentdept_code")
		.append(" order siblings by t.dept_code ");
		try {
			SQLQuery sqlquery = session.createSQLQuery(sqlstr.toString());
			sqlquery.addScalar("dept_code",Hibernate.STRING);
			sqlquery.addScalar("parentdept_code",Hibernate.STRING);
			sqlquery.addScalar("dept_name",Hibernate.STRING);
			List list = sqlquery.list();
			return list;
		} catch (HibernateException e) {
