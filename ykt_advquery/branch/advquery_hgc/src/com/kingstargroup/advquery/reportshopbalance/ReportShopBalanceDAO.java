...
public class ReportShopBalanceDAO {
	...
	public List getShopAccountCompare(String shopId,String beginDate,String endDate,String statType){
		...
		if (("bydate".equals(statType))||("byweek".equals(statType))){
			execSQL.append("select rsb.shop_id shopid,rsb.shop_name shopname,sum(rsb.trade_Num)tradenum,")
			.append(" sum(rsb.trade_Amt)tradeamt,sum(rsb.mng_Amt)mngamt,sum(rsb.total_Amt)totalamt")
		...
		try {
			...
			query.addScalar("tradeamt",Hibernate.FLOAT);
			query.addScalar("mngamt",Hibernate.FLOAT);
			query.addScalar("totalamt",Hibernate.FLOAT);
			List list = query.list();
