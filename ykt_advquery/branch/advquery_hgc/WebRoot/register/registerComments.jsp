...
<html>
  ...
  <body  >
    <%  
       Map regMap = (Map)request.getAttribute("regMap");
    %>
      
     <html:form  action="/registerComment.do?cmd=update">
     <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#f8f8f8">
     <tr><td>
       <c:out value="${regMap.cutName}"/> 
        <input type="hidden" name="custId" value="${regMap.custId}"/>
        <input type="hidden" name="regId" value="${regMap.regId}"/>
       <html:textarea   property="comments"   cols="40" rows="10" value="${regMap.comments}"></html:textarea>
      </td></tr>    
     </table>  
