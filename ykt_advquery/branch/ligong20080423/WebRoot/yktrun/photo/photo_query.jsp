  ...
  <tr>
    <td height="40"><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td nowrap="nowrap" width="20%"><select name="emptype" >
  ...
  <tr>
    <td height="40"><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
		...
		<%
			List typeList1 = new GetQueryConListDAO().getEmpType();
			String cutType1 = "";
			if (null!=session.getAttribute("cuttype")){
				cutType1 = session.getAttribute("cuttype").toString();
			}
			Iterator iter1 = typeList1.iterator();
			//for (int i = 0; i < typeList.size(); i++) {
			while (iter1.hasNext()){
				Object[] value= (Object[])iter1.next();
				if (cutType1.equals(value[0])){
		%>
		<option value="<%=value[0]%>" selected="selected"><%=value[1]%></option>
		<%}else{%>
		<option value="<%=value[0]%>"><%=value[1]%></option>
