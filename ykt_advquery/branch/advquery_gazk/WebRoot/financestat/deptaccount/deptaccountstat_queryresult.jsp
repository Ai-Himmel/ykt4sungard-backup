...
<ec:table items="result" 
		title='<%=deptName+GetProperty.getProperties("deptaccount.title",getServletContext())+dateRange%>' 
		filterable="false"
	...
	<ec:row highlightRow="true">
		...
		<ec:column property="saving" headerStyle="text-align:center"
			...
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("deptaccount.saving",getServletContext())%>' />	
		...
		<ec:column property="deposit" headerStyle="text-align:center"
			cell="currency"
			format="###,###,###,##0.00"
			style="text-align:right"
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("deptaccount.deposit",getServletContext())%>' />
		<ec:column property="managefee" headerStyle="text-align:center"
			cell="currency"
			format="###,###,###,##0.00"
			style="text-align:right"
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("deptaccount.managefee",getServletContext())%>' />
	</ec:row>
...
<cewolf:chart 
	id="verticalBar" 
	title='<%= deptName+GetProperty.getProperties("deptaccount.title",getServletContext())%>'
	type="<%=picType%>"
	yaxislabel="<%=yLabel%>"
	> 
