...
public class UserUtil extends BasicUtil {
	...
	public static  TLeadqueryOperator  getUserByUserId(String  username){
		UserDAO dao = UserDAO.getInstance();
		TLeadqueryOperator user = null;
		try {
			user = (TLeadqueryOperator) dao.findByPrimaryKey(TLeadqueryOperator.class, username);
			return user;
		} catch(HibernateException he) {
			return user;
		} finally {
	...
	public static List   getUsers(String userId,String userName){
		UserDAO dao = UserDAO.getInstance();
		try{
			 return dao.queryUsers(userId,userName);
