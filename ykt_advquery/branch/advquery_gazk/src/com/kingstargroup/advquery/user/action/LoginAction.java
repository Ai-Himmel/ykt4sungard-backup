...
import com.kingstargroup.advquery.util.MD5;
...
public class LoginAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, 
		...
		String unify = ParamUtil.getString(req,"unify");
		...
		if(unify.equals("")){
		 ...
		 //统一身份认证  
			...
			if (username.equals("")||!password.equalsIgnoreCase(md5.compute())) {
				req.setAttribute("unify",unify);
				return mapping.findForward("eadvquery.login");
			}
			 
			req.setAttribute("pageId", pageId);
			HttpSession session = req.getSession();
			  session.setAttribute("username", username);//身份证号
			  CustomerUtil cust = new CustomerUtil();
			  String name ="";
			  List list = cust.getCustomerBymanId(username);
			  ...
			  iniDate(req);
	        //判断我们的系统里是否有此人，如果没有，到登陆页
			TLeadqueryOperator user =	UserUtil.getUserByUserId(username);
			if(user == null){
				req.setAttribute("unify",unify);
				return mapping.findForward("eadvquery.login");
			}
			if(pageId!=null && !"".equals(pageId)){
			    return new ActionForward(pageId);
			}
		  }
		  HttpSession session = req.getSession();
		  session.setAttribute("username", username);//身份证号
