...
public class bccclt
{  
	  private static final Logger _log = Logger.getLogger(bccclt.class);
   // 函数名: SetDebugSwitch
   ...
   // 参数  : byte[] errormessage [OUT]: 返回的错误信息
	public native boolean CallRequest(long Handle, int drtpno, int branchno,int function, int timeout, int[] errcode,byte[] errormessage);
   ...
   public native boolean Broad(long Handle, int drtpno, int destno, int funcno, int[] errcode, byte[] errmsg);
	   ...
	   static {
	   		try {
	   			...
	   			System.loadLibrary("bccclt_adv");
	   		} catch (UnsatisfiedLinkError e) {
	   			System.err.println("不能加载so文件(can not load so file!):\n " + e.toString());
	   		}
