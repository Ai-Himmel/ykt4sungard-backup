...
public class GetQueryConListDAO {
	...
	public List getShopList(String operCode) {
		...
		if (isShopOwner(operCode)){
			sqlStr.append("Select shop_id,shop_name,dept_id,'' as dept_name from YKT_CUR.T_CIF_SHOP");
		}else{
	...
	/*public List getPosDeviceListByLimit(String operCode){
		...
		if (isShopOwner(operCode)){
			...
			.append(" where state_id<>5 and devtype in ('1005','0226')");
		}else{
			sqlStr.append("with shoplog(shop_id,shop_name,dept_id,dept_name)as(")
			.append(" select a.shop_id,a.shop_name,a.dept_id,'' from YKT_CUR.T_CIF_SHOP a")
			.append(" where shop_state=1 and shop_id in (select int(store_code) ")
	...
	public List getPosDeviceList(String posId){
		Session session = HibernateSessionFactory.currentSession();
		try {
			Query query = session.createQuery(" from TPifDevice where devtype in('1055','0226') and dev999_id=:posid");
	...
	public List getDeptList(){
		...
		sqlStr.append("Select dept_code,dept_name from YKT_CUR.T_CIF_DEPTFULL")
		.append(" where DEPT_LEVEL=1 order by dwglm");
		try {
			SQLQuery sqlquery = session.createSQLQuery(sqlStr.toString());
			sqlquery.addScalar("dept_code",Hibernate.STRING);
			sqlquery.addScalar("dept_name",Hibernate.STRING);
			List list = sqlquery.list();
			//yCollections.sort(list,new SortListColl());
			return list;
		} catch (Exception e) {
			e.printStackTrace();
			_log.error(e);
	...
	public List getEmpType(){
		Session session = HibernateSessionFactory.currentSession();
		try {
			SQLQuery sqlquery = session.createSQLQuery("Select distinct cut_type from YKT_CUR.T_CIF_PHOTO1 where cut_type is not null");
	...
	public List getCustNameByID(String custID){
		Session session = HibernateSessionFactory.currentSession();
		try { 
			Query query = session.createQuery(" from TCifCustomer where stuemp_no = :custID");
