...
public class ReportDataExport {
	...
	public ReportDataExport(String path, String endWeekDate, String createDate,
		...
		List areaConsume30DaysResult = ReportShopBalanceUtil
				.getAreaSalesCompare(consume30DaysDate, createDate, "bydate");
		// POS和圈存转账情况对比
		...
		try {
			...
			// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			// 各校区30天消费情况对比（笔数和金额）
			areaConsume30daysMoneyChart(areaConsume30DaysResult, session,
					document);
			areaConsume30daysNumChart(areaConsume30DaysResult, session,
					document);
			// 30周消费情况（金额和笔数）
			...
			billSavingCompare30DaysNumChart(billSaving30DaysResult, session,
					document);
			// 各校区POS充值情况对比（金额和笔数）
			posSavingCompare30DaysMoneyChart(posSaving30DaysResult, session,
					document);
			posSavingCompare30DaysNumChart(posSaving30DaysResult, session,
					document);
