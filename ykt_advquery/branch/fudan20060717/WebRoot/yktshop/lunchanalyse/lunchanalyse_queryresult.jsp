...
<ec:table items="result" 
	...
	<ec:row highlightRow="true">
		...
		<ec:column property="totalMeal"  headerStyle="text-align:center"
			filterable="false" 
			style="text-align:left"
			sortable="false" 
			title='<%= GetProperty.getProperties("lunchtable.meal",getServletContext())%>' />
		<ec:column property="totalDay"  headerStyle="text-align:center"
			filterable="false" 
			style="text-align:left"
			sortable="false" 
			title='<%= GetProperty.getProperties("lunchtable.day",getServletContext())%>' />	
		<ec:column property="totalAmount"  headerStyle="text-align:center"
			cell="currency"
			format="###,###,###,##0.00"
...
<script>
      	...
      	function renderCancelPage() {
      		self.location = '<%= ContextUtil.getCtxPath(request)%>/lunchanalyse.do';
		}
</script>
