...
<ec:table items="result" 
	...
	<ec:row highlightRow="true">
		...
		<ec:column property="consumenum" headerStyle="text-align:center"
			...
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("deptconsume.num",getServletContext())%>' />	
		...
		<ec:column property="avgmoney" headerStyle="text-align:center"
			cell="currency"
			format="###,###,###,##0.00"
			style="text-align:right"
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("deptconsume.avgmoney",getServletContext())%>' />
	</ec:row>
...
<cewolf:chart 
	id="verticalBar" 
	title='<%= deptName+GetProperty.getProperties("deptconsume.title",getServletContext())%>'
	type="<%=picType%>"
	yaxislabel="<%=yLabel%>"
	> 
