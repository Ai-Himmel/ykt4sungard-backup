...
public class GetQueryConListDAO {
	...
	public List getPosDeviceList(String posId){
		Session session = HibernateSessionFactory.currentSession();
		try {
			SQLQuery query = session.createSQLQuery("select DEVICE_NAME  from ykt_cur.T_PIF_DEVICE where DEVTYPE in('1055','0226') and DEV999_ID=:posid");
