...
<%String dateRange = request.getAttribute("rangedate").toString();%>
...
<ec:table items="result" 
		title='<%= GetProperty.getProperties("allaccount.title",getServletContext())+dateRange%>' 
		filterable="false"
	...
	<ec:row highlightRow="true">
		...
		<ec:column property="saving"  headerStyle="text-align:center"
			...
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("allaccount.saving",getServletContext())%>' />	
		<ec:column property="consume"  headerStyle="text-align:center"
			...
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("allaccount.consume",getServletContext())%>' />
		<ec:column property="ensure"  headerStyle="text-align:center"
			...
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("allaccount.ensure",getServletContext())%>' />
	</ec:row>
...
<cewolf:combinedchart
	id="combinedChart"
	layout="vertical"
	title='<%= GetProperty.getProperties("allaccount.title",getServletContext())%>'
	type="combinedxy"
	>
    <cewolf:colorpaint color='<%= GetProperty.getProperties("chart.bgcolor",getServletContext())%>'/>
		...
		<cewolf:plot type="xyline" yaxislabel="<%=yLabel%>">
	        ...
	        <cewolf:param name="4" value='<%= GetProperty.getProperties("chart.customer6",getServletContext()) %>'/>
	        <cewolf:param name="5" value='<%= GetProperty.getProperties("chart.customer5",getServletContext()) %>'/>
