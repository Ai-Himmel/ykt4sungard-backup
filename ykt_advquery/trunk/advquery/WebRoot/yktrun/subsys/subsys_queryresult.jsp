...
<%String dateRange = request.getAttribute("syssubtitle").toString();%>
...
<ec:table items="result" 
	...
	<ec:row highlightRow="true">
		<ec:column property="log_date" 
			filterable="false" 
			sortable="false"
			title="<%= statTypeShow%>" />
		<ec:column property="numb" 
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("subsys.num",getServletContext())%>' />	
	</ec:row>
...
<cewolf:chart 
	id="line" 
	title='<%= GetProperty.getProperties("subsys.title",getServletContext())%>'
	type="<%=picType%>"
	yaxislabel="<%=yLabel%>"
	> 
	<cewolf:colorpaint color='<%= GetProperty.getProperties("chart.bgcolor",getServletContext())%>'/>
	<cewolf:data>
		<cewolf:producer id="subsysChart" />
	</cewolf:data>
