...
<ec:table items="result" 
		title='<%= GetProperty.getProperties("totlesaving.title",getServletContext())+dateRange%>' 
		filterable="false"
	...
	<ec:row highlightRow="true">
		<ec:column property="balance_date"  headerStyle="text-align:center"
			...
			sortable="false"
			title="<%= statTypeShow%>" />
		<ec:column property="pos"  headerStyle="text-align:center"
			cell="currency"
			format="###,###,###,##0.00"
		...
		<ec:column property="transfer" headerStyle="text-align:center"
			cell="currency"
			format="###,###,###,##0.00"
			style="text-align:right"
			filterable="false" 
		...
		<ec:column property="cash"  headerStyle="text-align:center"
			cell="currency"
			format="###,###,###,##0.00"
			style="text-align:right"		
			filterable="false" 
		...
		<ec:column property="outlay"  headerStyle="text-align:center"
			cell="currency"
			format="###,###,###,##0.00"
			style="text-align:right"
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("totlesaving.outlay",getServletContext())%>' />
		<ec:column property="ticket"  headerStyle="text-align:center"
			cell="currency"
			format="###,###,###,##0.00"
			style="text-align:right"
			filterable="false" 
			sortable="false" 
			title='<%= GetProperty.getProperties("totlesaving.ticket",getServletContext())%>' />
	</ec:row>
...
<cewolf:chart 
	id="verticalBar" 
	title='<%= GetProperty.getProperties("totlesavingmoney.title",getServletContext())%>'
	type="<%=picType%>"
	yaxislabel="<%=yLabel%>"
	> 
	<cewolf:colorpaint color='<%= GetProperty.getProperties("chart.bgcolor",getServletContext())%>'/>
	<cewolf:data>
		<cewolf:producer id="TotleSavingMoneyBar" />
	</cewolf:data>
...
<cewolf:img 
	...
	<cewolf:chart id="pieChart"
		title='<%= GetProperty.getProperties("totlesaving.scale",getServletContext())%>'
		type="pie">
		<cewolf:colorpaint
			color='<%= GetProperty.getProperties("chart.bgcolor",getServletContext())%>' />
		<cewolf:data>
			<cewolf:producer id="TotleSavingMoneyPieData" />
		</cewolf:data>
