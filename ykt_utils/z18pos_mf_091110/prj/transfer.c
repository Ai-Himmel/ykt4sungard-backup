...
static uint8 do_update_blksheet(uint8 *card_data)
{
    uint32 card_id;
    uint8 blk_bit;
    blk_mem_info_t blk;
    ...
    if( (g_global_buf[blk.byte_offset] & blk_bit) == blk_bit)
    {
        // eeprom 中已经是正确的了
        return 0;
    }
...
static uint8 do_update_blksheet_ver(uint8 *blkver)
{
...
static uint8 do_setting_pos(COMM_TYPE_T *comm,COMM_OPERATION_T *op)
{
	uint8 cmd, ret,param;
	...
	do{
		...
		case KNG_COMM_CMD:
			...
			if((param & 0x01) != 0)
			{
				ret = z18pos_read_eeprom(0x00,16,1,comm->buffer2);
				if(ret)
				{
		...
		case KNG_SET_PHYID:
			...
			ret = EeSysWrite(0x00,8,comm->buffer2);
			if(ret != Z18_OK)
			{
				ret = KNG_POS_HARDWARE_ERROR;
				break;
		...
		case KNG_SET_KEY:
			...
			// 数据长度
			ret = EeSysRead(0x40,24,comm->buffer2);
			if(Z18_OK != ret)
			{
				ret = KNG_POS_HARDWARE_ERROR;
				break;
			...
			if(comm->buffer2[22] == 0x55 && comm->buffer2[23] == 0xAA)
			{
				...
				if(memcmp(comm->buffer2,comm->buffer+BUF_PARAM2+1,6)>=0)
				{
					flag1 = 0;
				}
			}
			...
			if(flag1)
			{
				...
				if(EeSysWrite(0x40,24,comm->buffer2) != Z18_OK)
				{
			...
			if(param == 0x01)
			{
				...
				set_2byte_int(crc_code,comm->buffer+24);
		...
		case KNG_SET_CONSUME_PARAM:
			...
			if(EeSysRead(0x60,32,comm->buffer2)!=Z18_OK)
			{
				ret = KNG_POS_HARDWARE_ERROR;
				break;
			}
			...
			if(EeSysWrite(0x60,32,comm->buffer2)!=Z18_OK)
			{
				ret = KNG_POS_HARDWARE_ERROR;
				break;
			}
		...
		case KNG_CARD_RIGHT:
			...
			// 后续包
			//debug_string("cardright[%d]\n",len1-512);
			for(crc_code = 0; crc_code < len;crc_code+=64)
			{
				send_len = (len - crc_code >= 64) ? 64 : (len - crc_code);
		...
		case KNG_CLEAR_SERIAL:
			...
			if(EeSysWrite(0x10,26,comm->buffer2)!=Z18_OK)
			{
				ret = KNG_POS_HARDWARE_ERROR;
				break;
			}
			// 备份块
			if(EeSysWrite(0xA0,26,comm->buffer2)!=Z18_OK)
			{
				ret = KNG_POS_HARDWARE_ERROR;
				break;
			}
		...
		case KNG_SET_WELCOME_MSG:
			...
			if(EeSysWrite(0xD0,16,comm->buffer+BUF_REQ_DATA) != Z18_OK)
			{
				ret = KNG_POS_HARDWARE_ERROR;
				break;
			}
		...
		case KNG_UPDATE_COL_DATE:
			...
			if(EeSysWrite(0x80,6,comm->buffer2+1)!=Z18_OK)
			{
				ret = KNG_POS_HARDWARE_ERROR;
				break;
			}
        ...
        case KNG_GET_BLKVER:
            ...
            if(calc_sum(comm->buffer+BUF_RESP_DATA,6)!=comm->buffer[BUF_RESP_DATA+6])
            {
