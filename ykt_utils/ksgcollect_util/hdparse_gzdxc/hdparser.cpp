...
char *phdHead = "卡号        余额        交易额      日期       时间      钱包流水号 POS流水号   原POS流水号    操作员号    终端序列号  附加数据  商品编号     钱包索引号 记录类型    消费模式     商户      餐段   MAC";

int HDPARSER_API parse_head(char *head)
{	
...
int HDPARSER_API parse_line(char *line,TRADE_SERIAL &tradeSerial)
{
	...
	*/
	...
	strcpy(tradeSerial.device_id,serialArr[9]);
	// 
	...
	else if((trade_type & 0x0F) == 0x0B ) // 补助
	{
		tradeSerial.in_balance = tradeSerial.out_balance - tradeSerial.trade_fee;
		tradeSerial.tx_mark = 0xFB;
		tradeSerial.serial_type = 930033;
	}
	else
