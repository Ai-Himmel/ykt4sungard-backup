...
static char g_xpack_path[2048] = {0};
 ...
 */
int APIDEF KSG_init(const char *ip, int port, int mainfunc)
{
	if (g_drtp_no > -1)
...
/**
 ...
 \return - 错误信息内容
 */
const char * APIDEF KSG_get_errmsg()
{
	return g_errormsg;
}
/**
 ...
 */
KSG_File_Handle APIDEF KSG_begin_request()
{
	...
	if(strlen(g_xpack_path)==0)
	{
		sprintf(g_xpack_path, "%s%s", g_module_path, "cpack.dat");
	}
	handle->mainfunc_ = g_mainfunc;
	handle->xhandle_ = NewXpackHandle(g_xpack_path);
	handle->download_funcno_ = 950998;
	handle->upload_funcno_ = 950999;
	if (handle->xhandle_)
	{
		// 加入链表
		Request_Handle *p = g_head_handle.next_handle_;
...
/**
 ...
 \param handle - 句柄
 */
void APIDEF KSG_free_request(KSG_File_Handle handle)
{
...
static int do_download_file(Request_Handle *req)
{
	...
	SetIntFieldByName(req->xhandle_,0,"lvol0", req->del_svrfile_);
	
	SetRequestType(req->xhandle_, req->download_funcno_);
	...
	else
	{
		...
		if (req->retcode_)
		{
			...
			sprintf(g_errormsg, "%d:%s", req->retcode_,req->retmsg_);
			return KSG_ERR_REQUEST;
		}
		// 请求成功
...
// 处理下载文件的请求
static int do_download_photofile(Request_Handle *req)
{
	int errcode, ret;
	char msg[512] = "";
	...
	SetRequestType(req->xhandle_, 950990);
...
static int do_send_request(Request_Handle *req)
{
	...
	char msg[512] = "";
	int funcno,branchno;
	if(req->private_func_ > 0)
		funcno = req->private_func_;
	else
		funcno = req->mainfunc_;
	if(req->next_branch_no_>0)
		branchno = req->next_branch_no_;
	else
		branchno = 0;
 	if (!CallRequest(req->xhandle_, g_drtp_no, branchno,
	                 funcno, req->timeout_, &errcode, msg))
	{
		sprintf(g_errormsg, "%d:%s", errcode, msg);
		return KSG_ERR_REQUEST;
	}
...
static int do_send_finish_pack(Request_Handle *req)
{
	...
	SetRequestType(req->xhandle_, req->upload_funcno_);
	/*
...
static int do_read_file(Request_Handle *req)
{
	...
	while(!feof(req->out_filep_))
	{
		...
		if(len > 0)
		{
			...
			//ResetPackHandle(req->xhandle_);
			//SetRequestType(req->xhandle_, req->upload_funcno_);
			req->private_func_ = GetPrivateFunctionForNext(req->xhandle_);
			req->next_branch_no_ = GetBranchNoForNext(req->xhandle_);
 ...
 */
int APIDEF KSG_send_request(KSG_File_Handle handle)
{
	DO_TEST_DRTP;
...
int APIDEF KSG_DownLoadPhotoFile(KSG_File_Handle handle)
{
	DO_TEST_DRTP;
	...
	if (req->xhandle_ == NULL)
	 ...
	 /* 下载文件 */
	return do_download_photofile(req);
}
 ...
 */
int APIDEF KSG_set_param(KSG_File_Handle handle, const char *param_name,
                            const void *value, size_t len)
{
	...
	else if(strcmp(param_name,"del_svrfile") == 0)
	{
		req->del_svrfile_ = atoi((const char*)value);
	}
	else if(strcmp(param_name,"download_funcno") == 0)
	{
		req->download_funcno_ = atoi((const char*)value);
	}
	else if(strcmp(param_name,"upload_funcno") == 0)
	{
		req->upload_funcno_ = atoi((const char *)value);
	}
	else if(strcmp(param_name,"xpack_path") == 0)
	{
		strcpy(g_xpack_path,(char*)value);
	}
	else
		return -1;
	return 0;
 ...
 */
int APIDEF KSG_set_request_field(KSG_File_Handle handle, const char *field, void *value, size_t len)
{
	return -1;
}
...
KSG_API int APIDEF KSG_Get_NextPrivFunc(KSG_File_Handle handle)
{
	Request_Handle *h = (Request_Handle*)handle;
	return h->private_func_;
}