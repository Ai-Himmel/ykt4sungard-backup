...
class MainFrame(wx.Frame):
    ...
    def setupAllPanel(self,parent):
        ...
        btn = wx.Button(panel,-1,"初始化黑名单")
        sizer.Add(btn,(3,6),(1,1))
        self.Bind(wx.EVT_BUTTON,self.OnInitBlk,btn)

        edt = wx.TextCtrl(panel,3001,"")
        sizer.Add(edt,(5,1),(1,5),flag=wx.EXPAND)
        btn = wx.Button(panel,-1,"设置欢迎信息")
        sizer.Add(btn,(5,6),(1,1))
        self.Bind(wx.EVT_BUTTON,self.OnSetWelcome,btn)
        
        btn = wx.Button(panel,-1,"设置卡类分组")
        sizer.Add(btn,(6,1),(1,1))
        self.Bind(wx.EVT_BUTTON,self.OnSetCardGroup,btn)
        ...
        lbl = wx.StaticText(panel,-1,"IP: ",style=wx.ALIGN_RIGHT)
        sizer.Add(lbl,(7,1),(1,1))
        edt = wx.TextCtrl(panel,3003,"10.49.188.13")
        sizer.Add(edt,(7,2),(1,1))
        lbl = wx.StaticText(panel,-1,"PORT: ",style=wx.ALIGN_RIGHT)
        sizer.Add(lbl,(7,3),(1,1))
        edt = wx.TextCtrl(panel,3004,"6002")
        sizer.Add(edt,(7,4),(1,1))
        btn = wx.Button(panel,-1,"设置服务器")
        sizer.Add(btn,(7,5),(1,1))
        self.Bind(wx.EVT_BUTTON,self.OnSetSvrIp,btn)
        
        # 黑名单
        lbl = wx.StaticText(panel,-1,"黑名单版本号: ",style=wx.ALIGN_RIGHT)
        sizer.Add(lbl,(8,1),(1,1))
        edt = wx.TextCtrl(panel,3005,"000000000000")
        sizer.Add(edt,(8,2),(1,1))
        btn = wx.Button(panel,-1,"设置版本号")
        sizer.Add(btn,(8,3),(1,1))
        self.Bind(wx.EVT_BUTTON,self.OnSetBlkVer,btn)
    ...
    def OnSetSvrIp(self,evt):
        ip = self.FindWindowById(3003)
        port = self.FindWindowById(3004)
        self.postest.testSetSvrIP(ip.GetValue(),int(port.GetValue()))
        self.log.log('设置服务器[%s][%s]' % (ip.GetValue(),port.GetValue()))
        
    def OnInitBlk(self,evt):
        self.log.log('初始化黑名单需要10几分钟，请稍后')
        self.postest.testClearBlklist()
        
    def OnSetCardGroup(self,evt):
        self.log.log('设置卡类分组')
        self.postest.testSetCardGroup()
        
    def OnSetBlkVer(self,evt):
        blkver= self.FindWindowById(3005)
        self.log.log('设置黑名单版本号[%s]' % blkver.GetValue())
        self.postest.testSetBlackVersion(blkver.GetValue())
...
class MyApp(wx.App):
    def OnInit(self):
        ...
        dlg = MainFrame(None,"z18车载机工具 v1.3")
        dlg.Show()
