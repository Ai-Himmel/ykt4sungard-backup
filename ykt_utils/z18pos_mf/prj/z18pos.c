...
int kng_check_pos_param()
{
	...
	ret = EeSysRead(POS_PHYID_ADDR,42,buffer);
	if(Z18_OK != ret)
		return ret;
...
#if USE_GPRS
    if(EeSysRead(POS_GPRS_PARA_ADDR,12,buffer)!=Z18_OK)
        return KNG_POS_HARDWARE_ERROR;
    calc_not(buffer,6,buffer+12);
    if(memcmp(buffer+6,buffer+12,6)==0)
    {
        memcpy(g_z18pos.svr_ip,buffer,4);
        g_z18pos.svr_port = get_2byte_int(buffer+4);
    }
#endif
...
static uint8 load_z18pos_param()
{
	...
	{
		print_error(KNG_POS_HARDWARE_ERROR,"reader");	
		return KNG_POS_HARDWARE_ERROR;
	}
	InitRtcTime();
	ReadRtcTime(g_z18pos.curr_time);

	if(EeSysRead(POS_WORKKEY_ADDR,24,g_rec_data) != Z18_OK)
	{
		debug_string(("read 0x40 addr\n"));
		print_error(KNG_POS_HARDWARE_ERROR,"EP 1");	
		return KNG_POS_HARDWARE_ERROR;
	}
	if( g_rec_data[22] != 0x55 || g_rec_data[23] != 0xAA)
	{
		print_error(KNG_POS_PARAM_ERROR,"EPD 1");
		return KNG_POS_PARAM_ERROR;
	}
	if(memcmp(g_rec_data+6,g_rec_data+14,6) != 0)
	{
		print_error(KNG_POS_PARAM_ERROR,"key");	
		return KNG_POS_PARAM_ERROR;
	}
	...
	// 消费参数表
	if(EeSysRead(POS_CONSUME_PARA_ADDR,32,g_rec_data) != Z18_OK)
	{
		print_error(KNG_POS_HARDWARE_ERROR,"EP 2");	
		return KNG_POS_HARDWARE_ERROR;
	}
	...
	{
		print_error(KNG_POS_PARAM_ERROR,"EPD 2");
		return KNG_POS_PARAM_ERROR;
	}
	...
	// TODO : 检查采集流水日期
	if(EeSysRead(POS_COL_DATE_ADDR,6,g_rec_data)!=Z18_OK)
	{
		print_error(KNG_POS_HARDWARE_ERROR,"EP 2");	
		return KNG_POS_HARDWARE_ERROR;
	}
	...
	if(memcmp(g_rec_data+3,g_rec_data+6,3) !=0)
	{
		...
		if(EeSysWrite(POS_COL_DATE_ADDR,6,g_rec_data) != Z18_OK)
		{
			print_error(KNG_POS_HARDWARE_ERROR,"EP 2");	
			return KNG_POS_HARDWARE_ERROR;
		}
	...
	for(crc_code = 0;crc_code < 298;crc_code+=64)
	{
		...
		{
			print_error(KNG_POS_HARDWARE_ERROR,"EP 3");	
			return KNG_POS_HARDWARE_ERROR;
		}
	}
	...
	{
		print_error(KNG_POS_PARAM_ERROR,"EPD 3");
		return KNG_POS_PARAM_ERROR;
	}
	...
	// 欢迎信息
	if(EeSysRead(POS_WELCOMEMSG_ADDR,32,g_rec_data) != Z18_OK)
	{
		print_error(KNG_POS_HARDWARE_ERROR,"EP 4");	
		return KNG_POS_HARDWARE_ERROR;
	}
	...
	{
		if(EeSysRead(POS_CARDGROUP_ADDR+crc_code,64,g_global_buf+crc_code) != Z18_OK)
		{
			print_error(KNG_POS_HARDWARE_ERROR,"EP 5");	
			return KNG_POS_HARDWARE_ERROR;
		}
	}
    for(crc_code = 0;crc_code < 256;crc_code++)
    {
        if(g_global_buf[crc_code] >= MAX_CARD_GROUP)
            g_z18pos.card_group[crc_code] = 0;
        else
            g_z18pos.card_group[crc_code] = g_global_buf[crc_code];
    }
    // 加载卡分组
#if ENABLE_BLKLST
	...
	for(;;)
	{
		...
		if(redraw)
		{
			...
			case 5:
			{
                ...
                #if ENABLE_BLKLST
                strcpy(param[2],"黑名单版本号");
                bcd2str(g_z18pos.blk_ver,6,param[3]);
                #endif
			}	
            break;
			case 6:
            ...
            case 7:
            {
                strcpy(param[0],"服务器IP");
                sprintf(param[1],"%d.%d.%d.%d",(uint16)g_z18pos.svr_ip[0],(uint16)g_z18pos.svr_ip[1],
                    (uint16)g_z18pos.svr_ip[2],(uint16)g_z18pos.svr_ip[3]);
                strcpy(param[2],"服务器端口");
                sprintf(param[3],"%d",g_z18pos.svr_port);
...
static uint8 load_psam_card()
{
	...
	memcpy(send.Command,"\x00\xA4\x04\x00",4);
	memcpy(send.DataIn,"\xD1\x56\x00\x01\x01\x80\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00",16);
	send.Lc=0x10;
	send.Le=0;
#else
...
#if ENABLE_MIFARE_CARD == 0 // CPU 卡
	print_line(1,10,"loading psam ....");
    ret = load_psam_card();
    if(ret)
    {
        print_error(ret,"PSAM");
        return;
    }
	print_line(1,10,"loading psam OK");
#endif 
	// 进入主循环
	InitRtcTime();
	ReadRtcTime(g_z18pos.curr_time);
...
uint8 update_pre_total(uint8* total)
{
	...
	if(EeSysWrite(POS_CONSUME_HIS_ADDR,8,buffer) == Z18_FAILED)
		return KNG_POS_HARDWARE_ERROR;
	return 0;
}
