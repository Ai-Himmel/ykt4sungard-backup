...
extern "C" {
	...
	/*
	   ...
	   修改原因：规范编码
	 */
	BOOL __stdcall DRTPUninitialize();
	/*
	   功能：释放资源
	   参数：无。
	   ...
	   修改原因：规范编码
	 */
	int __stdcall DRTPGetHost (char*  strIP,WORD* pwPort,
							   WORD wGatewayNo/*=0*/,
							   WORD wBroadcastPort/*=1976*/,
	...
	/*
	   ...
	   修改原因：规范编码
	 */
	HANDLE __stdcall DRTPConnect (LPCTSTR strIP,WORD wPort,WORD Encode);
	/*
	   功能：建立与路由通讯服务器的连接
	   参数：
	   ...
	   修改原因：规范编码
	 */
	BOOL __stdcall DRTPClose (HANDLE hConnection);
	/*
	   功能：关闭与路由通讯服务器的连接
	   参数：
	   ...
	   修改原因：规范编码
	 */
	BOOL __stdcall DRTPSend (HANDLE hConnection,WORD wDestNo,
							 WORD wFunction, char * pSendData,
							 int iLen,BYTE byFunctionPriority,
							 bool Designed/*=false*/);
	/*
	   功能：发送请求\应答数据
	   参数：
	...
	修改原因：规范编码
	 */
	int __stdcall DRTPReceive(HANDLE hConnection,char * recvBuffer,
							  int iBufferSize,
							  DWORD dwTimeout/*= 0*/);
	/*
	   功能：接收数据。
	   ...
	   修改原因：规范编码
	 */
	BOOL __stdcall DRTPCheckNetState (HANDLE hConnection,
	...
	/*
	   ...
	   修改原因：规范编码
	 */
	BOOL __stdcall DRTPCheckDataArrived (HANDLE hConnection,
										 DWORD dwTimeout/*=0*/);
	/*
	   功能：检查特定连接上是否有数据尚未接收。
	   参数：
	   ...
	   修改原因：规范编码
	 */
	int __stdcall DRTPGetLastError(HANDLE hConnection,
								   char* ErrorMsgBuffer,
								   int iMsgBufferSize);
	/*
	   功能：返回特定连接上的错误信息。
	...
	int __stdcall DRTPCloseSocket(HANDLE hConnection);
	/*
	   功能：关闭SOCKET连接
	   参数：
	   hConnection[in]：连接句柄。调用DRTPConnect时返回。
