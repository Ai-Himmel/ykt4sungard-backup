...
* 日期				2005-06-07
* 备注				基于公司 DRTP 功能
* $Id: drtpcall.c 53 2007-06-25 08:15:41Z xiang.li $
*/
...
int kcc_drtp_send(int handle,char * buf,int len,int mainfunc)
{
	...
	{
		kcc_write_log(LOG_ERR,"attach drtp server error!");
		return -1;
	}
	retries = 3;
	while(retries-- > 0)
	{
		...
		{
			// 重试的时候要重连
			kcc_write_log(LOG_DEBUG,"retry to connect drtp");
...
int kcc_des_drtp_connect()
{
	// FIXME : we can just return handle
	alarm(g_drtp_timeout);
	int handle = DrtpConnect((char*)g_des_drtp_svr_ip,g_des_drtp_svr_port,
							 &kcc_drtp_err_code,kcc_drtp_err_msg,-1);
	alarm(0);
	if( handle > 0 )
	{
	...
	kcc_write_log(LOG_ERR,"connect to drtp error [%s:%d]",
				  g_des_drtp_svr_ip,g_des_drtp_svr_port);
	return -1;
}

int kcc_des_drtp_send(int handle,char * buf,int len,int mainfunc)
{
    TAG_DRTPHANDLE tag_handle;
	int ret;
	int retries;
	if( mainfunc == 0 )
		mainfunc = g_des_drtp_func_no;

	if(!DrtpAttachServer(g_des_drtp_svr_id,mainfunc,handle,&tag_handle,
						&kcc_drtp_err_code,kcc_drtp_err_msg))
	{
		kcc_write_log(LOG_ERR,"attach drtp server error!");
		return -1;
