 ...
 * 日期					2005-07-13
 * 备注
 * $Id: kcc_util.c 48 2005-11-29 06:53:38Z cheng.tang $
 */
...
void kcc_die(int exit_code,const char * fmt,...)
{
	va_list args;
	va_start(args,fmt);
	kcc_write_log(LOG_ERR,fmt,args);
	va_end(args);
...
char * kcc_read_file_str(FILE * fp)
{
	...
	if(NULL == p)
	{
		kcc_die(1,"allocate memory error!");
	}
...
unsigned long kcc_get_tick_count()
{
	struct timeval tv;
	gettimeofday(&tv,NULL);
	return (long)(tv.tv_sec % 10000000 * 1000) + (tv.tv_usec / 1000);
}
...
int kcc_lock_file(const char * filename,int function)
{
	...
	{
		kcc_write_log(LOG_INFO,"file has been locked");
		goto L_END;
	}
	if(lockf(fileno(fp),function,0L))
	{
		kcc_write_log(LOG_ERR,"lock file error!");
		goto L_END;
	}
	return 0;
L_END:
