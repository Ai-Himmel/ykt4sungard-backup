...
{
   int ret;
   CReadXml ReadCfgXml(xmlcfgfile);
   ret=ReadCfgXml.initial();
   if(ret)
   {
        cout<<"Error: unable to parse file: "<<ReadCfgXml.getXmlCofigFileName()<<endl;
        return ret;
   }
   //加载银行配置
   xmlNodePtr curNode=NULL,curSubNode=NULL;   
   CFG_BANK cfgBank;
   ReadCfgXml.getFistNodeByNodePath("/config/bank",curNode);

   while(curNode!=NULL&&(curNode->type==XML_TEXT_NODE))curNode=curNode->next;
   if(curNode==NULL)
   {
       return -1;
   }
   while(1)
   {
        char*  val=NULL;
        string nodeval;
        curSubNode = curNode->xmlChildrenNode;
        while(curSubNode!=NULL) {
                if(curSubNode->type==XML_ELEMENT_NODE)
                {
	                val = (char*)xmlNodeListGetString(ReadCfgXml.getDoc(), curSubNode->xmlChildrenNode, 1); 
	                nodeval=u2g(val);
	                if(xmlStrcmp(curSubNode->name, (const xmlChar *) "bankCode")==0)
	                    cfgBank.bankCode=nodeval;
	                else if(xmlStrcmp(curSubNode->name, (const xmlChar *) "bankName")==0)
	                    cfgBank.bankName=nodeval;
	                else if(xmlStrcmp(curSubNode->name, (const xmlChar *) "corpID")==0)
	                    cfgBank.corpID=nodeval;
	                else if(xmlStrcmp(curSubNode->name, (const xmlChar *) "corpPwd")==0)
	                    cfgBank.corpPwd=nodeval;
	                else if(xmlStrcmp(curSubNode->name, (const xmlChar *) "signUrl")==0)
	                    cfgBank.signUrl=nodeval;
	                else if(xmlStrcmp(curSubNode->name, (const xmlChar *) "bankUrl")==0)
	                    cfgBank.bankUrl=nodeval;
	                xmlFree(val); 
                }
                curSubNode=curSubNode->next;
        }
        XmlFileCfg.cfgBankMap[cfgBank.bankCode]=cfgBank;
...
int main(int argc,char *argv[])
{
	int rtn;
	int ret=0;
	openlog("bankfront",LOG_PID|LOG_CONS|LOG_NDELAY,LOG_LOCAL3);
...
*/
//加载配置
	MyParameters *pMyParameters = MyParameters::getInstance();
	if(!pMyParameters->load())
	{
		DEBUG_RUNTIME_MSGOUT("加载配置文件失败\n");
		return -1;			 
	}
	ResetBPFunctions();
	if (argc>2)
...
//	DEBUG_RUNTIME_MSGOUT("开始处理业务处理单元\n");
 	g_pSvrLink->Processing(&g_BUnit);
//	DEBUG_RUNTIME_MSGOUT("结束处理业务处理单元\n");
      //if (g_pSvrLink->bExit) break;
   }
//DEBUG_RUNTIME_MSGOUT("退出处理业务处理单元\n");
...
*/
//	(MyParameters::getInstance())->destroyInstance();
//		shmdt(shm);
