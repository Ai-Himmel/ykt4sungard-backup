...
public class CheckUserAction extends Action {
	...
	public ActionForward checkuser(ActionMapping mapping, ActionForm form,
		...
		try {
			card = carddao.getCardState(hexcardno);
			String stateId = card.getStateId();
			if("2000".equals(stateId)){
				request.setAttribute("errmsg","卡状态注销不能更新，请到一卡通管理中心处理");
				return mapping.findForward("failure");
			}else if("1100".equals(stateId)){
				request.setAttribute("errmsg","卡已挂失不能更新，请到一卡通管理中心处理");
				return mapping.findForward("failure");
			}else if("1010".equals(stateId)) {
				request.setAttribute("errmsg","卡已冻结不能更新，请到一卡通管理中心处理");
				return mapping.findForward("failure");
			}
				...
				//	logger.info("==========bccret==is true=========");
					...
					if (0 == RetCode[0]) {
						...
						/*	bcc.GetStringFieldByName(xpackhandle, 0, "vsmess"
							...
							request.setAttribute("deptno",new String(deptno,0,i));
						...
						if (0 < RecordCount[0]) {
							...
							String vs_mess = new String(vsmess, 0, i);*/
							request.setAttribute("errmsg", "更新卡信息处理失败"+RetCode[0]);
							return mapping.findForward("failure");
						} else {
							request.setAttribute("errmsg", "更新卡信息处理失败"+RetCode[0]);
							return mapping.findForward("failure");
