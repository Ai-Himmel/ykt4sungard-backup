...
public class DB2CardDAO extends DB2ActionDAO {
		...
		try {
			...
			try {
				sql = "select cardno from ykt_cur.t_card where custid="
						...
						+ " and cardno<>" + cardno+" and cardphytype=(select cardphytype from ykt_cur.t_card a where a.cardno="+cardno+")";
				stm = conn.prepareStatement(sql);
				result = stm.executeQuery();
				if (result.next()) {
	...
	//´óÁ¬º£ÊÂÁÜÔ¡Ë®¿Ø·ÑÂÊ
	public WaterRateDTO getCardWaterRateById(String cardphyid)
	throws ClassNotFoundException, SQLException, Exception {
	String sql = "";
	Connection conn = null;
	PreparedStatement stm = null;
	...
	try {
		conn = getConnection();
		sql = "select rate.* from ykt_cur.t_card card,ykt_cur.t_waterrate rate where card.feetype=rate.feetype and card.cardphyid='"
				+ cardphyid
				+ "'"
				+ " and ('0'||lossflag||frozeflag||'0'||badflag||'00000') ='0000000000' and status='1'";
		stm = conn.prepareStatement(sql);
		...
		if (result.next()) {
			WaterRateDTO temp = new WaterRateDTO();
			temp.setFeetype(result.getLong("feetype"));
			temp.setRate1(result.getLong("rate1"));
			temp.setRate2(result.getLong("rate2"));
			temp.setRate3(result.getLong("rate3"));			
			return temp;
		}
		return null;
	} catch (SQLException e1) {
		logger.error("¶ÁÈ¡·ÑÂÊÐÅÏ¢Ê§°Ü[" + e1.getMessage() + "]¿¨ºÅ:" + cardphyid);
		throw (e1);
	} catch (Exception e) {
		e.printStackTrace();
		throw (e);
