...
//		reqip = "10.49.188.50";
		...
		int func = KSConfiguration.getInstance().getPropertyAsInt(
				"loginfunc_waterctl_trans_yjz", 850010);// 后台业务功能号
		...
		try {
			...
			if (0 != xpackhandle) {
				...
				bcc.SetIntFieldByName(xpackhandle, 0, "lcert_code".getBytes(),
						Integer.parseInt(tradecode));// 交易代码
				String watertype="2";
                if("1191".equals(tradecode)){
                      watertype="2";
                }else if("1192".equals(tradecode)){
                       watertype="3";
                }
				bcc.SetIntFieldByName(xpackhandle, 0, "lbank_acc_type".getBytes(),
						Integer.parseInt(watertype));// 钱包号
