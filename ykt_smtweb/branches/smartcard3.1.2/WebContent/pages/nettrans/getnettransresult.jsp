	...
	<head>
		...
		<%
			...
			String sname2 = request.getAttribute("sname2") == null ? ""
             ...
             String thirdpartname = request.getAttribute("thirdpartname") == null ? ""
                    : request.getAttribute("thirdpartname").toString();
		...
		function writeCard(){
            ...
            var stuemp_no = '<%=stuempno%>';
            var out_card_amount = '<%=outCardAmount%>';
            var sdate = '<%=sdate3%>';
            var sname = '<%=sname2%>';
            var third_part_name = '<%=thirdpartname%>';
			var url = serverbase+"getNettrans.do";
			var parms = "cardId="+card_id+"&money="+t_money+"&stuempno="+stuemp_no+"&serialNo="+serial_no+"&sdate3="+sdate+"&sname2="+sname+"&thirdpartname="+third_part_name;
			writeCardForm.all.RFScanner.PortNo=device_port;
			var ret = writeCardForm.all.RFScanner.NetTransWriteCard(url,card_id,out_card_amount,tx_cnt,parms);
			if(ret==0){
				msg.innerHTML="网费充值成功，<br><br>转账金额 "+t_money+" 元，<br><br>一卡通卡余额 "+parseFloat(out_card_amount)/100+" 元";
			}else if(ret==2){
				msg.innerHTML="网费充值失败，失败原因：第三方请求网络超时！请等待下一个交易日系统自动对账为您充值,请不要重复充值。";
			}else if(ret==-3){
				msg.innerHTML="网费充值失败，失败原因：第三方请求无应答！请等待下一个交易日系统自动对账为您充值,请不要重复充值。";
			}else if(ret==-10){//中途拔卡
				 window.location='/smartcardweb/pages/nettrans/recardwrite1.jsp';
			}else if(ret==-100){//写卡失败
                 window.location='/smartcardweb/pages/nettrans/recardwrite.jsp';
            }else{
                msg.innerHTML="网费充值失败，失败原因：第三方请求发送失败！请等待下一个交易日系统自动对账为您充值,请不要重复充值。";
			}
		}
