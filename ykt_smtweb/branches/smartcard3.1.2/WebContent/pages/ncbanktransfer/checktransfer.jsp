...
<html>
	<head>
		...
		String showbankcardno =request.getAttribute("showbankcardno") == null ? "" : request.getAttribute("showbankcardno").toString();
		String sbank = request.getAttribute("sbank")== null ? "" : request.getAttribute("sbank").toString();
		...
		function dobanktrans(){		    
			...
			if(ret == 0){
				var doworkret = document.all.RFScanner.BankTransWriteCard(serverbase+"ncbanktransfer.do",cardphyid,outCardAmount,dptCardCnt,"money="+money+"&cardphyid="+cardphyid+"&payCardCnt="+payCardCnt+"&refno="+refno+"&bankcardno="+bankcardno+"&bankcardinfo="+bankcardinfo+"&sbank="+sbank+"&radomstr="+radomstr);
				if(doworkret == 0){
				//转账成功		
				    window.location='/smartcardweb/pages/ncbanktransfer/transfersuccess.jsp';		
				}else if(doworkret == -1){
					//写卡失败
					window.location='/smartcardweb/pages/ncbanktransfer/recardwrite.jsp';//如果重写还不成功，就要走冲正流程					
				}else if(doworkret == 1){
					//转账失败
					window.location='/smartcardweb/pages/ncbanktransfer/transferfailue.jsp';					
				}else{
				    //转账失败
					window.location='/smartcardweb/pages/ncbanktransfer/transferfailue.jsp';	
				}
				...
				alert("请放置卡片...errcode="+ret);
				document.all.errmsg.innerHTML="请确认放好卡!转账过程可能最多需要等待30秒，请不要轻易移走卡片！";
				document.getElementById("send").style.display='';
				document.getElementById("cancel").style.display='';
				
			}else{
				alert("请正确放置卡片...errcode="+ret);
				document.all.errmsg.innerHTML="请确认放好卡!转账过程可能最多需要等待30秒，请不要轻易移走卡片！";
				document.getElementById("send").style.display='';
				document.getElementById("cancel").style.display='';
			}
	...
	%>
	<body scroll="no" onload="javascript:dobanktrans();">
								...
								<table width="80%" border="0" align="center" cellpadding="0"
									...
									<tr>
										<td height="80" align="center" class="show" id="errmsg"
											colspan="2">
											<html:errors /><b>转账请求发送中，请稍后...最多需要等待30秒，请不要轻易移走卡片</b>
											&nbsp;
										</td>
									</tr>
								</table>
										...
										<td align="center">
											<a onclick="javascript:dobanktrans();"
												...
												onmousedown="MM_swapImage('send','','/smartcardweb/pages/images/senda.gif',1)"><img
													...
													width="109" height="48" border="0" id="send" style="display: none;" /> </a>&nbsp;
										</td>
										<td>
											<a
												...
												onmousedown="MM_swapImage('cancel','','/smartcardweb/pages/images/cancela.gif',1)"><img
													...
													width="109" height="48" border="0" id="cancel" style="display: none;" /> </a>
										</td>
									</tr>
								</table>
							</div>
