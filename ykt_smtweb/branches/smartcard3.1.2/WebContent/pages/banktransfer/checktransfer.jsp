	...
	<head>
		...
		function dobanktrans(){		    
			...
			if(ret == 0){
				...
				var doworkret = document.all.RFScanner.BankTransWriteCard(serverbase+"banktransfer.do",cardphyid,outCardAmount,dptCardCnt,"money="+money+"&cardphyid="+cardphyid+"&payCardCnt="+payCardCnt+"&refno="+refno+"&bankcardno="+bankcardno+"&radomstr="+radomstr);
				if(doworkret == 0){
				//转账成功		
				    window.location='/smartcardweb/pages/banktransfer/transfersuccess.jsp';		
				}else if(doworkret == -1){
					//写卡失败
					window.location='/smartcardweb/pages/banktransfer/recardwrite.jsp';//如果重写还不成功，就要走冲正流程					
				}else if(doworkret == 1){
					//转账失败
