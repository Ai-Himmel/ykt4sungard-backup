...
<html>
       ...
       String cardphyno = request.getAttribute("cardphyno") == null ? "" : request
        ...
        <script language="javascript">          
              ...
              if(!document.all.send.disabled){
                ...
                if(rrt == 0){
                    ...
                    if(ret == 0){
                        ...
                        var tx_cnt =  putinForm.all.RFScanner.GetPacketInfo("pay_card_cnt");
                        if(parseFloat(putinForm.all.money.value)*100<=0){
                            alert("转账金额需大于0");
                            return false;
                        }
                        if(!fucCheckNUM(putinForm.all.money.value)){
                            alert("转账金额格式不正确，最多两位小数");
                            return false;
                        }
                        if(parseFloat(putinForm.all.money.value)*100<=parseFloat(bala)){
                            putinForm.all.cardphyno.value=cardphyid;
                            putinForm.all.cardbalance.value = bala;
                            putinForm.all.cardcnt.value = tx_cnt;
             ...
             function fucCheckNUM(NUM)
            {
             ...
             //说明是数字
             var indd = NUM.indexOf('.');
             if(indd>=0){
                if(NUM.length-indd>2||NUM.length-indd<1){
                    return false;
                }
                while(indd++<NUM.length){
                    if (fucPureCheckNUM(NUM.charAt(indd))==false)
                    {
                    //说明有字符不是数字
                     return false;
                    }
    ...
    <body scroll="no">
        <form action="/smartcardweb/cardcardputin.do" method="post"
            name="putinForm">
            <div id="mid_top"></div>
            <div id="middle">
                <div id="Layer1">
                        ...
                        <div id="login_middle">
                            ...
                            <div id="login_input">
                                <table width="560" border="0" cellpadding="0" cellspacing="0">                   
                                    ...
                                    <tr>
                                        <td align="right">
                                            <img src="/smartcardweb/pages/images/zzje.gif"
