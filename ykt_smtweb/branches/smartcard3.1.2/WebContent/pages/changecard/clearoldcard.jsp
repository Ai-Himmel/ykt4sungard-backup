...
<html>
	<head>
		...
		<%
			...
			String cardsubsidyno = request.getAttribute("cardsubsidyno") == null ? ""
					: request.getAttribute("cardsubsidyno").toString();	
		...
		function writeCard(){
            ...
            var cardsubsidyno = '<%=cardsubsidyno%>';
            var txCnt = '<%=txCnt%>';
            var cardBala = '<%=cardBala%>';
			writeCardForm.all.RFScanner.PortNo=device_port;
			var ret = writeCardForm.all.RFScanner.CardChangeClearOldCard(cardphyid,newcardphyid,showcardno,txCnt,cardBala,cardsubsidyno);
			if(ret==0){
