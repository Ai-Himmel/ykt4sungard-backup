		...
		try {
			...
			if (0 != xpackhandle) {
				...
				bcc.SetStringFieldByName(xpackhandle, 0, "scust_no".getBytes(),
						"web".getBytes());// 操作员
				String info = "充值房间号：["+room+"]";
				bcc.SetStringFieldByName(xpackhandle, 0, "vsmess".getBytes(),
						info.getBytes());
				...
				if (bccret) {
					...
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
							...
							String saddr_s = new String(saddr, 0, i);
							...
							if (i > 0) {
								request.setAttribute("saddr", saddr_s);
