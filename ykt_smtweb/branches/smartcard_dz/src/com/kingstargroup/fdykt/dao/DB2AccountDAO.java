...
return null;
	 ...
	 * @throws Exception
	 */
	public int getdevidFromip(String ip) throws ClassNotFoundException,SQLException, Exception {
		String sql = "";
		Connection conn = null;
		PreparedStatement stm = null;
		ResultSet result = null;
		...
		try {
			conn = getConnection();
			sql = "select d.device_id as deviceid from ykt_cur.t_pif_device d where d.state_id=1 and d.comadd='" + ip +"'";
			stm = conn.prepareStatement(sql);
			result = stm.executeQuery();
			if (result.next()) {
