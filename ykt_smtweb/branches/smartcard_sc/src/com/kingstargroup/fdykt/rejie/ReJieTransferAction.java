...
public class ReJieTransferAction extends Action {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(ReJieTransferAction.class);
	...
	private ActionForward dotrans(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		if (null == cardId || null == money || null == serialNo || null == chargeID || "".equals(cardId) || "".equals(money) || "".equals(serialNo) || "".equals(chargeID)) {
			String xmlout = XMLGenerateUtil.ElectXMLGenerate("1000", "请求数据错误!", "", "", "", "", null);
		...
		int func = KSConfiguration.getInstance().getPropertyAsInt("trans", 847317);// 后台业务功能号
		...
		try {
			...
			if (0 != xpackhandle) {
				...
				bcc.SetIntFieldByName(xpackhandle, 0, "lvol6".getBytes(), Integer.parseInt(serialNo));// 流水号
				...
				bcc.SetIntFieldByName(xpackhandle, 0, "lvol7".getBytes(), Integer.parseInt("1"));
				...
				if (bccret) {
					...
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
							...
							String xmlout = XMLGenerateUtil.ElectXMLGenerate("0000", "转账成功", String.valueOf(TransAmount[0]), out_card_amount, serial_no, cardId, null);
							...
							String xmlout = XMLGenerateUtil.ElectXMLGenerate("0001", "无数据返回!" + RetCode[0], "", "", "", "", null);
							XMLGenerateUtil.XMLSendOut(request, response, xmlout);
							return null;
						}
						...
						if (0 < RecordCount[0]) {
							...
							String xmlout = XMLGenerateUtil.ElectXMLGenerate("0002", vs_mess + RetCode[0], "", "", "", "", null);
							XMLGenerateUtil.XMLSendOut(request, response, xmlout);
							return null;
						} else {
							logger.info("---------0001=-------------" + RetCode[0]);
					...
					String xmlout = XMLGenerateUtil.ElectXMLGenerate("1001", "后台通讯暂时不可用，请稍后重试！ " + RetCode[0], "", "", "", "", null);
					XMLGenerateUtil.XMLSendOut(request, response, xmlout);
					return null;
				}
			}
			...
			String xmlout = XMLGenerateUtil.ElectXMLGenerate("1002", "系统出现异常，请稍后重试！" + RetCode[0], "", "", "", "", null);
			XMLGenerateUtil.XMLSendOut(request, response, xmlout);
			return null;
			...
			String xmlout = XMLGenerateUtil.ElectXMLGenerate("1003", "系统出现异常，请稍后重试！" + RetCode[0], "", "", "", "", null);
			XMLGenerateUtil.XMLSendOut(request, response, xmlout);
			return null;
			...
			System.out.println("---------1003=-------------" + RetCode[0]);
			String xmlout = XMLGenerateUtil.ElectXMLGenerate("1004", "系统初始化异常，请稍后重试！" + RetCode[0], "", "", "", "", null);
			XMLGenerateUtil.XMLSendOut(request, response, xmlout);
			return null;
		} finally {
			if (xpackhandle != 0) {
