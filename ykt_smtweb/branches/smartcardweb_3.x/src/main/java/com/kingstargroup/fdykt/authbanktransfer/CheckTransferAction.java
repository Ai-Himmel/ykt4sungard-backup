...
public class CheckTransferAction extends Action {
	...
	private static final Logger logger = Logger.getLogger(CheckTransferAction.class);
	...
	private ActionForward docheck(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		int func = KSConfiguration.getInstance().getPropertyAsInt("logicfunc_banktrans_check", 240020);// 后台业务功能号
		...
		try {
			...
			if (0 != xpackhandle) {
				...
				bcc.SetIntFieldByName(xpackhandle, 0, "lcert_code".getBytes(), deviceid);// 客户端id
				bcc.SetIntFieldByName(xpackhandle, 0, "lvol0".getBytes(), Integer.parseInt(String.valueOf(card.getCardno())));// 卡号
				bcc.SetIntFieldByName(xpackhandle, 0, "lvol5".getBytes(), Integer.parseInt(money) * 100);// 交易金额
				bcc.SetIntFieldByName(xpackhandle, 0, "lvol6".getBytes(), Integer.parseInt(card_bala));// 卡余额
				bcc.SetIntFieldByName(xpackhandle, 0, "lvol1".getBytes(), Integer.parseInt(dptCardCnt));// 充值次数
				...
				if (bccret) {
					...
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
							...
							bcc.GetStringFieldByName(xpackhandle, 0, "scust_auth".getBytes(), byteStr, 21);
							String bank_cardno = BccUtil.getStringFromByte(byteStr);// 银行卡号
							bcc.GetStringFieldByName(xpackhandle, 0, "stx_pwd".getBytes(), byteStr, 9);
							...
							bcc.GetStringFieldByName(xpackhandle, 0, "sname2".getBytes(), byteStr, 21);
