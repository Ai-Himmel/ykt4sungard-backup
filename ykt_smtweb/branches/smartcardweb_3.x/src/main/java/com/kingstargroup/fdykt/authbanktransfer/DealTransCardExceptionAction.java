...
public class DealTransCardExceptionAction extends Action {
	
	private static final Logger logger = Logger.getLogger(DealTransCardExceptionAction.class);

	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		KSDaoSessionFactory.createSession();
		ActionForward forward = dealWaterTransErr(mapping, form, request, response);
		try {
	...
	private ActionForward dealWaterTransErr(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		if (sphone3 == null || "".equals(sphone3) || saddr == null || "".equals(saddr)) {
			String xmlout = XMLGenerateUtil.XMLGenerate("2222", "请求信息格式不合法！", "", "", "", "", "", "", "", "", "", null);
		...
		try {
			...
			if (deviceid < 0) {
				String xmlout = XMLGenerateUtil.XMLGenerate("3333", "设备未注册，不能使用此功能！", "", "", "", "", "", "", "", "", "", null);
				XMLGenerateUtil.XMLSendOut(request, response, xmlout);
				return null;
			}
		} catch (Exception e1) {
		...
		int func = KSConfiguration.getInstance().getPropertyAsInt("logicfunc_reportcardexception", 850002);// 后台业务功能号
			...
			if (0 != xpackhandle) {
				...
				if (bccret) {
					...
					if (0 == RetCode[0]) {
						...
						String xmlout = XMLGenerateUtil.XMLGenerate("0000", "写卡异常处理成功", "", "", "", "", "", "", "", "", "", null);
						XMLGenerateUtil.XMLSendOut(request, response, xmlout);
						return null;

					} else {
						// 失败
						String xmlout = XMLGenerateUtil.XMLGenerate("0001", "写卡异常处理失败！请到卡务中心处理", "", "", "", "", "", "", "", "", "", null);
						XMLGenerateUtil.XMLSendOut(request, response, xmlout);
						return null;
					...
					String xmlout = XMLGenerateUtil.XMLGenerate("1001", "后台通讯暂时不可用，请稍后重试！ " + s, "", "", "", "", "", "", "", "", "", null);
					XMLGenerateUtil.XMLSendOut(request, response, xmlout);
					return null;
				}
			}
