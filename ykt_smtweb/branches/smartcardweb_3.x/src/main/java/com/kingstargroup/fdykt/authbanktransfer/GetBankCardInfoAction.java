...
public class GetBankCardInfoAction extends Action {
	...
	private static final Logger logger = Logger.getLogger(GetBankCardInfoAction.class);
	...
	private ActionForward docheck(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		int func = KSConfiguration.getInstance().getPropertyAsInt("logicfunc_bankcard_info", 240024);// 后台业务功能号
				...
				if (bccret) {
					...
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
							...
							bcc.GetDoubleFieldByName(xpackhandle, 0, "damt0".getBytes(), bankmoney);
