...
public class ElecCheckAction extends Action {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(ElecCheckAction.class);
	...
	private ActionForward docheck(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (0 != xpackhandle) {
				...
				bcc.SetIntFieldByName(xpackhandle, 0, "lbank_acc_type".getBytes(), 1);// 交易标志
				...
				bcc.SetIntFieldByName(xpackhandle, 0, "lvol7".getBytes(), Integer.parseInt(card_bala));// 卡交易前余额(分)
				...
				boolean bccret = bcc.CallRequest(xpackhandle, drtpno, branchno, mainfunction, 50000, errcode, errmsg);
				...
				if (bccret) {
					...
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
							...
							bcc.GetStringFieldByName(xpackhandle, 0, "sphone3".getBytes(), inStr, 300);
							String sphone3 = BccUtil.getStringFromByte(inStr);

							Arrays.fill(inStr, (byte) 0);
							bcc.GetStringFieldByName(xpackhandle, 0, "saddr".getBytes(), inStr, 300);
							String saddr = BccUtil.getStringFromByte(inStr);

							Arrays.fill(inStr, (byte) 0);
							bcc.GetStringFieldByName(xpackhandle, 0, "sdate3".getBytes(), inStr, 300);
							String sdate3 = BccUtil.getStringFromByte(inStr);

							Arrays.fill(inStr, (byte) 0);
							bcc.GetStringFieldByName(xpackhandle, 0, "stime3".getBytes(), inStr, 300);
							...
							Arrays.fill(inStr, (byte) 0);
							bcc.GetStringFieldByName(xpackhandle, 0, "sbank_acc".getBytes(), inStr, 300);
							String sbank_acc = BccUtil.getStringFromByte(inStr);
							...
							if (!"".equals(sbank_acc.trim()) && (!sbank_acc.trim().equals(room))) {
								request.setAttribute("sbank_acc", sbank_acc);
