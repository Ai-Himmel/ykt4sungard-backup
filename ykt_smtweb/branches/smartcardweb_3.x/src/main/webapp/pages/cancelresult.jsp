...
<html>
	<head>
		...
		<%
			...
			String blkVersion = request.getAttribute("blkversion")==null?"":request.getAttribute("blkversion").toString();
		%>
		<script language="JavaScript" type="text/JavaScript">
			function writeCard(){
				var blkVersion = '<%=blkVersion%>';
				var card_phyid='<%=cardphyid%>';
				var msg = document.getElementById("msg");
				if( '' == blkVersion){
					msg.innerHTML = "写卡失败！ 请返回重新解挂。";
				}else{
				    var ret=writeCardForm.RFScanner.UndoLoss(card_phyid,blkVersion);				    
				    if(ret==0)//解挂成功
				    {
