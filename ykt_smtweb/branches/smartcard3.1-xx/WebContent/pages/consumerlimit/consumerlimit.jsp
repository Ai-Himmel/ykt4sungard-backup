...
<html>
	<head>
        ...
        function addnum(num)
        {
			if(putinForm.all.nowinput.value=="1" && putinForm.all.maxpercount.value.length < 3 ){
				putinForm.all.maxpercount.value+=num;
			}else if(putinForm.all.nowinput.value=="2" && putinForm.all.maxperday.value.length < 6 ){
				putinForm.all.maxperday.value+=num;
			}
        }
        function cancelnum()
        {
		if(putinForm.all.nowinput.value=="1")
			...
			putinForm.all.maxperday.value=putinForm.all.maxperday.value.substring(0,putinForm.all.maxperday.value.length-1);		
        }
    	function readCard(){
    	 	...
    	 	if(putinForm.all.maxperday.value==''){
    	 		alert("请输入日累计消费限额！");
    	 		putinForm.all.maxperday.focus();
    	 		return false;
    	 	}
    	 	...
    	 	if(putinForm.all.maxpercount.value<0){
    	 	 ...
    	 	 if(!fucCheckNUM(putinForm.all.maxperday.value)){
                alert("日累计消费限额为整数数字！");
                putinForm.all.maxpercount.focus();
                return false;
            }
            
            if(putinForm.all.maxperday.value<0){
                alert("日累计消费限额应该不小于0！");
                putinForm.all.maxperday.focus();
                return false;
            }
        	...
        	var CardPhyId ='<%=CardPhyId%>';
				...
				if(ret == 0){
					...
					var payCardCnt = putinForm.all.RFScanner.GetPacketInfo("dpt_card_cnt");
							...
							<div id="login_input">
								<table width="800" border="0" cellpadding="0" cellspacing="0">
									...
									<tr>
										...
										<td>
											<input name="maxpercount" type="text" class="样式11"
												onfocus="javascript:putinForm.nowinput.value='1'" size="20"
												maxlength="7" />【您卡上的单次消费限额为<b><%=Integer.parseInt(maxpercount_o)/100%></b>元】
										</td>
									</tr>
									<tr>
										<td width="206" align="right">
											<img src="/smartcardweb/pages/images/rlj.gif"
												width="206" height="24" />
											
										</td>
										<td>
											<input name="maxperday" type="text" class="样式11"
												onfocus="javascript:putinForm.nowinput.value='2'" size="20"
												maxlength="9" />【您卡上的日累计消费限额为<b><%=Integer.parseInt(maxperday_o)/100%></b>元】
										</td>
									</tr>
									<tr>
