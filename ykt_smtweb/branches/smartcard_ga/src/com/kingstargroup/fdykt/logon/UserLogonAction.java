...
public class UserLogonAction
    ...
    try {
      if (manId.equals("") || password.equals("")) {
     ...
     String sql="select * from ykt_cur.t_cif_customer cus " +
                ...
                "where cus.man_id='" + manId +"' and card.password='" + ciphered +"' and card.state_id <> '2000'";
      ...
      //  String stuempno = ((CustomerDTO) customers.get(0)).getStuemp_no();
       ...
       // session.setAttribute("stuempno", stuempno);
		...
		if (accounts.size() == 0) {
			ArrayList lossaccounts = accountdao.getLossAccountInfo(String.valueOf(cutid));
