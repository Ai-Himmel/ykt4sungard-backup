...
public class WaterTransAction extends Action {
	...
	private ActionForward dotrans(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			logger.debug("cardId===="+cardId+", money===="+money+",===="+serialNo);
			if (0 != xpackhandle) {
				...
				if (bccret) {
					logger.debug("bccret=true");
					bcc.GetRetCode(xpackhandle, RetCode);
					bcc.GetRecordCount(xpackhandle, RecordCount);
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
							logger.debug("===1===RecordCount[0]==="+RecordCount[0]);
						...
						// 失败
						if (0 < RecordCount[0]) {
							logger.debug("===2===RecordCount[0]==="+RecordCount[0]);
							...
							logger.debug("===1===vs_mess==="+vs_mess+RetCode[0]);
							String xmlout = XMLGenerateUtil.MoblileXMLGenerate(
									"0002", vs_mess, "", "", "", "", "", null);
							XMLGenerateUtil.XMLSendOut(request, response,
									xmlout);
					...
					logger.debug("===2===vs_mess==="+errmsg+RetCode[0]);
					logger.error("connect:errcode=" + errcode[0] + "msg=" + s+RetCode[0]);
