	...
	public AccountDTO getAccountInfoByCardId(String cardId)
		...
		try {
			...
			if ("db2".equalsIgnoreCase(database)) {
				sql = "select a.*,c.begin_time,c.end_time, c.state_id,cus.classdept_no,ctf.Type_name ,cus.MED_FEETYPE "
				...
				sql = "select a.*,c.begin_time,c.end_time, c.state_id,cus.classdept_no,ctf.Type_name ,cus.MED_FEETYPE "
						...
						+ " where a.Card_id="
						+ cardId
						+ " and a.current_state=1 and a.ACT_TYPE=1"
						+ " and rownum=1";
			}
			try {
				...
				if (result.next()) {
					...
					temp_DTO.setMED_FEETYPE(result.getString("MED_FEETYPE"));
		...
		try {
			...
			if ("db2".equalsIgnoreCase(database)) {
				...
				sql = "select a.*,c.begin_time,c.end_time,c.state_id,cus.ClassDept_name,ctf.Type_name from ykt_cur.T_AIF_Account a "
						...
						+ "where a.Card_id>0 and cus.StuEmp_no='"
						+ stuempno
						+ "' and a.current_state=1 and rownum=1";
			}
