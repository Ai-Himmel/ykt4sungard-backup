...
public class DB2TradeSerialDAO extends DB2ActionDAO {
	...
	public ArrayList getTSByCardId(String cardId, int tradecode,
			String begindate, String enddate, int page)
		...
		try {
			...
			if ("db2".equalsIgnoreCase(database)) {
				sql = " select * from ("
						+ " select card_id,SERIAL_TYPE,Operate_Date,Operate_time,b_act_id,trade_fee "
						+ " from YKT_CUR.T_TIF_TRADESERIAL ts "
						+ " where ts.card_id="
						+ cardId
						/*+ " and  ts.operate_date>='"
						+ begindate
						...
						+ " and his.SERIAL_TYPE ="
			            + tradecode
						+ " )  tt order by Operate_Date desc,Operate_time desc fetch first 10 rows only";
			} else if ("oracle".equalsIgnoreCase(database)) {
				sql = "select * from ("
						+ " select card_id,SERIAL_TYPE,Operate_Date,Operate_time,b_act_id,trade_fee "
						+ " from ykt_his.t_tif_tradelog_his his "
						...
						+ " and his.SERIAL_TYPE ="
			            + tradecode
						+ " )  tt where rownum <=10 order by Operate_Date desc,Operate_time desc";
			}
			...
			while (result.next() && (count-- > 0)) {
				...
				/*temp_TradeSerialDTO.setSerial_type(getSerialType(result
						.getString("SERIAL_TYPE")));*/
				temp_TradeSerialDTO.setOperate_Date(result
						.getString("Operate_Date"));
				temp_TradeSerialDTO.setOperate_time(result
						.getString("Operate_time"));
				temp_TradeSerialDTO.setTrade_Fee(result.getFloat("trade_fee"));
				temp_TradeSerialDTO.setB_Act_id(BankUtil.hiddenbandcard(result.getString("b_act_id")));
				tradeserials.add(temp_TradeSerialDTO);
			}
			return tradeserials;
	...
	public ArrayList getAssistanceSerial(String cardId, String begindate,
		...
		try {
			...
			sql = "select * from ykt_cur.t_tif_subsidy where cust_no="
					...
					/*
					 * + " and tx_date>='" + begindate + "' and tx_date<='" +
					 * enddate
					 */
					+ " and status<>'1' order by status,subsidy_no desc ";
	...
	public int getSidyCountByCardId(String cardId, String begindate,
		...
		try {
			...
			sql = "select count(*) count from ykt_cur.t_tif_subsidy where cust_no="
					+ accDto.getCustomer_id()
			/*
			 * + " and tx_date>='" + begindate + "' and tx_date<='" + enddate + "' "
			 */;
			stm = conn.prepareStatement(sql);
			result = stm.executeQuery();
			result.next();
	...
	public int getCountByCardId(String cardId,int tradecode, String begindate, String enddate)
			throws ClassNotFoundException, SQLException, Exception {
		String sql = "";
		Connection conn = null;
		PreparedStatement stm = null;
    ...
    //查询所有的领取和未领取的补助
	public ArrayList getAssistanceSerial(String cardId,  int page) throws ClassNotFoundException,
			SQLException, Exception {
		String sql = "";
		Connection conn = null;
		PreparedStatement stm = null;
     ...
     //	查询所有的未领取的补助
	public ArrayList getSidyByCardId(String cardId,  int page) throws ClassNotFoundException,
			SQLException, Exception {
		String sql = "";
		Connection conn = null;
		PreparedStatement stm = null;
		...
		try {
			...
			sql = "select * from ykt_cur.t_tif_subsidy where cust_no="
					+ accDto.getCustomer_id() 
					+ " and status='2' order by status,get_date desc";
			logger.debug("sql [" + sql + "]");
			stm = conn.prepareStatement(sql);
			result = stm.executeQuery();
			ArrayList sidys = new ArrayList();
	...
	// 取得所有未领取的补助
	public int getSidyCountByCardId(String cardId)
			throws ClassNotFoundException, SQLException, Exception {
		String sql = "";
		Connection conn = null;
		PreparedStatement stm = null;
		...
		try {
			...
			conn = getConnection();
			sql = "select count(*) count from ykt_cur.t_tif_subsidy where cust_no="
					+ accDto.getCustomer_id() + " and status='2'";
