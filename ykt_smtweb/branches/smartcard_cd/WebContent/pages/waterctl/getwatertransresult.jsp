...
<html>
	<head>
		...
		<%
			...
			String serialNo1 = request.getAttribute("serialNo1") == null ? ""
					: request.getAttribute("serialNo1").toString();
		...
		function writeCard(){
			...
			var serial_no1 = '<%=serialNo1%>';
			var out_card_amount = '<%=outCardAmount%>';
			var out_card_amount1 = '<%=outCardAmount1%>';
			var url = serverbase+"getWaterTrans.do";
			var parms = "money="+t_money+"&serialNo="+serial_no+"&serialNo1="+serial_no1;
			writeCardForm.all.RFScanner.PortNo=device_port;
			var ret = writeCardForm.all.RFScanner.WaterTransWriteCard(url,card_id,out_card_amount,parms);
			if(ret==0){
				msg.innerHTML="水控转账成功，<br><br>转账金额 "+t_money+" 元，<br><br>一卡通卡余额 "+parseFloat(out_card_amount)/100+" 元，<br><br> 水控钱包卡余额 "+parseFloat(out_card_amount1)/100 +"元";
			}else if(ret==2){
				msg.innerHTML="水控转账失败，失败原因：网络超时！请重试...";
			}else if(ret==-2){
				msg.innerHTML="水控转账失败，失败原因：写卡失败！请正确放卡后重试...";
			}else if(ret==-3){
				msg.innerHTML="水控转账失败，失败原因：请求无应答！";
			}else if(ret==-4){
				window.location='/smartcardweb/pages/waterctl/recardwrite.jsp';
			}else{
				msg.innerHTML="水控转账失败，失败原因："+writeCardForm.all.RFScanner.GetRequestRetInfo("errmsg");
			}
		}
