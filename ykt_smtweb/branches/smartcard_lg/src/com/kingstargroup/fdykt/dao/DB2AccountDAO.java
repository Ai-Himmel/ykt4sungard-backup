...
public class DB2AccountDAO extends DB2ActionDAO{
	...
	public AccountDTO getAccountInfoByCardId(String cardId)
		...
		try {
			conn = getConnection();
			sql = "select a.*,c.begin_time,c.end_time, c.state_id,cus.classdept_no,ctf.Type_name from ykt_cur.T_AIF_Account as a "
					+ " join YKT_CUR.T_PIF_CARD as c on a.card_id=c.card_id "
					+ " join ykt_cur.t_cif_customer as cus on a.Customer_id=cus.cut_id "
					+ " join ykt_cur.t_cif_cuttypefee as ctf on cus.cut_type=ctf.cut_type "
					+ " where a.Card_id="
					+ cardId
					+ " and a.current_state=1 and a.ACT_TYPE=1 fetch first 1 rows only with ur";
			...
			if (result.next()) {
				...
				String deptName = (String)YKTDictionary.getInstance().getDictionValue(
						KSConstant.DICT_DEPTNAME,result.getString("classdept_no"));
	...
	public ArrayList getAccountInfo(String stuempno)
		...
		try {
			conn = getConnection();
			// 只查询正常和写卡失败的记录 
			sql = "select a.*,c.begin_time,c.end_time, c.state_id,cus.ClassDept_name,ctf.Type_name from ykt_cur.T_AIF_Account as a "
					+ "join YKT_CUR.T_PIF_CARD as c on a.card_id=c.card_id "
					+ "join ykt_cur.t_cif_customer as cus on a.Customer_id=cus.cut_id "
					+ "join ykt_cur.t_cif_cuttypefee as ctf on cus.cut_type=ctf.cut_type "
					+ "where a.Card_id>0 and a.StuEmp_no='"
					+ stuempno
					+ "' and a.current_state=1  fetch first 1 rows only with ur";
					//+ " (c.state_id = '" + KSConstant.CARDSTAT_NORMAL + "' or c.state_id='"
