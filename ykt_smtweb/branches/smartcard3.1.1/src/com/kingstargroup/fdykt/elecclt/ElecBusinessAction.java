...
public class ElecBusinessAction extends Action {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(ElecBusinessAction.class);
	...
	private ActionForward checklogin(ActionMapping mapping, HttpServletRequest request) {
		...
		String elecIndex = request.getParameter("elecIndex")==null?"":request.getParameter("elecIndex");
		if (session.getAttribute("cardphyid") == null){
			logger.info("REQ_URL:"+request.getRequestURL()+"?elecIndex="+elecIndex);
			session.setAttribute("REQ_URL", request.getRequestURL()+"?elecIndex="+elecIndex);
