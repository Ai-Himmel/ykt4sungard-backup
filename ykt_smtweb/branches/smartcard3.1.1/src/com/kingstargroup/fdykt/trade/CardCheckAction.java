...
public class CardCheckAction extends Action {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger
			.getLogger(CardCheckAction.class);
	...
	private ActionForward docheck(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("newCardName", session.getAttribute("name")
				.toString());

		// double cardBala = Double.parseDouble(card_bala);
		// cardBala = cardBala / 100;
		String hexcardno = Integer.toHexString(Integer.parseInt(reqcardno))
				.toUpperCase();
		...
		try {
			...
			if (0 != xpackhandle) {
				...
				bcc.SetIntFieldByName(xpackhandle, 0, "lvol7".getBytes(),
						Integer.parseInt(reqcardBalance));// 卡交易前余额(分)

				bcc.SetIntFieldByName(xpackhandle, 0, "lvol6".getBytes(),
						Integer.parseInt(reqcardcnt));// 交易次数
				...
				if (bccret) {
					...
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
							...
							request.setAttribute("outCardAmount",
									out_card_amount);
							logger.info("模拟入账成功,物理卡号：" + hexcardno + ",老卡卡号："
									+ oldCardNo + "，入账日期：" + sdate3 + "，流水号："
									+ serial_no + ",出卡值：" + out_card_amount
									+ "分");
							return mapping.findForward("success");
						} else {

							logger.info("模拟入账失败,后台处理失败,物理卡号：" + hexcardno
									+ ",老卡卡号：" + oldCardNo);
					...
					logger.info("模拟入账失败,后台处理失败,物理卡号：" + hexcardno + ",老卡卡号："
							+ oldCardNo + "connect:errcode=" + errcode[0]
							+ "msg=" + err_msg);
					errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionError(
