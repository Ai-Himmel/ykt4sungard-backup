...
public class NetComputeBusinessAction extends Action {
	...
	private ActionForward checklogin(ActionMapping mapping, HttpServletRequest request) {
		...
		String netIndex = request.getParameter("netIndex") == null ? "" : request.getParameter("netIndex");
		if (session.getAttribute("cardphyid") == null) {
			logger.info("REQ_URL:" + request.getRequestURL() + "?netIndex=" + netIndex);
			session.setAttribute("REQ_URL", request.getRequestURL() + "?netIndex=" + netIndex);
