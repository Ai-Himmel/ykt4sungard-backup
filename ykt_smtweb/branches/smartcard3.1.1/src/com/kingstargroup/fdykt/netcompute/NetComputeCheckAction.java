...
//		reqip = "10.49.188.50";
		...
		String netComputerPayCode = KSConfiguration.getInstance().getProperty("net_computer_pay_code", "4201");//支付代码
		String[] netComputerPayCodeArray = netComputerPayCode.split(",");
		String netIndex = request.getParameter("netIndex");
		String net_computer_pay_code;
		if (StringUtils.isNotBlank(netIndex)) {
			net_computer_pay_code = netComputerPayCodeArray[Integer.parseInt(netIndex)];
		} else {
			net_computer_pay_code = netComputerPayCodeArray[0];
		}
		logger.info("net computer pay code:" + net_computer_pay_code);
		...
		try {
			...
			boolean bccret = bcc.CallRequest(xpackhandle, drtpno, branchno,
				...
				if (bccret) {
					...
					logger.info("func[logicfunc_thirdpay]:850005");
					logger.info("支付代码【lcert_code】:"+net_computer_pay_code);
					logger.info("对方账号【sbank_acc】:"+stuempno);
					logger.info("转账金额【lvol1】:"+money);
					logger.info("卡号【lvol0】:"+card.getCardno());
					logger.info("RetCode:"+RetCode[0]);
					logger.info("RecordCount:"+RecordCount[0]);
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
