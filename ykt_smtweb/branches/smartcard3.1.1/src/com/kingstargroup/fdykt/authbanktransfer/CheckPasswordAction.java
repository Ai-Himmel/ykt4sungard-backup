...
public class CheckPasswordAction extends Action {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(CheckPasswordAction.class);
	...
	private ActionForward doLogin(ActionMapping mapping, HttpServletRequest request) {
		...
		try {
			...
			if (card == null) {
				...
				String sql = "select * from ykt_cur.t_customer cus " + "where cus.custid= " + card.getCustid();
