...
public class DateUtil {
	...
	/**
	 * Method name:checkSpeTimeSequence
	 * Description:   比较两个指定格式的日期先后关系
	 * Return: int  firstTime=second int=0 | firstTime>secondTime int>0 | firstTime<secondTime
	 * @param firstTime
	 * @param secondTime
	 * @param parttern
	 * @return
	 */
	public static int checkSpeTimeSequence(String firstTime, String secondTime,String parttern) {
		try {
			java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat(parttern);
			Date f = sdf.parse(firstTime);
			Date s = sdf.parse(secondTime);
			return f.compareTo(s);
		} catch (ParseException e) {
