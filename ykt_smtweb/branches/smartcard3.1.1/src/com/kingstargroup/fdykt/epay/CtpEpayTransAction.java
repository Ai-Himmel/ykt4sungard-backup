	...
	private ActionForward doPutin(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if(card==null){
				...
				String xmlout = XMLGenerateUtil.ElectXMLGenerate("1001", "操作失败,请先检查卡片状态是否为正常卡！",
						"", "", "", "", null);
				XMLGenerateUtil.XMLSendOut(request, response, xmlout);
				return null;
			}
		...
		try {
			...
			if (0 != xpackhandle) {
				...
				bcc.SetStringFieldByName(xpackhandle, 0, "scust_limit".getBytes(), password.getBytes());//卡密码
					...
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
							...
							String xmlout = XMLGenerateUtil.ElectXMLGenerate(
									"0000", "卡转电子钱包转账成功", out_card_amount,
