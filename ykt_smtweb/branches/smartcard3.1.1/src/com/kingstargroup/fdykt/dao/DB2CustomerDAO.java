...
public class DB2CustomerDAO extends DB2ActionDAO {
    ...
    public ArrayList<TCardphytypeDTO> getAvailCardPhytype(){
		
		Connection conn = null;
		PreparedStatement stm = null;
		ArrayList<TCardphytypeDTO> cardphytypeList = null;
		try {
			ResultSet result;
			cardphytypeList = new ArrayList<TCardphytypeDTO>();
			conn = getConnection();
			try {
				stm = conn.prepareStatement("select cardphytype,cardphytypename,useflag from ykt_cur.t_cardphytype where useflag=1");
				result = stm.executeQuery();
				while (result.next()) {
					TCardphytypeDTO temp = new TCardphytypeDTO();
					temp.setCardphytype(result.getLong("cardphytype"));
					temp.setCardphytypename(result.getString("cardphytypename"));
					temp.setUseflag(result.getInt("useflag"));
					cardphytypeList.add(temp);
				}
				return cardphytypeList;
			} catch (Exception e) {
				e.printStackTrace();
				return null;
			}finally {
	...
	public List getEnablePursetype(){
		String sql="";
		Connection conn = null;
		PreparedStatement stm = null;
		ResultSet result = null;
		...
		try {
			conn = getConnection();
			sql="select * from T_PURSETYPE t where enableflag=1 order by pursetype";
			try {
				stm = conn.prepareStatement(sql);
				result = stm.executeQuery();
				while (result.next()) {
					PursetypeDTO temp = new PursetypeDTO();
					temp.setPursetype(result.getLong("pursetype"));
					temp.setPursetypename(result.getString("pursetypename"));
					list.add(temp);
				}
				return list;
			}catch (Exception e) {
			...
			logger.error("读取卡钱包类型信息失败" + e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
