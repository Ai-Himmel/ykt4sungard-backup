	...
	<head>
		...
		function ReWriteCard(){
			...
			if(ret ==0){
				...
				//小钱包
				window.location='/smartcardweb/pages/waterctlkl/recardwrite1.jsp';
            }else{
            	 alert("水控钱包写卡失败，请放置好卡片后重试");
			}
		}
		function NoWriteCard(){
			var serverbase = "http://"+"<%=webserverip%>"+":"+"<%=webserverport%>"+"/"+"<%=webserverapp%>"+"/";
            var device_port = '<%=read_card_device_port%>';
            var serial_no_1 = writeCardForm.all.RFScanner.GetRequestRetInfo("serial_no_1");
            var serial_no_2 = writeCardForm.all.RFScanner.GetRequestRetInfo("serial_no_2");
            var params = "serial_no_1="+serial_no_1+"&serial_no_2="+serial_no_2+"&lvol3=0";
            var ret = 0;
            ret = writeCardForm.all.RFScanner.ReportWriteCardFailure(serverbase+"dealWaterTransCardExp.do",params);
            if(ret == 0){
