		...
		function writeCard(){
			...
			var msg = document.getElementById("msg");
			 var ret=customerPwdUpdateForm.all.RFScanner.ChangePassWord(
              	customerPwdUpdateForm.all.cardphyid.value,
              	customerPwdUpdateForm.all.newPwd.value);
              if(ret==0)//修改密码成功
              {
              	msg.innerHTML = "密码修改成功。";
              }else{
              	msg.innerHTML = "密码修改成功。但写卡失败，可以用新密码补改一次密码！errcode="+ret;
              }
		}
		</script>
		<style type="text/css">
								...
								<tr>
									<td height="320" align="center" class="样式13" id="msg">
											<input type="hidden" name="cardphyid"
												value="<%=session.getAttribute("cardphyid") == null ? ""
					: session.getAttribute("cardphyid")%>" />
