...
<html>
	<head>
		<title>新生换卡清除老卡</title>
		<link href="/smartcardweb/pages/css/css.css" rel="stylesheet"
			type="text/css" />
		<%
			String cardphyid = request.getAttribute("cardphyid") == null ? ""
					: request.getAttribute("cardphyid").toString();		
			String newcardphyid = request.getAttribute("newcardphyid") == null ? ""
                    : request.getAttribute("newcardphyid").toString();     		
			String showcardno = request.getAttribute("showcardno") == null ? ""
					: request.getAttribute("showcardno").toString();	
			String pay_card_cnt = request.getAttribute("pay_card_cnt") == null ? ""
					: request.getAttribute("pay_card_cnt").toString();	
			String dpt_card_cnt = request.getAttribute("dpt_card_cnt") == null ? "0"
					: request.getAttribute("dpt_card_cnt").toString();						
			String cardBala = request.getAttribute("cardBala") == null ? "0"
		...
		function writeCard(){
            ...
            var newcardphyid = '<%=newcardphyid%>';
            var showcardno = '<%=showcardno%>';
            var pay_card_cnt = '<%=pay_card_cnt%>';
            var dpt_card_cnt = '<%=dpt_card_cnt%>';
            var cardBala = '<%=cardBala%>';
            var ipServer= '<%=ipServer%>';
			var drtpPort= '<%=drtpPort%>';
			var svrFunc= '<%=svrFunc%>';
			writeCardForm.all.RFScanner.PortNo=device_port;
			var ret = writeCardForm.all.RFScanner.CardChangeClearOldCard(cardphyid,newcardphyid,showcardno,dpt_card_cnt,pay_card_cnt,cardBala,ipServer,drtpPort,svrFunc);
			if(ret==0){
				 window.location='/smartcardweb/pages/changecard/getnewcardcharge.jsp';
			}else{
                window.location='/smartcardweb/pages/changecard/oldcardrecardwrite.jsp';
			}
		}
