	...
	<head>
		...
		function ReWriteCard(){
            ...
            writeCardForm.all.RFScanner.PortNo=device_port;
              var ret = writeCardForm.all.RFScanner.CheckNewCard();
                if(ret == 0){
                    var rrret = writeCardForm.all.RFScanner.ReCardChangeNewCard(serverbase+"newcardchange.do");
		            if(rrret==0){
		                 window.location='/smartcardweb/pages/changecard/changecardsuccess.jsp';
		            }else if(rrret==1){
                        //后台返回失败
                         var showcardno =  writeCardForm.all.RFScanner.GetChangeCardinfo("showcardno");
                         writeCardForm.all.showcardno.value=showcardno;
                         writeCardForm.all.stage.value="3";
                         writeCardForm.all.error.value="3";
                         ...
                         writeCardForm.all.showcardno.value=showcardno;
                         writeCardForm.all.stage.value="3";
                         writeCardForm.all.error.value="1";
                         writeCardForm.submit();
		            }else{
		                 //已部分写卡
		                  var showcardno =  writeCardForm.all.RFScanner.GetChangeCardinfo("showcardno");
                         writeCardForm.all.showcardno.value=showcardno;
                         writeCardForm.all.stage.value="3";
                         writeCardForm.all.error.value="2";
                         writeCardForm.submit();
		            }
                }else if(ret == -1){
                    alert("请确认您放置的正式卡是否正确...");
                }else if(ret == 1){
                    alert("请放置正式卡...");
                }else{
                    alert("请正确放置正式卡...");
