...
<html>
	<head>
		...
		function writeCard(){
			...
			writeCardForm.all.RFScanner.BeginUpdateCard(cardphyid);
			if(""!=feetype){
            	writeCardForm.all.RFScanner.SetCardProperty("feetype",feetype);
			}
            if(""!=expiredate){
            	writeCardForm.all.RFScanner.SetCardProperty("expiredate",expiredate);
            }
            if(""!=stuempno){
            	writeCardForm.all.RFScanner.SetCardProperty("stuempno",stuempno);
            }
            var ret = writeCardForm.all.RFScanner.UpdateCard();
            if(ret==0){
				msg.innerHTML="卡信息更新成功!<br/><br/>";
			}else if(ret==2){
                msg.innerHTML="卡信息更新失败!<br/><br/>失败原因：卡号不正确！";
            }else if(ret==-5){
				msg.innerHTML="卡信息更新失败!<br/><br/>失败原因：写卡失败，请重新更新！";
			}else{
				msg.innerHTML="卡信息更新失败!<br/><br/>失败原因：未知错误(返回码："+ret+")!";
			}
		}
...
.样式13 {
	...
	<META HTTP-EQUIV="refresh" CONTENT="60;url=/smartcardweb/index.do">
