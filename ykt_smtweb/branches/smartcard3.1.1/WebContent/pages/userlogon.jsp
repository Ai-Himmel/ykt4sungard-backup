...
<%@page import="com.kingstargroup.fdykt.dto.TCardphytypeDTO"%>
<html>
	<head>
		<title>用户登陆</title>
		...
		<%
		ArrayList list = request.getAttribute("cardphytypeList")==null?null:(ArrayList)request.getAttribute("cardphytypeList");
        ...
        function addnum(num)
        {
			if(logonForm.all.nowinput.value=="1" && logonForm.all.stuempno.value.length < 15){
			...
			}else if(logonForm.all.nowinput.value=="2" && logonForm.all.password.value.length < 6){
				logonForm.all.password.value+=num;
			} else if(logonForm.all.nowinput.value=="3" && logonForm.all.idno.value.length < 18){
				if(logonForm.all.locknum.value=="1"){
					logonForm.all.idno.value+=num;
				}else{
					logonForm.all.idno.value+=num.toUpperCase();
				}	
			}
        }
        function numlock(){
		...
		else if(logonForm.all.nowinput.value=="3")
			logonForm.all.idno.value=logonForm.all.idno.value.substring(0,logonForm.all.idno.value.length-1);	
        }
        function tab()
        {
              if(logonForm.all.nowinput.value=="1")
                 logonForm.all.idno.focus();
              else if(logonForm.all.nowinput.value=="2")
       ...
       function logonSelect(cardtype,cardphytypename){
        	logonForm.all.cardphytype.value=cardtype;
        	logonForm.all.cardphytypename.value=cardphytypename;
        	logonForm.submit();
						...
						<table width="900" border="0" align="center" cellpadding="0"
							...
							<tr>
								<td height="250" align="center" valign="top">
									<p>
										<font color="#FF0000">&nbsp;请输入学工号（或者身份证号）与密码</font>
									</p>
									<table width="98%" border="0" align="right" cellpadding="0"
										...
										<tr>
											...
											<td>
												<input type="text" name="stuempno" class="样式11" size="18"
													maxlength="15"
													onfocus="javascript:logonForm.nowinput.value='1'" />
												<input type="hidden" name="cardphytype" value="0"/>
												<input type="hidden" name="cardphytypename" value="0"/>
											</td>
										</tr>
										<tr>
											<td align="center">
												<img src="/smartcardweb/pages/images/idno.gif" width="99"
													height="31" align="middle" />
											</td>
											<td>
												<input type="text" name="idno" class="样式11" size="18"
													maxlength="18"
													onfocus="javascript:logonForm.nowinput.value='3'" />
											</td>
										</tr>
										<tr>
											<td align="center">
												<img src="/smartcardweb/pages/images/ps.gif" width="99"
													height="29" align="middle" />
											</td>
											<td>
												<input name="password" type="password" class="样式11"
													onfocus="javascript:logonForm.nowinput.value='2'" size="18"
													maxlength="6" />
											</td>
										</tr>
										<tr>
										...
										<%
												...
												for(int i=0;i<list.size();i++){
													TCardphytypeDTO cardtypeDTO = (TCardphytypeDTO)list.get(i);
													cardphytypename=cardtypeDTO.getCardphytypename();
													cardtype=cardtypeDTO.getCardphytype()+"";
													if(cardtypeDTO.getCardphytype()==10){
												%>
												<a onclick="javascript:logonSelect(10,'<%=cardphytypename%>');"
													onmouseup="MM_swapImgRestore()"
													onmousedown="MM_swapImage('ick1','','/smartcardweb/pages/images/icka.gif',1)"><img
														src="/smartcardweb/pages/images/ick.gif" name="ick1"
														border="0" id="ick1" /> </a>
													
												<%		
													}else if(cardtypeDTO.getCardphytype()==20){
													%>
													<a onclick="javascript:logonSelect(20,'<%=cardphytypename%>');"
													onmouseup="MM_swapImgRestore()"
													onmousedown="MM_swapImage('ick2','','/smartcardweb/pages/images/icka.gif',1)"><img
														src="/smartcardweb/pages/images/ick.gif" name="ick2"
													    border="0" id="ick2" /> </a>
													<%											
													}else if(cardtypeDTO.getCardphytype()==30){
														
														%>
														<a onclick="javascript:logonSelect(30,'<%=cardphytypename%>');"
													onmouseup="MM_swapImgRestore()"
													onmousedown="MM_swapImage('sjk','','/smartcardweb/pages/images/sjka.gif',1)"><img
														src="/smartcardweb/pages/images/sjk.gif" name="sjk"
														border="0" id="sjk" /> </a>
														<%	
													}
												}
