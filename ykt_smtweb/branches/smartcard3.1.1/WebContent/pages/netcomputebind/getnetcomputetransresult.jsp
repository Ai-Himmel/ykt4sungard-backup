...
<html>
	<head>
		...
		function writeCard(){
			...
			var url = serverbase+"getNetComputeTransbind.do";
			...
			if(ret==0){
				msg.innerHTML="上机上网转账成功!<br/><br/>转账金额 "+t_money+" 元.<br/><br/>卡余额 "+parseFloat(out_card_amount)/100+" 元";
			}else if(ret==2){
				msg.innerHTML="上机上网转账失败!<br/><br/>失败原因：网络超时！请到卡务中心补写卡信息.";
			}else if(ret==-100){
				msg.innerHTML="上机上网转账失败!<br/><br/>失败原因：写卡失败，请重新操作！";
			}else if(ret==-3){
				msg.innerHTML="上机上网转账失败!<br/><br/>失败原因：请求无应答！请到卡务中心补写卡信息.";
			}else{
				msg.innerHTML="上机上网转账失败!<br/><br/>请到卡务中心补写卡信息.<br/><br/>失败原因："+writeCardForm.all.RFScanner.GetRequestRetInfo("errmsg");
			}
		}
