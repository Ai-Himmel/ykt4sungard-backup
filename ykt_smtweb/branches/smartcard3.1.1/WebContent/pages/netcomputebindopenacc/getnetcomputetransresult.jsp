...
<html>
	<head>
		...
		function writeCard(){
			...
			var stuempno = '<%=stuempno%>';
			var url = serverbase+"getNetComputeTransbindopenacc.do";
			var parms = "cardphyid="+cardphyid+"&money="+t_money+"&sphone3="+sphone3+"&saddr="+saddr+"&stuempno="+stuempno;
			...
			if(ret==0){
				msg.innerHTML="上网帐号开通成功!<br/><br/>转账金额 "+t_money+" 元.<br/><br/>卡余额 "+parseFloat(out_card_amount)/100+" 元";
			}else if(ret==2){
				msg.innerHTML="上网帐号开通失败!<br/><br/>失败原因：网络超时！请到卡务中心补写卡余额.";
			}else if(ret==-100){
				msg.innerHTML="上网帐号开通失败!<br/><br/>失败原因：写卡失败，请重新操作！";
			}else if(ret==-3){
				msg.innerHTML="上网帐号开通失败!<br/><br/>失败原因：请求无应答！请到卡务中心补写卡信息.";
			}else{
				msg.innerHTML="上网帐号开通失败!<br/><br/>请到卡务中心补写卡信息.<br/><br/>失败原因："+writeCardForm.all.RFScanner.GetRequestRetInfo("errmsg");
			}
		}
