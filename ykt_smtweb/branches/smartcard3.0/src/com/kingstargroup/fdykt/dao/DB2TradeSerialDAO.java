			...
			if ("db2".equalsIgnoreCase(database)) {
				if (accDto.getCustid() > 0) {
					sql = "select * from "
							...
							+ "(cardaftbal-cardbefbal) tradefee,cardbefbal,cardaftbal "
...
//							+ "' and t.status=3  ) tt where rownum <=1000 order by transdate desc,cardno desc, cardcnt desc,transtime desc";
							+ "' and t.status=3  " +
									...
									"or t.transcode=3101 or t.transcode=3270)) tt where rownum <=1000 order by transdate desc,transtime desc";
				} else {
					sql = "select * from "
							+ "(select custid,cardno,transcode,termid,transdate,transtime,cardcnt,"
							//+ "(case when (cardaftbal-cardbefbal)>0 then  (cardaftbal-cardbefbal) else amount end) tradefee,cardbefbal,cardaftbal "
		...
		try {
			...
			if (accDto.getCustid() > 0) {
				sql = "select count(*) count "
						+ "from ykt_cur.v_transdtl t where t.custid="
						+ accDto.getCustid() + " and t.transdate>='"
						+ begindate + "' and t.transdate<='" + enddate
						+ "' and t.status=3 ";
			} else {
				sql = "select count(*) count "
						+ "from ykt_cur.v_transdtl t where t.cardno='" + cardId
