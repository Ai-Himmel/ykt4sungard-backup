...
//		HttpSession session = request.getSession(false);
		...
		String outhexcardno = Integer.toHexString(Integer.parseInt(outcardphyno))
		.toUpperCase();
		...
		try {
			...
			if (0 != xpackhandle) {
				...
				if (bccret) {
					...
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
							...
							request.setAttribute("outcardAmount",
									out_card_amount);
							logger.info("卡卡转账模拟入账成功,物理卡号：" + hexcardno + "，入账日期：" + sphone3 + "，流水号："
									+ serial_no + ",出卡值：" + card_amount
									+ "分");
							return mapping.findForward("success");
						} else {

							logger.info("卡卡转账模拟入账失败,后台处理失败,物理卡号：" + hexcardno);
						...
						if (0 < RecordCount[0]) {
							...
							logger.info("卡卡转账模拟入账失败,后台处理失败,物理卡号：" + hexcardno + "返回错误信息："
									+ vs_mess);
							return mapping.findForward("failure");
						} else {
							...
							errors.add(ActionMessages.GLOBAL_MESSAGE,
									new ActionError("back.error", "后台处理失败"));
							saveErrors(request, errors);
							logger.info("卡卡转账模拟入账失败,后台处理失败,物理卡号：" + hexcardno+ "返回错误信息：无");
							return mapping.findForward("failure");
						}
					...
					logger.info("模拟入账失败,后台处理失败,物理卡号：" + hexcardno + "connect:errcode=" + errcode[0]
							+ "msg=" + err_msg);
					errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionError(
