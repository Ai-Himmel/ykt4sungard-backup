			...
			if(rrt == 0){
			  var sret=oldCardForm.all.RFScanner.DoScan();
	           if(sret==0){//读卡成功 
	               ...
	               oldCardForm.all.cardphyid.value=cardno;
	                var ret = document.all.RFScanner.ReadCardPocket(1,cardno,"");
		            if(ret == 0){
		                var bala = oldCardForm.all.RFScanner.GetPacketInfo("bala");
                        var cxCnt =  oldCardForm.all.RFScanner.GetPacketInfo("total_cnt");
                         oldCardForm.all.cardBala.value = bala;
                         oldCardForm.all.txCnt.value = cxCnt;
		                 var cardsubsidyno = document.all.RFScanner.ReadCardSidyNo(cardno,"");
		                 if(cardsubsidyno>=0){
		                    oldCardForm.all.cardsubsidyno.value= cardsubsidyno; 
		                    oldCardForm.submit();
		                 }else{
		                   alert("补助批次号读取失败");
		                 }
								...
								<table width="80%" border="0" align="center" cellpadding="0"
									...
									<tr>
										...
										<td height="60" class="样式13">
											...
											<input type="hidden" name="cardsubsidyno">
