...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function ReWriteCard(){
			...
			if(ret ==0){
				window.location='/smartcardweb/pages/cfgfee/cardwritesuccess.jsp';
			}else if(ret ==-100){//写卡失败
			     NoWriteCard();
			}else if(ret ==-10){//中途拔卡
			    window.location='/smartcardweb/pages/cfgfee/cardwriteerror1.jsp';
			}else{
			    window.location='/smartcardweb/pages/cfgfee/cardwriteerror2.jsp';  
			}
		}
		function NoWriteCard(){
			var serverbase = "http://"+"<%=webserverip%>"+":"+"<%=webserverport%>"+"/"+"<%=webserverapp%>"+"/";
			var ret = 0;
			ret = writeCardForm.all.RFScanner.ReverseNetTrans(serverbase+"dealCfgfeeTransErr.do");
			if(ret == 0){
				window.location='/smartcardweb/pages/cfgfee/cardwriteerror.jsp';
			}else{
				window.location='/smartcardweb/pages/cfgfee/cardwriteerror4.jsp';
			}
		}
	</script>
		<style type="text/css">
