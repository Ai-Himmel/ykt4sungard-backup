...
public class DB2TradeSerialDAO extends DB2ActionDAO {
		...
		try {
			...
			while (result.next() && (count-- > 0)) {
				...
				ttifsubsidydto.setStatus(getSidyStatus(result
						.getString("status")));
				ttifsubsidydto.setSubsidyNo(Integer.valueOf(result
						.getString("subsidy_no")));
	 ...
	 * @since 1.0
	 */
	public TTifSubsidyDTO getAssistanceByCardId(String cardId,String sidyno)
		...
		try {
			...
			if ("db2".equalsIgnoreCase(database)) {
				sql = "select * from ykt_cur.t_tif_subsidy where cust_no="
						...
						+ " and card_no = "
						+ accDto.getCard_id()
						+ " and status='2' and subsidy_no>"+sidyno+" order by subsidy_no fetch first 1 rows only";
			} else if ("oracle".equalsIgnoreCase(database)) {
				sql = "select * from ykt_cur.t_tif_subsidy where cust_no="
						+ accDto.getCustomer_id() + " and card_no = "
						+ accDto.getCard_id()
						+ " and status='2' and subsidy_no>"+sidyno+" and rownum=1 order by subsidy_no";
			}
			logger.debug("sql [" + sql + "]");
			stm = conn.prepareStatement(sql);
			result = stm.executeQuery();
			...
			logger.error("查询补助流水失败,cardId[" + cardId + "]");
			throw (e1);
		} catch (Exception e) {
			e.printStackTrace();
			throw (e);
	 ...
	 * @since 1.0
	 */
	public int getSidyCountByCardId(String cardId,String sidyno)
			throws ClassNotFoundException, SQLException, Exception {
		String sql = "";
		Connection conn = null;
		PreparedStatement stm = null;
		...
		try {
			...
			sql = "select count(*) count from ykt_cur.t_tif_subsidy where cust_no="
					+ accDto.getCustomer_id() + " and card_no ="+
					accDto.getCard_id()
					+" and status='2' and subsidy_no>"+sidyno;
			stm = conn.prepareStatement(sql);
			result = stm.executeQuery();
			if (result.next()) {
				return result.getInt("count");
	...
	public ArrayList getTradeSerialByCollDate(String stuempno,
		...
		try {
			...
			sql = "select tradelog.* from ykt_cur.T_TIF_TradeLog tradelog "
					+ "join ykt_cur.T_AIF_Account account on tradelog.Act_id=account.Account_id and account.purse_id=0"
	...
	public int getCountByCardId(String cardId, String begindate, String enddate)
		...
		try {
			...
			sql = "select count(*)  count from ("
					...
					+ "from ykt_his.t_tif_tradelog_his his "
					+ "join ykt_cur.T_AIF_Account account on his.Act_id=account.Account_id and account.purse_id=0 "
					...
					+ "from ykt_cur.T_TIF_TradeLog  log "
					+ "join ykt_cur.T_AIF_Account  account on log.Act_id=account.Account_id and account.purse_id=0 "
