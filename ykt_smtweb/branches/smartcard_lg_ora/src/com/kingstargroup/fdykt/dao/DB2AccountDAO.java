	...
	public AccountDTO getAccountInfoByCardId(String cardId)
		...
		try {
			...
			if ("db2".equalsIgnoreCase(database)) {
				sql = "select a.*,c.begin_time,c.end_time, c.state_id,cus.classdept_no,ctf.Type_name,dic.dict_caption from ykt_cur.T_AIF_Account a "
						...
						+ " join ykt_cur.t_pif_dictionary dic on a.card_type=dic.dict_value and dic.dict_no=18 "
						+ " where a.Card_id="
						+ cardId
				...
				sql = "select a.*,c.begin_time,c.end_time, c.state_id,cus.classdept_no,ctf.Type_name,dic.dict_caption from ykt_cur.T_AIF_Account a "
						...
						+ " where a.Card_id="
						+ cardId
						+ " and a.purse_id=0 and a.current_state=1 and a.ACT_TYPE=1 and rownum=1";
			}
			try {
				...
				if (result.next()) {
					...
					temp_DTO.setCard_type_name(result.getString("dict_caption"));
					return temp_DTO;
				}
			} finally {
				if (stm != null) {
