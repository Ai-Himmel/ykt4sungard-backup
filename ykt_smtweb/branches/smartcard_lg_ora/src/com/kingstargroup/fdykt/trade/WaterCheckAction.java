...
public class WaterCheckAction extends Action {
	...
	private ActionForward docheck(ActionMapping mapping, ActionForm form,
		...
		String water_bala = request.getParameter("waterBala");//元
		String card_bala = request.getParameter("cardBala");//分
		...
		try {
			...
			if (0 != xpackhandle) {
				...
				bcc.SetDoubleFieldByName(xpackhandle, 0, "damt2".getBytes(),
						Double.parseDouble(water_bala));//小钱包入卡值
					...
					if (0 == RetCode[0]) {
						if (0 < RecordCount[0]) {
							...
							bcc.GetDoubleFieldByName(xpackhandle, 0, "damt2"
									.getBytes(), OutCardAmount1);//小钱包出卡值
							...
							String out_card_amount1 =  String.valueOf(OutCardAmount1[0]);
							...
							request.setAttribute("serialNo1", serial_no1);
							request.setAttribute("outCardAmount", out_card_amount);
							request.setAttribute("outCardAmount1", out_card_amount1);
