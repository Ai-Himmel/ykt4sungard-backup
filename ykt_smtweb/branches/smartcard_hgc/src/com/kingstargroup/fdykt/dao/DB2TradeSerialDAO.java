...
public class DB2TradeSerialDAO extends DB2ActionDAO {
	...
	public String getDeviceName(int deviceId)throws Exception {
		...
		try{
			conn = getConnection();
		    sql =" select device_name as deviceName from  ykt_cur.t_pif_device where device_id="+deviceId; 
	...
	public ArrayList getTSByCardId(String cardId, String begindate,
		...
		try {
			...
			sql = "select  * from  (select Serial_type, Device_Id ,Operate_Date ,Operate_time ,(out_balance - in_balance) Op_Fee,out_balance ,trade_count"
				             ...
				             +"' and Operate_Date <='"
				             + enddate
				             +"' and serial_state = 2 and not (serial_type='930036') "
				             +" union all "
				             ...
				             +" '" 
				             +" and serial_state = 2 and not (serial_type='930036')"
				             +") tt where rownum<1000 order by Operate_Date desc ,trade_count desc";
			...
			try {
				...
				while (result.next() && (count-- > 0)) {
					...
					temp_TradeSerialDTO.setArea_name(getDeviceName(result
							.getInt("Device_Id")));
					/*temp_TradeSerialDTO.setDict_caption(result
							.getString("Comments"));*/
					temp_TradeSerialDTO.setTrade_count(String.valueOf(result.getLong("trade_count")));
					...
					temp_TradeSerialDTO.setFreeBala(result.getFloat("out_balance"));
	...
	public int getCountByCardId(String cardId, String begindate, String enddate)
		...
		try {
			...
			*/
			sql = "select  count(*) count from  (select Serial_type, Device_Id ,Operate_Date ,Operate_time ,(out_balance - in_balance) Op_Fee,out_balance ,trade_count"
	             +" from  ykt_his.t_tif_tradeserial_his  "
	             +" where customer_id='"
	             + accDto.getCustomer_id()
	             +"' and Operate_Date >='" 
