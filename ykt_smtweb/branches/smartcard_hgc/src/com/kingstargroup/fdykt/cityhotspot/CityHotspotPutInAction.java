		...
		try {
			...
			if (0 != xpackhandle) {
			  ...
			  //  bcc.SetStringFieldByName(xpackhandle, 0,"sserial1".getBytes(),pwd.getBytes());//转帐密码
				bcc.SetStringFieldByName(xpackhandle, 0, "sname".getBytes(),ip.getBytes());//送ip
				...
				if (bccret) {
					...
					// 请求失败
					request.setAttribute("errmsg", "后台通讯暂时不可用，请稍后重试!"+"bccret=false" );
					return mapping.findForward("failure");
				}
			}
