...
<html>
	<head>
		...
		<%
		 ...
		 String UndoLoss = request.getAttribute("UndoLoss")==null?"":request.getAttribute("UndoLoss").toString();
		 String cardNo = (String) session.getAttribute("cardno");
		 String blkver = request.getAttribute("blkver")==null?"":request.getAttribute("blkver").toString();
		%>
		
		<script language="javascript">
		  function write(){
		    var ret=returnform.all.RFScanner.UndoLoss("<%=paramKey%>",<%=cardNo%>,"<%=blkver%>");
		    if(ret != 0){
		        msg.innerHTML="请到卡管理中心去处理";
		     }
	...
	<%if("UndoLoss".equals(UndoLoss))
	  {%>
	  <body scroll="no" onload="write();">
	<%}else{%>
	 <body scroll="no" >
	<%}
	%>
