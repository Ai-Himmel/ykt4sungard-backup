	...
	<head>
		...
		<%
		   ...
		   // String paramKey = (String) session.getAttribute("paramKey");
		    ...
		    String chargeID = request.getAttribute("chargeID")== null?"":request.getAttribute("chargeID").toString();
		    String txCnt = request.getAttribute("txCnt")==null?"0":request.getAttribute("txCnt").toString();
		...
		function writeCard(){
			...
			var url = serverbase+"rejieTransfer.do";
			var parms = "money="+t_money+"&serialNo="+serial_no+"&chargeID="+chargeID+"&cardId="+card_id;;
			writeCardForm.all.RFScanner.PortNo=device_port;
			var ret = writeCardForm.all.RFScanner.ElectTransWriteCard(url,card_id,out_card_amount,<%=txCnt%>,parms)
			if(ret==0){                           
				msg.innerHTML="转账成功!<br/><br/>转账金额 "+t_money+" 元.<br/><br/>卡余额 "+parseFloat(out_card_amount)/100+" 元";
			}else if(ret==2){
				...
				msg.innerHTML="转账失败!<br/><br/>失败原因："+writeCardForm.all.RFScanner.GetRequestRetInfo("errmsg");
			}
		}
