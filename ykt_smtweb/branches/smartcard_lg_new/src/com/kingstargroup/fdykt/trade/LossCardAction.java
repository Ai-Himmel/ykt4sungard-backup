...
public class LossCardAction extends Action {
	...
	private ActionForward doLossCard(ActionMapping mapping,
		...
		try {
			...
			carddao.createLossInfo(lossinfo); // 如果该卡号被冻结,则挂失时对黑名单不处理
			if (state.substring(2, 3).equals("0")) { // 插入黑名单表 
				...
				carddao.createBlackSheet(blacksheet); // 广播黑名单 
				MesListDTO
				meslist =new MesListDTO();
			...
			/*
			 * long xpackhandle = 0; int drtpno = 0; int errcode[] = { 1 }; byte
			 * errmsg[] = new byte[300]; int branchno = 0;// 后台drtp功能号 int
			 * mainfunction = KSConfiguration.getInstance().getPropertyAsInt(
			 * "mainfunction", 9000); int func =
			 * KSConfiguration.getInstance().getPropertyAsInt(
			 * "logicfunc_card_loss", 847318);// 后台业务功能号 int tries =
			 * KSConfiguration.getInstance().getPropertyAsInt("tries", 3);//
			 * 默认重试次数 int[] RetCode = new int[1]; int[] RecordCount = new
			 * int[1]; bccclt bcc = null; try { bcc =
			 * BccctlInit.getBccctlInstance(); xpackhandle =
			 * bcc.NewXpackHandle(KSConfiguration.getInstance()
			 * .getProperty("cpackfile", "/opt/cpack.dat").getBytes());
			 * System.out.println("对媒体发送请求 功能号："+mainfunction+"后台业务："+func); if
			 * (0 != xpackhandle) { System.out.println("对媒体设置环境");
			 * bcc.SetRequestType(xpackhandle, func);
			 * bcc.SetIntFieldByName(xpackhandle, 0, "lvol0".getBytes(),
			 * Integer.parseInt(cutid));// 客户号
			 * bcc.SetIntFieldByName(xpackhandle, 0, "lvol1".getBytes(),
			 * Integer.parseInt("0"));// 交易卡号 bcc.SetIntFieldByName(xpackhandle,
			 * 0, "lvol2".getBytes(), Integer.parseInt("0"));// 交易卡号
			 * bcc.SetStringFieldByName(xpackhandle, 0, "semp_pwd" .getBytes(),
			 * password.getBytes());//密码 bcc.SetStringFieldByName(xpackhandle,
			 * 0, "sstatus0" .getBytes(), "1".getBytes());//校验密码
			 * bcc.SetStringFieldByName(xpackhandle, 0, "scust_limit"
			 * .getBytes(), "web".getBytes());//密码
			 * 
			 * boolean bccret = false; while (--tries >= 0) {
			 * System.out.println("对媒体发送请求:"+tries); bccret =
			 * bcc.CallRequest(xpackhandle, drtpno, branchno, mainfunction,
			 * 2000, errcode, errmsg); if (bccret) { break; } } if (bccret) {
			 * bcc.GetRetCode(xpackhandle, RetCode);
			 * bcc.GetRecordCount(xpackhandle, RecordCount); if (0 ==
			 * RetCode[0]) { return mapping.findForward("success"); } else { //
			 * 失败 if (0 < RecordCount[0]) { byte[] vsmess = new byte[300];
			 * bcc.GetStringFieldByName(xpackhandle, 0, "vsmess".getBytes(),
			 * vsmess, 300); int i; for (i = 0; i < vsmess.length; i++) { if
			 * (vsmess[i] == 0) { break; } } String vs_mess = new String(vsmess,
			 * 0, i); errors.add("lost", new ActionError(vs_mess)); if
			 * (!errors.isEmpty()) { saveErrors(request, errors); } return
			 * mapping.findForward("failure"); } else { errors.add("lost", new
			 * ActionError("后台处理失败")); if (!errors.isEmpty()) {
			 * saveErrors(request, errors); } return
			 * mapping.findForward("failure"); }
			 *  }
			 *  } else { // 请求失败
			 * 
			 * int i = 0; for (i = 0; i < errmsg.length; i++) { if (errmsg[i] ==
			 * 0) { break; } } String err_mess = new String(errmsg, 0, i);
			 * System.out.println("对媒体发送请求失败,错误码："+errcode[0]+"错误原因："+err_mess);
			 * errors.add("lost", new ActionError("errors.request")); if
			 * (!errors.isEmpty()) { saveErrors(request, errors); } return
			 * mapping.findForward("failure");
			 *  } } errors.add("lost", new ActionError("系统出现异常，请稍后重试！")); if
			 * (!errors.isEmpty()) { saveErrors(request, errors); } return
			 * mapping.findForward("failure");
			 *  } catch (Exception e) { // TODO: handle exception
			 * logger.error(e.getMessage()); errors.add("lost", new
			 * ActionError("系统出现异常，请稍后重试！")); if (!errors.isEmpty()) {
			 * saveErrors(request, errors); } return
			 * mapping.findForward("failure"); } catch (UnsatisfiedLinkError e) {
			 * logger.error(e.getMessage()); errors.add("lost", new
			 * ActionError("系统出现异常，请稍后重试！")); if (!errors.isEmpty()) {
			 * saveErrors(request, errors); } return
			 * mapping.findForward("failure"); } finally { if (xpackhandle != 0) {
