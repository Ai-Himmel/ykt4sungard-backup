...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<%
			...
			String cellphone = request.getAttribute("cellphone") == null ? ""
					: request.getAttribute("cellphone").toString();
		...
		function writeCard(){
			...
			var card_id = <%=cardId%>;
			var cell_phone = <%=cellphone%>;
			var t_money = <%=money%>;
			var serial_no = <%=serialNo%>;
			var out_card_amount = <%=outCardAmount%>;
			var url = serverbase+"getMobileTrans.do";
			var parms = "cellphone="+cell_phone+"&money="+t_money+"&serialNo="+serial_no;
			writeCardForm.RFScanner.PortNo=device_port;
			var ret = writeCardForm.RFScanner.MobileTransWriteCard(url,card_id,out_card_amount,parms);
			if(ret==0){
				msg.innerHTML="手机充值成功，充值金额 "+t_money+" 元，卡余额 "+parseFloat(out_card_amount)/100+" 元";
			}else if(ret==2){
				msg.innerHTML="手机充值失败，失败原因：网络超时！";
			}else if(ret==-2){
				msg.innerHTML="手机充值失败，失败原因：写卡失败！";
			}else if(ret==-3){
				msg.innerHTML="手机充值失败，失败原因：请求无应答！";
			}else{
				msg.innerHTML="手机充值失败，失败原因："+writeCardForm.RFScanner.GetRequestRetInfo("errmsg");
			}
		}
	...
	<body onload="javascript:writeCard();">
