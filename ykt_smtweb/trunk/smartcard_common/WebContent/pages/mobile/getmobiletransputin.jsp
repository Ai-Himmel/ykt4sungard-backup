...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">			
	        function addnum(num)
	        {
			if(putinForm.nowinput.value=="1" && putinForm.cellphone.value.length < 12 ){
				putinForm.cellphone.value+=num;
			}
			else if(putinForm.nowinput.value=="2" && putinForm.money.value.length < 5 ){
			...
			if(putinForm.nowinput.value=="1")
				putinForm.cellphone.value=putinForm.cellphone.value.substring(0,putinForm.cellphone.value.length-1);
			else if(putinForm.nowinput.value=="2")
				putinForm.money.value=putinForm.money.value.substring(0,putinForm.money.value.length-1);		
	        }      
		   	 function readCard(){
		   	 	if(putinForm.cellphone.value==''){
		   	 		alert("请输入充值手机号！");
		   	 		putinForm.cellphone.focus();
		   	 		return false;
		   	 	}
		   	 	if(!fucCheckNUM(putinForm.cellphone.value)){
		   	 	 	alert("手机号码为数字！");
		   	 		putinForm.cellphone.focus();
		   	 		return false;
		   	 	}
		   	 	if(putinForm.cellphone.value.length<11){
		   	 		alert("手机号码长度不正确！");
		   	 		putinForm.cellphone.focus();
		   	 		return false;
		   	 	}
