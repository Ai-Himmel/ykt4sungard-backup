...
//		AddField_C(pstpack, F_SCUST_NO, plist->GetAt(7));	// 客户号
		AddField_C(pstpack, F_SCUST_AUTH, sAccountsId);			// 账号
...
/* 语音电话按键规则转换函数(海事大学)                                   */
/************************************************************************/
void ChangeIdFromPhone(char *phone_id, char account_id[22])
{
	char *position = NULL;
	int cmp_count = 3;
	char data_num[3] = "";
	...
	while (*position != '\0' && *position != '#')
	{
		if ('*' == *position)
		{
			if (0 == strncmp(position, "*11", cmp_count))
			{
				account_id[pos_acc_id] = 'J';
			}
			else if (0 == strncmp(position, "*22", cmp_count))
			{
				account_id[pos_acc_id] = 'Y';
			}
			else if (0 == strncmp(position, "*33", cmp_count))
			{
				account_id[pos_acc_id] = 'L';
			}
			else if (0 == strncmp(position, "*44", cmp_count))
			{
				account_id[pos_acc_id] = 'K';
			}
			else
			{
				return ;	
		...
		{
			account_id[pos_acc_id] = phone_id[pos_phone];
			++pos_phone;
			++pos_acc_id;
			++position;
		}
