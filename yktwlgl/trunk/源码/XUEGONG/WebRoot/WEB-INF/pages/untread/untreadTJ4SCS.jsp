...
function DoExport(){
   untreadTJSearchForm.action="<c:url value="/lf/untreadTJAction.do"/>?method=tj4Scs&doexl=1";
   untreadTJSearchForm.submit();

}
function DHTJ(){
    untreadTJSearchForm.action = "<c:url value="/lf/untreadTJAction.do"/>?method=tj4Scs";
	untreadTJSearchForm.submit();
}
function DoView(scsid,dlid,xlid,wlid){
		untreadTJSearchForm.action="equipmentAction.do?method=loadList4TJ&dlid="+dlid+"&xlid="+xlid+"&wlid="+wlid+"&scsid="+scsid;
    	untreadTJSearchForm.submit();
}
      ...
      cellpadding=1 cellspacing=0 bordercolor=#3F738B bordercolordark=#ffffff id=maintable>
          <tbody>
            ...
            <tr align="center" valign="middle" bgcolor="#F6F4F5">
              ...
              <a href="javascript:DoView('<bean:write name="tj" property="scsid" />','<bean:write name="tj" property="dlid" />','<bean:write name="tj" property="xlid" />','<bean:write name="tj" property="wlid"/>')"><bean:write name="tj" property="untreadNum" /></a>&nbsp;
              </td>
              <%if(request.getAttribute("ifLimit").equals("0")){%>
...
function DoClear(){
    ...
    document.untreadTJSearchForm.elements["search_producterId"].value="";
