...
<html>
	<head>
		...
		function DoExport(){
   untreadTJSearchForm.action="<c:url value="/lf/untreadTJAction.do"/>?method=tj4TypeName&doexl=1";
   untreadTJSearchForm.submit();

}
function DHTJ(){
    untreadTJSearchForm.action = "<c:url value="/lf/untreadTJAction.do"/>?method=tj4TypeName";
	untreadTJSearchForm.submit();
}
function DoView(dlid,xlid,wlid){
        untreadTJSearchForm.action="equipmentAction.do?method=loadList4TJ&dlid="+dlid+"&xlid="+xlid+"&wlid="+wlid;
    	untreadTJSearchForm.submit();
	...
	<body bgcolor="#FFFFFF" leftmargin="0" topmargin="0" style="padding-left:10px">
		<br>
		<html:form action="/lf/untreadTJAction" method="post">
							...
							<td height="25" align="left" valign="top">
									<table width="150" height="20" border="0" cellpadding="0" cellspacing="0">
										<tr><td width="10" bgcolor="#0066CA">&nbsp;</td>
											<td bgcolor="#0066CA" class="mediumwhite">按类型名称统计</td>
      ...
      cellpadding=1 cellspacing=0 bordercolor=#3F738B bordercolordark=#ffffff id=maintable>
          <tbody>
            <tr align="center" valign=center>
              ...
              <td valign="middle" class="tableHeader" >退货数量</td>
              <%if(request.getAttribute("ifLimit").equals("0")){%>
              <td valign="middle" class="tableHeader" >退货金额</td>        
              <%}%>
            </tr>           
            ...
            <tr align="center" valign="middle" bgcolor="#F6F4F5">
              <td align="center" valign="middle" ><bean:write name="tj" property="dlmc" />&nbsp;</td>
              <td align="center" valign="middle" >
              <input type=hidden name="a2" value="<bean:write name="tj" property="dlmc" />">
              <bean:write name="tj" property="xlmc" />&nbsp;</td>
              <td align="center" valign="middle" ><bean:write name="tj" property="wlmc"/>&nbsp;</td>
              <td align="center" valign="middle" >
              <a href="javascript:DoView('<bean:write name="tj" property="dlid" />','<bean:write name="tj" property="xlid" />','<bean:write name="tj" property="wlid"/>')"><bean:write name="tj" property="untreadNum" /></a>&nbsp;
              </td>
              <%if(request.getAttribute("ifLimit").equals("0")){%>
              <td align="center" valign="middle" ><bean:write name="tj" property="untreadPrice" />&nbsp;</td>
              <%} %>
            </tr>                       
            </logic:iterate>
            </logic:present>
            ...
            <tr height="26" align="center" valign="middle" bgcolor="#FF9999" style="font-weight:bold"> 
              ...
              <td align="center" valign="middle" >--</td>
              <td align="center" valign="middle" ><bean:write name="tjt" property="untreadNum" />&nbsp;
              </td>
              <%if(request.getAttribute("ifLimit").equals("0")){%>
              <td align="center" valign="middle" ><bean:write name="tjt" property="untreadPrice" />&nbsp;</td>
              <%} %>
            </tr>
            </logic:iterate> 
           </logic:present>              
...
<script>
function DoClear(){
    document.untreadTJSearchForm.elements["search_dlid"].value="";
    document.untreadTJSearchForm.elements["search_xlid"].value="";
    document.untreadTJSearchForm.elements["search_wlid"].value="";
    document.untreadTJSearchForm.elements["search_regTimeStart"].value="";
    document.untreadTJSearchForm.elements["search_regTimeEnd"].value="";
}
...
function DoInit(){
  var dl,xl,wl,dept,wxs,uc,ifap,j;
     ...
     var dllist = document.untreadTJSearchForm.elements["search_dlid"];
     var xllist = document.untreadTJSearchForm.elements["search_xlid"];
     var wllist = document.untreadTJSearchForm.elements["search_wlid"];
   ...
   if(document.all.maintable!=null)
      uniteTable(document.all.maintable,2);
}
...
function changeXl(sel){
   ...
   document.untreadTJSearchForm.elements["search_xlid"].length=1; //清空原小类列表

   document.untreadTJSearchForm.elements["search_wlid"].length=1; //清空原名称列表      
      ...
      for (i=0;i < datacount; i++)
      {
         ...
         if (dataset[i][2] == id)
          {   
             ...
             document.untreadTJSearchForm.elements["search_xlid"].options[document.untreadTJSearchForm.elements["search_xlid"].length]=new Option(dataset[i][1], dataset[i][0]);              
   ...
   xllist = document.untreadTJSearchForm.elements["search_xlid"];
...
function changeWl(sel){
   ...
   if(id!=""){      
      for (i=0;i < datacount2; i++)
      {
         if (dataset2[i][3] == id)
          {   
             document.untreadTJSearchForm.elements["search_wlid"].options[document.untreadTJSearchForm.elements["search_wlid"].length]=new Option(dataset2[i][1], dataset2[i][0]); 
   ...
   wllist = document.untreadTJSearchForm.elements["search_wlid"];
