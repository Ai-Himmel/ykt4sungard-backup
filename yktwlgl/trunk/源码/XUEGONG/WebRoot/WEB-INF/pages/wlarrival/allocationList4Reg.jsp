...
function deleteDtl(){
    ...
    if(ec.elements["apiItemList"].checked==true){
      if(dtlState=='<%=Constants.wlANotSubID%>'||dtlState=='<%=Constants.wlNotPassID%>'){ 
    	ec.action="<c:url value = "/lf/wlArrivalDtlAction.do"/>?method=deleteAllocationInfo";
		ec.submit();
	  }else{
	    alert('当前到货明细已经提交不能删除安置信息！！');
	  }
    }else{
...
function modify(){
    ...
    if(ec.elements["apiItemList"].checked==true){
        if(dtlState=='<%=Constants.wlANotSubID%>'||dtlState=='<%=Constants.wlNotPassID%>'){                               
    	    ec.action = "<c:url value = "/lf/allocationAction.do"/>?method=loadAllocationInfo";
     		ec.submit();
     	}else{
    	...
    	if(counte==1){
    	  ...
    	  if(dtlState=='<%=Constants.wlANotSubID%>'||dtlState=='<%=Constants.wlNotPassID%>'){
   		  ...
   		  if(dtlState=='<%=Constants.wlANotSubID%>'||dtlState=='<%=Constants.wlNotPassID%>'){
    		ec.action = "<c:url value = "/lf/allocationAction.do"/>?method=loadAllocationInfos";
     		ec.submit();
     	  }else{
     	    alert('当前到货明细已经提交不能修改！！');
     	  }
...
function modify1(){
    ...
    if(ec.elements["apiItemList"].checked==true){
        if(dtlState=='<%=Constants.wlANotSubID%>'||dtlState=='<%=Constants.wlNotPassID%>'){                               
    	    ec.action = "<c:url value = "/lf/allocationAction.do"/>?method=loadAllocationInfo&flag=1";
     		ec.submit();
     	}else{
     	    alert('当前到货明细已经提交不能修改！！');
     	}
    	...
    	if(counte==1){
    	  ...
    	  if(dtlState=='<%=Constants.wlANotSubID%>'||dtlState=='<%=Constants.wlNotPassID%>'){
   		  ...
   		  if(dtlState=='<%=Constants.wlANotSubID%>'||dtlState=='<%=Constants.wlNotPassID%>'){
    		ec.action = "<c:url value = "/lf/allocationAction.do"/>?method=loadAllocationInfos&flag=1";
     		ec.submit();
     	  }else{
     	    alert('当前到货明细已经提交不能修改！！');
     	  }
...
function back1(){
 	wlArrivalDtlBillForm.action = "<c:url value="/lf/wlArrivalDtlAction4Search.do"/>?method=listWlArrivalDtlByState&search_dtlState=<%=request.getAttribute("str")%>";
	wlArrivalDtlBillForm.submit();
}
function allocation(){
	var i = 0;
...
//	    }
			...
			wlArrivalDtlBillForm.action="<c:url value = "/lf/allocationAction.do"/>?method=loadAllocationInfo";
			wlArrivalDtlBillForm.submit();
		}
	}else{
		alert('所有货物已经安置完毕！！！');
...
//	    }
			...
			wlArrivalDtlBillForm.action="<c:url value = "/lf/allocationAction.do"/>?method=loadAllocationInfo&flag=1";
			wlArrivalDtlBillForm.submit();
		}
	}else{
		alert('所有货物已经安置完毕！！！');
	...
	<body bgcolor="#FFFFFF" leftmargin="0" topmargin="0">
      ...
      <tr>
        ...
        <td height="40" align="right" valign="middle" colspan="2">
          <input name="Submit34" type="button" class="button_nor" value="新增安置"  onClick="add1()"><input name="Submit34" type="button" class="button_nor" value="修改"  onClick="modify1()"><input name="Submit33" type="button" class="button_nor" value="删除" onClick="deleteDtl()">
         
          <input name="Submit31" type="button" class="button_nor" value="返 回" onClick="back1()">
          </c:when>
          <c:when test="${flag=='0'}">
          <td height="40" align="left" valign="middle">
          			<input name="selectall" type="button" class="button_nor" value="全 选" onClick="DoSelectall()" >   
        ...
        <td height="40" align="right" valign="middle" colspan="2">
          <input name="Submit34" type="button" class="button_nor" value="新增安置"  onClick="add()"><input name="Submit34" type="button" class="button_nor" value="修改"  onClick="modify()"><input name="Submit33" type="button" class="button_nor" value="删除" onClick="deleteDtl()">
