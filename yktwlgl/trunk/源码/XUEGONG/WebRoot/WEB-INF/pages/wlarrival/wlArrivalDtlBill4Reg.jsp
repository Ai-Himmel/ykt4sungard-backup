...
<%
String flag="0";
if(request.getParameter("flag")!=null&&!request.getParameter("flag").equals("null"))
    flag=request.getParameter("flag"); 
%>
<SCRIPT language=javascript>
function doAddWl(){
window.open("<c:url value="/gyf/deptWlmcAction.do"/>?method=search4List&formName=wlArrivalDtlBillForm&addNum=1&wldl="+document.wlArrivalDtlBillForm.elements["s_dlid"].value+"&wlxl="+document.wlArrivalDtlBillForm.elements["s_xlid"].value,"newwindow","height=600,width=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=yes");
}
function baocun(elements){
    if(doCheckForm(elements)!= false){
   
        var tag = '<%=request.getAttribute("tag")%>';
	    	wlArrivalDtlBillForm.action = "<c:url value="/lf/wlArrivalDtlAction.do"/>?method=saveWlArrivalDtl&tag1="+tag+"&flag="+<%=flag%>;
...
function back(){
    wlArrivalDtlBillForm.action = "<c:url value="/lf/wlArrivalAction.do"/>?method=loadWlArrivalList4Reg";
	wlArrivalDtlBillForm.submit();
}
function dtlback(){
    wlArrivalDtlBillForm.action = "<c:url value="/lf/wlArrivalBillAction.do"/>?method=loadWlArrivalInfo4Reg&wlArrivalId="+document.wlArrivalDtlBillForm.wlArrivalId.value;
	wlArrivalDtlBillForm.submit();
}
function checkback(){
...
function doCheckForm(elements)
{
	...
	for(var i=0;i< len ;i++ )
	{
		...
		if(field.name=='wlid'){
					...
					if(field.value=='')
				    {
						msg(field.title+"请选择物料名称");
						field.select();
						return false;
				    }
		}
		if(field.name=='wlmodel'){

					if(field.value=='')
				    {
						msg(field.title+"物料型号必须输入");
						field.select();
						return false;
				    }
		}
		if(field.name=='usageCategory'){

					if(field.value=='')
				    {
						msg(field.title+"请选择用途大类");
						field.select();
						return false;
				    }
		}
		if(field.name=='ifAppendix'){

					if(field.value=='')
				    {
						msg(field.title+"请选择物料是否附件");
						field.select();
						return false;
				    }
		}
		if(field.name=='mtlSource')
				    {
						if(field.value=='')
				    {
						msg(field.title+"请选择物料来源");
						field.select();
						return false;
				    }
				    }
	    if(document.wlArrivalDtlBillForm.proPersonId.value=="")
				    {
						msg(field.title+"请选择采购经办人");
						field.select();
						return false;
				    }	 
	    if(field.name=='procurePrice'){					
					if(field.value!=''&&!isDouble(field.value))
				    {
		...
		if(field.name=='producterId'){

					if(field.value=='')
				    {
						msg(field.title+"请选择生产商");
						field.select();
						return false;
				    }
		}
		if(field.name=='providerId'){
					...
					if(field.value=='')
				    {
						msg(field.title+"请选择供应商");
						field.select();
						return false;
				    }
		}
		if(field.name=='maintainerid'){

					if(field.value=='')
				    {
						msg(field.title+"请选择维修商");
						field.select();
						return false;
				    }		
		}
	//alert("123="+document.wlArrivalDtlBillForm.arrPersonId.value);
	if(document.wlArrivalDtlBillForm.arrPersonId.value=="")
				    {
						msg(field.title+"请选择到货经办人");
						field.select();
						return false;
				    }
  ...
  <tr>
    <td align="left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        width="99%" border="1" align="center" cellpadding=1 cellspacing=0 bordercolor=#FFFFFF id=submenu1>
          <tbody>
			<html:hidden property="wlArrivalId" />
			<html:hidden property="wlArrivalDtlId" />
			<html:hidden property="purDtlId" />
			<tr align="center" valign=center>
              <td width="13%" align="right" valign="middle" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>物料名称</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3"><table><tr><td>
                ...
                <input type="hidden" name="new_wlid" value="">            
                 <input name="tempwlmc" style="input_box" maxlength="50" value="<bean:write name="wlmc" property="wlmc"/>" readonly="true" />
              	<html:select property="s_wlid" styleClass="select01" onchange="changeDW(this);changeWlid(this);">
                 <html:option value="">请选择	</html:option>	
				 <html:options collection="wllist" labelProperty="wlmc" property="id" />
                </html:select>
                </td><td><input name="addWlmc" type="button" value="查找" onClick="doAddWl()">
              	</td></tr></table>
			  </td>
              <td width="13%" align="right" valign="middle" class="tableHeader2" >物料小类</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3" colspan="3">
                <html:hidden property="xlid" styleClass="input_box"/>
                <input name="tempxlmc" style="input_box" maxlength="50" value="<bean:write name="deptXl" property="lbmc"/>" readonly="true" />
				<html:select property="s_xlid" styleClass="select01" onchange="changeWl(this)">
                 <html:option value="">请选择	</html:option>	
            ...
            <tr>
             <td width="13%" height="26" align="right" valign="middle" bgcolor="#FFFFFF" class="tableHeader2" >物料大类</td>
              ...
              <html:hidden property="dlid" styleClass="input_box"/>
               <input name="tempdlmc" style="input_box" maxlength="50" value="<bean:write name="deptDl" property="dlmc"/>" readonly="true" />
               <html:select property="s_dlid" styleClass="select01" onchange="changeXl(this)">
                 <html:option value="">请选择	</html:option>
              ...
              <td width="20%" align="left" valign="middle" class="tableHeader3" >
              <html:text property="wlsption" styleClass="input_box" /></td>
              <td width="13%" height="26" align="right" valign="middle" bgcolor="#FFFFFF" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>物料型号</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3" >
              <html:text property="wlmodel" styleClass="input_box" /></td>
             </tr>
            <tr align="center" valign=center>
            <td width="13%" height="26" align="right" valign="middle" bgcolor="#FFFFFF" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>用途大类</td>
              ...
              <html:select property="s_usageCategory" styleClass="select01" onchange="changeUsageCategory(this)">
				 ...
				 <html:options collection="uclist" labelProperty="usageCategoryName" property="id" />
                  ...
                  <input name="tempUsageCategory" style="input_box" maxlength="50" value="<bean:write name="uc" property="usageCategoryName"/>" readonly="true"/>
                  </td>
                  <td width="20">&nbsp;</td>
			  ...
			  <td width="13%" align="right" valign="middle" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>是否附件</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3" >
              
              	
              	<html:select property="ifAppendix" styleClass="select01" >
              	<html:option value="">请选择</html:option>	
              ...
              <td width="86%" align="left" valign="middle" class="tableHeader3" colspan="6">
              <html:textarea property="usage" styleClass="" rows="3" cols="80" readonly="true"/></td>
              
              </tr>
            <tr align="center" valign=center>
              <td width="13%" align="right" valign="middle" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>采购经办人</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3" >
              ...
              <html:text property="procurePrice" styleClass="input_box"/>元</td>
               <td width="13%" height="26" align="right" valign="middle" bgcolor="#FFFFFF" class="tableHeader2" >采购日期</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3" ><table border="0" cellspacing="0" cellpadding="0">
                <tr>
            ...
            <tr align="center" valign=center>
              <td width="13%" align="right" valign="middle" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>物料来源</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3" >
              ...
              <td width="13%" align="right" valign="middle" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>到货数量</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3" >
            ...
            <tr align="center" valign=center>
              <td width="13%" height="26" align="right" valign="middle" bgcolor="#FFFFFF" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>生产厂家</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3" ><table><tr><td>
              ...
              <input name="tempProducter" style="input_box" maxlength="50" value="<bean:write name="scsCompany" property="companyName"/>" readonly="true" />
            ...
            <tr align="center" valign=center>              
              <td width="13%" align="right" valign="middle" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>供应商</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3"><table><tr><td>
              ...
              <input name="tempProvider" style="input_box" maxlength="50" value="<bean:write name="gysCompany" property="companyName"/>" readonly="true"/>
			  ...
			  <td width="20%" align="left" valign="middle" class="tableHeader3" ><table border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td width="25" align="right" ><img id="selectaccountDate" src="<c:url value="/pages/components/calendar/skins/aqua/cal.gif"/>" border="0" alt="入帐日期" style="cursor:pointer"></td>
                        ...
                        <tr align="center" valign=center>
              <td width="13%" height="26" align="right" valign="middle" bgcolor="#FFFFFF" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>维修商</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3" ><table><tr><td>
              ...
              <input type="hidden" name="wxsid" value="<%=request.getParameter("wxsid")==null||request.getParameter("wxsid").equals("null")?"":request.getParameter("wxsid")%>"/>
              <input name="tempMaintainer" style="input_box" maxlength="50" value="<bean:write name="wxsCompany" property="companyName"/>" readonly="true"/>
           ...
           <script type="text/javascript">
            ...
            <tr align="center" valign=center>	
              <td width="13%" height="26" align="right" valign="middle" bgcolor="#FFFFFF" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>到货经办人</td>
              <td width="20%" align="left" valign="middle" class="tableHeader3" >
              ...
              <table><tr><td>
              <html:hidden property="acpid" styleClass="input_box"/><html:hidden property="s_deptacpid" styleClass="input_box"/>
              ...
              <input name="tempAcpPerson" style="input_box" maxlength="50" value="<bean:write name="otherperson" property="personName"/>" readonly="true"/>
            ...
            <tr align="center" valign=center>
              ...
              <td width="86%" colspan="5" align="left" valign="middle" class="tableHeader3" >
...
<c:if test="${flag==1}">
<input name="Submit2" type="button" class="button_nor" value="返 回" onClick="checkback()">
</c:if>
<c:if test="${flag==2}">
<input name="Submit2" type="button" class="button_nor" value="返 回" onClick="dtlback()">
</c:if>
</c:if>
<c:if test="${tag==1}">
<input name="Submit" type="button" class="button_nor" value="保 存" onClick="baocun(document.forms[0].elements)" >
&nbsp;
<input name="Submit2" type="button" class="button_nor" value="取 消" onClick="back1()">
</c:if>
</td>
      </tr>
      <tr>
...
function DoInit(){
  var dl,xl,wl,dept,wxs,uc,ifap,j;
	 ...
	 var dllist = document.wlArrivalDtlBillForm.elements["s_dlid"];
     var xllist = document.wlArrivalDtlBillForm.elements["s_xlid"];
     var wllist = document.wlArrivalDtlBillForm.elements["s_wlid"];
   ...
   for(var i=0;i<document.wlArrivalDtlBillForm.elements["s_dlid"].length;i++){//大类     
    
       if(document.wlArrivalDtlBillForm.elements["s_dlid"][i].value==dl){
            document.wlArrivalDtlBillForm.elements["s_dlid"][i].selected=true;   
            hasdl=true;         
       }
   }
   if(dl!='')
      changeXl(document.wlArrivalDtlBillForm.elements["s_dlid"]);
      for(var j=0;j<document.wlArrivalDtlBillForm.elements["s_xlid"].length;j++){
      if(document.wlArrivalDtlBillForm.elements["s_xlid"][j].value==xl){
	     document.wlArrivalDtlBillForm.elements["s_xlid"][j].selected=true;
	     hasxl=true;
	  }
	  }	
   if(xl!='')
        changeWl(document.wlArrivalDtlBillForm.elements["s_xlid"]);
   for(var j=0;j<document.wlArrivalDtlBillForm.elements["s_wlid"].length;j++){
      if(document.wlArrivalDtlBillForm.elements["s_wlid"][j].value==wl){
	     document.wlArrivalDtlBillForm.elements["s_wlid"][j].selected=true;
	     haswl=true;
	  }
   }
   if(wl!='')
       changeDW(document.wlArrivalDtlBillForm.elements["s_wlid"]);
   
   if(hasdl){
       document.wlArrivalDtlBillForm.elements["s_dlid"].style.display='';
       document.wlArrivalDtlBillForm.elements["tempdlmc"].style.display='none';
   }else{
       document.wlArrivalDtlBillForm.elements["s_dlid"].style.display='none';
       document.wlArrivalDtlBillForm.elements["tempdlmc"].style.display='';   
   }
   if(hasxl){
       document.wlArrivalDtlBillForm.elements["s_xlid"].style.display='';
       document.wlArrivalDtlBillForm.elements["tempxlmc"].style.display='none';
   }else{
       document.wlArrivalDtlBillForm.elements["s_xlid"].style.display='none';
       document.wlArrivalDtlBillForm.elements["tempxlmc"].style.display='';   
   }
   if(haswl){
       document.wlArrivalDtlBillForm.elements["s_wlid"].style.display='';
       document.wlArrivalDtlBillForm.elements["tempwlmc"].style.display='none';
   }else{
       document.wlArrivalDtlBillForm.elements["s_wlid"].style.display='none';
       document.wlArrivalDtlBillForm.elements["tempwlmc"].style.display='';   
   }
     
	 uc = document.wlArrivalDtlBillForm.elements["usageCategory"].value;
     var sccj = document.wlArrivalDtlBillForm.elements["producterId"].value;
     var gys = document.wlArrivalDtlBillForm.elements["providerId"].value;
     wxs = document.wlArrivalDtlBillForm.elements["maintainerid"].value;
     var acp = document.wlArrivalDtlBillForm.elements["acpid"].value;
  ...
  var hasacp=false;
   ...
   for(var j=0;j<document.wlArrivalDtlBillForm.elements["s_maintainerid"].length;j++){
      if(document.wlArrivalDtlBillForm.elements["s_maintainerid"][j].value==wxs){
   ...
   for(var j=0;j<document.wlArrivalDtlBillForm.elements["s_acpid"].length;j++){
      if(document.wlArrivalDtlBillForm.elements["s_acpid"][j].value==acp){
...
function changeXl(sel){
   ...
   document.wlArrivalDtlBillForm.elements["s_xlid"].length=1; //清空原小类列表

   document.wlArrivalDtlBillForm.elements["s_wlid"].length=1; //清空原名称列表      

   document.wlArrivalDtlBillForm.elements["wldw"].value="";//清空单位框
   ...
   if(id!="")
      ...
      for (i=0;i < datacount; i++)
      {
         ...
         if (dataset[i][2] == id)
          {   
             ...
             document.wlArrivalDtlBillForm.elements["s_xlid"].options[document.wlArrivalDtlBillForm.elements["s_xlid"].length]=new Option(dataset[i][1], dataset[i][0]);              
      ...
      {
          document.wlArrivalDtlBillForm.elements["s_wlid"].options[document.wlArrivalDtlBillForm.elements["s_wlid"].length]=new Option(dataset2[i][1], dataset2[i][0]);
        
      }
    }
   xllist = document.wlArrivalDtlBillForm.elements["s_xlid"];
}
...
function changeWl(sel){
   ...
   if(id!=""){      
      ...
      for (i=0;i < datacount; i++){
          if (dataset[i][0] == id){
             document.wlArrivalDtlBillForm.elements["s_dlid"].value=dataset[i][2];
          }
      }
    }else{
    ...
    wllist = document.wlArrivalDtlBillForm.elements["s_wlid"];
}
...
function changeDW(sel){
   ...
   if(id!=""){
      for (i=0;i < datacount2; i++)
      {
         if (dataset2[i][0] == id)
          {   
             document.wlArrivalDtlBillForm.elements["wldw"].value=dataset2[i][2];   
             document.wlArrivalDtlBillForm.elements["s_xlid"].value=dataset2[i][3];              
             for (j=0;j < datacount; j++){
                 if (dataset[j][0] == document.wlArrivalDtlBillForm.elements["s_xlid"].value){
                     document.wlArrivalDtlBillForm.elements["s_dlid"].value=dataset[j][2];
                 }
             }               
          }
      }
    }else{
       document.wlArrivalDtlBillForm.elements["wldw"].value="";
	}
}
...
function changeUpeqpNo(obj){
    ...
    if(document.all.new_upeqpNo.value==document.wlArrivalDtlBillForm.elements["equipmentInfo.equipmentNo"].value){    
...
if(document.all.ifChanged.value=='1'&&doCheckForm(wlArrivalDtlBillForm)){
	wlArrivalDtlBillForm.action="equipmentAction.do?method=update";
	wlArrivalDtlBillForm.submit();
	}
}
...
if(document.all.ifChanged.value=='1'){
	wlArrivalDtlBillForm.action="equipmentAction.do?method=batchupdate";
	wlArrivalDtlBillForm.submit();
	}
else{
    alert("您没有修改任何字段。");
...
function DoBack(){   
   wlArrivalDtlBillForm.action="equipmentAction.do?method=search4List&listNum='all'";
   wlArrivalDtlBillForm.submit();
}
</script>