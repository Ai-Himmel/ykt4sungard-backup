...
function baocun(elements){
    ...
    if(doCheckForm(elements)!= false){
    	allocationInfoForm.action = "<c:url value="/lf/allocationAction.do"/>?method=saveAllocationInfo";
		allocationInfoForm.submit();
	}
}
function back(elements){
		allocationInfoForm.action = "<c:url value="/lf/wlArrivalDtlAction.do"/>?method=loadAllocationList4Reg";
		allocationInfoForm.submit();
}
function baocun1(elements){

    if(doCheckForm(elements)!= false){
    	allocationInfoForm.action = "<c:url value="/lf/allocationAction.do"/>?method=saveAllocationInfo&flag=1";
		allocationInfoForm.submit();
	}
}
function back1(elements){
		allocationInfoForm.action = "<c:url value="/lf/wlArrivalDtlAction.do"/>?method=loadAllocationList4Reg&flag=1";
		allocationInfoForm.submit();
}
...
//
				...
				//		return false;
				  //  }
	//	}
		if(document.allocationInfoForm.rmid.value=='')
		{
		msg(field.title+"请选择房间");
		...
		if(field.name=='s_eqpstate'){

					if(field.value=='')
				    {
						msg(field.title+"请选择安置后状态！！");
						field.select();
						return false;
				    }
		}
      ...
      cellpadding=1 cellspacing=0 bordercolor=#FFFFFF       id=submenu1>
          <tbody>
            ...
            <tr align="center" valign=center>              
              ...
              <td width="13%" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>房间</td>
              <td width="37%" align="left" valign="middle" class="tableHeader3" >
              <table border="0" cellspacing="0" cellpadding="0">
                <tr align="center" valign="middle">
                  <td width="35">
                  <html:hidden property="rmid" styleClass="input_box"/>
                  <html:select property="s_rmid" styleClass="select01" onchange="changeBefore(this);changeRmid(this)">
                      <html:option value="">请选择</html:option>	
            ...
            <tr align="center" valign=center>
               <td width="13%" height="26" align="right" valign="middle" bgcolor="#FFFFFF" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>安置数量</td>
              <td width="37%" align="left" valign="middle" class="tableHeader3" >
            ...
            <tr align="center" valign=center>
              <td width="13%" class="tableHeader2" ><font color="red" style="font-weight:normal">*&nbsp;</font>安置后状态</td>
              <td width="37%" colspan="3" align="left" valign="middle" class="tableHeader3" >
              <html:hidden property="eqpstate" styleClass="input_box"/>
              <html:select property="s_eqpstate" styleClass="select01" onchange="changeEqpstate(this)">
					<html:option value="" >请选择</html:option>
					<html:option value="3_zaiyong" >在用</html:option>
					<html:option value="3_xianzhi" >闲置</html:option>
			 </html:select>			 
              </td>
            </tr>
            <tr align="center" valign=center>
...
<input name="Submit2" type="button" class="button_nor" value="返 回" onClick="javascript:back(document.forms[0].elements);"></td>
      ...
      <tr>
        <td height="40" align="right" valign="middle">
          <input name="Submit" type="button" class="button_nor" value="保 存" onClick="javascript:baocun1(document.forms[0].elements);">
&nbsp;
<input name="Submit2" type="button" class="button_nor" value="返 回" onClick="javascript:back1(document.forms[0].elements);"></td>
      </tr>
      </c:when>
      </c:choose>
      ...
      for (i=0;i < datacount2; i++)
      {
          ...
          {   
             ...
             for (j=0;j < datacount; j++){
                 if (dataset[j][0] == document.allocationInfoForm.elements["s_bdid"].value){
                     document.allocationInfoForm.elements["xqId"].value=dataset[j][2];
                 }
             }               
          }
      }
...
function changeEqpstate(sel){
    document.all.eqpstate.value=sel.value;
}
</SCRIPT>
