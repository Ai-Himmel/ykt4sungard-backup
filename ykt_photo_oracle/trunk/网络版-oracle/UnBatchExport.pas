...
begin
  ...
  ADOQuery2.Close;
  ADOQuery2.SQL.Clear;
  ADOQuery2.SQL.Add('select distinct in_date from t_cif_photo');
  ADOQuery2.Prepared;
  ADOQuery2.Open;
  if ADOQuery2.RecordCount>0 then
  begin
    ...
    begin
      cbbgrade.Items.Add(ADOQuery2.fieldbyname('in_date').AsString);
      ADOQuery2.Next;
    end;
  end;
  ProgressBar1.Position := 0;
...
begin
  ...
  if trim(edtName.Text)<>'' then
    currsql:=currsql+' and '+StuempName+'='+#39+trim(edtName.Text)+#39;
  if trim(cbbgrade.Text)<>'' then
    currsql:=currsql+' and '+InDate+'='+#39+trim(cbbgrade.Text)+#39;
