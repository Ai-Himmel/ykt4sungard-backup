...
var
  ...
  sDelPic, sDelData, sImpPic, sImpData, sMakeCard,scard,spersonAdd: string;
...
begin
  ...
  spersonAdd := Copy(ADOTable1.FieldByName('FUNC_SET').AsString,mdl_PersonAdd,1);
  ...
  if spersonAdd='1' then
    chkPersonAdd.Checked:=True
  else
    chkPersonAdd.Checked:=False;
...
var
  sQuery,sEditData,sLoadPic,sExpData,sPrint,sExpPic,
  sDelData, sImpPic, sImpData, sMakeCard,sCard,spersonAdd: string;
...
begin
  ...
  if chkMakeCard.Checked=True then //制卡
    ...
    sCard:='0';
  if chkPersonAdd.Checked=True then
    spersonAdd:='1'
  else
    spersonAdd:='0';
  ...
  slimit:=sQuery+sEditData+sLoadPic+sExpData+sPrint+sExpPic
      + sDelData + sImpPic + sImpData + sMakeCard+scard+spersonAdd;
end;
