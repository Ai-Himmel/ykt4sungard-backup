...
var
  ...
  var
    ...
    I: Integer;
  begin
    SQL := 'insert into PHOTO.T_CIF_PHOTO (';
    for I := 0 to CIF_Fields_Count - 1 do
  ...
  begin  
    FPhotoSelect.Sql.Text := 'select STUEMP_NO from PHOTO.T_CIF_PHOTO where STUEMP_NO = ' +
        '''' + Stuemp_no + '''';
    FPhotoSelect.Open;
...
begin
  ...
  Result := True;  
  BeforeImport;
  frmProgress.BeginHandle('导入教职工基本信息! ', 0, FImportTable.RecordCount);
...
var
  I: Integer;
begin
  CIF_Fields[0].EnName := 'CUT_TYPE';
  CIF_Fields[0].ChName := '类别';
  CIF_Fields[1].EnName := 'STUEMP_NO';
  CIF_Fields[1].ChName := '学号_工号';
  CIF_Fields[2].EnName := 'STUEMP_NAME';
  CIF_Fields[2].ChName := '姓名';
  CIF_Fields[3].EnName := 'SEX_NAME';
  CIF_Fields[3].ChName := '性别';
  CIF_Fields[4].EnName := 'DEPT_NAME';
  CIF_Fields[4].ChName := '一级单位';
  CIF_Fields[5].EnName := 'S_CODE';
  CIF_Fields[5].ChName := '二级单位';
  CIF_Fields[6].EnName := 'ID2';
  CIF_Fields[6].ChName := '身份证号码';
  CIF_Fields[7].EnName := 'IN_DATE';
