...
KSSHMT_API bool INITSMTEX(const char *SMTName, int tablecnt, T_TABLEINFO *tableinfo, char *eMsg);
...
// 参数  : int idxid [IN]: 已经创建的索引号
// 参数  : void *key [IN]: 用于比较的关键字数据
// 参数  : int cmp_oper，参考SHTCMP_TYPES中的清单
// 参数  : char *eMsg [OUT]: 当错误的时候，用于返回错误信息，应该定义 char eMsg[256];
KSSHMT_API CURSORHANDLE SMTCursorIndexOpen(SMTABLEHANDLE SMTHandle, int idxid, void *key, int cmp_oper, char *eMsg);


// Function name	  : SMTCursorIndexOpen2
// Author	        : CHENYH 2007-7-13 9:13:18
// Description	     : 这是根据索引在一个关键字区间检索，数据记录区间以游标方式返回
// Return type		  : CURSORHANDLE ：返回该查询的游标句柄，==NULL:失败
// Argument         : SMTABLEHANDLE SMTHandle [IN]: 已经打开的共享表句柄 
// Argument         : int idxid: 已经创建的索引号
// Argument         : void *key_l [IN]: 用于比较的关键字小的数据
// Argument         : void *key_b [IN]: 用于比较的关键字大的数据
// Argument         : int cmp_oper [IN]: 比较的类型，注意和SHTCMP_TYPES不同：
//							0:key_l<=key(record)<=key_b; 1:key_l<=key<key_b; 2:key_l<key<=key_b; 3:key_l<key<key_b
// Argument         : char *eMsg [OUT]: 当错误的时候，用于返回错误信息，应该定义 char eMsg[256];
KSSHMT_API CURSORHANDLE SMTCursorIndexOpen2(SMTABLEHANDLE SMTHandle, int idxid, void *key_l,void *key_b, int cmp_oper, char *eMsg);
...
// 描述  : 在游标中向上移动一个游标位置指针
// 返回  : bool ：，true：表示移动前的位置不是首个位置；false：表示可能已经是首个位置，也可能是其他错误
// 参数  : SMTABLEHANDLE SMTHandle [IN]: 已经打开的共享表句柄 
// 参数  : CURSORHANDLE csrhandle [IN]: 指定的查询游标句柄
// 参数  : char *eMsg [OUT]: 当错误的时候，用于返回错误信息，应该定义 char eMsg[256];
...
// 函数名: SMTCursorGo
// 编程  : 陈永华 2007-9-3 11:00:37
// 描述  : 在游标中向上移动到一个指定的数据单元
// 返回  : bool 返回是否成功移动。true-成功移动到指定的位置，false-参见错误信息eMsg
// 参数  : SMTABLEHANDLE SMTHandle [IN]: 已经打开的共享表句柄 
// 参数  : CURSORHANDLE csrhandle [IN]: 指定的查询游标句柄
// 参数  : unsigned int dataunit [IN]: 指定的游标数据单元号，有效的值为0 -- SMTCursorSize(...)的返回值-1
// 参数  : char *eMsg [OUT]: 当错误的时候，用于返回错误信息，应该定义 char eMsg[256];
KSSHMT_API bool SMTCursorGo(SMTABLEHANDLE SMTHandle, CURSORHANDLE csrhandle, unsigned int dataunit, char *eMsg);
...
// Function name	: SMTDump
// Author	    : CHENYH 2007-7-13 10:19:03
// Description	    : 将指定的内存表记录数据DUMP到一个文件中
// Return type		: int <0: fail; 否则即为保存的记录数
// Argument         : SMTABLEHANDLE SMTHandle [IN]: 被指定的已经打开了的共享表句柄 
// Argument         : const char *file [IN]: 内存表保存的文件名
// Argument         : char *eMsg [OUT]: 当错误的时候，用于返回错误信息，应该定义 char eMsg[256];
KSSHMT_API int SMTDump(SMTABLEHANDLE SMTHandle,const char *file, char *eMsg);


// Function name	: SMTRestore
// Author	    : CHENYH 2007-7-13 10:28:06
// Description	    : 将DUMP文件中的记录恢复(插入)到内存表中
// Return type		: int <0: 失败，参考eMsg信息；否则为恢复的记录数
// Argument         : SMTABLEHANDLE SMTHandle [IN]: 被指定的已经打开了的共享表句柄 
// Argument         : const char *file [IN]: 内存表保存的文件名
// Argument         : char *eMsg [OUT]: 当错误的时候，用于返回错误信息，应该定义 char eMsg[256];
KSSHMT_API int SMTRestore(SMTABLEHANDLE SMTHandle,const char *file, char *eMsg);
