...
#endif
  {
    //modified by 04211 for chinese
    //sprintf(tmp, "&#%lu;", c);
    sprintf(tmp, "&#x%X;", c);//modifyed by 04211
  }
...
{ register const char *t;
  ...
  { switch (c)
    ...
    default:
      ...
      if (soap->mode & SOAP_C_MBSTRING || c < 0)
      { wchar_t wc;
#ifdef WIN32 //added by 04211 for chinese
		register int m = 2;
		MultiByteToWideChar( CP_ACP, 0, t-1, 3, &wc, 1);
#else
  ...
  { 
    ...
    { if (m > 0)
      ...
      default:
		...
		//modified by 04211 for chinese
		//if (soap->mode & SOAP_C_MBSTRING)
		if (soap->mode & SOAP_C_MBSTRING || c < 0) //for the ASCII And Unicode 
        { 		  
#ifdef WIN32 //added by 04211 for chinese
		  wchar_t code[1];
		  code[0] = c & 0x7FFFFFFF;
		  WideCharToMultiByte( CP_ACP, 0, code, 1, buf, 3, NULL, NULL );
		  m = 2;
		  t = buf;
          *s++ = *t++;
          m--;
