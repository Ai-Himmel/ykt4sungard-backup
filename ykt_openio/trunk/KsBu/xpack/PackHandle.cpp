...
int CPackHandle::CheckEncodeHead(const void *buf, int blen,ST_PACKHEAD *ph)
{
   if (blen<(int)sizeof(ST_PACKHEAD))
      return(-1);
   memcpy(ph,buf,sizeof(ST_PACKHEAD));
   ...
   if (ph->recCount>MAXPACKROWS)
      return(-2);
   if (ph->firstflag>1||ph->nextflag>1)
      return(-3);

   return(CheckParmBits(ph->ParmBits));
}

int CPackHandle::CheckParmBits(unsigned char *parmbits)
{
   int mbyte;
   unsigned char bmask;
   int ifd=0;
   ...
   for (mbyte=0;mbyte<PARMBYTES;mbyte++)
   {
      for (bmask=0x01;bmask!=0x00;bmask<<=1,ifd++)
      {
         if (parmbits[mbyte]&bmask)
         {
            if (ifd>iLastPField)
               return(-10);
            switch (pdlist[ifd].wType)
            {
            case PDL_CHAR:          // 普通字符串模式，约定长度，最长不超过255
            case PDL_VCHAR:         // 变长字符，最长不超过255
            case PDL_UCHAR:         // 用于存放二进制数据的字段，比如权限等
            case PDL_INT:           // 整数字段，（由于在64位机中，long属于64位，而在32位机中则为32位，对于int则是固定的，因此本协议中不再采用long）
            case PDL_DOUBLE:         // double类型字段
               n++;
               break;
            default:
               return(-11);
...
int CPackHandle::CheckEncodeRows(const void *ebuf, int eblen, const ST_PACKHEAD *phead)
{
   int i;
   unsigned char *pc = (unsigned char *)ebuf;
   int elen = 0; // real data length
   int mbyte;
   unsigned char bmask;

   for (i=0;i<phead->recCount;i++)
   {
      int ipd=0;
      for (mbyte=0;mbyte<PARMBYTES;mbyte++)
      {
         for (bmask=0x01;bmask!=0x00;bmask<<=1,ipd++)
         {
            if (ipd>iLastPField)
            {
               mbyte = PARMBYTES;
            ...
            {
               int flen;
               switch (pdlist[ipd].wType)
               {
               case PDL_CHAR:
               case PDL_VCHAR:
                  if (iMode==0)
                  {
                     if (pdlist[ipd].wType==PDL_CHAR)
                     {
                        flen = pdlist[ipd].nLength;
                     }
                     else
                     {
                        short vsl;
                        memcpy(&vsl,pc+elen,sizeof(vsl));
                        HostByteOrder(&vsl,sizeof(vsl));
                        if (vsl>255)
                           return(-21);  // invalid at a vchar field
                        flen = vsl+sizeof(vsl);
                     }
                  }
                  else
               ...
               case PDL_DOUBLE:
                  ...
                  break;
               default:
                  return(-11);  // invalid field mask
               }
               elen += flen;
               if (elen>eblen)
                  return(-20); // short of eblen
            }
         }
      }
   }
