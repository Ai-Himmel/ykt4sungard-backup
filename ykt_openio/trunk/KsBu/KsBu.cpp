...
int CallBDFunc(int fno,TRUSERID *handle,ST_CPACK *rpack,ST_PACK *pArrays,int *iRetCode, char *szMsg)
{
	...
	*szMsg=0;
	...
	//g_pSvrLink->SetAnswerMode(AM_AUTO);
	switch(sendtype)
	{
	case 93:
		rtn = (*pFunc)(handle,rpack->head.RequestType,&rpack->pack,iRetCode,szMsg);
...
int main(int argc, char* argv[])
{
	...
	g_LogFile.WriteLogEx(1002, "传入参数:[%s],[%s]", argv[0],argv[1]);
#ifdef _DYNAMIC_LOAD	
//	char dll_name[] = "SIMPLEPLUGIN.dll";
	// 加载对接第三方动态库
	g_LogFile.WriteLogEx(1002, "传入参数:[%s]", argv[1]);
	if (rtn = g_LoadDll.LoadDataInfo(argv[1]))
	{
		g_LogFile.WriteLogEx(1002, "加载接口库失败--错误码:[%d]", rtn);
		return rtn;
	}

	g_LogFile.WriteLogEx(1002, "加载接口成功");
#endif
	// 初始化与BCC连接：
	do 
	{
	...
	//	char dll_name[] = "SIMPLEPLUGIN.dll";
	// 加载对接第三方动态库
	if (rtn = g_LoadDll.Init())
	{
		g_LogFile.WriteLogEx(1002, "初始化接口库失败--错误码:[%d]", rtn);
		return rtn;
	}
#endif
