...
/*
	if (ret = InitDB())
	{
//		itoa(ret, test_msg, 10);
//		::MessageBox(NULL, test_msg, "错误码", NULL);
//		g_LogFile.WriteLogEx(1002, "初始化第三方数据库失败--错误码:[%d]", ret);
		out_pack->lvol10 = 0;
...
*/
#ifdef _DEBUG
	g_LogFile.WriteLogEx(1002, "加载第三方数据库成功");
#endif

	GetDataInfo(&data_info, &error_code);
	/*
	itoa(error_code, test_msg, 10);
	::MessageBox(NULL, test_msg, "错误码", NULL);
	*/
	if (error_code == 0)
		out_pack->lvol10 = 10;
