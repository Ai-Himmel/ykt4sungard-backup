...
function calc_oper_pwd(oper_code: ansistring; pwd: ansistring; text: pansichar): longint; stdcall; far; external 'desdll.dll' name 'calc_oper_pwd';
...
function SMT_CloseMF280: longint; stdcall; external 'KS_ThirdInterface.dll' name 'SMT_CloseMF280';
  //加密函数
function encOperPwd(operCode, operPwd: AnsiString): ansistring;
...
begin
  ...
  try
    ...
    on e: exception do
    begin
      writelog('连接错误:' + e.Message+'连接：'+ORA_CONNECT_STRING);
    //  FuncShowMessage(Application.Handle, '连接错误:' + e.Message, 2);
      exit;
    end;
  end;
...
function encOperPwd(operCode, operPwd: AnsiString): ansistring;
var
  outOperPwd: array[0..16] of AnsiChar;
  st: Integer;
begin
  st := calc_oper_pwd(operCode, operPwd, @outOperPwd);
  if st <> 0 then
  begin
...
begin
  ...
  begin
    ...
    try
      input := String(encOperPwd(ansiString(trim(self.UserName)), ansistring(trim(Self.Password))));
    except
    end;
