...
uses
    SysUtils, Classes, Math,EncdDecd;
...
function HexToStr(Value: string): string;
        ...
        function DecryptString(Value: ansistring; Key: ansistring; KeyBit: TKeyBit = kb128): ansistring;
        function EncryptStream(Stream: TStream; Key: string; KeyBit: TKeyBit = kb128): TStream;
        function DecryptStream(Stream: TStream; Key: string; KeyBit: TKeyBit = kb128): TStream;
        procedure EncryptFile(SourceFile, DestFile: string; Key: string; KeyBit: TKeyBit = kb128);
        procedure DecryptFile(SourceFile, DestFile: string; Key: string; KeyBit: TKeyBit = kb128);

        { --  字符串加密函数 -- }
        function Elah(Value: string; Key: string; KeyBit: TKeyBit = kb128): string;

        { --  字符串解密函数  -- }
        function Dlah(Value: string; Key: string; KeyBit: TKeyBit = kb128): string;
        ...
        begin
            ...
            try
                ...
                begin
                    FillChar(AESKey192, SizeOf(AESKey192), 0);
                    Move(PChar(Key)^, AESKey192, Min(SizeOf(AESKey192), Length(Key)));
                    DecryptAESStreamECB(Stream, Stream.Size - Stream.Position, AESKey192, OutStrm);
                end;
                { --  256 位密匙最大长度为 32 个字符 -- }
                if KeyBit = kb256 then
                begin
                    FillChar(AESKey256, SizeOf(AESKey256), 0);
                    Move(PChar(Key)^, AESKey256, Min(SizeOf(AESKey256), Length(Key)));
                    DecryptAESStreamECB(Stream, Stream.Size - Stream.Position, AESKey256, OutStrm);
                end;
