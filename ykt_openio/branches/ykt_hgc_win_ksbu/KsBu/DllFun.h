...
#define LOAD_DLLFUNC(handle,name) \
	if (name) { NULL; }else { name = (LP##name)GetProcAddress(handle,#name); \
	if (NULL == name) \
	{ \
		::FreeLibrary(handle); \
		handle = NULL; \
...
// desdll
DCLR_DLLFUNC(int,encrypt_work_key_asc,(char src_text[17], char des_text[17]));
DCLR_DLLFUNC(int,decrypt_work_key_asc,(char src_text[17], char des_text[17]));
