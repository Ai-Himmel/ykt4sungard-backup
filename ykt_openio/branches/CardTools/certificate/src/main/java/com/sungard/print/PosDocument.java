...
public class PosDocument extends AbstractDocument {
    ...
    public void configPage(Graphics2D graphics2D, PageFormat pageFormat, int pageIndex) {
        ...
        //创建字体
        int fontSize = Configuration.getIntProperty(Constant.FONT_SIZE, 16);
        String fontName = Configuration.getProperty(Constant.FONT_NAME);
    ...
    public void drawCurrentPage(Graphics2D graphics2D, PageFormat pageFormat, int pageIndex) {
        ...
        int startRow = Configuration.getIntProperty(Constant.START_ROW, 1) * fontSize;
        int startColumn = (Configuration.getIntProperty(Constant.START_COLUMN, 1) - 1) * fontSize;
