...
public class KsCardInterfaceUtil {
    ...
    public static boolean ksRequestCard(Properties result) {
        return ksRequestCard(result, true);
    }
     ...
     *
     * @param result    .
     * @param showError 是否记录寻卡失败日志，showError=false时适用于某些场景，比如连续不断的自动寻卡，此时寻卡失败是由于用户未放置卡，无须连续的显示错误信息
     * @return .
     */
    public static boolean ksRequestCard(Properties result, boolean showError) {
        boolean success = false;
     ...
     * @return .
     */
    public static boolean ksPayCancelProcess(String refNo, String mac, String cardPhyId, int cardno, int cardBal, int dpsCnt,Properties result) {
        boolean success = false;
        Pointer pRefNo = null;
        Pointer pMac = null;
        Pointer pCardPhyId = null;
