    ...
    public static bccclt getBccctlInstance() {
        if (null == bcc) {
            ...
            if (StringUtils.isNotBlank(drtpip1)) {
                drtpno1 = bcc.AddDrtpNode(drtpip1.getBytes(), Configuration.getIntProperty(Constant.DRTPPORT1, 4000));
            }
            if (StringUtils.isNotBlank(drtpip2)) {
                drtpno2 = bcc.AddDrtpNode(drtpip2.getBytes(), Configuration.getIntProperty(Constant.DRTPPORT2, 4000));
            }
            return bcc;
        } else {
            return bcc;
