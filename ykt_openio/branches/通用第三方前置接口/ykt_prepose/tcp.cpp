...
#endif
int CTcpSocket::ConnectTcp(char *host, short port,char emsg[256])
{
   unsigned int inaddr;  
   struct hostent *ht;
   ...
   if (e<0) 
   {
      //char emsg[256];
		...
		if(e == WSAEWOULDBLOCK)
		{
			...
			int timeout = 3000;				//等待3秒
			tv.tv_sec = timeout/1000;
			tv.tv_usec = (timeout%1000)*1000;
			e =(select(m_socket+1, NULL,&rset, NULL, &tv));
			if (e<=0)
			{
				if(e ==0)  //超时
					strcpy(emsg,"连接服务器端超时");
				else
					e = GetLastError(emsg);
				Close();
				return(-3);
