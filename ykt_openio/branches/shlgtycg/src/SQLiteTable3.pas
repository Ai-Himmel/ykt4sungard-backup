...
begin
    for I := 0 to High(Bindings) do begin
        case Bindings[I].VType of
            ...
            vtPointer: begin
                    if (Bindings[I].VPointer = nil) then begin
            ...
            vtObject: begin
                    if (Bindings[I].VObject is TCustomMemoryStream) then begin
                        ...
                        if (sqlite3_bind_blob(Stmt, I + 1, @PAnsiChar(BlobMemStream.Memory)[BlobMemStream.Position],
                            BlobMemStream.Size - BlobMemStream.Position, SQLITE_STATIC) <> SQLITE_OK) then begin
                            RaiseError('Could not bind BLOB', 'BindData');
                        end;
                    end
                    else if (Bindings[I].VObject is TStream) then begin
...
begin
    if not self.fInTrans then begin
...
begin
    ...
    try
        ...
        iStepResult := Sqlite3_step(Stmt);
        while (iStepResult <> SQLITE_DONE) do begin
            case iStepResult of
                SQLITE_ROW: begin
                        Inc(fRowCount);
...
var
    ...
    iColNo: integer;
begin
    if Assigned(fResults) then begin
...
begin
    Result := False;
    if self.fRowCount > 0 then begin
        fRow := 0;
        Result := True;
    end;
end;
...
begin
    Result := False;
    if (self.fRowCount > 0) and (self.fRowCount > position) then begin
        fRow := position;
        Result := True;
    end;
end;
