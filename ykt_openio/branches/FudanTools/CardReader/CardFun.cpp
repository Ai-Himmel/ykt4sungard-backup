...
int CCardFunction::ReadCardInformation()
{
	...
	*/
	...
	if(CardNo == pc.ucCardNo && UserName == pc.ucName && CurTicks - LastTickCount < 10000) // 同一卡片3秒钟内读取忽略
	{
		return -2;
	}
...
//	str_card_info.Format("%s", show_card_id);
	...
	if (ReadCardPwd(card_pwd))
		return err_exce.SearchErr(KS_ERR_READCARDINFO);

	str_card_info.Format("%s", card_pwd);
...
int CCardFunction::ModifyTradeCount()
{
	int ret;
	int cnt = AfxGetMainWnd()->GetDlgItemInt(IDC_EDIT_CNT);
	ret = SMT_SetTxCnt(1,cnt);
	if(ret) { return err_exce.SearchErr(KS_ERR_WRTIEDEADLINEDATE); }
	ControlBuzzer();
	return 0;
}

int CCardFunction::ModifyPwd()
{
	CString pwd;
	int ret;
	AfxGetMainWnd()->GetDlgItemText(IDC_EDIT_PWD,pwd);
	if(pwd.GetLength() != 6)
	{
		AfxMessageBox("密码长度不正确!");
		return -1;
	}
	ret = WriteCardPwd((unsigned char*)(LPCTSTR)pwd);
	if(ret) { return err_exce.SearchErr(KS_ERR_WRTIEDEADLINEDATE); }
	ControlBuzzer();
	return 0;
}

int CCardFunction::ResetPacket()
{
	int ret;
	ret =SMT_PacketReset(1);
	if(ret)
	{
		return err_exce.SearchErr(KS_ERR_WRTIEDEADLINEDATE);
	}
	ControlBuzzer();
	return 0;
}

int CCardFunction::RepairPurse()
{
	int ret;
	ret = SMT_ClearCard();
	if(ret)
