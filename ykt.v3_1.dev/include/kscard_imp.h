...
#define CF_CARDSTRUCTVER	13		//卡结构版本（应用版本）
// 东莞新莞人卡
#define CF_CITYCODE         14      //城市代码
#define CF_INDUSTRYCODE     15      //行业代码
#define CF_INTERACTFLAG     16      //互联互通标识
#define CF_SUBFEETYPE       17      //卡子类别
#define CF_APPSERIALNO      18      //应用序列号
//持卡人信息
...
#define CF_LOCKTERMID       34      //锁卡交易时终端机号
#define CF_LOCKTRANSDATE    35      //锁卡交易日期
#define CF_LOCKTRANSTIME    36      //锁卡交易时间

#define CF_DAYSUMAMT		37		//当日累计交易金额
#define CF_LOCKCARDCNT		38		//锁卡交易时交易次数
//交易参数
#define CF_SINGLEMAXAMT		41		//单次消费限额
#define CF_DAYMAXAMT		42		//日消费累计限额
//交易明细
#define CF_TRANSDTL_BEGIN	44
#define CF_CARDCNT			44		//卡交易次数(最后一次交易)
#define CF_TRANSAMT			45      //交易金额(最后一次交易)
#define CF_TRANSTYPE		46		//交易类型(最后一次交易)
#define CF_TERMID			47      //交易终端机编号(最后一次交易)
#define CF_TRANSDATE		48		//交易日期(最后一次交易)
#define CF_TRANSTIME		49		//交易时间(最后一次交易)
...
///////////////////////////// 广州大学城相关
#define CF_MARCHANTCARD     70 //商户卡标志
#define CF_MARCHANTNO		71 //商户号
#define CF_DUTYNO			72 //职务代码
#define CF_IDCODE			73 //身份代码
#define CF_NATIONAL			74 //民族
#define CF_CITIZENSHIP		75 //国籍
#define CF_OPENCARDFLAG     76 //是否已开卡标志
#define CF_ISSUESVRNO		77 //服务注册号

#define CF_NORMALLYOPEN     80      //常开卡标志

// 新开普读卡器
#define CF_BATCHNO			81		// 卡批次号
#define CF_BATCHSEQNO		82		// 卡批次序号
#define CF_SUBSIDYBAL		83		// 补助钱包
#define CF_TOTALAMT			84		// 总额
#define CF_TODAY			85		// 本日
#define CF_MEALAMT			86		// 餐消费额累计
#define CF_LIBNO			87		// 图书证条码
...
typedef struct {
	...
	char	sVoucherNo[24];	//充值票据号
	char	sCardPwd[7];	//卡密码
	int     nTransType;		//交易类型 0-充值 1扣款
	int     nPayCode;		//支付代码
	int		nPayType;		//支付类型,消费类使用
	///////////////以下为输出值//////////////////////////////
	char	sRefNo[15];		//交易参考号
	char 	cRetryFlag;		//重试标志,如果重试标志不为0，则进行重试
	char    cGetProveFlag;	//需要取交易确认标志
	char    sMac[9];		//MAC
...
//
//新开普操作卡初始化
int __stdcall ks_publishopercard(const char * BatchNo, const char * BatchSeqNo, const char *ExpDate, const char *OperAcc, const char *OperName, const char *OperPwd, const char *StationNo);
...
unsigned char SAMID --- 设置要操作的卡座号,1、 2、 3 各为SAM1(大卡座) SAM2 SAM3 
...
//当出现中途拔卡时，而用户没有重新放卡，需要调用本函数取交易认证
//transflag1-充值 2消费 cardcnt 交易前消费或充值次数
...
// 充值2
int __stdcall ks_load4card_retry(ST_TRANSPACK *pack);
int __stdcall ks_load4card(ST_TRANSPACK *pack);
//////////////////////////////////////////////////////////////////////////
// 深圳凯路相关功能
#pragma pack(1)
typedef struct {
