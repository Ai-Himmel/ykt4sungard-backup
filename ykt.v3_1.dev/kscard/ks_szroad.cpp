...
int __stdcall ks_sr_readmoney(sr_water_pack_t *pack) {
    ...
    memset(buffer,0,sizeof buffer);
    blockno++;
    ret = g_Reader1.read_block(&g_Reader1Para,&g_cardinfo,blockno,buffer);
    if(ret) {
    ...
    if(check_buffer_sum(buffer,15,buffer[15])) {
        // 读备份钱包
        ret = g_Reader1.read_block(&g_Reader1Para,&g_cardinfo,blockno+1,buffer);
        if(ret) {
...
int __stdcall ks_sr_setmoeny(int money,int cnt) {
    ...
    ret = g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,blockno,buffer);
    if(!ret) {
        ret = g_Reader1.read_block(&g_Reader1Para,&g_cardinfo,blockno,buffer2);
        if(ret) {
            return KS_UNCONFIRM;
        }
        if(memcmp(buffer,buffer2,16)!=0)
            return KS_WRITECARD;
        g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,blockno+1,buffer);
    } else
        return KS_WRITECARD;
