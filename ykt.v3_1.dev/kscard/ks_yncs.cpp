...
static int hd_water_readmoney(ks_water_pack_t *pack) {
    ...
    if(g_Reader1.read_block(&g_Reader1Para,&g_cardinfo,blockno,buffer)) {
        return KS_READCARD;
    }
    if(check_buffer_or(buffer,15,buffer[15])) {
        if(g_Reader1.read_block(&g_Reader1Para,&g_cardinfo,blockno+1,buffer)) {
            return KS_READCARD;
        }
        if(check_buffer_or(buffer,15,buffer[15])) {
    ...
    if(g_Reader1.read_block(&g_Reader1Para,&g_cardinfo,blockno+2,buffer2)) {
        return KS_READCARD;
    }
    if(check_buffer_or(buffer2,15,buffer2[15])) {
        return KS_PURSEERROR;
    }
...
static int hd_water_writemoney(ks_water_pack_t *pack) {
    ...
    buffer[15] = calc_or(buffer,15);

    ret = g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,blockno,buffer);
    if(!g_Reader1.read_block(&g_Reader1Para,&g_cardinfo,blockno,buffer2)) {
    ...
    buffer[15] = calc_or(buffer,15);
    ret = g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,blockno+2,buffer);
    if(!g_Reader1.read_block(&g_Reader1Para,&g_cardinfo,blockno+2,buffer2)) {
        if(memcmp(buffer,buffer2,16)!=0)
            return KS_WRITECARD;
    } else
...
static int hd_water_recyclecard() {
    ...
    if(g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,blockno,buffer)) {
        return KS_WRITECARD;
    }
    if(g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,blockno+1,buffer)) {
        return KS_WRITECARD;
    }
    if(g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,blockno+2,buffer)) {
        return KS_WRITECARD;
    }
    return 0;
}
...
static int xt_water_writemoney(ks_water_pack_t *pack) {
    ...
    g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,blockno,buffer);
    if(g_Reader1.read_block(&g_Reader1Para,&g_cardinfo,blockno,buffer2)) {
        return KS_UNCONFIRM;
    }
    if(memcmp(buffer,buffer2,16)!=0)
...
static int do_format_xt_paramcard(ks_water_param_t *param) {
    ...
    for(i = 1; i < 15; ++i) {
        ret = g_Reader1.login_card(&g_Reader1Para,&g_cardinfo,i,MF_KEYA,orgin_keya);
        if(ret) {
            ret = ks_mf_card(phyid);
            if(ret)
                return KS_REQUESTCARD;
            ret = g_Reader1.login_card(&g_Reader1Para,&g_cardinfo,i,MF_KEYA,new_keya);
            if(ret) {
...
static int do_xt_water_paramcard(ks_water_param_t *param) {
    ...
    for (i = 1; i < 16; ++i) {
        ...
        if(ret) {
            ...
            ret = g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,block_no+3,card_key);
            if(ret) {
        ...
        // 写设置卡参数
        ret = g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,block_no,data);
        if(ret) {
            DEBUG_LOG(("写新天参数卡扇区[%d]失败",i));
            return KS_WRITECARD;
        }
        ret = g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,block_no+1,data1);
        if(ret) {
            DEBUG_LOG(("写新天参数卡扇区[%d]失败",i));
            return KS_WRITECARD;
        }
...
static int do_xt_water_initcard(ks_water_param_t *param) {
    ...
    for (i = 1; i < 16; ++i) {
        ...
        ret = g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,block_no+1,data);
        if(ret) {
            DEBUG_LOG(("写新天参数卡扇区[%d]失败",i));
            return KS_WRITECARD;
        }
        ret = g_Reader1.write_block(&g_Reader1Para,&g_cardinfo,block_no+2,data);
        if(ret) {
            DEBUG_LOG(("写新天参数卡扇区[%d]失败",i));
            return KS_WRITECARD;
        }
