...
static int do_init_log() {
    ...
    // 取kscard.dll的目录作为配置文件的目录
    HMODULE hDll = reinterpret_cast <HMODULE> (&__ImageBase);
    ::GetModuleFileName(hDll,path,1024);
...
int __stdcall ks_openport(int port,int band) {
    ...
    if(g_drtpip.length() > 0 && g_portno.length() > 0 && g_funcno.length() > 0) {
        int comm_ret = ks_comminit(g_drtpip.c_str(), atoi(g_portno.c_str()), atoi(g_funcno.c_str()));
        if(comm_ret != 0) {
            TRACE_LOG("连接后台服务失败");
        } else {
            TRACE_LOG("连接后台服务成功");
...
int __stdcall ks_readpsamtermno(char *termno,int need_request) {
    ...
    if(ret) {
        ks_setlasterr(c->GetLastErrMsg(NULL),ret);
        TRACE2_LOG(("读PSAM Termno失败,错误码[%d]错误信息[%s]",ret,c->GetLastErrMsg(NULL)));

        return ret;
    }
    TRACE2_LOG(("读PSAM Termno[%s]",termno));
    return 0;
}
//读PSAM卡终端交易流水号
...
int __stdcall ks_login(int termid,int& shopid,char shopname[61]) {
    ...
    */
    ...
    if(g_termno != "") {
        sprintf(sTermno, "%s", g_termno.c_str()); // 优先使用license.lcs 文件中的
    } else {
...
int __stdcall ks_load4card(ST_TRANSPACK *pack) {
    TRACE_FUNC("ks_load4card");
    ks_cardtype_base *c;
    GET_CARDTYPE(c);
    CardOperator oper(c);
    int ret;
    ret = oper.Deposite(*pack);
    if(ret) {
        return -1;
    }
    return 0;
...
int __stdcall ks_load4card_retry(ST_TRANSPACK *pack) {
    TRACE_FUNC("ks_load4card_retry");
    ks_cardtype_base *c;
    GET_CARDTYPE(c);
    CardOperator oper(c);
    unsigned char sMac[9]= {0};
    unsigned char sTac[9]= {0};
    int ret = c->GetTransProve(0,pack->nDpsCardCnt,sMac,sTac);
    if(ret) {
        //
        ret = oper.DepositeRetry(*pack);
        if(ret) {
            // 重试失败
            return -1;
        }
...
int __stdcall ks_publishopercard(const char * BatchNo, const char * BatchSeqNo, const char *ExpDate, const char *OperAcc, const char *OperName, const char *OperPwd, const char *StationNo) {
    TRACE_FUNC("ks_publishopercard");
    int ret=0;
    ks_cardtype_base *c;
    GET_CARDTYPE(c);
    ret=c->PublishOperCard(BatchNo, BatchSeqNo, ExpDate, OperAcc, OperName, OperPwd,StationNo);
    if(ret) {
        ks_setlasterr(c->GetLastErrMsg(NULL),ret);
        return ret;
    }
