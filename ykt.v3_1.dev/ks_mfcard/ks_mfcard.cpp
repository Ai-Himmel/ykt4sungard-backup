...
int ks_mfcard_type::InitCardTypeDLL() {
    ...
    } else if(m_keyAlg == KA_1ST || m_keyAlg == KA_SHU) {
...
int ks_mfcard_type::testLoadKey() {
    if(this->m_keyAlg == KA_1ST || this->m_keyAlg == KA_SHU) {
...
int ks_mfcard_type::LoadKeys4Clone(const char sKeyCardPin[7]) {
    if(m_keyType == 1) {
        if(IsLoadKeys())
            return 0;
        else
        ...
        return CCardType::LoadKeys4Clone(sKeyCardPin);
    }
}

int ks_mfcard_type::IsLoadKeys() {
    if(m_keyType == 1) {
        return (m_isLoadKey==1);
    } else {
        return CCardType::IsLoadKeys();
    }
}

int ks_mfcard_type::IsLoadCloneKeys() {
    if(m_keyType == 1) {
        return (m_isLoadKey==1);
    } else {
        return CCardType::IsLoadCloneKeys();
    }
}
int ks_mfcard_type::CloneKeyCard(const char sCloneCardPin[7]) {
    if(m_keyType == 1) {
        if(!IsLoadCloneKeys()) {
            SET_ERR("密钥未装载");
            return -1;
        ...
        return CCardType::CloneKeyCard(sCloneCardPin);
    }
}


int ks_mfcard_type::NewAuthCard_Init_Sect(unsigned char PhyNo[4]) {
    int sectno,nRet;
...
int	ks_mfcard_type::get_check_char(unsigned char buf[16]) {
    int i=0;
    unsigned int sum=0;
 ...
 * @return - 返回CRC校验值
 */
long ks_mfcard_type::CalAuthCardCRC(BYTE keya[6],BYTE keyb[6],int sect,int block,int pos) {
    BYTE buf[17] = "";
    long crc;
    int i;
    memcpy(buf,keyb,6);
    i = sprintf((char*)buf+6,"%c%c%02X",
                block+0x30,pos+0x30,sect);
    i += 6;
    memcpy((char *)buf + i,keya,6);
    // 计算校验位
...
int ks_mfcard_type::PublishKeyCard (unsigned char* sMasterKey,const char* sPin) {
    if(m_keyType == 1) {
        ...
        memset(buffer,0,sizeof(buffer));
        buffer[11] = CalAuthCardCRC(auth_key_a,auth_key_b,keysectno,0,4);
        nRet = m_Reader->login_card(m_ReaderPara,m_mfcard,sectno,MF_KEYA,card_key);
        if(nRet) {
            SET_ERR("授权卡登录扇区错误");
            return nRet;
...
int ks_mfcard_type::InitUserCard(const char *sShowCardNo) {
    int i,j,block,nRet;
    int len_cardno,len_asc;
    byte keya[6],keyb[6],buffer[16],ctrlbit[4],keya_ro_ctrlbit[4];
    byte init_keya[6],data_buffer[16],sBcdStr[5];
    char datetime[20]="";
    char temp[10] = "";
    char str_keyA[13]="";
    char str_keyB[13]="";
    ...
    len_cardno = strlen(sShowCardNo);		// 显示卡号最长10位
    if(len_cardno >10)
        len_cardno = 10;
    ...
    for(i=m_minSectNo; i< m_maxSectNo; ++i) {
        ...
        if(nRet) {
            ...
            for(block=0; block < 3; ++block) {
                if(i==1) {	// 写显示卡号
                    if(block==0) {
                        asc2bcd(sShowCardNo,len_cardno,sBcdStr,len_asc);
                        memset(data_buffer,0,16);
                        memcpy(data_buffer+7,sBcdStr,len_asc);
                    } else
...
int ks_mfcard_type::CalcCardKeyA(int sectno,mf_sect_t *sect) {
    ...
    if(m_keyAlg == KA_SHU) { // 上海大学版本
        if(memcmp(presentCardNo,this->m_OriginPhyNo,4)) {
            unsigned char sCipher[17],ucdata[16];
            ...
            int j,iTemp;
            for (j = 0,iTemp = 0; j < 8; j++) {
                iTemp += sCipher[j] * j;
            }
            j = iTemp % 10;
            memcpy(sect->key, ucdata + j, 6);
            sect->key_type = MF_KEYA;
...
int ks_mfcard_type::CalcMFSectKey(int sectno,mf_sect_t *sect) {
    ...
    if((sectno == m_lldfSect || sectno == m_lldfSect+1) && m_lldfSect != MF_MAX_SECTNO) {
        ...
        if(memcmp(this->m_OriginPhyNo,m_presentPhyNo,4)) {
            if(m_keyAlg == KA_1ST || m_keyAlg == KA_SHU) {
                memset(sCipher,0,sizeof sCipher);
...
int ks_mfcard_type::SaveKeys() {
    if(m_keyType == 1) {
        ...
        return CCardType::SaveKeys();
    }
}

int ks_mfcard_type::RecycleKeyCard() {
    if(m_keyType == 1) {
        // 读取授权卡
        int nRet,key_sectno;
        char strphyno[17],temp[16];
        unsigned char keya[6],phyno[8],keyb[6];
        ...
        return CCardType::RecycleKeyCard();
    }
}
...
int ks_mfcard_type::initAllCardDef() {
    ...
    if(m_keyAlg != KA_SHU && m_cardVer > 2008)
...
int ks_mfcard_type::lldf_water_publishcard(ks_water_pack_t *pack) {
    ...
    const unsigned char INIT_KEY[] = "\xFF\xFF\xFF\xFF\xFF\xFF";

    // 深圳领路达方税控卡初始值8，9 扇区
    static char* INIT_VAL[] = {
        "\x03\x47\x63\x17\x73\x00\x00\x00\x00\x00\x00\x00\x00\x45\x55\x43",
        "\x00\xF0\x6D\x73\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",

        "\x04\x0A\x52\x24\x73\x6D\x00\x00\x00\x06\x27\x01\x00\x01\x00\x00",
        "\x4F\x4B\x24\x73\x6D\x00\x00\x00\x06\x27\x01\x00\x01\x00\x00\x00",
        "\x04\x0A\x4B\x24\x73\x6D\x00\x00\x00\x06\x27\x01\x00\x01\x00\x00"
    };

    // 水控发卡，初始化
    for(int i =m_lldfSect; i <= m_lldfSect + 1; i ++) {
        // 计算 KeyA
        mf_sect_t sect = {0};
        nRet = CalcCardKeyA(i,&sect);
        if(nRet) {
            SET_ERR("计算扇区["<<i<<"]KeyA失败");
            return KS_WRITECARD;
        }
        ...
        nRet = m_Reader->login_card(m_ReaderPara,m_mfcard,i,MF_KEYA, INIT_KEY);
        if(nRet) {
            if(RequestCard(NULL)) {
                SET_ERR("寻卡失败");
                return KS_LOGINCARD;
            ...
            nRet = m_Reader->login_card(m_ReaderPara,m_mfcard,i,MF_KEYB, m_lldfKey);
            if(nRet) {
                SET_ERR("登录Mifare1扇区["<<i<<"]失败！");
                return KS_LOGINCARD;
            }
        }
        ...
        for(int block=0; block < 3; ++block) {
            memcpy(buffer, INIT_VAL[(i - m_lldfSect)*3 + block], 16); // 设置初始值

            nRet = m_Reader->write_block(m_ReaderPara,m_mfcard,i*4+block,buffer);
            if(nRet) {
                SET_ERR("初始化扇区["<<i<<"]写卡内容失败，请重试");
                return KS_WRITECARD;
            }
        ...
        memcpy(buffer,sect.key,6);
        memcpy(buffer+6,MF1_INIT_CTRLBIT,4);
        memcpy(buffer+10,m_lldfKey,6);
...
int ks_mfcard_type::hd_water_login_fix(ks_water_pack_t *pack) {
    ...
    ret = LoginCardSect(sectno);
    if(ret) {
        if(RequestCard(phyid))
            return KS_REQUESTCARD;

        memcpy(loginkey,"\xFF\xFF\xFF\xFF\xFF\xFF",6);
        if(CalcMFSectKey(sectno,&m_cardSect[sectno])) {
            SET_ERR("计算密钥错误");
            return KS_LOGINCARD;
        }
        memcpy(keyb,m_cardSect[sectno].key,6);
        ...
        CalcCardKeyA(sectno,&m_cardSect[sectno]);
        memcpy(keya,m_cardSect[sectno].key,6);
