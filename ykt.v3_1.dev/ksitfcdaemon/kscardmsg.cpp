...
KsCardMsg::KsCardMsg(const KsCardMsg &rhs) {
    this->msgtype_ = rhs.msgtype_;
    PARA_MAP_TYPE::const_iterator iter;
    this->msg_field_.clear();
    /*
    for(iter = rhs.msg_field_.begin();iter != rhs.msg_field_.end();++iter)
    {
    	this->msg_field_.insert(iter);
    }
    */
    if(rhs.msg_field_.size()>0) {
        this->msg_field_.insert(rhs.msg_field_.begin(),rhs.msg_field_.end());
    }
    parseLevel_ = 0;
}
KsCardMsg& KsCardMsg::operator=(const KsCardMsg &rhs) {
    this->msgtype_ = rhs.msgtype_;
    PARA_MAP_TYPE::const_iterator iter;
    this->msg_field_.clear();
    if(rhs.msg_field_.size()>0) {
...
bool KsCardMsg::SetStrPara(const std::string &para_name,const std::string &para_value,int force/* = 0*/) {
    PARA_MAP_TYPE::iterator iter;
    iter = msg_field_.find(para_name);
    if(iter != msg_field_.end()) {
        if(0==force)
            return false;
        msg_field_.erase(iter);
    }
    msg_field_.insert(PARA_MAP_TYPE::value_type(para_name,para_value));
    return true;
}
bool KsCardMsg::SetIntPara(const std::string &para_name,int para_value,int force/* = 0*/) {
    char v[256] = {0};
    sprintf(v,"%d",para_value);
    return SetStrPara(para_name,v,force);
}