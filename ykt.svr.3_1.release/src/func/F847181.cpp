...
int F847181(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg) {
    ...
    ret=DB_t_tif_rcvdtl_read_lock_by_c0_and_tx_date_and_tx_time_and_cardno_and_deviceid_and_serial_no(tx_date,tx_time,cardno,deviceid,serial_no,&tRcvdtl);
    if(ret) {
        if(DB_NOTFOUND==ret) {
            writelog(LOG_ERR,"tx_date[%s],tx_time[%s],cardno[%d],deviceid[%s],serial_no[%d]",tx_date,tx_time,cardno,deviceid,serial_no);
    ...
    tTradeserial.maindeviceid=rPack->lvol11;
    des2src(tTradeserial.opercode,rPack->sdate1);
    //取流水号,从帐处理
