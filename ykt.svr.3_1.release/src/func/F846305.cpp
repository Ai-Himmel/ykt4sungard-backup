...
int RetWaterAmount(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg) {
    int ret = 0;
    int waterpack[10];
    memset(waterpack,0,sizeof waterpack);

    ret = GetRetWaterPack(waterpack);
    for(int i = 0; i<10; i++) {
        if(waterpack[i]==0)
            break;

        rPack->lvol1 = waterpack[i];
        //rPack->lvol0  卡号
        ret = F850020(handle, iRequest,rPack,pRetCode,szMsg);
        if(ret)
            return ret;
    }
    return ret;
...
int F846305(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg) {
    int ret=0;
    int freefeeflag=0;
    double dPrestoreFee=0;

    // 退水控小钱包
    ret = RetWaterAmount(handle,iRequest,rPack,pRetCode,szMsg);
    //if(ret)
    //	return ret;
    ...
    trans.usecardflag=1;
    pAccTrans->SetCardCntAndCardBal(rPack->lvol5,rPack->lvol6,0);
...
//		}
    ...
    if(trans.fundtype>4) {
        ...
        if(ret)
            return ret;
        if(amtcmp(trans.inputamt,trans.totalfeeamt)>0) {
            ret=CalcPrestoreFee(trans.feetype,trans.inputamt-trans.totalfeeamt,&dPrestoreFee);
            if(ret) {
                writelog(LOG_ERR,"CalcPrestoreFee ret=%d",ret);
                return ret;
            }
            /*
            if(amtcmp(dPrestoreFee,0)!=0)
            {
            	//trans.cardaftbal=D4U5(trans.cardaftbal-dPrestoreFee);
            	ret=MoveAccBal2Frozebal(trans.cardaccno,dPrestoreFee);
            	if(ret)
            		return ret;
            }
            */
    ...
    //取可用余额作为出卡值
    tCardAccount.availbal -=dPrestoreFee;
    tCardAccount.frozebal+=dPrestoreFee;
    ...
    transdtl.managefee=trans.totalfeeamt+dPrestoreFee;
    ...
    outPack->lvol8=D2I(pAccTrans->trans.cardaftbal*100);
    outPack->lvol11=D2I(tCardAccount.singlemaxamt*100);
    outPack->lvol12=D2I(tCardAccount.daycostmaxamt*100);
