...
//		{
        //以卡余额为准
        trans.inputamt=D4U5(tCardAccount.balance-trans.cardaftbal-tCardAccount.frozebal);
        trans.unusedamt = trans.inputamt;
        trans.transamt=trans.inputamt;
        trans.fundtype=0;//0表示从帐户扣
        //营业外支
...
//		}
    ...
    //下面退款按账户金额来退
    if(amtcmp(trans.cardaftbal +tCardAccount.frozebal-tCardAccount.balance ,0)<=0)
        trans.cardaftbal+=tCardAccount.frozebal;
...
// 电子钱包退款
int doEAccount(int custid, double* refund_amt) {
    CAccTrans *pAccTrans=CAccTrans::getInstance();
    pAccTrans->trans.transcode=TC_EACCDRAW;
    int ret=pAccTrans->InitTrans();
    if(ret)
        return ret;
    TRANS& trans=pAccTrans->trans;
    ...
    ret=DB_t_netacc_read_by_custid(custid,&tNetAccount);
    if(ret) {
        if(DB_NOTFOUND==ret)
            return ERRINFO(E_NOTEXIST_EACCNO,custid);
        else
            return E_DB_ACCOUNT_R;
    }
    ...
    trans.inputamt=tNetAccount.balance;
    trans.unusedamt = trans.inputamt;
...
int F846303(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg) {
    ...
    // 电子钱包退款
    double refund_amt = 0;
    doEAccount(trans.custid, &refund_amt);
    ...
    sprintf(outPack->vsmess, "%s\r\n电子钱包应退款 %.2f 元", trans.remark, refund_amt);
