...
int F846301(TRUSERID *handle, int iRequest, ST_PACK *rPack, int *pRetCode, char *szMsg) {
    ...
    pAccTrans->SetInputAmt(rPack->damt0);
    ret = GetSysParaVal(SYSPARA_MAXCHARGEFEE, maxCharge);
    double maxValue = 500;
    if (ret) {
        writelog(LOG_ERR, "未找到最大充值金额 code=[%d]，默认为500", ret);
    } else {
        maxValue = atof(maxCharge);
    }
    if (amtcmp(rPack->damt0, maxValue) > 0) {
    ...
    if (rPack->lvol3 != 0) {
        if (tCardphytype.cardphytypecode != rPack->lvol3) {
            ERRTIP("卡物理类型代码%d与系统代码%d不符", rPack->lvol3, tCardphytype.cardphytypecode);
            return E_COMMON_ERR;
        }
    }
    if (tCustomer.custid) {
...
//  }
    ...
    if (amtcmp(trans.cardaftbal, 0) > 0) {
        trans.aftdpscnt++;
        ret = CalcPrestoreFee(trans.feetype, trans.cardaftbal, &dPrestoreFee);
        if (ret) {
    ...
    // 创建网络账户
    NewEpayAccount(tCustomer.custid, trans.termid, NULL, NULL);
