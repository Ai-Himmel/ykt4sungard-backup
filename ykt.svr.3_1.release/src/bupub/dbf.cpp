    ...
    /* ****** Updated by CHENYH at 2003-10-9 22:44:03 ******
       if (f_ptr->f_type=='X')
       {
          if (valuelen==0 || valuelen>f_ptr->len)
             valuelen = f_ptr->len;
          if (*(WORD *)(buffer-f_ptr->addr)<valuelen)
             valuelen = *(WORD *)(buffer-f_ptr->addr);
          memcpy(value,buffer+f_ptr->addr+sizeof(WORD),valuelen);
       }
       else
    */
    {
        if (valuelen==0 || valuelen>f_ptr->len)
            valuelen = f_ptr->len;
        memcpy(value,buffer+f_ptr->addr,valuelen);
    }
...
int CDBF::f4setvalue(char *fieldname, void *value) {
    ...
    if (f_ptr==NULL)
        return(-1);
    memcpy(buffer+f_ptr->addr,value,f_ptr->len);
    return(0);
}
...
int CDBF::f4r_field(char *fieldname, void *fieldbuf) {
    ...
    */
    ...
    if (f_ptr==NULL)
        return(0);
    offset = dbf_rec_pos(current_recno)+f_ptr->addr;
    lseek(handle,offset,SEEK_SET);
