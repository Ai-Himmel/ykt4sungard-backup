...
#pragma comment(lib,".\\Lib\\CTPack.lib")

static void SetValue(ST_PACKHEAD *head, ST_PACK *pack,int ids,char *data)
{
	long lv;
	double dv;
...
CDRTPHelper::CDRTPHelper(CString ip,int port,int funcno)
:m_serverIP(ip),m_serverPort(port),m_funcNo(funcno),m_drtpHandle(NULL)
{
	//
	Reset();
}
...
{
	Close();
	m_drtpHandle = DRTPConnect(m_serverIP, m_serverPort, 0);
	if(!m_drtpHandle)
	{
		return -1;
	}
...
void CDRTPHelper::AddField(int field,char * value)
{
...
int CDRTPHelper::SendRequest(int timeout /*=0*/)
{
	...
	while(--retries >= 0)
	{
		bRet = DRTPSend(m_drtpHandle, 0, m_funcNo,(char*)m_drtpBuf
			, len, 2, false);
		// TODO : 是否需要重新连接 Drtp
		if(bRet)
		{
	...
	while(--retries >= 0)
	{
		iRet = DRTPReceive(m_drtpHandle,(char*)m_drtpBuf, sizeof m_drtpBuf
			,timeout*1000);
		if (iRet>0)
		{
			break;
	...
	int nCount = 0;
	bRet = DecodeBufWithArray((BYTE*)m_drtpBuf,len, &m_requestPack
		,m_dataArray, &nCount, 0);
	// 记录返回结果集数
	m_currRecordCount = m_requestPack.head.recCount;
	return 0;
}
CString CDRTPHelper::GetErrMsg()
{
	memset(m_errmsg,0,sizeof m_errmsg);
	DRTPGetLastError(m_drtpHandle,m_errmsg,sizeof m_errmsg);
	return CString(m_errmsg);
}
CString CDRTPHelper::GetReturnMsg()
{
	CString retmsg(m_requestPack.pack.vsmess);
	return retmsg;
}
