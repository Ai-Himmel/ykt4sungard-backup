...
CKSSreissueWorkFlow::CKSSreissueWorkFlow(CDialog *dlg) : CKSWorkflow(dlg)
{
	m_key = _T("Sreissuecard");
}
CKSSreissueWorkFlow::~CKSSreissueWorkFlow()
{

}

CString CKSSreissueWorkFlow::GetWorkflowID()
{
	return m_key;
}


int CKSSreissueWorkFlow::OnFormKeyDown(UINT nChar, UINT nFlags)
{
	if(IsTerminate())
	{
		return 0;
...
int CKSSreissueWorkFlow::DoWork()
{
	CDRTPHelper *drtp = NULL;
	CKSNLDlg *dlg = (CKSNLDlg*)GetMainWnd();
	CString password;
	...
	int ret = RET_WF_ERROR;
		dlg->SetLimtText(5);
	if (dlg->InputQuery("请输入学工号", "",Stuemp_no, 20) != 0)
	{
		ret = RET_WF_TIMEOUT;
		goto L_END;
	}
	...
	drtp = dlg->GetConfig()->CreateDrtpHelper();
	drtp->SetRequestHeader(KS_BANK_TX_CODE, 1);
	drtp->AddField(F_SCLOSE_EMP, "930001");
	drtp->AddField(F_SCUST_LIMIT2,(LPSTR)(LPCTSTR)Stuemp_no);
...
//////////////////////////////////////////////////////////////////////////
// CKSCancelSreissueWorkFlow
/*
int CKSCancelSreissueWorkFlow::ReadCard(CKSCardManager* manager,KS_CARD_INFO *cardinfo)
{
	int ret = manager->ReadCardInfo(cardinfo,
		CKSCardManager::ciCardId|CKSCardManager::ciBalance,20);
	if(ret)
...
CKSCancelSreissueWorkFlow::CKSCancelSreissueWorkFlow(CDialog *dlg) : CKSWorkflow(dlg)
{
	m_key = _T("CancelSreissuecard");
}
CKSCancelSreissueWorkFlow::~CKSCancelSreissueWorkFlow()
{

}

CString CKSCancelSreissueWorkFlow::GetWorkflowID()
{
	return m_key;
}


int CKSCancelSreissueWorkFlow::OnFormKeyDown(UINT nChar, UINT nFlags)
{
	if(IsTerminate())
	{
		return 0;
...
int CKSCancelSreissueWorkFlow::DoSreissue(CKSCardManager* manager,
								  KS_CARD_INFO *cardinfo,const ST_PACK *rec)
{
	CKSNLDlg* dlg = (CKSNLDlg*)GetMainWnd();
	int ret;
...
int CKSCancelSreissueWorkFlow::DoWork()
{
	CDRTPHelper *drtp = NULL;
	CKSNLDlg *dlg = (CKSNLDlg*)GetMainWnd();
    ...
    {
		dlg->ShowTipMessage("取消校园卡挂失成功!", 2);	
	}	
	return RET_WF_SUCCESSED;
L_END:
	if (drtp)
...
//	ret = card.ReadCardInfo(&cardInfo, CKSCardManager::ciBalance, 5);
	...
	while (true)
	{
		...
		if (drtp->HasMoreRecord())
		{
			...
			//////////////////////////////////////////////////////////////////////////
			// 新添加写文本函数, 查看卡类型是否相符合(写卡类型和卡余额)
			/*
			SMT_ReadWaterPackInfo(&tempWater);				// 读卡失败不返回, 写文本数据为0
			ReadCard(&card, &tempCardInfo);					// 读卡失败不返回, 写文本数据为0
			if (water.cardtype != tempWater.cardtype)	
			{
				if (WriteLogFile(water.cardtype, tempWater.cardtype, cardInfo.balance, tempCardInfo.balance))
				{
					// 打开文件出错
				}
			}
...
int CKNetChargeTransferWorkFlow::DoWork()
{
	...
	dlg->SetLimtText(1);
	if (dlg->InputQuery("请输入宿舍号", "", strMateRoomNo, 10) != 0)
	{
		ret = RET_WF_TIMEOUT;
		goto L_END;
	}
	cMateRoom = strMateRoomNo.GetAt(0);
	if (strMateRoomNo.GetLength() != 6 || (cMateRoom != '9' && cMateRoom != '8'))
	{
		dlg->ShowTipMessage("输入的宿舍号不对, 请重新操作", 5);
		ret = RET_WF_ERROR;
		goto L_END;		
	...
	while (true)
	{
		...
		drtp->SetRequestHeader(KS_BANK_TX_CODE, 1);
		drtp->AddField(F_SCLOSE_EMP, "240210");
// 		drtp->AddField(F_LVOL0, cardinfo.cardid);
// 		drtp->AddField(F_SEMP_PWD, (LPSTR)(LPCTSTR)strPassword);
// 		drtp->AddField(F_DAMT0, cardinfo.balance);
// 		drtp->AddField(F_DAMT1, (LPSTR)(LPCTSTR)strBalance);
// 		drtp->AddField(F_LVOL3, cardinfo.tx_cnt);
		drtp->AddField(F_SNAME, (LPSTR)(LPCTSTR)dlg->GetConfig()->m_systemNo);
		drtp->AddField(F_SNAME2, (LPSTR)(LPCSTR)strMateRoomNo);
		dlg->ShowTipMessage("正在读取宿舍信息, 请等待...", 1);
		SetStopKey(false);
		if (drtp->SendRequest(10000))
		{
		...
		if (dlg->InputQuery("请输收费金额", "", strBalance, 10) != 0)
		{
			ret = RET_WF_TIMEOUT;
			goto L_END;
		}
		...
		drtp->Reset();
		drtp->SetRequestHeader(KS_BANK_TX_CODE, 1);
		drtp->AddField(F_SCLOSE_EMP, "240211");
		drtp->AddField(F_LVOL0, cardinfo.cardid);
		drtp->AddField(F_SEMP_PWD, (LPSTR)(LPCTSTR)strPassword);
		drtp->AddField(F_DAMT0, cardinfo.balance);							  
		drtp->AddField(F_LVOL3, cardinfo.tx_cnt);							 
		drtp->AddField(F_DAMT1, (LPSTR)(LPCTSTR)strBalance);					  
		drtp->AddField(F_SNAME, (LPSTR)(LPCSTR)dlg->GetConfig()->m_systemNo); 
		drtp->AddField(F_SNAME2, (LPSTR)(LPCSTR)cMateRoom);
		if (drtp->Connect())
		{
			dlg->ShowTipMessage(drtp->GetErrMsg().c_str());
			goto L_END;
...
// 				response->Connect();
// 				response->SetRequestHeader(KS_BANK_TX_CODE, 1);
// 				response->AddField(F_SCLOSE_EMP, "240216");
// 				int serialNo = atoi(pData->sserial1);
// 				response->AddField(F_LVOL1, serialNo);
// 				if (response->SendRequest(3000))
// 				{
...
//				}
			...
			dlg->ClearMessageLine();
			SetStopKey(false);
			tipMsg.Format("您的收费金额为%s元", (LPSTR)(LPCTSTR)strBalance);
			dlg->AddMessageLine(tipMsg);
			tipMsg.Format("您的余额为%d元", cardinfo.balance);
			dlg->AddMessageLine(tipMsg);
			dlg->DisplayMessage(3);				  
			break;
		}
