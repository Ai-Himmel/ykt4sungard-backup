...
/* 挂失卡工作流定义                                                   */
/************************************************************************/
class CKSSreissueWorkFlow : public CKSWorkflow
{
protected:
	virtual int DoWork();
public:
	CKSSreissueWorkFlow(CDialog * dlg);
	virtual ~CKSSreissueWorkFlow(void);
	virtual CString GetWorkflowID();
	virtual int OnFormKeyDown(UINT nChar, UINT nFlags);
};

//xiao test CancelSreissue
/************************************************************************/
/* 取消挂失卡工作流定义        暂时没用                                           */
/************************************************************************/
/*
class CKSCancelSreissueWorkFlow : public CKSWorkflow
{
protected:
	virtual int DoWork();
	int ReadCard(CKSCardManager* manager,KS_CARD_INFO *cardinfo);
	int DoSreissue(CKSCardManager* manager,KS_CARD_INFO *cardinfo,
		const ST_PACK *rec);
public:
	CKSCancelSreissueWorkFlow(CDialog * dlg);
	virtual ~CKSCancelSreissueWorkFlow(void);
	virtual CString GetWorkflowID();
	virtual int OnFormKeyDown(UINT nChar, UINT nFlags);
};
*/
