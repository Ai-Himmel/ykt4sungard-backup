...
void CKSConfig::LoadSystemCfg(const CString& basePath)
{
  ...
  //   v = inifile.ReadString("SYSTEM","CROOM_ID","cr010101");
//	croom_id.Format("%s",v.c_str());
		...
		if (sect.substr(0, 4) == "FUNC")
		{
			...
			if (!v.empty())
			{
				if (app->m_param.number == 1)
				{
					...
					else if (v == "NetCharge2")
					{
						m_workflows.AddWorkflow(new CKNetChargeTransfer2WorkFlow(dlg));		   // 网络收费2	
					}
					...
					{
						m_workflows.AddWorkflow(new CKRemainBoardReckonWorkFlow(dlg));		   // 剩余搭伙费清算		
					}
					else 
					{
						//
