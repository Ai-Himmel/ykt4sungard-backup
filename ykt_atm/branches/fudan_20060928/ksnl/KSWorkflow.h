...
protected:
	...
	int ReadCard(CKSCardManager *manager, KS_CARD_INFO *cardinfo);
	int DoTransfer(CKSCardManager *manager, KS_CARD_INFO *cardinfo, const ST_PACK *rec);
	int WriteBackMoney(CKSCardManager *manager, KS_CARD_INFO *cardinfo, double money);
};

/************************************************************************/
/* 网络收费2                                                             */
/************************************************************************/
class CKNetChargeTransfer2WorkFlow : public CKSWorkflow
{
public:
	CKNetChargeTransfer2WorkFlow(CDialog *dlg);
	~CKNetChargeTransfer2WorkFlow();
	virtual CString GetWorkflowID();
protected:
	virtual int DoWork();
	int ReadCard(CKSCardManager *manager, KS_CARD_INFO *cardinfo);
...
/* 剩余搭伙费清算收费                                                   */
/************************************************************************/
class CKRemainBoardReckonWorkFlow : public CKSWorkflow
{
public:
	CKRemainBoardReckonWorkFlow(CDialog *dlg);
	~CKRemainBoardReckonWorkFlow();
	virtual CString GetWorkflowID();
protected:
	virtual int DoWork();
	int ReadCard(CKSCardManager *manager, KS_CARD_INFO *cardinfo);
	int DoWriteCard(CKSCardManager *manager, KS_CARD_INFO *cardinfo, const ST_PACK *rec);
};