...
int CKSCardManager::SetMoney(int cardid,int money)
{
	...
	KS_CARD_INFO cardinfo;
	memset(&cardinfo,0,sizeof cardinfo);
	if(ReadCardInfo(&cardinfo,ciCardId|ciBalance) != 0)
	{
		return -1;
	}
	if(cardinfo.cardid != cardid)
	...
	if(D2I(cardinfo.balance*100) == money)
	{
		return 0;
	}
	written_ret = SMT_PacketSetMoney(ms_globalPackageNo,money);
	int money_buf[1];
	int cnt_buf[1];
	read_ret = ReadBalance(1,money_buf,cnt_buf);
	if( read_ret != 0 && written_ret != 0)
	{
		m_errno = ERR_PACKAGE_SET_MONEY;
		return -1;
	}
	if(money_buf[0]==money)
	{
		return 0;
	}
	return ERR_PACKAGE_SET_MONEY;
}
