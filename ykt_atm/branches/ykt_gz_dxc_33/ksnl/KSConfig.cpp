		...
		//		AfxMessageBox(sect.c_str());
				if(!v.empty())
				{
					...
					if(workflow != NULL)
					{
						...
						workflow->m_payCode = inifile.ReadInteger(sect.c_str(),"paycode",0);
...
void CKSConfig::InitWorkflows(void)
{
	...
	//					m_workflows.AddWorkflow(new CKMendCardWorkflow(dlg));				   // 补写卡信息
					...
					else if(v == "xtwaterpacketrf")
					{
						m_workflows.AddWorkflow(new CKSXTWaterPacketTransferWorkFlow(dlg));	
					}
					else if(v == "qryxtwatervol")
					{
						m_workflows.AddWorkflow(new CKSXTQueryWaterVolumnWorkFlow(dlg));	
					}
					else if(v == "xt2jdwatertrf")
					{
						m_workflows.AddWorkflow(new CKSXT2JDTransferWorkFlow(dlg));	
					}
					else if(v == "jdwaterpacketrf")
					{
						m_workflows.AddWorkflow(new CKSJDWaterPacketTransferWorkFlow(dlg));	
					}
					else if(v == "qryjdwatervol")
					{
						m_workflows.AddWorkflow(new CKSJDQueryWaterVolumnWorkFlow(dlg));	
					}
					else if(v == "jd2xtwatertrf")
					{
						m_workflows.AddWorkflow(new CKSJD2XTTransferWorkFlow(dlg));	
					}
					else 
					{
					}
	...
	//					m_workflows.AddWorkflow(new CKNetChargeTransferWorkFlow(dlg));
					...
					else if (v == "MobileTransfer")					// 手机
					{
						m_workflows.AddWorkflow(new CKSMobileTransWorkFlow(dlg));	
					}
					else if(v == "TelecomTransfer")					// 电信
					{
						m_workflows.AddWorkflow(new CKSTelecomTransWorkFlow(dlg));
					}
					/*
					else if (v == "qryTrfAct")
					{
...
//						m_workflows.AddWorkflow(new CKSQueryEleTransferAccoutsWorkFlow(dlg));  // 查询电控转帐				
					...
					else if(v == "GDNetChargeTransfer")				// 网费转账
					{
						m_workflows.AddWorkflow(new CKGDNetChargeTransferWorkFlow(dlg));	
					}
					else if(v == "GDQueryNetCharge")				// 网费查询
					{
						m_workflows.AddWorkflow(new CKGDQueryNetChargeWorkFlow(dlg));	
					}
					else
					{
						//
