...
DEF_KSFUNC(SMT_ReadDeadLineDate)
DEF_KSFUNC(SMT_ChangeDeadLineDate)

//新天水控
DEF_KSFUNC(SMT_ReadWaterPackInfo_XT)
DEF_KSFUNC(SMT_TransWaterPacket_XT)
DEF_KSFUNC(SMT_WriteWaterPackInfo_XT)
DEF_KSFUNC(XT2JD_Card)

//竟达水控
DEF_KSFUNC(SMT_ReadWaterPackInfo2)
DEF_KSFUNC(SMT_TransWaterPacket)
DEF_KSFUNC(JD2XT_Card)
...
int CKSCardManager::ciCustId = 0x0F;
int CKSCardManager::ciAllInfo = 0xFF;
...
int CKSCardManager::ReadCardInfo(KS_CARD_INFO * cardinfo,int ciBit,int timeout/*=-1*/)
{
	...
	if(ciBit & ciCustId)
	{
		if(SMT_ReadCutId(&custid) != 0)
		{
			m_errno = ERR_READCUSTID;
			return -1;
		}
		tmp.custid = custid;
	}
	...
	if(ciBit & ciPhyId)
	{
		if(ReadSerial(serial) !=0)
		{
			m_errno = ERR_READ_CARDPHYID;
			return -1;
		}
		HexToAscii(serial, 4, (char*)tmp.phyid);
	}
	if(ciBit & ciPswd)
	{
		if(ReadCardPwd((unsigned char*)tmp.password) !=0)
		{
			m_errno = ERR_READ_CARDPWD;
			return -1;
		}
	}
...
//	TRACE("%d\n", ReadBalance);
	...
	LOAD_KSFUNC(gsCardDLLHandle,SMT_ReadCutId);
//	TRACE("%d\n", ReadCardNO);
...
//	TRACE("%d\n", SMT_WritePersonalPassword);
	...
	LOAD_KSFUNC(gsCardDLLHandle,ReadCardPwd);
//	TRACE("%d\n", ReadSerial);
...
//	LOAD_KSFUNC(gsCardDLLHandle,SMT_ReadDeadLineDate);
	LOAD_KSFUNC(gsCardDLLHandle,SMT_ChangeDeadLineDate);

	// 新天水控
	LOAD_KSFUNC(gsCardDLLHandle,SMT_ReadWaterPackInfo_XT);
	LOAD_KSFUNC(gsCardDLLHandle,SMT_TransWaterPacket_XT);
	LOAD_KSFUNC(gsCardDLLHandle,SMT_WriteWaterPackInfo_XT);
	LOAD_KSFUNC(gsCardDLLHandle,XT2JD_Card);
	
	//竟达水控
	LOAD_KSFUNC(gsCardDLLHandle,SMT_TransWaterPacket);
	LOAD_KSFUNC(gsCardDLLHandle,SMT_ReadWaterPackInfo2);
	LOAD_KSFUNC(gsCardDLLHandle,JD2XT_Card);
