...
{
	// 修改为在需要时创建 modify by cash 2008-8-25
	//InitWorkflows();
...
//			{
		...
		//		AfxMessageBox(sect.c_str());
				...
				if(!v.empty())
				{
					menuid = inifile.ReadInteger(sect.c_str(),"menuid",-1);
					// 如果传递过来的参数与配置不一致,则不加载
					if(menuid != app->m_param.number && app->m_param.number > 0
						&& v != "exit")
						continue;
					CKSWorkflow * workflow = m_workflows.FindWorkflow(CString(v.c_str()));
					if(!workflow)
					{
						workflow = CreateWorkflowFactory(v);
						if(workflow == NULL)
							continue;
						m_workflows.AddWorkflow(workflow);
					}
					if(workflow != NULL)
					{
...
CKSWorkflow *CKSConfig::CreateWorkflowFactory(const std::string &wf_name)
{
	CKsnlApp * app = (CKsnlApp*)AfxGetApp();
	CDialog * dlg = (CDialog *)AfxGetMainWnd();
	CKSWorkflow * wf = NULL;
	if (wf_name == "subsidy")
	{
		wf = new CKSSubsidyWorkFlow(dlg);	               // 补助领取  
	}
	else if (wf_name == "qrysubsidy") 
	{
		wf = new CKSQuerySubsidyWorkflow(dlg);			   // 查询补贴
	}
	else if (wf_name == "qrywaterwf_nameol")
	{
		wf = new CKSQueryWaterVolumnWorkFlow(dlg);	
	}
	else if (wf_name == "waterpacketrf")
	{
		wf = new CKSWaterPacketTransferWorkFlow(dlg);
	}
	else if (wf_name == "ModifyCardInfo")
	{
		//					m_workflows.AddWorkflow(new CKModifyCardInfoWorkflow(dlg));			   // 更新卡信息
	}
	else if (wf_name == "QueryMendCard")											   
	{
		//					m_workflows.AddWorkflow(new CKQueryMendCardWorkflow(dlg));			   // 查询补写卡信息
	}
	else if (wf_name == "MendCard")
	{
		//					m_workflows.AddWorkflow(new CKMendCardWorkflow(dlg));				   // 补写卡信息
	}
	else if (wf_name == "MobileTransfer")
	{
		wf = new CKSMobileTransWorkFlow(dlg);	
	}
	else if (wf_name == "QueryNetCharge")
	{
		//					m_workflows.AddWorkflow(new CKQueryNetChargeWorkFlow(dlg));	
	}
	else if (wf_name == "NetCharge")
	{
		//					m_workflows.AddWorkflow(new CKNetChargeTransferWorkFlow(dlg));
	}
	/*
	else if (wf_name == "qryTrfAct")
	{
	...
	*/
	else if (wf_name == "ElePackTrf")
	{
		//						m_workflows.AddWorkflow(new CKSElePacketTransferWorkFLow(dlg));		   // 电控转帐		
	}
	else if (wf_name == "ElePackAccTrf") 
	{
	...
	else if (wf_name == "GZYCTTransfer")
	{
		wf = new CGZYCTTransWorkFlow(dlg);
	}
	else if( wf_name == "GZELECTransfer")
	{
		wf = new CGZElecTransWorkFlow(dlg);
	}
	else if( wf_name == "exit" )
	{
		wf = new CKSExitWorkFlow(dlg);
	}
	return wf;
}
// 初始化所有工作流
...
//						m_workflows.AddWorkflow(new CKSQuerySubsidyWorkflow(dlg));			   // 查询补贴
					...
					else if (v == "qrywatervol")
					{
						m_workflows.AddWorkflow(new CKSQueryWaterVolumnWorkFlow(dlg));	
					}
					else if (v == "waterpacketrf")
					{
						m_workflows.AddWorkflow(new CKSWaterPacketTransferWorkFlow(dlg));
					}
	...
	//					m_workflows.AddWorkflow(new CKMendCardWorkflow(dlg));				   // 补写卡信息
					...
					else if (v == "MobileTransfer")
					{
						m_workflows.AddWorkflow(new CKSMobileTransWorkFlow(dlg));	
					}
					else 
					{
						//
