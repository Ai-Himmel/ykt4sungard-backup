...
type
  TfrmMain = class(TForm)
    ...
    procedure NPrint_WClick(Sender: TObject);
...
procedure TfrmMain.NPrint_WClick(Sender: TObject);
begin
  {
...
begin
  TLogger.GetInstance.Debug('开始登陆拍照系统');
...
begin
  ...
  if (cbbarea.Text<>'') then
    area := subString(cbbArea.Text,'-','l')
  else
    area := '';
  queryBaseInfo(Trim(edtStuempNo.Text),area,'');
end;
...
procedure TfrmMain.getPhotoNum;
var
  sqlstr:string;
begin
...
begin
  try
    cbbMenu.Items.Clear;
    cbbMenu.Items.LoadFromFile(apppath+'printset\printmenu_w.txt');
    menuCount := cbbMenu.Items.Count;
    for i := 0 to menuCount-1 do
    begin
      custM := TMenuItem.Create(Self);
