  ...
  try
    ...
    begin
      ...
      begin
        ...
        WVCardFreeze.FieldByName('摘要').Data.SetString(trim(edtMemo.Text));
        ...
        begin
          ...
          errList.Add('客户号为['+custid+']姓名为['+custname+']的卡冻结卡失败,失败原因--'+WVCardFreeze.FieldByName('返回信息').Data.AsString);
        end;
        Sleep(100);
      end;
    end;
    ...
    begin
      if Application.MessageBox('批量冻结有失败记录，你要保存该失败记录吗？',PChar(Application.Title),MB_ICONQUESTION+mb_yesno)=idyes then
      begin
        if dlgSave1.Execute then
          errList.SaveToFile(dlgSave1.FileName+'_批量冻结卡失败记录.txt');
          //errList.SaveToFile(appPath+'\PATCHLOG\批量冻结卡失败记录'+datestr+'.txt');
        Context.GetDialogSystem.ShowMessage('该批卡冻结完成');
      end;
    end
    else
    begin
...
begin
  try
    ...
    //qrySource.SQL.SaveToFile('c:\text.txt');
    qrySource.Open;
  except
    on e:Exception do
    begin
      Context.GetDialogSystem.ShowMessage('打开数据表失败，请检查打开的Excel文件是否正确-'+e.Message);
      Exit;
    end;
  end;
  ...
  try
    ...
    begin
      sCustId := qrySource.FieldByName('学工号').AsString;
      //if Trim(sCustId)='####' then
      //  Exit;
      WorkView.FieldByName('钱包号').Data.SetInteger(1);
      WorkView.FieldByName('学工号').Data.SetString(sCustId);
      WorkView.FieldByName('客户号').Data.SetInteger(0);
      try
        ...
        on e:Exception do
        begin
          errList.Add('学工号为['+sCustId+']的卡冻结查询失败,失败原因--'+e.Message);
          qrySource.Next;
          Continue;
        end;
      end;
