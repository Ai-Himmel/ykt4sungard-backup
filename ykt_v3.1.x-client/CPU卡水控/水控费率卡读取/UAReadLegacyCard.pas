...
uses
  ...
  RzEdit, RzSpnEdt, UIncrementComboBox, WaterDll, Buttons,CardDll;

type
  TfaqhreturnWaterManageCard = class(TfaqhSimpleQueryTemp)
    Panel1: TPanel;
    btnModify: TImageButton;
    lbl1: TLabel;
    ...
    rgUse: TRadioGroup;
    Label36: TLabel;
    rgFeeUnit: TRadioGroup;
    rgFeeType: TRadioGroup;
    Label7: TLabel;
...
begin
  ...
  begin
    Context.GetDialogSystem.ShowMessage('请选择你要查看的收费类别！');
    Exit;
  end;
  ifeeType := StrToInt(Copy(selType,0,Pos('-',selType)-1));
  //cardManage:=nil;
    try
      ...
      if re<>0 then
      begin
        Context.GetDialogSystem.ShowMessage('读取卡类型失败,请放置好卡！');
        Exit;
      end;
      if Trim(LowerCase(BytesToStr(cardType)))='lycj' then
      begin
        Context.GetDialogSystem.ShowMessage('该卡为采集卡，不能读取其信息！');
        Exit;
      end;
      }
      ...
      re := ks_water_read_legacycard(4,@feeRateCard);
      if re<>0 then
      begin
        Context.GetDialogSystem.ShowMessage('读取收费类别信息失败，请放置好卡！');
        Exit;
      end;
      readRateValue(ifeeType);
      btnModify.Enabled := True;
    except
      Context.GetDialogSystem.ShowMessage('读取费率设置卡信息失败，请放好卡！');
    end;
end;
...
begin
  ...
  begin
    Context.GetDialogSystem.ShowMessage('请先设置费率，然后再修改卡信息！');
    Exit;
  end;
  ...
  //try
    ...
    try
      ...
      if re<>0 then
      begin
        Context.GetDialogSystem.ShowMessage('修改费率设置卡失败，请重试！');
        Exit;
      end
      else
      begin
        KNG_beep;
        Context.GetDialogSystem.ShowMessage('修改费率设置卡成功！');
        btnModify.Enabled := False;
      end;
    except
      Context.GetDialogSystem.ShowMessage('设置费率设置卡信息失败，请放好卡！');
    end;
  //finally
    //cardManage.Destroy;
  //end;
...
initialization
  TUIStdClassFactory.Create('水控设置卡读取',TfaqhreturnWaterManageCard);
