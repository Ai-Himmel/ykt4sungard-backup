...
uses
  ...
  StdCtrls, RzLabel, ExtCtrls, RzPanel, Mask, RzEdit;

type
  TFrmPatchCardHint = class(TForm)
    btnOk: TButton;
    btnCancle: TButton;
    btnNext: TButton;
...
var
  FrmPatchCardHint: TFrmPatchCardHint;

implementation

uses UPatchCardIssue;

{$R *.DFM}

procedure TFrmPatchCardHint.btnOkClick(Sender: TObject);
begin
...
procedure TFrmPatchCardHint.btnCancleClick(Sender: TObject);
begin
  PatchOperaKind := 2;
  Close;
end;

procedure TFrmPatchCardHint.btnNextClick(Sender: TObject);
begin
  PatchOperaKind := 3;
  Close;
end;

function TFrmPatchCardHint.checkValue: Boolean;
begin
  result:=True;
...
procedure TFrmPatchCardHint.FormShow(Sender: TObject);
begin
