...
type
  TfaqhBusiReportFC = class(TfaTreeRptTemp)
    WorkView2: TWorkView;
...
var
  faqhBusiReportFC: TfaqhBusiReportFC;
...
procedure TfaqhBusiReportFC.Init;
begin
  inherited;
...
procedure TfaqhBusiReportFC.RefreshCurrentTree;
begin
  try
...
procedure TfaqhBusiReportFC.WorkViewFieldsMonitors0ValueChanged(
...
procedure TfaqhBusiReportFC.tvCustClick(Sender: TObject);
begin
...
procedure TfaqhBusiReportFC.btnQueryClick(Sender: TObject);
var
  tmpData:TDataSet;
  //devName:string;
  qry:TZQuery;
begin
  ...
  if execSQL_SLite(delPosMeal)<>0 then
    Exit;
  //execSQL(delPosMeal);
  qry := nil;
  try
    ...
    begin
      qry.Close;
      qry.SQL.Clear;
      qry.SQL.Add(irptPosMeal(tmpData));
      qry.ExecSQL;
      tmpData.Next;
    end;
  ...
  try
    rptReport.LoadFromFile(sPreAppPath+'report\商户分餐分类结算表.rmf');
...
procedure TfaqhBusiReportFC.getLeave;
var
  i,j:integer;
  currlevel:integer;
  totlevel:integer;
...
procedure TfaqhBusiReportFC.BuildTree;
begin
...
procedure TfaqhBusiReportFC.InternalBuildTree(AInfo: TUIInfo;
  Parent: TTreeNode);
var
  I: Integer;
  Node: TTreeNode;
...
initialization
  TUIStdClassFactory.Create('商户分餐分类结算表',TfaqhBusiReportFC);
