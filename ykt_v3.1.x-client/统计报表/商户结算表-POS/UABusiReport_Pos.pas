...
uses
  ...
  RM_Preview, RzButton, ZAbstractRODataset, ZAbstractDataset,
  ZAbstractTable, ZDataset, UTreeRptTemp,KsBranchTree,contnrs, RM_e_Xls;
type
  PMyRec = ^TMyRec;
  TMyRec = record
    id: Integer;
...
type
  TfaqhBusiReportPOS = class(TfaTreeRptTemp)
    WorkView2: TWorkView;
...
var
  faqhBusiReportPOS: TfaqhBusiReportPOS;
...
procedure TfaqhBusiReportPOS.Init;
begin
  inherited;
...
procedure TfaqhBusiReportPOS.RefreshCurrentTree;
begin
  try
    WorkView2.FieldByName('商户标识').Data.SetInteger(0);
    QueryRequest2.SendCommand;

    FDataSetFieldID:='lvol3';
    FDataSetParentFieldID:='lvol1';
    FDataSetFieldName:='sall_name';
    FDataSet:= (WorkView2.FieldByName('查询结果集').Data.AsObject) as TDataSet;
    BuildTree;
...
procedure TfaqhBusiReportPOS.WorkViewFieldsMonitors0ValueChanged(
...
procedure TfaqhBusiReportPOS.tvCustClick(Sender: TObject);
begin
...
procedure TfaqhBusiReportPOS.btnQueryClick(Sender: TObject);
var
  tmpData:TDataSet;
  qry:TZQuery;
begin
  ...
  if execSQL_SLite(delrptPosLeager)<>0 then
    Exit;
  //execSQL(delrptPosLeager);
  qry := nil;
  try
    ...
    begin
      qry.Close;
      qry.SQL.Clear;
      qry.SQL.Add(irptPosLeager(tmpData));
      qry.ExecSQL;
      tmpData.Next;
    end;
  ...
  rptReport.LoadFromFile(sPreAppPath+'report\商户POS分类结算表.rmf');
...
procedure TfaqhBusiReportPOS.getLeave;
var
  i,j:integer;
  currlevel:integer;
  totlevel:integer;
...
procedure TfaqhBusiReportPOS.BuildTree;
begin
...
procedure TfaqhBusiReportPOS.InternalBuildTree(AInfo: TUIInfo;Parent: TTreeNode);
var
  I: Integer;
  Node: TTreeNode;
...
initialization
  TUIStdClassFactory.Create('商户POS分类结算表',TfaqhBusiReportPOS);
