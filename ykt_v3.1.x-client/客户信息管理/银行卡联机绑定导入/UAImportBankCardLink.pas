...
type
  TfaqhImportBankCardLink = class(TfaqhSimpleQueryTemp)
    Panel1: TPanel;
    btnImport: TBitBtn;
...
var
  faqhImportBankCardLink: TfaqhImportBankCardLink;

implementation

uses KsFrameworks, KSClientConsts,UCMqhCustomerImp,SmartCardCommon;
     
{$R *.DFM}

procedure TfaqhImportBankCardLink.Init;
begin
  inherited;
  WorkView.Reset;
end;

procedure TfaqhImportBankCardLink.btnOpenClick(Sender: TObject);
begin
  //inherited;
...
procedure TfaqhImportBankCardLink.btnQueryClick(Sender: TObject);
var
  sqlstr:string;
  i:Integer;
begin
...
procedure TfaqhImportBankCardLink.iniGridSize;
begin

end;

procedure TfaqhImportBankCardLink.btnImportClick(Sender: TObject);
var
  xH,f,i:integer;
  stuempNo:string;
  custId:string;
...
begin
  ...
  begin
    Context.GetDialogSystem.ShowMessage('要导入的数据小于2条，请直接添加银行卡绑定信息！');
    Exit;
  end;
  errList := nil;
  try
    ...
    try
      ...
      on e:Exception do
      begin
        Context.GetDialogSystem.ShowMessage('学工号['+stuempNo+']导入失败，原因--'+e.Message);
        Exit;
      end;
    end;
    if WorkView.FieldByName('返回码').Data.AsString<>'0' then
    begin
      Context.GetDialogSystem.ShowMessage('学工号['+stuempNo+']导入失败，原因--'+WorkView.fieldbyname('返回信息').Data.AsString);
    ...
    if errList.Count>0 then
      ...
      begin
        errList.SaveToFile(dlgSave1.FileName);
      end;
...
procedure TfaqhImportBankCardLink.btnPrintClick(Sender: TObject);
begin
  //inherited;
  closeDataSet;
end;
...
{-------------------------------------------------------------------------------
  ...
  返回值:    无
-------------------------------------------------------------------------------}
procedure TfaqhImportBankCardLink.closeDataSet;
begin
...
procedure TfaqhImportBankCardLink.GridDrawColumnCell(Sender: TObject;
...
initialization
  TUIStdClassFactory.Create('银行卡联机绑定导入',TfaqhImportBankCardLink);
