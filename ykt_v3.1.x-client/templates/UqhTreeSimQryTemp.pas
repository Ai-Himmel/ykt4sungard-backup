unit UqhTreeSimQryTemp;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  USimpleQueryTemp, DBActns, ActnList, WorkViews, WVDBBinders, WVCmdReq, Db,
  DBCtrls, Grids, DBGrids, StdCtrls, ImageCtrls, ExtCtrls, UICtrls,
  ComCtrls, KCDataAccess,WVCommands,SafeCode, WVCtrls, RzDBGrid, Buttons,
  RzTreeVw, RzPanel, RzSplit,KsBranchTree,contnrs;
...
type
  TfaqhTreeSimQryTemp = class(TfaSimpleQueryTemp)
...
{ TfaqhSimpleQueryTemp }

procedure TfaqhTreeSimQryTemp.BindProperIdObjects(WorkComBox: TWVComboBox;
  SourceSL: TStrings; ChosenStr: string; IsQuery: Boolean);
var
...
procedure TfaqhTreeSimQryTemp.btnCustClassCancel(Sender: TObject);
var
...
procedure TfaqhTreeSimQryTemp.btnCustClassConfirm(Sender: TObject);
var
  ComboxName : string;
  I : Integer;
...
function TfaqhTreeSimQryTemp.CheckQuery(const SrcStr: string): Boolean;
begin
  Result := False;
  if Pos(S_ISQUERY,SrcStr)=1 then Result := True;
end;

function TfaqhTreeSimQryTemp.ContainName(Name, SubName: string): Boolean;
var
  tempStr : string;
begin
  Result := False;
...
constructor TfaqhTreeSimQryTemp.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TfaqhTreeSimQryTemp.CreateContractListStr;
var
...
destructor TfaqhTreeSimQryTemp.Destroy;
begin
  FInfoAdmin.Free ;
  FDataSet.Free;
  inherited;
...
procedure TfaqhTreeSimQryTemp.FindCaseForContract(Sender: TObject);
begin
...
procedure TfaqhTreeSimQryTemp.GBExit(Sender: TObject);
begin
  if Sender is TGroupBox then
    (Sender as TGroupBox).Visible := False;
end;

function TfaqhTreeSimQryTemp.GetKindsFromContract(
...
function TfaqhTreeSimQryTemp.GetProPerContract(
...
function TfaqhTreeSimQryTemp.GetStr(const Str: string): string;
begin
  Result := StringReplace(Trim(Str),' ','',[rfReplaceAll]);
end;

procedure TfaqhTreeSimQryTemp.Init;
{
var
  I : Integer;
  tempName : String;
...
procedure TfaqhTreeSimQryTemp.qhCusClassNamePreQuery;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
procedure TfaqhTreeSimQryTemp.qhCusListPreQuery;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
procedure TfaqhTreeSimQryTemp.qhExchPreQuery(WorkField : TWVField);
var
  I : Integer;
...
procedure TfaqhTreeSimQryTemp.qhKindsAndSeatInit;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
procedure TfaqhTreeSimQryTemp.ReadTreeInfoFromDataSet(DataSet: TDataSet;CInfoAdmin: TInfoAdmin);
var
...
function TfaqhTreeSimQryTemp.Related(ParentCtrl, ChildCtrl, ParentSub,
...
procedure TfaqhTreeSimQryTemp.SynCustClass( CustClassComBoBox : TWVComboBox;
  ParentGB: TGroupBox);
...
procedure TfaqhTreeSimQryTemp.VarGroupBoxCreate(Sender: TObject);
var
  GroupBox : TGroupBox;
  I,J : Integer;
