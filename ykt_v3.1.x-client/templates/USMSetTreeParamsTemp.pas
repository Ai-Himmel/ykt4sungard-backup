...
type
  ...
  public
    ...
    {在DBGRID中定位指定纪录}
    procedure   GetCurrentDbGrid(CurrentTreeId:Integer);
  end;
...
procedure   TfaSMSetTreeParamsTemp.GetCurrentDbGrid(CurrentTreeId:Integer);
begin
  FSearchOptions.FieldName := FDataSetFieldID;
  FSearchOptions.SearchValue := IntToStr(CurrentTreeId);
...
procedure TfaSMSetTreeParamsTemp.ReadTreeInfoFromDataSet(DataSet: TDataSet;CInfoAdmin:TInfoAdmin);
var
...
begin
  ...
  begin
    ...
    if( curr.Parent <> nil ) then
      FCurrentID:=StrToInt(TTreeNodeInfoData(curr.Parent.Data).GetID)
    else
      FCurrentID:=StrToInt(TTreeNodeInfoData(curr.Data).GetID)
  end
  else Exit;
  ...
  WorkView.FieldByName(FWorkViewParentFieldID).Data.SetInteger(FCurrentID);
...
procedure TfaSMSetTreeParamsTemp.TreeViewDblClick(Sender: TObject);
begin
...
procedure TfaSMSetTreeParamsTemp.pcPagesPaintTabBackground(Sender: TObject;
