unit UqhSetParamsTemp;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  USetParamsTemp, DBActns, ActnList, WorkViews, WVDBBinders, WVCmdReq, Db,
  DBCtrls, Grids, DBGrids, StdCtrls, ImageCtrls, ExtCtrls, UICtrls,
  WVCtrls, RzDBNav, RzButton,
  RzRadChk, RzDBGrid, RzTabs, Buttons;

const
  S_ISQUERY = '查询.';
  S_SeperatedStr = '-';
  //KS_RefreshCurrentDBGrid = WM_User + 3000;

type
  TfaqhSetParamsTemp = class(TfaSetParamsTemp)
  private
...
//    procedure qhKindsSeatQuery(WorkField : TWVField);
  protected
    ...
    //procedure  RefreshCurrentDBGrid(var Message:TMessage); Message KS_RefreshCurrentDBGrid;
  public
    { Public declarations }
  end;
...
{procedure  TfaqhSetParamsTemp.RefreshCurrentDBGrid(var Message:TMessage);
begin
...
procedure TfaqhSetParamsTemp.BindProperIdObjects(WorkComBox: TWVComboBox;
  SourceSL: TStrings; ChosenStr: string; IsQuery: Boolean);
var
...
procedure TfaqhSetParamsTemp.btnCustClassCancel(Sender: TObject);
var
...
procedure TfaqhSetParamsTemp.btnCustClassConfirm(Sender: TObject);
var
  ComboxName : string;
  I : Integer;
...
function TfaqhSetParamsTemp.CheckQuery(const SrcStr: string): Boolean;
begin
  Result := False;
  if Pos(S_ISQUERY,SrcStr)=1 then Result := True;
end;

function TfaqhSetParamsTemp.ContainName(Name, SubName: string): Boolean;
var
  tempStr : string;
begin
  Result := False;
...
procedure TfaqhSetParamsTemp.GBExit(Sender: TObject);
begin
  if Sender is TGroupBox then
    (Sender as TGroupBox).Visible := False;
end;

function TfaqhSetParamsTemp.GetStr(const Str: string): string;
begin
  Result := StringReplace(Trim(Str),' ','',[rfReplaceAll]);
end;

procedure TfaqhSetParamsTemp.Init;
var
  I : Integer;
  tempName : String;
...
procedure TfaqhSetParamsTemp.qhCusClassNamePreQuery;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
procedure TfaqhSetParamsTemp.qhCusListPreQuery;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
procedure TfaqhSetParamsTemp.qhExchPreQuery(WorkField : TWVField);
var
  I : Integer;
...
procedure TfaqhSetParamsTemp.qhKindsAndSeatInit;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
function TfaqhSetParamsTemp.Related(ParentCtrl, ChildCtrl, ParentSub,
...
procedure TfaqhSetParamsTemp.SynCustClass( CustClassComBoBox : TWVComboBox;
  ParentGB: TGroupBox);
...
procedure TfaqhSetParamsTemp.VarGroupBoxCreate(Sender: TObject);
var
  GroupBox : TGroupBox;
  I,J : Integer;
