...
uses KsFrameworks,KSClientConsts, KSDataDictionary,
   SmartCardCommon, UPatchCardHint_nmg,FileCtrl;
...
begin
  ...
  begin
    getFeeResult;
    FrmPatchCardHint_nmg := TFrmPatchCardHint_nmg.Create(self);
    try                                           
      FrmPatchCardHint_nmg.PStuempNo :=
        RzListView1.Items.Item[StrToInt(CurrentOperateRecordNo.Strings[0])].SubItems[1];
      FrmPatchCardHint_nmg.PName :=
        RzListView1.Items.Item[StrToInt(CurrentOperateRecordNo.Strings[0])].SubItems[2];
      FrmPatchCardHint_nmg.PDept :=
        RzListView1.Items.Item[StrToInt(CurrentOperateRecordNo.Strings[0])].SubItems[6];
      FrmPatchCardHint_nmg.PCustType :=
        RzListView1.Items.Item[StrToInt(CurrentOperateRecordNo.Strings[0])].SubItems[3];

      FrmPatchCardHint_nmg.patchShowCardNo:=ShowCardNo;
      FrmPatchCardHint_nmg.patchEndDate:=sEndDate;
      FrmPatchCardHint_nmg.patchTipInfo:= tipInfo;

      if Trim(TmpMsg) = '' then
        FrmPatchCardHint_nmg.PWarning := '请检查以上信息是否与您即将要发卡的信息一致！'
      else
        FrmPatchCardHint_nmg.PWarning := TmpMsg;
      FrmPatchCardHint_nmg.PatchCardAccount := IssueCount;
      FrmPatchCardHint_nmg.PatchCardSuccess := self.SuccessCount;
      FrmPatchCardHint_nmg.PatchCardError := self.FailCount;
      FrmPatchCardHint_nmg.PatchCardGived := self.GivedCount;
      FrmPatchCardHint_nmg.PatchOneCardError := OneCardError;
      FrmPatchCardHint_nmg.PTotMoney := '合计'+edtMoney.Text+' 元';
      FrmPatchCardHint_nmg.ShowModal;
      pb1.Max := 100;
      pb1.Position := 0;
      if FrmPatchCardHint_nmg.PatchOperaKind = 2 then
      begin
        setEditValue;
        CurrentOperateRecordNo.Clear;
        Exit;
      end;
      if FrmPatchCardHint_nmg.PatchOperaKind = 3 then
      begin
        seteditValue();
        OneCardError := True;
        ...
        FrmPatchCardHint_nmg.Close;
        Continue;
      end
      else if FrmPatchCardHint_nmg.PatchOperaKind = 1 then
      begin
        setEditValue;
        NewPCardID := '';
        ...
        begin
          ...
          begin
            ...
            //end;
            sendDate:=FrmPatchCardHint_nmg.edtEndDate.Text;
  ...
  try
    ...
    begin
      try
        ...
        begin
          ErrorMsg := '该卡补助开始金额设置失败，请直接到补助开水金额设置里设置金额！';
        end;
      except
