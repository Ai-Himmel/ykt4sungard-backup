...
long CCheckBlackVersion::ProcessData(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	strcpy(filename,pDocEx->pChildDoc[pDocEx->nPortNo]->DeviceInfo.sRegNo);
	cTool.trim(filename);
	...
	else if( pTask->nFunc == 930004 ) //删除黑名单	
	{
		pTask->nRepeat1=999;
		...
		for( nTry=0; nTry<3; nTry++)
		{
			if( (iResult=SendRequest(pDocEx->pChildDoc[pDocEx->nPortNo], pBuffer)) == RET_OK )
			{
				if( (iResult=UnMakeDeviceBufferEx(pBuffer)) == RET_OK )
				{
					//sprintf(msg,"删除黑名单成功[未下版本]: 设备:%s, 卡号:%d, 功能号:%ld......", pDocEx->pChildDoc[pDocEx->nPortNo]->m_szDeviceID, card,pTask->nFunc);	
					//WriteLog(msg);																
					//cTool.AddLineToFile(filename,msg,strlen(msg));
					flag = 1;
					Sleep(300);
					break;
				}
		...
		if( !flag )
		{
			sprintf(msg,"删除黑名单失败[未下版本]: 设备:%s, 卡号:%d, 功能号:%ld......", pDocEx->pChildDoc[pDocEx->nPortNo]->m_szDeviceID, card,pTask->nFunc);	
			//cTool.AddLineToFile(filename,msg,strlen(msg));
			flag = 0;
			GetResultMessage(iResult, data.sMsg);
			ReportTaskResult(pTask, iResult, &data);
		...
		for( nTry=0; nTry<3; nTry++)
		{
			if( (iResult=SendRequest(pDocEx->pChildDoc[pDocEx->nPortNo], pBuffer)) == RET_OK )
			{
				if( (iResult=UnMakeDeviceBufferEx(pBuffer)) == RET_OK )
				{
					sprintf(msg,"更改黑名单版本号成功: 设备:%s, 卡号:%d, 功能号:%ld......", pDocEx->pChildDoc[pDocEx->nPortNo]->m_szDeviceID, card,pTask->nFunc);	
					//WriteLog(msg);
					//cTool.AddLineToFile(filename,msg,strlen(msg));
		...
		//sprintf(msg,"删除黑名单版本失败[未下版本]: 设备:%s, 卡号:%d......", pDocEx->pChildDoc[pDocEx->nPortNo]->m_szDeviceID, card);	
		//cTool.AddLineToFile(filename,msg,strlen(msg));		
	}
	else if( pTask->nFunc == 930005 ) //广播黑名单	
	{
		pTask->nRepeat1=999;
		BYTE Buf[128];
		...
		pBuffer->nTimeOut1=1000;
		pBuffer->nTimeOut2=1000;
		SendRequest(pDocEx->pChildDoc[pDocEx->nPortNo], pBuffer);				
	...
	{
		iResult=RET_SYSERROR;
		ReportLog(pDocEx, pTask, iResult, "黑名单操作失败,参数无效,终端没有操作.", "");	
		sprintf(msg,"黑名单操作标志:%c,功能号:%ld,卡号:%ld",pTask->szMemo[45],pTask->nFunc,card);
		//WriteLog(msg);
		//cTool.AddLineToFile(filename,msg,strlen(msg));
		GetResultMessage(iResult, data.sMsg);
		ReportTaskResult(pTask, iResult, &data);
