...
long __stdcall RecvDataPro(char *pszBuffer, int nLen, int nTimeOut)
{
	...
	if( iResult <= 0 )
	{
		...
		if( iResult == -1 )
		{
			...
			if( iResult > 0 )
			{
				WriteLog("三九前置与金仕达重连后接收数据成功(RecvDataPro),内容:%s\n",pszBuffer);
				return iResult;
			}
		}
		return RET_NG;
...
long __stdcall SendData(HANDLE& hHandle, int nDest, int nFunc, char *pszData, int nLen, int nPriority, bool bRouter)
{
	...
	{
		WriteLog("三九前置向金仕达发送数据成功(SendData),发送内容:%s\n",pszData);
		return RET_OK;
	}
	else
	{
...
long __stdcall RecvData(HANDLE& hHandle, char *pszBuffer, int nLen, int nTimeOut)
{
	...
	if( iResult <= 0 )
	{
		...
		if( !strcmp(szText, "10054") )
		{
			...
			if( iResult > 0 )
			{
				WriteLog("三九前置重连金仕达后接收数据成功(RecvData),接收内容:%s\n",pszBuffer);
				return iResult;
			}
		}
...
//#if 0
	while( !bEnd )
	{
		...
		nLen = sizeof(buffer);
		nlen = RecvData(hHandle, buffer, nLen, 0);
		if( nlen <= 0 )
		{
			ConnectPool.Free(hHandle);
...
long __stdcall ReadSmartTaskPlan(TSSmartTaskPlan *pTaskPlan)
{
	...
	if( !bFristRun )
	{
		...
		pTaskPlan->nTaskPlanID = 100 ;
		strcpy(pTaskPlan->szTaskName, "收数");
		pTaskPlan->nTaskCycle = 1 ;
		GetCurDateTime(pTaskPlan->szBeginTime);
		pTaskPlan->nRepeatTime = 0 ;                 //持续多长时间（分钟）
		pTaskPlan->nRepeatTimes = 0;                //重复次数

		int k=0;
		long i = 0 ;

		for( i=0; i< gnDocCount; i++)
		{
			if( !gpSmartDoc[i].m_nParentID )
			{
				pTaskPlan->pTask[k].nTaskID = i + 100 ;
				pTaskPlan->pTask[k].nTaskPlanID = 01;
				pTaskPlan->pTask[k].nAuthID = gpSmartDoc[i].m_nAuthID ;
				strcpy(pTaskPlan->pTask[k].szTaskCode, "01");
				strcpy(pTaskPlan->pTask[k].szName, "实时收数");
				strcpy(pTaskPlan->pTask[k].szType, "01");
				pTaskPlan->pTask[k].nPriority = 0 ;
				pTaskPlan->nTask++;  //设备总数量
				k++;
			}
		}
		bResult = true ;
		printf("First Run ---------gnDocCount=%d\n",gnDocCount);
		return RET_OK;
	}
	else
	{
		...
		while( !bEnd )
		{
			...
			nlen = RecvDataPro(buffer, nLen, 1000);
			if( nlen <= 0 )
			{
				return RET_SYSERROR;
			}
			...
			{
				printf("金仕达没有任务下达!!!\n");
				return RET_SYSERROR;		//金仕达没有任务下达
			}
			if( apack.head.firstflag == 1 )
			{
				//WriteLog("first run\n");
				//printf("---------apack.head.firstflag == 1-----------\n");
...
/*	bool bResult = false;
	...
	for( i=0; i< gnDocCount; i++)
	{
		if( !gpSmartDoc[i].m_nParentID )
		{
			...
			strcpy(pTaskPlan->pTask[k].szName, "实时收数");
			strcpy(pTaskPlan->pTask[k].szType, "01");
			pTaskPlan->pTask[k].nPriority = 1;
			pTaskPlan->nTask++;  //设备总数量
			k++;
		}
	}
	printf("Reload ---------gnDocCount=%d\n",gnDocCount);
	bResult = true ;
	*/
...
{
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
	//return RET_OK;//debug 20050402
	if( GetTickCount() - pDoc->m_dwDBTick < (DWORD)gTick || pDoc->m_nSignIn != 1 )
	{
		return RET_OK;
	}
	...
	SetValue(&rpack.head,&rpack.pack, F_SHOLDER_AC_NO, szDateTime);	
	SetValue(&rpack.head,&rpack.pack, F_SSERIAL0, pDoc->m_sDownBlackTime);
	SetValue(&rpack.head,&rpack.pack, F_DAMT0, GetValue(info.nDiskFreeSpace/info.nDiskTotalSpace*100) );
	SetValue(&rpack.head,&rpack.pack, F_DAMT1, GetValue(info.nUserMemory/info.nMemoryTotal*100));
	SetValue(&rpack.head,&rpack.pack, F_DAMT2, GetValue(100-info.nCPU));
	SetValue(&rpack.head,&rpack.pack, F_SHOLDER_AC_NO2, szDateTime);
	SetValue(&rpack.head,&rpack.pack, F_DAMT3, GetValue(1));
	...
	if( EncodeBuf(&rpack, szBuffer, &nLen, omsg) )
	{
		...
		if( SendData(hHandle, iServerNo, iFunc, (char*)szBuffer, nLen, 1, FALSE) == RET_OK )
		{
			...
			memset(szBuffer, 0, sizeof(szBuffer));
			//int nlen = RecvData(hHandle, (char*)szBuffer, nLen, 0);
			int nlen = RecvData(hHandle, (char*)szBuffer, nLen, 1000);
			if( nlen > 0 )
			{
				ST_CPACK apack;
				memset(&apack, 0, sizeof(apack));
				if( DecodeBuf((unsigned char*)szBuffer,nlen,&apack,omsg) )
				{
					...
					{
						ConnectPool.Free(hHandle);
						//printf("发送设备监控心跳包成功! 服务号:%d, 子功能号:%d, , 设备ID=%s", iServerNo, iFunc, pDoc->m_szDeviceID);
					...
					else
					{
						...
						//printf("发送设备监控心跳包失败! 设备ID=%s, 发送设备监控心跳包时返回:%s\n", pDoc->m_szDeviceID, apack.pack.vsmess);
	...
	WriteTickLog("发送设备监控心跳包失败! 服务号:%d, 子功能号:%d, 设备ID=%s", iServerNo, iFunc, pDoc->m_szDeviceID);
	//DEBUG
	return RET_OK;
	//return RET_SYSERROR;
}
