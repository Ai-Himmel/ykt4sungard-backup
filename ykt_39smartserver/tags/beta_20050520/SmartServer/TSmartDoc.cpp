...
long TSmartDocObj::ExecuteTask(TSmartTaskObj *pObj, long nIndex)
{
	...
	{
		sprintf(strText, "设备%d被分派执行任务. 任务代码:%s.", 
	...
	catch(...)
	{
		printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1异常抛出!\n");
		ProcessRequest(true, &m_CmdBuffer);
		m_pServer->OnDocError(this, "----执行任务失败,重试!");//"执行任务时有未知的异常!");
		return RET_SYSERROR;
	}
...
void TSmartDocObj::TaskFinished(long nResult)
{
	...
	if( m_pTaskObj->IsTaskFinished() )
	{
		...
		ReportLog("任务:%s (%d) 已经结束!", m_pTaskObj->szTaskName, m_pTaskObj->nTaskPlanID);
		printf("任务:%s (%d) 已经结束!", m_pTaskObj->szTaskName, m_pTaskObj->nTaskPlanID);
	} 
...
long TSmartDocObj::RecvCmdBuffer(TCmdBuffer *pCmdBuffer)
{
	...
	if( nRecvLen <= 0 ) 
	{
		ReportLog("接收%.8X包长度非法",m_nAuthID);
	...
	//ReportLog("接收到%.8X完整数据包",m_nAuthID);//,pCmdBuffer->m_nRecvBytes);
	printf("接收到%.8X完整数据包\n",m_nAuthID);
	unsigned char c[256];
...
//			printf("没有表面数据!\n");
	...
	catch(...)
	{
		...
		throw TException("业务层处理数据失败!重新执行!!!");
		//ReportLog("业务层处理数据时发生异常! 请检查业务层程序!");
		//throw TException("业务层处理数据时发生异常! 请检查业务层程序!");
	}
...
void TSocketSmartDocObj::Open()
{
	...
	ReportLog("TSocketSmartDocObj终端设备%s打开成功! 地址:%s, 端口:%s\n", m_szDeviceID, m_szAddr, m_szPort);
}
