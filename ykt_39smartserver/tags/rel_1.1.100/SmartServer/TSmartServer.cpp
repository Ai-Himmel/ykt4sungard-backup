...
{	
	bool bReloadFlag = false;
	 ...
	 //Add by dengjun 20061008
	if( m_nNoTask_Tick  >2000  || m_nNoTask_Tick<0  ) m_nNoTask_Tick   = 2000;
	if( m_nCollData_Tick>5000  || m_nCollData_Tick<0) m_nCollData_Tick = 5000;
	...
	try
	{
		...
		m_CollDataTaskObj.nTask		   = 0;		//服务总数
		...
		{	
			...
			if(nTick-nCollDataTick>=m_nCollData_Tick+60000)
			{
				nCollDataTick = nTick;
				ExecuteCollDataTask();
			...
			for(int t=0; t< iCount; t++)
			{
				...
				{				
					if( pObj->IsExecuted() )
					{
						...
						//{
							ReportLog("任务:%s(%d), 任务代码:%s, 设备:%08X, 准备暂停原有任务!",pTaskObj->szTaskName, pTaskObj->nTaskPlanID, pTaskObj->pTask[nIndex].szTaskCode, pObj->m_nAuthID);
							//add by hhd
							//m_DocLock.Unlock();
							//return false;
							//comment by hhd end
							...
							{	
								...
								*/
								bResult = false;
								ReportLog("任务:%s(%d), 任务代码:%s, 设备:%08X, 任务执行失败!",pTaskObj->szTaskName, pTaskObj->nTaskPlanID, pTaskObj->pTask[nIndex].szTaskCode, pObj->m_nAuthID);
								//2005-12-20 wenjian comment end
							}
							else 
							{								
								printf("任务:%s(%d), 任务代码:%s, 设备:%08X, 任务首次执行成功!\n", 
								pTaskObj->szTaskName, pTaskObj->nTaskPlanID, 
								pTaskObj->pTask[nIndex].szTaskCode, pObj->m_nAuthID);
								bResult = true ;
							}
					...
					{			
						...
						{								
							printf("任务:%s(%d), 任务代码:%s, 设备:%08X, 任务执行成功!\n", 
							pTaskObj->szTaskName, pTaskObj->nTaskPlanID, 
							pTaskObj->pTask[nIndex].szTaskCode, pObj->m_nAuthID);
							bResult = true ;
						}
