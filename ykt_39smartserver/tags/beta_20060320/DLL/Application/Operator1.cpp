...
bool COperator::ModifyPassword(TSSmartTask *pTask, TSSmartDoc *pDoc, unsigned char *ucRawData)
{
	...
	unsigned char AccountNum[17], AccountNum1[17];
	unsigned char OldPassWord[17];
	unsigned char NewPassWord[17];
	DWORD  nSerial = 0 ;
	unsigned char CardSerial[11], PlainKey[17], SourceKey[17];

	memset(OldPassWord, 0, sizeof(OldPassWord));
	memset(NewPassWord, 0, sizeof(NewPassWord));

	sprintf((char*)AccountNum,"%07ld000000000", cardid);
//	memset(AccountNum,'0',17);
	sprintf((char *)AccountNum1,"%.2X%.2X%.2X%.2X%.2X",ucRawData[8],ucRawData[9],ucRawData[10],ucRawData[11],ucRawData[12]);
	...
	{
		strcpy(cCardSerialNo, Data.szSerial);
	}

	if( strlen(cCardSerialNo) ==  8 )  
	{
		memcpy(CardSerial, cCardSerialNo, 8);
	}
	else
	{
		char   temp[256];

		nSerial = atol(cCardSerialNo);
		sprintf(temp,"%08X",nSerial);
		memcpy(temp+8,temp+6,2);
		memcpy(temp+10,temp+4,2);
		memcpy(temp+12,temp+2,2);
		memcpy(temp+14,temp,2);
		memcpy(temp,temp+8,8);
		memcpy(CardSerial,temp,8);
	}
	...
	if( strCS2 != strCS1 )
	{
		printf("修改密码时, 密码不正确! 旧密码:%s, 新密码:%s.\n", strCS1.GetBuffer(0), strCS2.GetBuffer(0));
		return false;		
	}
	else*/
	{
