...
long CSingInServer::ProcessData(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	for(int i=0; i<3; i++)
	{
		...
		if(RET_OK==ret)
		{
			...
			{
				printf("-----!!!!校验%.8X返回包失败-时钟!错误码=[%d]\n",pDocEx->pChildDoc[m_nIndex]->m_nAuthID,ret);
			}
		}
		else
		{
			printf("-----!!!!下传终端%.8X时钟失败!错误码=[%d]\n",pDocEx->pChildDoc[m_nIndex]->m_nAuthID,ret);
			Sleep(400);
		}
	}
...
//	printf("------设置授权信息------------");
	...
	for(i=0; i<3; i++)
	{
		...
		if(RET_OK==ret)
		{
			...
			{
				printf("-----!!!!校验%.8X返回包失败-授权!错误码=[%d]\n",pDocEx->pChildDoc[m_nIndex]->m_nAuthID,ret);
			}
		}
		else
		{
			printf("-----!!!!设置终端%.8X授权信息失败!错误码=[%d]\n",pDocEx->pChildDoc[m_nIndex]->m_nAuthID,ret);
			Sleep(500);
		}
	...
	for(i=0; i<3; i++)
	{
		...
		if(RET_OK==ret)
		{
			...
			{
				printf("-----!!!!校验%.8X返回包失败-消费开关!错误码=[%d]\n",pDocEx->pChildDoc[m_nIndex]->m_nAuthID,ret);
			}
		}
		else
		{
			printf("-----!!!!设置终端%.8X消费开关失败!错误码=[%d]\n",pDocEx->pChildDoc[m_nIndex]->m_nAuthID,ret);
			Sleep(500);
		}
	}
