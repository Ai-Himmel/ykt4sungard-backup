...
long COperator::ProcessStatusRequest(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	//表示主设备不通
	if( pBuffer == NULL || (UnMakeDeviceBuffer(pBuffer)!=RET_OK)) 
	{		
	...
	//LPORT服务器
	if(  !strcmp(pDocEx->pChildDoc[m_nIndex]->m_szMacCode, "5301") )
	{
		...
		if( NPort < 24 ) CardResult.sVersion[i]=0x00;  //结束符
