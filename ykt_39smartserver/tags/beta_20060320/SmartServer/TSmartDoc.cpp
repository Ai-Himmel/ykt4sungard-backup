...
long TSmartDocObj::ExecuteTask(TSmartTaskObj *pObj, long nIndex)
{
	...
	try
	{
		...
		{
			sprintf(strText, "设备%d被分派执行任务. 任务代码:%s.", DocEx.pChildDoc[DocEx.nPortNo]->m_nAuthID, pObj->pTask[nIndex].szTaskCode);
			WriteLog(strText);
		}
	...
	{
		sprintf(errmsg,"----(TException)执行任务失败,重试!%s,[LPORT=%.8X::任务名:%s::任务代码%s::线程ID=%ld]",(char*)e.GetText(),m_nAuthID,pObj->szTaskName,pObj->pTask[nIndex].szTaskCode,(DWORD)this->m_hThread);
	...
	{
		sprintf(errmsg,"----(...)执行任务失败,重试![LPORT=%.8X::任务名:%s::任务代码%s::线程ID=%ld]",m_nAuthID,pObj->szTaskName,pObj->pTask[nIndex].szTaskCode,(DWORD)this->m_hThread);
		m_pServer->OnDocError(this, errmsg);
		WriteTaskLog(errmsg);
		return RET_SYSERROR;  //update by dengjun 20051126
		//return ProcessRequest(true, &m_CmdBuffer);
...
long TSmartDocObj::ProcessRequest(bool bBroad, TCmdBuffer *pCmdBuffer)
{
	...
	try
	{
		...
		if( nRet != RET_NG )
		{
			...
			char szLog[1024];
			sprintf(szLog, "设备:%d, 执行任务<任务:%s(%d), 任务代码:%s>返回! 返回结果(%d):%s",
	...
	{
		sprintf(errmsg,"----(ProcessRequest(...))业务层处理数据失败![LPORT=%.8X::任务名:%s::任务代码%s::线程ID=%ld]",m_nAuthID,m_pTaskObj->szTaskName,m_pTaskObj->pTask[m_nIndex].szTaskCode,(DWORD)this->m_hThread);
		WriteTaskLog(errmsg);
		ReportLog(errmsg);
	//	ReportLog("业务层处理数据失败!重新执行 %s!!!\n",m_pTaskObj->szTaskName);
	}
	return nRet;
}
