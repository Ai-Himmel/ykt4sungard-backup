...
// 编程  : 陈永华 2004-2-18 16:34:50
// 描述  : 用于根据字段位图估算一条记录编码需要的缓存长度
// 返回  : int 编码最大长度
...
// 函数名: ShowPackHead
// 编程  : 陈永华 2004-2-22 20:04:24
// 描述  : 将phead中的数据用可显示方式存放到buf中
// 返回  : int 最后在buf中存放的长度
// 参数  : char *buf OUT
// 参数  : ST_PACKHEAD *phead IN
int ShowPackHead(char *buf,ST_PACKHEAD *phead);

// 如下为了方便CPACK数据赋值和设置位图而设定的几个定义：
/*
   使用方法：对于F_Sxxxx、F_USxxxx 或 F_VSxxxx 应该使用DAddFieldC;
            对于F_Lxxxx、F_Dxxxx 应该使用DAddFieldID;
  参数:  
  p: ST_CPACK *p;
  b: 即需要赋值的位图F_xxxxx 如 F_LVOL0
  v: 指ST_PACK中定义的变量名称 如 lvol0
  d: 具体的符合位图要求类型的值或变量 
*/
#define DAddFieldC(p,b,v,d) { SetParmBit(&(p->head),b); memcpy(p->pack.v,d,sizeof(p->pack.v)); }
#define DAddFieldID(p,b,v,d) { SetParmBit(&(p->head),b); p->pack.v = d; }

#define CAddFieldC(p,b,v,d) { SetParmBit(&(p.head),b); memcpy(p.pack.v,d,sizeof(p.pack.v)); }
#define CAddFieldID(p,b,v,d) { SetParmBit(&(p.head),b); p.pack.v = d; }
