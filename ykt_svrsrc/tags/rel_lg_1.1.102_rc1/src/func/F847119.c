...
/*
    ...
    {
        ret = DB_t_tif_writefailed_read_by_card_id_and_purese_no_and_deal_flag(tCard->card_id,cnt_purse_no,&deal_flag[0],tWriteFailed);
        if (ret)
        {
          if (DB_NOTFOUND == ret)
            continue;
    ...
    if (PURESE_COUNT == cnt_purse_no)
    {
        ret = E_WRITEFAILED_DEAL_FLAG_N;
        goto L_RETU_INNER;
    }
*/
...
//  writelog(LOG_ERR,"XXX");
    ...
    writelog(LOG_ERR,"operate_date[%s]",operate_date);
    writelog(LOG_ERR,"logictime[%s]",logicdate);
    ...
    if (847300 == serial_type)                                                //小钱包是不用入账的
        tradeserial->serial_state = SERISTAT_NONEEDDEBT;
    else
	...
	tradeserial->reviseserial_no = D2I(dSerialno);											//原写卡失败流水号
