...
int F847101(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	//writelog(LOG_DEBUG,"begin read cust info");
	ret = queryCustidAndExpTime(tCard.physical_no,CARDINF_NEW_STATE,&custid,&tCardInf);
	if(ret)
	{
	...
	if(IsInvalidDateTime(card_endtime,"YYYYMMDD") != 0)
	{
	...
	*/
	...
	if(ret)
	{
		*pRetCode=ret;
		...
		if(E_AMT_LACK == ret)
		{
			sprintf(szMsg,"卡余额为[%s]",acc_outbuf);
		}
		goto L_RETU;
	}
