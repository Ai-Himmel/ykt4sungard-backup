...
make_dist()
{
	...
	DIRS="bin lib log install src/func src/public src/db src/tool src/include"
	for i in $DIRS 
	do
		$MKDIR -p $DEST_DIR/$i
		if [ $? -ne 0 ];then
...
make_tar()
{
	...
	DEST_DIR="$BASEDIR/dist/ykt"
	DIRS="bin log install lib src/func src/public src/db src/tool src/include  src/buinc  src/bupub"
	for i in $DIRS 
	do
		$MKDIR -p $DEST_DIR/$i
		if [ $? -ne 0 ];then
	...
	DEST_FILES="bu0 bu1 function0.lst function1.lst ksmbcc.ini bala.ini start.ini sall cpack.dat proccfg.cfg"
	copy_files "$DEST_FILES" $BASEDIR/bin $DEST_DIR/bin
	DEST_FILES="ksmbcc"
	copy_files_check_version "$DEST_FILES" "$BASEDIR/bin" "$DEST_DIR/bin"
	# copy sbin directory
	...
	libbupubs.a libdrtpapi.a libdrtpapp3.a libdrtplib.a libksshmt.a libxpack.a"
	copy_files_check_version "$DEST_FILES" "$BASEDIR/lib" "$DEST_DIR/lib"

	DEST_FILES="*.act makefile esqlc.* gf .clean.sh .gendb.sh"
	...
	$CP -r $BASEDIR/src/bupub $DEST_DIR/bupub
	$CP -r $BASEDIR/src/buinc $DEST_DIR/buinc
