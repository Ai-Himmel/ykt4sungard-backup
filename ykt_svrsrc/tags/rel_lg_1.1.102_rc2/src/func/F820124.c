...
int F820124(TRUSERID *handle,int iRequest,ST_PACK *in_pack,int *pRetCode,char *szMsg)
{
	...
	if(0==v_phytype)
	{
		DB_t_pif_phydevice_open_select_by_c1();
		
		while(1){
			
			memset(&aphydev,0,sizeof(aphydev));
			
			r=DB_t_pif_phydevice_fetch_select_by_c1(&aphydev);
      	...
      	Strncpy_t(out_pack->usset1,aphydev.comments,sizeof(out_pack->usset1));
   	...
   	DB_t_pif_phydevice_close_select_by_c1();
   	return 0;
   	...
   	L_Ret_if:
		...
		r=DB_t_pif_phydevice_read_by_phytype(v_phytype,&aphydev);
		if(SC_NOTFOUND==r)
		{
			*pRetCode=E_DB_PHYDEVICE_N;
