...
int BackupData()
{
	...
	int ret=0;
	char logic_date[8+1]="";
	T_t_tif_tradelog		t_tradelog;
	T_t_tif_tradelog_his	t_tradelog_his;
	T_t_tif_tradeserial		t_tradeserial;
	T_t_tif_tradeserial_his	t_tradeserial_his;
...
int BackupAccount()
{
	...
	//备份帐户表，T_t_aif_account------>T_t_tif_act_daysum
	ret=DB_t_tif_act_daysum_del_by_operate_date(logic_date);
	if((0!=ret)&&(100!=ret))
	{
		writelog(LOG_ERR,"Delete from t_tif_act_daysum where date=[%s],errcode=[%d]",logic_date,ret);
		return 	ret;
	}
	...
	while(1)
	{
		...
		/*
		////生成报表过程开始
		strcpy(act_daysum.operate_date,logic_date);
		strcpy(act_daysum.operate_time,getsystime(NULL));
		strncpy(act_daysum.account_id,account.account_id,sizeof(act_daysum.account_id));
		act_daysum.act_type=account.act_type;
		act_daysum.customer_id=account.customer_id;
		strncpy(act_daysum.cut_name,account.cut_name, sizeof(act_daysum.cut_name));
		act_daysum.cut_type=account.cut_type;
		strncpy(act_daysum.stuemp_no,account.stuemp_no,sizeof(act_daysum.stuemp_no));
		act_daysum.card_id=account.card_id;
		act_daysum.purse_id=account.purse_id;
		//act_daysum.card_state=account.card_state;
		act_daysum.card_type= account.card_type;
		strncpy(act_daysum.subno,account.subno,sizeof(act_daysum.subno));
		strncpy(act_daysum.password,account.password,sizeof(act_daysum.password));
		act_daysum.current_state=account.current_state;
		strncpy(act_daysum.isautotra,account.isautotra,sizeof(act_daysum.isautotra));
		act_daysum.last_bala=account.last_bala;
		act_daysum.last_freebala=account.last_freebala;
		act_daysum.last_frozebala=account.last_frozebala;
		act_daysum.cur_bala=account.cur_bala;
		act_daysum.cur_freebala=account.cur_freebala;
		act_daysum.cur_frozebala=account.cur_frozebala;
		act_daysum.out_bala=account.out_bala;
		act_daysum.in_bala=account.in_bala;
		act_daysum.out_count=account.out_count;
		act_daysum.in_count=account.in_count;
		act_daysum.card_balance=account.card_balance;
		act_daysum.consume_count=account.consume_count;
		strncpy(act_daysum.open_date,account.open_date,sizeof(act_daysum.open_date));
		//暂时没有赋值，
		act_daysum.today_out_bala=0;
		act_daysum.today_in_bala=0;
		act_daysum.today_out_count=0;
		act_daysum.today_in_count=0;
		strncpy(act_daysum.reserve_1,account.reserve_1,sizeof(act_daysum.reserve_1));
		strncpy(act_daysum.reserve_2,account.reserve_2,sizeof(act_daysum.reserve_2));
		strncpy(act_daysum.reserve_3,account.reserve_3,sizeof(act_daysum.reserve_3));
		strncpy(act_daysum.comments,account.comments,sizeof(act_daysum.comments));
		act_daysum.nobala_num=account.nobala_num;

		ret=DB_t_tif_act_daysum_add(&act_daysum);
		if(ret)
		{
			writelog(LOG_ERR," DB_t_tif_act_daysum_add error,err[%d],record[%d]",ret,cnt);
			db_rollback();
			return 	ret;
		}
		*/
		...
		if(account.act_type==ACCTYPE_PERMAIN)
		{
			...
			{
				account.nobala_num=account.nobala_num+1;
			}
			else
			{
...
/*
		printf("========================================\n");
		printf("v_oper.oper_code	=[%s]\n",v_oper.oper_code);
		printf("v_oper.serial_type	=[%d]\n",v_oper.serial_type);
		printf("v_oper.trade_count	=[%d]\n",v_oper.trade_count);
		printf("v_oper.trade_amt	=[%lf]\n",v_oper.trade_amt);
		printf("v_oper.in_amt		=[%lf]\n",v_oper.in_amt);
		printf("v_oper.deposit_amt	=[%lf]\n",v_oper.deposit_amt);
		printf("v_oper.cost_amt		=[%lf]\n",v_oper.cost_amt);
		printf("v_oper.cash_amt		=[%lf]\n",v_oper.cash_amt);
		printf("v_oper.maindevice_id	=[%d]\n",v_oper.maindevice_id);
		printf("v_oper.device_id	=[%d]\n",v_oper.device_id	);
		printf("cnt=[%d]\n",cnt);
*/
		////生成报表过程开始
...
/*
		writelog(LOG_DEBUG,"t_oper.seri_type[%d]",t_oper.seri_type);
		writelog(LOG_DEBUG,"t_oper.balance_date[%s]",t_oper.balance_date);
		writelog(LOG_DEBUG,"t_oper.maindevice_id[%d]",t_oper.maindevice_id);
		writelog(LOG_DEBUG,"t_oper.device_id[%d]",t_oper.device_id);
		writelog(LOG_DEBUG,"t_oper.operator_code[%s]",t_oper.operator_code);
*/
...
int DelTradeDate()
{
	...
	//删除消息队列表
	ret=DB_t_tif_meslist_del_all_by_step_commit();
	if((0!=ret)&&(100!=ret))
	{
		writelog(LOG_ERR,"DB_t_tif_tradeserial_del_all error,errcode=[%d]",ret);
		db_rollback();
