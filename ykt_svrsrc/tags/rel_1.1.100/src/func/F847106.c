...
int F847106(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	switch(type)
	{
		case TYPE_CASH:	//现金
			...
			break;
		case TYPE_BILL:	//支票
			tradeserial.serial_type = TXCODE_SAVING_BILL;
			IA.dArrInAmt[2]=tradeserial.trade_fee;
			break;
		case TYPE_FUNDBOOK:	//经费本
			tradeserial.serial_type = TXCODE_SAVING_FUNDBOOK;
			IA.dArrInAmt[3]=tradeserial.trade_fee;
			break;
		case TYPE_OTHER:	//其他
			tradeserial.serial_type = TXCODE_SAVING_OTHER;
	...
	sprintf(sMsg,"充值前卡余额%.2lf 元 卡当前余额:%.2lf元",tradeserial.in_balance,tradeserial.out_balance);
	strcat(out_pack->vsmess,sMsg);
