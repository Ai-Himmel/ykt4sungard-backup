...
#define MAXNUM_CFGTXFEE		200		//最大交易配置项个数
#define MAXNUM_CFGSPLIT		40		//最大交易类型个数
...
//		writelog(LOG_DEBUG,"formula[%s]num_exp[%s]amt[%.4lf]",sFormula,num_exp,dExpCalAmt);
		...
		ret = DB_t_aif_account_read_lock_by_cur_and_account_id(tTradelogDb.act_id, &tAccount);
		if (ret)
		{
		...
		if(strcmp(tAccount.subno,CfgSplit.ArrCfgsplit[iTxType].dbsubno)!=0)
		{
			DB_t_aif_account_free_lock_cur();
			writelog(LOG_ERR,"tradecode config_cnt[%d]TxType[%d]act_id[%s] tAccount.subno[%s]config dbsubno[%s]",config_cnt,iTxType,tAccount.account_id,tAccount.subno,CfgSplit.ArrCfgsplit[iTxType].dbsubno);
			return E_SPLIT_CFG;
		}
		strcpy(tTradelogDb.subno,tAccount.subno);		//借方科目号
	//	writelog(LOG_DEBUG,"debit No[%d]account[%s]account.cur_bala[%lf]cur_freebala[%lf]cur_frozebala[%lf]amt[%lf]",i,account.account_id,account.cur_bala,account.cur_freebala,account.cur_frozebala,tTradelogDb.op_fee);
		idxSubCfg=GetIndexSubjectCfgBySubNo(tAccount.subno);
		if(idxSubCfg<0)
		{
			writelog(LOG_ERR, "GetIndexSubjectCfgBySubNo[%d] ",tAccount.subno);
			DB_t_aif_account_free_lock_cur();
			return E_DB_SUBJECT_N;
		}
		if (SUBTYPE_FUND== CfgSubject.ArrSubject[idxSubCfg].subtype|| SUBTYPE_COST== CfgSubject.ArrSubject[idxSubCfg].subtype )
		{
			tAccount.cur_bala = D4U5(tAccount.cur_bala+ tTradelogDb.op_fee,6);
			tAccount.cur_freebala = D4U5(tAccount.cur_freebala+ tTradelogDb.op_fee,6);
		}
		else
		{
			if(strncmp(tAccount.subno,"201",3)==0)
			{
				//消费时从冻结金额中扣除搭伙费
				switch(CfgSplit.ArrCfgsplit[iTxType].fundtype)
				{
					case FUNDTYPE_BOARD://搭伙费
						...
						if(amtcmp(tAccount.cur_frozebala,tTradelogDb.op_fee)<0)
						{
							dExpCalAmt=tAccount.cur_frozebala;
							tTradelogDb.op_fee = dExpCalAmt;		//借方发生额
						}
						tAccount.cur_bala =D4U5(tAccount.cur_bala- tTradelogDb.op_fee,6);			//总余额
						tAccount.cur_frozebala = D4U5(tAccount.cur_frozebala - tTradelogDb.op_fee,6);
						break;
					default:
						tAccount.cur_bala =D4U5(tAccount.cur_bala- tTradelogDb.op_fee,6);			//总余额
						tAccount.cur_freebala =D4U5(tAccount.cur_freebala-tTradelogDb.op_fee,6);	//可用余额
						//更新卡余额
						if(USE_CARD_TYPE_ONLINE==IA->iUseCardFlag)
						{
							if(amtcmp(dOutCardBala,0)>=0)
							{
								tAccount.card_balance=dOutCardBala;					//卡余额
								if(IA->iTxCnt>0)
									tAccount.consume_count=IA->iTxCnt;			//消费次数增加
							}
							else if(amtcmp(dInCardBala,0)>=0)
							{
								...
								tAccount.card_balance=IA->dOutCardBala;							//账户卡余额
 								if(IA->iTxCnt>0)
									tAccount.consume_count=IA->iTxCnt;
							}
						}
						else if(USE_CARD_TYPE_OFFLINE==IA->iUseCardFlag)
						{
							if(tAccount.consume_count<IA->iTxCnt)
							{
								if(amtcmp(dOutCardBala,0)>=0)
								{
									tAccount.card_balance=dOutCardBala;
									if(IA->iTxCnt>0)
										tAccount.consume_count=IA->iTxCnt;			//消费次数增加
								}
								else if(amtcmp(dInCardBala,0)>=0)
		...
		//判断余额是否小于0,不允许透支,现金除外
		if(strncmp(tAccount.subno,"101",3)!=0)
		{
			if(amtcmp(tAccount.cur_bala,0)<0||amtcmp(tAccount.cur_freebala,0)<0)
			{
				writelog(LOG_ERR,"host serialno[%d]debit No[%d]account[%s]account.cur_bala[%lf]cur_freebala[%lf]cur_frozebala[%lf]amt[%lf]",IA->iSerialNo,i,tAccount.account_id,tAccount.cur_bala,tAccount.cur_freebala,tAccount.cur_frozebala,tTradelogDb.op_fee);
				DB_t_aif_account_free_lock_cur();
				return E_BALANCE_SHORTAGE;
			}
		}
		tTradelogDb.new_fee    = tAccount.cur_bala;						//借方帐户总余额
		tTradelogDb.cur_frozebala=tAccount.cur_frozebala;					//冻结金额
		ret = DB_t_aif_account_update_lock_by_cur(&tAccount);
		if (ret)
		{
			writelog(LOG_ERR, "update account err[%d] account[%s]", ret,tAccount.account_id);
			if (DB_NOTFOUND == ret)
				return E_ACTNO_NOT_EXIST;
			else
		...
		memset(&tAccount, 0, sizeof(tAccount));
		ret = DB_t_aif_account_read_lock_by_cur_and_account_id(tTradelogCr.act_id, &tAccount);
		if (ret)
		{
		...
		if(tAccount.current_state != ACCOUNTSTAT_REGIST)
			return E_ACTNO_LOGOUT;
		if(strcmp(tAccount.subno,CfgSplit.ArrCfgsplit[iTxType].crsubno)!=0)
		{
			writelog(LOG_ERR,"tradecode config_cnt[%d]TxType[%d]act_id[%s] tAccount.subno[%s]config dbsubno[%s]",config_cnt,iTxType,tAccount.account_id,tAccount.subno,CfgSplit.ArrCfgsplit[iTxType].crsubno);
			DB_t_aif_account_free_lock_cur();
			return E_SPLIT_CFG;
		}
		strcpy(tTradelogCr.subno, tAccount.subno);			//贷方科目号
//		writelog(LOG_DEBUG,"credit No[%d]tAccount[%s]account.cur_bala[%lf]cur_freebala[%lf]cur_frozebala[%lf]amt[%lf]",i,account.account_id,account.cur_bala,account.cur_freebala,account.cur_frozebala,ArrTradelog[i+1].op_fee);
		...
		if (SUBTYPE_FUND== CfgSubject.ArrSubject[idxSubCfg].subtype|| SUBTYPE_COST== CfgSubject.ArrSubject[idxSubCfg].subtype )
		{
			tAccount.cur_bala =D4U5(tAccount.cur_bala- tTradelogCr.op_fee,6);							//当前总余额
			tAccount.cur_freebala =D4U5(tAccount.cur_freebala-tTradelogCr.op_fee,6);							//当前可用余额
		}
		else
		{
			...
			if(strncmp(tAccount.subno,"201",3)==0)
			{
				tAccount.cur_bala =D4U5(tAccount.cur_bala+ tTradelogCr.op_fee,6);				//总余额
				switch(CfgSplit.ArrCfgsplit[iTxType].fundtype)
				{
					case FUNDTYPE_BOARD:						//搭伙费
						tAccount.cur_frozebala =D4U5(tAccount.cur_frozebala+ tTradelogCr.op_fee,6);
						break;
					default:
						tAccount.cur_freebala =D4U5(tAccount.cur_freebala+ tTradelogCr.op_fee,6);	//可用余额
						//更新卡余额
						if(USE_CARD_TYPE_ONLINE==IA->iUseCardFlag)
						{
							if(IA->iTxCnt>0)
		...
		if(strncmp(tAccount.subno,"101",3)!=0)
		{
			if(amtcmp(tAccount.cur_bala,0)<0||amtcmp(tAccount.cur_freebala,0)<0)
			{
				writelog(LOG_ERR,"host serialno[%d]credit No[%d+1]account[%s]account.cur_bala[%lf]cur_freebala[%lf]cur_frozebala[%lf]amt[%lf]",IA->iSerialNo,i,tAccount.account_id,tAccount.cur_bala,tAccount.cur_freebala,tAccount.cur_frozebala,tTradelogDb.op_fee);
				DB_t_aif_account_free_lock_cur();
				return E_BALANCE_SHORTAGE;
			}
		}
		tTradelogCr.new_fee = tAccount.cur_bala;			//贷方帐户余额
		tTradelogCr.cur_frozebala=tAccount.cur_frozebala;		//冻结金额
		ret = DB_t_aif_account_update_lock_by_cur(&tAccount);
		if (ret)
		{
			writelog(LOG_ERR, "update account err[%d] account[%s]", ret,tAccount.account_id);
