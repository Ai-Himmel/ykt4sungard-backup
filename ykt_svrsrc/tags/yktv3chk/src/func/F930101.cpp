...
static int SaveDoorJSBSerial(T_t_attend_txdtl* tTxdtl)
{
	...
	if (ret)
	{
		writelog(LOG_DEBUG,"serial_no[%d]phycardid[%s]cardno[%d]deviceid[%s]tx_date[%s]tx_time[%s]",
			tTxdtl->serial_no,tTxdtl->phycardno,tTxdtl->cardno,tTxdtl->deviceid,tTxdtl->tx_date,tTxdtl->tx_time);
		if(DB_REPEAT==ret)
			return 0;
		else
