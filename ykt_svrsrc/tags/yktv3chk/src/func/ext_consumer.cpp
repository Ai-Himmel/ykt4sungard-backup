...
//	ext_phone_trans_t phone_trans_struct;
	...
	memcpy(send_data+25,phone_trans_struct->custid,22);
...
static int get_third_part_account(ST_PACK *in_pack,int trade_code,InAcc *IA){
	...
	//根据商户ID查询得到商户信息
	ret=DB_t_shop_read_by_shop_id(shpid, &shop);
	if(ret)
	{
	...
	ret=DB_t_aif_account_read_by_customer_id_and_act_type(shop.custid,ACCTYPE_SHOPMAIN,&account);
	if(ret)
	{
	...
	memset(&account,0,sizeof(account));
	ret=DB_t_aif_account_read_by_cardno_and_purseno(in_pack->lvol0,0,&account);
	if(ret)
	{
...
static int pre_check_term_trade(ST_PACK *in_pack,ST_PACK *out_pack,transfer_control_t *trans_ctl)
{
	...
	ret = get_shpId_by_devId_date_time(&shop_id,tDevice.deviceid,in_pack->sdate0
		,curr_time,0);
	if(ret)
	{
		return ret;
	...
	ret = DB_t_device_read_lock_by_cur_and_deviceid(in_pack->lvol4,&tDevice);
	if(ret)
	{
		if(DB_NOTFOUND == ret)
...
static int pre_check_fee(ST_PACK *in_pack,ST_PACK *out_pack,transfer_control_t *trans_ctl)
{
	...
	ret = get_stuempno_by_cardno(in_pack->lvol0,stuempno);
	if (ret)
	{
		return ret;
	}
...
static int search_account_from_term_id(ST_PACK *in_pack,int trade_code,InAcc *IA)
{
	...
	ret = DB_t_device_read_by_deviceid(in_pack->lvol4,&tDevice);
	if(ret)
	{
		if(DB_NOTFOUND == ret)
			return E_DB_DEVICE_N;
	...
	ret = DB_t_shop_read_by_shop_id(shop_id,&tShop);
	if(ret)
	{
		if(DB_NOTFOUND == ret)
			return E_DB_SHOP_N;
	...
	memset(&tAccount,0,sizeof tAccount);
	ret=DB_t_aif_account_read_by_customer_id_and_act_type(tShop.custid,ACCTYPE_SHOPMAIN,&tAccount);
	if(ret)
	{
		writelog(LOG_ERR,"read_by_customer_id_and_act_type ret[%d]custid[%d]",ret,tShop.custid);
		if(DB_NOTFOUND==ret)
			return  E_SHOP_ACTNO_NOT_EXIST;
		else
			return E_DB_ACCOUNT_R;
...
static int do_term_trade(ST_PACK *in_pack,char ext_msg[256])
{
	...
	else if(!ret)
	{
		...
		ret = DB_t_device_update_lock_by_cur(&tDevice);
		if(ret)
		{
...
//系统正式入账
 int enter_account(ST_PACK *in_pack,InAcc *IA,ST_PACK *out_pack){
	...
	strcpy(IA->sTxAccDate,tradeserial.enteract_date);		//记账日期
	strcpy(IA->sTxAccTime,tradeserial.enteract_time);		//记账时间
	strcpy(IA->sMdeOper,tradeserial.opercode);			//操作员
