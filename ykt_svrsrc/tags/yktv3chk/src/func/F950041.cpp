...
int F950041(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	else if((ret = DB_t_device_read_by_deviceid(rPack->lcert_code,&tDevice)))
	{
		if(DB_NOTFOUND == ret)
	...
	else if((ret = DB_t_shop_pos_open_select_by_c0_and_deviceid(rPack->lcert_code)))
	{
	...
	else
	{
		while(1)
		{
			...
			if((ret = DB_t_shop_pos_fetch_select_by_c0(&tShopPos)))
			{
				if(ret == DB_NOTFOUND)
				{
			...
			if((ret = DB_t_shop_read_by_shop_id(tShopPos.shop_id,&tShop)))
			{
				*pRetCode = ret;
				break;
			}
	...
	ret = DB_t_feetype_open_select_by_cur1();
	if(ret)
	{
		if(DB_NOTFOUND == ret)
	...
	{
		memset(&tFeeType,0,sizeof &tFeeType);
		ret = DB_t_feetype_fetch_select_by_cur1(&tFeeType);
		if(ret)
		{
			if(DB_NOTFOUND == ret)
			{
		...
		rows++;
		if(tFeeType.feetype < 0 || tFeeType.feetype > 255
			|| tFeeType.is_outfee[0] != '1' )
			continue;
		writelog(LOG_DEBUG,"fee[%s][%d][%c]",tFeeType.fee_name,tFeeType.feetype
			,tFeeType.is_outfee[0]);
		_SET_BIT(buf,tFeeType.feetype-1);
