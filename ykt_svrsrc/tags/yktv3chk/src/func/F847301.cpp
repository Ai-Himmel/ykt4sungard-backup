...
//    char temp_account_id[16 + 1] = "2081";             //个人临时帐户
    ...
    */
    ...
    if (strncmp(tCard.cardattr, TYPE_YES, 1) != 0)
    {
        writelog(LOG_ERR,"card_state=[%s]",tCard.cardattr);
        *pRetCode = E_TRANS_SCHACC_DESTORY;
        goto L_RETU;
    }

    if (STATE_TRUE == tCard.cardattr[CARDSTAT_TYPE_LOST])
    {
        writelog(LOG_ERR,"card_state=[%s]",tCard.cardattr);
    ...
    if (STATE_TRUE == tCard.cardattr[CARDSTAT_TYPE_FREEZE])
    {
        writelog(LOG_ERR,"card_state=[%s]",tCard.cardattr);
    ...
    if (STATE_TRUE == tCard.cardattr[CARDSTAT_TYPE_WFAIL])
    {
