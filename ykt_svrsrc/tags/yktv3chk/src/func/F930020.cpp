...
int F930020(TRUSERID *handle,int iRequest,ST_PACK *in_pack,int *pRetCode,char *szMsg)
{
	...
	AddXmlItemInt(tMesList.incontent,XML_KEY_SERIALNO,in_pack->lvol4);
	AddXmlItemInt(tMesList.incontent,XML_KEY_OUTNUM,in_pack->lvol6);
	AddXmlItemInt(tMesList.incontent,XML_KEY_SUBSIDYFEE,in_pack->lvol7);
	AddXmlItemStr(tMesList.incontent,XML_KEY_SUBSIDYTYPE,in_pack->sstatus0);
	AddXmlItemStr(tMesList.incontent,XML_KEY_SUBSIDYFLAG,in_pack->scurrency_type);
	AddXmlItemStr(tMesList.incontent,XML_KEY_ENDDT,in_pack->sdate1);
