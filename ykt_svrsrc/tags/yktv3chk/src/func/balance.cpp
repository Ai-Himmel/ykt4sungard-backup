...
//			}
		...
		//广播黑名单
		AddXmlItemInt(tMesList.incontent, XML_KEY_CARDID,cardno);
...
int SearchLeafshop_and_Trans(int shop_id,char* Independ_or_not)
{
	...
	ret=DB_t_shop_read_by_shop_id(shop_id,&t_shop);
	if(ret)
	{
	...
	//循环判断下边的子节点是否为叶子商户
	ret=DB_t_shop_open_select_by_cur2_and_dept_id(shop_id);
	if(ret)
	{
		writelog(LOG_ERR,"DB_t_shop_open_select_by_cur2_and_dept_id ret[%d]shop_id[%d]",ret,shop_id);
		return E_DB_SHOP_R;
	}
	while(1)
	{
		...
		if(ret)
		{
			if(ret==DB_NOTFOUND)
			{
				if(cnt==0)
					{
						writelog(LOG_DEBUG,"DB_t_shop_fetch_select_by_cur2 have not record,shop_id=[%d]!",t_shop.shop_id);
						break;
					}
				else
					break;
...
*/
	...
	while(1)
	{
		...
		t_tradelog_his.feetype           =t_tradelog.feetype  ;
		...
		t_tradelog_his.maindeviceid      =t_tradelog.maindeviceid ;
		t_tradelog_his.deviceid          =t_tradelog.deviceid     ;
	...
	while(1)
	{
		...
		t_tradeserial_his.maindeviceid  =t_tradeserial.maindeviceid ;
		t_tradeserial_his.deviceid      =t_tradeserial.deviceid     ;
		strncpy(t_tradeserial_his.devphyid   ,t_tradeserial.devphyid  	,sizeof(t_tradeserial_his.devphyid   ));
		strncpy(t_tradeserial_his.showid         ,t_tradeserial.showid        	,sizeof(t_tradeserial_his.showid         ));
		t_tradeserial_his.cardno        =t_tradeserial.cardno       ;
		t_tradeserial_his.purseno       =t_tradeserial.purseno      ;
		...
		t_tradeserial_his.depositfee    =t_tradeserial.depositfee   ;
		...
		t_tradeserial_his.custid    =t_tradeserial.custid   ;
		strncpy(t_tradeserial_his.opercode      ,t_tradeserial.opercode     	,sizeof(t_tradeserial_his.opercode      ));
...
//	T_t_shop 			t_shop;
//	T_t_shop_log		t_shop_log;
	T_t_shop_pos		t_shop_pos;
	T_t_shop_pos_log	t_shop_pos_log;
	T_t_syskey 		t_pif_syskey;
...
//	strcat(tmp_date,"000000");
	...
	//删除商户信息表的数据
	ret=DB_t_shop_del_all();
	if((0!=ret)&&(100!=ret))
	{
		writelog(LOG_ERR,"DB_t_shop_del error,errorcode[%d]",ret);
		return 	E_DB_SHOP_D;
	}
	//拷贝临时信息表的数据到商户信息表中

	ret=DB_t_shop_log_open_select_by_cur2_and_updatedate(logic_date);
	if(ret)
	{
		writelog(LOG_ERR,"DB_t_shop_log_open_select_by_cur1 error,errorcode[%d]",ret);
		return 	ret;
	}
	while(1)
	{
		...
		ret=DB_t_shop_log_fetch_select_by_cur2(&t_shop_log);
		if(ret)
		{
			if(ret==DB_NOTFOUND)
			{
				...
				writelog(LOG_DEBUG,"DB_t_shop_log_fetch_select_by_cur2 error,errcode=[%d]!",ret);
				return   E_DB_SHOPDEPTFEE_R;
		}

		//strncpy(t_shop.updatedate  ,logic_date, sizeof(t_shop.updatedate  ));
		t_shop.shop_id		  =t_shop_log.shop_id;
		t_shop.custid             =t_shop_log.custid ;
		...
		strncpy(t_shop.addr     ,t_shop_log.addr    , sizeof(t_shop.addr     ))      ;
		...
		strncpy(t_shop.opendate     ,t_shop_log.opendate  ,   sizeof(t_shop.opendate     ))   ;
		strncpy(t_shop.comments    ,t_shop_log.comments   , sizeof(t_shop.comments    ))      ;

		cnt++;
		ret=DB_t_shop_add(&t_shop);
		if(ret)
		{
			writelog(LOG_ERR,"Insert DB_t_shop_add record[%d],err[%d]",cnt,ret);
			db_rollback();
			return 	E_DB_SHOP_I;
		}
		strncpy(t_shop_log.updatedate,next_date,sizeof(t_shop_log.updatedate));
		ret=DB_t_shop_log_add(&t_shop_log);
		if(ret)
		{
			writelog(LOG_ERR,"DB_t_shop_log_add record[%d],err[%d]",cnt,ret);
			db_rollback();
			return ret;
	...
	*/
	...
	//拷贝临时信息表的数据到商户信息表中
	ret=DB_t_shop_pos_log_open_select_by_cur1_and_updatedate(logic_date);
	if(ret)
	{
		writelog(LOG_ERR,"DB_t_shop_pos_log_open_select_by_cur1_and_updatedate error,errorcode[%d]",ret);
		return 	ret;
	}
	while(1)
	{
		...
		ret=DB_t_shop_pos_log_fetch_select_by_cur1(&t_shop_pos_log);
		if(ret)
		{
			if(ret==DB_NOTFOUND)
			{
			...
			{
				writelog(LOG_DEBUG,"DB_t_shop_pos_log_fetch_select_by_cur1 error,errcode=[%d]",ret);
				return   ret;
			}
		}

		t_shop_pos.id            =t_shop_pos_log.id        ;
		t_shop_pos.deviceid     =t_shop_pos_log.deviceid ;
		...
		strncpy(t_shop_pos.starttime    ,t_shop_pos_log.starttime ,sizeof(t_shop_pos.starttime  ));
		strncpy(t_shop_pos.endtime      ,t_shop_pos_log.endtime   ,sizeof(t_shop_pos.endtime    ));
		...
		cnt++;
		ret=DB_t_shop_pos_add(&t_shop_pos);
		if(ret)
		{
			writelog(LOG_ERR,"Insert DB_t_shop_pos record[%d],err[%d]",cnt,ret);
			db_rollback();
			return 	E_DB_SHOP_POS_I;
		}
		strncpy(t_shop_pos_log.updatedate,next_date,sizeof(t_shop_pos_log.updatedate));
		ret=DB_t_shop_pos_log_add(&t_shop_pos_log);
		if(ret)
		{
			writelog(LOG_ERR,"DB_t_shop_pos_log_add record[%d],err[%d]",cnt,ret);
			db_rollback();
			return ret;
	...
	ret=DB_t_syskey_read_lock_by_cur_and_key_code(KEYTYPE_TRADESERIAL, &t_pif_syskey);
	if(ret)
	{
	...
	t_pif_syskey.keyvalue=1;
	ret=DB_t_syskey_update_lock_by_cur(&t_pif_syskey);
	if(ret)
	{
	...
	while(1)
	{
		...
		else
		{
			...
			while(1)
			{
				...
				if(ret)
				{
					writelog(LOG_ERR,"DB_t_shop_read_by_shop_id error,shop_id=[%d],errcode=[%d]",t_shop.dept_id,ret);
					return   ret;
				}
