...
int F846307(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	if(lossflag=='1')
	{
		...
		tCard.cardattr[CARDSTAT_TYPE_LOST]='1';
		tCard.lossflag[0]='1';
		getsysdate(tCard.lossdate);		
		pAccTrans->trans.transcode=TC_CARDLOSS;
	}
	else
	{
		if(tCard.cardattr[CARDSTAT_TYPE_LOST]=='0')
		{
				return E_CARDNO_NOLOST;
		}
		tCard.cardattr[CARDSTAT_TYPE_LOST]='0';
		tCard.lossflag[0]='0';
		strcpy(tCard.lossdate,"");
		pAccTrans->trans.transcode=TC_CARDUNLOSS;
	}
