...
#define MAXNUM_CFGSPLIT		90		//最大交易类型个数
...
{
	int num;
	T_t_cfgsplit  ArrCfgsplit[MAXNUM_CFGSPLIT];
}CFG_SPLIT;
typedef struct
{
	int num;
...
int GetIndexTxFeeCfgByTxCode(int tx_code,int feetype)
{
	register int index=0;
	for(index=0;index<CfgFee.num;index++)
	{
		if((CfgFee.ArrCfgFee[index].tx_code==tx_code)&&(CfgFee.ArrCfgFee[index].feetype==feetype))
			return index;
	}
	return -1;
}
...
//	int idx=0;
	...
	T_t_cardaccount  tCardAccount;
	T_t_shopaccount  tShopAccount;
	T_t_inneraccount tInnerAccount;

	memset(&tCardAccount,0,sizeof(tCardAccount));
	memset(&tShopAccount,0,sizeof(tShopAccount));
	memset(&tInnerAccount,0,sizeof(tInnerAccount));

	T_t_cardaccdtl	tCardAccDtl;
	T_t_inneraccdtl tInnerAccDtl;
	T_t_shopacctdtl tShopAccDtl;
	...
	if(idxTxCfg<0)
	{
		...
		if(idxTxCfg<0)
		{
			writelog(LOG_ERR,"tx_code[%d]feetype[%d]",IA->iTradeNo,IA->iFeeType);
...
//			case TXTYPE_TOLL_DEPOSIT:		//收押金
			...
			case TXTYPE_DEDUCT_DEPOSIT:	//扣押金
				...
				ret=DB_t_card_read_by_cardno(IA->iCardNo,&tCard);
				if(ret)
				{
					if(DB_NOTFOUND==ret)
...
//		writelog(LOG_DEBUG,"formula[%s]num_exp[%s]amt[%.4lf]",sFormula,num_exp,dExpCalAmt);
		...
		if(tAccountDb.status == ACCOUNTSTAT_LOGOUT)
		{
	...
	//	writelog(LOG_DEBUG,"debit No[%d]account[%s]account.cur_bala[%lf]cur_freebala[%lf]cur_frozebala[%lf]amt[%lf]",i,account.account_id,account.cur_bala,account.cur_freebala,account.cur_frozebala,tTradelogDb.op_fee);
			...
			if(strncmp(tAccountDb.subno,"201",3)==0)
			{
				...
				switch(CfgSplit.ArrCfgsplit[iTxType].fundtype)
				{
					...
					default:
						...
						//更新卡余额
						if(USE_CARD_TYPE_NEWCARD==IA->iUseCardFlag)
						{
							getsysdate(tAccountDb.reserve_1);
							IA->dOutCardBala=tAccountDb.cur_freebala;
							tAccountCr.card_balance=tAccountDb.cur_freebala;
							if(IA->iTxCnt>0)
								tAccountDb.consume_count=IA->iTxCnt;			//消费次数增加
						}
						else if(USE_CARD_TYPE_ONLINE==IA->iUseCardFlag)
						{
							//联机交易保留交易金额的交易时间
							getsysdate(tAccountDb.reserve_1);
						...
						{
							if((tAccountDb.consume_count<=IA->iTxCnt)||(strncmp(IA->sTxDate,tAccountDb.reserve_1,8)>0))
							{
			...
			else
			{
				...
				switch(iTxType)
				{
					...
					case TXTYPE_DEDUCT_DEPOSIT:	//扣押金
						...
						ret=DB_t_card_read_lock_by_cur_and_card_id(IA->iCardNo,&tCard);
						if(ret)
						{
							if(DB_NOTFOUND==ret)
								return E_CARDNO_NOT_EXIST;
			...
			{			
				if(strcmp(CfgSplit.ArrCfgsplit[iTxType].dbsubno,CfgSplit.ArrCfgsplit[iTxType].crsubno)!=0)
				{
					if(strncmp(CfgSplit.ArrCfgsplit[iTxType].crsubno,SUBJECT_INDIVIDUALSAVING,3)==0)
						...
						strcpy(tTradelogCr.act_id, IA->sArrInActno[3]);	//贷方账号						
				}
				else
				{
		...
		if(tAccountCr.status == ACCOUNTSTAT_LOGOUT)
		{
			DB_t_aif_account_free_lock_cur();
...
//		writelog(LOG_DEBUG,"credit No[%d]tAccount[%s]account.cur_bala[%lf]cur_freebala[%lf]cur_frozebala[%lf]amt[%lf]",i,account.account_id,account.cur_bala,account.cur_freebala,account.cur_frozebala,ArrTradelog[i+1].op_fee);
				...
				switch(CfgSplit.ArrCfgsplit[iTxType].fundtype)
				{
					...
					default:
						...
						else  if(USE_CARD_TYPE_ONLINE==IA->iUseCardFlag)
						{
							...
							{
								//有入卡值,没有出卡值,计算出卡值,卡余额
								dCardBala = dCardBala + D4U5(tTradelogCr.op_fee,2);	//卡余额
								IA->dOutCardBala=D4U5(dCardBala,2);
						...
						{
							if((tAccountCr.consume_count<=IA->iTxCnt)||(strncmp(IA->sTxDate,tAccountCr.reserve_1,8)>0))
							{
			...
			else
			{
				...
				switch(iTxType)
				{
					...
					case TXTYPE_TOLL_DEPOSIT_FUNDBOOK:
						...
						ret=DB_t_card_read_lock_by_cur_and_cardno(IA->iCardNo,&tCard);
						if(ret)
						{
							DB_t_aif_account_free_lock_cur();
							if(DB_NOTFOUND==ret)
						...
						tCard.depositfee=tTradelogCr.op_fee;
						ret=DB_t_card_update_lock_by_cur(&tCard);
						if(ret)
						{
							DB_t_aif_account_free_lock_cur();
		...
		DB_t_aif_account_free_lock_cur();
		//增加借方分录流水
		tTradelogDb.maindeviceid = IA->iMainDevId;		//工作站标识
		tTradelogDb.deviceid= IA->iDevId;				//终端设备ID
		tTradelogDb.serino = IA->iSerialNo;				//流水号
		tTradelogDb.seri_type= IA->iTradeNo;				//交易码
		tTradelogDb.feetype=iTxType;						//分录序号
		...
		//增加贷方分录流水
		tTradelogCr.maindeviceid = IA->iMainDevId;		//工作站标识
		tTradelogCr.deviceid= IA->iDevId;				//终端设备ID
		tTradelogCr.serino = IA->iSerialNo;				//流水号
		tTradelogCr.seri_type = IA->iTradeNo;				//交易码
		tTradelogCr.feetype=iTxType;						//分录序号
		...
		{
			if(db_amt_flag>0)
				sprintf(pMessage+strlen(pMessage),"%s:余额增加%.2lf元", tAccountDb.custname,dExpCalAmt);
			else
				sprintf(pMessage+strlen(pMessage),"%s:余额减少%.2lf元", tAccountDb.custname,dExpCalAmt);
			if(cr_amt_flag>0)
				sprintf(pMessage+strlen(pMessage)," %s:余额增加%.2lf元",tAccountCr.custname,dExpCalAmt);
			else
				sprintf(pMessage+strlen(pMessage)," %s:余额减少%.2lf元", tAccountCr.custname,dExpCalAmt);
		}
	}
