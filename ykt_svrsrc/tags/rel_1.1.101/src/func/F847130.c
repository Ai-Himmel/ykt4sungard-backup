...
int F847130(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	des2src(tSubsidyTmp.stuemp_no,rPack->scust_auth);	//学号
	des2src(tSubsidyTmp.subsidy_no,rPack->sdate0);		//补助批次号
	...
	if(ret)
	{
		sprintf(szMsg,"学号%s的日期格式不正确,正确格式为YYYYMMDD",tSubsidyTmp.stuemp_no);
		*pRetCode=ret;
		goto L_RETU;
	}
	...
	{
		//最后一条,开始把临时表数据导入到正式表
		writelog(LOG_DEBUG,"导入第%d条记录,批次号:%s,学号:%s",tSubsidyTmp.seqno,tSubsidyTmp.batch_no,tSubsidyTmp.stuemp_no);
		iCnt=0;
		...
		while(1)
		{
			...
			if(ret)
			{
				...
				sprintf(szMsg,"批次号%s,第%d条学号为%s的记录有误,可能该学号的补助记录已经存在",tSubsidy.batch_no,tSubsidy.seqno,tSubsidy.stuemp_no);
				goto L_RETU;
			}
	...
	if(flag!=2)
	{
		sprintf(out_pack->vsmess,"导入第%d条记录,批次号:%s,学号:%s",tSubsidyTmp.seqno,tSubsidyTmp.batch_no,tSubsidyTmp.stuemp_no);
	}
