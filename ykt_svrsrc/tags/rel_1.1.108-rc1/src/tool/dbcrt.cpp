...
static pcrecpp::RE re_table_field("(\\w+)[ \\t]+(character|char|varchar2|varchar|integer|double|float|number\
|time|date|datetime|decimal)(\\([0-9,]+\\))?[ \t]?([^,]*)"
,pcrecpp::RE_Options().set_caseless(true));

static pcrecpp::RE re_action_define("action[ \\t]+(modename|read_by|read_or_by|update_by\
|del_by|read_lock_by|update_lock_by|free_lock_by|free_lock|open_select_by|fetch_select_by\
|close_select_by|add|open_select_between|open_select_for_update_by|read_blur_by\
...
static int do_parse_field(const string &field_name,const string& field_type,const string& field_len,const string &field_other)
{
	...
	{
		//printf("length [%s]\n",field_len.c_str());
		for(i = 0,j = 0;i < field_len.length();++i)
		{
			if(field_len[i] == '(')
				continue;
	...
	else if( (strncmp(dbtype,"number",6) == 0) || (strncmp(dbtype,"decimal",7) == 0))
	{
		if(prec > 0)
