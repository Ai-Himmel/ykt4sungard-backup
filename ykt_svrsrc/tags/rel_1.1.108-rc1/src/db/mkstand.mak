...
PREPCOMP_CXX=proc code=cpp cpp_suffix=cpp maxopencursors=50 MODE=ANSI TYPE_CODE=ANSI CLOSE_ON_COMMIT=NO def_sqlcode=yes dynamic=ANSI define=ESQL_ORA LINES=YES config="$(HOME)/ykt/bin/proccfg.cfg" include="/usr/include"  include="$(ORACLE_HOME)/precomp/public" include="$(ORACLE_HOME)/rdbms/public" include="$(HOME)/ykt/src/buinc" include="$(HOME)/ykt/src/include" include="$(HOME)/ykt/src/include/dbincl" include="$(HOME)/ykt/src/public"
