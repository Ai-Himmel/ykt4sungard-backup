...
ifeq ($(UNAME),SunOS)
	...
	CCFLAGS=-D_DRTP_V4 -D_MT -DSUNOS -Wall
...
ifeq ($(UNAME),HP-UX)
	...
	LIBS=-lpthread 
	CCFLAGS= -mt -D_DRTP_V4 -DUNIX -DHP_UX -Agcc -AC99 -Wall
	CXXFLAGS= -mt -D_DRTP_V4 -DUNIX -DHP_UX -Ag++ -Wall
endif
ifeq ($(UNAME),Linux)
	CC=gcc
	LIBS=-lstdc++ -lpthread -lnsl
	CCFLAGS= -D_DRTP_V4 -D_MT -Wall -O2 -DLINUX -Wall
	CXXFLAGS= -D_DRTP_V4 -D_MT -Wall -O2 -DLINUX -Wall
endif
ARFLAGS=-r
...
.sqc.ec:
	@if [ $(DBTYPE) = DB2 ]; \
	then \
		$(PREPROCESSOR_C) -DESQL_DB2 $< > $@; \
	else \
		$(PREPROCESSOR_C) -DESQL_ORA $< > $@; \
	fi

.sqC.eC:
	@if [ $(DBTYPE) = DB2 ]; \
	then \
		$(PREPROCESSOR_CXX) -DESQL_DB2 $< > $@; \
	else \
		$(PREPROCESSOR_CXX)  -DESQL_ORA $< > $@; \
	fi
