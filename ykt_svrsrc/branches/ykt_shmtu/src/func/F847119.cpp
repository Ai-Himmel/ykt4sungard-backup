...
static int do_process(T_t_pif_card *tCard, T_t_tif_writefailed *tWriteFailed, ST_PACK *rPack, T_t_tif_tradeserial *tradeserial)
{
	...
	case 240104://领补助
		...
		else if((card_use_cnt+1)>seri_use_cnt)
		{
			...
			{
                        if (amtcmp((seri_in_bala_next-seri_out_bala),0.0)==0) 
                        	{
					writelog(LOG_ERR,"已经写卡成功了",ret);
					amt_flag=1;
	...
	ret = GetParameter(GLOBE_MAXCARDBALANCE,temp);
	if(ret)
	{
		goto L_RETU_INNER;
	}
	max_purse_amount = D4U5(atof(temp),2);
	if(tradeserial->out_balance<0||amtcmp(tradeserial->out_balance,max_purse_amount) > 0)
	{
...
int F847119(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	// 更新账户表的卡余额
	ret = DB_t_aif_account_read_lock_by_c0_and_card_id_and_purse_id(rPack->lvol0,0,&tAccount);
	if(ret)
	{
	...
	tAccount.card_balance = tradeserial.out_balance;
	getsysdatetime(tAccount.reserve_1);
