...
{	
	...
	ret = DB_t_tif_savdtl_read_lock_by_cur_and_tx_date_and_seqno(
		tTradeserial->operate_date,tTradeserial->other_seri_no,&tSavdtl);
	if(ret)
	{
		if(DB_NOTFOUND != ret)
...
int doRev240104(T_t_tif_tradeserial *tTradeserial)
{
	int ret=0;
	int row=0;
	...
	ret=DB_t_tif_subsidy_open_select_for_update_by_c1_and_cust_no_and_status(tTradeserial->customer_id, "3");
	if(ret)
	{
	...
	while(1)
	{
		...
		if((memcmp(tSubsidy.get_date,tTradeserial->operate_date,8)==0)
			&&(memcmp(tSubsidy.get_time,tTradeserial->operate_time,6)==0))
		{
			tSubsidy.status[0]='2';
	...
	ret = DB_t_tif_writefailed_read_by_card_id_and_serial_no_and_tx_date
		(tTradeserial->card_id,tTradeserial->other_seri_no,tTradeserial->operate_date,&tWriteFailed);
	if(ret)
	{
		if(ret != DB_NOTFOUND)
	...
	{
		getsysdate(tWriteFailed.deal_date);
		getsystime(tWriteFailed.deal_time);
		tWriteFailed.deal_serial_no = tTradeserial->serial_no;
		tWriteFailed.deal_flag[0]='N';
		ret =DB_t_tif_writefailed_update_by_card_id_and_serial_no_and_tx_date
			(tTradeserial->card_id,tTradeserial->other_seri_no,tTradeserial->operate_date,&tWriteFailed);
		if(ret)
		{
			return E_DB_WRITEFAILED_U;
		}
	}
	return 0;
}
