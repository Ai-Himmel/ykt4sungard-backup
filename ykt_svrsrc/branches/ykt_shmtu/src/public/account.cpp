			...
			if(strncmp(tAccountDb.subno,"201",3)==0)
			{
				...
				switch(CfgSplit.ArrCfgsplit[iTxType].fundtype)
				{
					...
					default:
						...
						{
						    strcpy(sTxDateandtime,IA->sTxDate);
						    strcat(sTxDateandtime,IA->sTxTime);
							if((tAccountDb.consume_count<=IA->iTxCnt)||(strncmp(sTxDateandtime,tAccountDb.reserve_1,14)>=0))
							{
								strcpy(tAccountDb.reserve_1,sTxDateandtime);
								if(amtcmp(dOutCardBala,0)>=0)
								{
				...
				switch(CfgSplit.ArrCfgsplit[iTxType].fundtype)
				{
					...
					default:
						...
						else if(USE_CARD_TYPE_OFFLINE==IA->iUseCardFlag)
						{
						    strcpy(sTxDateandtime,IA->sTxDate);
						    strcat(sTxDateandtime,IA->sTxTime);
							if((tAccountCr.consume_count<=IA->iTxCnt)||(strncmp(sTxDateandtime,tAccountCr.reserve_1,14)>0))
							{
								strcpy(tAccountCr.reserve_1,sTxDateandtime);
								if(IA->iTxCnt>0)
									tAccountCr.consume_count=IA->iTxCnt;			//消费次数增加
								if(amtcmp(dOutCardBala,0)>=0)
								{
