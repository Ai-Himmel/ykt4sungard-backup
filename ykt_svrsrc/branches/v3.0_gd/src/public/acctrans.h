...
#define TRANSTYPE_MOBILETRANS   		234 //消费退款
...
#define TRANSTYPE_SHOPBOARDFEE2		901	//搭伙费分成划拨
...
//    int  cardcnt;		//卡交易次数
    ...
    char draccno[11];	//输入值:借方账号
    char craccno[11];	//输入值:贷方账号
    int  fundtype;		//资金类型 1-现金2-支票3-经费本4-个人帐户存款5-银行存款
    char voucherno[21];	//凭证号
    char drsubjno[6];	//输出值:借方科目
    char crsubjno[6];	//输出值:贷方科目
...
public:
	...
	int GenTransdtl(T_t_transdtl *dtl,int status,const char *drtphandle = NULL);
private:
	int checkResult;
	T_t_card *checkCard;
