...
{
	int ret;
    memset(drtphandle_,0,sizeof drtphandle_);
...
void CPaymentBase::set_drtphandle(void *handle)
{
    GetTransHandle(handle,drtphandle_);
}

void CPaymentBase::get_drtphandle(void *handle)
{
    SetTransHandle(handle,drtphandle_);
}
...
//		return E_INPUTNULL_TERMID;
	...
	ret = pAccTrans->GenTransdtl(&dtl_,DTLSTATUS_INIT,drtphandle_);
 	if(ret)
	{
...
int CPaymentBase::get_refno()
{
    ...
    des2src(tRefno.drtphandle,drtphandle_);
...
int CNormalPayTrans::PackData(ST_PACK *rPack,ST_CPACK& sPack)
{
	...
	sPack.pack.lvol1=D4U5(pAccTrans->trans.inputamt*100,0);		//卡号
	sPack.pack.lvol7=D4U5(pAccTrans->trans.cardbefbal*100,0);	//卡余额
...
int CGDCTPayment::pre_transfer(TRUSERID* handle,ST_PACK *in_pack,ST_PACK *out_pack)
{
	...
	strcpy(sPack.pack.sbank_code,"1000");
    strcpy(sPack.pack.vsvarstr1,drtphandle_);
