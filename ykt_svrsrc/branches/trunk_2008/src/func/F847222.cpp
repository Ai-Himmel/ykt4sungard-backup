...
static int do_process_trade(trade_param_t * param)
{
	...
	if(shop.shop_state != SHOPSTAT_REGIST)
		return 0;
	...
	if(ret)
	{
		...
		sprintf(param->szMsg,"商户号[%d]未开启",shop.shop_id);
		if(DB_NOTFOUND == ret)
			return E_FAN_ACC_NOT_EXISTS;
		return E_DB_ACCOUNT_R;
	}
	if(account.current_state != ACCOUNTSTAT_REGIST)
	{
		sprintf(param->szMsg,"商户[%d: %s]已销户",param->out_shop_id,shop.shop_name);
	...
	if(ret)
	{
		...
		sprintf(param->szMsg,"商户[%d: %s]入账失败",shop.shop_id,shop.shop_name);
		return ret;
	}
	// 更新账户余额
