...
static int do_fronze_by_stuempno(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	if(strlen(rPack->scust_auth)>0)
		{
			ret = get_cut_id_by_stuemp_no(rPack->scust_auth,&cust_id);
			if(ret)
			{
				writelog(LOG_ERR,"find cust_id by stuemp_no error, stuemp_no[%s]",rPack->scust_auth);
				return ret;
			}
	}
	else if(strlen(rPack->scust_no)>0)
		{
	...
	if(ret)
	{
		writelog(LOG_ERR,"stuemp_no not exists [%s]",rPack->scust_auth);
		if(DB_NOTFOUND == ret)
			return DB_NOTFOUND;
		return E_DB_CARD_R;
	}
...
static int do_fronze_by_stuempno_ex(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	if(ret)
	{
		writelog(LOG_ERR,"froze card error ,stuemp_no_1[%s],stuemp_no2[%s],ret[%d]",rPack->scust_no,rPack->scust_auth,ret);
		if(db_rollback())
		{
	...
	writelog(LOG_DEBUG,"froze card success , stuemp_no[%s] ",rPack->scust_auth);
	return 0;
}
int F847117(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	int ret;
  if((strlen(rPack->scust_auth)>0)||(strlen(rPack->scust_no)>0))
	{
		// 通过学号挂失
