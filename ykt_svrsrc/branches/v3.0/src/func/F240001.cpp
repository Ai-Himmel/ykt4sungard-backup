...
int F240001(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	*/
	...
	ret=DB_t_transdtl_read_lock_by_c0_and_accdate_and_termid_and_termseqno(pAccTrans->trans.sysdate,trans.termid,pAccTrans->trans.termseqno, &transdtl);
	if(ret)
	{
	...
	trans.cardcnt=transdtl.cardcnt;
	...
	des2src(out_pack->sdate0,pAccTrans->trans.sysdate);
