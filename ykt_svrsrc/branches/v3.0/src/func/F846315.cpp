...
//		DB_t_transdtl_free_lock_by_c0();
	...
	if(vTransdtl.cardcnt>0&&vTransdtl.cardno>0)
	{
		ret=UpdateCardBitmap(vTransdtl.cardno,vTransdtl.cardcnt,CARDBITMAPTYPE_RESET,0);
		if(ret)
			return ret;
	}
	memcpy(&transdtl,&vTransdtl,sizeof(transdtl));
	...
	if('1'!=authrightflag)
	{
		transdtl.cardaftbal=vTransdtl.cardbefbal;
		transdtl.cardbefbal=vTransdtl.cardaftbal;
