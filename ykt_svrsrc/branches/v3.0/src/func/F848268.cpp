...
static int do_848268(TRUSERID *handle,int iRequest,ST_PACK *in_pack,int *pRetCode,char *szMsg)
{
	...
	ret = DB_t_doordevcardlist_read_by_deviceid_and_custid(
	...
	// 判断是否已经删除过
	if(door_card.adddelflag[0] == ADDDELFLAG_DEL)
	{
	...
	getsysdatetime(door_card.updatetime);
	door_card.adddelflag[0] = ADDDELFLAG_DEL;
	door_card.downstatus = DOWNSTATUS_DOWN;
	ret = DB_t_doordevcardlist_update_by_deviceid_and_custid(
		in_pack->lvol1,in_pack->lvol0,&door_card);
	if(ret)
	{
