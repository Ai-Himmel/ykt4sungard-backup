...
int do_reverse_public_card(int card_no);

//服务费查询
//服务费充值:发卡预充值 + 校园卡充值
double get_svr_fee_charge(char *account_id,char *begin_date,char *end_date);

double subsidy_in_card(int cut_id,char *begin_date,char *end_date);

// 进入个人账户的金额
double get_person_acc_in(char *account_id,char *begin_date,char *end_date);

// 已经领取的服务费
double svr_fee_in_card(int cut_id,char *begin_date,char *end_date);

// 未领取的服务费
double svr_fee_no_in_card(int cut_id,char *begin_date,char *end_date);

//支取金额
double get_draw_money(char *account_id,char *begin_date,char *end_date);

int Get_svrfee_feetype(int svrfee_feetype[30],int *cnt);
//#ifdef __cplusplus
//}
//#endif
#endif
