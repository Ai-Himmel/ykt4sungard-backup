...
static int Is_right_type(int type,int fee_type)
{
	int	ret = 0;
	char *sep = ", ";
	char fee_list[40] = "";
	T_t_tif_cfgtxfee cfgtxfee;
	memset(&cfgtxfee,0,sizeof cfgtxfee);
	ret = DB_t_tif_cfgtxfee_read_by_tx_code_and_fee_type(TXCODE_CARD_OPEN_CASH,fee_type,&cfgtxfee);
	if(ret)
		return E_DB_CFGTXFEE_R;
	des2src(fee_list,cfgtxfee.fee_list);

	char *token = strtok(fee_list,sep);
	while( token != NULL )
   	{	
   		if(type == atoi(token))
			return 0;
...
int F847101(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	ret = Is_right_type(type,tCustomer.fee_type);
	if(ret)
