...
static int Is_used(int card_no)
{
	int ret = 0;
	T_t_aif_account tAccount;
	memset(&tAccount,0,sizeof tAccount);
	
	ret = DB_t_aif_account_read_by_card_id_and_purse_id(card_no,PURSE_NO_ONE,&tAccount);
	if(ret)
	{
		if(DB_NOTFOUND ==ret)
...
int F847183(TRUSERID *handle, int iRequest, ST_PACK *rPack, int *pRetCode, char *szMsg)
{
	...
	old_seri_type = tTradeserial.serial_type;
	switch (old_seri_type)
	{
	case 847106: // 现金充值冲正		
	...
	des2src(tTradeserial.reserve_1,rPack->semail);		// 备注
	...
	switch (old_seri_type)		// 发卡冲正
	{
	case 847101: // 发正式卡
	case 847124:	//发记名临时卡
	case 847125: 	//发非记名临时卡
	case 847126: 	//发VIP卡
		out_pack->lvol4 = 1;			// 清空卡信息
		ret = do_reverse_public_card(card_no);
		if(ret)
			return ret;
		break;
	default:
	...
	SetCol(handle, 0);
	SetCol(handle, F_LSERIAL1, F_DAMT2, F_DAMT3,F_LVOL4,F_VSMESS, 0);
