...
int F240003(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	des2src(tradeserial.b_act_id,rPack->scust_auth);		//银行卡号－－－－更港添加数据库字段
	tradeserial.sys_id=rPack->lvol2;						//外部系统标识，此处为区分多个银行--- 农业银行为1
	tradeserial.condition_id=SELF_TRANS;				//自动/自助转帐标识
	des2src(tradeserial.oper_code,OPER_SYSTEM_KEY);	//操作员代码
	tradeserial.other_seri_no=rPack->lvol0;				//原始流水号
	tradeserial.tmark=0;		
