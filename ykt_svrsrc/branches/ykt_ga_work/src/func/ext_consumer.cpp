...
#define PRE_LOGIN_ELECT_TRANS				240203	 //电控转账
#define PRE_ENTER_ACCOUNT_WATER_TRANS_NEWINNEW	240204	//新中新水控转账
...
static int check_water_newinnew(ST_PACK *in_pack,ST_PACK *out_pack,transfer_control_t *trans_ctl){
	int ret = 0;
	char c_drtbranch[10]="";
	char c_bccbasefuncno[10]="";
	int DRTP_BRANCH =0;
	...
	ret=GetParameter(2401, c_drtbranch);
	if(ret){
		writelog(LOG_ERR,"get_parameter error,error code=[%d]",ret);
		return ret;
	}
	ret=GetParameter(2402, c_bccbasefuncno);
	if(ret){
		writelog(LOG_ERR,"get_parameter error,error code=[%d]",ret);
		return ret;
	}
	...
	memset(&ArrayPack, 0, sizeof(ArrayPack));
	ResetNormalCPack(&sPack, 0, 1);
	SetHeadCol(&sPack, F_LVOL0,F_LVOL1,F_LVOL2,F_LVOL3,F_LVOL4,0);       // 设置发送windows BU的头部视图信息
	sPack.head.RequestType = 847352;
	sPack.pack.lvol3 = 1;
	sPack.pack.lvol4 = tCard.cosumer_id;
	ret = ExtCall(0, DRTP_BRANCH, BCC_BASEFUNCNO, 0, TIMEOUT, &sPack, &aPack, &ArrayPack);
	writelog(LOG_ERR,"get_parameter error,error code=[%d],retcode=[%d]",ret,aPack.head.retCode);
	if (ret < 0||aPack.head.retCode!=0)
	{
		if(ret<0)
		{
			writelog(LOG_ERR,"Call 847352 error,errcode=[%d],retCode=[%d]",ret,aPack.head.retCode);
			return ret;
		}
		else
		{
...
static int water_request(ST_PACK *in_pack,char ext_msg[256]){
	int ret = 0;
	char c_drtbranch[10]="";
	char c_bccbasefuncno[10]="";
	int DRTP_BRANCH =0;
	...
	DRTP_BRANCH = atoi(c_drtbranch);
	BCC_BASEFUNCNO = atoi(c_bccbasefuncno);
	writelog(LOG_ERR,"DRTP_BRANCH=[%d],BCC_BASEFUNCNO=[%d]",DRTP_BRANCH,BCC_BASEFUNCNO);
	ResetNormalCPack(&aPack, 0, 1);
	memset(&ArrayPack, 0, sizeof(ArrayPack));
	ResetNormalCPack(&sPack, 0, 1);
	SetHeadCol(&sPack, F_LVOL0,F_LVOL1,F_LVOL2,0);       // 设置发送windows BU的头部视图信息
	sPack.head.RequestType = 847352;
	
	sPack.pack.lvol0 = tCard.cosumer_id;//客户号
	sPack.pack.lvol1= D2I(in_pack->damt1*100);//交易金额
	sPack.pack.lvol2=0;//标识:存钱
	...
	if (ret < 0||aPack.head.retCode!=0)
	{
		if(ret<0)
		{
			writelog(LOG_ERR,"Call 847352 error,errcode=[%d],retCode=[%d],[%s]",ret,aPack.head.retCode,aPack.pack.vsmess);
			return ret;
		}
		else
		{
...
static const ext_consumer_request_t  g_transfer_process[] = {
	...
	{PRE_ENTER_ACCOUNT_WATER_TRANS_NEWINNEW,TXCODE_CONSUME,check_water_newinnew,get_third_part_account,water_request,NULL,NULL},
