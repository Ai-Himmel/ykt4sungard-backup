...
*******************************************************/
   ...
   // 参数  : const char * XpackDescribleFile [IN]: XPack结构定义文件名，不能超过1024字节长，如: "/xpacks/cpack.dat"
	BCCCLT_API  BCHANDLE NewXpackHandle(const char * XpackDescribleFile);
   ...
   // 参数  : BCHANDLE Handle [IN]: 指定的XPack数据区句柄
	BCCCLT_API  bool ResetPackHandle(BCHANDLE Handle);
   ...
   // 参数  : BCHANDLE DestHandle   [OUT]: 被复制的目标XPack数据区句柄，内部数据被覆盖，但不会新派生一个句柄
	BCCCLT_API  bool CopyHandle(BCHANDLE SourceHandle,BCHANDLE DestHandle);
   ...
   // 返回  : bool 
   // 参数  : BCHANDLE Handle [IN]: 指定的XPack数据区句柄
	BCCCLT_API  bool DeleteXpackHandle(BCHANDLE Handle);
   ...
   // 参数  : char * FieldName [IN]: 指定的字段名称,如"vsmess"
	BCCCLT_API  int GetFieldTypeByName(BCHANDLE Handle,char * FieldName);
   ...
   // 参数  : int* Value [OUT]: 返回得到的值
	BCCCLT_API  bool GetIntFieldByName(BCHANDLE Handle,int Row,char * FieldName,int * Value);
   ...
   // 参数  : double* Value [OUT]: 返回得到的值
	BCCCLT_API  bool GetDoubleFieldByName(BCHANDLE Handle,int Row,char * FieldName,double * Value);
   ...
   // 参数  : int ValueBufferSize [IN]: Value缓存区的长度（即可以存放的最大长度） 一般可以为 <=256
	BCCCLT_API  bool GetStringFieldByName(BCHANDLE Handle,int Row,char * FieldName,char * Value,int ValueBufferSize);
   ...
   // 参数  : char * FieldName [IN]: 字段名称 如"vsmess"
   // 参数  : int Value [IN]: 数据值
	BCCCLT_API  bool SetIntFieldByName(BCHANDLE Handle,int Row,char * FieldName,int Value);
   ...
   // 参数  : double Value [IN]: 数据值
	BCCCLT_API  bool SetDoubleFieldByName(BCHANDLE Handle,int Row,char * FieldName,double Value);
   ...
   // 参数  : char * Value [IN]: 数据值，不可以超过255长度
	BCCCLT_API  bool SetStringFieldByName(BCHANDLE Handle,int Row,char * FieldName,char * Value);
   ...
   // 参数  : int RequestType [IN]: 对应的业务功能号
	BCCCLT_API  bool SetRequestType(BCHANDLE Handle,int RequestType);
   ...
   // 参数  : BCHANDLE Handle [IN]: 指定的XPack数据区句柄
	BCCCLT_API  bool IsEof(BCHANDLE Handle);
   ...
   // 参数  : int * RecordCount [OUT]: 返回解包后的记录数
	BCCCLT_API  bool GetRecordCount(BCHANDLE Handle,int * RecordCount);
   ...
   // 参数  : int* RetCode [OUT]: 返回收到数据包中的返回码
	BCCCLT_API  bool GetRetCode(BCHANDLE Handle,int * RetCode);
   ...
   // 参数  : BCHANDLE Handle [IN]: 指定的XPack数据区句柄
	BCCCLT_API  int GetPrivateFunctionForNext(BCHANDLE Handle);
   ...
   // 参数  : BCHANDLE Handle [IN]: 指定的XPack数据区句柄
	BCCCLT_API  int GetBranchNoForNext(BCHANDLE Handle);
   ...
   // 参数  : int RecordCount [IN]: 设置的记录数
	BCCCLT_API  bool SetRecordCount(BCHANDLE Handle,int RecordCount);
   ...
   // 参数  : int MaxRetCount [IN]: 指定的最多记录数，0则由应用服务器端 [0..100]; 
	BCCCLT_API  bool SetMaxRetCount(BCHANDLE Handle,int MaxRetCount);
   ...
   // 参数  : BCHANDLE Handle [IN]: 指定的XPack数据区句柄
	BCCCLT_API  int GetXPackType(BCHANDLE Handle); 
   ...
   // 参数  : int RawDataLength [IN]: 要设置数据块长度
	BCCCLT_API  bool SetRawRecord(BCHANDLE Handle, int Row, char * RawData, int RawDataLength);
   ...
   // 参数  : int RawDataSize [IN]: RawData数据块的可存放的最长长度
	BCCCLT_API  int GetRawRecord(BCHANDLE Handle, int Row, char * RawData, int RawDataSize);
   ...
   // 参数  : BCHANDLE Handle [IN]: 指定的XPack数据区句柄
	BCCCLT_API  int GetMaxColumn(BCHANDLE Handle);
   ...
   // 参数  : BCHANDLE Handle [IN]: 指定的XPack数据区句柄
   // 参数  : char * FieldName [IN]: 字段名称 如"vsmess"
	BCCCLT_API  int GetFieldColumn(BCHANDLE Handle, char * FieldName);
   ...
   // 参数  : int* FieldLength [OUT]: 指定的字段长度
	BCCCLT_API  bool GetFieldInfo(BCHANDLE Handle,int Col, char * FieldName, int * FieldType, int * FieldLength);
   ...
   // 参数  : BCHANDLE Handle [IN]: 指定的XPack数据区句柄
	BCCCLT_API  bool HaveNextPack(BCHANDLE Handle);
   ...
   // 参数  : char * errormessage [OUT]: 返回的错误信息
	BCCCLT_API  bool CallRequest(BCHANDLE Handle, int drtpno, int branchno,int function, int timeout, int *errcode,char * errormessage);
   ...
   // 参数  : int* errcode [OUT]: 返回的错误码
   // 参数  : char * errormessage [OUT]: 返回的错误信息
	BCCCLT_API  bool CallNext(BCHANDLE Handle, int timeout, int* errcode,char * errormessage);
   ...
   // 返回  : int ：返回最大可设置多少条记录
   // 参数  : BCHANDLE Handle [IN]: 指定的XPack数据区句柄
   BCCCLT_API  int EmulateMaxRows(BCHANDLE Handle);
   ...
   BCCCLT_API bool Broad(BCHANDLE Handle, int drtpno, int destno, int funcno, int *errcode, char *errmsg);
