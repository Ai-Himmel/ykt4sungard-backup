			...
			if(TXCODE_CARD_DBCR!=IA->iTradeNo)
			{
				if(strcmp(CfgSplit.ArrCfgsplit[iTxType].dbsubno,CfgSplit.ArrCfgsplit[iTxType].crsubno)!=0)
				{
					if(strncmp(CfgSplit.ArrCfgsplit[iTxType].dbsubno,SUBJECT_INDIVIDUALSAVING,3)==0)
						...
						strcpy(tTradelogDb.act_id, IA->sArrInActno[4]);	//借方账号(shop)
					else if(strncmp(CfgSplit.ArrCfgsplit[iTxType].dbsubno,SUBJECT_ESAVING,3)==0)
						strcpy(tTradelogDb.act_id, IA->sArrInActno[5]);	//借方账号(shop)						
					else
...
//						writelog(LOG_DEBUG,"sArrInActno[0]=%s",tTradelogCr.act_id);
					}
...
//						writelog(LOG_DEBUG,"sArrInActno[1]=%s",tTradelogCr.act_id);
					}
					else if(strncmp(CfgSplit.ArrCfgsplit[iTxType].crsubno,SUBJECT_DEPOSITSHOPSAVING,3)==0)
					{
						strcpy(tTradelogCr.act_id, IA->sArrInActno[4]);	//贷方账号(shop)
//						writelog(LOG_DEBUG,"sArrInActno[4]=%s",tTradelogCr.act_id);
					}
					else if(strncmp(CfgSplit.ArrCfgsplit[iTxType].crsubno,SUBJECT_ESAVING,3)==0)
					{
						strcpy(tTradelogCr.act_id, IA->sArrInActno[5]);	//借方账号(shop)						
//						writelog(LOG_DEBUG,"sArrInActno[5]=%s",tTradelogCr.act_id);
					}
					else
					{
						strcpy(tTradelogCr.act_id, IA->sArrInActno[3]);	//贷方账号						
//						writelog(LOG_DEBUG,"sArrInActno[3]=%s",tTradelogCr.act_id);
					}
				}
				else
				{
...
//				writelog(LOG_DEBUG,"sArrInActno[1]=%s",tTradelogCr.act_id);
		...
		case TYPE_CONFIG:
			...
			writelog(LOG_DEBUG,"tTradelogCr.act_id=%s",tTradelogCr.act_id);
			break;
		default:
