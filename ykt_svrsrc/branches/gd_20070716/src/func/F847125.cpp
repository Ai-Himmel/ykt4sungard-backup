...
int F847125(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	SetCol(handle,F_LVOL0,F_LVOL1,F_LVOL5,
				...
				F_DAMT2,F_DAMT3,F_DAMT4,F_DAMT5,F_LSERIAL1,F_VSMESS,
	...
	sprintf(h_showcardid,"%010d",atoi(rPack->sholder_ac_no));
...
//先增加客户
	...
	ret = DB_t_cif_cuttypefee_read_by_cut_type(hi_cuttype,&tCuttype);
	if(ret)
	{
		if(DB_NOTFOUND == ret)
		{
	...
	tCustomer.cut_id=atol(rPack->sholder_ac_no);
	...
	{
		*pRetCode = E_CUSTOMER_NO_NULL;
		goto L_RETU;
	}
	if(strlen(rPack->sholder_ac_no)>8)
	{
		*pRetCode = E_CUSTOMER_NO_TOO_BIG;
		goto L_RETU;
	}
	...
	tCard.type_id = CT_TEMP_ANONYM;											//卡类别
