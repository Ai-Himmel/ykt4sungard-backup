...
//		}
	...
	tTradeserial.serial_state = SERISTAT_NONEEDDEBT;										//流水状态
...
//	tTradeserial.reviseserial_no=dSerialno;											//原写卡失败流水号
	...
	//更新账户中的卡余额
	ret=DB_t_aif_account_read_lock_by_c0_and_card_id_and_purse_id(tTradeserial.card_id, 0, &t_aif_account);
	if(ret)
	{
	...
	if(rPack->lvol2!=IS_NO)
	{
		...
		tTradeserial.trade_count = rPack->lvol1;
		t_aif_account.card_balance=rPack->damt1;
	}
	else
	{
		// 如果写卡余额
		tTradeserial.in_balance = rPack->damt0;
		tTradeserial.out_balance = rPack->damt0;
		tTradeserial.trade_fee = 0;
		tTradeserial.trade_count = rPack->lvol1;
	}
	ret=DB_t_aif_account_update_lock_by_c0(&t_aif_account);
	if(ret)
	{
