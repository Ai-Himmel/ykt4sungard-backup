...
int F849001(TRUSERID *handle,int iRequest,ST_PACK *in_pack,int *pRetCode,char *szMsg)
{
	...
	else
	{
		...
		ret = DB_t_pif_subsystem_read_by_system_id(in_pack->lcert_code,&tSubsystem);
		if(ret)
		{
			if(DB_NOTFOUND == ret)
	...
	ret = GetParameter(GLOBE_DEV_DAY_TRADE_LIMIT,temp);
	if(ret)
	{
		*pRetCode = ret;
		goto L_RETU;
	...
	ret = GetParameter(GLOBE_DEV_ONCE_TRADE_LIMIT,temp);
	if(ret)
	{
		*pRetCode = ret;
		goto L_RETU;
	...
	if(termno != ID_TERM_NO && termno != ID_TERM_NO2)
	{
		...
		ret = DB_t_pif_device_read_lock_by_cur_and_device_id(dev_id,&tDevice);
		if(ret)
		{
			if(DB_NOTFOUND == ret)
		...
		if(strcmp(tDevice.devtype,DEVITYPE_CONSUME_TERM))
		{
			DB_t_pif_device_free_lock_cur();
			*pRetCode = E_DB_DEVICE_N;
			goto L_RETU;
		}
		out_pack->lvol1 = tDevice.lasttraserno;
		out_pack->lvol5 = 1;
	}
	else
	{
