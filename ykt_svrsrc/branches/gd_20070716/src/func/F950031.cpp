...
//	tRcvdtl.run_reason = rPack->lbank_acc_type2;				//启动原因
	...
	if (ret)
	{
		writelog(LOG_DEBUG,"DB_t_tif_rcvdtl_add ERR,serial_no[%d]card_no[%d]device_id[%s]tx_date[%s]tx_time[%s],ret[%d]",tRcvdtl.serial_no,tRcvdtl.card_no,tRcvdtl.device_id,tRcvdtl.tx_date,tRcvdtl.tx_time,ret);
		if(DB_REPEAT==ret)
			return 0;
		else
