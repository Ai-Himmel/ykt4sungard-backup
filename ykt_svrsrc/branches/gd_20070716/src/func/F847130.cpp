...
int F847130(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	SetCol(handle,0);
	SetCol(handle,F_SCUST_LIMIT,F_LVOL12,F_VSVARSTR0,F_VSMESS,0);
	...
	if(D4U5(tSubsidyTmp.amount,2) != tSubsidyTmp.amount)
	{
	...
	*/
	...
	memcpy(&tSubsidy,&tSubsidyTmp,sizeof(tSubsidy));
