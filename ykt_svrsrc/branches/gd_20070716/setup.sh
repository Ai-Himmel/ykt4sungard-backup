...
make_dist()
{
	...
	# copy bin directory
	DEST_FILES="$YKT_BU bu0 bu1 bu2 function0.lst function1.lst function2.lst ksmbcc.ini bala bala.ini sall blksvr start start.ini rcv_acc cpack.dat"
	...
	# copy lib directory
	DEST_FILES="libbupubs.a libxpack.a"
	...
	find $DEST_DIR/ -name '.svn' -type d | xargs $RM -rf > /dev/null 2>&1
	cd $BASEDIR/dist
...
make_tar()
{
	...
	DEST_FILES="bu0 bu1 bu2 function0.lst function1.lst function2.lst ksmbcc.ini bala.ini start.ini sall cpack.dat proccfg.cfg"
	copy_files "$DEST_FILES" $BASEDIR/bin $DEST_DIR/bin
	DEST_FILES="ksmbcc"
	copy_files_check_version "$DEST_FILES" "$BASEDIR/bin" "$DEST_DIR/bin"
	# copy sbin directory
	$CP -rf $BASEDIR/sbin $DEST_DIR/sbin
	# copy lib directory
	DEST_FILES="libboost_thread.a  libkcrypto.a  libbupubs.a libdrtplib.a libxpack.a"
	copy_files_check_version "$DEST_FILES" "$BASEDIR/lib" "$DEST_DIR/lib"
	...
	find $DEST_DIR/ -name '*.o' -type f -exec rm {} \;
	find $DEST_DIR/ -name '.svn' -type d | xargs $RM -rf > /dev/null 2>&1
	cd $BASEDIR/dist
