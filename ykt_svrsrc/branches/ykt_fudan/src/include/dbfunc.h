...
int process930117(T_t_door_device_cardlist *cardlist,int addflag);
int process930121(int cardid,int addflag,char * version,char *devphyid);
...
//圈存监控
int DB_MONITORS_UPDATE(char * host, char * state);

//库存管理
 int GetTypeFromShowNo(char *showcardno);								//根据显示卡号查询卡类别
int GetCardtypeFromShowNo(char *showcardno,int *cardtype);				//查询卡领入类别
int UseShowCardNo(char *opercode,char *showcardno,int cardtype);			//发卡
int UseShowCardNoBack(char *opercode,char *showcardno,int cardtype);		//回收卡
#ifdef __cplusplus
}
#endif
#endif
