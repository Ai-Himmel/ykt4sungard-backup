...
int F847123(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	trim(rPack->scust_no);
	trim(rPack->sdate0);
	if((strcmp(tCard.showid,rPack->scust_no)==0)&&(strcmp(tCard.physical_no,rPack->sbank_acc)==0))
	{
		writelog(LOG_DEBUG,"This card is same to that in database!!!");
	}
	else
	{		
	...
	carddtl.cardtype=GetTypeFromShowNo(rPack->scust_no);
	//库存中使用新卡
	ret = UseShowCardNo(rPack->scust_limit,rPack->scust_no,carddtl.cardtype);
	if(ret)
		return ret;
	...
	carddtl.transcnt=1;
	strcpy(carddtl.summary,"新生换卡使用新卡");
	des2src(carddtl.cardphyid,rPack->sbank_acc);
	des2src(carddtl.showcardno,rPack->scust_no);	
	...
	// add by 汤成 2005-8-8
	// 增加向名单表写入记录
	// modify by tc 2009-10-12 区分新生换卡与补办卡标志
	ret = InsertToBlkList(card_id,8);
	if(ret)
	{
		*pRetCode = ret;
		goto L_RETU;
