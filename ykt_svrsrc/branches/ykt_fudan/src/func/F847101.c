...
int F847101(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	ret = GetCardtypeFromShowNo(h_showcardid, &cardtype);
	if(ret)
		return ret;

	//学生的第一张卡是用临时卡发行的，以后会做换卡
	if(cardtype != CT_NORMAL && cardtype != CT_TEMP)
		return E_CARDTYPE_NORMAL;
	
	ret = UseShowCardNo(rPack->scust_limit,h_showcardid,cardtype);
	if(ret)
		return ret;
