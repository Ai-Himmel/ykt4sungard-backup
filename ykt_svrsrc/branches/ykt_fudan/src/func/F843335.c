...
int F843335(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	writelog(LOG_DEBUG,"rakeoff_ratio :damt8   [%0.2f]",rPack->damt8);
...
//先增加客户
	...
	tShop.rakeoff_ratio=rPack->damt8;  //佣金类型
	des2src(tShop.rakeoff_type,rPack->sphone);  //佣金类型
