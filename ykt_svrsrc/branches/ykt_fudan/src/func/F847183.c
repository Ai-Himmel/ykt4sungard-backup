...
static int process847106(T_t_tif_tradeserial *tTradeserial)
{	
	...
	ret = DB_t_tif_savdtl_read_by_tx_date_and_seqno(
		tTradeserial->collect_date,tTradeserial->reviseserial_no,&tSavdtl);
	if(ret)
	{
		if(DB_NOTFOUND != ret)
	...
	{
		//不是现金充值流水
		sprintf(tSavdtl.comments,"流水号[%d],操作时间[%s]被冲正",tSavdtl.seqno,tSavdtl.tx_date);
		tSavdtl.seqno = tTradeserial->serial_no;
		des2src(tSavdtl.tx_date,tTradeserial->collect_date);
		getsystime(tSavdtl.tx_time);
		tSavdtl.amount = -tSavdtl.amount;
		tSavdtl.tx_code = TXCODE_TX_REVEAL;
		des2src(tSavdtl.oper_no,tTradeserial->oper_code);
