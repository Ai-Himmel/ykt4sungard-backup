...
static int UpdateConferSerial(T_t_door_txdtl* tTxdtl)
{
	...
	ret = SaveConferenceSerial(tTxdtl,confid,tCard.cosumer_id,sign);
	if(ret)
	{
...
static int InsertKQSerial(T_t_door_txdtl* tTxdtl)
{
	...
	*/
	...
	if(ret)
	{
		if(DB_REPEAT==ret)
			return 0;
		writelog(LOG_ERR,"SaveKQSerial error,errcode=[%d]",ret);
		return ret;
	}
	return 0;
}
...
int F930101(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	//printf("Start:Device=[%s],systime=[%s]\n",rPack->sdate0,getsystimestamp(NULL));
...
//		tTxdtl.err_code=ret;
	...
	//printf("End-success:Device=[%s],systime=[%s],errcode=[%d]\n",rPack->sdate0,getsystimestamp(NULL),*pRetCode);
	return 0;
L_RETU:
	//printf("End-failed:Device=[%s],systime=[%s],errcode=[%d]\n",rPack->sdate0,getsystimestamp(NULL),*pRetCode);
	return -1;
}
