...
int F847106(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	char sAccBeginMonth[5]="";
	char sMsg[256]="";
	double dMaxCardBalance=0;
	double totalFundBookMoney = 0;		//本年中卡片的经费本充值总额
	double maxFundBookMoney = 0;		//单张卡片一年经费本充值最大值
	...
	switch(type)
	{
		...
		case TYPE_FUNDBOOK:	//经费本
			memset(sMaxCardBalance,0,sizeof(sMaxCardBalance));
			ret=GetParameter(GLOBE_MAXFUNDBOOK,sMaxCardBalance);
			if(ret)
			{
				*pRetCode=ret;
				goto L_RETU;
			}
			maxFundBookMoney=atof(sMaxCardBalance);
			
			ret=GetParameter(GLOBE_ACC_BEGINMONTH,sAccBeginMonth);
			if(ret)
			{
				*pRetCode=ret;
				goto L_RETU;
			}
			totalFundBookMoney = get_totalFundBookMoney_by_card_no(card_id,sAccBeginMonth,sysdate);
			if(amtcmp(totalFundBookMoney+tradeserial.trade_fee,maxFundBookMoney)>0)
			{
				*pRetCode= E_EXCEED_MAX_FUNDBOOK;
				goto L_RETU;
			}
