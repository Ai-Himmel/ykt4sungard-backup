...
//	tCard.deposit_fee=0;
	...
	*/
	if(USE_CARD_TYPE_ONLINE==IA->iUseCardFlag)					//有卡交易
	{
		...
		{
			if(amtcmp( IA->dInCardBala - tAccount.cur_freebala, max_diff_money) > 0)
			{
...
//	writelog(LOG_DEBUG,"input card_id[%d] card_id[%d]",origin_card_id,tCard.card_id);
	...
	//carddtl.cardtype=tCard.type_id;
	carddtl.cardtype = GetTypeFromShowNo(tCard.showid);
	if(carddtl.cardtype == E_CARDTYPE)									// 遗留问题，以前的卡在卡表中可能没有显示卡号
