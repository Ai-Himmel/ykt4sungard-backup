...
int F847104(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	des2src(h_showcardid,rPack->scust_no);
	trim(h_showcardid);
	...
	ret = UseShowCardNo(rPack->scust_limit,h_showcardid,CT_NORMAL);
	if(ret)
		return ret;
    ...
    */
	...
	if(ret)
	{
	 ...
	 if(rPack->lvol4==0)			// 坏卡登记,不收成本费
 	{
 		if(strlen(rPack->scust_no2) !=10)
			return E_NOTEXIST_SHOWCARDNO;
/*
		// solaris 上这样调用，前台接收不到数据
		writelog(LOG_ERR,"in  showcardno[%s],opcode[%s] ",rPack->scust_no2,rPack->scust_limit);
		des2src(rPack->scust_no, rPack->scust_no2);
		des2src(rPack->sorder1,rPack->scust_limit);
		writelog(LOG_ERR,"showcardno[%s],opcode[%s] ",rPack->scust_no,rPack->sorder1);
		ret = F846107( handle,  iRequest, rPack, pRetCode, szMsg);
		if(ret)
			return ret;		
*/
		memset(&carddtl,0,sizeof(carddtl));
		...
		carddtl.cardtype=GetTypeFromShowNo(rPack->scust_no2);
		carddtl.usetype[0]=CARDUSETYPE_BAD;
		
		carddtl.transcnt= 1;
		strcpy(carddtl.summary,"坏卡登记");
		des2src(carddtl.showcardno,rPack->scust_no2);

		des2src(carddtl.opercode,rPack->scust_limit);
		
		writelog(LOG_ERR,"opcode[%s],showcardno[%s],seqno[%d]",carddtl.opercode,carddtl.showcardno,carddtl.termseqno);
		ret=DB_t_pif_carddtl_add(&carddtl);
		if(ret)
		{
			if(DB_REPEAT==ret)
	...
	writelog(LOG_DEBUG,"交易卡号[%d]",out_pack->lvol0);
