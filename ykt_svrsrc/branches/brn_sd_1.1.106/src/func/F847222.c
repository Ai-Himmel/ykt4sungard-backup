...
static int process(InAcc *IA,trade_param_t *p)
{
	...
	IA->dArrInAmt[0] = D4U5(p->trade_fee,2);
	//writelog(LOG_DEBUG,"account[%s],op_fee[%f]",IA->sArrInActno[0],IA->dArrInAmt[0]);
	IA->iFeeType = 1;
	IA->iUseCardFlag = 0;				
	...
	p->remain_balance = D4U5(p->remain_balance-p->trade_fee,2);
	return 0;
}
...
static int do_process_trade(trade_param_t * param)
{
	...
	if(ret)
	{
		if(DB_NOTFOUND == ret)
		{
			writelog(LOG_DEBUG,"account read cut_id[%d],act_type[%d]",shop.cut_id,ACCTYPE_SHOPMAIN);
	...
	{
		//只计算商户余额
		param->free_balance = D4U5(param->free_balance+account.last_bala,2);
		return 0;
	}
	// 计算交易金额
	...
	writelog(LOG_DEBUG,"shop_id[%d],account bala[%f],trade bala[%f]",param->shop_id,account.cur_bala,param->trade_fee);
	ret = process(IA,param);
	if(ret)
	{
		writelog(LOG_ERR,"process account failed,ret [%d]shopid[%d]subindex[%d],tradefee[%f]",
			ret,param->out_shop_id,param->index,param->trade_fee);
		return ret;
	}
	// 更新账户余额
	...
	account.last_bala =  D4U5(account.last_bala-param->trade_fee,2);
	account.last_freebala =D4U5(account.last_freebala- param->trade_fee,2);
	if(amtcmp(account.last_bala,0)< 0)
	{
	...
	writelog(LOG_DEBUG,"商户[%d]解款金额[%lf]余额[%lf]",shop.shop_id,param->trade_fee,account.last_bala);
	return ret;
}
...
static int do_parse_shop(trade_param_t * param)
{
	...
	while(1)
	{
		...
		if(ret)
		{
			writelog(LOG_ERR,"叶子商户解款失败shop[%d][%s],ret[%d]"
				,shop_node.leaf_id,shop_node.shop_name,ret);
...
int F847222(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	writelog(LOG_ERR,"trade money , shopid[%d],money[%lf]",param.shop_id,param.remain_balance);
