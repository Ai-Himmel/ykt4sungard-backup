...
int elec_room_deposit_search(ST_PACK *in_pack,ST_PACK *out_pack){
	...
	writelog(LOG_ERR,"electrity deposit money=[%s]",out_pack->sstation0);
...
//系统模拟入账
 int tend_to_enter_account(ST_PACK *in_pack,InAcc *IA,int trade_code,T_t_tif_tradeserial *tradeserial){
	...
	if(amtcmp(tradeserial->trade_fee, 0) <=0)
		return E_INPUT_AMT;
...
static const ext_consumer_request_t  g_transfer_process[] = {
	...
	{PRE_ENTER_ACCOUNT_WATER_TRANS,TXCODE_CONSUME,TIMEOUT_REDO_FORBIT , NULL,get_third_part_account,NULL,deal_water_ctl_out_pack,record_small_serial},
