...
//	ST_PACK *out_pack = &(aPack.pack);
 ...
 //   tTradeSerial.trade_fee = in_pack->damt0;
    ...
    ret = DB_t_tif_tradeserial_read_lock_by_cur_and_serial_no(in_pack->lvol5, &tradeserial);
    if (ret)
    {
...
*/
	...
	if (ret)
	{
		writelog(LOG_ERR,"read tablet_cif_shop  shpid[%d]",shop_id);
		if (DB_NOTFOUND == ret)
		{
		    DB_t_tif_tradeserial_free_lock_cur();
		    return E_SHP_ID_NOT_EXIST;
...
//                tTradeSerial.serial_state = SERISTAT_NODEBT;			// 流水状态
                tradeserial.sys_id = NETCHARGE_CODE;                   // 超时标记
...
SetHeadCol(&aPack, F_LVOL11, 0);
    ...
    if (1 == out_pack->lvol11)
    {
        ...
        tTradeSerial.trade_count = in_pack->lvol1 + 1;                               
        des2src(tTradeSerial.oper_code, "system");                                                     
        tTradeSerial.sys_id = in_pack->lvol3;	
        ...
        out_pack->damt1 = in_pack->damt1;                                       // 传出扣费金额
        out_pack->damt0 = D4U5(in_pack->damt0 - in_pack->damt1,2);              // 传出的出卡值
//        PutRow(handle,out_pack,pRetCode,szMsg);     
    }
...
static const ext_consumer_request_t  g_transfer_process[] = {
	...
	{NET_FEE_TRANS,TXCODE_NETFEE_TRANS,do_netcharge_check,get_third_part_account,NULL,do_netcharge_acc,NULL},
