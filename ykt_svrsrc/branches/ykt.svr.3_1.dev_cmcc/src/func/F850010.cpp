...
int F850010(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	{
		ERRTIP("钱包类型[%d]未启用",pursetype.enableflag);
		return E_COMMON_ERR;
	}
	trans.cardflag=1;
	trans.usecardflag=1;	
...
//			cardbalance.dpscnt=rPack->lvol3+1;
	...
	{
		//联机水控充值
		pAccTrans->trans.transcode=TC_WATERDEPOSIT;
		pAccTrans->trans.transtype=TRANSTYPE_WATERDEPOSIT;
	...
	else
	{
		...
		pAccTrans->trans.transtype=TRANSTYPE_WATERTRANS;
		ret=pAccTrans->DoTransByTransType();
		if(ret)
			return ret;	
		ret=pAccTrans->doTransEnd();
