...
int F240157(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
  ...
  char logicdate[11]="";										//业务日期
	...
	if(strncmp(tCard.cardstatus,CARDSTAT_REG,4)!=0)
	{
		...
		else if(STATE_TRUE==tCard.cardstatus[CARDSTAT_TYPE_LOST])
			*pRetCode=E_CARD_LOST;
		else if(STATE_TRUE==tCard.cardstatus[CARDSTAT_TYPE_FREEZE])
			*pRetCode=E_CARD_FREEZE;
		else if(STATE_TRUE==tCard.cardstatus[CARDSTAT_TYPE_WFAIL])
...
//	tradeserial.deviceid = rPack->lvol7;							//采集设备标识
	...
	out_pack->damt4=tradeserial.cardaftbal;		//出卡值
