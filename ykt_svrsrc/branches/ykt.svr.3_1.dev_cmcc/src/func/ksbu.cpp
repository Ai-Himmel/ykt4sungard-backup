...
int CallBDFunc(int fno,TRUSERID *handle,ST_CPACK *rpack,ST_PACK *pArrays,int *iRetCode, char *szMsg)
{
   ...
   *iRetCode = pAccTrans->Reset(rpack);
   if(*iRetCode)
   	 goto NEXT;
#ifdef _DEBUG1
CMemoryState cm1, cm2, cm3;
...
NEXT:
		...
		db_rollback();
		snprintf(szMsg,256,"%s%s\n%s",pAccTrans->GetErrMsgTip(*iRetCode).c_str(),sRetMsg,g_sqlmsg);
		writelog(LOG_ERR,"Func[%d]errcode[%d]errmsg[%s]",rpack->head.RequestType,*iRetCode,szMsg);
		//事物回滚后需要重新取流水号
		pAccTrans->trans.termseqno=0;
		pAccTrans->trans.errcode = *iRetCode;
		des2src(pAccTrans->trans.remark,szMsg);
		r=pAccTrans->end();
		if(r)
		{
			writelog(LOG_ERR,"acctrans end error ");			
			db_rollback();
		}
		else
			db_commit();
...
*/
	...
	else
	{
		...
		else
		{
			...
			if(r)
			{
				//get_errcode_msg(r,szMsg);
				strcpy(szMsg,pAccTrans->GetErrMsg(r).c_str());
			}
			else
			{
				r=db_commit();
...
int main(int argc,char *argv[])
{
   ...
   SetLogShowLevel(0);   
   char szCompileDate[20]=__DATE__;
   vector<string> strlist;
   split2list(szCompileDate,' ',strlist);
   if(strlist[0].compare("Jan")==0)
   	  strlist[0]="01";
   else if(strlist[0].compare("Feb")==0)
   	  strlist[0]="02";
   else if(strlist[0].compare("Mar")==0)
   	  strlist[0]="03";
   else if(strlist[0].compare("Apr")==0)
   	  strlist[0]="04";
   else if(strlist[0].compare("May")==0)
   	  strlist[0]="05";
   else if(strlist[0].compare("Jun")==0)
   	  strlist[0]="06";
   else if(strlist[0].compare("Jul")==0)
   	  strlist[0]="07";
   else if(strlist[0].compare("Aug")==0)
   	  strlist[0]="08";
   else if(strlist[0].compare("Sep")==0)
	  strlist[0]="09";
   else if(strlist[0].compare("Oct")==0)
	  strlist[0]="10";
   else if(strlist[0].compare("Nov")==0)
	  strlist[0]="11";
   else if(strlist[0].compare("Dev")==0)
	  strlist[0]="12";
   sprintf(g_szVerNo,"ykt_bu 3.1.1 (%s%s%s)",strlist[2].c_str(),strlist[0].c_str(),strlist[1].c_str());
   if(argc==2)
   {
