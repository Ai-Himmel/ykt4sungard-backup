...
static int add_serial(T_t_tif_tradeserial *tradeserial)
{
	int ret = 0;
...
static bool isPersonAccount(char *account_id,int *card_id,int *customer_id)
{
	int ret = 0;
	T_t_aif_account tAccount;
	memset(&tAccount,0,sizeof(tAccount));
	ret = DB_t_aif_account_read_by_account_id_and_act_type(account_id,1,&tAccount);
	if(ret)
	{
		return false;
	}
	*card_id = tAccount.card_id;
	*customer_id = tAccount.customer_id;
	return true;
}
static int process(InAcc *IA,T_t_tif_tradeserial *p)
{
	...
	strcpy(IA->sMdeOper,p->reserve_1);			//操作员(发生错帐的操作员)
...
int F847127(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	if(isPersonAccount(rPack->sbank_acc,&card_id,&customer_id) && 
		isPersonAccount(rPack->sbank_acc2,&card_id2,&customer_id2))
	{
		tradeserial.card_id = card_id;
		tradeserial.customer_id = customer_id;
		ret = add_serial(&tradeserial);						//第一个插入流水
		if(ret)
		{
			*pRetCode=ret;
			goto L_RETU;
		...
		tradeserial.serial_no = D2I(dUniqno);							//流水号
		tradeserial.card_id = card_id2;
		tradeserial.customer_id = customer_id2;
		ret = add_serial(&tradeserial);							//第二个插入流水
		if(ret)
		{
			*pRetCode=ret;
			goto L_RETU;
	...
	else if(isPersonAccount(rPack->sbank_acc,&card_id,&customer_id))
	{
		tradeserial.card_id = card_id;
		tradeserial.customer_id = customer_id;
		ret = add_serial(&tradeserial);						//第一个插入流水
	...
	else if (isPersonAccount(rPack->sbank_acc2,&card_id2,&customer_id2))
	{
		tradeserial.card_id = card_id2;
		tradeserial.customer_id = customer_id2;
		ret = add_serial(&tradeserial);							//第二个插入流水
