...
int BackupData()
{
	...
	*/
	//备份来帐流水表
/*
	ret=Db_t_tif_rcvdtl_backup();
	if(ret)
	{
		writelog(LOG_ERR,"Db_t_tif_rcvdtl_backup ERROR,ERRCODE=[%d]",ret);
		return 	ret;
	}
*/
...
int CreateOperReport()
{
	...
	//生成tradelog_grouping报表
	ret=gen_report_tradelog_grouping(logic_date);
	if(ret)
	{
		writelog(LOG_ERR,"Insert gen_report_tradelog_grouping record[%d],err[%d]",cnt,ret);
		db_rollback();
		return 	ret;
	}
	...
	//生成school_trade报表
	//ret=gen_report_school_trade(logic_date);
	//if(ret)
	//{
	//	writelog(LOG_ERR,"Insert gen_report_school_trade record[%d],err[%d]",cnt,ret);
	//	db_rollback();
	//	return 	ret;
	//}
...
int CreateShopDevMealReport()
{
	...
	while(1)
	{
		...
		t_device_meal.shop_id=v_device_meal.shop_id;
		t_device_meal.meal_id=v_device_meal.meal_id;
		t_device_meal.device_id=v_device_meal.device_id;
		...
		des2src(t_device_meal.device_name,v_device_meal.device_name);
		des2src(t_device_meal.meal_name,v_device_meal.meal_name);
...
int CreateShopDevwaterReport()
{

	T_v_tif_report_water	v_device_water;
	T_t_tif_report_device_water 	t_device_water;
	...
	ret=DB_t_tif_report_device_water_del_by_balance_date(logic_date);
	if((0!=ret)&&(DB_NOTFOUND!=ret))
	{
		writelog(LOG_ERR,"DB_t_tif_report_device_water_del_by_balance_date error,errcode[%d]",ret);
		return 	ret;
	}

	ret=DB_v_tif_report_water_open_select_by_cur1();
	if(ret)
	{
		writelog(LOG_ERR,"Open v_tif_report_device_water view error,errcode[%d]",ret);
		return 	E_DB_V_REPORT_DEVICE_WATER_R;
	}

	while(1)
	{
		memset(&v_device_water,0,sizeof(v_device_water));
		memset(&t_device_water,0,sizeof(t_device_water));
		ret=DB_v_tif_report_water_fetch_select_by_cur1(&v_device_water);
		if(ret)
		{
			DB_v_tif_report_water_close_select_by_cur1();
			if(ret==DB_NOTFOUND)
			{
				if(cnt==0)
				{
					writelog(LOG_DEBUG,"There havn't record from v_tif_report_device_WATER view!");
					return   0;
				}
				else		break;
			}
		...
		cnt++;
		////生成报表过程开始
		des2src(t_device_water.collect_date,v_device_water.collect_date);
		des2src(t_device_water.balance_date,logic_date);
		t_device_water.shop_id=v_device_water.shop_id;
		t_device_water.device_id=v_device_water.device_id;
		t_device_water.trade_count=v_device_water.trade_count;
		t_device_water.trade_fee=v_device_water.trade_fee;
		t_device_water.mng_fee=v_device_water.mng_fee;
		t_device_water.total_fee=v_device_water.total_fee;
		t_device_water.water_container=v_device_water.water_container;
		t_device_water.total_container=v_device_water.total_container;
		des2src(t_device_water.device_name,v_device_water.device_name);
		des2src(t_device_water.data_sign," ");
		des2src(t_device_water.reserve_1," ");
		des2src(t_device_water.reserve_2," ");
		t_device_water.rakeoff_amt=0;
		ret=DB_t_tif_report_device_water_add(&t_device_water);
		if(ret)
		{
			writelog(LOG_ERR,"Insert DB_t_tif_report_device_water record[%d],err[%d]",cnt,ret);
			db_rollback();
			return 	E_DB_V_REPORT_DEVICE_WATER_I;
		}
...
int DelTradeDate()
{
	...
	*/
	...
	//删除来帐流水表
//		ret=Db_t_tif_rcvdtl_del_all_by_step_commit();
//		if((0!=ret)&&(100!=ret))
//		{
//			writelog(LOG_ERR,"Db_t_tif_rcvdtl_del_all_by_step_commit error,errcode=[%d]",ret);
//			db_rollback();
//			return 	0;
//		}
	/* 不删除来帐流水表
...
//		if(ret)
//		{
//			writelog(LOG_ERR,"DelTxdtlData ret[%d]",ret);
//			db_rollback();
//			return 	ret;
//		}
	*/
	...
	writelog(LOG_INFO,"Delete t_tif_tradeserial and t_tif_tradelog rcvdtl succeed!");
	return 0;
}
...
int  PayInCashForOperateGroup()
{
	...
	while(1)
	{
		...
		cnt++;
		writelog(LOG_ERR,"cnt=[%d],group=[%s]",cnt,group_cash.group_id);

		ret=ReadFromOpenGroupAndRepOper(group_cash.group_id,&cash);
		if(ret)
		{
			writelog(LOG_ERR,"ReadFromOpenGroupAndRepOper group_id[%s] error,errcode=[%d]",group_cash.group_id,ret);
			goto LRet;
		}
...
int Bala_rakeoff_transfer_day()
{
	...
	ret=DB_t_cif_shop_open_select_by_cur4();
	if(ret)
	{
		writelog(LOG_ERR,"DB_t_cif_shop_open_select_by_cur4  error,errcode=[%d]",ret);
		goto LRet;
	}
	while(1)
	{
		ret=DB_t_cif_shop_fetch_select_by_cur4(&t_shop);
		if(ret)
		{
			if(ret==DB_NOTFOUND)
			{
				if(cnt==0)
				{
					writelog(LOG_DEBUG,"DB_t_cif_shop_fetch_select_by_cur4 is not record!");
					break;
				}
				else		break;
			}
			...
			{
				writelog(LOG_ERR,"DB_t_cif_shop_fetch_select_by_cur4 error,errcode=[%d] !",ret);
				goto LRet;
			}
		}
		cnt++;
		if(t_shop.shop_state==1&&t_shop.is_leaf[0]=='1')
		{
			ret=rakeoff_transfer_day(t_shop.shop_id);
			if(ret)
			{
				writelog(LOG_ERR,"rakeoff_transfer_day error,shop_id=[%d],errcode=[%d]!",t_shop.shop_id,ret);
				goto LRet;
			}
		}
		continue;
...
int Bala_agio_transfer_month()
{
	...
	if(ret)
	{
		writelog(LOG_ERR,"GetLogicDate err [%d]",ret);
		return ret;
	}
	memcpy(temp,logic_date+6,2);
	temp[2]=0;

	ret=GetParameter(GLOBE_MONTH_ACC_TIME,buffer);
	if(ret)
	{
		writelog(LOG_ERR,"Read syspara err [%d]",ret);
		return ret;
	...
	if(atoi(temp)!=atoi(buffer))	return 0;

	ret=get_before_month_with_day(logic_date,month_logic_date);
	if(ret)
	{
		writelog(LOG_ERR,"get_before_month_with_day err [%d]",ret);
		return ret;
	}
	...
	while(1)
	{
		...
		if(ret)
		{
			if(ret==DB_NOTFOUND)
			{
				if(cnt==0)
				{
					writelog(LOG_DEBUG,"DB_t_cif_shop_fetch_select_by_cur4!");
					break;
				}
				else		break;
			}
		...
		if(t_shop.shop_state==1&&t_shop.is_leaf[0]=='1')
		{
			ret=GetSumShopBalanceMonth(month_logic_date,logic_date,t_shop.shop_id,&rake_amt,&total_amt);
			if(ret)
			{
				writelog(LOG_ERR,"GetSumShopBalanceMonth error,shop_id=[%d],errcode=[%d]!",t_shop.shop_id,ret);
				goto LRet;
			}
			else
			{
				ret=agio_transfer_month(t_shop.shop_id,rake_amt,total_amt);
				if(ret)
				{
					writelog(LOG_ERR,"agio_transfer_month error,shop_id=[%d],errcode=[%d]!",t_shop.shop_id,ret);
					if(ret==DB_NOTFOUND)
					{
						continue;
					}
