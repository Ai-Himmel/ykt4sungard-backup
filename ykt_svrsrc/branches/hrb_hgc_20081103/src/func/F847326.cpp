...
int R_ErrorMsg(char *R_Msg,ST_PACK *in_pack)
{
	char ErrMsg[1024]="";	//Save Error Message
	sprintf(ErrMsg,"学工号scust_auth=[%s], 姓名semail=[%s], 缴费标识lvol0=[%d]   学期sphone=[%s]",in_pack->scust_auth,in_pack->semail,in_pack->lvol0,in_pack->sphone);
	des2src(R_Msg,ErrMsg);
	return 0;
}
...
static int DoInsertRegisterLog(ST_PACK *rPack)
{
	...
	ret = Get_Schoolterm_by_name(rPack->scust_auth,rPack->sphone,&reg_id);
	if(ret)
		return ret;
	t_tif_register_pay_log.reg_id=reg_id;
...
int DoRegisterInput(ST_PACK *rPack)
{
	...
	des2src(semester_name,rPack->sphone);
	register_src.register_flag = rPack->lvol0;					// regiser flag
	register_src.flag = 0;										// update flag
	register_src.reserver_no1 = rPack->lvol1;					// register login and logout flag
	ret = Get_Schoolterm_by_name(stuemp_no,semester_name,&reg_id);
	if(ret)
		return ret;
	
	register_src.reg_id = reg_id;
	...
	if (ret)
	{
		...
		{	
			...
			if (ret)
			{
				...
				ret = E_DB_REGISTER_FREEZIE_ADD_ERR;
				goto LRet;
			}
		}
		else
...
int DoRegisterFileUpdate(ST_PACK *rPack)
{
	...
	while (1)
	{
		...
		des2src(stuemp_no,hrbgc_zcjf.stuemp_no);
		...
		des2src(semester_name,hrbgc_zcjf.semeseter);
		ret = Get_Schoolterm_by_name(stuemp_no,semester_name,&reg_id);
		if(ret)
			return ret;
		register_src.reg_id= reg_id;
