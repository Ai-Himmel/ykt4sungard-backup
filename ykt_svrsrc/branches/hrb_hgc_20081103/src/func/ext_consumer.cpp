...
static int get_third_part_account(ST_PACK *in_pack,int trade_code,InAcc *IA){
	...
	if(ret){
		writelog(LOG_ERR,"DB_t_tradecode_shop_open_select_by_c0_and_trade_code error,trade_code[%d]errcode=[%d]",trade_code,ret);
		return 	ret;
	}
	while(1){
		memset(&tradeshop,0,sizeof(tradeshop));
...
//系统模拟入账
 int tend_to_enter_account(ST_PACK *in_pack,InAcc *IA,int trade_code,T_t_tif_tradeserial *tradeserial){
	...
	tradeserial->device_id = atoi(in_pack->sname);
	...
	//des2src(tradeserial->in_account_id, in_pack->sall_name);
	des2src(tradeserial->b_act_id,in_pack->sall_name);
...
//系统正式入账
 int enter_account(ST_PACK *in_pack,InAcc *IA,ST_PACK *out_pack,T_t_tif_tradeserial *tradeserialtest){
	int ret = 0;
	int i=0;
	int	card_id = 0;
	char logicdate[11]="";
	...
	tradeserial.sys_id = in_pack->lvol11;						// timeout mark flag
...
static int do_netcharge_check(ST_PACK *in_pack,ST_PACK *out_pack,transfer_control_t *trans_ctl)	//(ST_PACK *in_pack,TRUSERID *handle,int *pRetCode,char *szMsg)
{
	...
	ret =DB_t_tradecode_shop_open_select_by_c0_and_trade_code(TXCODE_NETFEE_TRANS);
	if(ret){
		writelog(LOG_ERR,"DB_t_tradecode_shop_open_select_by_c0_and_trade_code error TXCODE_NETFEE_TRANS[%d],errcode=[%d]",TXCODE_NETFEE_TRANS,ret);
		return 	ret;
	}
	while(1){
		memset(&tradeshop,0,sizeof(tradeshop));
		ret =DB_t_tradecode_shop_fetch_select_by_c0(&tradeshop);
		writelog(LOG_ERR,"cnt[%d],tradeshop.shopid[%d]",cnt,tradeshop.shop_id);
		if(ret){
			DB_t_tradecode_shop_close_select_by_c0();
			if(cnt==0)
			{
...
static const ext_consumer_request_t  g_transfer_process[] = {
	...
	{HGC_RJ_NETWORK, TXCODE_NET_CHARGE_EXT, pre_check_user_id, get_third_part_account, transfer_balance, NULL, NULL},
	{NET_FEE_TRANS,TXCODE_NETFEE_TRANS,do_netcharge_check,get_third_part_account,do_netcharge_acc,NULL,NULL},
