...
/****************************************************
      ...
      修改记录	:
*****************************************************/
int CreateShopReport_Water()
{
	T_v_tif_shopdeptfee_water 	v_shop_balance_water;
	T_t_tif_report_shop_balance_water 	t_shop_balance_water;
...
//清除当天的结算数据
	...
	ret=DB_t_tif_report_shop_balance_water_del_by_balance_date(logic_date);
	if((0!=ret)&&(100!=ret))
	{
		writelog(LOG_ERR,"Delete t_tif_report_shop_balancewater report error,errorcode[%d]",ret);
		return	ret;
	}

	ret=DB_v_tif_shopdeptfee_water_open_select_by_c5();
	if(ret)
	{
		writelog(LOG_ERR,"Open v_tif_shopdeptfee_water view error,errorcode[%d]",ret);
		return	E_DB_SHOPDEPTFEE_WATER_R;
	}

	while(1)
	{	
		memset(&v_shop_balance_water,0,sizeof(v_shop_balance_water));
		memset(&t_shop_balance_water,0,sizeof(t_shop_balance_water));
		ret=DB_v_tif_shopdeptfee_water_fetch_select_by_c5(&t_shop_balance_water);
		if(ret)	
		 {
			 DB_v_tif_shopdeptfee_water_close_select_by_c5();
			 if(ret==DB_NOTFOUND)
			 {
				if(cnt==0)
					{
						writelog(LOG_DEBUG,"There havn't record from T_v_tif_shopdeptfee_water view!");
						return	 0;
					}
				else		break;
			 }
		...
		strncpy(t_shop_balance_water.balance_date,logic_date,sizeof(t_shop_balance_water.balance_date));
		t_shop_balance_water.total_amt=t_shop_balance_water.trade_amt+t_shop_balance_water.mng_amt;
		ret=DB_t_tif_report_shop_balance_water_add(&t_shop_balance_water);
		if(ret)
		{
				writelog(LOG_ERR,"Insert t_tif_report_shop_balance_water record[%d],err[%d]",cnt,ret);
				db_rollback();
				return	E_DB_TABLE_ERRACC_I;
		}
