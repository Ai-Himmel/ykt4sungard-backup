...
db2 "select rtrim(creator)||'.'||rtrim(name) from SYSIBM.SYSTABLES where TYPE='T' and creator like 'YKT\_%'  escape '\' "|grep -v ^NAME|grep -v "-"|grep -v "selected" |grep "^YKT_"|sed 's/ //g' > tmp.lst
list=`cat tmp.lst`
for i in $list
do 
