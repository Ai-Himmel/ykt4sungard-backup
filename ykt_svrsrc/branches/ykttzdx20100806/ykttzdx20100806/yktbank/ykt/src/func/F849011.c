...
int F849011(TRUSERID *handle,int iRequest,ST_PACK *in_pack,int *pRetCode,char *szMsg)
{
	...
	if(TXCODE_WATER_REWRITE_CARD == in_pack->lvol2)
	{
		...
		{
			ret = GetXmlValue(tCard.comments,sizeof buf,XML_KEY_TRADECODE,buf);
			if(ret)
			{
				*pRetCode = ret;
				goto L_RETU;
			...
			ret = GetXmlValue(tCard.comments,sizeof buf,XML_KEY_SERIALNO,buf);
			if(ret)
			{
				*pRetCode = ret;
				goto L_RETU;
			...
			ret = GetXmlValue(tCard.comments,sizeof buf,XML_KEY_OPERATEDATE,buf);
			if(ret)
			{
				*pRetCode = ret;
				goto L_RETU;
