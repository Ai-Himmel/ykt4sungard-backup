...
int F250002(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
    ...
    writelog(LOG_ERR,"rPack->ssectypes [%s] ",rPack->ssectypes);
	...
	memset(&ElectTrans,0,sizeof(ElectTrans));
	writelog(LOG_INFO,"requestbefore 00000");
	if(rPack->lvol1==1)
	{//正式请求
	writelog(LOG_INFO,"request 111111");
		...
		writelog(LOG_INFO,"try request 33333");
