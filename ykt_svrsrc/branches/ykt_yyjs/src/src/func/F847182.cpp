...
int F847182(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	ret=DB_t_cif_shop_pos_open_select_by_c0_and_device_id(devid);
	if(ret)
	{
	...
	while(1)
	{
		...
		ret=DB_t_cif_shop_pos_fetch_select_by_c0(&tShopPos);
		if(ret)
		{
			if(DB_NOTFOUND==ret)
			{
		...
		row++;
		ret=DB_t_cif_shop_read_by_shop_id(tShopPos.shop_id,&tShop);
		if(ret)
		{
			...
			DB_t_cif_shop_pos_close_select_by_c0();
			goto L_RETU;
		}
