...
/*
	ret=DB_t_cif_cuttypefee_read_by_cut_type(rPack->lvol0,&cuttypefee);
	if(ret)
	{
		writelog(LOG_ERR,"cuttype[%d]",rPack->lvol0);
		if(DB_NOTFOUND==ret)
	...
	switch(rPack->lvol1)
	{
		case 847101:				//发行正式卡(新生卡,不收任何费用)
			if(IS_YES==cuttypefee.is_firstcostfee)
			{
				pp->damt3= cuttypefee.deposit_fee;		//押金金额
				pp->damt4=0;
				pp->damt5 = cuttypefee.in_fee;			//手续费
			}
			else
			{
				pp->damt3=0;						//押金金额
		...
		case 847104:				//补办卡
			...
			{
				pp->damt4 =cuttypefee.cost_fee;		//卡成本费
			}
			pp->damt3=0;							//押金金额
			pp->damt5 = 0;							//手续费
			break;
