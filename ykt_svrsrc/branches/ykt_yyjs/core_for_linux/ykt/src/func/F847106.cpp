...
int F847106(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	printf("\n the card_id: %d\n",card_id);//modify by ghw destination: test
	...
	/*if(1)
	{
		...
		tradeserial.out_balance=rPack->damt1+tradeserial.trade_fee;
		...
		des2src(tradeserial.physical_id,physical_no);
		...
		out_pack->lserial1 = D2I(dSerialNo);	//流水号
		out_pack->damt2=rPack->damt1+rPack->damt0;		//出卡值
		out_pack->damt3=0;			//搭伙费
