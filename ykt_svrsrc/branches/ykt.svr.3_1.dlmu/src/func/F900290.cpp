...
int Select_By_ShopBalance_Bet_Where_1(TRUSERID *handle,int *pRetCode,char *szMsg,int shopid);
...
int Select_By_ShopBalance_Bet_Where_1(TRUSERID *handle,int *pRetCode,char *szMsg,int shopid)
{
		...
		SetCol(handle,F_LVOL0,F_SALL_NAME,F_LVOL1,F_LVOL2,F_DAMT0,F_DAMT1,F_DAMT2,F_SORDER0,F_STX_PWD,0); 
	   	...
	   	{                                              
			...
			if(i==pv_tif_shopdeptfee.shopid||i==pv_tif_shopdeptfee.dept_id)
			{
				out_pack->lvol0 = pv_tif_shopdeptfee.shopid;                     
