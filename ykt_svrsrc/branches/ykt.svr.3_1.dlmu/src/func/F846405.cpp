...
int F846405(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	switch(rPack->sstation0[0])
	{
		//添加
		case 'A':
			pAccTrans->trans.transcode=MAKETRANSCODEADD(TCM_WATERFEE);
		...
		//删除
		case 'D':
			pAccTrans->trans.transcode=MAKETRANSCODEDEL(TCM_WATERFEE);
			ret=pAccTrans->InitTrans();
			if(ret)
				return ret; 		
		...
		//修改
		case 'U':
			pAccTrans->trans.transcode=MAKETRANSCODEUPD(TCM_WATERFEE);
			ret=pAccTrans->InitTrans();
			if(ret)
				return ret;
			break;
		case 'I':
			pAccTrans->trans.transcode=MAKETRANSCODEIMP(TCM_WATERFEE);
			ret=pAccTrans->InitTrans();
			if(ret)
				return ret; 		
