...
//	创建账号
	...
	ret=DB_t_subjectmap_read_by_maptype(SUBJMAPTYPE_NETACCNO,&subjectmap);
	if(ret)
	{
		if(DB_NOTFOUND==ret)
	...
	ret=DB_t_subject_read_by_subjno(subjectmap.subjno,&subject);
	if(ret)
	{
		if(DB_NOTFOUND==ret)
	...
	if(subject.subjclass!=SUBJCLASS_DEBT)
	{
		ERRTIP("卡户对应的科目类型错误");
		return E_COMMON_ERR;
	}
	...
	if(ret)
	{
		...
		des2src(tEAccount.subjno,subjectmap.subjno);
