...
(SELECT dicttype, dictval, dictcaption
   ...
   UNION ALL
	...
	UNION ALL	
    SELECT distinct -70 dicttype, to_char(subjlevel) dictval, subjlevel||'级' dictcaption FROM ykt_cur.t_subject
   UNION ALL
    SELECT distinct -71 dicttype, subjno dictval, subjname dictcaption FROM ykt_cur.t_subject where endflag=1 and subjclass=2	
   UNION ALL
