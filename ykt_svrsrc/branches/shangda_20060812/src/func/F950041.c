...
int F950041(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	else if(tDevice.phytype= 1000)
	{
		memset(rPack->sdate0,0,sizeof(rPack->sdate0));
		memcpy(rPack->sdate0,tDevice.devphy999_id,8);
		return F930006(handle,iRequest,rPack,pRetCode,szMsg);
	}
