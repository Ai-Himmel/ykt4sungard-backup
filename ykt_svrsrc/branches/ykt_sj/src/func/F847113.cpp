...
int F847113(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
  	int ret = 0;
	int	card_id = 0;
	char	Operator[33+1] = "";
	...
	{
		ret=DB_t_cif_customer_read_by_stuemp_no(rPack->scust_limit,&customer);
		if (ret)
			{
			*pRetCode=E_DB_CUSTOMER_R;
				goto L_RETU; 
		}
		writelog(LOG_ERR,"cut_name[%s],scard0[%s]",customer.cut_name,rPack->scard0);
		if (strncmp(customer.cut_name,rPack->scard0,strlen(customer.cut_name))!=0)
		{
		*pRetCode= E_STUEMPANDNAME;
		goto L_RETU; 
		}
	}
	card_id = rPack->lvol1; 
	...
	if(ret)
	{
		*pRetCode=ret;
		goto L_RETU;
													...
													//卡标识													//客户号
	des2src(Operator,rPack->semp);									//操作员号
	maindevice_id = rPack->lvol6;											//上传工作站标识
	device_id = rPack->lvol7;												//采集设备标识
	cut_id=rPack->lvol8;
	if(maindevice_id==0&&device_id!=0) 
	{
		DB_t_pif_device_read_by_device_id(device_id, &t_device);
		maindevice_id=t_device.subsystem_id;
	}

	des2src(stuemp_no,rPack->scust_limit);
	if(0==card_id)
	{
		if(0==cut_id)
		{
...
//	
	...
	if(strlen(Operator)==0)
		strcpy(tradeserial.oper_code,"system");
	else
		des2src(tradeserial.oper_code,Operator);
