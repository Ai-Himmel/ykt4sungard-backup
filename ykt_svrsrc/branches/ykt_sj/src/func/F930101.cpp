...
//		tTxdtl.work_mark=rPack->lvol10;					//上下班标志
	...
	// 根据东华大学需求增加考勤的采集时间(tx_date, tx_time)和上传时间(col_date, col_time)
	if (strlen(tTxdtl.phycard_no) == 0)
	{
	    sprintf(phycard_no, "%08d", tTxdtl.card_no);
	}	
	memcpy(tTxdtl.phycard_no, phycard_no, sizeof(tTxdtl.phycard_no) - 1);	
	...
	if (ret)
	{
		...
		if (DB_REPEAT == ret)
		{
			writelog(LOG_ERR, "流水重复card=[%d],collect_date=[%s],collect_time=[%s]", tTxdtl.card_no, tTxdtl.tx_date, tTxdtl.tx_time);
			return 0;
		}
		
		writelog(LOG_ERR, "保存流水失败card=[%d],collect_date=[%s],collect_time=[%s]", tTxdtl.card_no, tTxdtl.tx_date, tTxdtl.tx_time);
