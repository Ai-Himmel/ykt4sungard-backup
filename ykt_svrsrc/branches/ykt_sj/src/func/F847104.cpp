 ...
 * 修改日期: 2008－3－26
 * 修改人员: 汤成
 * 修改描述: 由于修改的下发补助时即记入个人账户，因此补办卡时只将未领取补助的卡号更新为新的用户卡号
 * 版本信息:
 * 备注信息:
 * --------------------------------------------*/
...
static int process(InAcc *IA,T_t_tif_tradeserial *p)
{
	...
	ret = DB_t_aif_account_read_by_account_id(IA->sArrInActno[0],&account);
	if(ret)
	{
		if(DB_NOTFOUND==ret)
	...
	p->out_balance=account.cur_freebala;		//出卡值
	return 0;
}
static int do_change_subsidy(T_t_aif_account *account,int old_card_id)
{
	...
	if(ret)
	{
		if(DB_NOTFOUND == ret)
			return 0;
		writelog(LOG_ERR,"open subsidy card_id[%d],ret[%d]",old_card_id,ret);
	...
	while(1)
	{
		...
		if(ret)
		{
			if(DB_NOTFOUND == ret)
				break;
			writelog(LOG_ERR,"read subsidy card_id[%d],ret[%d]",old_card_id,ret);
		}
		// 新的交易卡号
		//tSubsidy.card_no=account->card_id;
...
int F847104(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
    ...
    */
	...
	// 更新未领取金额
	tAccount.cur_freebala += tAccount.reserve_bala;
	tAccount.reserve_bala = 0.0;
	tAccount.subsidy_no = 0;
	...
	// 更新未领取补助信息表
	ret = do_change_subsidy(&tAccount,lost_card_id);
	if(ret)
	{
		*pRetCode = ret;
		goto L_RETU;
	...
	IA.iArrInFeeSwitch[0]=CHECK_FLAG(rPack->lvol0);
	IA.iArrInFeeSwitch[1]=CHECK_FLAG(rPack->lvol1);
	IA.iArrInFeeSwitch[2]=CHECK_FLAG(rPack->lvol2);
	IA.iArrInFeeSwitch[3]=CHECK_FLAG(rPack->lvol3);
	IA.iArrInFeeSwitch[4]=CHECK_FLAG(rPack->lvol4);
	IA.iArrInFeeSwitch[5]=CHECK_FLAG(rPack->lvol5);
	IA.iArrInFeeSwitch[6]=CHECK_FLAG(rPack->lvol6);
	IA.iArrInFeeSwitch[7]=CHECK_FLAG(rPack->lvol7);
	IA.iArrInFeeSwitch[8]=CHECK_FLAG(rPack->lvol8);
	IA.iArrInFeeSwitch[9]=CHECK_FLAG(rPack->lvol9);
	IA.iArrInFeeSwitch[10]=CHECK_FLAG(rPack->lvol10);
