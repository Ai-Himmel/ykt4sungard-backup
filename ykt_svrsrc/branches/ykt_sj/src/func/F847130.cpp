...
int F847130(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	if(rPack->scust_type[0]=='1')
	{
		...
		/*
		ret = calc_import_max_subsidy_no(tSubsidyTmp.batch_no,tSubsidyTmp.oper_code,&subsidy_no);
		if(ret)
		{
			*pRetCode = ret;
		...
		*/
		//最后一条,开始把临时表数据导入到正式表
		writelog(LOG_DEBUG,"导入第%d条记录,导入批次号:%s,客户号%d",
				 tSubsidyTmp.seqno,tSubsidyTmp.batch_no,tSubsidyTmp.cust_no);
		iCnt=0;
		...
		while(1)
		{
			...
			if(((tSubsidyTmpOut.amount>0)&&incr==-1 )||((tSubsidyTmpOut.amount<0)&&incr==1))
			{
				*pRetCode=E_SUBSIDY_AMOUNT_ERROR;
				sprintf(szMsg,"导入批次号%s,第%d条记录有误,系统不允许一批补助存在正负两种金额",
						tSubsidy.batch_no,tSubsidy.seqno);
				goto L_RETU;
			}
			...
			if(ret)
			{
				...
				sprintf(szMsg,"导入批次号%s,第%d条记录有误,可能该学号的补助记录已经存在",
						tSubsidy.batch_no,tSubsidy.seqno);
				goto L_RETU;
			}
		...
		sprintf(out_pack->vsmess,"导入批次号:%s,共成功导入%d条补助信息,补助金额总共%.2lf元",
				tSubsidyTmp.batch_no,iCnt,dTotalAmt);
		flag=2;
	}
	if(flag!=2)
	{
