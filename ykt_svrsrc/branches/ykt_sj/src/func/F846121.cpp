 ...
 * 
 * 修改日期: 2009-12-21
 * 要把操作时间也加上，以免同一天的连续几笔都失败，那么交易次数是一样的
 * --------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int F846121(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	char phy_cardno[9] = "";
	char op_date[9] = "00000000";
	char op_time[7]="000000";
	...
	getsysdate(sysdate);
	getsystime(systime);
	ret = DB_t_pif_fixcard_read_by_cardno_and_tradecnt_and_operate_date_and_operate_time(card_no, trade_cnt, op_date,op_time, &tFixcard);
	if(ret ==0)			
	...
	else
	{
		...
		des2src(tFixcard.operate_time,op_time);
