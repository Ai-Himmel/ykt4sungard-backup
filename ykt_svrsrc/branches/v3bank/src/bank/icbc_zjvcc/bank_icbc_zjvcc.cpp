...
void str2struct(char *buf,Message *j) //拆分
{
	int len,i;
	char *p,*q,*str[20];
	len=strlen(buf);
...
static int Bank_Process(char *type , TRUSERID *handle,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	if(Connect(&SockNode,bank_ip,bank_port,bank_delay,szMsg)==false)  //连接银行
	{
		return E_TRANS_BANK_NETERR;
	} 
	if(Send(&SockNode,send_buf,szMsg)==false) //发送数据到银行
	{
		return E_TRANS_BANK_NETERR;
	} 
	if(Receive(&SockNode,recv_buf,sizeof(recv_buf),bank_delay,szMsg)==false) //接收银行返回信息
	{	
		return E_TRANS_BANK_NOANSWER;
	} 
	if(Is_success(recv_buf,&recv_Msg,szMsg)==false) //判断银行返回交易是否成功
	{
		return E_TRANS_BANK_RECVERR;
	}  
	if(Result(type,&recv_Msg,handle,pRetCode,szMsg)==false) //返回结果
	{	
