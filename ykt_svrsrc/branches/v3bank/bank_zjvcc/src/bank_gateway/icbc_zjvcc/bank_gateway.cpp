...
static void doSetdata(char* type,KS_YKT_Clt &ykt_clt,Message &msg) //设置数据
{
...
static void doSetErr(KS_YKT_Clt &ykt_clt,Message &msg,char errmsg[]) //出错处理
{
...

{
	...
	if(strcmp(type,"A")==0)
		des2src(msg.retmessage,"签约成功7");
	else if(strcmp(type,"D")==0)
		des2src(msg.retmessage,"撤约成功7");
	return 0;
}

static int doResponse(char *recv_buf,int recv_len,char *send_buf,int &send_len)
{
	KS_YKT_Clt ykt_clt;
	int retcode = 0;
	Message msg;
	str2struct(recv_buf,&msg);
	print_recv_msg(&msg);
	if(strcmp(msg.txcode,"10000")==0)  //10000：新增委托关系
	{
		//retcode = doBind(ykt_clt,msg);
		retcode =doRun("A",ykt_clt,msg);
	}
	else if(strcmp(msg.txcode,"10001")==0) //  10001：撤销委托关系
	{
		//retcode = doUnbind(ykt_clt,msg);
		retcode =doRun("D",ykt_clt,msg);
	}
	else
	{
		return E_TRANS_NONSUPPORT;
	}
	struct2str(&msg,send_buf);
