...
public:
   ...
   // 参数  : int timeout : 等待接收数据的超时时间，毫秒计
	int Recv(char *vptr, int nDesignedSize, int init_timeout=3);  // 通过本链路接收数据 返回  : int -1:被对方正常关闭；-10:输入的nDesignedSize<=0；-2:因为链路错误而被关闭；0:超时；-3:在接收数据的时候，出错而被关闭
	...
	bool ConnectTcp(char* host, short port,long timeout = 5);
