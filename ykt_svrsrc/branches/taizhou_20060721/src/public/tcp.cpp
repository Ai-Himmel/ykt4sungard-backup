...
bool CTcpSocket::Listen(short iPort)
{
	if (m_socket==INVALID_SOCKET)
		return(false);
	sockaddr_in addr;
...
#endif
bool CTcpSocket::ConnectTcp(char *host, short port,long timeout)
{
	unsigned int inaddr;  
	struct hostent *ht;
	...
	// 2006-8-14 modify by 汤成
	// 修改为 non-blocking 连接方式
	if(enable_nonblock(true))
	{
		Close();
		return false;
	}
	...
	tv.tv_sec = timeout; 
	tv.tv_usec = 0;
	e = select((int)m_socket + 1,&rdset,&wrset,NULL,timeout ? &tv : NULL);
	if(e <= 0)
	{
		Close();
		return false;
	}
	if(FD_ISSET(m_socket,&rdset) || FD_ISSET(m_socket,&wrset))
	{
		socklen_t optlen = sizeof(e);
		getsockopt(m_socket,SOL_SOCKET,SO_ERROR,(char*)&e,&optlen);
		if(e)
		{
			Close();
			return false;
		}
		if(recv(m_socket,0) == -1)
		{
			Close();
			return false;
		}
	}
	if(enable_nonblock(false))
	{
		Close();
		return false;
	}
...
// 2006-8-14 modify by 汤成
// 增加设置 non-blocking 标志的开关函数
int CTcpSocket::enable_nonblock(bool flag)
{
	if(m_socket == INVALID_SOCKET)
		return -1;
#ifdef WIN32
