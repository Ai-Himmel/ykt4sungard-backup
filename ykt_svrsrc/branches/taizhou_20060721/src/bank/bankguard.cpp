...
int main(int argc,char *argv[])
{
	...
	INNER_TRANS_REQUEST bank_trans;
	MSG8583	 boc_trans;
	...
	while(true)
	{
		...
		if((now - lastflush) > intflush)
		{
			...
			if( ret == 0)
			{
				...
				ret = Do_DeTrans(&boc_trans,&tradeserial); //, &tradeserial);
				if(ret == 0)
				{
					// 更新该冲正记录状态（发送时间、次数）
					// 保留银行端流水号BankSn
					sprintf(tradeserial.reserve_1,"%ld", boc_trans.mainTrcNo);
				...
				{
					DB_t_tif_tradeserial_done_flush(&tradeserial);
					writelog(LOG_INFO,"补发冲正记录失败,流水号[%ld],错误[%d]", tradeserial.serial_no,ret);
				}
				ret=db_commit();
				if(ret)
				{
