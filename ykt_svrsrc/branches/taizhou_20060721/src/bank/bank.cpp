...
//	char temp[5]="";
	...
	{
	      	// 发送时，端口有问题，包括被关闭
		writelog(LOG_ERR,"Send to bank error,ret=[%d], errcode=[%d] ",ret, errno);
		return E_TRANS_BANK_NETERR;
	}
	else if (0==ret)
	{
	...
	if(sock.CheckDataArrived(delay_time))
	{
		sock.Accept(rcv_sock);
	...
	writelog(LOG_ERR,"bank no answer!!");
	return E_TRANS_BANK_NOANSWER;
}
...
*/
int Do_DeTrans  ( MSG8583 *pBankTrans,T_t_tif_tradeserial *pTradeserial)
{
	...
	ret=DB_t_tif_tradeserial_read_by_serial_no(pTradeserial->other_seri_no, &old_Trans);
	if(ret)
	{
		writelog(LOG_ERR,"DB_t_tif_tradeserial_read_by_serial_no error,error code=[%d]",ret);
		return E_TRANS_UNKNOW_ERROR;
	}
	
	memcpy( BankReqPak.msgType,"0400", sizeof(BankReqPak.msgType)-1);					//0//交易代码   
	//查询交易，为了满足位图要求，填写0
	BankReqPak.bitMapExt=1;															//1//扩展位图
	strncpy(BankReqPak.priActNum,pTradeserial->b_act_id,sizeof(pTradeserial->b_act_id)-1);	//2//主账号--银行卡号
	//strncpy(BankReqPak.priActNum,"4563516107002318007",sizeof(BankReqPak.priActNum)-1);	
	strncpy(BankReqPak.procCode,"488833",sizeof(BankReqPak.procCode)-1);					//3//过程码
	BankReqPak.txnAmunt=old_Trans.trade_fee;														//4//交易金额
	strncpy(BankReqPak.txnDatTim,"061102121212",sizeof(BankReqPak.txnDatTim));				//7//银行后台交易日期--填写0
	strncpy(BankReqPak.companyid,"44630021",sizeof(BankReqPak.companyid)-1);				//8//公司代码
	//填写要冲正的流水号
	BankReqPak.tracAudNum=pTradeserial->serial_no;														//11//流水号
	...
	//客户标识，要修改为客户号，或者学工号
	sprintf(BankReqPak.agtcus,"%019d",pTradeserial->customer_id);			//43//客户标识
	//strncpy(BankReqPak.agtcus,"4002699752",sizeof(BankReqPak.agtcus)-1);					//43//客户标识
	BankReqPak.pinDat=100;															//52//pindat
	strcpy(BankReqPak.OrgDatElm.acqInstID,"4446301");
	strcpy(BankReqPak.OrgDatElm.forInst,"4446301");
	strcpy(BankReqPak.OrgDatElm.msgType,"0200");
	BankReqPak.OrgDatElm.tracNum=pTradeserial->other_seri_no;
	memcpy(BankReqPak.OrgDatElm.txnDatTim,old_Trans.operate_date+2,4);
	memcpy(BankReqPak.OrgDatElm.txnDatTim+4,old_Trans.operate_time,6);

writelog(LOG_ERR,"BankReqPak.OrgDatElm.acqInstID=[%s]",BankReqPak.OrgDatElm.acqInstID);
writelog(LOG_ERR,"BankReqPak.OrgDatElm.msgType=[%s]",BankReqPak.OrgDatElm.msgType);
writelog(LOG_ERR,"BankReqPak.OrgDatElm.tracNum=[%d]",BankReqPak.OrgDatElm.tracNum);
writelog(LOG_ERR,"BankReqPak.OrgDatElm.txnDatTim=[%s]",BankReqPak.OrgDatElm.txnDatTim);
	...
	memset(&BankReqPak,0,sizeof(BankReqPak));
	ret=UnPack8583((unsigned char*)head.msg,&BankReqPak,2);
...
writelog(LOG_ERR,"BankAnsPak.OrgDatElm.acqInstID=[%s]",BankAnsPak.OrgDatElm.acqInstID);
writelog(LOG_ERR,"BankAnsPak.OrgDatElm.msgType=[%s]",BankAnsPak.OrgDatElm.msgType);
writelog(LOG_ERR,"BankAnsPak.OrgDatElm.tracNum=[%d]",BankAnsPak.OrgDatElm.tracNum);
writelog(LOG_ERR,"BankAnsPak.OrgDatElm.txnDatTim=[%s]",BankAnsPak.OrgDatElm.txnDatTim);
...
writelog(LOG_ERR,"BankAnsPak.tracAudNum=[%d],BankReqPak.tracAudNum=[%d]",BankAnsPak.tracAudNum,BankReqPak.tracAudNum);
	...
	memcpy(pBankTrans,&BankAnsPak,sizeof(BankAnsPak));
...
///*
	...
	//对总数
	ret=DB_his_t_tif_tradeserial_get_count_by_serial_type(&temp1,240001,forward_date);
	if(ret!=100&&ret!=0)
	{
		writelog(LOG_ERR,"DB_t_tif_tradeserial_get_count_by_serial_type err,errcode=[%d]",ret);
		goto LRet;
	}
	trans_count=trans_count+temp1;
