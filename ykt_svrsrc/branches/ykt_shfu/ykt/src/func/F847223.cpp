...
static int do_calc_shop_balance(int shop_id,double * balance)
{
	...
	while(1)
	{
		...
		writelog(LOG_ERR,"cif_shop_tree_fetch shop_id[%d]",shop_id);
	...
	writelog(LOG_ERR,"end while free_bala[%f]",free_bala);
	*balance = free_bala;
	return 0;
}
...
int F847223(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	ret = do_calc_shop_balance(shop_id,&shop_bala);
	if(ret)
	{
