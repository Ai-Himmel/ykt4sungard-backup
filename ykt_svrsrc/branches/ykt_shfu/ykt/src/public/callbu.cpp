...
int init_bccextpack(T_t_pif_device *device,BCC_EXT_PACK *ext_pack)
{
	int ret;
	T_t_subsysparams subsystem;
	memset(&subsystem,0,sizeof subsystem);
	ret = DB_t_subsysparams_read_by_sysid(device->subsystem_id,&subsystem);
	if(ret)
	{
		if(DB_NOTFOUND == ret)
	...
	ext_pack->drtp_node = subsystem.bcc_drtp_no;
	ext_pack->bcc_main_func = subsystem.bcc_main_func;
	return 0;
}
