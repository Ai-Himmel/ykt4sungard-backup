...
static int check_device(char* device_id,T_t_pif_device* device)
{
	int ret=0;
	...
	ret=get_devinfo_by_phyid(device_id,device);
	if(ret)
	{
		writelog(LOG_ERR,"device_id[%s]ret[%d]",device_id,ret);
		return ret;
	}
	if(strcmp(device->devtype,DEVITYPE_JSB) && strcmp(device->devtype,DEVITYPE_MOBILE_JSB))
	{
		return E_DEVTYPE_NOT_JSP;
	}
	return 0;
}
int check_cardno(int card_no,T_t_pif_card* tCard)
{
	int ret=0;
...
int F848006(TRUSERID *handle,int iRequest,ST_PACK *in_pack,int *pRetCode,char *szMsg)
{
	...
	ret=check_device(tDoorDeviceCardlist.device_id,&tDevice);
	if(ret)
	{
		*pRetCode=ret;
	...
	ret=check_cardno(tDoorDeviceCardlist.card_no,&tCard);
	if(ret)
	{
		card_status = ret;
	...
	if(strcmp(tDevice.devtype,DEVITYPE_MOBILE_JSB))
	{
	...
	tDoorDeviceCardlist.cut_id = tCard.cosumer_id;
	// 关闭 db_chk_err
	g_dbchkerr = 0;
	...
	{
		// 如果是手持设备,不需要下传名单
		if(strcmp(DEVITYPE_MOBILE_JSB,tDevice.devtype)!=0)
		{
