...
//	把水控小钱包余额转入消费钱包
	...
	Pack.lvol0=rPack->lvol1; 		//卡号
	Pack.damt1=rPack->damt2;	//钱包1余额
	Pack.damt3=rPack->damt3;	//钱包2余额;
	Pack.damt4=rPack->damt3; 	//钱包2转出金额
	Pack.lvol4=rPack->lvol10;		//钱包1卡交易次数
	Pack.lvol5=rPack->lvol11;		//钱包2卡交易次数
	des2src(Pack.scust_no, rPack->scust_limit);	//操作员号
	
	if(amtcmp(Pack.damt3,0)>0)
	{
		ret=F847307(handle,TXCODE_CALLBACK_CARD,&Pack,pRetCode,szMsg);
		if(ret)
		{
			writelog(LOG_ERR,"F847307 ret[%d]",*pRetCode);
			goto L_RETU;
		}
		rPack->damt2=Pack.damt5;//入卡值
	}
	writelog(LOG_DEBUG,"F847307 处理成功");
	memset(&tCard,0,sizeof(tCard));
