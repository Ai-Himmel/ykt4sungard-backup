...
static int doTrans(int tradecode,ST_PACK *rPack,T_t_transdtl  *transdtl,char *errmsg)
{
	...
	SetHeadCol(&sPack,F_LCERT_CODE,F_SCUST_AUTH,F_LVOL0,F_LVOL1,F_LVOL2,F_LVOL6,F_SDATE0,F_STIME0,F_SNAME,
		F_SHOLDER_AC_NO,F_SPHONE,F_SCUST_LIMIT,F_SCUST_LIMIT2,F_SCUST_AUTH,F_SPHONE2,F_SPHONE3,F_SCUST_AUTH2,F_DAMT0,F_SBANK_PWD,
		F_VSVARSTR0,F_VSVARSTR1,F_SORDER2,F_SEMP_PWD,0);      
	...
	{
            //return E_DB_AREA_R;
		writelog(LOG_ERR,"read area err custid[%d] ",customer.custid);
	}
	...
	sPack.pack.lvol2= customer.custid;							// 客户号
	...
	des2src(sPack.pack.sphone2,area.remark);				        //学校代码
	...
	des2src(sPack.pack.sname,transdtl->voucherno);					// 银行流水号
...
//	}
	...
	trans.aftdpscnt=transdtl.cardcnt;
	trans.dpscnt=trans.aftdpscnt-1;
	...
	ret=CalcPrestoreFee(trans.feetype,trans.inputamt,&dPrestoreFee);
	if(ret)
	{
	...
	transdtl.managefee = dPrestoreFee;
