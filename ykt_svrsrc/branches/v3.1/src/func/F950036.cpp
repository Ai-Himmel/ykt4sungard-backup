...
int F950036(TRUSERID *handle, int iRequest, ST_PACK *in_pack, int *pRetCode, char *szMsg)
{
    ...
    writelog(LOG_INFO, "接收考勤前置刷卡流水：deviceid=[%d],FDEVICEID=[%d],DEVICENO=[%d],CARDNO=[%d],CARDPHYID=[%s]",
             in_pack->lvol5, in_pack->lvol2, in_pack->lvol1, in_pack->lvol0, in_pack->sdate1);
    // 查询设备ID号
    if (in_pack->lvol5 > 0)
    {
    ...
    if (ret)
    {
        writelog(LOG_ERR, "获取设备信息失败：deviceid=[%d],FDEVICEID=[%d],DEVICENO=[%d],ret=[%d]",
                 in_pack->lvol5, in_pack->lvol2, in_pack->lvol1, ret);
        if (ret == E_DB_DEVICE_N)
        {
            writelog(LOG_ERR, "上传记录找不到对应的设备：termid[%d]type[%s]parent[%d][%d],sys[%d],phytype[%d]",
    ...
    else
    {
        ...
        des2src(tDoordtl.showcardno, tCard.showcardno);
        if (!ret)
        ...
        */
        // 为防止流水丢失，流水号取自设备上传
        tDoordtl.devseqno = in_pack->lvol6;
        // 保存门禁流水
