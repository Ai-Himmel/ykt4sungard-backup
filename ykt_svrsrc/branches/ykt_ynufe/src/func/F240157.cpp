...
int F240157(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	T_t_tif_report_trans_comp tReportcomp;
	ST_CPACK aPack;
	...
	memset(&tReportcomp,0,sizeof tReportcomp);
	...
	{
		if(tCard.cosumer_id != tDiffTransfer.cut_id)
		{
			writelog(LOG_ERR,"input card_id [%d],cut_id[%d],DiffTransfer card_id[%d],cut_id[%d]",
				card_id,tCard.cosumer_id,tDiffTransfer.card_id,tDiffTransfer.cut_id);
			DB_t_tif_diff_transfer_free_lock_by_c0();
			*pRetCode=E_CARDNO_SERIAL_NE;
			goto L_RETU;
		}
		tDiffTransfer.card_id = card_id;				// 将圈存补写的卡号记录下来
	}
...
//	tradeserial.device_id = rPack->lvol7;							//采集设备标识
	...
	ret = DB_t_tif_report_trans_comp_read_by_cmp_date(rPack->sdate0,&tReportcomp);
	if(ret)
		return ret;

	tReportcomp.adjust_flag =1;
	tReportcomp.localauto_amt+=  rPack->damt1;
	tReportcomp.localauto_num += 1;
	ret = DB_t_tif_report_trans_comp_update_by_cmp_date(rPack->sdate0,&tReportcomp);
	if(ret)
		return ret;
