...
static int doReverse(char* oldaccdate,int oldtermid,int oldtermseqno,char *cardphyid)
{
	...
	if(strcmp(transdtl.opercode,trans.opercode)!=0)
	{
		DB_t_transdtl_free_lock_by_c0();
		writelog(LOG_ERR,"开户操作员[%s]冲正操作员[%s]",transdtl.opercode,trans.opercode);
		return E_REVOPER;
	}	
...
//	getfmtsysdatetime(tCardBook.lastsaved);
	...
	*/
	...
	ret=DB_t_account_read_by_cardno_and_purseno(transdtl.cardno,transdtl.purseno,&tCardAccount);
	if(ret)
	{
