...
static int doReverse(char* oldaccdate,int oldtermid,int oldtermseqno)
{
	...
	ret=DB_v_transdtl_read_lock_by_c0_and_accdate_and_termid_and_termseqno(oldaccdate,oldtermid,oldtermseqno,&vTransdtl);
	if(ret)
	{
	    if(DB_NOTFOUND==ret)
	...
	if(vTransdtl.transcode!=TC_POSDRAW&&
		vTransdtl.transcode!=TC_POSDRAW&&
		vTransdtl.transcode!=TC_CARDPAYMENT)
	{
		DB_v_transdtl_free_lock_by_c0();
	...
	ret=DB_v_transdtl_update_lock_by_c0(&vTransdtl);
	if(ret)
	{
...
//	strcpy(cardphyid,tCard.cardphyid);
	...
	transdtlrev.cardaftbal=transdtlrev.cardbefbal+(vTransdtl.cardbefbal-vTransdtl.cardaftbal);
	...
	trans.cardflag=1;
	trans.cardno=transdtlrev.cardno;
	trans.cardcnt=transdtlrev.cardcnt;
