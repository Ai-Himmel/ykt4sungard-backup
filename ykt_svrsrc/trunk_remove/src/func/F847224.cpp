...
int F847224(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	writelog(LOG_DEBUG,"计算商户余额shopid[%d]",shopid);
	shopid = strtoul(rPack->sbank_acc,NULL,10);
	memset(&shop,0,sizeof shop);
	...
	ret = do_calc_shop_balance(shop.shopid,&(out_pack->damt1));
	if(ret)
	{
		writelog(LOG_DEBUG,"计算商户余额错误shopid[%d]",shop.shopid);
		*pRetCode = ret;
		goto L_RETU;
	}
