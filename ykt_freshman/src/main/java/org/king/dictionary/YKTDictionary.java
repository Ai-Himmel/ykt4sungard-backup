...
import org.king.utils.exception.DictionaryLoadErrException;
...
public class YKTDictionary {
	...
	public YKTDictionEntry getDictionEntry(String dict) throws DictionaryLoadErrException {
		DictEntry entry = (DictEntry) _dicts.get(dict);
		if (null == entry) {
			return null;
		}
		...
		if (currTick - entry._timeTick > _timeInterval) {
			...
			if (!entry._entry.loadDiction(entry._dict)) {
				throw new DictionaryLoadErrException("¼ÓÔØ×Öµä[" + dict + "]Ê§°Ü");
			}
	...
	private DictEntry getDictEntry(String dict) throws DictionaryLoadErrException {
		DictEntry entry = (DictEntry) _dicts.get(dict);
		if (null == entry) {
			return null;
		}
	...
	public Object getDictionValue(String dict, String key) {
		...
		try {
			...
			return null;
		} catch (DictionaryLoadErrException e) {
