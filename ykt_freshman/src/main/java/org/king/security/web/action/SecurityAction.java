	...
	public ActionForward login(ActionMapping mapping, ActionForm form,
		...
		if (account == null) {
			try {
				...
				//String is_config = "/opt/WebSphere/AppServer/config/client.properties";
		...
		if ((destinatedUrl != null) && (destinatedUrl.length() > 0)) {
			...
			MainpageAccess mainpageAccess=new MainpageAccess();
			try {
				...
				mainpageAccess.setUserName((String)session.getAttribute("userName"));
				mainpageAccess.setTrueName((String)session.getAttribute("personName"));
				mainpageAccess.setAccessDate(new java.sql.Date(Integer.parseInt(dateStr.substring(0,4))-1900,Integer.parseInt(dateStr.substring(5,7))-1,Integer.parseInt(dateStr.substring(8))));		    
				mainpageAccess.setAccessTime(new Time(Integer.parseInt(timeStr.substring(0,2)),Integer.parseInt(timeStr.substring(3,5)),0));
				mainpageAccess.setIpAddress(request.getRemoteAddr());
				
				 if(userAccessService==null)
					 System.out.println("service is null");
				 			 
				 userAccessService.saveMain(mainpageAccess);
