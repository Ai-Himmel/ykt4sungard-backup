...
public class GzCheckinsubminAction extends BaseAction{
...
//			起始页
			 ...
			 String viewclassNo=request.getParameter("viewclassNo");
          	 //指定班级的新生报到信息列表
			 String sqlstring="";
			 sqlstring="select new Map(ci.id as id,ci.studentNo as studentNo,ci.enrollNo as enrollNo,ci.enrollYear as enrollYear,s.stuName as stuName,s.gender as gender,s.nationality as nationality,s.department as department,s.majorin as majorin,c.classNo as classNo,ci.ifCheckedin as ifCheckedin,ci.reason as reason,ci.ifChecked as ifChecked,ci.ifReserve as ifReserve,ci.ifLeaveSchool as ifLeaveSchool,ci.ifGotMaterial as ifGotMaterial,ci.ifGotEcard as ifGotEcard,ci.ifGotStucard as ifGotStucard,ci.memo as memo,ci.ifSubmitted as ifSubmitted,ci.updatorId as updatorId,ci.updateTime as updateTime) from Stucheckininfo ci,Studentmodel s,Classmodel c where ci.studentNo=s.studentNo and s.yxClass.classId=c.classId and c.yxCollege.yxSchool.schoolId='0002' and c.curYear='"+DateUtil.getNow("yyyy")+"' and ci.ifCheckedin='0' ";
...
//					国籍
	...
	public void ExportList(List list,HttpServletResponse response){
       ...
       //Populate the first row of the spreadsheet with the column names
		...
		columnNames.add(4,"报到状态");
		columnNames.add(5,"未报到原因");						
		columnNames.add(6,"特殊情况");
