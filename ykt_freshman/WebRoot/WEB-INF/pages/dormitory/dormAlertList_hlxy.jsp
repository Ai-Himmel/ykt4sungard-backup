...
<%@ page import="org.king.classmanage.web.help.StudentWithAllInfoDorm"%>
...
function onSearch(){
	...
	&&(document.all.roomNumber.value=="")){
		document.forms[0].action='DormitoryAdjustAction.do?method=searchPeopleAdjust&getrole=2';
		return true;
	}
	document.forms[0].action='DormitoryAdjustAction.do?method=searchPeopleAdjust&getrole=2';
	return true;		
	...
	<table width="96%" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		<tr>
			<td align="center" valign="top">
				<html:form action="DormitoryAdjustAction.do?method=savePeopleAlert&&getrole=2">
					<tr>
						<td>
									...
									<tbody>
										<tr align="center" valign="middle">
											...
											<td align="left" valign="middle">
												<html:text property="stuNo" styleClass="input_box" style="width:120"/>
											</td>
										</tr>
										<tr align="center" valign=middle>
											<td width="30%" height="24" align="right" valign="middle" bgcolor="#DCC9CA" class="tableHeader2">
											...
											<td align="left" valign="middle">
												<html:text property="stuName" styleClass="input_box" style="width:120" />
											</td>
										</tr>
										<tr align="center" valign=middle>
											<td width="30%" height="24" align="right" valign="middle" bgcolor="#DCC9CA" class="tableHeader2">
								...
								<table width=99% border=1 align=center cellpadding=1 cellspacing=0 bordercolor=#FFFFFF id=submenu1>
									...
									<tr align="center" valign=middle>
										...
										<tr align="center" valign=middle>
											...
											<td align="left" valign="middle">
												<html:text property="roomNumber" styleClass="input_box" style="width:120"/>
											</td>
										</tr>
										<tr align="center" valign=middle>
											<td height="24" colspan="2" align="right" valign="middle" class="medium">
...
<script language="JavaScript">
	...
	if(request.getAttribute("unbindStudentList")!=null){	
		...
		for(k=0;k<unbindStudentList.size();k++){
			StudentWithAllInfoDorm unbindStu= (StudentWithAllInfoDorm)unbindStudentList.get(k);
			String stuId = unbindStu.getStudentId();
			String stuNo = unbindStu.getStudentNo();
			String stuName = unbindStu.getStuName();
			String gender = unbindStu.getGender();
			String district = unbindStu.getDistrict();
			String nation = unbindStu.getNation();
	%>
		unbindStudents[<%=k%>]= new Array("<%=stuId%>","<%=stuNo%> <%=stuName%>[<%=gender%>][<%=district%>]","<%=gender%>","0","null"); 
	...
	if(request.getAttribute("bindStudentList")!=null){	
		...
		for(m=0;m<bindStudentList.size();m++){
			...
			if(bindStu.get("district")!=null){
				district = bindStu.get("district").toString();	
			}		
			dormitoryId = bindStu.get("dormitoryId").toString();
			accomodation = bindStu.get("accomodation").toString();
	%>
		bindStudents[<%=m%>]= new Array("<%=stuId%>","<%=stuNo%> <%=stuName%>[<%=gender%>][<%=district%>]","<%=gender%>","<%=accomodation%>","<%=dormitoryId%>"); 
	...
	<%	
		...
		if(bindStudentList.size()==0&&request.getAttribute("dormmo")!=null){
						...
						accomodation = Integer.toString(dormmo.getDistributeNum());
		}		
	%>
	if(document.all["mydormitoryId"].value==""){
		...
		if(document.getElementById("targetRoom")!=null){
			document.getElementById("targetRoom").innerHTML='<strong class="medium">目标宿舍已住人员 <font color="#7F0055">[<%=gender%>][<%=bindStudentList.size()%>/<%=accomodation%>]</font></strong>'
		}
	}
