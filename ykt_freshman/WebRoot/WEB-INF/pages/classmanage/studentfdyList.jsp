...
function newpage(){
	window.open ('studentAction.do?method=load4StudentfdyListColumnSelect','studentListColumnSelect','height=400,width=600,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no'); 
}
		...
		<table width="<bean:write name="width"/>" border="0" align="center" cellpadding="0" cellspacing="0">
			<tr>
				<td height="30" align="center" valign="middle" class="tdborder02">
					<TABLE  align="center">
						<tr>
							<td>
								<strong><font class="medium"> <%String myclassNo = "";
			if (request.getAttribute("myclassNo") != null) {
				myclassNo = request.getAttribute("myclassNo").toString();

				%><%=myclassNo%>班<%} else {

			%>本班<%}

			%>新生基本信息查询</font></strong>
			...
			<tr>
				<td align="left" valign="top" width="50%">
					<html:form action="studentAction.do?method=load4fdySearch">
			...
			<tr>
				...
				<a href="javascript:DoExport()">导出Excel<IMG src="./pages/style/default/images/xls.gif" border="0"></a>
								<strong><font class="medium"> 
							    <logic:notEmpty name="list_Assistant">
								    <logic:iterate name="list_Assistant" id="assistant" type="org.king.classmanage.domain.Managermodel">
								 	导师:<c:out value="${assistant.managerName}"/>
								 	</logic:iterate>
							 	 </logic:notEmpty>								
								</font></strong>				
					&nbsp;
						...
						<tbody>
							...
							<logic:equal name="studentlc" value="1" property="studentNo">
								...
								<td valign="middle" class="tableHeader">
									...
									<logic:equal name="studentlc" value="1" property="fromProvince">
								<td valign="middle" class="tableHeader">
								...
								<td valign="middle" class="tableHeader">
									<a style="font-size:14px;color: black" href="javascript:DoSort('location')"><bean:message bundle="classmanage" key="studentsearch.location" /></a>
								</td>
								</logic:equal>
								...
								<td valign="middle" class="tableHeader">
									<a style="font-size:14px;color: black" href="javascript:DoSort('examNo')">报到状态</a>
								</td>
								</logic:equal>
								...
								<pg:pager url="./studentAction.do" items="<%=itemNo%>" index="center" maxPageItems="100" maxIndexPages="10" isOffset="<%=true%>" export="offset,currentPageNumber=pageNumber" scope="request">
									...
									<logic:iterate name="studentmodels" id="studentwithallinfo" indexId="k">
										<pg:item>
											...
											<logic:equal name="studentlc" value="1" property="examNo">
											<td align="center" valign="middle">
												<c:choose><c:when test="${studentwithallinfo.examNo == '1'}">已报到
                                                </c:when><c:otherwise>
                                                        未报到
...
NotExport();
  ...
  document.studentForm.action="studentAction.do?method=load4fdySearch";
