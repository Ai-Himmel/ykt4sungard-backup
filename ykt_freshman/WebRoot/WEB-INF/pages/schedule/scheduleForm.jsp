...
function on_validate(){
	document.all("schedulemanage.fromtime").value=document.all("stime").getTime();
	var start=document.all("stime").getTime();
	var end=document.all("etime").getTime();
	document.all("schedulemanage.totime").value=document.all("etime").getTime();
	var toTiHour=parseInt(end.substring(0,2),10);
	var toTiMini=parseInt(end.substring(3,5),10);
	var fromTiHour=parseInt(start.substring(0,2),10);
	var fromTiMini=parseInt(start.substring(3,5),10);
	if(document.all("schedulemanage.fromdate").value == ''){
		alert('请选择日期');			
		return false;
	}else if(document.all("schedulemanage.fromtime").value == ''){
		alert('请选择起始时间');			
		return false;
	}else if(document.all("schedulemanage.positon").value == ''){
		alert('请输入地点');				
		return false;
	}else if(document.all("schedulemanage.object").value == ''){
		alert('请输入对象');				
		return false;
	}else if(document.all("schedulemanage.collegeId").value==''){
		alert('请选择书院');			
		return false;	
	}else if(document.all("schedulemanage.mission").value == ''){
		alert('请输入任务');			
		return false;		
	}else if(document.all("schedulemanage.totime").value == ''){
		alert('请选择结束时间');	
		return false;
	}else if(document.all("schedulemanage.remark").value != '' && document.all("schedulemanage.remark").value.length > 200){
		alert('"备注"描述最多不能超过200字');
		return false;
	}else if(document.all("schedulemanage.mission").value.length > 200){
		alert('"任务"描述最多不能超过200字');
		return false;
	}else if(toTiHour <= fromTiHour){
		if(toTiHour < fromTiHour){
			alert('结束时间不可小于起始时间');
			return false;
		}
		else if(toTiMini - fromTiMini < 5){
		    alert('时间间隔至少为5分钟');
		    return false;
		}
	}
	else if(toTiHour<6 || fromTiHour<6 || fromTiHour>22 || toTiHour>22){
			alert('日程安排时间必须介于上午6：00到晚上21：55');
			return false;
	}
...
function ok(){
window.open('shuyuanrichenganpai1.htm','_self');
}
...
window.open('shuyuanrichenganpai1.htm','_self');
	...
	<body bgcolor="#FFFFFF" leftmargin="0" topmargin="0">
		<html:form method="post" action="scheduleAction.do?method=save" onsubmit="javascript:return on_validate();">
<logic:present name="scheduleId">
				<%String scheduleId = request.getAttribute("scheduleId").toString();%>
				<html:hidden property="schedulemanage.scheduleId" value="<%=scheduleId%>" />
			</logic:present>
			<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
					<td height="30" align="center" valign="middle" class="tdborder02">
						<strong><font class="medium">书院日程安排</font></strong>
					</td>
				</tr>
				<tr>
							...
							<tbody>
								<tr align="center" valign="middle">
									...
									<td width="35%" align="left" valign="middle" class="tableHeader3">
										<html:select styleClass="select01" property="schedulemanage.fromdate">
            								<html:options collection="dateList" property="id.dictValue" labelProperty="dictCaption"/>
	       							    </html:select>
									</td>
									...
									<td colspan="3" align="left" valign="middle" class="tableHeader3">
										<html:select  styleClass="select01" property="schedulemanage.collegeId">								
												<option>请选择书院</option>
												<option value="ALL" <logic:equal name="scheduleForm" property="schedulemanage.collegeId" value="ALL">selected</logic:equal>>全体</option>
											...
											<tr>
												...
												<td width="130">
													<OBJECT id="stime" class="TxtInput" border="1" style="HEIGHT: 16; WIDTH: 120;" tabIndex="4" type="text/x-scriptlet" VIEWASTEXT>
														<PARAM NAME="URL" VALUE="timesetup.do?timeStr=<logic:present name="scheduleForm" property="schedulemanage.fromtime"><bean:write name="scheduleForm" property="schedulemanage.fromtime"/></logic:present><logic:notPresent name="scheduleForm" property="schedulemanage.fromtime">0600</logic:notPresent>">
													</OBJECT>
												</td>
											</tr>
										</table>
											...
											<tr>
												...
												<td width="130">
													<OBJECT id="etime" class="TxtInput" border="1" style="HEIGHT: 16; WIDTH: 120;" tabIndex="4" type="text/x-scriptlet" VIEWASTEXT>
														<PARAM NAME="URL" VALUE="timesetup.do?timeStr=<logic:present name="scheduleForm" property="schedulemanage.totime"><bean:write name="scheduleForm" property="schedulemanage.totime"/></logic:present><logic:notPresent name="scheduleForm" property="schedulemanage.totime">0605</logic:notPresent>">
													</OBJECT>
												</td>
											</tr>
										</table>
								...
								<tr align="center" valign="middle">
									...
									<td colspan="3" align="left" valign="middle" class="tableHeader3">
										<html:text property="schedulemanage.positon" styleClass="input_box" style="width:200"  maxlength="50" />
									</td>
								</tr>
								<tr align="center" valign="middle">
									<td height="28" align="right" valign="middle" bgcolor="#FFFFFF" class="tableHeader2">
									...
									<td colspan="3" align="left" valign="middle" class="tableHeader3">
										<html:text property="schedulemanage.object" styleClass="input_box" style="width:200"  maxlength="50" />
									</td>
								</tr>
								<tr align="center" valign="middle">
									...
									<td colspan="3" align="left" valign="middle" class="tableHeader3">
										<html:textarea property="schedulemanage.mission" cols="70" rows="2"/>
									</td>
								</tr>
								<tr align="center" valign="middle">
									<td align="right" valign="middle" bgcolor="#FFFFFF" class="tableHeader2">
									...
									<td colspan="3" align="left" valign="middle" class="tableHeader3">
										<html:textarea property="schedulemanage.remark" cols="70" rows="2" />
									</td>
								</tr>
							</tbody>
						</table>
				...
				<tr>
<%if(request.getAttribute("popedem")==null) {%>
					<td height="40" align="right" valign="middle">
<logic:notPresent name="scheduleId">
						<input name="Submit" type="submit" class="button_nor" value="确 定"/>
</logic:notPresent>						
<logic:present name="scheduleId">
						<input name="Submit" type="submit" class="button_nor" value="修 改"/>
						&nbsp;&nbsp;
						<input name="Submit2" type="button" class="button_nor" value="删 除" onclick="javascript:if(confirm('删除这项日程？')){document.scheduleForm.action='scheduleAction.do?method=delete';document.scheduleForm.submit();}"/>
</logic:present>
						&nbsp;&nbsp;
...
<%}%>
		...
		function howtoreturn(){
			window.location.href='scheduleAction.do?method=load4List';
		}		
