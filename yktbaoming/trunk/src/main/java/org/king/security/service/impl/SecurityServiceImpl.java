...
import org.king.business.domain.TPifCard;
...
public class SecurityServiceImpl extends BaseService implements SecurityService {
	...
	public Account login(String name, String password) throws BusinessException {
		...
		else if (!MyUtils.toMD5(password).equals(account.getPassword())) {
			List yktli = accountDAO.find("select a from TPifCard a,TCifCustomer b where b.cutId=a.cosumerId and a.stateId='1000' and b.stuempNo='" + name+"' ");
			
			TPifCard tPifCard=null;
			if(yktli!=null&&yktli.size()>0){
			    tPifCard=(TPifCard)yktli.get(0);	
			    try{
				    if(!PasswordUtil.DESEnc(password).equals(tPifCard.getPassword()))
	        	        throw new BusinessException("password not match");
