...
//			} catch (Exception e) {
//				errors
//						.add("User login", new ActionMessage(
//								"errors.LoginError"));
//			}
//		}
		...
		if ((destinatedUrl != null) && (destinatedUrl.length() > 0)) {
			...
			BmAccessInfo bmAccessInfo=new BmAccessInfo();
			try {
				...
				bmAccessInfo.setUserName((String)session.getAttribute("userName"));
				bmAccessInfo.setTrueName((String)session.getAttribute("personName"));
				bmAccessInfo.setAccessDate(dateStr);		    
				bmAccessInfo.setAccessTime(timeStr);
				bmAccessInfo.setIpAddress(request.getRemoteAddr());
				
				 if(userAccessService==null)
					 System.out.println("service is null");
				 			 
				 userAccessService.saveAccess(bmAccessInfo);
				 
			}catch (Exception e) {
				e.printStackTrace();				
			}	 
			
			//导入名单的学生才能登陆，否则提示不具有报名资格
			Set roles  = account.getRoles();
			if ((roles != null) && (roles.size() ==1)) {
				Iterator i = roles.iterator();
				Role role = (Role)i.next();
				if(role.getName().equals("考生")){
					List stuli=userAccessService.findMain("select a.studentNo from BmStudent a where a.studentNo='"+userName+"'");
					if(stuli==null||stuli.size()==0)
						return (mapping.findForward("nobmright"));	
					else{
						session.setAttribute("isStu","1");//欢迎页面显示考生流程图
						
						String todays=DateUtil.getNow("yyyy-MM-dd HH:mm");
						List stubmli=userAccessService.findFirst("select new Map(a.id as id,(a.bmTest.bmTestCat.testCatName||' '||a.bmTest.testDate||' '||a.bmTest.testBeginTime||'---'||a.bmTest.testEndTime) as testInfo," +
...
//		}
	...
	public ActionForward changeAccountPWD(ActionMapping mapping,
	   ...
	   if("1".equals(account.getPerson().getPersonType())){
