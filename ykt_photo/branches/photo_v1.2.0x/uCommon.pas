...
begin
  ...
  sqlstr := sqlstr+' in ('+#39+'1000'+#39+','+#39+'1001'+#39+') and '+cardCustId+'='+custId;
...
var
  ...
  icustId:Integer;
begin
  sqlStr:='select '+keyValue+' from '+tblSysKey+' where '+keyCode+'='+#39+'T_CIF_CUSTOMER'+#39;
  tmpQuery:=nil;
  icustId:=0;
  try
    tmpQuery:=TADOQuery.Create(nil);
...
begin
  ...
  custStr:=custStr+#39+formatdatetime('yyyymmddhhmmss',now())+#39+',';
  custStr:=custStr+sFeeType+')';
  keyStr := 'update '+tblSysKey+' set '+keyValue + '='+inttostr(icustId);
  keyStr := keyStr + ' where '+keyCode+'='+#39+'T_CIF_CUSTOMER'+#39;;
  custQuery:=nil;
  //keyQuery := nil;
  try
