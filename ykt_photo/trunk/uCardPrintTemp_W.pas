...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls, Buttons, jpeg,inifiles,ADODB,
...
begin
  ...
  {
  try
    myFile := TiniFile.Create(apppath+'photo.ini');
  ...
  try
  try
    myFile := TiniFile.Create(apppath+'printset'+'\'+postype+'.ini');
...
begin
  ...
  imgBG.Top := 0;
  imgBG.Left := 0;
  imgBG.Width := pageWidth;
  imgBG.Height := pageheight;
  if bgvisible then
  ...
  try
    ...
    begin
      if Trim(cbbBackGround.Text)='' then
        ...
        imgBG.Picture.LoadFromFile(apppath+'printset\'+trim(cbbBackGround.Text));
    end;
  except
...
begin
  ...
  cbbBackGround.Items.LoadFromFile(apppath+'printset\background.txt');
end;
...
begin
  //打印背景
  //imgPPhoto.Picture.LoadFromFile(apppath+'printset\'+trim(cbbBackGround.Text));
  //pnl1.Print(True);
  rpt1.LoadFromFile(apppath+'print.rmf');
  t1 := TRMPictureView(rpt1.FindObject('Picture1'));
  if t1 <> nil then
  begin
    try
    if Trim(cbbBackGround.Text)='' then
      t1.Picture.LoadFromFile(apppath+'printset\'+bgname)
    else
      t1.Picture.LoadFromFile(apppath+'printset\'+trim(cbbBackGround.Text));
    except
      ShowMessage('找不到指定的背景，请查看配置是否正确！');
      Exit;
    end;
