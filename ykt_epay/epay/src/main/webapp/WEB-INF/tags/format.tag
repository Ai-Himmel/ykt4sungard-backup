...
<%
    ...
    Format datetimeFormat = Contants.DATE_TIME_FORMAT;
    Format timeFormat = Contants.TIME_FORMAT;
    ...
    if (StringUtils.isNotBlank(value)) {
       if(value.length()==14){
        ...
        result = datetimeFormat.format(array);
       }else if(value.length()==8){
        Object[] array = {value.substring(0, 4), value.substring(4, 6), value.substring(6, 8)};
        result = dateFormat.format(array);
       }else if (value.length()==6){
        Object[] array = {value.substring(0, 2), value.substring(2, 4), value.substring(4, 6)};
        result = timeFormat.format(array);
       }
