    ...
    public BufferResponseWrapper handle(HttpServletRequest request, BufferResponseWrapper responseWrapper) {
        ...
        try {
            if (tree != null || state != null || jsf_sequence != null) {
                ...
                if (state != null)
                    responseWrapper.output("    var state = AjaxAnywhere.findInstance(aaInstanceId).findForm().elements[\"jsf_state_64\"]\n" +
                            "    if (state!=null && state.tagName.toLowerCase() == \"input\")\n" +
                            "                state.value=\"" + state + "\";"
                    );
                if (tree != null)
                    responseWrapper.output("    var tree = AjaxAnywhere.findInstance(aaInstanceId).findForm().elements[\"jsf_tree_64\"]\n" +
