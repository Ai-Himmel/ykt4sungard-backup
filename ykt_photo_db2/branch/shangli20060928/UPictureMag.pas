...
begin
  ...
  try
    MyADOQuery.Connection := dm.ADOConnection1;
    MyADOQuery.Close;
    MyADOQuery.SQL.Clear;
    MyADOQuery.SQL.Add('select * from ' + PersonTableName + ' where ' + PersonNo +
      ' = ' + QuotedStr(FPerNo));
    MyADOQuery.Open;
    ...
    TmpId := Trim(MyADOQuery.FieldByName(PersonNO).AsString);
    TmpName := Trim(MyADOQuery.FieldByName(PersonName).AsString);
    Flen := length(TmpId);
    NoCaption := '';
    for i := 1 to Flen do
    begin
...
begin
  ...
  imgSource.Left := 0;
  imgSource.Top := 0;
  //sbDestinate.SetBounds(sbDestinate.Left, sbDestinate.Top, FPhotoWidth, FPhotoHeight);
  imgDestinate.Update;
end;
