...
var XMLHttpReq;
    ...
    function DisplayHot() {
	    ...
	    if(flag==1){
	    	document.location='<%=ContextUtil.getCtxPath(request)%>/preview.do?confid=<%=request.getAttribute("confid")%>&attendeesign=prev2&selecttype=totle&attendtime=<%=StringUtil.nullConvertToString(request
							.getAttribute("attendtime"))%>';
	    }
	}
...
{
    setTimeout("window.location.href='<%=ContextUtil.getCtxPath(request)%>/preview.do?confid=<%=ParamUtil.getString(request, "confid")%>&selecttype=delegray'", duration);
  	sendRequest();
    interval();
}
