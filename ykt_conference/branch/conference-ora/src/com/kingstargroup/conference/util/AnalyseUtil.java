...
public class AnalyseUtil extends BasicUtil {
	 ...
	 * @return
	 */
	public static List getGroupCount(Integer conId) {
		AnalyseDAO dao = AnalyseDAO.getInstance();	
		List result = new ArrayList();
		List list = null;
		try {
			list = dao.getGroupCount(conId);
			for (int i=0;i<list.size();i++){
				Object[] objects = (Object[]) list.get(i);
				HashMap map = new HashMap();
				map.put("groupId",convent(objects[0]));
				map.put("groupName", convent(objects[1]));
	 ...
	 * @return
	 */
	public static List getAttendeeListByGroup(Integer conId, String sign,String groupId) {
		AnalyseDAO dao = AnalyseDAO.getInstance();	
		DictionaryDAO dictdao = DictionaryDAO.getInstance();
		List result = new ArrayList();
		List list = null;
		try {
			list = dao.getTTAttendeeListByGroup(conId, sign,groupId);
			for (int i=0, l=list.size();i<l;i++){
				Object[] objects = (Object[]) list.get(i);
