...
public class AttendeeGroupDAO extends BasicDAO {
	...
	public List getCustInfo(String custno, String custname, String duty,
		...
		sqlStr
				...
				.append(" where cust.cut_Id not in(" + custIds + ")")
				.append(
						" and cust.cut_id not in(select cust_id from YKT_CONF.T_ATTENDEE_LIST where con_id="
								+ confid + ")");
		if (!"".equals(custno.trim())) {
			sqlStr.append(" and cust.stuemp_No = '" + custno + "'");
		}
		if (!"".equals(custname)) {
	...
	/**
			...
			* Return: List<br>
			* Args: @param physicalno
			* Args: @return
			* Args: @throws HibernateException
			* Modify History: <br>
			* 操作类型    操作人     操作时间       操作内容<br>
			* ======================================<br>
			...
			* @since 1.0
			*/
	public List getCustInfo(String physicalno)
			throws HibernateException {
		Session s = getSession();
		StringBuffer sqlStr = new StringBuffer();
		sqlStr
				.append("select cust.cut_id as cutId,cust.cut_type as cutType,cuttype.type_name as typeName,")
				.append("cust.cut_Name as cutName,cust.stuemp_No as stuempNo,")
				.append("cust.classdept_No as deptNo,dept.dept_Name as deptName,")
				.append("cust.s_code as sCode,special.s_name as sName,cust.sex,dict.dict_caption as dictCaption ")
				.append("from ykt_cur.T_CIF_CUSTOMER cust left join YKT_CUR.T_PIF_CARD card on ") 
				.append("cust.cut_id=card.COSUMER_ID left join YKT_CUR.T_Cif_Dept dept on ")
				.append("cust.classdept_No=dept.dept_Code left join YKT_CUR.T_CIF_CUTTYPEFEE cuttype on ")
				.append("cust.CUT_TYPE=cuttype.CUT_TYPE left join YKT_CUR.T_PIF_DICTIONARY dict on ")
				.append("cust.sex=dict.dict_value and dict.dict_no=39 ")
				.append("left join YKT_CUR.T_CIF_SPECIALITY special on ")
				.append("cust.s_code=special.s_code ")
				.append(" where card.PHYSICAL_NO='"+physicalno+"' and card.state_id<>'2000'");
	...
	public List getReplaceCustInfo(int confid, String custno, String custname,
		...
		sqlStr
				...
				.append(
						" on cust.cut_Id=att.cust_Id left join YKT_CONF.T_DUTY duty on")
				.append(" cust.stuemp_No=duty.stuemp_No").append(
						" where (att.ATTEND_SIGN='00' or att.ATTEND_SIGN='13' or att.ATTEND_SIGN='23') and att.con_Id=" + confid);
		if (!"".equals(custno)) {
			sqlStr.append(" and cust.stuemp_No like '%" + custno + "%'");
		}
		if (!"".equals(custname)) {
	...
	public List getLeaveCustInfo(int confid, String custno, String custname,
		...
		sqlStr
				...
				.append(
						" on cust.cut_Id=att.cust_Id left join YKT_CONF.T_DUTY duty on")
				.append(" cust.stuemp_No=duty.stuemp_No").append(
						" where (att.ATTEND_SIGN='00' or att.ATTEND_SIGN='30') and att.con_Id=" + confid);
		...
		sqlStr.append(" union   select cust_id as cutId ,cut_name as cutName ,'' as stuempNo ,replacer_duty  as duty  ,")
		.append(" dept_name  as deptName ,attend_sign  as attendSign ")
		.append("  from    ykt_conf.T_Attendee_List ")
		.append(" where (ATTEND_SIGN='00' or ATTEND_SIGN='30') and cust_id <0 and con_Id="+ confid);
		if (!"".equals(custname)) {
			sqlStr.append(" and cut_name like '%" + custname + "%'");
		}
		if (!"".equals(duty)) {
			sqlStr.equals(" and replacer_duty like '%" + duty + "%'");
		}
...
//				" duty.duty as duty,dept.dept_Name as deptName ,att.attend_Sign as attendSign")
//		.append(
//				" from ykt_conf.T_Attendee_List att left join ykt_cur.T_Cif_Customer cust ")
...
//						+ " and att.ATTEND_SIGN='00' ");
		...
		.append(
				"select cust.cut_Id as cutId,cust.cut_Name as cutName,cust.stuemp_No as stuempNo,")
		.append(
				" '' as duty,dept.dept_Name as deptName ,att.attend_Sign as attendSign")
		.append(
				" from ykt_conf.T_Attendee_List att left join ykt_cur.T_Cif_Customer cust ")
		.append("on cust.cut_Id=att.cust_Id left join ykt_cur.T_Cif_Dept dept ")
		.append(
...
//						"duty.duty as duty,dept.dept_Name as deptName ,att.attend_Sign as attendSign ")
//				.append(
//						"from (select a.COSUMER_ID,his.attend_sign,his.confid from YKT_CUR.T_PIF_CARD a, ")
//				.append(
//						"ykt_conf.T_RECORDHIS his where a.physical_no=his.physical_no and his.dealornot='0')att ")		
//				
//				.append(
//						"left join ykt_cur.T_Cif_Customer cust on cust.cut_Id=att.COSUMER_ID  left join ykt_cur.T_Cif_Dept dept ")
...
//						"where att.confid=" + confid);
	...
	public List getCustInfobyids(int confid, String custid)
		...
		sqlStr
				...
				.append(
						"(select a.COSUMER_ID,his.attend_sign,his.confid from YKT_CUR.T_PIF_CARD a, ")
				.append(
						"ykt_conf.T_RECORDHIS his where a.physical_no=his.physical_no)att ")
				.append(
						"on cust.cut_Id=att.COSUMER_ID left join YKT_CONF.T_DUTY duty on ")
				.append("cust.stuemp_No=duty.stuemp_No ").append(
						"where att.confid=" + confid);
		if (!"".equals(custid)) {
	...
	public List getCustGroupList(int groupid) throws HibernateException {
		...
		sqlStr
				.append(
						"select cust.cut_Id as cutId,cust.cut_Name as cutName,cust.stuemp_No as stuempNo,")
				.append("duty.duty as duty,dept.dept_Name as deptName,cust.sex as sex ")
				.append(
						"from ykt_cur.T_Cif_Customer cust left join ykt_cur.T_Cif_Dept dept on ")
				.append(
						" cust.classdept_No=dept.dept_Code left join YKT_CONF.T_DUTY duty on")
				.append(" cust.stuemp_No=duty.stuemp_No")
				.append(
						" where cust.cut_Id in(select cust_Id from ykt_conf.T_Attendee_Group where group_Id="
								+ groupid + ")");
		try {
			SQLQuery q = s.createSQLQuery(sqlStr.toString());
			q.addScalar("cutId", Hibernate.INTEGER);
		...
		sqlStr
				...
				.append(
						" (select cust_Id from ykt_conf.T_Attendee_Group where group_Id="
								+ groupid + ")")
				.append(" and cust.cut_Id not in(" + custIds + ")")
				.append(
						" and cust.cut_id not in(select cust_id from YKT_CONF.T_ATTENDEE_LIST where con_id="
								+ confid + ")");
		try {
	...
	public List getAttendedList(int confId, List attendedList)
		...
		sqlStr
				...
				.append(
						" cust.classdept_No=dept.dept_Code left join YKT_CONF.T_DUTY duty on")
				.append(" cust.stuemp_No=duty.stuemp_No")
				.append(" where cust.cut_Id in(" + custIds + ")")
				.append(
						" or cust.cut_id in (select cust_id from YKT_CONF.T_ATTENDEE_LIST where con_id="
								+ confId + ")");
		try {
			SQLQuery q = s.createSQLQuery(sqlStr.toString());
			q.addScalar("cutId", Hibernate.INTEGER);
	...
	public List getAllAttendedList(int confId, List attendedList)
		...
		sqlStr
				...
				.append(
						"on att.delegray=dele.DLGT_ID left join YKT_CUR.T_Pif_Dictionary dic ")
				.append(
				...
				.append("where cust.cut_Id in(" + custIds + ") ").append(
						"order by fullatt.delegray,cust.cut_Id");
