...
public class ConferenceUtil extends BasicUtil {
	...
	public static boolean checkMeetingRoom(String roomId, String signTime, String beginDate,
			String beginTime, String endDate, String endTime){
		ConferenceDAO dao = ConferenceDAO.getInstance();
		List confList = null;
		try {
			confList = dao.checkMeetingRoom(roomId, signTime, beginDate, beginTime, endDate, endTime);
		...
		try {
			for (int i = 0; i < ids.length; i++) {
				TConference conf = (TConference) dao.findByPrimaryKey(
						TConference.class, new Integer(ids[i]));
				conf.setStatus("9");
	 ...
	 * @since 1.0
	 */
	public static boolean editConference(Integer confId, String confName,
