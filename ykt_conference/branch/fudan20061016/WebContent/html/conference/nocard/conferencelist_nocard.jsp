...
<body   bgcolor="#EFEFEF" leftmargin="0" topmargin="0">
	<form name="conferencelist"  method=post >
...
<form name="NoCardNextForm" action="<%=ContextUtil.getCtxPath(request)%>/nocardnext.do" method=post >
	<input type=hidden name=confid value="">
	<table width="800" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr> 
		<td height="40" align="right" valign="middle" class="tdborder02"> 
			<input name="Submit" type="button" class="button_nor" value="<bean:message key="button.imp"/>" onclick="sub()" >
			<input name="button" type="button" class="button_nor" value="<bean:message key="button.query"/>" onclick="query()" >
		</td>
	</tr>
	</table>
</form>
<script>
    function sub(){
       document.NoCardNextForm.submit();
    }
   
	function getRadioValue(col){
		document.NoCardNextForm.confid.value=getSelectedRadioValue(col);
	}
	
	function query(){
	   document.NoCardNextForm.action="<%=ContextUtil.getCtxPath(request)%>/qryimpattend.do";
	   document.NoCardNextForm.submit();
	}
</script>
