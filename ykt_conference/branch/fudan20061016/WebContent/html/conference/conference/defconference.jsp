...
<%
			String conname = request.getAttribute("conname") == null ? ""
			: request.getAttribute("conname").toString();
	String confroom = request.getAttribute("confroom") == null ? ""
			: request.getAttribute("confroom").toString();
	String type = request.getAttribute("type") == null ? "" : request
			.getAttribute("type").toString();
	String begindate = request.getAttribute("begindate") == null ? ""
			: request.getAttribute("begindate").toString();
%>
<body bgcolor="#EFEFEF" leftmargin="0" topmargin="0">
	<form id="DeleteConferenceForm" name="DeleteConferenceForm"
		action="<%=ContextUtil.getCtxPath(request)%>/queryconference.do"
		method="post">
			...
			<tr>
				<td height="25" align="left" valign="middle"
					...
					<span class="mediumwhite"><strong><bean:message
								key="conference.list" /> </strong> </span>
				</td>
			</tr>
			<tr>
				...
				<td align="center" valign="top">
					<ec:table var="pres" items="result" filterable="false"
						...
						<ec:row highlightRow="true">
							...
							getServletContext())%>'>
								<input name="ids_allbox" type="checkbox" value="${pres.conId}">
								<a
									href="<%=ContextUtil.getCtxPath(request)%>/conf/editconference.do?conid=${pres.conId}">${pres.conId}</a>
							</ec:column>
	...
	<form id="confForm" name="confForm"
		action="<%=ContextUtil.getCtxPath(request)%>/conf/copyconference.do" method="post">
			...
			<tr>
				<td  height="40" align="right" valign="middle"
					...
					<input name="Submit" type="button" class="button_nor"
						value='<bean:message key="button.create"/>'
						onclick="javascript:createPage();">
					&nbsp;&nbsp;&nbsp;
					
					<input name="Submit" type="button" class="button_nor"
						value='<bean:message key="button.copy"/>'
						onclick="javascript:copyconf();">
					&nbsp;&nbsp;&nbsp;
	...
	<script language="javascript">
  	function createPage() {
  		self.location = '<%=ContextUtil.getCtxPath(request)%>/conf/createconference.do';
	}
	function copyconf(){
		 var confid = "";		 
		  if(document.forms[0].ids_allbox!=null&&document.forms[0].ids_allbox.length>1){
		 	for(var i=0;i<document.forms[0].ids_allbox.length;i++){
				 if( document.forms[0].ids_allbox[i].checked==true){
				 	confid += document.forms[0].ids_allbox[i].value;
				 	 break;
				 }
		      }	
		 }else if(document.forms[0].ids_allbox!=null){
		 	confid = document.forms[0].ids_allbox.value;
		 }
       document.confForm.confid.value=confid;
       if(document.confForm.confid.value==''){
			alert("请选择需要复制的会议！");
			return false;
		}
        document.confForm.action='<%=ContextUtil.getCtxPath(request)%>/conf/copyconference.do';
		...
		if (window.confirm(str)){
			var confids = "";
			 if(document.forms[0].ids_allbox!=null&&document.forms[0].ids_allbox.length>1){
			 	for(var i=0;i<document.forms[0].ids_allbox.length;i++){
					 if( document.forms[0].ids_allbox[i].checked==true){
					 	confids += document.forms[0].ids_allbox[i].value+",";
					 }
			      }	
			 }else if(document.forms[0].ids_allbox!=null){
			 	confids = document.forms[0].ids_allbox.value+",";
			 }
	       document.confForm.confid.value=confids;
	        if( document.confForm.confid.value==''){
				alert("请选择需要复制的会议！");
				return false;
			}
	        document.confForm.action='<%=ContextUtil.getCtxPath(request)%>/deleteconference.do';
	        document.confForm.submit();
		}else{
			return false;
		}
