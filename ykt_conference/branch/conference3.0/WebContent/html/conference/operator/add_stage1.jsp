...
<body bgcolor="#EFEFEF" leftmargin="0" topmargin="0">
	...
	<form name="CreateOperatorForm" id="CreateOperatorForm"
		action="<%=ContextUtil.getCtxPath(request)%>/usermanage/add_result.do"
		method="post">
		<input type=hidden name=cmd value="stage1">
		<input type=hidden name=userLimitString
			value='<%=ParamUtil.getString(request, "userLimitString")%>'>
		<input type=hidden name=deptLimitString
			value='<%=ParamUtil.getString(request, "deptLimitString")%>'>
		<table width="800" border="0" align="center" cellpadding="0"
			...
			<c:if
				test='<%=SessionErrors.contains(request,
							"conference.error.createuserfailure")%>'>
				<tr>
					<td>
						<font class="portlet-msg-error" style="font-size: 11;"> <bean:message
								key="conference.error.user.exists" /> </font>
					</td>
				</tr>
			</c:if>
			<c:if
				test='<%=SessionErrors.contains(request,
							"conference.editpassword.unpairedpassword")%>'>
				<tr>
					<td>
						<font class="portlet-msg-error" style="font-size: 11;"> <bean:message
								key="conference.editpwd.error" /> </font>
					</td>
				</tr>
			</c:if>
			<c:if
				test='<%=SessionErrors.contains(request,
							"conference.opercode.error")%>'>
				<tr>
					<td>
						<font class="portlet-msg-error" style="font-size: 11;"> <bean:message
								key="conference.opercode.error" /> </font>
					</td>
				</tr>
			</c:if>
			<c:if
				test='<%=SessionErrors.contains(request,
							"conference.password.error")%>'>
				<tr>
					<td>
						<font class="portlet-msg-error" style="font-size: 11;"> <bean:message
								key="conference.password.error" /> </font>
					</td>
				</tr>
			</c:if>
			<tr>
				<td height="25" align="left" valign="middle"
					...
					<span class="mediumwhite"><strong>
							&nbsp;&nbsp;&nbsp; <bean:message key="operator.create" /> </strong>
					</span>
				</td>
			</tr>
			<tr>
				...
				<td align="center" valign="top">
					<table width="100%" border="1" cellspacing="0" cellpadding="2"
						...
						<tr>
							...
							<td align="left" valign="middle" bgcolor="#E6ECF2">
								<input name=opername class="input_box" style="width: 200"
									maxlength=50
						...
						<tr>
							...
							<td align="left" valign="middle" bgcolor="#E6ECF2">
								<input name=operpwd type="password" class="input_box"
									style="width: 200" maxlength=50>
								<a class="portlet-msg-error"><html:errors property="operpwd" />
								</a>
							</td>
						</tr>
						<tr>
							...
							<td align="left" valign="middle" bgcolor="#E6ECF2">
								<input name=pwdagain type="password" class="input_box"
									style="width: 200" maxlength=50>
								<a class="portlet-msg-error"><html:errors
										property="pwdagain" />
								</a>
								...
								<select name="deptcode" class="select01" style="width: 200">
									<%
										...
										while (iter.hasNext()) {
											...
											String deptcode = dept.get("deptCode").toString();
											if (deptcode.equals(ParamUtil.getString(request, "deptcode"))) {
									%>
