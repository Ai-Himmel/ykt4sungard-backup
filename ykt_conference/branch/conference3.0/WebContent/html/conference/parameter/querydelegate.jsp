...
<form action="<%=ContextUtil.getCtxPath(request)%>/querydelegate.do" name="delegatesForm" method="post" >
	...
	<tr>
		<td height="25" align="left" valign="middle" background="<%=ContextUtil.getCtxPath(request)%>/html/commons/images/image_lines.jpg">
			<span class="mediumwhite"><strong>
				&nbsp;&nbsp;&nbsp;
				<bean:message key="delegate.query" />
			</strong></span>
		</td>
	</tr>
	<tr>
...
<script>
	function renderCancelPage() {
		self.location = '<%= ContextUtil.getCtxPath(request)%>/viewdelegates.do';
	}
</script>
</body>