...
<body bgcolor="#FFFFFF" leftmargin="0" topmargin="0"
	ondblclick="javascript:holeset();">
	<%
				TConference conference = (TConference) request
				.getAttribute("conference");

		int totleattendee = Integer.parseInt(request
				.getAttribute("totleattendee") == null ? "0" : request
				.getAttribute("totleattendee").toString());
		int signednum = Integer
				.parseInt(request.getAttribute("signednum") == null ? "0"
				: request.getAttribute("signednum").toString());
		int leavenum = Integer
				.parseInt(request.getAttribute("leavenum") == null ? "0"
				: request.getAttribute("leavenum").toString());
		List leavelist = (List) request.getAttribute("leavelist");
		int replacenum = Integer.parseInt(request
				.getAttribute("replacenum") == null ? "0" : request
		...
		int csignednum = Integer.parseInt(request
				.getAttribute("csignednum") == null ? "0" : request
				.getAttribute("csignednum").toString());
		List csignedlist = (List) request.getAttribute("csignedlist");
		int unsignnum = Integer
				.parseInt(request.getAttribute("unsignnum") == null ? "0"
				: request.getAttribute("unsignnum").toString());
				...
				<table width="100%">
					<tr>
						<td width="75%">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
									...
									<td width="55%" height="25" align="center" valign="middle"
										class="medium4">
										<a
									...
									<td width="45%" align="center" valign="middle" class="medium4">
										<a
											href='<%=ContextUtil.getCtxPath(request)%>/preview.do?confid=<%=ParamUtil.getString(request, "confid")%>&selecttype=totle&attendeesign=unsign'>
											<bean:message key="sign.nosign" />[<%=unsignnum%>]</a>
									</td>
									<td align="center" valign="middle">
										&nbsp;
									</td>
								...
								<tr>
															...
															<td height="100%" align="center" valign="top"
																...
																<table width="93%" border="0" cellspacing="0"
																	...
																	<tr align="left" valign="middle">
																		<td class="medium3" width="25%">
																			<bean:message key="attendee.custname" />
																		</td>
																		<td class="medium3" width="40%">
									...
									<td align="center" valign="top" class="tdborder03">
										<table width="93%" border="0" cellspacing="0" cellpadding="0">
											...
											<tr align="left" valign="middle">
												<td class="medium4"
													title="<%=StringUtil.nullConvertToString(row2.get("dutyName"))%>">
													<%=row2.get("cutName")%>
												</td>
												<td class="medium4">
													<%=row2.get("deptName")%>
								...
								<tr>
									<td align="center" valign="top" class="tdborder03" colspan="4">
										<table width="95%" border="0" cellspacing="0" cellpadding="0">
											...
											<tr align="center" valign="middle">
												<td class="medium3" width="40%" align="left">
													<bean:message key="attendee.dept" />
												</td>
												<td class="medium3" width="15%">
											...
											<%
											if(deptList!=null){
												Iterator iter5 = deptList.iterator();
												while (iter5.hasNext()) {
													HashMap row5 = (HashMap) iter5.next();
											%>
											<tr align="center" valign="middle">
												<td class="medium4" align="left"
													title="<%=StringUtil.nullConvertToString(row5.get("deptName"))%>">
													<%=row5.get("deptName")%>
												</td>
												<td class="medium4">
													<a href="javascript:viewDetail('attendee','<%=row5.get("deptName")%>')" >
														<%=row5.get("attendee")%>
													</a>
												</td>
												<td class="medium4">
													<%if("0".equals(row5.get("signednum"))){ %>
														<%=row5.get("signednum")%>
													<%} else{ %>
														<a href="javascript:viewDetail('sign','<%=row5.get("deptName")%>')" >
															<%=row5.get("signednum")%>
														</a>
													<%} %>
												</td>
												<td class="medium4">
													<%if("0".equals(row5.get("unsignnum"))){ %>
														<%=row5.get("unsignnum")%>
													<%} else{ %>
														<a href="javascript:viewDetail('unsign','<%=row5.get("deptName")%>')">
															<%=row5.get("unsignnum")%>
														</a>
													<%} %>
												...
												<td class="medium4">
													<%if("0".equals(row5.get("leavenum"))){ %>
														<%=row5.get("leavenum")%>
													<%} else{ %>
														<a href="javascript:viewDetail('leave','<%=row5.get("deptName")%>')">
															<%=row5.get("leavenum")%>
														</a>
													<%} %>
						...
						<td width="240" align="center" valign="top">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
									<td height="25" align="center" valign="middle" class="medium4">
										<bean:message key="sign.preview" />
									</td>
								</tr>
								<tr>
									<td align="center" valign="top">
								...
								<tr>
									<td align="center" valign="middle" class="medium4">
										<a
								...
								<tr>
									<td height="25" align="center" class="medium4">
										<a
	...
	<form id="viewDetail" name="viewDetail" action="<%=ContextUtil.getCtxPath(request)%>/preview.do?confid=<%=ParamUtil.getString(request, "confid")%>&selecttype=totle" method="post">
		<input type="hidden" name="deptName"/>
		<input type="hidden" name="attendeesign"/>
	</form>
</body>
...
var XMLHttpReq;
	...
	function sendRequest() {
        ...
        var url = "<%=ContextUtil.getCtxPath(request)%>/preview.do?confid=<%=request.getAttribute("confid")%>&attendeesign=prevtest&selecttype=totle&attendtime=<%=StringUtil.nullConvertToString(request
    ...
    function DisplayHot() {
	    ...
	    if(flag==1){
	    	document.location='<%=ContextUtil.getCtxPath(request)%>/preview.do?confid=<%=request.getAttribute("confid")%>&attendeesign=prev&selecttype=totle&attendtime=<%=StringUtil.nullConvertToString(request
							.getAttribute("attendtime"))%>';
	    }
	}
...
function changinterface(interface){
	...
	function viewDetail(attendeesign,deptName){
		document.forms.viewDetail.attendeesign.value = attendeesign;
		document.forms.viewDetail.deptName.value = deptName;
		document.forms.viewDetail.submit();
	}
</script>
