...
import com.kingstargroup.conference.hibernate.form.TDept;
...
public class GetDictionary extends BasicUtil{
	...
	public static List getDeptList(String operCode){
		DictionaryDAO dao = DictionaryDAO.getInstance();
		List custList = null;
		List deptList = new ArrayList();
		try {
			custList = dao.getDeptList(operCode);
			int custsize = custList.size();
			for (int i = 0; i < custsize; i++) {
				Object[] objects = (Object[]) custList.get(i);
				HashMap confMap = new HashMap();
	...
	public static String getDeptNameByDeptCode(String deptcode){
		...
		TDept dept = null;
		try {
			dept = (TDept)dao.findByPrimaryKey(TDept.class,deptcode);
		} catch (HibernateException he) {
			he.printStackTrace();
		}finally{
			closeSession();
	 ...
	 * @since 1.0
	 */
	public static String getDev999IdByDevId(long devId){
		DictionaryDAO dao = DictionaryDAO.getInstance();
		TPifDevice dev = null;
		try {
			dev = (TPifDevice)dao.findByPrimaryKey(TPifDevice.class,new Long(devId));
		} catch (HibernateException he) {
			return null;
		}finally{
			//closeSession();
	 ...
	 * @since 1.0
	 */
	public static String getCustNameById(long custid){
		DictionaryDAO dao = DictionaryDAO.getInstance();
		try {
			return ((TCustomer)dao.findByPrimaryKey(TCustomer.class,new Long(custid))).getCustname();
		} catch (HibernateException he) {
			return null;
		}finally{
			closeSession();
		 ...
		 * @since 1.0
		 */
	public static String getConfNameById(long confid){
		DictionaryDAO dao = DictionaryDAO.getInstance();
		try {
			return ((TConference)dao.findByPrimaryKey(TConference.class,new Long(confid))).getConName();
		} catch (HibernateException he) {
			return null;
		}finally{
			closeSession();
	...
	public static TDictionary getCustType(String dictValue) {
		DictionaryDAO dao = DictionaryDAO.getInstance();	
		TDictionary custType = null;
		try {
