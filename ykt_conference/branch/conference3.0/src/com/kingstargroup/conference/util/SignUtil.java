...
public class SignUtil extends BasicUtil {
	 ...
	 * @since 1.0
	 */
	public static boolean attendeeNoCardSign(long confid, String[] custid,
			String signtime) {
		...
		try {
			...
			for (int i = 0; i < custid.length; i++) {
				Iterator iter = dao.getCustListBy(confid,
						Long.parseLong(custid[i])).iterator();
				while (iter.hasNext()) {
	...
	public static boolean ecardAttendeeNoCardSign(long confid, String[] custid,
			String signtime) {
		SignDAO dao = SignDAO.getInstance();
		ConferenceDAO confDao = ConferenceDAO.getInstance();
		Transaction t = beginTransaction();
		...
		try {
			...
			meetTime = conf.getConBegindate() + conf.getConBegintime();
			for (int i = 0; i < custid.length; i++) {
				List list = dao.getCustListBy(confid,
						Long.parseLong(custid[i]));
				if(list.size()>0){
					//会议中有
					TAttendeeList att = (TAttendeeList)list.get(0);
					if (nowtime.compareTo(meetTime) >= 0) {
						// 迟到
						att.setAttendSign("22");
					}
					...
					if (nowtime.compareTo(meetTime) >= 0) {
						// 迟到
						dao.getandUpdatecheckFlagbyCustid(confid, custid[i],"22");
					}
					if (nowtime.compareTo(meetTime) < 0) {
						// 正常
						dao.getandUpdatecheckFlagbyCustid(confid, custid[i],"12");
					}
				}
	...
	public static boolean attendeeReplacebyhand(int confid, String custidA,
		...
		try {
			...
			if (his != null) {
				...
				hisid =  String.valueOf(his.getConfid());
				while (iter.hasNext()) {
					TAttendeeList att = (TAttendeeList) iter.next();
					tempid = String.valueOf(att.getId().getConId());
	 ...
	 * @since 1.0
	 */
	public static List getSignedNumByConfid(int confid,String deptName) {
		SignDAO dao = SignDAO.getInstance();
		List result = new ArrayList();
		List list = null;
		try {
			list = dao.getSignedNumByConfid(confid,deptName);
	 ...
	 * @since 1.0
	 */
	public static List getUnSignNumByConfid(int confid,String deptName) {
		SignDAO dao = SignDAO.getInstance();
		List result = new ArrayList();
		List list = null;
		try {
			list = dao.getUnSignNumByConfid(confid,deptName);
			for (int i = 0, l = list.size(); i < l; i++) {
	 ...
	 * @since 1.0
	 */
	public static List getLeaveNumByConfid(int confid,String deptName) {
		SignDAO dao = SignDAO.getInstance();
		List result = new ArrayList();
		List list = null;
		try {
			list = dao.getLeaveNumByConfid(confid,deptName);
			for (int i = 0, l = list.size(); i < l; i++) {
				Object[] objects = (Object[]) list.get(i);
				String dutyName = convent(objects[3]);
				if(i+1<l){
	 ...
	 * @return
	 */
	public static List getDeptListByConfid(int confid) {
		SignDAO dao = SignDAO.getInstance();
		List result = new ArrayList();
		List list = null;
		try {
			list = dao.getDeptListByConfid(confid);
			for (int i = 0, l = list.size(); i < l; i++) {
				Object[] objects = (Object[]) list.get(i);
				HashMap map = new HashMap();
				map.put("deptName", convent(objects[0]));
				map.put("attendee", convent(objects[1])==""?"0":convent(objects[1]));
				map.put("signednum", convent(objects[2])==""?"0":convent(objects[2]));
				map.put("unsignnum", convent(objects[3])==""?"0":convent(objects[3]));
				map.put("leavenum", convent(objects[4])==""?"0":convent(objects[4]));
				result.add(map);
			}
			return result;
		} catch (HibernateException he) {
	 ...
	 * @return
	 */
	public static List getAttendeeByConfid(int confid,String deptName) {
		SignDAO dao = SignDAO.getInstance();
		List result = new ArrayList();
		List list = null;
		try {
			list = dao.getAttendeeByConfid(confid,deptName);
			for (int i = 0, l = list.size(); i < l; i++) {
