...
import com.kingstargroup.conference.hibernate.form.TDeptLimitId;
...
import com.kingstargroup.conference.hibernate.form.TOperLimitId;
...
public class SynchronizeUtil extends BasicUtil {
	...
	public boolean synchData(String databaseIP, String port, String dbname,
		...
		try {
			...
			rsh = new BeanListHandler(TCustomer.class);
			...
			while (iter.hasNext()) {
				...
				key.setCustId(Long.parseLong(map.get("custId").toString()));
				key.setGroupId(Long.parseLong(map.get("groupId").toString()));
			...
			while (iter.hasNext()) {
				...
				key.setConId(Long.parseLong(map.get("conId").toString()));
				key.setCustId(Long.parseLong(map.get("custId").toString()));
				...
				attendeelist.setCardNo(map.get("cardNo") == null ? 0 : Long.parseLong(map.get("cardNo")
						.toString()));
				attendeelist.setDelegray(map.get("delegray") == null ? 0 : Long.parseLong(map.get("delegray").toString()));
			...
			while (iter.hasNext()) {
				...
				TDeptLimitId id = new TDeptLimitId();
			...
			// TDevConferenceId id = new TDevConferenceId();
			...
			while (iter.hasNext()) {
				...
				TOperLimitId id = new TOperLimitId();
			...
			while (iter.hasNext()) {
				TCustomer cifcustomer = (TCustomer) iter.next();
