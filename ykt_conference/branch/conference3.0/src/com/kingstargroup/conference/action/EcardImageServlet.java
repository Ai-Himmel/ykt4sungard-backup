...
public class EcardImageServlet extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse res)
		...
		String conferencepath = KSConfiguration.getInstance().getProperty("conferencepath","/opt/tomcat5/conference");
		String suffix = "/WEB-INF/classes/com/kingstargroup/conference/action/temp.jpg";
		try {
					...
					if("0".equals(id)){
						if(attList!=null&&attList.size()>0){
							...
							if(!"FUDAN".equals(CONSTANT.VERSION)){
								photo = AnalyseUtil.getPhoto(Long.valueOf(custId));
							}else{
								photo = AnalyseUtil.getfudanPhoto(stuempNo);
							}
							if (photo == null) {
								//没有头像
								ServletOutputStream out = res.getOutputStream();
