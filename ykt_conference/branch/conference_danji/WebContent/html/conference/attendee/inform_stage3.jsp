...
<body bgcolor="#EFEFEF" leftmargin="0" topmargin="0">
	...
	<form name="AttendeeInformForm"
		action="<%=ContextUtil.getCtxPath(request)%>/attendeeinform.do"
		method=post>
		...
		<input type=hidden name=ifquery
			value='<%=ParamUtil.getString(request, "ifquery")%>'>
		<input type=hidden name=gselect
			value='<%=ParamUtil.getString(request, "gselect")%>'>
				...
				<td align="center" valign="top">
					<table width="800" border="0" cellspacing="0" cellpadding="0">
						<tr align="center" valign="top">
							<td width="45%">
								<font color="#FF0000"> <aa:zone name="hjwquerylist">
										<select name="querysrclist" size="16" style="width:350"
											multiple
											ondblclick="transferOption(this.form.querysrclist, this.form.groupdestlist,false)">
											<%
													if (null != result) {
							...
							<td width="10%" valign="middle">
								<p>
									<input name="Submit22" type="button" class="button_nor"
										...
										onclick="transferOption(this.form.querysrclist, this.form.groupdestlist,false)">
								</p>
								<p>
									<input name="Submit23" type="button" class="button_nor"
										value="<bean:message key="button.move"/>"
										onclick="returnOption(this.form.groupdestlist, this.form.querysrclist,false)">
								</p>
								<p>
									<input name="Submit22" type="button" class="button_nor"
										value="<bean:message key="button.addall"/>"
										onclick="transferOption(this.form.querysrclist, this.form.groupdestlist,true)">
								</p>
								<p>
									<input name="Submit23" type="button" class="button_nor"
										value="<bean:message key="button.moveall"/>"
										onclick="returnOption(this.form.groupdestlist, this.form.querysrclist,true)">
								</p>
							</td>
							<td width="45%">
								<font color="#FF0000"> <select name="groupdestlist"
										size="16" style="width:350" multiple
										ondblclick="returnOption(this.form.groupdestlist, this.form.querysrclist,false)">
										<%
										...
										%>
										<option
											value="<%=rowtest.get("cutId")%>'$'"><%=rowtest.get("cutName")%>&nbsp;&nbsp;
      ...
      <td align="center" valign="top" ><table width="800" border="0" cellspacing="0" cellpadding="0">
          <tr align="center" valign="top"> 
            <td align="left" valign="middle">
              <input type="checkbox" name="ifadd" value="checkbox">
...
<script>
  	function setCmdValue(cmd) {      		
		var querylist="";
		if (cmd == 'next') {
			if (document.AttendeeInformForm.ifadd.checked){
				cmd = 'stage3';
			}else{
			  cmd = 'stage4';
			}
		...
		for (var i = 0; i <=document.AttendeeInformForm.groupdestlist.options.length-1; ++i){
			document.AttendeeInformForm.groupdestlist.options[i].selected = true;
		}
