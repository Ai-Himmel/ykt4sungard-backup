...
public class SynchronizeUtil extends BasicUtil {
	...
	public boolean Chekdatabase(String databaseIP, String port, String dbname,
		...
		return SynchDBUtils.checkDBstatus(CONSTANT.DB_CONFIG, databaseIP, port, dbname,
				username, password);
	}
	...
	public boolean checkuser(String databaseIP, String port, String dbname,
			...
			String password) {
		SynchDBUtils dbutil = new SynchDBUtils(CONSTANT.DB_CONFIG, databaseIP, port, dbname,
		...
		try {
			...
			ResultSetHandler rsh = new BeanListHandler(TOperator.class,
	...
	public boolean synchData(String databaseIP, String port, String dbname,
		...
		try {
			...
			strsql = "select CON_ID as conId, CUST_ID as custId, PHYSICAL_NO as physicalNo, CARD_NO as cardNo, "
					...
					+ "REPLACER_DUTY as replacerDuty, REPLACER_COMMENTS as replacerComments, DELEGRAY,cut_name as cutName,dept_name as deptName, stuemp_no as stuempNo,sex "
			...
			while (iter.hasNext()) {
				...
				attendeelist.setDelegray(map.get("delegray") == null ? Integer
						.valueOf("0") : IntegerUtil.getInteger((BigDecimal)map.get("delegray")));
				...
				attendeelist.setCutName(map.get("cutName") == null ? "" : map
						.get("cutName").toString());
				attendeelist.setDeptName(map.get("deptName") == null ? "" : map
						.get("deptName").toString());
				attendeelist.setStuempNo(map.get("stuempNo") == null ? "" : map
						.get("stuempNo").toString());
				attendeelist.setSex(map.get("sex") == null ? "" : map
						.get("sex").toString());
				//cut_name as cutName,dept_name as deptName, stuemp_no as stuempNo,sex
			...
			while (iter.hasNext()) {
				...
				if(("5.5").equals(funclist.getFuncCode())){
					continue;
				}
				if(("5.6").equals(funclist.getFuncCode())){
					continue;
				}
				if(("6.2").equals(funclist.getFuncCode())){
					continue;
				}
				if(("6.2").equals(funclist.getFuncCode())){
					continue;
				}
				if(("7.2").equals(funclist.getFuncCode())){
					continue;
				}
	...
	public boolean backsynchData(String databaseIP, String port, String dbname,
		...
		try {
			...
			Query query_his = session.createQuery("from TRecordhis where confid in("+confs+")");
			...
			while (iter_his.hasNext()) {
				...
				String sql = ("insert into ykt_conf.t_recordhis(id,confid,physical_no,checktime,attend_sign,dealornot) values("
						+ rh.getId()
						+","
						+ rh.getConfid()
						+ ",'"
