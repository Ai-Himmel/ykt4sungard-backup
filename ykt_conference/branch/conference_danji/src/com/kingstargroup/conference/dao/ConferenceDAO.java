...
public class ConferenceDAO extends BasicDAO {
	...
	public List getConference(String username) throws HibernateException {
		...
		try {
			SQLQuery q = s
					.createSQLQuery("select conf.con_id as conId, "
			...
			q.addScalar("signTime", Hibernate.STRING);
	...
	public List getConferenceByQuery(String confName, String confType,
		...
		sqlStr
				...
				.append(
						" and conf.organiger_Id in (select dept_Id from T_Dept_Limit where oper_Code='"
								+ username + "')");
		if (!confName.equals("")) {
	...
	public TConference getConIdByTime() {
	...
	public List getConferenceByQuery1(String custno, String custname,
		...
		sqlStr
				...
				.append(
						"att.STUEMP_NO as stuempNo,cust.CLASSDEPT_NO as classdeptNo,att.DEPT_NAME as deptName,att.ATTEND_SIGN as attendSign ")
				.append(
						"from ykt_conf.T_Conference conf,YKT_CONF.T_ATTENDEE_LIST att,YKT_CUR.T_CIF_CUSTOMER cust ")
				.append(
						"where conf.con_id=att.con_id and att.CUST_ID=cust.CUT_ID and conf.status<>'9' ")
				.append(
		...
		if (null != confDept && (!"-".equals(confDept))
				&& (!"".equals(confDept))) {
			sqlStr.append(" and cust.CLASSDEPT_NO='" + confDept + "'");
		}
		...
		sqlStr
				.append(" order by cust.CLASSDEPT_NO,conf.con_Begindate desc,conf.con_Begintime desc");
		try {
			SQLQuery q = s.createSQLQuery(sqlStr.toString());
			info.info(sqlStr.toString());
			q.addScalar("conId", Hibernate.INTEGER);
	...
	public List getConferenceByQuery3(String custno, String custname,
		...
		sqlStr
				.append(
						"select cust.CLASSDEPT_NO as classdeptNo,conf.con_id as conId,count(att.cust_id) as allcount,sum(case att.ATTEND_SIGN when '00' then 0 else 1 end) as shouldcount ")
				.append(
						"from ykt_conf.T_Conference conf,YKT_CONF.T_ATTENDEE_LIST att,YKT_CUR.T_CIF_CUSTOMER cust ")
				.append(
						"where conf.con_id=att.con_id and att.CUST_ID=cust.CUT_ID and conf.status<>'9' ")
		...
		sqlStr
				.append(" group by cust.CLASSDEPT_NO,conf.con_id");
		try {
			SQLQuery q = s.createSQLQuery(sqlStr.toString());
			info.info(sqlStr.toString());
			q.addScalar("classdeptNo", Hibernate.STRING);
	...
	public List getConferenceByQuery6(String custno, String custname,
		...
		sqlStr
		.append(
				"select distinct cust.CLASSDEPT_NO as deptCode,att.DEPT_NAME as deptName,att.CUST_ID as custId,att.CUT_NAME as custName ")
		.append(
				"from ykt_conf.T_Conference conf,YKT_CONF.T_ATTENDEE_LIST att,YKT_CUR.T_CIF_CUSTOMER cust ")
		.append(
				"where conf.con_id=att.con_id and att.CUST_ID=cust.CUT_ID  and conf.status<>'9' ")
		...
		sqlStr.append(" order by cust.CLASSDEPT_NO");
		try {
			SQLQuery q = s.createSQLQuery(sqlStr.toString());
			info.info(sqlStr.toString());
	...
	public List getConferenceByQuery7(String custno, String custname,
		...
		sqlStr
		.append(
				"select distinct cust.CLASSDEPT_NO as classdeptNo,att.cust_id as custId,att.CUT_NAME as cutName ")
		.append(
				"from ykt_conf.T_Conference conf,YKT_CONF.T_ATTENDEE_LIST att,YKT_CUR.T_CIF_CUSTOMER cust ")
		.append(
				"where conf.con_id=att.con_id and att.CUST_ID=cust.CUT_ID and conf.status<>'9' ")
		...
		sqlStr.append(" order by cust.CLASSDEPT_NO,att.cust_id");
		try {
			SQLQuery q = s.createSQLQuery(sqlStr.toString());
			info.info(sqlStr.toString());
			q.addScalar("classdeptNo", Hibernate.STRING);
