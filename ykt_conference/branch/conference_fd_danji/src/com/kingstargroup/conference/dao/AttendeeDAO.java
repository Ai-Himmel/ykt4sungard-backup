...
public class AttendeeDAO extends BasicDAO {
	...
	public void deleteAllAttendeeList(int confId,String attendees) throws HibernateException{
		Session s = getSession();
		if("".equals(attendees)){
			attendees="0";
		}
		try {
			Query q = s.createQuery("delete from TAttendeeList where id.conId="+confId+" and id.custId not in("+attendees+")");
			q.executeUpdate();
		} catch (HibernateException he) {
			errlog.error(he);
			throw he;
	...
	public List getAttendeeListBy(
		...
		StringBuffer sqlStr = new StringBuffer();
		sqlStr.append("select cust.cut_Id as cutId,cust.cut_Name as cutName,cust.stuemp_No as stuempNo,")
		.append(" duty.duty as dutyName,dept.dept_Name as deptName from ykt_cur.T_Cif_Customer cust ")
		.append(" left join ykt_cur.T_Cif_Dept dept on cust.classdept_No=dept.dept_Code ")
		.append(" left join YKT_CONF.T_DUTY duty on cust.stuemp_No=duty.STUEMP_NO where ")
		.append(" cust.cut_Id in(select cust_Id from ykt_conf.T_Inform_List where con_Id="+confid+")");
		if (!"".equals(custno)){
