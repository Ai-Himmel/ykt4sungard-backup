...
public class AttendeeDAO extends BasicDAO {
	...
	public List getAttendeeListByConfId(int confid)throws HibernateException{
		...
		sqlStr.append("select new map(cust.cutId as cutId,cust.cutName as cutName,cust.stuempNo as stuempNo,")
		.append("cust.dutyName as dutyName,dept.deptName as deptName)")
		.append("from TCifCustomer cust ,TCifDept dept where cust.classdeptNo=dept.deptCode and ")
		.append("cust.cutId in(select id.custId from TInformList where id.conId="+confid+")");
		try {
	...
	public List getAttendeeListBy(
		 ...
		 * @since 1.0
		 */
	public void deleteMesList(int devid,String dev999id,int cardno) throws HibernateException{
		Session s = getSession();
		StringBuffer sqlStr = new StringBuffer();
		sqlStr.append(" from TTifMeslist where deviceId="+devid+" and devphy999Id='"+dev999id+"'")
