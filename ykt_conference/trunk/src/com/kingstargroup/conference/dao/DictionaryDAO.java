...
public class DictionaryDAO extends BasicDAO{
	...
	public List getDeptList() throws HibernateException{
		Session s = getSession();
		try {
			SQLQuery q = s.createSQLQuery("select DEPT_CODE,DEPT_NAME from ykt_cur.T_Cif_Deptfull where dept_Level=1");
	 ...
	 * @since 1.0
	 */
	public List getConfList()throws HibernateException{
		Session s= getSession();
		...
		sqlStr.append(" from TConference where organigerId")
		.append(" in(select id.deptId from TDeptLimit where id.operCode='"+WebKeys.USER_KEY+"')")
	 ...
	 * @since 1.0
	 */
	public List getGroupListByOper()throws HibernateException{
		Session s = getSession();
		try {
			Query q = s.createQuery(" from TGroup where operId='"+WebKeys.USER_KEY+"'");
			return q.list();
		} catch (HibernateException he) {
			errlog.error(he);
			throw he;
	 ...
	 * @since 1.0
	 */
	public List getAttendeeGroupListByOper()throws HibernateException{
		Session s = getSession();
		StringBuffer sqlStr = new StringBuffer();
		...
		.append(" group.group_Id in(select group_Id from ykt_conf.T_Group where oper_Id='"+WebKeys.USER_KEY+"')");
		try {
