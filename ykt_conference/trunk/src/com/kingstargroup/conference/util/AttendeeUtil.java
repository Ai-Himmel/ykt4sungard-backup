...
public class AttendeeUtil extends BasicUtil {
	...
	public static boolean addMesListByConfId(int confid) {
		...
		try {
			...
			// 共用部分
			mesList.setFunid(new Integer(930117));
			mesList.setLevel(new Integer(2));
			mesList.setSendcount(new Integer(0));
			mesList.setMaxSendCnt(new Integer(20));
			mesList.setDelFlag("0");
			mesList.setEcode(new Integer(9999));
			mesList.setStateid(new Integer(1));
			mesList.setIndt(datetime.toString());
			mesList.setStartdt("");
			mesList.setEnddt("");
			mesList.setEmsg("");
			mesList.setOutcontent("");

			// 获取设备列表
			Iterator devIter = dictdao.getDeviceListByConfid(confid).iterator();
			// 获取名单列表
			Iterator attIter = dao.getAttListByConfId(confid).iterator();
			while (devIter.hasNext()) {
				TTifMeslist delMesList = new TTifMeslist();
				TPifDevice dev = (TPifDevice) devIter.next();
				String dev999id = GetDictionary.getDev999IdByDevId(Integer
						.parseInt(dev.getDeviceId().toString()));
				...
				delMesList.setDevphy999Id(dev999id);
				dao.save(delMesList);

				//循环5次下发时间段
				for (int i=0 ;i<5;i++){
					sendTime(dao,confid,datetime.toString(),dev,dev999id);
				}
				// 添加下发名单消息队列
				while (attIter.hasNext()) {
					TAttendeeList attendee = (TAttendeeList) attIter.next();
					TTifMeslist addMesList = new TTifMeslist();
					// 删除已经存在的消息队列
					dao.deleteMesList(Integer.parseInt(dev.getSubsystemId()
							.toString()), dev999id, Integer.parseInt(attendee
							.getCardNo().toString()));
					BeanUtils.copyProperties(addMesList, mesList);
					StringBuffer addMesContent = new StringBuffer();
					addMesContent.append("<CARDID>").append(
							attendee.getCardNo()).append("</CARDID>").append(
							"<FTFLAG>").append(0).append("</FTFLAG>");
					log.info(addMesContent);
					...
					addMesList.setDevphy999Id(dev999id);
					dao.save(addMesList);
				}
			}
	...
	private static void sendTime(AttendeeDAO dao,int confid,String datetime,TPifDevice dev,String dev999id){
		// 为设备下发时间段
		ConferenceDAO confdao = ConferenceDAO.getInstance();
		TConference conf = (TConference) confdao.findByPrimaryKey(
				TConference.class, new Integer(confid));
