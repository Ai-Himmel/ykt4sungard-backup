...
namespace MIDSVR{
	...
	int MID_Request_Handler::process_request(ACE_Message_Block *mblk,int data_len)
	{
		...
		if(mblk->length() == 0)
			return -1;
		char xmldata[500]="";
		memcpy(xmldata,(const void *)mblk->rd_ptr(),sizeof(xmldata)-1);
		...
		try
		{
			...
			TrxCode=GetParamIntVal("TrxCode");
			TrxDate=GetParam("TrxDate");
			TrxTime=GetParam("TrxTime");
			switch(TrxCode){
			case 1000:
			...
			ACE_DEBUG((LM_DEBUG,"TrxCode[%d]",TrxCode));
			ACE_DEBUG((LM_DEBUG,"TrxDate[%s]",TrxDate.c_str()));
			ACE_DEBUG((LM_DEBUG,"TrxTime[%s]",TrxTime.c_str()));
			//ACE_DEBUG((LM_DEBUG,"xmldata[%s]",xmldata));
		}
		catch (...)
		{
			ACE_DEBUG((LM_ERROR,"处理数据失败"));
			return -1;
		}
...
int Mid_Increase_Balance::DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer,KeyValueMap &map){
	...
	do 
	{
		if(!obj->SendRequest(1000))
		{
			...
			{
				char code[6];
				map.insert(KeyValueMap::value_type(std::string("RetCode"),
					ACE_OS::itoa(obj->GetReturnCode(),code,5)));
			...
			else
			{
				if(obj->HasMoreRecord())
				{
					...
					map.insert(KeyValueMap::value_type(std::string("HostTrxId"),
						ACE_OS::itoa(HostTrxId,code,16)));
					map.insert(KeyValueMap::value_type(std::string("Amount"),
						ACE_OS::itoa((int)KSGMathUtil::D4U5(Amount*100),code,5)));
					return 0;
				}
			}	
		}
