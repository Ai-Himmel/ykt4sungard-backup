...
int KSGLoopDevInterLoader::LoadInterface(KSGDeviceManager *manager)
{
	try
	{
		KSG_ADD_DEVICE_INTERFACE(KSG_LOOP_DEV,KSG_LOOP_DEV_TYPE,TK_HEARTBEAT_TASK);
		KSG_ADD_DEVICE_INTERFACE(KSG_LOOP_DEV,KSG_LOOP_DEV_TYPE,TK_RESTART_GATEWAY);
		return 0;
	}
	catch(KSGException& )
	{
...
namespace LOOPDEV
{
	...
	int LoopDeviceSvcHandler::handle_input(ACE_HANDLE fd)
	{
		...
		if(peer().recv_n(mb->wr_ptr(),sizeof(int))<= 0)
		{
	...
	int LoopDeviceSvcHandler::svc()
	{
		...
		do
		{
			...
			{
				ACE_Time_Value tv(3);
				if((ret = ACE::handle_read_ready(peer().get_handle(),&tv)))
				{
					if(ret != EWOULDBLOCK)
						break;
				}
				if(handle_input(peer().get_handle()))
				{
	...
	int LoopDeviceSvcHandler::close(u_long flags)
	{
		safe_close();
		return 0;
	}
		...
		else
		{
			...
			try
			{
				while(true)
				{
					...
					if(IsTerminated())
					{
						while(!_s_LoopReactor.event_loop_done())
