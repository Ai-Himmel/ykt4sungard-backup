...
{
	ACE_GUARD(ACE_Recursive_Thread_Mutex,mon,_task_list_mutex);
...
{
	ACE_GUARD_RETURN(ACE_Recursive_Thread_Mutex,mon,_task_list_mutex,KSG_Task_Queue::task_type());
...
{
	ACE_GUARD_RETURN(ACE_Recursive_Thread_Mutex,mon,_task_list_mutex,0);
...
int KSG_Task_Queue_Pool::Queue_List_Cond::wait(ACE_Time_Value *time_out)
{
	...
	_mutex.acquire();
	if(time_out)
		ret = _cond.wait(_mutex,time_out);
	else
		ret = _cond.wait();
	_mutex.release();
	return ret;
...
int KSG_Queue_Task_Scheduler_Impl::open(const std::string &key)
{
	...
	if(element != _head_of_sleeping->_head)
...
{
	KSG_Task_Queue *tail;
	KSG_Task_Queue *old_next;
	tail = head->_head->previous();
	if(tail)
	{
		old_next = tail->next(queue);
		queue->next(old_next);
	}
	else
	{
...
{
	KSG_Task_Queue *next_queue = head->_head->next();
	if(next_queue)
	{
		head->_head->next(queue);
...
int KSG_Queue_Task_Scheduler_Impl::push_queue(KSG_Task_Queue *queue)
{
	...
	{
		ACE_GUARD_RETURN(ACE_Thread_Mutex,mon,q->_mutex,-1);
