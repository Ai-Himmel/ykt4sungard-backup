...
int HDAddNotice::DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer)
{
	...
	{
		ACE_DEBUG((LM_ERROR,"设备机号不正确[%d][%s]"
	...
	if(obj)
	{
		...
		obj->AddField(F_SBANK_CODE2,crc);
		obj->AddField(F_LVOL6,req->IndexOfEW); // 钱包号
		...
		4. 充值冲正时,交易金额是 负值
		*/
		obj->AddField(F_LVOL9,req->Amount); //入卡值 
...
HD_REGISTER_HANDLER(MT_CHECK_AUTH_CARD,HDCheckAuthCardNotice);
int HDCheckAuthCardNotice::DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer)
{
	size_t packlen;
	int ret;
	int term_no;
	...
	// 默认返回一般错误
	this->set_resp_need_mac();
	resp.SetFieldValue(FIELD_RESPONSECODE,RC_OVERDUECARD);
	resp.SetFieldValue(FIELD_TERMINALSN,req->TerminalSN);
	resp.SetFieldValue(FIELD_ADDRESS,req->Address);
	resp.SetFieldValue(FIELD_TERMINALID,req->TerminalID);
	ACE_OS::sprintf(phyid,"%.010d",req->TerminalSN);
