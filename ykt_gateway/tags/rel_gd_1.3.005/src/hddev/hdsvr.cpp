...
namespace HDDEV
{
		...
		const ACE_Asynch_Read_Stream::Result &result)
	{
		...
		else
		{
			...
			ACE_DEBUG((LM_DEBUG,"继续读取数据...len[%d]",(data_len - (mb.length()-MSG_BUF_LEN_HEADER))));
			if( _reader.read(mb
	...
	*/
	...
	do
	{
		...
		if(readbytes == 0)
		{
			...
			{
				ACE_DEBUG((LM_INFO,"当前流水记录文件的大小不正确"));
			}
			return 0;
		}
		bufsize += readbytes;
								  ...
								  ,std::string& filepath)
{
	...
	try
	{
		if(HDRecordFileReader::lock_ccu(node->GetDevId()))
		{
			...
			else
			{
				...
				if(rec_file_len != HDRecordFileReader::RECORD_BUF_LEN * recordcnt)
				{
					ACE_DEBUG((LM_ERROR,"下载记录文件大小与请求包中文件大小不一致except[%d]received[%d]"
						,recordcnt * HDRecordFileReader::RECORD_BUF_LEN,rec_file_len));
...
int HDPOS_Consume_Upload::DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer)
{
	...
	{
		ACE_DEBUG((LM_INFO,"保存POS机批上送的流水成功！"));
		resp.SetFieldValue(FIELD_RESPONSECODE,RC_SUCCESS);
		ret = TASK_SUCCESS;
	}
	return 1;
