...
#define     MACHINE_TYPE_5031   "\x50\x31"  //noport
#define		DRTP_FREE		0
#define		DRTP_CLOSE		1
#define		DRTP_BUSY		2
...
{	
	...
	DWORD	dLastTaskTickCount;		//最后一次获取任务时间
	int		nHasTask;				//是否有任务
}SMARTPOS;
...
typedef struct SMART_INI_FILE_PARA
{
	...
	int			bSupportSubsidy;	//支持补助功能
	int			lCollForceTime;		//强制采集流水时间，单位分
	int			lCollForceLastTime; //强制采集流水持续时间，单位为分
	bool		bWaterMode;			//水控服务模式
}SMARTINIFILEPARA;
