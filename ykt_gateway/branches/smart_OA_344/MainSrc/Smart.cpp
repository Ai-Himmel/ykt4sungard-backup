...
{	
	...
	memset(m_SmartPosConnCnts,0,sizeof m_SmartPosConnCnts);
}
...
{	
	...
	if( !memcmp(m_MacCode,MACHINE_TYPE_5301,2) 
		|| !memcmp(m_MacCode,MACHINE_TYPE_0121,2)
		|| !memcmp(m_MacCode,MACHINE_TYPE_0226,2))
	{
...
{		
	...
	for( i=0; i<m_nTotalPos; i++)//轮询处理状态
	{
		...
		if( memcmp(MACHINE_TYPE_0244,m_SmartPos[i].ucpMachineCode,2)&&
			memcmp(MACHINE_TYPE_0245,m_SmartPos[i].ucpMachineCode,2) )
		{
			if( m_nTotalPos<=1 )	  Sleep(SMT_WAITFORTIME1-100);
			else if( m_nTotalPos==2 ) Sleep(SMT_WAITFORTIME1-100);
			else if( m_nTotalPos==3 ) Sleep(SMT_WAITFORTIME1-100);
			else					  Sleep(SMT_WAITFORTIME1);		
		}
		else
		{
		...
		if( !PostRecv((char*)OutData, sizeof(OutData)/2, SMT_WAITFORTIME2) )
		{
			if(m_SmartPosConnCnts[i]++>50)
			{
				m_SmartPosConnCnts[i] = 0;
				sprintf(m_szErrorText,"未收到终端-->%.8X 机号-%03ld 完整数据包[%s 耗时: %ld 毫秒]",
					m_Data.HexToInt(m_SmartPos[i].ucpIDNo,4),
					(int)m_SmartPos[i].ucMacNo,
					m_IpPar.cIPAddress,
					GetTickCount()-m_nActionTime);
			...
			{
				m_SmartPos[i].nCommunicationStatus=0xFF;
				sprintf(m_szErrorText,"服务器-->%.8X 机号-%03ld 端口-%03ld [%.8X]无设备",
					m_Data.HexToInt((unsigned char*)m_regno,4),
					(int)m_SmartPos[i].ucMacNo,
					(int)m_SmartPos[i].nConnectPort,
					m_Data.HexToInt(m_SmartPos[i].ucpIDNo,4));
				//if( g_IniFilePara.bWriteDebugLog )		m_pDialog->AddString(m_szErrorText);
			}
			m_SmartPos[i].nNotCommCount++;
		...
		m_SmartPos[i].nNotCommCount=0;
		if(memcmp(m_MacCode,MACHINE_TYPE_5301,2)==0)
		{
			//memcpy(OutData,OutData2,)
		}
		//检查数据包的完整性和有效性
		if( !ValidateData(OutData,m_nReadBytes) )
		{
		...
		switch(m_SmartPos[i].nCommunicationStatus=OutData[2] )
		{
			...
			case 0x1B:
				...
				break;
			case 0x0B:		//授权号错
				sprintf(m_szErrorText,"收到设备[%.8X]返回的状态码:%ld,授权号错..........",
					m_Data.HexToInt(m_SmartPos[i].ucpIDNo,4),m_SmartPos[i].nCommunicationStatus);
				WriteLog(m_szErrorText);
				m_pDialog->AddString(m_szErrorText);	
				SMT_ProcessIDError(m_SmartPosPostion);
				break;
...
bool CSmart::SMT_SettleAccount( int nPosition)
{
	...
	if(memcmp(this->m_MacCode,MACHINE_TYPE_5301,2)==0)
	{
		Doc.level = 1;
		Doc.level_array[0] = this->m_MacNum;
		Doc.level_array[1] = m_SmartPos[nPosition].nConnectPort;
	}
...
bool CSmart::SMT_SetOpenSaleCtrl(SMARTCOMMPACK* Doc)
{
	...
	SendLen=m_Data.BuildData( Doc->ucMacNo,1,commision, ucCommandByte,Doc->nSndDataLen,InData,OutData,Doc->level, Doc->level_array);
...
{		
	...
	m_pDialog->AddString(m_szErrorText);
	if( memcmp(MACHINE_TYPE_0121,m_SmartPos[nPostion].ucpMachineCode,2) == 0
		|| memcmp(MACHINE_TYPE_0232,m_SmartPos[nPostion].ucpMachineCode,2) == 0
		|| memcmp(MACHINE_TYPE_5031,this->m_MacCode,2) == 0) //计时数据
	{
	...
	Doc.level = 0;
	if(memcmp(MACHINE_TYPE_5301,this->m_MacCode,2)==0)
	{
		// WLPORT 确认数据
		Doc.level = 1;
		Doc.level_array[0] = this->m_MacNum;
		Doc.level_array[1] = m_SmartPos[nPostion].nConnectPort;
	}
...
{	
	...
	SendLen=m_Data.BuildData( Doc->ucMacNo,1,Doc->ucRegNo, ucCommandByte,Doc->nSndDataLen,Doc->ucSndData,OutData,Doc->level, Doc->level_array);
	...
	Sleep(SMT_WAITFORTIME1+SMT_WAITFORTIME2);
...
{		
	...
	SendLen=m_Data.BuildData( Doc->ucMacNo,Doc->nFlag,Doc->ucpIDNo, 
		Doc->ucCmd,Doc->nSndDataLen,Doc->ucSndData,OutData,Doc->level, Doc->level_array);
...
{	
	...
	for( int i=nStartInvoice; i<=nEndInvoice; i++ )
	{
		...
		SendLen=m_Data.BuildData( CommPack->ucMacNo,1,commision, ucCommandByte,CommPack->nSndDataLen,
			InData,OutData,CommPack->level, CommPack->level_array);
...
{	
	...
	try
	{
		...
		while( !IsShutdown() )
		{
			if(memcmp(m_MacCode,MACHINE_TYPE_0244,2)!=0 &&
				memcmp(m_MacCode,MACHINE_TYPE_0245,2)!=0)
			{
				// sleep 一下
				Sleep(300);
			}
			...
			switch( Doc.nTaskCode)
			{
				...
				case SMT_SMARTFRONTGETTIMEWHITELIST:	//下传/删除白名单/所有白名单
					if(Doc.ucCmd == 0xAB)
						result = SMT_SetWhiteCard(&Doc);
					else if(Doc.ucCmd == 0xAC)
						result = SMT_DelWhiteCard(&Doc);
					else if(Doc.ucCmd == 0xAD)
					{
						// not support !
					}
					break;
...
bool CSmart::SMT_SetWhiteListAndVer(SMARTCOMMPACK *Doc)
{
	...
	else
	{
		...
		m_Data.BcdToAsc(ucSnd,6,(BYTE*)blkver);
		Doc->nSndDataLen=4;
		if(Doc->ucCmd == 0xAB)
			sprintf(m_szErrorText,"开始增加一条白名单[机号=%ld IP=%s][Card=%d Ver=%s]............"
			,Doc->ucMacNo,m_IpPar.cIPAddress,Doc->nMemo,blkver);
		else
			sprintf(m_szErrorText,"开始删除一条白名单[机号=%ld IP=%s][Card=%d Ver=%s]............"
			,Doc->ucMacNo,m_IpPar.cIPAddress,Doc->nMemo,blkver);
		WriteLog(m_szErrorText);
		m_pDialog->AddString(m_szErrorText);
		if( !SMT_ExecuteCmd411(Doc) ) //下传卡号
		{
			sprintf(m_szErrorText,"已收到终端-->%08X 下传白名单失败1[卡号=%ld IP=%s, %ld 状态->%ld]",
...
bool CSmart::SMT_ExecuteCmd411(SMARTCOMMPACK* Doc)		
{
	...
	for( int i=0; i<3; i++ )
	{
		...
		m_nTaskResultStatus	= 6;
		bResult				= false;
		SendLen=m_Data.BuildData( Doc->ucMacNo,Doc->nFlag,Doc->ucpIDNo, Doc->ucCmd,Doc->nSndDataLen,Doc->ucSndData,OutData,Doc->level,Doc->level_array);
		...
		else if( Doc->ucCmd==0xAB)	Sleep(nSleep);
		else if( Doc->ucCmd==0xAC)	Sleep(nSleep);
...
{	
	...
	m_SmartPosPostion=0;
	for(i = 0;i<this->m_nTotalPos;++i)
	{
		if(memcmp(cTask->ucRegNo,m_SmartPos[i].ucRegNo,4)==0)
		{
			m_SmartPosPostion = i;
			break;
		}
...
bool CSmart::Smt_CollectDataInit()
{
	...
	for( int i=0; i<m_nTotalPos; i++)
	{
		...
		if(memcmp(this->m_MacCode,MACHINE_TYPE_5301,2)==0)
		{
			level_array[0] = this->m_MacNum;
			level_array[1] = m_SmartPos[i].nConnectPort;
			m_SmartPos[i].nSndLen=m_Data.BuildData( m_SmartPos[i].ucMacNo,1,m_SmartPos[i].ucpIDNo, ucCommandByte,0,NULL,m_SmartPos[i].ucSendData,1,level_array);
		}
		else
		{
...
{	
	bool bFlag;
	SMARTCOMMPACK cDoc;
	sprintf(m_szErrorText,"开始增加一条白名单[机号=%ld Card=%d IP=%s]............"
		,Doc->ucMacNo,Doc->nMemo,m_IpPar.cIPAddress);	
	WriteLog(m_szErrorText);
	m_pDialog->AddString(m_szErrorText);
	Doc->ucCmd=0xAB;
	memcpy(&cDoc,Doc,sizeof(cDoc));
	...
	return SMT_ExecuteCmd411(Doc);
	*/
	if( !(bFlag=SMT_ExecuteCmd411(Doc))||Doc->nRecDataLen!=1 )
	{
		sprintf(m_szErrorText,"已收到终端-->%08X 下传白名单失败1[卡号=%ld IP=%s, %ld 状态->%ld]",
			m_Data.HexToInt(Doc->ucpIDNo,4),
			Doc->nMemo,
	...
	Sleep(50);
	// 判断卡号
	cDoc.nSndDataLen = 0x03;
	cDoc.ucCmd		 = 0xB0;
	cDoc.nRecDataLen = 0x00;
	cDoc.nFlag		 = 0;
	/*
	cDoc.ucSndData[0] = Doc->nMemo<<16;
	cDoc.ucSndData[1] = Doc->nMemo<<8;
	cDoc.ucSndData[2] = Doc->nMemo;
	*/
	memcpy(cDoc.ucSndData,Doc->ucSndData+1,3); // 卡号

	if( !SMT_ExecuteCmd411(&cDoc))
	{
		sprintf(m_szErrorText,"已收到终端-->%08X 下传白名单失败3[卡号=%ld IP=%s, %ld 状态->%ld]",
			m_Data.HexToInt(cDoc.ucpIDNo,4),
			cDoc.nMemo,
			m_IpPar.cIPAddress,
			cDoc.nMessageID,
...
{	
	...
	if(!SMT_ExecuteCmd411(Doc))
	{
		sprintf(m_szErrorText,"已收到终端-->%08X 删除白名单失败1[卡号=%ld IP=%s, %ld 状态->%ld]",
			m_Data.HexToInt(Doc->ucpIDNo,4),
			Doc->nMemo,
			m_IpPar.cIPAddress,
			Doc->nMessageID,
