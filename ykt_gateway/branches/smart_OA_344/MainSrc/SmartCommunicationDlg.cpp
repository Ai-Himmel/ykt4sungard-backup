...
BOOL CSmartCommunicationDlg::OnInitDialog()
{
	...
	CString title;
	title.Format("一卡通前置机 ver : %s %s %s ",SVR_VERSION,__DATE__,__TIME__);
...
bool CSmartCommunicationDlg::Init()
{
	...
	// 前置机采集流水时间
	memset(buf,0,sizeof buf);
	mfile.GetProfileStr("system.ini","SLEEP","COLLFORCE",buf);
	mData.trim(buf);
	if(!strlen(buf)) g_IniFilePara.lCollForceTime = 5;
	else			 g_IniFilePara.lCollForceTime = atol(buf);

	// 前置机采集流水持续时间
	memset(buf,0,sizeof buf);
	mfile.GetProfileStr("system.ini","SLEEP","COLLFORCELAST",buf);
	mData.trim(buf);
	if(!strlen(buf)) g_IniFilePara.lCollForceLastTime = 2;
	else			 g_IniFilePara.lCollForceLastTime = atol(buf);

	// 前置机水控模式工作
	memset(buf,0,sizeof buf);
	mfile.GetProfileStr("system.ini","SLEEP","WATERMODE",buf);
	mData.trim(buf);
	if(!strlen(buf)) g_IniFilePara.bWaterMode = false;
	else			 g_IniFilePara.bWaterMode = (atol(buf)==1) ? true : false;
	...
	// 补助标志
	memset(buf,0,sizeof buf);
	mfile.GetProfileStr("system.ini","DEVICEQUERYCMD","SUBSIDY",buf);
	mData.trim(buf);	
	if( atol(buf)>0 )
	{
		g_IniFilePara.bSupportSubsidy=true;
	}
	else
	{
		g_IniFilePara.bSupportSubsidy=false;
	}
	//创建金仕达连接数
	memset(buf,0,sizeof buf);
...
{		
	...
	AddString(msg);
	WriteLog(msg);	
	if( !g_CInterface.SMT_GetAllInitBlackList(SMT_SMARTFRONTDOWNALLBLACK) )
	{
		strcpy(m_ErrMsg,g_CInterface.m_szText);
		goto To_SystemEnd;
	}
	...
	if(g_IniFilePara.bSupportSubsidy)
	{
		//获取补助名单
	...
	WriteLog(m_ErrMsg);	
	//启动所有通过TCP连接的服务	
	if(g_LTcpsmart.size()>300)
	{
		sprintf(m_ErrMsg,"前置系统总共需要启动链路服务数超过300个.",g_LTcpsmart.size());
		AddString(m_ErrMsg);	
		WriteLog(m_ErrMsg);	
		goto To_SystemEnd;
	}
...
bool CSmartCommunicationDlg::SMT_UpBeforeFile(XDHANDLE& nHandler)
{
	...
	for( int i=0; i<VFileName.size(); i++ )	//将还未上传的文件记录全部上传
	{
		...
		strcat(sTempPath,"\\");
		strcat(sTempPath,VFileName[i].GetBuffer(100)); // 本地流水文件名

		strcpy(sBakPath,m_DealPath);
		strcat(sBakPath,"\\Bak\\Bak_");
		strcat(sBakPath,VFileName[i].GetBuffer(100)); // 备份流水文件名		
		//remove(sBakPath);
		...
		{
			bStatus=false;
			for( int k=0; k<10; k++ ) //一条记录如果上传10次还不成功,则全部重新上传
			{				
				if( !g_StartFlag ) 	
				{
					VFileContent.clear();
				...
				memset(sData,0,sizeof sData);
				strncpy(sData,(LPCTSTR)VFileContent[j],128);
...
bool CSmartCommunicationDlg::GetDevicePara()
{
	...
	for( j=0; j<nLinkcount; j++ )
	{
		...
		// 注： 以太网设备按这样的方式处理
		if( !SocketSmart.m_nTotalPos ) //没有子设备时，则为直连设备
		{
		...
		else if( !memcmp(g_VLinkIP[j].ucpMachineCode,MACHINE_TYPE_5301,2)) // WLPORT
		{
			//SocketSmart.m_nPortTotal=ntotal;
		}
...
bool CSmartCommunicationDlg::SMT_GetOneTaskResult(SMARTTASKRESULT *mTaskResult)
{
	...
	if( LpTaskResult!=g_LSaveTaskResult.end() )
	{
