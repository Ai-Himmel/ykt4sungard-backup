...
static int s_last_errcode = 0;

#ifndef NO_DEBUG_OUPUT
#define DEBUG_OUTPUT(msg) do { \
	memset(s_errmsg,0,sizeof s_errmsg); \
	strncpy(s_errmsg,msg,sizeof s_errmsg - 1); \
	s_last_errcode = errno; \
	perror(msg); } while(0)

#else
#define DEBUG_OUTPUT(msg) perror(msg)
#endif

GLOBALDEF int FtpErrMsg(char msg[1024])
{
	strcpy(msg,s_errmsg);
	return errno;
}
...
static int readresp(char c, netbuf *nControl)
{
	...
	{
		DEBUG_OUTPUT("Control socket read failed");
		return 0;
	}
	if (ftplib_debug > 1)
				   ...
				   netbuf *nControl, int typ, int mode)
{
	...
	{
		// modify by cash at 2007-1-4
		// remember close filp
		if(local && local != stdin && local != stdout)
			fclose(local);
		return 0;
	}
