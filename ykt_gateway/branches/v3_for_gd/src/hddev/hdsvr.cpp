...
int HDDEV::HDParsePosRecord(BYTE* buf,TRADERECORDSTRUCT* trade)
{
	...
	trade->shop_id = trade->SubsidyNo;			// 商户号
...
int HDInitKeyHandler::DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer)
{
	...
	resp.SetFieldValue(FIELD_TERMINALSN,req->TerminalSN);
	resp.SetFieldValue(FIELD_ADDRESS,req->Address);
	resp.SetFieldValue(FIELD_ADDITIONALDATA2,buf,72);
	/*
	...
	*/
		...
		//|| ACE_OS::strcmp(peer_addr.get_host_addr(),"0.0.0.0")==0)
	{
		...
		// 维护密钥
		if(xutil::StringUtil::Str2Hex(
			KsgGetGateway()->GetConfig()->_maintainKey.c_str(),(unsigned char*)buf+24,16) == -1)
			return -1;

		// 消费密钥
		if(xutil::StringUtil::Str2Hex(
			KsgGetGateway()->GetConfig()->_tradeKey.c_str(),(unsigned char*)buf+40,16) == -1)
			return -1;

		// 充值密钥
		ACE_OS::memset(buf+56,0xFF,16);
...
int HDOnlineNotice::DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer)
{
	...
	//ACE_DEBUG((LM_INFO,"设备[%s]在线通知,max_ver[%s]",phyid,KsgGetGateway()->get_max_blk_version()));
	/*
...
{	
	...
	//obj->AddField(F_SDATE1,sn);
	obj->AddField(F_SPHONE3,sn);
	// 修改 2007-8-6 , 广州大学城的修改
	// 餐次
	ACE_OS::sprintf(temp,"%d",trade->MealId);
	obj->AddField(F_STX_PWD,temp);

	// 商户号
	obj->AddField(F_LVOL3,trade->shop_id); // 通讯版本与 39 兼容

	if(tx_mark == E_999_SUBISIDY)
	{
		// 如果是补助流水
		...
		if(!obj->SendRequest(2000))
		{
			...
			else
			{
				...
				if(obj->HasMoreRecord())
				{
					...
					resp.SetFieldValue(FIELD_RESPONSECODE,RC_SUCCESS);
					ACE_DEBUG((LM_INFO,"充值交易成功!!dev[%s]serialno[%d]",phyid,req->TraceOfPOS));
				}
			}
		}
	}
...
int HDAddCancelNotice::DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer)
{
	...
	if(obj)
	{
		...
		if(!obj->SendRequest(5000))
		{
			...
			{
				resp.SetFieldValue(FIELD_RESPONSECODE,RC_SUCCESS);
				ACE_DEBUG((LM_INFO,"充值冲正交易成功!!dev[%s]serialno[%d]",phyid,req->TraceOfPOS));
			}
		}
	}
	return 1;
...
int HDPOS_Consume_Upload::DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer)
{
	...
	{
		ACE_DEBUG((LM_TRACE,"保存POS机批上送的流水成功！[%d]",count));
		resp.SetFieldValue(FIELD_RESPONSECODE,RC_SUCCESS);
		ret = TASK_SUCCESS;
	}
	return 1;
