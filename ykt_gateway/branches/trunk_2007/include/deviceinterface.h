...
*/
typedef ACE_Singleton<AbstractObject<int,KSGTaskExecutor>,ACE_SYNCH_MUTEX> KSGTaskExecutorFactory;
...
*/
template<class AtomicType,class Base>
class NormalConcreteFactoryUnit : Base
{
	//<! 父对象中产品类型列表类型
	typedef typename Base::ProductList BaseProductList;
protected:
	//<! 父对象中最后一个产品类型
	typedef typename BaseProductList::Tail ProductList;
	AtomicType* atomic_obj_;
public:
	NormalConcreteFactoryUnit():atomic_obj_(NULL)
	{}
	virtual ~NormalConcreteFactoryUnit()
	{
		if(atomic_obj_)
			delete atomic_obj_;
	}
...
struct KSGDeviceInterfaceFactory
{
	typedef ConcreteFactory<
		AbstractDeviceInterfaceFactory,
		// modify 2007-10-22 汤成
		NormalConcreteFactoryUnit,
		TList> Result; 
};
...
*/
#define KSG_ADD_DEVICE_INTERFACE(dev,devtype,di_clazz) do{\
	KSGDevice* _d = NULL; \
	KSG_GET_DEVICE_OBJECT(dev,devtype,_d); \
	if(_d) _d->AddInterface(new di_clazz); else throw DevTypeNotExists(); }while(0)
