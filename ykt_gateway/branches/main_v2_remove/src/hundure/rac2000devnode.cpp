		...
		while(drtp->HasMoreRecord() && door_count < 2)
		{
			...
			case 1:
				...
				if(node->GetDeviceType() == HUNDURE_RAC2000P)
				{
					...
					ACE_DEBUG((LM_INFO,"下载门1 卡名单,卡号[%s]week[%d]status[%d]ret[%d]",
						phyno_dec.c_str(),weekno,status,ret));
			...
			case 2:
				...
				if(node->GetDeviceType() == HUNDURE_RAC2000P)
				{
					...
					ret = hacAddCard(nodeid,phyno_dec.c_str(),phyno_dec.length()
						,NULL,0,weekno,status,hd,g_rac_default_timeout);
					ACE_DEBUG((LM_INFO,"下载门2 卡名单,卡号[%s]week[%d]status[%d]ret[%d]",
						phyno_dec.c_str(),weekno,status,ret));
				}
				else
				{
...
int ID_HNDR_RAC2000_UL_Event::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	while(exec_times-->0)
	{
		...
		{
			ACE_DEBUG((LM_INFO,"采集汉军 RAC [%d] 事件[%d] 条",task_node->GetDevId(),success));
