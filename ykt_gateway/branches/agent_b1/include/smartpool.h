...
template<
	class T,
	class ThreadModel = ACE_Thread_Mutex
>
class SmartObjectAllocator 
{
private:
	boost::object_pool<T> _ref_pool;
	ThreadModel _mutex;
public:
	SmartObjectAllocator()
	{}
	~SmartObjectAllocator()
	{}
	T* malloc()
	{
		ACE_GUARD_RETURN(ThreadModel,mon,_mutex,NULL);
		return _ref_pool.malloc();
	}
	void free(T *p)
	{
		ACE_GUARD(ThreadModel,mon,_mutex);
		_ref_pool.free(p);
	}
};
...
private:
	...
	//<! 内存池
	static SmartObjectAllocator<unsigned int> s_ref_pool;
public:
	//! default constructor
	RefCounted():_pRefCnt(s_ref_pool.malloc())
	{
		//_pRefCnt = s_ref_pool.malloc();
...
template<class T> 
SmartObjectAllocator<unsigned int> RefCounted<T>::s_ref_pool;
...
public:	
	...
	//! default constructor
	PoolSmartObject():_pool(NULL),SP(NULL)
	{
...
public:
	...
	//<! 缓存池可用对象队列
	typedef std::list<PointerType> PoolQueueType;
	//<! 创建缓存对象的functor 类型
...
L_LOCK_AGAIN:
	...
	void Alloc(SmartObject& obj)
	{
		...
		PointerType p = _freeQueue.front();
