...
int _stdcall MakeBuf_SetWhiteVersion(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer,unsigned char *date);/// 功能：下传白名单版本/// 参数：*date ---- 六子节白名单版本
...
long _stdcall Smart_CardDes(unsigned char *str1, 
						   unsigned char *str2,
						   unsigned char *str3,
						   unsigned char *str4,
						   char cFlag);
...
extern byte * DoubleStrToHex(unsigned char *str,unsigned char *hex);
extern byte* SetClassToHex(unsigned char *str,unsigned char *hex);

//定义日志
extern void WriteLog(const char *format, ...);
extern void WriteLogEx(TSSmartDocEx *pDocEx, const char *format, ...);
extern void GetCurDateTime(char *pszDateTime);
extern void DebugPrint(const char *format, ...);
extern void GetCurDateTime(char *pszDateTime);
extern void GetCurPath(LPTSTR pszDir);
extern int  GetBinaryValue(char *pszString);
...
extern TCHAR szOperPath[MAX_PATH];
