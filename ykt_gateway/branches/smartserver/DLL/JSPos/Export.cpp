...
//初始化LPORT
long TaskBufferProc999(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);
long TaskProc999(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);

//add by dengjun 20080306 设定特殊的普通日子
long TaskBufferProc201(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);
long TaskProc201(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);

//add by dengjun 20080306 设定寒假和暑假的开始截止日期及周末规则
long TaskBufferProc202(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);
long TaskProc202(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);

//add by dengjun 20080306 3.3.	设定国家公假日
long TaskBufferProc203(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);
long TaskProc203(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);

//add by dengjun 20080306 3.4.	下传假节日时间段
long TaskBufferProc204(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);
long TaskProc204(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);

//add by dengjun 20080306 3.5.	下传假节日时间段可交易的卡类
long TaskBufferProc205(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);
long TaskProc205(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer);
...
static TaskFunction TaskFunc[] = 
{
	...
	201, "设定特殊的普通日子",		TaskBufferProc201, TaskProc201,	
	202, "设寒假和暑假日期周末规则",TaskBufferProc202, TaskProc202,	
	203, "设定国家公假日",			TaskBufferProc203, TaskProc203,	
	204, "下传假节日时间段",		TaskBufferProc204, TaskProc204,	
	205, "下传假节日时间段可交易的卡类",TaskBufferProc205, TaskProc205,	
	999, "初始化LPORT",				TaskBufferProc999, TaskProc999,	
	-1,  "", NULL, NULL 
};
...
//	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "135"); //add by dengjun 20050830
	...
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "999"); //add by dengjun 20060918
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "201"); //add by dengjun 20080306
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "202"); //add by dengjun 20080306
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "203"); //add by dengjun 20080306
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "204"); //add by dengjun 20080306
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "205"); //add by dengjun 20080306
...
extern "C" long __stdcall DLLSupportInfo(TSSmartSupport *pSupport)
{
	...
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "201"); //add by dengjun
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "202"); //add by dengjun
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "203"); //add by dengjun
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "204"); //add by dengjun
	strcpy(pSupport->TaskCode[pSupport->nTaskCount++], "205"); //add by dengjun
...
long TaskBufferProc136(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	printf("下传或删除白名单");
	sprintf(msg,"开始下传白名单[%d]",cardno);
	ReportLog(pDocEx, pTask, RET_OK, msg, "");
...
long TaskProc136(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	addr = pDocEx->pChildDoc[n]->DeviceInfo.nAddr;
	...
	if( pBuffer->pBuffer[1] != addr ) 
	{
		nResult = RET_ADDR_ERROR;
		sprintf(msg,"下传/删除白名单,addr1[%d],addr2[%d]",addr,pBuffer->pBuffer[1]);
		ReportLog(pDocEx, pTask, nResult, msg, "");
		GetResultMessage(nResult, data.sMsg);
		ReportTaskResult(pTask, nResult, &data);
		return nResult;
	}
	TSAttachData *pHead = (TSAttachData*)pTask->pData ;
	int cardno = atol(pHead->szValue);
	sprintf(msg,"下传/删除白名单[%d]",cardno);
	ReportLog(pDocEx, pTask, RET_OK, msg, "");
	GetResultMessage(RET_OK, data.sMsg);
	ReportTaskResult(pTask, RET_OK, &data);
	return RET_OK;
...
long TaskProc142(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	pTask->nRepeat1=999;
	int  n = pDocEx->nPortNo;
	int nResult = UnMakeDeviceBufferEx(pBuffer);
...
{
	//int  n = pDocEx->nPortNo;	
	//MakeBuf_SetLportToFactorySetting(&pDocEx->pChildDoc[n]->DeviceInfo, pBuffer);	
	MakeBuf_SetLportToFactorySetting(pDocEx->pDeviceInfo, pBuffer);	
	return RET_OK;
}
...
{	
	...
	if( nResult != RET_OK ) 
	{
		ReportLog(pDocEx, pTask, nResult, "初始化LPORT", "");
		return nResult;
	}	
	ReportLog(pDocEx, pTask, RET_OK, "初始化LPORT", "");	
	return RET_OK;
}
...
{	
	...
	//注册号
	SVC_DSP_2_HEX((byte*)pDocEx->pChildDoc[pDocEx->nPortNo]->DeviceInfo.sRegNo,pBuffer->pBuffer,8);
	pBuffer->nBufferSize+=4;
	strcpy((char*)sStr,pTask->sData);	
	DoubleStrToHex(sStr,pBuffer->pBuffer+pBuffer->nBufferSize);
	pBuffer->nBufferSize+=(strlen((char*)sStr)/2);
	MakeBuf_SetSpecialDate(&pDocEx->pChildDoc[pDocEx->nPortNo]->DeviceInfo, pBuffer);
	//SVC_HEX_2_DSP(pBuffer->pBuffer,sStr,pBuffer->nCmdSize);
	//printf("\n...........%s\n\n",(char*)sStr);	
	return RET_OK;
}
...
{	
	...
	if( nResult != RET_OK ) 
	{
		ReportLog(pDocEx, pTask, nResult, "设置特殊日子", "");
		GetResultMessage(nResult, data.sMsg);
		ReportTaskResult(pTask, nResult, &data);
		return nResult;
	}
	ReportLog(pDocEx, pTask, RET_OK, "设置特殊日子", "");
	GetResultMessage(RET_OK, data.sMsg);
	memcpy(data.ssValue1,pBuffer->pBuffer,pBuffer->nBufferSize);
	data.nValue1 = pBuffer->nBufferSize;
	ReportTaskResult(pTask, RET_OK, &data);
...
{	
	...
	SVC_DSP_2_HEX((byte*)pDocEx->pChildDoc[pDocEx->nPortNo]->DeviceInfo.sRegNo,pBuffer->pBuffer,8);
	pBuffer->nBufferSize+=4;
	memcpy(sStr,pTask->sData,strlen(pTask->sData)-1);	
	DoubleStrToHex(sStr,pBuffer->pBuffer+pBuffer->nBufferSize);
	pBuffer->nBufferSize+=(strlen((char*)sStr)/2);
	pBuffer->pBuffer[pBuffer->nBufferSize]=pTask->sData[strlen(pTask->sData)-1]-0x30;
	pBuffer->nBufferSize++;
	MakeBuf_SetHolidayDate(&pDocEx->pChildDoc[pDocEx->nPortNo]->DeviceInfo, pBuffer);	
//SVC_HEX_2_DSP(pBuffer->pBuffer,sStr,pBuffer->nCmdSize);
//printf("\n...........%s\n\n",(char*)sStr);	
	return RET_OK;
}
...
long TaskProc202(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	if( nResult != RET_OK ) 
	{
		ReportLog(pDocEx, pTask, nResult, "设定寒假和暑假的开始截止日期及周末规则", "");
		GetResultMessage(nResult, data.sMsg);
		ReportTaskResult(pTask, nResult, &data);
		return nResult;
	}
	ReportLog(pDocEx, pTask, RET_OK, "设定寒假和暑假的开始截止日期及周末规则", "");
	GetResultMessage(RET_OK, data.sMsg);
	memcpy(data.ssValue1,pBuffer->pBuffer,pBuffer->nBufferSize);
	data.nValue1 = pBuffer->nBufferSize;
	ReportTaskResult(pTask, RET_OK, &data);
...
{	
	...
	DoubleStrToHex(sStr,pBuffer->pBuffer+pBuffer->nBufferSize);
	pBuffer->nBufferSize+=(strlen((char*)sStr)/2);
	MakeBuf_SetGongJiaDate(&pDocEx->pChildDoc[pDocEx->nPortNo]->DeviceInfo, pBuffer);	
...
long TaskProc203(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	if( nResult != RET_OK ) 
	{
		ReportLog(pDocEx, pTask, nResult, "设定国家公假日", "");
		GetResultMessage(nResult, data.sMsg);
		ReportTaskResult(pTask, nResult, &data);
		return nResult;
	}
	ReportLog(pDocEx, pTask, RET_OK, "设定国家公假日", "");
	GetResultMessage(RET_OK, data.sMsg);
	memcpy(data.ssValue1,pBuffer->pBuffer,pBuffer->nBufferSize);
	data.nValue1 = pBuffer->nBufferSize;
	ReportTaskResult(pTask, RET_OK, &data);
...
long TaskBufferProc204(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	SVC_DSP_2_HEX((byte*)pDocEx->pChildDoc[pDocEx->nPortNo]->DeviceInfo.sRegNo,pBuffer->pBuffer,8);
	pBuffer->nBufferSize+=4;
    if( strlen(pTask->sData)%2 )
	{
		sBuf[0]=pTask->sData[0];
		i=atol(sBuf);
		strcpy((char*)sStr,pTask->sData+1);
	}
	else
	{
		memcpy(sBuf,pTask->sData,2);
		i=atol(sBuf);
		strcpy((char*)sStr,pTask->sData+2);
	}	
	pBuffer->pBuffer[4]=i;
	pBuffer->nBufferSize++;
	DoubleStrToHex(sStr,pBuffer->pBuffer+pBuffer->nBufferSize);
	pBuffer->nBufferSize+=(strlen((char*)sStr)/2);
	MakeBuf_SetHolidayTime(&pDocEx->pChildDoc[pDocEx->nPortNo]->DeviceInfo, pBuffer);	
...
long TaskProc204(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	if( nResult != RET_OK ) 
	{
		ReportLog(pDocEx, pTask, nResult, "下传假节日时间段", "");
		GetResultMessage(nResult, data.sMsg);
		ReportTaskResult(pTask, nResult, &data);
		return nResult;
	}
	ReportLog(pDocEx, pTask, RET_OK, "下传假节日时间段", "");
	GetResultMessage(RET_OK, data.sMsg);
	memcpy(data.ssValue1,pBuffer->pBuffer,pBuffer->nBufferSize);
	data.nValue1 = pBuffer->nBufferSize;
	ReportTaskResult(pTask, RET_OK, &data);
...
long TaskBufferProc205(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	pBuffer->pBuffer[4]=i;	
	pBuffer->nBufferSize++;
	SetClassToHex(sStr,pBuffer->pBuffer+pBuffer->nBufferSize);
	pBuffer->nBufferSize+=32;
	MakeBuf_SetHolidayClass(&pDocEx->pChildDoc[pDocEx->nPortNo]->DeviceInfo, pBuffer);	
//SVC_HEX_2_DSP(pBuffer->pBuffer,sStr,pBuffer->nCmdSize);
//printf("\n...........%s\n\n",(char*)sStr);		
	return RET_OK;	
}
...
long TaskProc205(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	if( nResult != RET_OK ) 
	{
		ReportLog(pDocEx, pTask, nResult, "下传假节日时间段可交易的卡类", "");
		GetResultMessage(nResult, data.sMsg);
		ReportTaskResult(pTask, nResult, &data);
		return nResult;
	}
	ReportLog(pDocEx, pTask, RET_OK, "下传假节日时间段可交易的卡类", "");
	GetResultMessage(RET_OK, data.sMsg);
	memcpy(data.ssValue1,pBuffer->pBuffer,pBuffer->nBufferSize);
	data.nValue1 = pBuffer->nBufferSize;
	ReportTaskResult(pTask, RET_OK, &data);
