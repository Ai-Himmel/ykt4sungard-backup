...
static COperator::TSMacTypeArray  MacTypeArray[] =
{
	...
	MACHINE_TYPE_0248, "0248", "转帐机", 
	MACHINE_TYPE_0236, "0236", "独立式淋浴计费器",
	MACHINE_TYPE_1055, "1055", "手持机（计费）",
	MACHINE_TYPE_5501, "5501", "扇型通道机",
	MACHINE_TYPE_1056, "1056" ,"手持机(计费+补助)",	
...
//		return RET_VALIDPACKET;
	...
	BroadcastPro(PACKET_CMD_1001, pDocEx->pChildDoc[m_nIndex],ucRawData, nLen);
...
//处理终端设备状态
long COperator::ProcessStatusRequest(TSSmartDocEx *pDocEx, TSSmartTask *pTask, TSCmdBuffer *pBuffer)
{
	...
	WriteLog("进入处理终端设备状态函数ProcessStatusRequest(),注册号=%s,机号=%ld\n",pDocEx->pDeviceInfo->sRegNo,pDocEx->pDeviceInfo->nAddr);
	
	memset(sTemp,      0x00, sizeof sTemp      );
	memset(&CardResult,0x00, sizeof CardResult );	
	memset(ucRawData,  0x00, sizeof ucRawData  );	
	memcpy(CardResult.sValid,"930058",6);  //功能号
	memset(CardResult.sVersion,'1', sizeof(CardResult.sVersion)); //初始值全部赋不通

	iResult = UnMakeDeviceBuffer(pBuffer);
	if( iResult != RET_OK )			//表示主设备不通
	{	
		CardResult.nCardID = 1;		//服务器不通
		CardResult.nFlag   = 0;
		OutputBlackCard(pDocEx->pChildDoc[pDocEx->nPortNo], RET_OK, &CardResult);			
		WriteLog("进入处理终端设备状态函数UnMakeDeviceBuffer()失败,服务不通\n");
		return RET_OK;
	}	
	WriteLog("进入处理终端设备状态函数UnMakeDeviceBuffer()收到合法数据,服务通\n");
	//LPORT服务器	
	if( !memcmp(pDocEx->pChildDoc[pDocEx->nPortNo]->m_szMacCode, (char*)"5301",4))
	{
		NPort=(int)pBuffer->pBuffer[6];
		if( NPort >24 ) NPort = 24;
		memcpy(ucRawData,pBuffer->pBuffer+7,NPort);		
		for( int i=0; i<NPort; i++)
		{
			...
			else if( (int)ucRawData[i] == (int)0xFF )
			{
				sTemp[i]='F';    //未接终端
			}
			//add by dj 20080620
			else if( (int)ucRawData[i] == 30 ) //关门(计时宝)
			{
				sTemp[i]='5'; 
			}
			else if( (int)ucRawData[i] == 31 ) //开门(计时宝)
			{
				sTemp[i]='6'; 	
			}
			else if( (int)ucRawData[i] == 32 ) //门锁无电(计时宝)
			{
				sTemp[i]='7'; 
			}
			else 
		...
		memcpy(CardResult.sVersion,sTemp,NPort);
		CardResult.nCardID = 0;			//服务器通
		CardResult.nFlag   = NPort;		//端口总数
	}
	else //以太网计时宝
	{
		//add by dj 20080620
		if(      pBuffer->pBuffer[2]==30 )
		{
			CardResult.nCardID = 5;		//门关
		}
		else if( pBuffer->pBuffer[2]==31 )
		{
			CardResult.nCardID = 6;		//门开
		}
		else if( pBuffer->pBuffer[2]==32 )
		{
			CardResult.nCardID = 7;		//无电
		}
		else
		{
			CardResult.nCardID = 0;		//通
		}		
		CardResult.nFlag   = 0;	
		memset(CardResult.sVersion,0x00,sizeof CardResult.sVersion );
	}
	OutputBlackCard(pDocEx->pChildDoc[pDocEx->nPortNo], RET_OK, &CardResult);			
	return	RET_OK;	
}
...
MachineType COperator::GetMachineType(TSSmartDoc *pSmartDoc)
{
	...
	{
		if( !strncmp(pSmartDoc->m_szMacCode, MacTypeArray[i].pCode,8) )
