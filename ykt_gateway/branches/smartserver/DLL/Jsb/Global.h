...
//int _stdcall MakeBuf_DownWhiteCardNo(TSDeviceInfo *pDevice, TSCmdBuffer *pBuffer,unsigned char *CardNo);/// 功能：下传白名单(注意：一次只能下传一条)/// 参数： CardNo----3字节
...
int _stdcall MakeBuf_SetMode4Holiday(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer,int No,unsigned char *Buffer);/// 功能：设置模式三节假日信息/// 参数：///    No:节假日总条数///    Buffer:节假日信息
int _stdcall MakeBuf_SetMode4TimePhaseInfo(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer,unsigned char *Buffer);/// 功能：设置模式三时间段信息/// 参数:///    WeekInfo:星期信息 1Byte///    No:时间段数       1Byte///    Buffer:时间段信息 No*8 Byte
