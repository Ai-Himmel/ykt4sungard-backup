...
static TSResultMsg  ResultMsg[] = 
{
	 ...
	 RET_NG                 ,"-1000:失败/错误", 
	 RET_TIMEOUT            ,"-1001:超时", 
	 RET_TERMINATE          ,"-1002:中断操作", 
	 RET_SYSERROR           ,"-2000:系统错误", 
	 RET_DBBUSY             ,"-3000:数据库资源分配完", 
	 RET_DBQUERYERROR       ,"-3001:数据库查询错误", 
	 RET_DBEXECERROR        ,"-3002:数据库DDL错误", 
	 RET_NOTSUPPORT         ,"-3003:不支持的硬件类型", 
	 RET_VALIDPACKET        ,"-3004:非法的数据包", 
	 RET_VALIDMARK          ,"-3005:MARK值错", 
	 RET_VALIDMACNO         ,"-3006:机号错", 
	 RET_TASKWHERE_ERROR	,"-3007:设备条件不满足", 
};
...
unsigned int __stdcall TThread::ThreadProc(LPVOID lpvoid)
{
	...
	sprintf(szText,"线程ID=%ld",(DWORD)pThread->m_hThread);
