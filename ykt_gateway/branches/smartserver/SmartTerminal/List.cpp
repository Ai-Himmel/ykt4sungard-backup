...
#define CMD_SETASSISTLIST			0x0C    //设置补助名单  //update dengjun
...
/*
	unsigned char dsp[128];
	memset(dsp, 0x00, sizeof dsp );
	SVC_HEX_2_DSP(Buffer,dsp,i);
	printf("%s\n",dsp);
*/
	return i;
}
...
int _stdcall MakeBuf_GetAssisList(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer)
{
	...
	len           = 3;
	//pBuffer->nCmdSize = PubCommands(addr,ucCommandByte,len,pBuffer->pBuffer,pDevice->nLevel,pDevice->sLevelArray);
	pBuffer->nCmdSize = PubCommands(addr,ucCommandByte,len,pBuffer->pBuffer,0,NULL);
	pBuffer->nTimeOut1 = 50;
	pBuffer->nTimeOut1 = 1000;
	return RET_OK;
}
...
int _stdcall MakeBuf_SetIncFeeRate(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer, unsigned char cCard, unsigned char nRate)
{
	...
	printf(".............................%02x%02x  %02x%02x\n",pBuffer->pBuffer[4],pBuffer->pBuffer[5],cCard,nRate);
	pBuffer->nCmdSize = PubCommands(addr,ucCommandByte,len,pBuffer->pBuffer,pDevice->nLevel,pDevice->sLevelArray);
	pBuffer->nTimeOut1 = 50;
	pBuffer->nTimeOut1 = 1000;
...
int _stdcall MakeBuf_GetIncFeeRate(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer, unsigned char cCard)
{
	unsigned char ucCommandByte,ucRegNo[4];
	int           addr,len;
	ucCommandByte = CMD_GETINCFEERATE;
	addr          = pDevice->nAddr;
	len           = 5;
	SVC_DSP_2_HEX((unsigned char *)pDevice->sRegNo,ucRegNo,4);
	memcpy(pBuffer->pBuffer,ucRegNo,4);
...
/// Ver: 1.0 ------- 20080306
/// Auth:Lina
int _stdcall MakeBuf_SetSpecialDate(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer)
{
	unsigned char ucCommandByte;
	int           addr;
	ucCommandByte = 0x93;
	addr          = pDevice->nAddr;	
	pBuffer->nCmdSize = PubCommands(addr,ucCommandByte,pBuffer->nBufferSize,pBuffer->pBuffer,pDevice->nLevel,pDevice->sLevelArray);
	pBuffer->nTimeOut1 = 2000;
	pBuffer->nTimeOut1 = 1000;
	return RET_OK;
}

/// 功能：2 设定寒假和暑假的开始截止日期及周末规则
/// 参数：
/// Ver: 1.0 ------- 20080306
/// Auth:Lina
int _stdcall MakeBuf_SetHolidayDate(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer)
{
	unsigned char ucCommandByte;
	int           addr;
	ucCommandByte = 0x95;
...
/// Ver: 1.0 ------- 20080306
/// Auth:Lina
int _stdcall MakeBuf_SetGongJiaDate(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer)
{
	unsigned char ucCommandByte;
	int           addr;
	ucCommandByte = 0x97;
...
/// Ver: 1.0 ------- 20080306
/// Auth:Lina
int _stdcall MakeBuf_SetHolidayTime(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer)
{
	unsigned char ucCommandByte;
	int           addr;
	ucCommandByte = 0x99;
...
/// Ver: 1.0 ------- 20080306
/// Auth:Lina
int _stdcall MakeBuf_SetHolidayClass(TSDeviceInfo *pDevice,TSCmdBuffer *pBuffer)
{
	unsigned char ucCommandByte;
	int           addr;
	ucCommandByte = 0x9B;
