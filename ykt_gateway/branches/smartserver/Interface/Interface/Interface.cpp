		...
		if( SendData(hHandle, iServerNo, iFunc, (char*)buffer, nLen, 1, FALSE) == RET_OK ) 
		{
			...
			if( nlen > 0 )
			{
				...
				if( DecodeBuf((unsigned char*)buffer,nlen,&apack,omsg) )
				{
					...
					{
						char str[16]="";
						sprintf(str,"%d",apack.pack.lcert_code);
						WritePrivateProfileString("REGISTER", "REGISTERNO", str, szINIFile);
...
bool CInterfaceApp::MakeFirstTaskPlan(TSSmartTaskPlan *pTaskPlan, ST_CPACK *pPacket, ST_PACK *pArray, int nRow)
{
	...
	if( pPacket->pack.lvol2 == 930057 )	
	{
		...
		for(int p=0; p< gnDocCount; p++)
		{
			...
			Sleep(1); //为该函数:GetTickCount()总共可以取得到不同的值;
		}		
		if( pTaskPlan->nTask > 0 )	iCount = 1 ;
		WriteLog("心跳监控任务下载完成:总共[ %ld]任务 iCount=%ld\n",pTaskPlan->nTask,iCount);
	}//定时初始化LPORT为出厂状态
	else if( pPacket->pack.lvol2 == 999999 )	
	{
		pTaskPlan->nTaskPlanID	= GetTickCount()-5;
		sprintf(pTaskPlan->szTaskName,"功能号:%d", pPacket->head.RequestType); //任务计划名称
		pTaskPlan->nTaskCycle	=0;                //任务计划执行周期
		...
		{			
			//找出LPORT数据服务器
			if( !memcmp(gpSmartDoc[p].m_szMacCode,"5301",4) )
			{
				pTaskPlan->pTask[pTaskPlan->nTask].nTaskID	   = GetTickCount();
				pTaskPlan->pTask[pTaskPlan->nTask].nTaskPlanID = pTaskPlan->nTaskPlanID; 
				pTaskPlan->pTask[pTaskPlan->nTask].nAuthID     = gpSmartDoc[p].m_nAuthID ;
				strcpy(pTaskPlan->pTask[pTaskPlan->nTask].szDeviceID, gpSmartDoc[p].m_szDeviceID);
				strcpy(pTaskPlan->pTask[pTaskPlan->nTask].szTaskCode,"999");
				pTaskPlan->pTask[pTaskPlan->nTask].nPriority	= 6;					
				pTaskPlan->pTask[pTaskPlan->nTask].nFunc=999999;
				pTaskPlan->nTask++;				
				Sleep(1); 
				//WriteLog("装载LPORT初始化任务:设备ID[ %s ],机型[%s]\n",gpSmartDoc[p].m_szDeviceID,gpSmartDoc[p].m_szMacCode);
			}
		}		
		if( pTaskPlan->nTask > 0 )	iCount = 1 ;
		WriteLog("装载LPORT初始化任务总数[%ld]\n",pTaskPlan->nTask);
	}//终端状态
	...
	else if( pPacket->pack.lvol2 == 930005 )
	{
		...
		{	//changed by hhd at 20061130 增加判断是否为计时宝设备"0121"，否则不下发广播任务
			if((!gpSmartDoc[i].m_nParentID)&&(strcmp(gpSmartDoc[i].m_szMacCode,"0121")))
			{
...
//修改GETVALUE的函数原型，直接返回
//Changed by hhd at 2007-12-29
...
*/
//为了解决GetValue的多线程不安全赋值问题，
//重载SETVALUE函数，支持整数参数传入，内部转换
//add by hhd at 2007-12-29
void SetValue(ST_PACKHEAD *head, ST_PACK *pack,int ids,long data)
	{
	long lv;
//	double dv;
#define PC_HHD(a)	{}//CHAR[]
#define PI_HHD(a)	{ SetParmBit(head,ids);lv=data;memcpy(&pack->a,&lv,sizeof(pack->a));}//INT
#define PD_HHD(a)	{}//LONG
	...
	case F_SHOLDER_AC_NO:                
		PC_HHD(sholder_ac_no);           
		break;                       
	case F_SHOLDER_AC_NO2:               
		PC_HHD(sholder_ac_no2);          
		break;                       
	case F_SHOLDER_TYPE:                 
		PC_HHD(sholder_type);            
		break;                       
	case F_SHOLDER_TYPE2:                
		PC_HHD(sholder_type2);           
		break;                       
	case F_SNAME:                        
		PC_HHD(sname);                   
		break;                       
	...
	case F_SWITHDRAW_PWD:                
		PC_HHD(swithdraw_pwd);           
		break;                       
	case F_SWITHDRAW_PWD2:               
		PC_HHD(swithdraw_pwd2);          
		break;                       
	case F_SEMP_PWD:                     
		PC_HHD(semp_pwd);                
		break;                       
	...
	case F_SCURRENCY_TYPE:               
		PC_HHD(scurrency_type);          
		break;                       
	case F_SCURRENCY_TYPE2:              
		PC_HHD(scurrency_type2);         
		break;                       
	case F_SBRANCH_CODE0:                
		PC_HHD(sbranch_code0);           
		break;                       
	case F_SBRANCH_CODE1:                
		PC_HHD(sbranch_code1);           
		break;                       
	case F_SBRANCH_CODE2:                
		PC_HHD(sbranch_code2);           
		break;                       
	case F_USSET0:                       
		PC_HHD(usset0);                  
		break;                       
	...
	case F_LBANK_ACC_TYPE:               
		PI_HHD(lbank_acc_type);          
		break;                       
	case F_LBANK_ACC_TYPE2:              
		PI_HHD(lbank_acc_type2);         
		break;                       
	case F_SMAIN_FLAG:                   
		PC_HHD(smain_flag);              
		break;                       
...
//			delete pHead;
	...
	case 930004:	//删除黑名单
	case 930005:	//广播黑名单
		memset(pTaskPlan->pTask[i].szMemo,0x00,sizeof pTaskPlan->pTask[i].szMemo);
		pTaskPlan->pTask[i].nTaskID = GetTickCount();
		pTaskPlan->pTask[i].nTaskPlanID=pTaskPlan->nTaskPlanID; 
		pTaskPlan->pTask[i].nAuthID = nAuthID;                  
		strcpy(pTaskPlan->pTask[i].szDeviceID, pDeviceID);      
		...
		else if(nRequest == 930004 )
		{
			...
			//sprintf(Errmsg,"[任务]***********删除黑名单,终端ID[%s],卡号[%ld],版本[%s],TaskId=%ld.............",pDeviceID,pPacket->lvol0,pPacket->sserial0,nRequest);
			//cTool.AddLineToFile(pDeviceID,Errmsg,strlen(Errmsg));
			//printf(Errmsg);
			//printf("\n");			
		}		
	...
	case 930210: //特殊普通日子
		{
			...
			pTaskPlan->pTask[i].nAuthID = nAuthID;                  
			strcpy(pTaskPlan->pTask[i].szDeviceID, pDeviceID);      
			strcpy(pTaskPlan->pTask[i].szTaskCode,"201");           
			pTaskPlan->pTask[i].nPriority = 5;
			strcpy(pTaskPlan->pTask[i].sData,pPacket->vsvarstr0);
			pTaskPlan->nTask++;
			WriteLog("下传特殊普通日子(%s)",pTaskPlan->pTask[i].sData);
		}
		break;
	case 930211: //设置寒暑假及周末规则(930211)
		{
			pTaskPlan->pTask[i].nTaskID = GetTickCount();
			pTaskPlan->pTask[i].nTaskPlanID=pTaskPlan->nTaskPlanID; 
			pTaskPlan->pTask[i].nAuthID = nAuthID;                  
			strcpy(pTaskPlan->pTask[i].szDeviceID, pDeviceID);      
			strcpy(pTaskPlan->pTask[i].szTaskCode,"202");           
			pTaskPlan->pTask[i].nPriority = 5;
			strcpy(pTaskPlan->pTask[i].sData,pPacket->vsvarstr0);			
			pTaskPlan->nTask++;
			WriteLog("设置寒暑假及周末规则(%s)",pTaskPlan->pTask[i].sData);
		}
		break;
	case 930212: //设置法定节假日(930212)
		{
			pTaskPlan->pTask[i].nTaskID = GetTickCount();
			pTaskPlan->pTask[i].nTaskPlanID=pTaskPlan->nTaskPlanID; 
			pTaskPlan->pTask[i].nAuthID = nAuthID;                  
			strcpy(pTaskPlan->pTask[i].szDeviceID, pDeviceID);      
			strcpy(pTaskPlan->pTask[i].szTaskCode,"203");           
			pTaskPlan->pTask[i].nPriority = 5;
			strcpy(pTaskPlan->pTask[i].sData,pPacket->vsvarstr0);			
			pTaskPlan->nTask++;
			WriteLog("设置法定节假日(%s)",pTaskPlan->pTask[i].sData);
		}
		break;
	case 930213: //4.设置节假日时间段参数(930213)
		{
			pTaskPlan->pTask[i].nTaskID = GetTickCount();
			pTaskPlan->pTask[i].nTaskPlanID=pTaskPlan->nTaskPlanID; 
			pTaskPlan->pTask[i].nAuthID = nAuthID;                  
			strcpy(pTaskPlan->pTask[i].szDeviceID, pDeviceID);      
			strcpy(pTaskPlan->pTask[i].szTaskCode,"204");           
			pTaskPlan->pTask[i].nPriority = 5;
			strcpy(pTaskPlan->pTask[i].sData,pPacket->vsvarstr0);			
			pTaskPlan->nTask++;
			WriteLog("设置节假日时间段参数(%s)",pTaskPlan->pTask[i].sData);
		}
		break;
	case 930214: //下传节假日卡权限类型(930214)
		{
			pTaskPlan->pTask[i].nTaskID = GetTickCount();
			pTaskPlan->pTask[i].nTaskPlanID=pTaskPlan->nTaskPlanID; 
			pTaskPlan->pTask[i].nAuthID = nAuthID;                  
			strcpy(pTaskPlan->pTask[i].szDeviceID, pDeviceID);      
			strcpy(pTaskPlan->pTask[i].szTaskCode,"205");           
			pTaskPlan->pTask[i].nPriority = 5;
			strcpy(pTaskPlan->pTask[i].sData,pPacket->vsvarstr0);			
			pTaskPlan->nTask++;
			WriteLog("下传节假日卡权限类型(%s)",pTaskPlan->pTask[i].sData);
		}
		break;
	default:
...
//sprintf(msg+strlen(msg),"[返回]###########删除黑名单-->返回给金仕达:状态:%ld",iResult);
			SetValue(&rpack.head,&rpack.pack, F_SDATE0, pTask->szDeviceID);
			memset(buf, 0x00, sizeof buf );
			memcpy(buf,pTask->szMemo,8);
//sprintf(msg+strlen(msg),"卡号:%u",atol(buf));
			SetValue(&rpack.head,&rpack.pack, F_LVOL0, GetValue(atol(buf)));
			memset(buf, 0x00, sizeof buf );
			memcpy(buf,pTask->szMemo+8,12);
//sprintf(msg+strlen(msg),"版本:%s",buf);
//sprintf(msg+strlen(msg),"任务号:%ld",pTask->nFunc);
//cTool.AddLineToFile(pTask->szDeviceID,msg,strlen(msg));
//memset(msg,0x00,sizeof msg);
//strcpy(msg,"                        ");
//cTool.AddLineToFile(pTask->szDeviceID,msg,strlen(msg));
			SetValue(&rpack.head,&rpack.pack, F_SSERIAL0, buf);			
	...
	case 930105:
		...
		WriteLog("设备:%s, 返回包(930105),下传计时宝时段.....返回码:%d, 消息:%s..\n",pTask->szDeviceID,  iResult, pData->sMsg);
		break;
	case 930020://增加补助名单
		SetValue(&rpack.head,&rpack.pack, F_SDATE0, pTask->szDeviceID);		
	...
	case 930210: //特殊普通日子
		SetValue(&rpack.head,&rpack.pack, F_SDATE0, pTask->szDeviceID);		
		WriteLog("设备:%s, 返回包(930210),特殊普通日子.....返回码:%d, 消息:%s..\n",pTask->szDeviceID,  iResult, pData->sMsg);
		break;
	case 930211: //设置寒暑假及周末规则(930211)
		SetValue(&rpack.head,&rpack.pack, F_SDATE0, pTask->szDeviceID);		
		WriteLog("设备:%s, 返回包(930211),设置寒暑假及周末规则.....返回码:%d, 消息:%s..\n",pTask->szDeviceID,  iResult, pData->sMsg);
		break;
	case 930212: //设置法定节假日(930212)
		SetValue(&rpack.head,&rpack.pack, F_SDATE0, pTask->szDeviceID);		
		WriteLog("设备:%s, 返回包(930212),设置法定节假日.....返回码:%d, 消息:%s..\n",pTask->szDeviceID,  iResult, pData->sMsg);
		break;
	case 930213: //4.设置节假日时间段参数(930213)
		SetValue(&rpack.head,&rpack.pack, F_SDATE0, pTask->szDeviceID);		
		WriteLog("设备:%s, 返回包(930213),设置节假日时间段参数.....返回码:%d, 消息:%s..\n",pTask->szDeviceID,  iResult, pData->sMsg);
		break;
	case 930214: //下传节假日卡权限类型(930214)
		SetValue(&rpack.head,&rpack.pack, F_SDATE0, pTask->szDeviceID);		
		WriteLog("设备:%s, 返回包(930214),下传节假日卡权限类型.....返回码:%d, 消息:%s..\n",pTask->szDeviceID,  iResult, pData->sMsg);
		break;
	default:
		break;
	}
...
long CInterfaceApp::OutputResultBlackCard(TSSmartDoc *pDoc, long nFlag, TSBlackCard *pCard)
{
	...
	{
		WriteLog("开始向金仕达发送心跳监控数据包......[%s]\n",pCard->sValid);
		nRet = SendDeviceTick(pDoc);
		//WriteLog("已经向金仕达发送心跳监控数据包完成[%s]\n",pCard->sValid);
	}
