		...
		else
		{
			...
			try
			{
				...
				case ptReal:
					...
					break;
				case ptPreempt:
					ACE_DEBUG((LM_TRACE,"抢占式任务！taskid[%d]",taskId));
					res = queue->push_preemptive_task(ret);
					break;
				default:
				case ptNormal:
					res = queue->push_task(ret);
					ACE_DEBUG((LM_TRACE,"队列任务数[%d]",queue->count_of_tasks()));
					break;
				}
				if(res)
				{
...
void KSGTaskManager::free_task_id(int id)
{
	...
	if(i != _tasks.end())
	{
		//ACE_DEBUG((LM_DEBUG,"释放指令对象id[%d]",id));
...
void TaskExecScheduler::Run()
{
	...
	std::string retmsg;
	{
		...
		if(task)
		{
			...
			if(node == NULL)
			{
				ACE_DEBUG((LM_TRACE,"指令[%d]没有执行设备",task->GetTaskId()));
				return;
			}
			...
			try
			{
				...
				ACE_DEBUG((LM_TRACE,"执行指令[%d]dev[%d]ret[%d]",
			...
			{
				// 连接设备失败
				ACE_DEBUG((LM_TRACE,"无法连接到设备，该线程等待5秒后重试"));
				retmsg = "连接设备失败";
			...
			if(task->NeedResponse())
			{
				...
				if(obj->Connect() || obj->SendRequest(5000))
					ACE_DEBUG((LM_ERROR,"指令应答失败...taskid[%d]",task->GetTaskId()));
			}
		}
		else
			notask = true;
		ACE_DEBUG((LM_TRACE,"指令队列执行指令结束"));
	}
	if(notask)
	{
		// 没有指令执行要进入 Idle 状态
