...
CDRTPHelper::CDRTPHelper(const char* ip,int port,int funcno,int drtp_no)
:m_serverPort(port),m_funcNo(funcno),m_drtpHandle(0),m_drtpNo(drtp_no)
,m_rec_cnt(0)
{
...
int CDRTPHelper::Connect(void)
{
	...
	/*
	if(!DrtpAttachServer(m_drtpNo,m_funcNo,m_drtpHandle,&m_tag,&m_errcode,m_errmsg))
	{
		try
		{
			testDrtpHandle();
	...
	ACE_DEBUG((LM_TRACE,"连接DRTP成功，返回Handle[%d]",m_drtpHandle));
	return 0;
}
...
{
	SetValue(&(m_requestPack.head),&(m_requestPack.pack),field,value,len);
}
...
int CDRTPHelper::testDrtpHandle()
{
	if(m_drtpHandle > 0)
	{
		ACE_DEBUG((LM_ERROR,"服务器返回错误码[%d][%s][%08X]"
			,m_errcode,m_errmsg,m_drtpHandle));
		switch(m_errcode)
		{
...
{
	std::string retmsg(m_responsePack.pack.vsmess);
	return retmsg;
}
int CDRTPHelper::GetReturnCode()
{
	return m_responsePack.head.retCode;
}
int CDRTPHelper::GetErrCode()
{
	return m_errcode;
}
bool CDRTPHelper::HasMoreRecord()
{
	...
	{
		// 有后继包
		if(m_responsePack.head.nextflag)
		{
			// 读取后继包
			ACE_DEBUG((LM_TRACE,"取后继数据包"));
			SetNextRequest();
			if(SendRequest(m_timeout))
			{
				m_errcode = 100;// 读取后续包超时
				return false;
			}
		}
		else
	...
	// 如果没有记录
	if(m_currRecordCount == 0 || m_responsePack.head.retCode != 0)
		return false;
	//
	if(m_currIndex == 0)
	{
		// 第一条记录在 m_responsePack.pack 中
		m_currRecord = &(m_responsePack.pack);
	}
	else
	{
...
void CDRTPHelper::SetNextRequest()
{
	...
	memcpy(&m_requestPack.head.hook,&m_responsePack.head.hook
		,sizeof(m_requestPack.head.hook));
}
