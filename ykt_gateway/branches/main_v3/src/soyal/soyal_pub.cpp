...
int SOYALDEV::SetCardData(ACE_HANDLE handler,int ctrlid,DownCard_info info,int cardtype)
{
	...
	SHORT_2_BUF_BE(info.ipasswd,tmp);						// 密码	（只支持4位）
	memcpy(mydatapack.data+3,tmp,2);

	if(cardtype == 0)						// ct_mafiar_one
	{
	...
	mydatapack.data[9] = 0xC5;					// mode,读卡加密码才能开门
	...
	mydatapack.data[15] = 0x02;						// level,与非上班时段level相同，即非上班时段刷卡加密码进门
	ret = SendRev(handler,&mydatapack,datalen,recvbuf,recvlen);
	if(ret)
		return ret;
	if(ctrlid != recvbuf[2])			// 校验控制器号
...
int SOYALDEV::SetTimeZone(ACE_HANDLE handler,int ctrlid,int zoneidx,int zonelink,int zonelevel,TimeGroup timegroup)
{
	int ret = 0;
	byte recvbuf[256]="";
	size_t recvlen =0;
	...
	mydatapack.data[0]= zoneidx + 1;		//Set time zone  63 time zone 01h～3Fh
	...
	mydatapack.data[3]= zonelevel;				//level
...
int SOYALDEV:: CtrlReader(ACE_HANDLE handler,int ctrlid,int reader,int flag)
{
	...
	if(reader != recvbuf[2])
	{
		printf("CtrlReader readerid not match,reader[%d],recvreader[%d]",reader,recvbuf[2]);
		return -1;
	}
	return 0;
}
