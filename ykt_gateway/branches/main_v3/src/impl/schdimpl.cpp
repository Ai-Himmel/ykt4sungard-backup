...
KSG_ADD_SCHD_CLASS(1007,KSGTaskDoorBatchDlCard);
/**
 * 任务队列的策略需要进行修改
 */
	...
	while(!IsTerminated())
	{
		try
		{
			...
			while(obj->HasMoreRecord())
			{
				...
				ACE_DEBUG((LM_TRACE,"下载指令，ID[%d]func[%d]",taskid,funcid));
				if(devid == 0 || devid == 1)
				{
			...
			{
				// 如果没有指令可以接收,等待5000ms
				ACE_DEBUG((LM_TRACE,"没有指令接收进入等待状态"));
				KSG_SLEEP(5000);
			}
			// 通知所有的 observer
		}
...
void KSGTaskCollHeartBeat::Run()
{
	while(!IsTerminated())
	{
		...
		{
			// 
			ACE_DEBUG((LM_TRACE,"采集心跳任务异常[%s]",ex.what()));
		}
	}
}
...
void KSGTaskCollHeartBeat::CollHeartBeatVisitor::Visit(KSGDeviceNode &node)
{
	...
	TaskManager::PtrType task = KsgGetGateway()->GetTaskManager()->AddSysTask(TK_HEARTBEAT_TASK
...
void KSGTaskCollectSerial::Run()
{
	...
	while(!IsTerminated())
	{
		...
		{
			// 
			ACE_DEBUG((LM_TRACE,"采集流水任务异常[%s]",ex.what()));
		}
	}
}
...
void KSGTaskCollectSerial::CollectSerialVisitor::Visit(KSGDeviceNode& node)
{
	...
	try
	{
		...
		{
			ACE_DEBUG((LM_TRACE,"下达采集设备[%d] 流水的任务!",node.GetDevId()));
		}
	}catch(KSGException& )
	{
		// 生成任务失败
...
void KSGTaskCollDeviceState::Run()
{
	while(!IsTerminated())
	{
		try
		{
			...
			ACE_DEBUG((LM_TRACE,"采集一次心跳。。。"));
			//KSG_SLEEP(KsgGetGateway()->GetConfig()->_heartbeat*KsgGetGateway()->GetConfig()->_heartbeatIntval);
		...
		catch(std::exception& ex)
		{
			// 
			KSG_DUMP_EXCEPTION(ex);
			ACE_DEBUG((LM_TRACE,"采集设备状态任务异常[%s]",ex.what()));
			KSG_SLEEP(KsgGetGateway()->GetConfig()->_heartbeat);
		}
	}
...
void KSGTaskCollDeviceState::CollDeviceState::Visit(KSGDeviceNode& node)
{
	...
	if(node.GetDeviceType() != KSG_SERVER_DEV)
	{
		/*
		if(now_time.sec() >= (KsgGetGateway()->GetConfig()->_heartbeat / 1000 * KsgGetGateway()->GetConfig()->_heartbeatIntval)/2)
		{
			ACE_DEBUG((LM_INFO,"设备[%s]下达回响测试任务",node.GetPhyId().c_str()));
			std::stringstream buf;
			buf<<"<devphyid>"<<node.GetPhyId()<<"</devphyid>";
			try
			{
	...
	if(now_time.sec() > KsgGetGateway()->GetConfig()->_heartbeat / 1000 * KsgGetGateway()->GetConfig()->_heartbeatIntval)
		state = 1;
...
int KSGTaskCollDeviceState::CollDeviceState::send_state()
{
	...
	ACE_OS::strncpy(now_str+2,now_time.c_str(),sizeof(now_str)-3);
...
void KSGTaskDownloadBlackCard::DownloadBlackCardVisitor::Visit(KSGDeviceNode& node)
{
	...
	*/
	...
	int blk_card_cnt = 50;
	std::string v;
	if(node.get_param(DEV_BLKCOUNT,v)==0)
	{
		blk_card_cnt = atoi(v.c_str());
		if(blk_card_cnt == 0)
			blk_card_cnt = 5;
	}
	
	static const int card_len = 10;
	//ACE_DEBUG((LM_DEBUG,"blklist-下载设备黑名单,dev,id[%d]",node.GetDevId()));
	try
	{
		DRTPPoolType::SmartObject obj = KsgGetDrtpPool()->Alloc();
		if(!obj)
		...
		if(obj->SendRequest(5000))
		{
			...
			ACE_DEBUG((LM_TRACE,"blklist-接收指令超时等待结束!"));
			return;
		}
		if(obj->GetReturnCode())
		{
			ACE_DEBUG((LM_ERROR,"blklist-下载黑名单失败，返回[%d][%s]",
				obj->GetReturnCode(),obj->GetReturnMsg().c_str()));
			return;
		}
		count = 0;
	...
	{
		KSG_DUMP_EXCEPTION(ex);
		ACE_DEBUG((LM_ERROR,"blklist-下载黑名单线程处理异常!"));
		return;
	}
	/*
	catch(...)
	...
	*/
	...
	if(count == 0)
	{
		ACE_DEBUG((LM_INFO,"blklist-设备[%d][%s]黑名单版本已经是最新的版本了"
	...
	try
	{
		...
		TaskManager::PtrType task = KsgGetGateway()->GetTaskManager()->AddSysTask(TK_BATCH_DL_BLKCARD
			,buf.str(),&node,ptNormal);
		if(task->GetTaskId() !=0 )
			ACE_DEBUG((LM_TRACE,"blklist-下传设备黑名单,dev[%s]maxvol[%s]count[%d]"
				,node.GetPhyId().c_str(),max_vol,count));
		else
			ACE_DEBUG((LM_ERROR,"blklist-生成指令失败！"));
	}
	catch(TaskAlreadyExistsException& )
	{
		// 指令已存在,可能还没有执行完成
	...
	ACE_DEBUG((LM_DEBUG,"blklist-下载设备黑名单完成,dev,id[%d],count[%d]"
		,node.GetDevId(),count));
	return;
}
void KSGTaskDownloadBlackCard::Run()
{
	while(!IsTerminated())
	{
		...
		try
		{
			...
			Notify();
			ACE_DEBUG((LM_DEBUG,"blklist-下载黑名单完成...sleep[%d]",KsgGetGateway()->GetConfig()->_serialIntval));
			// 采集一次流水
			//KsgGetGateway()->GetConfig()->_serialIntval * 50;
			KSG_SLEEP(KsgGetGateway()->GetConfig()->_heartbeat);
		}
...
void KSGTaskDoorBatchDlCard::Run()
{
	while(!IsTerminated())
	{
		...
		catch(std::exception& ex)
		{
			// 
			KSG_DUMP_EXCEPTION(ex);
			ACE_DEBUG((LM_TRACE,"下载门禁单任务异常[%s]",ex.what()));
			KSG_SLEEP(KsgGetGateway()->GetConfig()->_heartbeat);
		}
	}
	ACE_DEBUG((LM_NOTICE,"下载任务线程退出"));
...
void KSGTaskDoorBatchDlCard::BatchDownloadCardVisitor::Visit(KSGDeviceNode& node)
{
	if(node.GetState() == KSGDeviceNode::dsError)
		return;
	if(!node.Accept(*this))
	...
	try
	{
		TaskManager::PtrType task = KsgGetGateway()->GetTaskManager()->AddSysTask(930118
			,"",&node,ptNormal);
	}
	catch(TaskAlreadyExistsException& )
	{
		// 指令已存在,可能还没有执行完成
		ACE_DEBUG((LM_TRACE,"下载黑名单任务已经在队列中等待执行了!dev[%d]",node.GetDevId()));
	}
	catch(KSGException&)
	{
		ACE_DEBUG((LM_ERROR,"生成任务失败"));
