	...
	for(i = 1;i <= door_idx; ++i)
	{
		...
		{		
			...
			else
			{
				...
				ret = hsGCUDeleteUserRecord(hd,task_node->GetTermId(),0,0
...
int ID_HNDR_GCU_DL_Card::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	{
		//ACE_DEBUG((LM_DEBUG,"xxxx删除卡[%s]",phyno_dec.c_str()));
		ret = hsGCUDeleteUserRecord(hd,task_parent->GetTermId(),0,0
												 ...
												 ,stPollList_GCU *poll_list,int recv_record)
{
	...
	for(success = 0; success < recv_record;++success)
	{
		...
		ACE_DEBUG((LM_INFO,"GCU 采集门禁事件:设备[%d: %d],日期时间[%s],卡号dec[%s],hex[%s]event[%s]"
			,node->GetDevId(),device_id,date_str,poll_list[success].cCard,hex_str.c_str(),poll_list[success].cStatusCode));
...
int ID_HNDR_GCU_Batch_DL_Card2::success_download(int list_id,int device_id)
{
	DRTPPoolType::SmartObject drtp = KsgGetDrtpPool()->Alloc();
	if(!drtp)
		return TASK_ERR_EXECUTE;
...
int ID_HNDR_GCU_Batch_DL_Card2::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	static const int max_cardlist_count = 500;
	struct_CardFormat this_card[max_cardlist_count];
	ACE_OS::memset(this_card,0,sizeof(struct_CardFormat) * max_cardlist_count);
...

	{
		...
		drtp->SetRequestHeader(BATCH_DOWNLOAD_CARDLIST);
		drtp->AddField(F_LVOL0,task_node->GetDevId());
		ACE_DEBUG((LM_INFO,"GCU device_id[%d]",task_node->GetDevId()));
		...
		while(drtp->HasMoreRecord() && door_idx < max_cardlist_count)
		{
			...
			ACE_DEBUG((LM_INFO,"GCU下载卡名单list_id[%d],doorcount[%d],phyno[%s]",list_id,doorcount,phyno.c_str()));
			...
			if(strcmp(phyno_dec.c_str(),this_card[door_idx].cCardNo) !=0)
			{
				...
				ACE_OS::strcpy(this_card[door_idx].cCardNo,phyno_dec.c_str());
				this_card[door_idx].iCardLen = ACE_OS::strlen(this_card[door_idx].cCardNo);
				this_card[door_idx].iDisplayID = data->lvol1; // 借用保存客户号

			}
			
			if(doorcount)		// 最多支持16个门，从0开始
			{
				for(i=0;i<16;i++)
				{
					unsigned char b;
					...
					if(t != 0xFF)
					{
						this_card[door_idx].cTimeZone[i] = static_cast<char>(t+1);
						this_card[door_idx].iDoors++;
					}					
				}				
			}
	...
	ACE_DEBUG((LM_INFO,"door_idx [%d]",door_idx));
	for(i = 1;i <= door_idx; ++i)
	{
		retries = 3;
		custid = this_card[i].iDisplayID;
		...
		{		
			...
			if(this_card[i].iDoors > 0)
			{
				...
				ACE_DEBUG((LM_INFO,"下载GCU名单GCU[%d:%d],phyno_convert[%s],phyno[%s],week[%s],ret[%d]"
					,task_node->GetDevId(),task_node->GetTermId(),this_card[i].cCardNo,phyno.c_str(),temp,ret));
			}
			else
			{
				//ACE_DEBUG((LM_DEBUG,"xxxx删除卡[%s]",phyno_dec.c_str()));
				ret = hsGCUDeleteUserRecord(hd,task_node->GetTermId(),0,0
					,this_card[i].cCardNo,&gcu_errcode,g_gcu_default_timeout);
				ACE_DEBUG((LM_INFO,"删除GCU名单GCU[%d:%d]phyno_convert[%s],phyno[%s],ret[%d]"
					,task_node->GetDevId(),task_node->GetTermId(),this_card[i].cCardNo,phyno.c_str(),ret));
				// 卡号不存在
				if(gcu_errcode == 0x06)
					ret = 0;
			}
		...
		if(success_download(list_id,task_node->GetDevId()))
		{
			ACE_DEBUG((LM_ERROR,"下载名单成功，应答系统失败，dev[%d],custid[%d]",task_node->GetDevId(),custid));
			return TASK_ERR_EXECUTE;
		}
