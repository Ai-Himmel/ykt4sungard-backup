...
{
	ACE_DEBUG((LM_INFO,"event_date len [%d],event_date[%s]",strlen(event_date),event_date));
	int len = strlen(event_date);
	int index = 0;
	for (int i = 0;i<len && index<maxlen;i++)
	{
		if(event_date[i]>='0' && event_date[i]<='9')
			date_str[index++] = event_date[i];
	}
	/*
	struct tm date_tm;
	ACE_DEBUG((LM_INFO,"event_date len [%d],event_date[%s]",strlen(event_date),event_date));
	...
	catch (...)
	{
		...
		memcpy(date_str,event_date,4);
		memcpy(date_str+4,event_date+5,2);
		memcpy(date_str+6,event_date+8,2);
		memcpy(date_str+8,event_date+11,2);
		memcpy(date_str+10,event_date+14,2);
		memcpy(date_str+12,event_date+17,2);
	}
	*/
	return 0;
}
...
int HNDR_event_code_2_ks_event(const HNDR_Event_Code_Def_t *defs,int event_code)
{
	...
	return event_code + HNDR_EVENT_CODE_BASE;
}
...
int HNDR_BF430::make_handler(KSGDeviceNode* node,ACE_HANDLE* handler)
{
	...
	if(child)
	{
		...
		{
			ret = hacOpenChannel((char*)ip.c_str(),port,&hd);
			if(ret)
				hacCloseChannel(hd);
			goto L_FINISH;
		}
...
L_FINISH:
		...
		{
			ACE_DEBUG((LM_ERROR,"连接汉军设备失败,dev[%d],ret[%d][%s:%d]",
				node->GetDevId(),ret,ip.c_str(),port));
		...
		{
			ACE_DEBUG((LM_ERROR,"连接汉军设备失败,dev[%d],未知错误,ret[%d][%s:%d]",
				node->GetDevId(),ret,ip.c_str(),port));
			node->SetState(KSGDeviceNode::dsOffline);
			return -1;
		}
...
bool HNDR_Device_Base::Accept(BaseVisitor& guest,KSGDeviceNode* visitor)
{
	...
	{
		if(visitor->GetDeviceType() == HUNDURE_RAC2000P || visitor->GetDeviceType() == HUNDURE_RAC2000G
			||visitor->GetDeviceType() == HUNDURE_GCU3 || visitor->GetDeviceType() == HUNDURE_NCU3 )
			return true;
		else
			return false;
	}
...
int HNDR_DevInterfaceLoader::LoadInterface(KSGDeviceManager *manager)
{
	try
	{
		...
		KSG_ADD_DEVICE_INTERFACE(KSG_HUNDURE_DEV,HUNDURE_RAC2000G,ID_HNDR_RACP_Batch_DL_Card2);
		...
		KSG_ADD_DEVICE_INTERFACE(KSG_HUNDURE_DEV,HUNDURE_RAC2000P,ID_HNDR_RACP_Batch_DL_Card2);
		...
		KSG_ADD_DEVICE_INTERFACE(KSG_HUNDURE_DEV,HUNDURE_GCU3,ID_HNDR_GCU_Batch_DL_Card2);
#endif
	}
	catch (...)
	{
