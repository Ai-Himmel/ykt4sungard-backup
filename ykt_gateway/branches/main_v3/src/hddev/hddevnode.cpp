...
void HDDeviceLoader::SetupDeviceGroup(KSGDeviceNode* node)
{
	...
	node->set_param(DEV_BLKCOUNT,"50");
...
int HDDevInterfaceLoader::LoadInterface(KSGDeviceManager *manager)
{
	try
	{
		...
		KSG_ADD_DEVICE_INTERFACE(KSG_HD_DEV,KSG_JSB_DEV,IHDCollectKQSerial);
...
//////////////////////////////////////////////////////////////////////
//
int IHDCollectKQSerial::DeleteRecord(KSGDeviceNode* node,Task* task)
{
	...
	if( packlen <= 0)
	{
		ACE_DEBUG((LM_ERROR,"d PackRequestStruct err"));
		return TASK_ERR_COMMON;
	}
	ACE_Time_Value tv = KSGGetTaskTimeoutIntval();
	if(HDSendBuffer(handler,buf,packlen,&tv) != 0)
	{
		ACE_DEBUG((LM_ERROR,"d HDSendBuffer err"));
...
int IHDCollectKQSerial::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	ACE_DEBUG((LM_DEBUG,"开始采集流水"));
	...
	if( packlen <= 0)
	{
		ACE_DEBUG((LM_DEBUG,"PackRequestStruct err"));
		return TASK_ERR_COMMON;
	}
	ACE_Time_Value tv = KSGGetTaskTimeoutIntval();
	if(HDSendBuffer(handler,buf,packlen,&tv) != 0)
	{
		ACE_DEBUG((LM_DEBUG,"HDSendBuffer err"));
		return TASK_ERR_TIMEOUT;
	}
	packlen = HDRecvBuffer(handler,buf,sizeof buf,&tv);
	if( packlen <= 0)
	{
		ACE_DEBUG((LM_DEBUG,"HDRecvBuffer err"));
		return TASK_ERR_COMMON;
	}
	MESSAGETYPE msg_type;
	if(UnPackResponseStruct(req,&msg_type,buf,packlen)!=0)
	{
		ACE_DEBUG((LM_DEBUG,"UnPackResponseStruct err"));
		return TASK_ERR_COMMON;
	}
	if(req->ResponseCode != RC_SUCCESS)
	{
		ACE_DEBUG((LM_DEBUG,"ResponseCode err"));
		return TASK_ERR_COMMON;
	}
...
int IHDCollectKQSerial::CollectRecord(HD8583STRUCT& record)
{
	...
	ACE_DEBUG((LM_DEBUG,"CollectRecord"));
	...
	for(int i=0;i<10;i++)
	{
		...
		if(strlen((char*)tmp)==0)								// 流水采集完了
			break;
		sprintf(phyno,"%02X%02X%02X%02X",tmp[0],tmp[1],tmp[2],tmp[3]);
...
int IHDBatchDownloadBlkCard::dowload_blkcard(KSGDeviceNode* node,Task* task,std::string &ret_ver)
{
	...
	try
	{
		...
		try
		{
			for(i = 0;i < count; ++i)
			{
				if(xutil::StringUtil::Str2Hex(cardid + i*10,(unsigned char*)data+i*5,5)==-1)
					return TASK_ERR_PARAM;
...
int IHDCollPosHisSerial::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	for(index = 1,i = start_no;i <= end_no;i+=record_per_req,++index)
	{
		...
		ACE_DEBUG((LM_DEBUG,data,packlen));
...
int IHDBatchDownloadKQCard::GetBlkVer(KSGDeviceNode *task_node,Task* task,int &blkver)
{
	...
	req.SetFieldValue(FIELD_TERMINALSN,sn);
	req.SetFieldValue(FIELD_ADDRESS,addr);
	req.SetFieldValue(FIELD_DATEANDTIME,datetime);
...
int ID_GCU_DL_Timesect::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	req.SetFieldValue(FIELD_ADDRESS,addr);
	req.SetFieldValue(FIELD_TERMINALID,task_node->GetTermId());
	req.SetFieldValue(FIELD_VERSIONOFLIST,seqno);
	tmp[0]=1;
	req.SetFieldValue(FIELD_ADDITIONALDATA1,tmp,4);
...
int ID_GCU_DL_Week::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	try
	{
		seqno = task->GetParams().GetParamIntVal(XML_KEY_SEQNO);
		zoneinfo[0] = seqno;
		zoneinfo[1] =task->GetParams().GetParamIntVal(XML_KEY_WEEK_DAY7);
		zoneinfo[2] =task->GetParams().GetParamIntVal(XML_KEY_WEEK_DAY1);
		zoneinfo[3] =task->GetParams().GetParamIntVal(XML_KEY_WEEK_DAY2);
		zoneinfo[4] =task->GetParams().GetParamIntVal(XML_KEY_WEEK_DAY3);
		zoneinfo[5] =task->GetParams().GetParamIntVal(XML_KEY_WEEK_DAY4);
		zoneinfo[6] =task->GetParams().GetParamIntVal(XML_KEY_WEEK_DAY5);
		zoneinfo[7] =task->GetParams().GetParamIntVal(XML_KEY_WEEK_DAY6);
	}
	catch (...)
	{
		return TASK_ERR_PARAM;
