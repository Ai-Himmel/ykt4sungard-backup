...
class KSGTaskDoorBatchDlCard  : public KSGScheduler
{
	DECLARE_FACTORY_CREATOR(KSGTaskDoorBatchDlCard)
public:
	class BatchDownloadCardVisitor : public BaseVisitor,public DefaultDeviceVisitor
	{
	public:
		virtual void Visit(KSGDeviceNode& node);
	};
	KSGTaskDoorBatchDlCard():KSGScheduler(1007)
	{}
	virtual ~KSGTaskDoorBatchDlCard()
	{}
	virtual void Run();
private:
	BatchDownloadCardVisitor _visitor;
	inline void NewDLCardTask(KSGDeviceNode* node)
	{
		_visitor.Visit(*node);
	}
protected:
