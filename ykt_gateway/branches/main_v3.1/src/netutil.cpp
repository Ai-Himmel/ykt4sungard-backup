...
int KSGNetUtil::recv_buffer(ACE_HANDLE handle,unsigned char *buf,size_t &maxbuf,ACE_Time_Value *timeout)
{
	ssize_t ret;
	size_t m;
	ACE_SOCK_Stream stream(handle);
	...
	do 
	{
		ret = stream.recv_n(buf+l,m-l,&tv,&transfered);
		if(ret<0)
		{
...
int KSGNetUtil::recv_buffer_n(ACE_HANDLE handle,unsigned char *buf,size_t len_n)
{
	int ret = 0;
	size_t l = 0;
	size_t recv_len = len_n;
	ACE_Time_Value tv(0,500*1000);			// 500 毫秒
	while (l<len_n)
	{
		
		ret = ACE::handle_read_ready(handle,&tv);
		if(ret==0)
			return TASK_ERR_TIMEOUT;
		else if(ret == -1)
			return TASK_INVALID_CONN;
		
		ret = recv_buffer(handle,buf+l,recv_len,&tv);
		if(ret)
			return ret;
		l+=recv_len;
		recv_len = len_n-l;
