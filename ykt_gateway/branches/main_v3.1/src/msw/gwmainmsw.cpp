...
LONG WINAPI KSG_UnhandledExceptionFilter(PEXCEPTION_POINTERS pExceptionInfo)
{
	char expt_msg[512] = "";
	int i;
	// LIST : 2009-4-12 : 在 vc9 中是没有办法直接将 void * 用 %08X 打印出来， 先强转成 unsigned long !
	// 
	ACE_OS::sprintf(expt_msg,"exception cause at addr[%08X] code[%X]\n"
		,(unsigned long)pExceptionInfo->ExceptionRecord->ExceptionAddress
