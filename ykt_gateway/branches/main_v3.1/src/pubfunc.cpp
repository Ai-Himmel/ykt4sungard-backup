...
int KsgGetUserParam(const std::string &third,const std::string &param,std::string &value)
{
	...
	obj->AddField(F_SNAME,third.c_str());
	obj->AddField(F_SNAME2,param.c_str());
	if(obj->Connect())
		return -1;
	if(!obj->SendRequest(5000))
	{
		...
		{
			ACE_DEBUG((LM_ERROR,"获取参数错误，参数[%s][%s]ret[%d]",
				third.c_str(),param.c_str(),obj->GetReturnCode()));
			return -1;
		}
	}
	if(obj->HasMoreRecord())
