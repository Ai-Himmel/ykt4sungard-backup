...
mask_(0)
{
	...
	// 3 分钟超时
	max_client_timeout_ = ACE_Time_Value(KsgGetGateway()->GetConfig()->_reactor_timeout);
...
KSG_Reactor_Listener_Task::KSG_Reactor_Listener_Task():scheduler_(REACTOR_SCHEDULER::instance())
,work_thr_(REACTOR_SCHEDULER::instance())
{
	//
}
...
int KSG_Reactor_Scheduler::push_handle(KSG_Service_Handler *handle,ACE_Message_Block *mblk)
{
	...
	*/
	...
	ACE_DEBUG((LM_DEBUG,"请求队列任务[%d]",process_list_.size()));
	// 唤醒其它队列
	event_.signal();
	return 0;
...
int KSG_Reactor_Work_Thr::do_task2(KSG_Reactor_Scheduler::SERVER_HANDLER *handle)
{
	...
	{
		ACE_DEBUG((LM_DEBUG,"处理请求[%08X]",handle->handle_));
	...
	{
		// 请求有问题，需要关闭
		ACE_DEBUG((LM_DEBUG,"发送应答失败，关闭请求[%08X]",handle->handle_));
