...
int ID_SOYAL_DL_Card::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	while(drtp->HasMoreRecord() )
	{
		...
		list_id = data->lvol0;
		if(strlen(data->sdate2)>0)						// 物理卡号
			memcpy(pre_phyno,data->sdate2,8);
		else
...
int ID_SOYAL_DL_Week::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	try
	{
		...
		link = task->GetParams().GetParamIntVal("LINK");
	...
	ret = SetTimeZone(handler,nodeid,seqno,link,timegroup);
	if(ret)
	{
...
int ID_SOYAL_Ctrl_Door::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	else			// 时段开门设置
	{
		...
		cmdcode = 0x14;		// 开启
		addrL = 0x06;		// 地址
		ret = CtrlReaderAutoOpen(handler,nodeid,reader,cmdcode,addrL);		// 20*20
		if(ret)
		{
			ACE_DEBUG((LM_ERROR,"CtrlReaderAutoOpen errcode[%d],ctrldevid[%d],ctrlid[%d],reader[%d]",ret,devid,nodeid,reader));
			return ret;
		}
		...
		cmdcode = 0x01;		// 到时自动开门
		addrL = 0x16;		// 地址
		CtrlReaderAutoOpen(handler,nodeid,reader,cmdcode,addrL);		// 24*001
		if(ret)
		{
			ACE_DEBUG((LM_ERROR,"CtrlReaderAutoOpen errcode[%d],ctrldevid[%d],ctrlid[%d],reader[%d]",ret,devid,nodeid,reader));
			return ret;
		...
		ACE_DEBUG((LM_DEBUG,"时段开门成功,ctrldevid[%d],ctrlid[%d],reader[%d]",devid,nodeid,reader));
	}
