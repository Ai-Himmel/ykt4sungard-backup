...
namespace HDDEV
{
	...
	{
	public:
		enum { MAX_MESBUF_LEN = 4096 , MSG_BUF_LEN_HEADER = 2};
	...
	//!水控机申请消费(由后台判断该卡是否符合消费要求)
	class HDApplyConsumeHandler : public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDApplyConsumeHandler)
	public:
		HDApplyConsumeHandler():HDResponseHandler(MT_APPLYCONSUME)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};
	...
	private:
		int do_offline(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
		int do_online(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	public:
	...
	public:
		...
		{}
		int SaveRecord(char *buf,char *phyid,char *termno);
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};
	...
	public:
		...
		HDRecordFileReader(const std::string& filepath,int device_id,HD_RECORD_TYPE rtype = POS_RECORD)
			:_filepath(filepath),_file_handler(ACE_INVALID_HANDLE),_device_id(device_id),_record_type(rtype),bColHisSeri(false)
		{}
		~HDRecordFileReader();
		int ReadFile();
		int SavePosRecord(BYTE* buf);
		int SaveTradeRecord(TRADERECORDSTRUCT *trade,int posdatatype,int crc_status = 0);

		int SaveAttendRecord(ATTENDRECORDSTRUCT *record,int cardtype = 0);
		int SaveAttendBuffer(BYTE *buf,int cardtype = 0);
	};
	...
	//! pos机向后台签到
	class HDPosEnrollHandle : public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDPosEnrollHandle)
	public:
		HDPosEnrollHandle():HDResponseHandler(MT_POSENROLL)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};

	//! 联机消费/联机消费冲正
	class HDOnlineConsumeHandler : public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDOnlineConsumeHandler)
	public:
		HDOnlineConsumeHandler():HDResponseHandler(MT_ONLINE_CONSUME)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};
	...
	//! 上送pos流水/卡流水
	class HDPOS_SeriUpLoadHandler : public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDPOS_SeriUpLoadHandler)
	public:
		HDPOS_SeriUpLoadHandler():HDResponseHandler(MT_POSSERI_UPLOAD)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};

	//! 联机充值/联机充值冲正
	class HDOnlineDepositHandle : public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDOnlineDepositHandle)
	public:
		HDOnlineDepositHandle():HDResponseHandler(MT_ONLINE_DEPOSIT)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};

	//! 下载黑名单
	class HDDownloadBlkCardHandle : public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDDownloadBlkCardHandle)
	public:
		HDDownloadBlkCardHandle():HDResponseHandler(MT_DOWNLOAD_BLKCARD)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};

	class HDOnlineNotice1: public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDOnlineNotice1)
	public:
		HDOnlineNotice1():HDResponseHandler(MT_ONLINENOTICE3)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};

	//! GCU 在线通知
	class HDGCUOnlineNotice : public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDGCUOnlineNotice)
	public:
		HDGCUOnlineNotice():HDResponseHandler(MT_GCUONLINENOTICE1)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};

	//! GCU 上送门禁事件
	class HDGCURequestHandler : public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDGCURequestHandler)
	public:
		HDGCURequestHandler():HDResponseHandler(MT_SENDACCESSEVENT1)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};

	//! 电大多媒体取电
	class HDGetElecRequestHandler : public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDGetElecRequestHandler)
	public:
		HDGetElecRequestHandler():HDResponseHandler(MT_GETELECTRICAL)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};

	//! CPU卡充值机签到
	class HDAddPOSSignIn : public HDResponseHandler
	{
		DECLARE_FACTORY_CREATOR(HDAddPOSSignIn)
	public:
		HDAddPOSSignIn():HDResponseHandler(MT_GETELECTRICAL)
		{}
		int DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer);
	};
