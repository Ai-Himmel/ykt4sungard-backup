...
#define HDA_FILENAME_ARECORD1			"ARecord.dat"
#define HDA_FILENAME_ARECORD2			"ARecord2.dat"
...
enum FILEID		// CCU文件索引号
{
	...
	FILEID_GRECORD1					= 14,	// 门禁记录文件1
	FILEID_GRECORD2					= 15,	// 门禁记录文件2
	FILEID_ARECORD1					= 16,	// 考勤记录文件1
	FILEID_ARECORD2					= 17,	// 考勤记录文件2
};
...
enum MESSAGETYPE	// 信息码(为偶数的值表示报文发起方为终端一方，为奇数表示报文发起方为后台一方)
{
	...
	MT_COLLECT_SERIAL1				= 0x45, // 指定流水号补采记录
	...
	MT_GETELECTRICAL				= 0x5E,	// 电大多媒体取电
	MT_DLKQPARAM					= 0x5F,	// 电大考勤配置参数
	MT_TRADE1						= 0x60,	// 消费
	...
	MT_CHECK_AUTH_CARD				= 0x6d, 

	MT_ONLINE_DEPOSIT				= 0x70,	// 联机充值/联机充值冲正
	MT_ONLINE_CONSUME				= 0x74,	// 联机消费/联机消费冲正
	MT_DOWNLOAD_BLKCARD				= 0x76,	// 下载黑名单
	MT_POSSERI_UPLOAD				= 0x78,	// 上送pos流水
	MT_POSENROLL					= 0x7a,	// pos机向后台签到
	MT_ONLINENOTICE3				= 0x7c,	// 在线通知

	MT_SETWATERFEE					= 0x8D,	// 下载水控费率

	MT_SETJOWTIME2					= 0x91,	// 设置打铃时段
	MT_SETCARDTIME2					= 0x93,	// 设置刷卡时段
	MT_SENDATTENDEVENT1				= 0x94,	// 上送考勤事件

	MT_DLMANAGECARD					= 0xA9,	// 电大多媒体取电下载管理卡
	...
	MT_CTRLPENSTOCK					= 0xD1, // 远程水阀控制
	MT_APPLYCONSUME					= 0xD2, // 水控机申请消费
...
enum RESPONSECODE// 报文响应码
{
	...
	RC_CARDBALANCE_EXCEED			= 48,	// 卡余额超限
	...
	RC_CARD_CANCEL					= 104,	// 卡已注销
	RC_TRADE_FAILED					= 105,  // 后台交易失败
	RC_BALANCE_EXCEED				= 106,  // 充值金额超限
	RC_CARD_INVALID					= 107,  // 卡状态异常
	RC_OTHER_ERR					= 108,	// 其他错误
};
...
struct TRADERECORDSTRUCT// 交易记录数据结构
{
	...
	unsigned short SubsidyNo;		// 补助批次号（）
	unsigned long MealId;			// 餐次号（时段号）
	BYTE MAC[4]					;   // MAC
};
...
#define KSG_HD_GCU_DEV	KSG_GCU_DEV
#define KSG_HD_KQ_CCU	"5303"
#define KSG_HD_PENSTOCK "0244"
