...
namespace HDDEV
{
	...
	private:
		int DeleteRecord(KSGDeviceNode* node,Task* task);
		int CollectRecord(KSGDeviceNode* node,HD8583STRUCT& record);
	public:
	...
	// 下载时间段
	class ID_GCU_DL_Timesect : public KSGDeviceInterface
	{
		DECLARE_FACTORY_CREATOR(ID_GCU_DL_Timesect)
	private:
		int get_day_time(unsigned char *zoneinfo,Task* task,const char *param_name);
	public:
		ID_GCU_DL_Timesect():KSGDeviceInterface(TK_DOOR_DL_TIME_SECT)
		{}
		int ExecuteTask(KSGDeviceNode* node,Task* task);
	};

	// 下载时间周
	class ID_GCU_DL_Week : public KSGDeviceInterface
	{
		DECLARE_FACTORY_CREATOR(ID_GCU_DL_Week)
	public:
		ID_GCU_DL_Week():KSGDeviceInterface(TK_DOOR_DL_WEEK)
		{}
		int ExecuteTask(KSGDeviceNode* node,Task* task);
	};
	// 远程开门
	class ID_CTRL_DOOR : public KSGDeviceInterface
	{
		DECLARE_FACTORY_CREATOR(ID_CTRL_DOOR)
	public:
		ID_CTRL_DOOR():KSGDeviceInterface(TK_CTRL_DOOR_OPEN)
		{}
		int ExecuteTask(KSGDeviceNode* node,Task* task);
	};
	//!< 批量下载白名单
	...
	// 远程开门
	class ID_GCU_Ctrl_Door : public KSGDeviceInterface
	{
		DECLARE_FACTORY_CREATOR(ID_GCU_Ctrl_Door)		
	public:
		ID_GCU_Ctrl_Door():KSGDeviceInterface(TK_CTRL_DOOR_OPEN)
		{}
		int ExecuteTask(KSGDeviceNode* node,Task* task);
	};
	//!< 下载考勤名单
	class IHDBatchDownloadKQCard : public KSGDeviceInterface
	{
		DECLARE_FACTORY_CREATOR(IHDBatchDownloadKQCard)
	public:
		IHDBatchDownloadKQCard():KSGDeviceInterface(930118)
		{}
		int GetBlkVer(KSGDeviceNode *task_node,Task* task,int &blkver);
		int ExecuteTask(KSGDeviceNode* node,Task* task);
	};
	...
	public:
		...
		{}
		int ExecuteTask(KSGDeviceNode* node,Task* task);
		int CPUExecuteTask(KSGDeviceNode* node,Task* task);
	};
	//! 下传餐次
	...
	//! 电大下载管理卡(多媒体取电)
	class IHDDLManageCard : public KSGDeviceInterface
	{
		DECLARE_FACTORY_CREATOR(IHDDLManageCard)
	public:
		IHDDLManageCard():KSGDeviceInterface(TK_DL_MANAGE_CARD)
		{}
		//获取黑名单版本号
		int GetBlkVer(KSGDeviceNode *task_node,Task* task,int &blkver1,int &blkver2);
		int ExecuteTask(KSGDeviceNode* node,Task* task);
	};

	//! 电大下载考勤配置参数(多媒体取电)
	class IHDDLKQPARAM : public KSGDeviceInterface
	{
		DECLARE_FACTORY_CREATOR(IHDDLKQPARAM)
	public:
		IHDDLKQPARAM():KSGDeviceInterface(TK_DL_KQ_PARAM)
		{}
		int ExecuteTask(KSGDeviceNode* node,Task* task);
	};

	//! 后付费水控费率下载
	class IHDDLWaterFeeCfg : public KSGDeviceInterface
	{
		DECLARE_FACTORY_CREATOR(IHDDLWaterFeeCfg)
	private:
		int SendRequest(HD8583STRUCT &req,ACE_HANDLE handler);
	public:
		IHDDLWaterFeeCfg():KSGDeviceInterface(TK_DL_WATERFEE)
		{}
		int ExecuteTask(KSGDeviceNode* node,Task* task);
	};
