...
public:
	...
	/*!
	 \param handle - 
	 \param buf - 接收数据包的缓存
	 \param maxlen - [in/out] 输入参数为缓冲区的大小，输出表示接收到得数据的长度
	 \param timeout - NULL 表示一直等待
	 \return 0表示成功，否则返回错误码
	*/
	static int recv_buffer(ACE_HANDLE handle,unsigned char *buf,size_t &maxlen,ACE_Time_Value *timeout);

	// 接收len_n 定长的数据
	static int recv_buffer_n(ACE_HANDLE handle,unsigned char *buf,size_t len_n);
};
#endif // _NETUTIL_H_
