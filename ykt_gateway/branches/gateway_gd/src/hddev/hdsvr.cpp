...
int HDDEV::HDParsePosRecord(BYTE* buf,TRADERECORDSTRUCT* trade)
{
	...
	BUF_2_INT_LE(trade->TraceOfPOS,buf+20);	// 流水号
	BUF_2_INT_LE(trade->OrgTransTrace,buf+24); // 原交易流水号
	BUF_2_INT_LE(trade->OperatorID,buf+28);    // 操作员卡号
	...
	BUF_2_SHORT_LE(trade->shop_id,buf+43);
	trade->dinner_id = *(buf+45);
	memcpy(trade->MAC,buf+46,4);
...
#define CCU_MTU 1024
	...
	{
		ACE_DEBUG((LM_INFO,"handle[%d]发送数据,recv[%d],send[%d]"
			,get_handler_id(),_recv_count,_send_count));
		_send_count--;
		if(_request_handler)
		{
		...
		const ACE_Asynch_Read_Stream::Result &result)
	{
		...
		_recv_count--;
		ACE_DEBUG((LM_INFO,"handle[%d]接收数据,recv[%d],send[%d]"
			,get_handler_id(),_recv_count,_send_count));
		update_io_time();
...
{	
	...
	case 0x01:
		...
		break;
	case 0x02:
		ACE_DEBUG((LM_NOTICE,"冲正交易,金额[%d][%d]",trade->Balance,trade->Amount));
	...
	obj->AddField(F_SDATE1,sn);
	// 修改 2007-8-6 , 广州大学城的修改
	// 餐次
	ACE_OS::sprintf(temp,"%d",trade->dinner_id);
	obj->AddField(F_STX_PWD,temp);
	// 商户号
	obj->AddField(F_LBANK_ACC_TYPE,trade->shop_id); // 通讯版本与 39 兼容
	...
	*/
	...
	{
		obj->AddField(F_LVOL8,trade->Amount);
		obj->AddField(F_LVOL1,0);
	}
	else
	{
	...
	while(retries-->0)
	{
		...
		if(!obj->SendRequest(1000))
		{
			...
			{
				ACE_DEBUG((LM_DEBUG,"上传流水成功dev[%s][%d]",sn,trade->TraceOfPOS));
				return 0;
			}
		}
		// 1 秒后重试
	...
	*/
	...
	if(ret == 0)
	{
		...
		else
		{
			...
			{
				ACE_DEBUG((LM_NOTICE,"清除 CCU 流水文件成功!dev[%s]",devid));
			}
		}
		...
		// 如果无法删除流水文件，还是先导入流水先
		HDRecordFileReader reader(filepath,node->GetDevId());
		ret = reader.ReadFile();
		if(ret)
		{
			...
			else
			{
				...
				if(rec_file_len != HDRecordFileReader::RECORD_BUF_LEN * recordcnt)
				{
					ACE_DEBUG((LM_ERROR,"dev[%s]下载记录文件大小与请求包中文件大小不一致except[%d]received[%d]"
						,node->GetPhyId().c_str(),recordcnt * HDRecordFileReader::RECORD_BUF_LEN,rec_file_len));
		...
		if(!obj->SendRequest(1000))
		{
			...
			else
			{
				...
				if(obj->HasMoreRecord())
				{
					...
					if(seqno <= 0)
					{
						ACE_DEBUG((LM_ERROR,"查询补助返回批次号错误seqno[%d]",seqno));
						return 1;
					}
					// 补贴金额
...
int HDPOS_Check_Blackcard::DoResponse(HD8583STRUCT& req,HD8583STRUCT& resp,ACE_HANDLE peer)
{
	...
	resp.SetFieldValue(FIELD_RESPONSECODE,RC_SUCCESS);
	if(count >= blk_card_cnt)
	{
		// 表示结束
		resp.SetFieldValue(FIELD_ADDITIONALDATA3,blk_buf,1);
	}
	unsigned long encode_ver;
	if(count == 0)
	{
		ACE_DEBUG((LM_DEBUG,"设备[%s]请求更新黑名单,后台黑名单版本已经全部更新",devphyno));
		encode_ver = req->VerOfList;
	}
	else
	{
		ACE_DEBUG((LM_DEBUG,"设备[%s]请求更新黑名单,一共更新[%d][%s]条"
			,devphyno,count,max_vol));
		resp.SetFieldValue(FIELD_ADDITIONALDATA2,blk_buf,count*card_len);
		encode_ver = EncodeVersionNum(max_vol); // 压缩版本号
	}
