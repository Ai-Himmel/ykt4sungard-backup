...
class KSGTask_Pool_CheckPolicy
{
private:
	typedef KSGTask<KSGDeviceNode> TaskType;
public:
	void OnNewCheck(TaskType *p);
	void OnDeleteCheck(TaskType *p);
};
...
public:
	...
	//typedef SmartPtrPool<TaskType,boost::object_pool<TaskType>,KSGMultiThreadmModel<10000> > TaskObjPoolType;
	typedef KSG_SmartPool<TaskType,KSG_Memory_Pool_Allocator<TaskType,KSGMultiThreadmModel<10000>,KSGTask_Pool_CheckPolicy > > TaskObjPoolType;
	typedef TaskObjPoolType::SmartPtr PtrType;
	typedef TaskType* PointerType;
private:
...
public:
	...
	*/
	PtrType NewTask(int taskId,int code,const std::string& content
					,KSGDeviceNode* node,TaskPriority pri = ptNormal,void * arg = 0);
