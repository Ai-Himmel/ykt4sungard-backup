	...
	if(obj->HasMoreRecord())
	{
		while(obj->HasMoreRecord())
		{
			...
			try
			{
				...
				if(strcmp(data->scurrency_type,KSG_WIEGAND_CARD_10) == 0)
				{
					device->card_type(KSGDeviceNode::ct_wiegand_10);
				}
				else if(strcmp(data->scurrency_type,KSG_WIEGAND_CARD_8) == 0)
				{
					device->card_type(KSGDeviceNode::ct_wiegand_8);
				}
				else
...
int KSGateway::start_scheduler_algorithm()
{
	...
	ACE_DEBUG((LM_DEBUG,"启动[%d]个调度算法",count));
	return 0;
}
...
ACE_THR_FUNC_RETURN task_scheduler_thr_func(void * args)
{
	...
	if(schd)
	{
		...
		while(theGlobalGateWay->status() == KSGateway::stRunning)
		{
			...
			ACE_DEBUG((LM_DEBUG,"调度任务运行中..."));
		}
		ACE_DEBUG((LM_DEBUG,"结束调度任务流程！"));
...
int KSGateway::setup_all_proactor_acceptor()
{
	...
	if(idstr.size() <= 0)
	{
		ACE_DEBUG((LM_INFO,"未注册任何侦听服务 ..."));
		return 0;
	}
	ACE_DEBUG((LM_INFO,"ACE Proactor 启动 [%d] 个线程",num_of_threads));
...
static int do_process_bcc_task(KSG_BCC_Svr_Handler *req)
{
	...
	if(req->has_more_req_rows())
	{
		...
		else
		{
			...
			ACE_DEBUG((LM_DEBUG,"BCC 请求任务,request[%d]taskid[%d]",task_code,taskid));
		}
		return 0;
	}
	return -1;
