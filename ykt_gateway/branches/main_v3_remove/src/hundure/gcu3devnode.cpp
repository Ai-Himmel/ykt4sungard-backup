...
// modify by 汤成 , 经过测试在下名单时，至少需要 2000 
static const int g_gcu_default_timeout = 2500;
...
int ID_HNDR_GCU_Batch_DL_Card::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	
	{
		...
		while(drtp->HasMoreRecord() && door_idx < max_cardlist_count)
		{
			...
			phyno = data->sdate0;
			// 如果是 wiegand 读卡器
			if(task_node->card_type() == KSGDeviceNode::ct_wiegand_10)
			{
				char temp[15] = "";
				KsgCardID_M1_2_WG(phyno.c_str(),temp,10);
				phyno_dec = temp;
			}
			else if(task_node->card_type() == KSGDeviceNode::ct_wiegand_8)
			{
				char temp[15] = "";
	...
	for(i = 1;i <= door_idx; ++i)
	{
		...
		{		
			...
			if(this_card[i].iDoors > 0)
			{
				...
				//memset(this_card[i].cTimeZone,0xFF,sizeof(this_card[i].cTimeZone)-1);
...
int ID_HNDR_GCU_DL_Card::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	std::string phyno_dec;
	// 如果是 wiegand 读卡器
	if(task_parent->card_type() == KSGDeviceNode::ct_wiegand_10)
	{
		char temp[15] = "";
		KsgCardID_M1_2_WG(phyno.c_str(),temp,10);
		phyno_dec = temp;
	}
	else if(task_parent->card_type() == KSGDeviceNode::ct_wiegand_8)
	{
		char temp[15] = "";
		KsgCardID_M1_2_WG(phyno.c_str(),temp,8);
		phyno_dec = temp;
