...
static const int g_rac_default_timeout = 2500;
...
int ID_HNDR_RACP_Batch_DL_Card::ExecuteTask(KSGDeviceNode *node,Task *task)
{
	...
	ACE_ASSERT(task_node != NULL);
	{
		...
		while(drtp->HasMoreRecord())
		{
			...
			if(task_node->card_type() == KSGDeviceNode::ct_wiegand_8)
			{
				char temp[12] = "";
				KsgCardID_M1_2_WG(phyno.c_str(),temp,8);
				memcpy(pre_phyno,temp,strlen(temp));
			}
			else if(task_node->card_type() == KSGDeviceNode::ct_wiegand_10)
			{
				char temp[12] = "";
...
int ID_HNDR_RACP_DLUL_Card::ExecuteTask(KSGDeviceNode* node,Task* task)
{
	...
	phyno_dec = temp;
	*/
	if(parent_node->card_type() == KSGDeviceNode::ct_wiegand_8)
	{
		char temp[14] = "";
		KsgCardID_M1_2_WG(phyno.c_str(),temp,8);
		phyno_dec = temp;
	}
	else if(parent_node->card_type() == KSGDeviceNode::ct_wiegand_10)
	{
		char temp[14] = "";
		KsgCardID_M1_2_WG(phyno.c_str(),temp,10);
		phyno_dec = temp;
													 ...
													 stPollList *poll_list,int record,int zipped)
{
	...
	for(success = 0; success < record;++success)
	{
		...
		ACE_DEBUG((LM_DEBUG,"RAC2000 门禁事件[%s]",date_str));
