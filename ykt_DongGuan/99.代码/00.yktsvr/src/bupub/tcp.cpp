...
bool CTcpSocket::CreateNB(int addressfamily,int type)
{
	Close();
   //m_socket = socket(m_nAddressFamily,type,IPPROTO_TCP);
	m_socket = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
...
bool CTcpSocket::CheckConnectSuccess(int timeout)
{
	int result=0;
	if (m_socket==INVALID_SOCKET) 
		return(-1);
	...
	if (timeout>=0)
	{
		...
		result=(select(m_socket+1, &rset, &rset, NULL, &tv));
		if (result<0)
		{
		 	Close();
		 	return false;
...
bool CTcpSocket::ConnectTcpEx(const char *host,short port,int timeout)
{
	unsigned int inaddr;  
   	struct hostent *ht;
	...
	if(!CheckConnectSuccess(timeout))
	{
		return false;
	}
	else
	{
		/*
		char buffer[2];
		int result=recv(m_socket,buffer,0,0);
		if(result < 0)
			return false;
		*/
		int error;
