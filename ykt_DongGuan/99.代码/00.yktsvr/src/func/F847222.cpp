...
static int process(InAcc *IA,trade_param_t *p)
{
	...
	des2src(IA->sTxDate,p->transdate);			//交易日期
	des2src(IA->sTxTime,p->transtime);			//交易时间
	des2src(IA->sTxCollectDate,p->transdate);		//采集日期
	des2src(IA->sTxCollectTime,p->transtime);		//采集时间
	des2src(IA->sTxAccDate,p->transdate);		//记账日期
	des2src(IA->sTxAccTime,p->transtime);		//记账时间
...
static int do_process_trade(trade_param_t * param)
{
	...
	ret = DB_t_shop_read_by_shop_id(param->out_shop_id,&shop);
	...
	if(ret)
	{
		writelog(LOG_ERR,"shop id[%d] custid[%d]",shop.shopid,shop.custid);
		if(DB_NOTFOUND == ret)
	...
	writelog(LOG_DEBUG,"商户[%d]解款金额[%.2lf]余额[%.2lf]"
		,shop.shopid,param->trade_fee,account.last_bala);
	return ret;
}
...
static int do_parse_shop(trade_param_t * param)
{
	...
	//	,param->shopid,param->remain_balance);
	ret = DB_v_cif_shop_tree_open_select_by_c1_and_shop_id(param->shopid);
	if(ret)
	{
...
int F847222(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	shopid = atoi(rPack->sbank_acc);
	if(shopid <= 0)
	{
		*pRetCode = E_NOTEXIST_SHOPACCNO;
		goto L_RETU;
	}
	...
	ret = DB_t_shop_read_by_shop_id(shopid,&shop);
	if(ret)
	{
		if(DB_NOTFOUND == ret)
			*pRetCode  = E_NOTEXIST_SHOPACCNO;
	...
	// 借用显示卡号字段保存商户号
	sprintf(serial.showid,"%d",param.shopid);
	...
	serial.cardbefbal = param.remain_balance;
	serial.trade_fee = param.remain_balance;

	des2src(param.transdate,serial.operate_date);
	des2src(param.transtime,serial.operate_time);
	des2src(param.opercode,rPack->scard0);
	
	ret = do_parse_shop(&param);
	if(ret)
	{
		writelog(LOG_ERR,"商户解款失败shop[%d] ret [%d]",param.shopid,ret);
		*pRetCode = ret;
		goto L_RETU;
	}
