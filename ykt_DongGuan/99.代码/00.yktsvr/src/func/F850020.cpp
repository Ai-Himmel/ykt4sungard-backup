...
// 退水控钱包到主钱包
static int waterbalareturn(CAccTrans& ats,ST_PACK *rPack,ST_PACK *outPack)
{
	int ret=0;
	...
	// 脱机水控，无卡时不退款
	if((rPack->lvol2 ==0)  &&(pursetype.onlineflag==0))
	{
		writelog(LOG_INFO,"脱机水控无卡不退款");
		return 0;
	}
	...
	if(rPack->lvol2)		// 有卡
		...
		ret = checklosecardacc(trans.cardno,&tCard,&tAccount);
	if(ret)
	{
	...
	outPack->damt0 = D4U5(cardbalance.balance - trans.inputamt,2);
	cardbalance.balance = 0;
	cardbalance.cardbal=0;	
	writelog(LOG_INFO,"小钱包不平金额[%f]",outPack->damt0);
	...
	{
		//联机水控退款
		ats.trans.transtype=TRANSTYPE_WATERREFUND;
	...
	else
	{
		...
		ats.trans.transtype=TRANSTYPE_SHOPREFUND;
		ret=ats.DoTransByTransType();
		if(ret)
			return ret;	
		ret=ats.doTransEnd();
	...
	outPack->lvol9=D2I(ats.trans.cardaftbal*100);
	outPack->lvol8=D2I(ats.trans.totaltransamt*100);
...
static int waternobalareturn(CAccTrans& ats,ST_PACK *rPack,ST_PACK *outPack)
{
	int ret =0;
	T_t_card  tCard;
	T_t_pursetype  pursetype;
	...
	ats.trans.transcode=TC_WATERNOBALARET;
	...
	if(pursetype.onlineflag)
	{
		ats.trans.transtype=TRANSTYPE_WATERNOBALAREFUND;
		ret=ats.DoTransByTransType();
		if(ret)
			return ret;	
		ret=ats.doTransEnd();
...
int F850020(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	ats.trans.transcode=TC_WATERBALRETURN;
	des2src(ats.trans.opercode,rPack->semp);
	ats.trans.termid=rPack->lwithdraw_flag;
	ret=ats.InitTrans();
	if(ret)
	...
	{
		ats.Reset();
		ats.trans.inputamt= outPack->damt0;	
