...
static int CardCloseWithCard(ST_PACK *rPack)
{
	...
	if(tCardAccount.foregift>0)
	{
		...
		//退押金
		ats.trans.transtype=TRANSTYPE_RETURNFOREGIFT;
	...
	if(amtcmp(tCardAccount.balance,0)<0)//处理负账户
	{
		...
		if(amtcmp(trans.inputamt,ats.sysPara.dAutoRenewAccMaxAmt)>0)
		{
	...
	/*
		if(amtcmp(cardbal - availbal,ats.sysPara.dAutoRenewAccMaxAmt)>0)
		{
...
//		}
	...
	sprintf(trans.remark,"销户成功,%s,总共应退款%.2lf元",ats.remark.c_str(),returncashamt);
	return 0;
}
//无卡退卡
