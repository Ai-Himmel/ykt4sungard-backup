...
int F846305(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	ats.trans.transcode=TC_CARDRENEW;
	ret=ats.InitTrans();
	if(ret)
		return ret;			
...
//	ret=IsExistOtherCardByCustid(tCard.custid,tCard.cardno);
	...
	if('1'==tCard.badflag[0])
	{
		...
		if(strncmp(enddate,ats.trans.transdate,8)>0)
		{
			DB_t_card_free_lock_by_c0();	
			ERRTIP("请等到期后再补办");
	...
	else if('1'==tCard.lossflag[0])
	{
		...
		{
			ret=calcEndDate(tCard.lossdate,ats.sysPara.iRenewCardDayCnt,enddate);
			if(ret)
			{
				DB_t_card_free_lock_by_c0();		
				return ret;
	...
	des2src(tNewCard.opendate,ats.trans.transdate);
...
//	T_t_cardver cardver;
	...
	des2src(tCardnoChange.changedate,ats.trans.accdate);
...
//		}
    ...
    outPack->lvol8=D2I(ats.trans.cardaftbal*100);
