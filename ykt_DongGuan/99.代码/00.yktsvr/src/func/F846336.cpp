...
int F846336(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	if(transflag)
	{
		ats.trans.transcode=TC_EACC2CARD;
	}
	else
	{
		ats.trans.transcode=TC_CARD2EACC;
	}	
...
//	des2src(transdtl.remark,rPack->ssectypes);
	...
	ats.trans.feetype=tCard.feetype;
	ats.trans.custid=tCard.custid;
	strcpy(ats.trans.cardaccno,tCardAccount.accno);
	strcpy(ats.trans.eaccno,tNetAccount.accno); 	
	if(TC_CARD2EACC==trans.transcode)
	{
		...
		if(amtcmp(ats.trans.inputamt,trans.cardbefbal)>0)
			return ERRINFO(E_CARD_BALANCE_SHORTAGE,trans.cardbefbal);
		strcpy(ats.trans.draccno,tCardAccount.accno);
		strcpy(ats.trans.craccno,tNetAccount.accno);		
	}
	else
	{
		char inpwd_crypt[65]="";
		...
		if(amtcmp(ats.trans.inputamt,tNetAccount.balance)>0)
			return ERRINFO(E_EACC_BALANCE_SHORTAGE,ats.trans.inputamt-tNetAccount.balance);
	
		double CardMaxBal=0;
		ret=GetPurseMaxBal(PURSE_NO_ONE,CardMaxBal);
		if(ret)
			return ret;
		if(amtcmp(ats.trans.inputamt+trans.cardbefbal,CardMaxBal)>0)
			return ERRINFO(E_AMT_EXCEED_MAX,CardMaxBal);	
		strcpy(ats.trans.draccno,tNetAccount.accno);		
		strcpy(ats.trans.craccno,tCardAccount.accno);
	}
	ats.trans.feetype=0;
	ats.trans.usecardflag=1;	
	ats.trans.custid=tCard.custid;
	...
	transdtl.cardaftbal=ats.trans.cardaftbal;
	...
	char temp[128]="";
	sprintf(temp,"卡余额:%.2f",ats.trans.cardaftbal);
	ats.remark+=temp;
