...
int F847120(TRUSERID *handle,int iRequest,ST_PACK *rPack,int *pRetCode,char *szMsg)
{
	...
	ret=DB_t_card_read_lock_by_c0_and_cardno(cardno, &card);
	if(ret)
	{
	...
	case 847119:
		...
		ret = DB_t_tif_writefailed_read_by_cardno_and_purese_no_and_deal_serial_no(card.cardno, cnt_purse_no, rPack->lserial1, &tWriteFailed);
		if (ret)
		{
			if (ret != DB_NOTFOUND)
			{
		...
		ret = DB_t_tif_writefailed_update_by_cardno_and_purese_no_and_deal_serial_no(card.cardno, cnt_purse_no, rPack->lserial1, &tWriteFailed);
		if (ret)
		{
		...
		ret = DB_t_tif_writefailed_read_by_cardno_and_deal_flag(card.cardno, &deal_flag[0], &tWriteFailed);
		if (ret)
		{
			if (DB_NOTFOUND == ret)
			{
				...
				{
					card.cardattr[CARDSTAT_TYPE_WFAIL] = STATE_FALSE;
				}
			}
		}
		break;
	case 847123:
		if('3'!=card.cardattr[CARDSTAT_TYPE_REG])
		{
			DB_t_card_free_lock_by_c0();
			*pRetCode=E_CARD_STATE_NOT_CHANGE;
			goto L_RETU;
		}
		card.cardattr[CARDSTAT_TYPE_REG]=STATE_TRUE;		//修改状态
		break;
	default:
		DB_t_card_free_lock_by_c0();
