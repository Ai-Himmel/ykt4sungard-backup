...
int CPaymentBase::init(TRUSERID *handle,ST_CPACK *aPack,ST_PACK *in_pack)
{
	...
	des2src(ats.trans.shopaccno,shop_.accno);
	return 0;
}
...
//		return E_INPUTNULL_TERMID;
	...
	ret = ats.CheckAcc(&card_,&account_,&dtl_);
	if(ret)
		return ret;
	// 保持第三方账号
	...
	ret = ats.GenTransdtl(&dtl_,DTLSTATUS_INIT);
	if(ret)
	{
...
int CPaymentBase::acc(ST_PACK *rPack,ST_PACK *out_pack,T_t_transdtl **transdtl)
{
	...
	*/
	...
	trans.unusedamt = trans.inputamt;
	ats.SetSummary(payment_.summary);
	// 卡的检查
	memset(&card_,0,sizeof(card_));
	// 检查账户
	memset(&account_,0,sizeof(account_));
	memset(&tempdtl,0,sizeof tempdtl);
	ret = ats.CheckAcc(&card_,&account_,&tempdtl);
	if(ret)
		return ret;
...
int CNormalPayTrans::PackData(ST_PACK *rPack,ST_CPACK& sPack)
{
	...
	int ret=0;
	CAccTrans& ats=CAccTrans::GetInst();
	if(strlen(ats.trans.stuempno)<1)
	{
		ret=GetCustBaseInfoByCustID(ats.trans.custid,
		ats.trans.custname,ats.trans.stuempno,
		ats.trans.remark);
		if(ret)
			return ret;
	}
	...
	strcpy(sPack.pack.sdate0,ats.trans.accdate);
	strcpy(sPack.pack.stime0,ats.trans.acctime);
	strcpy(sPack.pack.sphone3,ats.trans.refno);
	sPack.pack.lwithdraw_flag=ats.trans.termid;
	...
	des2src(sPack.pack.spager,ats.trans.stuempno);		//学号
	des2src(sPack.pack.sall_name,ats.trans.custname);	//姓名
	sprintf(sPack.pack.sbank_acc2,"%d",ats.trans.cardno);
	sPack.pack.lvol0=ats.trans.cardno;					//卡号
	sPack.pack.lvol1=D4U5(ats.trans.inputamt*100,0);		//
	sPack.pack.lvol7=D4U5(ats.trans.cardbefbal*100,0);	//卡余额
	sPack.pack.lvol2=ats.trans.custid;					//客户号
	des2src(sPack.pack.sbranch_code0,ats.trans.remark);	//校区
	...
	switch(payment_.payeeacctype)
	{
		case PAT_INPUT:
			...
			break;
		case PAT_CARDNO:
			sprintf(sPack.pack.sbank_acc,"%d",ats.trans.cardno);
			break;
		case PAT_CUSTID:
			sprintf(sPack.pack.sbank_acc,"%d",ats.trans.custid);
			break;
		case PAT_STUEMPNO:
			des2src(sPack.pack.sbank_acc,ats.trans.stuempno);
			break;
		default:
...
int CGDMobileTrans::pre_transfer(TRUSERID* handle,ST_PACK *rPack,ST_PACK *out_pack)
{
	...
	int iAmt=D4U5(rPack->damt1*100,0);
...
//	out_pack->lwithdraw_flag=ats.trans.termid;
//	des2src(out_pack->sdate3,ats.trans.accdate);
//	out_pack->lserial1=ats.trans.termseqno;
//	des2src(out_pack->sname2,ats.trans.refno);	
	return 0;
}
...
int CGDCTPayment::PackData(ST_PACK *rPack,ST_CPACK& sPack)
{
	...
	sprintf(sPack.pack.sbank_acc2,"%d",ats.trans.cardno);
	sPack.pack.lvol0=ats.trans.cardno;					//卡号
	sPack.pack.lvol1=D4U5(ats.trans.inputamt*100,0);		//卡号
	sPack.pack.lvol7=D4U5(ats.trans.cardbefbal*100,0);	//卡余额
	sPack.pack.lvol2=ats.trans.custid;					//客户号
	des2src(sPack.pack.sbranch_code0,ats.trans.remark);	//校区
	sPack.pack.lvol5 = ats.trans.termseqno;              // 交易流水号
	if(rPack->lsafe_level)
	{
		sprintf(sPack.pack.vsvarstr0,"%d",rPack->lsafe_level);	//水控余额
	}
