...
NEXT:
		...
		db_rollback();
		BKConfiguration *cfg = BKConfiguration::instance();
		char funcErrMsg[256]={0};
		snprintf(szMsg,256,"%s%s\n%s",cfg->get_errmsg(*iRetCode,funcErrMsg),sRetMsg,g_sqlmsg);
...
int main(int argc,char *argv[])
{
   ...
   if(BKConfiguration::instance()->load_config()!=0)
   {
   		DEBUG_RUNTIME_MSGOUT("加载系统参数失败！\n");
		return(-200);
   }
   BKConfiguration::instance()->load_errmsg();
