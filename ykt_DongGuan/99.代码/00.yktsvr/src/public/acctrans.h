...
//	int  purseno;		//钱包号
//	int  cardcnt;		//卡交易次数
//	double  befcardbal;	//交易前卡余额
//	double  aftcardbal;	//交易后卡余额
...
typedef map<unsigned int,T_t_cfgsplit>  MAPCFGSPLIT;
...
//生成主机唯一流水号
int GetHostSno(int *hostseqno);

//生成新的交易参考号
int GetNewRefno(char *refno);
int GetOperSeqno(int operid,int* operseqno);
class CAccTrans
{
private:
...
public:
	...
	int statdate;    //统计日期
	int accdate;	//记账日期
	int settledate;	//结算日期,记账日期
	int hostdate;	//主机日期
	int hosttime;	//主机时间
    	char hostdatetime[15];//系统时间 yyyymmddhhmiss
   	SYSPARA   	   sysPara;
	...
	int GetNewRefno(char *refno);
    	int SaveOperdtl();
	int GetSettleDate(int *iSettleDate=NULL);
	...
	int UpdInnerAcc(char *accno,int dcflag);
    	int AddAccDtl(int dcflag,int acctype);
    	int DoSplitTrans();
