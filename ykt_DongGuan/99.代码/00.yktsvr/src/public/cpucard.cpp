...
int cpu_calc_mac(cpu_trade_t *trade)
{
	...
	{
		if(trade->trade_type == 0)
			i = sprintf(mac_data,"%08X%08X02%s80",trade->balance,trade->tradeamt,termno);
		else
			i = sprintf(mac_data,"%08X%08X06%s80",trade->balance,trade->tradeamt,termno);
		cpu_pboc_calc_mac(mac_data,trade->sk,mac1);
		if(strncasecmp(trade->mac1,mac1,8)!=0)
		{
			writelog(LOG_ERR,"trans mac1 error");
			return E_INPUT_ERROR;
		}
		// ¼ì²éMAC
	}		
