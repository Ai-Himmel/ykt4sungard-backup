...
type
  TfaqhMonthAccount = class(TfaReportMachineTemp)
    Panel1: TPanel;
    Label2: TLabel;
    Label3: TLabel;
    Label4: TLabel;
...
var
  faqhMonthAccount: TfaqhMonthAccount;
...
procedure TfaqhMonthAccount.Init;
begin
  inherited;
...
procedure TfaqhMonthAccount.btnQueryClick(Sender: TObject);
var
  tempData:TDataSet;
  qry:TZQuery;
...
procedure TfaqhMonthAccount.btnCalcClick(Sender: TObject);
begin
  if Application.MessageBox('你确定要期末结转吗？',PChar(Application.Title),MB_ICONQUESTION+mb_yesno)=idno then
    Exit;
  wvrVOper.SendCommand;
  if wvVOper.FieldByName('返回码').Data.AsString='0' then
    Context.GetDialogSystem.ShowMessage('期末结转成功！');
end;

initialization
  TUIStdClassFactory.Create('期末结转',TfaqhMonthAccount);
