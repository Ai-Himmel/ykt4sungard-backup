...
type
  TfaqhBusiStatAcc = class(TfaTreeRptTemp)
    Panel1: TPanel;
...
var
  faqhBusiStatAcc: TfaqhBusiStatAcc;
...
procedure TfaqhBusiStatAcc.Init;
begin
  inherited;
...
procedure TfaqhBusiStatAcc.btnQueryClick(Sender: TObject);
var
...
begin
  ...
  if chkall.Checked then
    WorkView.FieldByName('商户账号').Data.SetString('')
  else
  ...
  try
    ...
    begin
      qry.Close;
      qry.SQL.Clear;
      qry.SQL.Add(insertBusiStatRpt(tmpData));
      qry.ExecSQL;
      tmpData.Next;
    end;
  finally
  ...
  try
    rptReport.LoadFromFile(sPreAppPath+'report\Account\商户汇总表.rmf');
...
procedure TfaqhBusiStatAcc.btnBusiQClick(Sender: TObject);
var
  tmpItem: TListItem;
  tmpdata:TDataSet;
begin
...
procedure TfaqhBusiStatAcc.lvBusiClick(Sender: TObject);
var
  i:Integer;
begin
  try
...
procedure TfaqhBusiStatAcc.rgQTypeClick(Sender: TObject);
begin
...
initialization
  TUIStdClassFactory.Create('商户汇总表',TfaqhBusiStatAcc);
