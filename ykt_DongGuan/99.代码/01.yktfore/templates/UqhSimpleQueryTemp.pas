...
type
  TfaqhSimpleQueryTemp = class(TfaSimpleQueryTemp)
  private
...
procedure TfaqhSimpleQueryTemp.BindProperIdObjects(WorkComBox: TWVComboBox;
  SourceSL: TStrings; ChosenStr: string; IsQuery: Boolean);
var
...
procedure TfaqhSimpleQueryTemp.btnCustClassCancel(Sender: TObject);
var
...
procedure TfaqhSimpleQueryTemp.btnCustClassConfirm(Sender: TObject);
var
  ComboxName : string;
  I : Integer;
...
function TfaqhSimpleQueryTemp.CheckQuery(const SrcStr: string): Boolean;
begin
  Result := False;
  if Pos(S_ISQUERY,SrcStr)=1 then Result := True;
end;

function TfaqhSimpleQueryTemp.ContainName(Name, SubName: string): Boolean;
var
  tempStr : string;
begin
  Result := False;
...
procedure TfaqhSimpleQueryTemp.CreateContractListStr;
var
...
procedure TfaqhSimpleQueryTemp.FindCaseForContract(Sender: TObject);
begin
...
procedure TfaqhSimpleQueryTemp.GBExit(Sender: TObject);
begin
  if Sender is TGroupBox then
    (Sender as TGroupBox).Visible := False;
end;

function TfaqhSimpleQueryTemp.GetKindsFromContract(
...
function TfaqhSimpleQueryTemp.GetProPerContract(
...
function TfaqhSimpleQueryTemp.GetStr(const Str: string): string;
begin
  Result := StringReplace(Trim(Str),' ','',[rfReplaceAll]);
end;

procedure TfaqhSimpleQueryTemp.Init;
{
var
  I : Integer;
  tempName : String;
...
procedure TfaqhSimpleQueryTemp.qhCusClassNamePreQuery;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
procedure TfaqhSimpleQueryTemp.qhCusListPreQuery;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
procedure TfaqhSimpleQueryTemp.qhExchPreQuery(WorkField : TWVField);
var
  I : Integer;
...
procedure TfaqhSimpleQueryTemp.qhKindsAndSeatInit;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
function TfaqhSimpleQueryTemp.Related(ParentCtrl, ChildCtrl, ParentSub,
...
procedure TfaqhSimpleQueryTemp.SynCustClass( CustClassComBoBox : TWVComboBox;
  ParentGB: TGroupBox);
...
procedure TfaqhSimpleQueryTemp.VarGroupBoxCreate(Sender: TObject);
var
  GroupBox : TGroupBox;
  I,J : Integer;
