...
uses
  ...
  UReportTemp, DBActns, ActnList, WorkViews, WVDBBinders, WVCmdReq, Db,
  DBCtrls, Grids, DBGrids, StdCtrls, ImageCtrls, ExtCtrls, UICtrls,
  ComCtrls, KCDataAccess,WVCommands,SafeCode, WVCtrls, RPDesignInfo,
  RPDBDesignInfo, Spin;
...
type
  TfaqhReportTemp = class(TfaReportTemp)
  private
...
procedure TfaqhReportTemp.BindProperIdObjects(WorkComBox: TWVComboBox;
  SourceSL: TStrings; ChosenStr: string; IsQuery: Boolean);
var
...
procedure TfaqhReportTemp.btnCustClassCancel(Sender: TObject);
var
...
procedure TfaqhReportTemp.btnCustClassConfirm(Sender: TObject);
var
  ComboxName : string;
  I : Integer;
...
function TfaqhReportTemp.CheckQuery(const SrcStr: string): Boolean;
begin
  Result := False;
  if Pos(S_ISQUERY,SrcStr)=1 then Result := True;
end;

function TfaqhReportTemp.ContainName(Name, SubName: string): Boolean;
var
  tempStr : string;
begin
  Result := False;
...
procedure TfaqhReportTemp.GBExit(Sender: TObject);
begin
  if Sender is TGroupBox then
    (Sender as TGroupBox).Visible := False;
end;

function TfaqhReportTemp.GetStr(const Str: string): string;
begin
  Result := StringReplace(Trim(Str),' ','',[rfReplaceAll]);
end;

procedure TfaqhReportTemp.Init;
var
  I : Integer;
  tempName : String;
...
procedure TfaqhReportTemp.qhCusClassNamePreQuery;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
procedure TfaqhReportTemp.qhCusListPreQuery;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
procedure TfaqhReportTemp.qhExchPreQuery(WorkField : TWVField);
var
  I : Integer;
...
procedure TfaqhReportTemp.qhKindsAndSeatInit;
var
  I : Integer;
  TempWVComBox : TWVComboBox;
begin
...
function TfaqhReportTemp.Related(ParentCtrl, ChildCtrl, ParentSub,
...
procedure TfaqhReportTemp.SynCustClass( CustClassComBoBox : TWVComboBox;
  ParentGB: TGroupBox);
...
procedure TfaqhReportTemp.VarGroupBoxCreate(Sender: TObject);
var
  GroupBox : TGroupBox;
  I,J : Integer;
