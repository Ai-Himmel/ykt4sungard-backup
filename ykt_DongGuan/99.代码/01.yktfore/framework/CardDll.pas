...
uses
  ...
  WVCmdReq,KSClientConsts,WaterDll;
...
const
  ...
  // 东莞新莞人卡
  CF_CITYCODE = 14;          //城市代码
  CF_INDUSTRYCODE = 15;      //行业代码
  CF_INTERACTFLAG = 16;      //互联互通标识
  CF_SUBFEETYPE = 17;        //卡子类别
  CF_APPSERIALNO = 18;       //应用序列号
...
//设置钱包金额2
function ks_load4card(transpack:PTRANSPACK):Integer;
  stdcall;far; external CARD_DLL name 'ks_load4card';

//重新设置钱包金额2
function ks_load4card_retry(transpack:PTRANSPACK):Integer;
  stdcall;far; external CARD_DLL name 'ks_load4card_retry';
...
//读取PSAM终端号                    const data:PChar
function ks_readpsamtermno(termno:PChar;need_request:Integer):Integer;
  stdcall;far; external CARD_DLL name 'ks_readpsamtermno';
...
type
  ...
  public
    ...
    function getAppSerialNo: string;
    procedure setAppSerialNo(strData: string);
  end;
...
function readPsamTermNo(var termId:string): Integer;
...
function readPsamTermNo(var termId:string): Integer;
var
  reTermId : array[0..20] of Char;
  st:Integer;
begin
  FillChar(reTermId,21,0);
  st := ks_readpsamtermno(PChar(@reTermId),0);
  if st = 0 then
  begin
    termId := reTermId;
    Result := 0;
...
begin
  ...
  try
    ...
    if st<>0 then
    begin
      context.GetDialogSystem.ShowMessage('读卡失败，请放置好卡!');
      result := '';
      exit;
    end;
...
begin
  ...
  st := cm.setfieldreadmode(CF_APPSERIALNO);
  if st<>0 then
    retStr := retstr+','+IntToStr(CF_APPSERIALNO);
...
function TCM.getAppSerialNo: string;
var
  st:Integer;
  strData:string;
begin

  st := getCardData(CF_APPSERIALNO,strData);
  if st=0 then
  begin
    if strData='' then
      Result := '0'
...
procedure TCM.setAppSerialNo(strData: string);
begin
  setfieldwritemode(CF_APPSERIALNO);
  setCardData(CF_APPSERIALNO,strData);
end;
end.
