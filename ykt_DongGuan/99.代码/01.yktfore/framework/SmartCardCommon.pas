...
var
  ...
  procedure rePrintOrder(rpt:string);
...
var
  rptReport:TRMReport;
begin
  if FileExists(rpt)=False then
  begin
    ShowMessage('回执文件不存在！');
    Exit;
  end;
  if Application.MessageBox('你确定要重新打印回执吗？',PChar(Application.Title),MB_ICONQUESTION+mb_yesno)=idno then
    Exit;
  rptReport := nil;
  try
    try
      rptReport := TRMReport.Create(nil);
      rptReport.LoadFromFile(rpt);
      if showrpt=1 then
        rptReport.ShowReport
      else
        rptReport.PrintReport;
    except
      ShowMessage('你没有安装打印机，不能打印！');
    end;
  finally
    rptReport.Destroy;
  end;
