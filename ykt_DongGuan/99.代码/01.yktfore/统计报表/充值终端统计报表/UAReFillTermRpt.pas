...
type
  TfaqhTermRefillRpt = class(TfaReportMachineTemp)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
...
var
  faqhTermRefillRpt: TfaqhTermRefillRpt;
...
procedure TfaqhTermRefillRpt.Init;
begin
  inherited;
  endSign := True;
  {
...
procedure TfaqhTermRefillRpt.WorkViewFieldsMonitors0ValueChanged(
...
procedure TfaqhTermRefillRpt.btnQueryClick(Sender: TObject);
var
  tmpData:TDataSet;
  qry:TZQuery;
begin
  ...
  try
    ...
    begin
      qry.Close;
      qry.SQL.Clear;
      qry.SQL.Add(irptTermReFillRptSql(tmpData));
      qry.ExecSQL;
      tmpData.Next;
    end;
  ...
  try
    rptReport.LoadFromFile(sPreAppPath+'report\充值终端统计报表.rmf');
...
initialization
  TUIStdClassFactory.Create('充值终端统计报表',TfaqhTermRefillRpt);
