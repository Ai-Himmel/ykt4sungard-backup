...
type
  TfaqhPubCardStatRpt = class(TfaReportMachineTemp)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
...
var
  faqhPubCardStatRpt: TfaqhPubCardStatRpt;
...
procedure TfaqhPubCardStatRpt.Init;
begin
  inherited;
  endSign := True;
  {
...
procedure TfaqhPubCardStatRpt.WorkViewFieldsMonitors0ValueChanged(
...
procedure TfaqhPubCardStatRpt.btnQueryClick(Sender: TObject);
var
  tmpData:TDataSet;
  qry:TZQuery;
begin
  ...
  try
    ...
    begin
      qry.Close;
      qry.SQL.Clear;
      qry.SQL.Add(irptPubCardRptSql(tmpData));
      qry.ExecSQL;
      tmpData.Next;
    end;
    qry.Close;
  ...
  try
    rptReport.LoadFromFile(sPreAppPath+'report\发卡统计报表.rmf');
...
initialization
  TUIStdClassFactory.Create('发卡统计报表',TfaqhPubCardStatRpt);
