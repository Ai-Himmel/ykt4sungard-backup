...
uses
  ...
  RM_e_bmp, RM_e_htm, RzRadGrp;

type
  TfaqhExpendTermRpt = class(TfaReportMachineTemp)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
...
var
  faqhExpendTermRpt: TfaqhExpendTermRpt;
...
procedure TfaqhExpendTermRpt.Init;
begin
  inherited;
  endSign := True;
  {
...
procedure TfaqhExpendTermRpt.WorkViewFieldsMonitors0ValueChanged(
...
procedure TfaqhExpendTermRpt.btnQueryClick(Sender: TObject);
var
  tmpData:TDataSet;
  qry:TZQuery;
begin
  ...
  WorkView.FieldByName('日期类型').Data.SetInteger(rgDateType.ItemIndex+1);
  inherited;
  ...
  try
    ...
    begin
      qry.Close;
      qry.SQL.Clear;
      qry.SQL.Add(irptExpendRptSql(tmpData));
      qry.ExecSQL;
      tmpData.Next;
    end;
    qry.Close;
  ...
  try
    rptReport.LoadFromFile(sPreAppPath+'report\消费终端统计报表.rmf');
...
initialization
  TUIStdClassFactory.Create('消费终端统计报表',TfaqhExpendTermRpt);
