...
type
  TfaqhOperRpt = class(TfaReportMachineTemp)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
...
procedure TfaqhOperRpt.WorkViewFieldsMonitors0ValueChanged(
...
procedure TfaqhOperRpt.btnQueryClick(Sender: TObject);
var
  tmpData:TDataSet;
  qry:TZQuery;
begin
  ...
  try
    ...
    begin
      qry.Close;
      qry.SQL.Clear;
      qry.SQL.Add(irptOperRptSql(tmpData));
      qry.ExecSQL;
      tmpData.Next;
    end;
    qry.Close;
  ...
  try
    rptReport.LoadFromFile(sPreAppPath+'report\操作员业务报表.rmf');
...
initialization
  TUIStdClassFactory.Create('操作员业务报表',TfaqhOperRpt);
