...
type
  TfaqhPkgFileRpt = class(TfaReportMachineTemp)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
...
var
  faqhPkgFileRpt: TfaqhPkgFileRpt;
...
procedure TfaqhPkgFileRpt.Init;
begin
  inherited;
  endSign := True;
  {
...
procedure TfaqhPkgFileRpt.WorkViewFieldsMonitors0ValueChanged(
...
procedure TfaqhPkgFileRpt.btnQueryClick(Sender: TObject);
var
  tmpData:TDataSet;
  qry:TZQuery;
begin
  ...
  try
    ...
    begin
      qry.Close;
      qry.SQL.Clear;
      qry.SQL.Add(irptPkgFileRptSql(tmpData));
      qry.ExecSQL;
      tmpData.Next;
    end;
    qry.Close;
  ...
  try
    rptReport.LoadFromFile(sPreAppPath+'report\包裹文件统计报表.rmf');
...
initialization
  TUIStdClassFactory.Create('包裹文件统计报表',TfaqhPkgFileRpt);
