...
type
  TfaqhNonCardChange = class(TfaqhSimpleQueryTemp)
    Panel1: TPanel;
    Label2: TLabel;
    Label3: TLabel;
    Label4: TLabel;
...
var
  faqhNonCardChange: TfaqhNonCardChange;
...
procedure TfaqhNonCardChange.Init;
begin
  inherited;
...
procedure TfaqhNonCardChange.btnQueryClick(Sender: TObject);
begin
...
procedure TfaqhNonCardChange.btnReadQClick(Sender: TObject);
var
  cardNo:string;
begin
  edtAppSerialNo.Text:='';
...
procedure TfaqhNonCardChange.btnPrintClick(Sender: TObject);
var
  sDate,sTime:string;
  appSerialNo:string;
  phyId:string;
...
begin
  ...
  try
    ...
    WVChangeCardOper.FieldByName('申请单号').Data.SetInteger(Grid.DataSource.DataSet.fieldbyname('lvol1').AsInteger);
    WVChangeCardOper.FieldByName('原卡号').Data.SetString(Grid.DataSource.DataSet.fieldbyname('lvol0').AsString);
    //WVChangeCardOper.FieldByName('应用序列号').Data.SetString(Grid.DataSource.DataSet.fieldbyname('sphone3').AsString);
    ...
    rOperSerialNo := WVChangeCardOper.FieldByName('流水号').Data.AsString;
    //写卡信息，更新有效期和状态
    ...
    //终端交易序列号
    tp.TermSeqno := 1;//StrToInt64(rOperSerialNo);
    //终端编号
    ...
    begin
      try
        ...
        rptReport.Dictionary.Variables.AsString['titleType'] := '不记名卡换卡';
        rptReport.Dictionary.Variables.AsString['serialNo'] := rOperSerialNo;
        ...
        rptReport.SaveToFile(sPreAppPath+'report\RtnTicket\nonchangeoper.rmp');
        if showrpt=1 then
...
procedure TfaqhNonCardChange.btnRePrintClick(Sender: TObject);
begin
  rePrintOrder(sPreAppPath+'report\RtnTicket\nonchangeoper.rmp');
end;

initialization
  TUIStdClassFactory.Create('非记名卡换卡',TfaqhNonCardChange);
