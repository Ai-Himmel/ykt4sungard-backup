...
type
  TfaqhReNewApp = class(TfaqhSimpleQueryTemp)
    Panel1: TPanel;
    Label2: TLabel;
    Label3: TLabel;
    Label4: TLabel;
...
procedure TfaqhReNewApp.Init;
begin
  inherited;
end;

procedure TfaqhReNewApp.btnQueryClick(Sender: TObject);
begin
  inherited;
  btnLoss.Enabled := True;
end;

procedure TfaqhReNewApp.btnLossClick(Sender: TObject);
var
  cardNo:Integer;
  appCardNo:string;
  sDate,sTime:string;
...
begin
  ...
  if Application.MessageBox('你确定要申请补办吗？',PChar(Application.Title),MB_ICONQUESTION+mb_yesno)=idno then
    Exit;
  try
  ...
  begin
    Context.GetDialogSystem.ShowMessage('补办申请成功！');
    rptReport := nil;
    ...
    begin
      try
        try
          ...
          rptReport.SaveToFile(sPreAppPath+'report\RtnTicket\renewapp.rmp');
          if showrpt=1 then
...
procedure TfaqhReNewApp.btnReadCardClick(Sender: TObject);
var
  cardNo:string;
begin
  edtAppSerialNo.Text:='';
...
procedure TfaqhReNewApp.btnPrintClick(Sender: TObject);
begin
  rePrintOrder(sPreAppPath+'report\RtnTicket\renewapp.rmp');
end;

initialization
  TUIStdClassFactory.Create('补办申请',TfaqhReNewApp);
